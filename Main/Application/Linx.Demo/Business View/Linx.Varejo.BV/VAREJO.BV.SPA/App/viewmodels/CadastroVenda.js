...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroVenda' ]; settings.parentVM.CadastroVenda = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('CadastroVenda', parentVM.internalUIs) === -1) parentVM.internalUIs.push('CadastroVenda');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('varejo-bv-spa-CadastroVenda', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroVenda', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#CadastroVenda_gbVendaWizard').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    complement.renderscyCadastroVenda_dGrid(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'VendaWizard';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'VendaWizard', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'VendaWizard', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('VendaWizard');
        ...
        return dataContext.getVendaWizardByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'VendaWizard'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getVendaWizardByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearVendaWizard(getBandeiraRede(), clearComplete);
    ...
    var createVendaWizard = function() {
        dataBind('dataView', true);
        var entity = dataContext.createVendaWizard();
    ...
    var createAndNotifyVendaWizard = function() {
        var entity = createVendaWizard();
        notifyPresentation('');
        return entity;
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroVenda_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroVenda_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroVenda_formViewer')[0];
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            VendaWizardDatetimeVenda_typeRange: ko.observable('R'), VendaWizardDatetimeVenda_begin: ko.observable(null), VendaWizardDatetimeVenda_end: ko.observable(null), VendaWizardDatetimeVenda_predefFilter: ko.observableArray([]), VendaWizardDatetimeVenda_predefValue: ko.observable(null),
        VendaWizardBigIntVenda_begin: ko.observable(null), VendaWizardBigIntVenda_end: ko.observable(null),
        VendaWizardDecimalVenda_begin: ko.observable(null), VendaWizardDecimalVenda_end: ko.observable(null),
        VendaWizardIdVenda_begin: ko.observable(null), VendaWizardIdVenda_end: ko.observable(null),
        VendaWizardIntVenda_begin: ko.observable(null), VendaWizardIntVenda_end: ko.observable(null),
        VendaWizardSmallIntVenda_begin: ko.observable(null), VendaWizardSmallIntVenda_end: ko.observable(null),
        VendaWizardIdCliente: ko.observable(null),
        VendaWizardIdLoja: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.VendaWizardDatetimeVenda_typeRange('R'); entitySearchRange.VendaWizardDatetimeVenda_begin(null); entitySearchRange.VendaWizardDatetimeVenda_end(null); entitySearchRange.VendaWizardDatetimeVenda_predefFilter([]); entitySearchRange.VendaWizardDatetimeVenda_predefValue(null);
        entitySearchRange.VendaWizardBigIntVenda_begin(null); entitySearchRange.VendaWizardBigIntVenda_end(null);
        entitySearchRange.VendaWizardDecimalVenda_begin(null); entitySearchRange.VendaWizardDecimalVenda_end(null);
        entitySearchRange.VendaWizardIdVenda_begin(null); entitySearchRange.VendaWizardIdVenda_end(null);
        entitySearchRange.VendaWizardIntVenda_begin(null); entitySearchRange.VendaWizardIntVenda_end(null);
        entitySearchRange.VendaWizardSmallIntVenda_begin(null); entitySearchRange.VendaWizardSmallIntVenda_end(null);
        entitySearchRange.VendaWizardIdCliente(null);
        entitySearchRange.VendaWizardIdLoja(null);
    };
    entitySearchRange.has_VendaWizardBigIntVenda = ko.computed(function(){ return (entitySearchRange.VendaWizardBigIntVenda_begin() != null || entitySearchRange.VendaWizardBigIntVenda_end() != null); });
    entitySearchRange.has_VendaWizardDecimalVenda = ko.computed(function(){ return (entitySearchRange.VendaWizardDecimalVenda_begin() != null || entitySearchRange.VendaWizardDecimalVenda_end() != null); });
    entitySearchRange.has_VendaWizardIdVenda = ko.computed(function(){ return (entitySearchRange.VendaWizardIdVenda_begin() != null || entitySearchRange.VendaWizardIdVenda_end() != null); });
    entitySearchRange.has_VendaWizardIntVenda = ko.computed(function(){ return (entitySearchRange.VendaWizardIntVenda_begin() != null || entitySearchRange.VendaWizardIntVenda_end() != null); });
    entitySearchRange.has_VendaWizardSmallIntVenda = ko.computed(function(){ return (entitySearchRange.VendaWizardSmallIntVenda_begin() != null || entitySearchRange.VendaWizardSmallIntVenda_end() != null); });
    entitySearchRange.has_VendaWizardDatetimeVenda = ko.computed(function(){ return (entitySearchRange.VendaWizardDatetimeVenda_typeRange() == 'R' && (entitySearchRange.VendaWizardDatetimeVenda_begin() != null || entitySearchRange.VendaWizardDatetimeVenda_end() != null) || (entitySearchRange.VendaWizardDatetimeVenda_typeRange() == 'P' && entitySearchRange.VendaWizardDatetimeVenda_predefFilter().length > 0)); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroVenda_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            createAndNotifyVendaWizard: createAndNotifyVendaWizard,
            ...
            __moduleId__: 'pkg_varejo-bv-spa/viewmodels/CadastroVenda'
        };
