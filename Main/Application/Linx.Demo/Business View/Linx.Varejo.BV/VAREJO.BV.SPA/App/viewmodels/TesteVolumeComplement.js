...
var complementCtor = function() {
    var complement = {
    isAutomatic: true
    , renderscyTesteVolume_dGrid: function(vm) {
    ...
    $('#scyTesteVolume_dGrid_headers').live('focus  keydown', function (evt) {
        var keyCode = window.event ? evt.which : evt.keyCode;
        if (keyCode === 9) {
            var cols = $('#scyTesteVolume_dGrid').igGrid('option', 'columns');
            var dataView = $('#scyTesteVolume_dGrid').data('igGrid').dataSource._dataView
            if (dataView.length === 0) return;
            var firstRow = dataView[0].RowDataId;
            clear = vm.status() === 'C';
            if (vm.status() === 'C')
                $('#scyTesteVolume_dGrid').igGridUpdating('startEdit', firstRow, 0);
            else {
                ...
                cols.some(function (entry) {
                    if (entry.key !== 'RowDataId' && !entry.hidden) {
                        if (verifyCanEditCol(entry.key, clear, entity)) {
                            $('#scyTesteVolume_dGrid').igGridUpdating('startEdit', firstRow, indexColumn);
                            return true;
                        }
                        indexColumn++;
                    }
    ...
    var getVisibleColumns = function(metaDataControl) {
       ...
       var visibleColumns = '';
       if($('#scyTesteVolume_dGrid').data('igGrid') === undefined) return '';
       var cols = $('#scyTesteVolume_dGrid').igGrid('option', 'columns');
       if (cols) {
    ...
    var isElementHided = function (grid, forceCreating) {
      if (!grid) grid = $('#scyTesteVolume_dGrid');
      return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialog').is(':visible'));
    }
    var refreshData = true;
    var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'scyTesteVolume_dGrid_container', dataBind: function (commitData, forceCreating) {
       var grid = $('#scyTesteVolume_dGrid');
       ...
       if (!started) {
           ...
           started = true;
           commitData = false;
           $('#scyTesteVolume_dGrid_groupbyarea').addClass('hide');
       }
       if (grid.igGridUpdating('isEditing')) {
    ...
    function createDataGrid(grid) {
        ...
        vm.gridSaveStates[gridId] = {
            ...
            closePopover: function () {
                $('#scyTesteVolume_dGrid_LayoutBtn').igPopover('hide');
            },
        ...
        grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
            ...
            cellClick: function(evt, ui) {
                 if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                     ...
                     if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                     {
                          ...
                          showMultimidia(entity, e, table, key, vm.TesteVolume());
                     }
                 }
                 if (typeof vm.OnGridClientClick === 'function') {
                     entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                     vm.OnGridClientClick('scyTesteVolume_dGrid', ui.colKey, entity);
                 }
                 var grid = $('#scyTesteVolume_dGrid');
            ...
            features: [
                          ...
                          columnSorting: function (evt, ui) { 
                              $.grep(ui.owner.grid._visibleColumnsArray, function (e) { 
                                  if (e.key === ui.columnKey && e.dataType === 'string') 
                                      return $('#scyTesteVolume_dGrid').igGridSorting('option', 'caseSensitive', false); 
                                  else if (e.key === ui.columnKey) 
                                      return $('#scyTesteVolume_dGrid').igGridSorting('option', 'caseSensitive', true); 
                              }); 
                          } 
                        ...
                        { name: 'Hiding', 
                            columnHidden: function (evt, ui) {
                               showMultimidiaLazy('#scyTesteVolume_dGrid');
                            },
                       ...
                       ,{ name: 'Updating', horizontalMoveOnEnter: true,
                          ...
                          columnSettings: [{ columnKey: 'DatetimeVendaItem', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid === 'function') {vm.OnPropertyChangeDataGrid('scyTesteVolume_dGrid', 'DatetimeVendaItem', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: 'ComboboxVendaItem', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('ComboboxVendaItem', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('LX_VENDA_ITEM', ''),  textKey: 'name', valueKey: 'id', enableClearButton: false }}, { columnKey: "BigIntVendaItem" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyTesteVolume_dGrid', 'BigIntVendaItem', ui.oldValue, ui.value);}},  maxLength: 24, maxValue: null, minValue: 0, dataMode: 'long' } }, { columnKey: "DecimalVendaItem" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyTesteVolume_dGrid', 'DecimalVendaItem', ui.oldValue, ui.value);}},  maxLength: 14, maxValue: 99999999999.99, minValue: 0, dataMode: 'decimal', minDecimals: 2, maxDecimals: 2 } }, { columnKey: "IdVenda" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyTesteVolume_dGrid', 'IdVenda', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "IdVendaItem" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyTesteVolume_dGrid', 'IdVendaItem', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, dataMode: 'int' } }, { columnKey: "IntVendaItem" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyTesteVolume_dGrid', 'IntVendaItem', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "SmallIntVendaItem" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyTesteVolume_dGrid', 'SmallIntVendaItem', ui.oldValue, ui.value);}},  maxLength: 6, maxValue: null, minValue: 0, dataMode: 'short' } }, { columnKey: "StringVendaItem" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyTesteVolume_dGrid', 'StringVendaItem', ui.oldValue, ui.value);}}, maxLength: 50 } }],
                          ...
                          editCellStarted: function(evt, ui){
                              ...
                              var columns = $('#scyTesteVolume_dGrid').igGridUpdating('option', 'columnSettings');
                              var currentCol = null;
                              currentRow = ui.rowID;
            ...
            if ((typeof vm.OnDataGridCreated === 'function')){
                vm.OnDataGridCreated('scyTesteVolume_dGrid');
            }
            ...
            selectionrowselectionchanged = function (evt, ui) {
                if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                    if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                        $(document).undelegate('#scyTesteVolume_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        ui.owner.clearSelection();
                        ui.owner.selectRow(ui.row.index);
                        selectedRowId = ui.row.id;
                        $(document).delegate ('#scyTesteVolume_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                    }
                    selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                 } 
            };
            $(document).delegate('#scyTesteVolume_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
            $('#scyTesteVolume_dGrid > tbody tr').live('focus', function(evt) {
                var grid = $('#scyTesteVolume_dGrid'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
            ...
            grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                if (vm.TesteVolume().status() === 'Q') vm.TesteVolume().dataToolbar.viewInfo();
            });
        }
        vm.addDataSource({ key: 'scyTesteVolume_dGrid', name: 'dataView', itemsSource: itemsSource });
    }
