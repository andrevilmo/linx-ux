...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'PivotGrid' ]; settings.parentVM.PivotGrid = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('PivotGrid', parentVM.internalUIs) === -1) parentVM.internalUIs.push('PivotGrid');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('varejo-bv-spa-PivotGrid', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: PivotGrid', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#PivotGrid_gbExpander_57bdf120db9744b1b24833ece4d4c6ee').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    complement.renderpivot13e2caab32de42d09b5bded3f72a9cbaPivotGrid_pivotVendaItem(vm);
    ...
    var visibleColumns = 'BigIntVendaItem,BitVendaItem,ComboboxVendaItem,DatetimeVendaItem,DecimalVendaItem,IdVenda,IdVendaItem,IntVendaItem,SmallIntVendaItem,StringVendaItem,BigIntVendaItemKpiInfo';
    ...
    var removeFormViewControl = function () {
        var front = $('#PivotGrid_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#PivotGrid_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#PivotGrid_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#PivotGrid_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            __moduleId__: 'pkg_varejo-bv-spa/viewmodels/PivotGrid'
        };
