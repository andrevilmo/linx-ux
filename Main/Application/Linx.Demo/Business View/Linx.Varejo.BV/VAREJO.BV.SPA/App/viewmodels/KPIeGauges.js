...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'KPIeGauges' ]; settings.parentVM.KPIeGauges = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('KPIeGauges', parentVM.internalUIs) === -1) parentVM.internalUIs.push('KPIeGauges');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('varejo-bv-spa-KPIeGauges', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: KPIeGauges', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#KPIeGauges_gbVenda').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    $('#KPIeGauges_tcVendaTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#KPIeGauges_tcVendaTabControl');

    complement.renderKPIeGauges_dGridVendaItem(vm);

    complement.renderscyKPIeGauges_dGrid(vm);
    ...
    var visibleColumns = 'IdVenda,BigIntVenda,BigIntVendaKpiInfo';
    ...
    var removeFormViewControl = function () {
        var front = $('#KPIeGauges_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#KPIeGauges_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#KPIeGauges_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#KPIeGauges_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            __moduleId__: 'pkg_varejo-bv-spa/viewmodels/KPIeGauges'
        };
