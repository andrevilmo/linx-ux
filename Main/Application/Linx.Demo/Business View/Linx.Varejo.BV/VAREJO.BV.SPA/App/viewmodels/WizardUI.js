define(['durandal/app', 'pkg_varejo-bv-spa/services/WizardExampleContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_varejo-bv-spa/viewmodels/WizardUIComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'WizardUI' ]; settings.parentVM.WizardUI = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('WizardUI', parentVM.internalUIs) === -1) parentVM.internalUIs.push('WizardUI');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('varejo-bv-spa-WizardUI', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: WizardUI', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#WizardUI_wizWizard_3ca1e4bbaa6c4d9dac76c927e968e514').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });

    complement.renderWizardUI_wizWizard_3ca1e4bbaa6c4d9dac76c927e968e514(vm);

    $('#WizardUI_tcClienteTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#WizardUI_tcClienteTabControl');

    complement.renderWizardUI_dGridVenda(vm);

    complement.renderWizardUI_dGridVendaAtacado(vm);
    ...
    var visibleColumns = 'IdCliente,StringCliente,BitCliente,BigIntCliente,ComboboxCliente,DatetimeCliente,IdEstado,DecimalCliente,SmallIntCliente,IntCliente';
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#WizardUI_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootNamespace: 'VAREJO.BV.WizardExample',
            ...
            __moduleId__: 'pkg_varejo-bv-spa/viewmodels/WizardUI'
        };
