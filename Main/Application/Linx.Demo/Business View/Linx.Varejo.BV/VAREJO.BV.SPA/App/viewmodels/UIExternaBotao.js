...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'UIExternaBotao' ]; settings.parentVM.UIExternaBotao = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('UIExternaBotao', parentVM.internalUIs) === -1) parentVM.internalUIs.push('UIExternaBotao');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('varejo-bv-spa-UIExternaBotao', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: UIExternaBotao', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        complement.renderUIExternaBotao_dGridLoja(vm);
            ...
            for (var idxElem = toList.length - 1; idxElem >= 0; idxElem--) {
                    if (toList[idxElem].ChangeState !== 'N') {
                       ...
                       if (toList[idxElem].ChangeState == 'I') {
                           fromObj = _.where(fromList, { TemporaryIdLoja: toList[idxElem]['IdLoja'] });
                       } else {
                           fromObj = _.where(fromList, { IdLoja: toList[idxElem]['IdLoja'] });
                       }
    ...
    var removeFormViewControl = function () {
        var front = $('#UIExternaBotao_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#UIExternaBotao_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#UIExternaBotao_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#UIExternaBotao_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            __moduleId__: 'pkg_varejo-bv-spa/viewmodels/UIExternaBotao'
        };
