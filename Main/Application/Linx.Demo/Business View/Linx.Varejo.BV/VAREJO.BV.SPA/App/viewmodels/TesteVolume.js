define(['durandal/app', 'pkg_varejo-bv-spa/services/DiversosContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_varejo-bv-spa/viewmodels/TesteVolumeComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'TesteVolume' ]; settings.parentVM.TesteVolume = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('TesteVolume', parentVM.internalUIs) === -1) parentVM.internalUIs.push('TesteVolume');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('varejo-bv-spa-TesteVolume', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: TesteVolume', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#TesteVolume_gbVendaItem').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    complement.renderscyTesteVolume_dGrid(vm);
    ...
    var removeFormViewControl = function () {
        var front = $('#TesteVolume_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#TesteVolume_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#TesteVolume_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#TesteVolume_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootNamespace: 'VAREJO.BV.Diversos',
            ...
            __moduleId__: 'pkg_varejo-bv-spa/viewmodels/TesteVolume'
        };
