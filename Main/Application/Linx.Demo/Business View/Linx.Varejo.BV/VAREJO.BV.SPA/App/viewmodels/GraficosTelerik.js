...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'GraficosTelerik' ]; settings.parentVM.GraficosTelerik = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.GraficosTelerik = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('GraficosTelerik', parentVM.internalUIs) === -1) parentVM.internalUIs.push('GraficosTelerik');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('varejo-bv-spa-GraficosTelerik', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: GraficosTelerik', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#GraficosTelerik_gbExpander_37f817b7d644405aa758fc7c1f130380').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    $('#GraficosTelerik_tcVendaTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#GraficosTelerik_tcVendaTabControl');

    complement.renderGraficosTelerik_chartCustomControl636279343(vm);

    complement.renderGraficosTelerik_chartCustomControl691654859(vm);

    complement.renderGraficosTelerik_chartCustomControl691825781(vm);

    complement.renderGraficosTelerik_chartCustomControl697522109(vm);

    complement.renderGraficosTelerik_chartCustomControl697894734(vm);

    $('#GraficosTelerik_gbVendaItem').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    complement.renderscyGraficosTelerik_dGrid(vm);
    ...
    var visibleColumns = 'BigIntVendaItemKpiInfo';
    ...
    var removeFormViewControl = function () {
        var front = $('#GraficosTelerik_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#GraficosTelerik_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#GraficosTelerik_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#GraficosTelerik_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            __moduleId__: 'pkg_varejo-bv-spa/viewmodels/GraficosTelerik'
        };
