define(['durandal/app', 'pkg_varejo-bv-spa/services/OlapGraficosGaugeContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_varejo-bv-spa/viewmodels/GraficosFlexMonstersComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'GraficosFlexMonsters' ]; settings.parentVM.GraficosFlexMonsters = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.GraficosFlexMonsters = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('GraficosFlexMonsters', parentVM.internalUIs) === -1) parentVM.internalUIs.push('GraficosFlexMonsters');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('varejo-bv-spa-GraficosFlexMonsters', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: GraficosFlexMonsters', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#GraficosFlexMonsters_gbCustomContainer_382822e39efb4588ac61cdbe779ba88d').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    $('#GraficosFlexMonsters_tcVendaTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#GraficosFlexMonsters_tcVendaTabControl');

    complement.renderpivotf099f286c61c485b91d23071b82aeb92GraficosFlexMonsters_pivotVendaItem(vm);

    complement.renderpivot31acee23e1864c76a03e55c35af308c5GraficosFlexMonsters_pivotCopy_612517062_VendaItemPagedList(vm);

    complement.renderpivot913a276f49fa4d689f64e3acfe76cbd4GraficosFlexMonsters_pivotCopy_612534828_VendaItemPagedList(vm);

    complement.renderpivot4327eb3b2c614bbebc853dcbba1bab52GraficosFlexMonsters_pivotCopy_612556078_VendaItemPagedList(vm);

    complement.renderpivot7f7e3959de82442294e28ba60be5f421GraficosFlexMonsters_pivotCopy_612565250_VendaItemPagedList(vm);

    complement.renderpivot683bc16d4adc4b5a851cdbbd555495abGraficosFlexMonsters_pivotCopy_612581937_VendaItemPagedList(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'VendaItem';
    ...
    var visibleColumns = 'BigIntVendaItem,BitVendaItem,ComboboxVendaItem,DatetimeVendaItem,DecimalVendaItem,IdVenda,IdVendaItem,IntVendaItem,SmallIntVendaItem,StringVendaItem,BigIntVendaItemKpiInfo,Ano,Mes,Valor';
    ...
    var getGaugesFormularioRanges = function (succeeded) {
        if (vm.kpiGaugesFormulario == null) dataContext.getGaugesFormularioRanges().then(querySucceeded);
        else if (succeeded) succeeded(vm.kpiGaugesFormulario.ranges, vm.kpiGaugesFormulario.min, vm.kpiGaugesFormulario.max);
        return true;
    
        function querySucceeded(data) {
            vm.kpiGaugesFormulario = { ranges: [], min: 0, max: 0 };
            for (var r in data.results) {
                vm.kpiGaugesFormulario.ranges.push({ from: data.results[r].StartValue, to: data.results[r].EndValue, color: data.results[r].Color });
                if (vm.kpiGaugesFormulario.min > data.results[r].StartValue) vm.kpiGaugesFormulario.min = data.results[r].StartValue;
                if (vm.kpiGaugesFormulario.max < data.results[r].EndValue) vm.kpiGaugesFormulario.max = data.results[r].EndValue;
            }
            if (succeeded) succeeded(vm.kpiGaugesFormulario.ranges, vm.kpiGaugesFormulario.min, vm.kpiGaugesFormulario.max);
        }
    };
    var getGaugesFormularioGaugeGrid = function (succeeded) {
       if (vm.kpiGaugesFormulario == null) {
           getGaugesFormularioRanges(succeeded);
       }
       if (vm.kpiGaugesFormulario != null) {
           var ranges = '';
           for (var i in vm.kpiGaugesFormulario.ranges) {
               ranges += '{"from" : "' + vm.kpiGaugesFormulario.ranges[i].from + '", "to" : "' + vm.kpiGaugesFormulario.ranges[i].to + '" , "color" : "' + vm.kpiGaugesFormulario.ranges[i].color + '" }';
               if (i < vm.kpiGaugesFormulario.ranges.length - 1)
                   ranges += ",";
           }
           ranges = "[" + ranges + "]";
           var obj = {min: vm.kpiGaugesFormulario.min, max: vm.kpiGaugesFormulario.max, ranges: ranges };
           return obj;
        }
       else {
    ...
    var getKPIGridRanges = function (succeeded) {
        if (vm.kpiKPIGrid == null) dataContext.getKPIGridRanges().then(querySucceeded);
        else if (succeeded) succeeded(vm.kpiKPIGrid.ranges, vm.kpiKPIGrid.min, vm.kpiKPIGrid.max);
        return true;
    
        function querySucceeded(data) {
            vm.kpiKPIGrid = { ranges: [], min: 0, max: 0 };
            for (var r in data.results) {
                vm.kpiKPIGrid.ranges.push({ from: data.results[r].StartValue, to: data.results[r].EndValue, color: data.results[r].Color });
                if (vm.kpiKPIGrid.min > data.results[r].StartValue) vm.kpiKPIGrid.min = data.results[r].StartValue;
                if (vm.kpiKPIGrid.max < data.results[r].EndValue) vm.kpiKPIGrid.max = data.results[r].EndValue;
            }
            if (succeeded) succeeded(vm.kpiKPIGrid.ranges, vm.kpiKPIGrid.min, vm.kpiKPIGrid.max);
        }
    };
    var getKPIGridGaugeGrid = function (succeeded) {
       if (vm.kpiKPIGrid == null) {
           getKPIGridRanges(succeeded);
       }
       if (vm.kpiKPIGrid != null) {
           var ranges = '';
           for (var i in vm.kpiKPIGrid.ranges) {
               ranges += '{"from" : "' + vm.kpiKPIGrid.ranges[i].from + '", "to" : "' + vm.kpiKPIGrid.ranges[i].to + '" , "color" : "' + vm.kpiKPIGrid.ranges[i].color + '" }';
               if (i < vm.kpiKPIGrid.ranges.length - 1)
                   ranges += ",";
           }
           ranges = "[" + ranges + "]";
           var obj = {min: vm.kpiKPIGrid.min, max: vm.kpiKPIGrid.max, ranges: ranges };
           return obj;
        }
       else {
           return {min: 0, max: 0, ranges: "[]" };
    ...
    var getKpiColor = function(ranges, value) {
        var result = 'white';
        if (typeof value === 'string')
            value = eval(value);
        if (ranges.length > 0) {
            if (value < eval(ranges[0].from))
                result = ranges[0].color;
            else if (value > eval(ranges[ranges.length - 1].to))
                result = ranges[ranges.length - 1].color;
            else {
                for (var i in ranges)
                {
                    if (value >= eval(ranges[i].from) && value <= eval(ranges[i].to)) {
                        result = ranges[i].color;
                        break;
                    }
                }
            }
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'VendaItem', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'VendaItem', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('VendaItem');
        ...
        return dataContext.getVendaItemByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'VendaItem'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getVendaItemByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearVendaItem(getBandeiraRede(), clearComplete);
    ...
    var removeFormViewControl = function () {
        var front = $('#GraficosFlexMonsters_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#GraficosFlexMonsters_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#GraficosFlexMonsters_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#GraficosFlexMonsters_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var getSPAVersion = function(){
       ...
       var packageName = 'pkg_varejo-bv-spa';
    ...
    var projectTemplateFiles = [
            {
                ...
                name: '(Nenhum)',
                viewName: 'GraficosFlexMonsters',
                projectName: 'VAREJO.BV.SPA',
                content: '',
                id: '',
                selected: true
            }
            ,
            {
                pivotName: 'Copy_612517062_VendaItemPagedList',
                layoutFullName: getSPAVersion() + '/pivotTableLayouts/',
                name: '(Nenhum)',
                viewName: 'GraficosFlexMonsters',
                projectName: 'VAREJO.BV.SPA',
            ...
            ,
            {
                pivotName: 'Copy_612534828_VendaItemPagedList',
                layoutFullName: getSPAVersion() + '/pivotTableLayouts/',
                name: '(Nenhum)',
                viewName: 'GraficosFlexMonsters',
                projectName: 'VAREJO.BV.SPA',
            ...
            ,
            {
                pivotName: 'Copy_612556078_VendaItemPagedList',
                layoutFullName: getSPAVersion() + '/pivotTableLayouts/',
                name: '(Nenhum)',
                viewName: 'GraficosFlexMonsters',
                projectName: 'VAREJO.BV.SPA',
            ...
            ,
            {
                pivotName: 'Copy_612565250_VendaItemPagedList',
                layoutFullName: getSPAVersion() + '/pivotTableLayouts/',
                name: '(Nenhum)',
                viewName: 'GraficosFlexMonsters',
                projectName: 'VAREJO.BV.SPA',
            ...
            ,
            {
                pivotName: 'Copy_612581937_VendaItemPagedList',
                layoutFullName: getSPAVersion() + '/pivotTableLayouts/',
                name: '(Nenhum)',
                viewName: 'GraficosFlexMonsters',
                projectName: 'VAREJO.BV.SPA',
    ...
    var getLayoutsFiles = function () {
        ...
        {
            rootNamespace: 'VAREJO.BV.SPA',
            viewName: 'GraficosFlexMonsters',
            pivotName: 'VendaItem',
            pivotDataSource: 'VendaItem'
        },
            function (data) {
    ...
    var vm = {
            ...
            rootBmTypeName: 'VENDA_ITEM',
            rootDataTypeName: 'VendaItem',
            rootNamespace: 'VAREJO.BV.OlapGraficosGauge',
            ...
            getGaugesFormularioRanges: getGaugesFormularioRanges,
            kpiGaugesFormulario: null,
            getGaugesFormularioGaugeGrid: getGaugesFormularioGaugeGrid,
            getKPIGridRanges: getKPIGridRanges,
            kpiKPIGrid: null,
            getKPIGridGaugeGrid: getKPIGridGaugeGrid,
            ...
            __moduleId__: 'pkg_varejo-bv-spa/viewmodels/GraficosFlexMonsters'
        };
