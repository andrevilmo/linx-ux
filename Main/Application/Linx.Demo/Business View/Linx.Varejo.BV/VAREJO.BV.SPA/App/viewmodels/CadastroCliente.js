...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroCliente' ]; settings.parentVM.CadastroCliente = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.CadastroCliente = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('CadastroCliente', parentVM.internalUIs) === -1) parentVM.internalUIs.push('CadastroCliente');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('varejo-bv-spa-CadastroCliente', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroCliente', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#CadastroCliente_gbClienteWizard').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    complement.renderscyCadastroCliente_dGrid(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'ClienteWizard';
    ...
    var visibleColumns = 'BigIntCliente,BitCliente,ComboboxCliente,DatetimeCliente,DecimalCliente,IdCliente,IdEstado,IntCliente,SmallIntCliente,StringCliente';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'ClienteWizard', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'ClienteWizard', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('ClienteWizard');
        ...
        return dataContext.getClienteWizardByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'ClienteWizard'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getClienteWizardByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearClienteWizard(getBandeiraRede(), clearComplete);
    ...
    var createClienteWizard = function() {
        dataBind('dataView', true);
        var entity = dataContext.createClienteWizard();
    ...
    var createAndNotifyClienteWizard = function() {
        var entity = createClienteWizard();
        notifyPresentation('');
        return entity;
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroCliente_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroCliente_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroCliente_formViewer')[0];
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            ClienteWizardDatetimeCliente_typeRange: ko.observable('R'), ClienteWizardDatetimeCliente_begin: ko.observable(null), ClienteWizardDatetimeCliente_end: ko.observable(null), ClienteWizardDatetimeCliente_predefFilter: ko.observableArray([]), ClienteWizardDatetimeCliente_predefValue: ko.observable(null),
        ClienteWizardBigIntCliente_begin: ko.observable(null), ClienteWizardBigIntCliente_end: ko.observable(null),
        ClienteWizardDecimalCliente_begin: ko.observable(null), ClienteWizardDecimalCliente_end: ko.observable(null),
        ClienteWizardIdCliente_begin: ko.observable(null), ClienteWizardIdCliente_end: ko.observable(null),
        ClienteWizardIntCliente_begin: ko.observable(null), ClienteWizardIntCliente_end: ko.observable(null),
        ClienteWizardSmallIntCliente_begin: ko.observable(null), ClienteWizardSmallIntCliente_end: ko.observable(null),
        ClienteWizardIdEstado: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.ClienteWizardDatetimeCliente_typeRange('R'); entitySearchRange.ClienteWizardDatetimeCliente_begin(null); entitySearchRange.ClienteWizardDatetimeCliente_end(null); entitySearchRange.ClienteWizardDatetimeCliente_predefFilter([]); entitySearchRange.ClienteWizardDatetimeCliente_predefValue(null);
        entitySearchRange.ClienteWizardBigIntCliente_begin(null); entitySearchRange.ClienteWizardBigIntCliente_end(null);
        entitySearchRange.ClienteWizardDecimalCliente_begin(null); entitySearchRange.ClienteWizardDecimalCliente_end(null);
        entitySearchRange.ClienteWizardIdCliente_begin(null); entitySearchRange.ClienteWizardIdCliente_end(null);
        entitySearchRange.ClienteWizardIntCliente_begin(null); entitySearchRange.ClienteWizardIntCliente_end(null);
        entitySearchRange.ClienteWizardSmallIntCliente_begin(null); entitySearchRange.ClienteWizardSmallIntCliente_end(null);
        entitySearchRange.ClienteWizardIdEstado(null);
    };
    entitySearchRange.has_ClienteWizardBigIntCliente = ko.computed(function(){ return (entitySearchRange.ClienteWizardBigIntCliente_begin() != null || entitySearchRange.ClienteWizardBigIntCliente_end() != null); });
    entitySearchRange.has_ClienteWizardDecimalCliente = ko.computed(function(){ return (entitySearchRange.ClienteWizardDecimalCliente_begin() != null || entitySearchRange.ClienteWizardDecimalCliente_end() != null); });
    entitySearchRange.has_ClienteWizardIdCliente = ko.computed(function(){ return (entitySearchRange.ClienteWizardIdCliente_begin() != null || entitySearchRange.ClienteWizardIdCliente_end() != null); });
    entitySearchRange.has_ClienteWizardIntCliente = ko.computed(function(){ return (entitySearchRange.ClienteWizardIntCliente_begin() != null || entitySearchRange.ClienteWizardIntCliente_end() != null); });
    entitySearchRange.has_ClienteWizardSmallIntCliente = ko.computed(function(){ return (entitySearchRange.ClienteWizardSmallIntCliente_begin() != null || entitySearchRange.ClienteWizardSmallIntCliente_end() != null); });
    entitySearchRange.has_ClienteWizardDatetimeCliente = ko.computed(function(){ return (entitySearchRange.ClienteWizardDatetimeCliente_typeRange() == 'R' && (entitySearchRange.ClienteWizardDatetimeCliente_begin() != null || entitySearchRange.ClienteWizardDatetimeCliente_end() != null) || (entitySearchRange.ClienteWizardDatetimeCliente_typeRange() == 'P' && entitySearchRange.ClienteWizardDatetimeCliente_predefFilter().length > 0)); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroCliente_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootDataTypeName: 'ClienteWizard',
            ...
            createAndNotifyClienteWizard: createAndNotifyClienteWizard,
            ...
            __moduleId__: 'pkg_varejo-bv-spa/viewmodels/CadastroCliente'
        };
