...
var complementCtor = function() {
    var complement = {
    isAutomatic: true
    , renderGraficosTelerik_chartCustomControl636279343: function(vm) {
    var itemsSource = { dataBind: function (commitData) {
    if (commitData) {
        return;
    }
    var chart = $("#chartGraficosTelerik_chartCustomControl636279343");
    var innerList = [];
try{
  var innerList = formatTimeZone(unwrapObservableArray(vm.dataView, vm, true));
}catch(e){}

	chart.kendoChart({
	title: {text: 'Gráfico Barra'  , color: '#8B8386'}
,chartArea: {	background: 'transparent'}
	,legend: {position: 'top', labels: {color: '#8B8386'}}
	,seriesColors: ['Orange', 'Green', 'Red', 'Blue', 'NavajoWhite', 'Yellow', 'Purple', 'MediumSeaGreen', 'DarkSalmon', 'Goldenrod', 'SkyBlue', 'Gray', 'SlateBlue', 'MediumPurple', 'RosyBrown']
	,dataSource: {data: innerList, group: { field: 'Ano'}  }
	 ,series: [{
	name: 'Valor'
	,type: 'bar'
	,tooltip: {visible: true, shared: true, template: " Valor : #= kendo.format('R${0}', value) # " }
	,field: 'Valor'
}
] 
	 ,valueAxis: [{
	color: '#8B8386'
	,labels: {format: 'R${0}', rotation: 0 }
}
] 
});
$(window).on("resize", function () {
   $(chart.selector + " svg").width(Number($('.k-content').width()));
   $(chart.selector + " svg").height(Number($('.k-content').height()));
   if($(chart).data("kendoChart") != undefined)
    ...
    vm.addDataSource({ key: 'GraficosTelerik_chartCustomControl636279343', name: 'dataView', itemsSource: itemsSource });
}

, renderGraficosTelerik_chartCustomControl691654859: function(vm) {
    var itemsSource = { dataBind: function (commitData) {
    if (commitData) {
        return;
    }
    var chart = $("#chartGraficosTelerik_chartCustomControl691654859");
    var innerList = [];
try{
  var innerList = formatTimeZone(unwrapObservableArray(vm.dataView, vm, true));
}catch(e){}
	...
	chart.kendoChart({
	title: {text: 'Gráfico Coluna'  , color: '#8B8386'}
,chartArea: {	background: 'transparent'}
	,legend: {position: 'top', labels: {color: '#8B8386'}}
	,seriesColors: ['Orange', 'Green', 'Red', 'Blue', 'NavajoWhite', 'Yellow', 'Purple', 'MediumSeaGreen', 'DarkSalmon', 'Goldenrod', 'SkyBlue', 'Gray', 'SlateBlue', 'MediumPurple', 'RosyBrown']
	,dataSource: {data: innerList, group: { field: 'Ano'}  }
...
$(window).on("resize", function () {
   ...
   if($(chart).data("kendoChart") != undefined)
    ...
    vm.addDataSource({ key: 'GraficosTelerik_chartCustomControl691654859', name: 'dataView', itemsSource: itemsSource });
}

, renderGraficosTelerik_chartCustomControl691825781: function(vm) {
    var itemsSource = { dataBind: function (commitData) {
    if (commitData) {
        return;
    }
    var chart = $("#chartGraficosTelerik_chartCustomControl691825781");
    var innerList = [];
try{
  var innerList = formatTimeZone(unwrapObservableArray(vm.dataView, vm, true));
}catch(e){}
	...
	if (vm.status() === 'Q') {
		chart.kendoChart({
	title: {text: 'Gráfico Pizza'  , color: '#8B8386'}
,chartArea: {	background: 'transparent'}
	,legend: {position: 'top', labels: {color: '#8B8386'}}
	,seriesColors: ['Orange', 'Green', 'Red', 'Blue', 'NavajoWhite', 'Yellow', 'Purple', 'MediumSeaGreen', 'DarkSalmon', 'Goldenrod', 'SkyBlue', 'Gray', 'SlateBlue', 'MediumPurple', 'RosyBrown']
	,dataSource: {data: innerList }
,seriesDefaults: {
labels: {
	template: kendo.template('Valor: #= kendo.format("{0:C2}", value) # ')
	,position: "outsideEnd"
	,visible: true
...
series: [{
	...
	,tooltip: {visible: true, template: "Valor: #= kendo.format('{0:C2}', value) # " }
...
$(window).on("resize", function () {
   ...
   if($(chart).data("kendoChart") != undefined)
    ...
    vm.addDataSource({ key: 'GraficosTelerik_chartCustomControl691825781', name: 'dataView', itemsSource: itemsSource });
}

, renderGraficosTelerik_chartCustomControl697522109: function(vm) {
    var itemsSource = { dataBind: function (commitData) {
    if (commitData) {
        return;
    }
    var chart = $("#chartGraficosTelerik_chartCustomControl697522109");
    var innerList = [];
try{
  var innerList = formatTimeZone(unwrapObservableArray(vm.dataView, vm, true));
}catch(e){}
	...
	chart.kendoChart({
	title: {text: 'Gráfico Linha'  , color: '#8B8386'}
,chartArea: {	background: 'transparent'}
	,legend: {position: 'top', labels: {color: '#8B8386'}}
	,seriesColors: ['Orange', 'Green', 'Red', 'Blue', 'NavajoWhite', 'Yellow', 'Purple', 'MediumSeaGreen', 'DarkSalmon', 'Goldenrod', 'SkyBlue', 'Gray', 'SlateBlue', 'MediumPurple', 'RosyBrown']
	,dataSource: {data: innerList   }
...
$(window).on("resize", function () {
   ...
   if($(chart).data("kendoChart") != undefined)
    ...
    vm.addDataSource({ key: 'GraficosTelerik_chartCustomControl697522109', name: 'dataView', itemsSource: itemsSource });
}

, renderGraficosTelerik_chartCustomControl697894734: function(vm) {
    var itemsSource = { dataBind: function (commitData) {
    if (commitData) {
        return;
    }
    var chart = $("#chartGraficosTelerik_chartCustomControl697894734");
    var innerList = [];
try{
  var innerList = formatTimeZone(unwrapObservableArray(vm.dataView, vm, true));
}catch(e){}
	...
	chart.kendoChart({
	title: {text: 'Gráfico Area'  , color: '#8B8386'}
,chartArea: {	background: 'transparent'}
	,legend: {position: 'top', labels: {color: '#8B8386'}}
	,seriesColors: ['Orange', 'Green', 'Red', 'Blue', 'NavajoWhite', 'Yellow', 'Purple', 'MediumSeaGreen', 'DarkSalmon', 'Goldenrod', 'SkyBlue', 'Gray', 'SlateBlue', 'MediumPurple', 'RosyBrown']
	,dataSource: {data: innerList   }
...
$(window).on("resize", function () {
   ...
   if($(chart).data("kendoChart") != undefined)
    ...
    vm.addDataSource({ key: 'GraficosTelerik_chartCustomControl697894734', name: 'dataView', itemsSource: itemsSource });
}


, renderscyGraficosTelerik_dGrid: function(vm) {
    ...
    $('#scyGraficosTelerik_dGrid_headers').live('focus  keydown', function (evt) {
        var keyCode = window.event ? evt.which : evt.keyCode;
        if (keyCode === 9) {
            var cols = $('#scyGraficosTelerik_dGrid').igGrid('option', 'columns');
            var dataView = $('#scyGraficosTelerik_dGrid').data('igGrid').dataSource._dataView
            if (dataView.length === 0) return;
            var firstRow = dataView[0].RowDataId;
            clear = vm.status() === 'C';
            if (vm.status() === 'C')
                $('#scyGraficosTelerik_dGrid').igGridUpdating('startEdit', firstRow, 0);
            else {
                ...
                cols.some(function (entry) {
                    if (entry.key !== 'RowDataId' && !entry.hidden) {
                        if (verifyCanEditCol(entry.key, clear, entity)) {
                            $('#scyGraficosTelerik_dGrid').igGridUpdating('startEdit', firstRow, indexColumn);
                            return true;
                        }
                        indexColumn++;
                    }
    ...
    var getVisibleColumns = function(metaDataControl) {
       ...
       var visibleColumns = '';
       if($('#scyGraficosTelerik_dGrid').data('igGrid') === undefined) return '';
       var cols = $('#scyGraficosTelerik_dGrid').igGrid('option', 'columns');
       if (cols) {
    ...
    var isElementHided = function (grid, forceCreating) {
      if (!grid) grid = $('#scyGraficosTelerik_dGrid');
      return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialog').is(':visible'));
    }
    var refreshData = true;
    var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'scyGraficosTelerik_dGrid_container', dataBind: function (commitData, forceCreating) {
       var grid = $('#scyGraficosTelerik_dGrid');
       ...
       if (!started) {
           ...
           started = true;
           commitData = false;
           $('#scyGraficosTelerik_dGrid_groupbyarea').addClass('hide');
       }
       if (grid.igGridUpdating('isEditing')) {
    ...
    function createDataGrid(grid) {
        ...
        vm.gridSaveStates[gridId] = {
            ...
            closePopover: function () {
                $('#scyGraficosTelerik_dGrid_LayoutBtn').igPopover('hide');
            },
        ...
        grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
            ...
            cellClick: function(evt, ui) {
                 if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                     ...
                     if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                     {
                          ...
                          showMultimidia(entity, e, table, key, vm.GraficosTelerik());
                     }
                 }
                 if (typeof vm.OnGridClientClick === 'function') {
                     entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                     vm.OnGridClientClick('scyGraficosTelerik_dGrid', ui.colKey, entity);
                 }
                 var grid = $('#scyGraficosTelerik_dGrid');
            ...
            features: [
                          ...
                          columnSorting: function (evt, ui) { 
                              $.grep(ui.owner.grid._visibleColumnsArray, function (e) { 
                                  if (e.key === ui.columnKey && e.dataType === 'string') 
                                      return $('#scyGraficosTelerik_dGrid').igGridSorting('option', 'caseSensitive', false); 
                                  else if (e.key === ui.columnKey) 
                                      return $('#scyGraficosTelerik_dGrid').igGridSorting('option', 'caseSensitive', true); 
                              }); 
                          } 
                        ...
                        { name: 'Tooltips', columnSettings:[{ columnKey: "BigIntVendaItemKpiInfo", allowTooltips: true }] },
                        { name: 'Resizing' }, 
                        { name: 'Hiding', 
                            columnHidden: function (evt, ui) {
                               showMultimidiaLazy('#scyGraficosTelerik_dGrid');
                            },
                       ...
                       ,{ name: 'Updating', horizontalMoveOnEnter: true,
                          ...
                          columnSettings: [{ columnKey: "BigIntVendaItemKpiInfo" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyGraficosTelerik_dGrid', 'BigIntVendaItemKpiInfo', ui.oldValue, ui.value);}}, maxLength: 0 } }],
                          ...
                          editCellStarted: function(evt, ui){
                              ...
                              var columns = $('#scyGraficosTelerik_dGrid').igGridUpdating('option', 'columnSettings');
                              var currentCol = null;
                              currentRow = ui.rowID;
            ...
            if ((typeof vm.OnDataGridCreated === 'function')){
                vm.OnDataGridCreated('scyGraficosTelerik_dGrid');
            }
            ...
            selectionrowselectionchanged = function (evt, ui) {
                if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                    if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                        $(document).undelegate('#scyGraficosTelerik_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        ui.owner.clearSelection();
                        ui.owner.selectRow(ui.row.index);
                        selectedRowId = ui.row.id;
                        $(document).delegate ('#scyGraficosTelerik_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                    }
                    selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                 } 
            };
            $(document).delegate('#scyGraficosTelerik_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
            $('#scyGraficosTelerik_dGrid > tbody tr').live('focus', function(evt) {
                var grid = $('#scyGraficosTelerik_dGrid'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
            ...
            grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                if (vm.GraficosTelerik().status() === 'Q') vm.GraficosTelerik().dataToolbar.viewInfo();
            });
        }
        vm.addDataSource({ key: 'scyGraficosTelerik_dGrid', name: 'dataView', itemsSource: itemsSource });
    }
