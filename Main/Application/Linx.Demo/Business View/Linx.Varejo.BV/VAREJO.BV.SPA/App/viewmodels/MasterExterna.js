...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'MasterExterna' ]; settings.parentVM.MasterExterna = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('MasterExterna', parentVM.internalUIs) === -1) parentVM.internalUIs.push('MasterExterna');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('varejo-bv-spa-MasterExterna', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: MasterExterna', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#MasterExterna_gbCliente').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    $('#MasterExterna_gbExpander_076506479b344e7e9b224b380868eb19').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    $('#MasterExterna_tcTabControl_e6137edad57b495b91481a4abf4c8cb6').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#MasterExterna_tcTabControl_e6137edad57b495b91481a4abf4c8cb6');

    $('#MasterExterna_gbExpander_4744f4dacd9c4ce18813fff6d8f6b110').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    $('#MasterExterna_gbExpander_da260f1f1ec84d41a2f4fdeacb082c72').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    $('#MasterExterna_gbExpander_9e852204ec924648b68e3567f57f0b0e').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    complement.renderscyMasterExterna_dGrid(vm);
    ...
    var visibleColumns = 'BigIntCliente,BitCliente,ComboboxCliente,DatetimeCliente,DecimalCliente,IdCliente,IntCliente,SmallIntCliente,StringCliente,IdEstado';
    ...
    var ExternaBotao360152687_Click = function () {
    
      var str = 'VAREJO.BV.SPA/UIExternaBotao'; while (str.indexOf('.') !== -1) { str = str.replace('.', '-'); };
      var currentModal = (modal.inUse ? modal2 : modal);
      currentModal.showModal('pkg_' + strExtract('#' + str.toLowerCase(), '#', '/') + '/viewmodels/' + strExtract(str + '#', '/', '#'), {  executeQuery: true, parentUI: {vm: vm}, querySetters: {}, toolbarSettings: {removeDataToolbar:false,canAddNew:true,canClear:true,canCustomSearch:true,canDelete:true,canEdit:true,canLayout:true,canNavigate:true,canPrint:true,canSearch:true,noBusyLoading:false} }, 'ExternaBotao', ['Ok'], 'large')
    
    }
    var OpenView1317444765_Click = function () {
    
      var str = 'VAREJO.BV.SPA/OpenView'.toLowerCase();
      while (str.indexOf('.') !== -1) { str = str.replace('.', '-').replace('/', '-'); };
      vm.common.go('#' + str, 'objectQuery=null&executeQuery=true')
    ...
    var removeFormViewControl = function () {
        var front = $('#MasterExterna_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#MasterExterna_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#MasterExterna_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#MasterExterna_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            ExternaBotao360152687_Click: ExternaBotao360152687_Click,
            OpenView1317444765_Click: OpenView1317444765_Click,
            ...
            __moduleId__: 'pkg_varejo-bv-spa/viewmodels/MasterExterna'
        };
