define(['durandal/app', 'pkg_varejo-bv-spa/services/MestreDetalheSubDetalhesContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_varejo-bv-spa/viewmodels/UIPaiFilhaNetaComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    //#region quick search
    var quickSearchTerm = '';
    function repoFormatResult(repo, element, ops, escapeMarkup) {
        //Adjust last quick qearch term
        quickSearchTerm = ops.term;
        ...
        //Header
        if (repo.id === "-1") {
             markup += '<div class="col-sm-12"><label class="control-label" style="font-weight: bold">String Cliente</label></div>';
        }
        else {
        //Data
        markup += '<div class="col-sm-12"><label class="control-label">' + repo.StringCliente + '</label></div>';
        }
    ...
    function repoFormatSelection(repo) {
        ...
        var quickSearchJExpression = 'Cliente{';
        if (repo.id === "-1") //Select all
        {
           quickSearchJExpression += '(;'
           quickSearchJExpression += 'StringCliente#Like#S' + '%' + encode(quickSearchTerm) + '%';
           quickSearchJExpression += ';)'
        }
        else { //Select only one element
           if (!isNullOrEmpty(repo.StringCliente)) {
               quickSearchJExpression += 'StringCliente#==#S' + encode(repo.StringCliente.toString());
           }
        }
        quickSearchJExpression += '}';
        _canQuickSearch = false;
        refreshToolbar();
        _canQuickSearch = true;
        dataToolbar.query(false, null, quickSearchJExpression);
        return repo.StringCliente;
    }
    
    var quickSearch = function () {
        createQuickSearch(dataContext.getDataServiceUrl() + "/GetClienteQuickSearch?$inlinecount=allpages",
        repoFormatResult, repoFormatSelection);
    }
    //#endregion 
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('varejo-bv-spa-UIPaiFilhaNeta', function (data) {
    ...
    var compositionComplete = function() {
        $('#UIPaiFilhaNeta_gbCustomContainer_7ffb00cc4a1b49168a8589eb4d89cb30').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    $('#UIPaiFilhaNeta_gbExpander_941629aa753a47a4adfd10b2e0cf4196').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });
    ...
    var visibleColumns = 'StringCliente,BigIntCliente,ComboboxCliente,IdEstado,IdPais,IdCliente,DatetimeCliente,DecimalCliente,BitCliente,IntCliente,StringPais,StringEstado,SmallIntCliente';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        return dataContext.getClienteByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
            ...
            for (var idxElem = toList.length - 1; idxElem >= 0; idxElem--) {
                    if (toList[idxElem].ChangeState !== 'N') {
                       ...
                       if (toList[idxElem].ChangeState == 'I') {
                           fromObj = _.where(fromList, { TemporaryIdCliente: toList[idxElem]['IdCliente'] });
                       } else {
                           fromObj = _.where(fromList, { IdCliente: toList[idxElem]['IdCliente'] });
                       }
    ...
    var canQuickSearch = ko.computed(function () { return (status() === 'Q' || status() === 'C') && _canQuickSearch && _canClear && _canSearch && !isChildVM(); });
    ...
    var entitySearchRange = {
        ...
        VendaItemIdVenda_begin: ko.observable(null), VendaItemIdVenda_end: ko.observable(null),
        ...
        VendaAtacadoIdCliente_begin: ko.observable(null), VendaAtacadoIdCliente_end: ko.observable(null),
        ...
        ClienteIdPais: ko.observable(null),
        ClienteStringEstado: ko.observable(null),
        ClienteStringPais: ko.observable(null),
        VendaIdLoja: ko.observable(null),
        VendaStringLoja: ko.observable(null)
    };
    entitySearchRange.clear = function(){
        ...
        entitySearchRange.ClienteIdPais(null);
        entitySearchRange.ClienteStringEstado(null);
        entitySearchRange.ClienteStringPais(null);
        entitySearchRange.VendaIdLoja(null);
        entitySearchRange.VendaStringLoja(null);
    };
    ...
    entitySearchRange.has_VendaItemIdVenda = ko.computed(function(){ return (entitySearchRange.VendaItemIdVenda_begin() != null || entitySearchRange.VendaItemIdVenda_end() != null); });
    ...
    entitySearchRange.has_VendaAtacadoIdCliente = ko.computed(function(){ return (entitySearchRange.VendaAtacadoIdCliente_begin() != null || entitySearchRange.VendaAtacadoIdCliente_end() != null); });
    ...
    var vm = {
            ...
            rootNamespace: 'VAREJO.BV.MestreDetalheSubDetalhes',
            ...
            __moduleId__: 'pkg_varejo-bv-spa/viewmodels/UIPaiFilhaNeta'
        };
