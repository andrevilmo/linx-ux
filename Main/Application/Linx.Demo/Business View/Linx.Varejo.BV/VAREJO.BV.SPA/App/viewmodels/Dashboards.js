...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'Dashboards' ]; settings.parentVM.Dashboards = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('Dashboards', parentVM.internalUIs) === -1) parentVM.internalUIs.push('Dashboards');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('varejo-bv-spa-Dashboards', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: Dashboards', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#Dashboards_gbVendaItem').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    $('#Dashboards_gbExpander_5cade9e1fd504018984fe5c5cf182122').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    $('#Dashboards_gbExpander_32a5f9608c1d43389522cc8847eac239').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    complement.renderscyDashboards_dGrid(vm);
    ...
    var visibleColumns = 'SmallIntVendaItem,IntVendaItem,BigIntVendaItem,DecimalVendaItem,BitVendaItem,ComboboxVendaItem,DatetimeVendaItem,IdVenda,IdVendaItem,BigIntVendaItemKpiInfo,StringVendaItem';
    ...
    var removeFormViewControl = function () {
        var front = $('#Dashboards_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#Dashboards_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#Dashboards_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#Dashboards_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            __moduleId__: 'pkg_varejo-bv-spa/viewmodels/Dashboards'
        };
