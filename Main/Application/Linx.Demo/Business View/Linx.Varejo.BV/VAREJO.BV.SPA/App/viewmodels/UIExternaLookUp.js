...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'UIExternaLookUp' ]; settings.parentVM.UIExternaLookUp = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.UIExternaLookUp = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('UIExternaLookUp', parentVM.internalUIs) === -1) parentVM.internalUIs.push('UIExternaLookUp');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('varejo-bv-spa-UIExternaLookUp', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: UIExternaLookUp', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        complement.renderUIExternaLookUp_dGridEstado(vm);
    ...
    var visibleColumns = 'BigIntEstado,BitEstado,ComboboxEstado,DatetimeEstado,DecimalEstado,IdEstado,IdPais,IntEstado,SmallIntEstado,StringEstado';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        return dataContext.getEstadoByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
            ...
            for (var idxElem = toList.length - 1; idxElem >= 0; idxElem--) {
                    if (toList[idxElem].ChangeState !== 'N') {
                       ...
                       if (toList[idxElem].ChangeState == 'I') {
                           fromObj = _.where(fromList, { TemporaryIdEstado: toList[idxElem]['IdEstado'] });
                       } else {
                           fromObj = _.where(fromList, { IdEstado: toList[idxElem]['IdEstado'] });
                       }
    ...
    var removeFormViewControl = function () {
        var front = $('#UIExternaLookUp_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#UIExternaLookUp_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#UIExternaLookUp_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#UIExternaLookUp_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            __moduleId__: 'pkg_varejo-bv-spa/viewmodels/UIExternaLookUp'
        };
