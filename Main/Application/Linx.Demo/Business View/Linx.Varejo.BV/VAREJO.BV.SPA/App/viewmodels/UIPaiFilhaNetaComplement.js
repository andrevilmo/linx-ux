...
, renderUIPaiFilhaNeta_dGridVenda: function(vm) {
    var self = this;
    self.vm = vm;
    ...
    $('#UIPaiFilhaNeta_dGridVenda_headers').live('focus  keydown', function (evt) {
        var keyCode = window.event ? evt.which : evt.keyCode;
        if (keyCode === 9) {
            var cols = $('#UIPaiFilhaNeta_dGridVenda').igGrid('option', 'columns');
            var dataView = $('#UIPaiFilhaNeta_dGridVenda').data('igGrid').dataSource._dataView
            if (dataView.length === 0) return;
            var firstRow = dataView[0].RowDataId;
            clear = vm.status() === 'C';
            if (vm.status() === 'C')
                $('#UIPaiFilhaNeta_dGridVenda').igGridUpdating('startEdit', firstRow, 0);
            else {
                ...
                cols.some(function (entry) {
                    if (entry.key !== 'RowDataId' && !entry.hidden) {
                        if (verifyCanEditCol(entry.key, clear, entity)) {
                            $('#UIPaiFilhaNeta_dGridVenda').igGridUpdating('startEdit', firstRow, indexColumn);
                            return true;
                        }
                        indexColumn++;
                    }
    ...
    var getVisibleColumns = function(metaDataControl) {
       ...
       var visibleColumns = '';
       if($('#UIPaiFilhaNeta_dGridVenda').data('igGrid') === undefined) return '';
       var cols = $('#UIPaiFilhaNeta_dGridVenda').igGrid('option', 'columns');
       if (cols) {
    ...
    var isElementHided = function (grid, forceCreating) {
      if (!grid) grid = $('#UIPaiFilhaNeta_dGridVenda');
      return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialogVenda').is(':visible'));
    }
    var refreshData = true;
    var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'UIPaiFilhaNeta_dGridVenda_container', dataBind: function (commitData, forceCreating) {
       var grid = $('#UIPaiFilhaNeta_dGridVenda');
       ...
       if (!started) {
           ...
           started = true;
           commitData = false;
           $('#UIPaiFilhaNeta_dGridVenda_groupbyarea').addClass('hide');
       }
       if (grid.igGridUpdating('isEditing')) {
    ...
    function createDataGrid(grid) {
        ...
        vm.gridSaveStates[gridId] = {
            ...
            closePopover: function () {
                $('#UIPaiFilhaNeta_dGridVenda_LayoutBtn').igPopover('hide');
            },
        ...
        grid.igGrid({ height: (getGridHeightSuggested() * 0.75), width: '100%',
            ...
            cellClick: function(evt, ui) {
                 ...
                 if (typeof vm.OnGridClientClick === 'function') {
                     entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                     vm.OnGridClientClick('UIPaiFilhaNeta_dGridVenda', ui.colKey, entity);
                 }
                 var grid = $('#UIPaiFilhaNeta_dGridVenda');
            ...
            columns: [
                ...
                { key: 'StringLoja', headerText: 'String Loja', width: '421px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
            ...
            features: [
                          ...
                          columnSorting: function (evt, ui) { 
                              $.grep(ui.owner.grid._visibleColumnsArray, function (e) { 
                                  if (e.key === ui.columnKey && e.dataType === 'string') 
                                      return $('#UIPaiFilhaNeta_dGridVenda').igGridSorting('option', 'caseSensitive', false); 
                                  else if (e.key === ui.columnKey) 
                                      return $('#UIPaiFilhaNeta_dGridVenda').igGridSorting('option', 'caseSensitive', true); 
                              }); 
                          } 
            },
                        ...
                        { name: 'RowSelectors', enableCheckBoxes: vm.allowMultiSelectionInSearch(), enableRowNumbering: false, checkBoxStateChanged: function(evt, ui){ 
                           if ((typeof vm.OnDataGridRowChecked === 'function')){
                               vm.OnDataGridRowChecked('UIPaiFilhaNeta_dGridVenda', self.selectedItems());
                           }
                           ...
                           if ((selectedRows.length == dataViewLength) || (selectedRow == null && selectedRows.length > 0)){
                               rowId = [];
                               rowId['id'] = 1;
                               selectGridCurrentItem(vm.goToKey, 'RowDataId', rowId, vm.currentDataItem().currentVenda, getDataSource());
                        ...
                        { name: 'Tooltips', columnSettings:[{ columnKey: "BitVenda", allowTooltips: true },{ columnKey: "DatetimeVenda", allowTooltips: true },{ columnKey: "BigIntVenda", allowTooltips: true },{ columnKey: "StringLoja", allowTooltips: false },{ columnKey: "ComboboxVenda", allowTooltips: true },{ columnKey: "DecimalVenda", allowTooltips: true },{ columnKey: "IdCliente", allowTooltips: true },{ columnKey: "IdVenda", allowTooltips: true },{ columnKey: "IdLoja", allowTooltips: false },{ columnKey: "StringVenda", allowTooltips: true },{ columnKey: "IntVenda", allowTooltips: true },{ columnKey: "SmallIntVenda", allowTooltips: true }] },
                        { name: 'Resizing' }, 
                        { name: 'Hiding', 
                            columnHidden: function (evt, ui) {
                               showMultimidiaLazy('#UIPaiFilhaNeta_dGridVenda');
                            },
                        ...
                        ,{ name: 'ColumnMoving' }
                       ,{ name: 'GroupBy', emptyGroupByAreaContent: 'Arraste para esta área a(s) coluna(s) que deseja agrupar.', initialExpand: false, groupedColumnsChanged: function (evt, ui) { $('#UIPaiFilhaNeta_dGridVenda_groupbyarea').toggleClass('is-grouped', (ui.groupedColumns.length > 0)); } }
                       ...
                       ,{ name: 'Updating', horizontalMoveOnEnter: true,
                          ...
                          columnSettings: [{ columnKey: "StringLoja", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpLoja", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, validateOnClearState:false, maxValue:null, maxLength: 50, defaultValue: '' } }, { columnKey: "IdLoja", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpLoja", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, validateOnClearState:false, maxValue:2147483647, maxLength: 12, defaultValue: null } }, { columnKey: 'DatetimeVenda', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid === 'function') {vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVenda', 'DatetimeVenda', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: 'ComboboxVenda', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('ComboboxVenda', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('LX_VENDA', ''),  textKey: 'name', valueKey: 'id', enableClearButton: false }}, { columnKey: "BigIntVenda" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVenda', 'BigIntVenda', ui.oldValue, ui.value);}},  maxLength: 24, maxValue: null, minValue: 0, dataMode: 'long' } }, { columnKey: "DecimalVenda" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVenda', 'DecimalVenda', ui.oldValue, ui.value);}},  maxLength: 14, maxValue: 99999999999.99, minValue: 0, dataMode: 'decimal', minDecimals: 2, maxDecimals: 2 } }, { columnKey: "IdCliente" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVenda', 'IdCliente', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "IdVenda" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVenda', 'IdVenda', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, dataMode: 'int' } }, { columnKey: "StringVenda" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVenda', 'StringVenda', ui.oldValue, ui.value);}}, maxLength: 50 } }, { columnKey: "IntVenda" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVenda', 'IntVenda', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "SmallIntVenda" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVenda', 'SmallIntVenda', ui.oldValue, ui.value);}},  maxLength: 6, maxValue: null, minValue: 0, dataMode: 'short' } }],
                          ...
                          editCellStarted: function(evt, ui){
                              ...
                              var columns = $('#UIPaiFilhaNeta_dGridVenda').igGridUpdating('option', 'columnSettings');
                              var currentCol = null;
                              currentRow = ui.rowID;
            ...
            if ((typeof vm.OnDataGridCreated === 'function')){
                vm.OnDataGridCreated('UIPaiFilhaNeta_dGridVenda');
            }
            ...
            selectionrowselectionchanged = function (evt, ui) {
                if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                 ...
                 if ((typeof vm.OnDataGridRowChecked === 'function')){ vm.OnDataGridRowChecked('UIPaiFilhaNeta_dGridVenda', self.selectedItems()); }
            };
            $(document).delegate('#UIPaiFilhaNeta_dGridVenda', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
        }
        vm.addDataSource({ key: 'UIPaiFilhaNeta_dGridVenda', name: 'VendaList', itemsSource: itemsSource });
    }

, renderUIPaiFilhaNeta_dGridVendaAtacado: function(vm) {
    var getDataSource = function() {
        var source = null;
        try {
    ...
    $('#UIPaiFilhaNeta_dGridVendaAtacado_headers').live('focus  keydown', function (evt) {
        var keyCode = window.event ? evt.which : evt.keyCode;
        if (keyCode === 9) {
            var cols = $('#UIPaiFilhaNeta_dGridVendaAtacado').igGrid('option', 'columns');
            var dataView = $('#UIPaiFilhaNeta_dGridVendaAtacado').data('igGrid').dataSource._dataView
            if (dataView.length === 0) return;
            var firstRow = dataView[0].RowDataId;
            clear = vm.status() === 'C';
            if (vm.status() === 'C')
                $('#UIPaiFilhaNeta_dGridVendaAtacado').igGridUpdating('startEdit', firstRow, 0);
            else {
                var entity = findElementByKey(getDataSource(), 'RowDataId', firstRow);
                var indexColumn = 0;
                cols.some(function (entry) {
                    if (entry.key !== 'RowDataId' && !entry.hidden) {
                        if (verifyCanEditCol(entry.key, clear, entity)) {
                            $('#UIPaiFilhaNeta_dGridVendaAtacado').igGridUpdating('startEdit', firstRow, indexColumn);
                            return true;
                        }
                        indexColumn++;
                    }
    ...
    var getVisibleColumns = function(metaDataControl) {
       if (metaDataControl) return '';
       var visibleColumns = '';
       if($('#UIPaiFilhaNeta_dGridVendaAtacado').data('igGrid') === undefined) return '';
       var cols = $('#UIPaiFilhaNeta_dGridVendaAtacado').igGrid('option', 'columns');
       if (cols) {
         for (var idx = 0; idx < cols.length; idx++) {
             if (cols[idx].hidden !== true) visibleColumns += (visibleColumns === '' ? '' : ',') + cols[idx].key;
    ...
    var isElementHided = function (grid, forceCreating) {
      if (!grid) grid = $('#UIPaiFilhaNeta_dGridVendaAtacado');
      return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialogVendaAtacado').is(':visible'));
    }
    var refreshData = true;
    var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'UIPaiFilhaNeta_dGridVendaAtacado_container', dataBind: function (commitData, forceCreating) {
       var grid = $('#UIPaiFilhaNeta_dGridVendaAtacado');
       if (started && typeof grid.data('igGridUpdating') === 'undefined') { started = false; }
       if (commitData && started) {
           if (grid.igGridUpdating('isEditing')) {
               grid.igGrid('commit');
       ...
       if (!started) {
           ...
           started = true;
           commitData = false;
           $('#UIPaiFilhaNeta_dGridVendaAtacado_groupbyarea').addClass('hide');
       }
       if (grid.igGridUpdating('isEditing')) {
            grid.igGridUpdating('endEdit', true);
       }
    ...
    function createDataGrid(grid) {
        ...
        vm.gridSaveStates[gridId] = {
            ...
            closePopover: function () {
                $('#UIPaiFilhaNeta_dGridVendaAtacado_LayoutBtn').igPopover('hide');
            },
            applyLayout: function (layoutInfo) {
                var _this = this;
                if (isNull(layoutInfo) && (!_this.currentLayout() || _this.currentLayout().Id === 0)) {
        ...
        grid.igGrid({ height: (getGridHeightSuggested() * 0.75), width: '100%',
            ...
            cellClick: function(evt, ui) {
                 ...
                 if (typeof vm.OnGridClientClick === 'function') {
                     entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                     vm.OnGridClientClick('UIPaiFilhaNeta_dGridVendaAtacado', ui.colKey, entity);
                 }
                 var grid = $('#UIPaiFilhaNeta_dGridVendaAtacado');
                 var isEditing = grid.igGridUpdating('isEditing');
                 if (!isEditing)
            ...
            features: [
                        { name: 'Sorting', type: 'local', caseSensitive: true,
                          columnSorting: function (evt, ui) { 
                              $.grep(ui.owner.grid._visibleColumnsArray, function (e) { 
                                  if (e.key === ui.columnKey && e.dataType === 'string') 
                                      return $('#UIPaiFilhaNeta_dGridVendaAtacado').igGridSorting('option', 'caseSensitive', false); 
                                  else if (e.key === ui.columnKey) 
                                      return $('#UIPaiFilhaNeta_dGridVendaAtacado').igGridSorting('option', 'caseSensitive', true); 
                              }); 
                          } 
            },
                        { name: 'Filtering', mode: 'advanced', filterDropDownItemIcons: false, filterDropDownWidth: 200, allowFiltering: true, type: 'local', renderFC: false, renderFilterButton: true, 
                        ...
                        { name: 'Hiding', 
                            columnHidden: function (evt, ui) {
                               showMultimidiaLazy('#UIPaiFilhaNeta_dGridVendaAtacado');
                            },
                            columnShown: function (evt, ui) {
                               showMultimidiaLazy('#UIPaiFilhaNeta_dGridVendaAtacado');
                            }
                       ...
                       ,{ name: 'Updating', horizontalMoveOnEnter: true,
                          ...
                          showReadonlyEditors: false,
                          showDoneCancelButtons: false,
                          columnSettings: [{ columnKey: 'DatetimeVendaAtacado', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid === 'function') {vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVendaAtacado', 'DatetimeVendaAtacado', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: 'ComboboxVendaAtacado', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('ComboboxVendaAtacado', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('LX_VENDA_ATACADO', ''),  textKey: 'name', valueKey: 'id', enableClearButton: false }}, { columnKey: "BigIntVendaAtacado" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVendaAtacado', 'BigIntVendaAtacado', ui.oldValue, ui.value);}},  maxLength: 24, maxValue: null, minValue: 0, dataMode: 'long' } }, { columnKey: "DecimalVendaAtacado" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVendaAtacado', 'DecimalVendaAtacado', ui.oldValue, ui.value);}},  maxLength: 14, maxValue: 99999999999.99, minValue: 0, dataMode: 'decimal', minDecimals: 2, maxDecimals: 2 } }, { columnKey: "IdCliente" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVendaAtacado', 'IdCliente', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "IdVendaAtacado" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVendaAtacado', 'IdVendaAtacado', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, dataMode: 'int' } }, { columnKey: "IntVendaAtacado" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVendaAtacado', 'IntVendaAtacado', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "SmallIntVendaAtacado" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVendaAtacado', 'SmallIntVendaAtacado', ui.oldValue, ui.value);}},  maxLength: 6, maxValue: null, minValue: 0, dataMode: 'short' } }, { columnKey: "StringVendaAtacado" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVendaAtacado', 'StringVendaAtacado', ui.oldValue, ui.value);}}, maxLength: 50 } }],
                          rowDeleting: function (evt, ui) {
                              deletedIndex = ui.element.context.rowIndex;
                              var entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowID);
                              if (entity) {
                          ...
                          editCellStarted: function(evt, ui){
                              var lstRefreshDados = null;
                              var columns = $('#UIPaiFilhaNeta_dGridVendaAtacado').igGridUpdating('option', 'columnSettings');
                              var currentCol = null;
                              currentRow = ui.rowID;
                              columns.forEach(function (entry, index) {
                                 if (entry.columnKey === ui.columnKey) currentCol = entry;
            ...
            if ((typeof vm.OnDataGridCreated === 'function')){
                vm.OnDataGridCreated('UIPaiFilhaNeta_dGridVendaAtacado');
            }
            var selectionrowselectionchanged = null, selectedRowId = -1;
            selectionrowselectionchanged = function (evt, ui) {
                if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                    if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                        $(document).undelegate('#UIPaiFilhaNeta_dGridVendaAtacado', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        ui.owner.clearSelection();
                        ui.owner.selectRow(ui.row.index);
                        selectedRowId = ui.row.id;
                        $(document).delegate ('#UIPaiFilhaNeta_dGridVendaAtacado', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                    }
                    selectGridCurrentItem(vm.goToKey, 'RowDataId', ui, vm.currentDataItem().currentVendaAtacado, getDataSource()); 
                 } 
            };
            $(document).delegate('#UIPaiFilhaNeta_dGridVendaAtacado', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
            $('#UIPaiFilhaNeta_dGridVendaAtacado > tbody tr').live('focus', function(evt) {
                var grid = $('#UIPaiFilhaNeta_dGridVendaAtacado'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
        ...
        vm.addDataSource({ key: 'UIPaiFilhaNeta_dGridVendaAtacado', name: 'VendaAtacadoList', itemsSource: itemsSource });
    }

, renderUIPaiFilhaNeta_dGridVendaItem: function(vm) {
    var getDataSource = function() {
        var source = null;
        try {
    ...
    $('#UIPaiFilhaNeta_dGridVendaItem_headers').live('focus  keydown', function (evt) {
        var keyCode = window.event ? evt.which : evt.keyCode;
        if (keyCode === 9) {
            var cols = $('#UIPaiFilhaNeta_dGridVendaItem').igGrid('option', 'columns');
            var dataView = $('#UIPaiFilhaNeta_dGridVendaItem').data('igGrid').dataSource._dataView
            if (dataView.length === 0) return;
            var firstRow = dataView[0].RowDataId;
            clear = vm.status() === 'C';
            if (vm.status() === 'C')
                $('#UIPaiFilhaNeta_dGridVendaItem').igGridUpdating('startEdit', firstRow, 0);
            else {
                var entity = findElementByKey(getDataSource(), 'RowDataId', firstRow);
                var indexColumn = 0;
                cols.some(function (entry) {
                    if (entry.key !== 'RowDataId' && !entry.hidden) {
                        if (verifyCanEditCol(entry.key, clear, entity)) {
                            $('#UIPaiFilhaNeta_dGridVendaItem').igGridUpdating('startEdit', firstRow, indexColumn);
                            return true;
                        }
                        indexColumn++;
                    }
    ...
    var getVisibleColumns = function(metaDataControl) {
       if (metaDataControl) return '';
       var visibleColumns = '';
       if($('#UIPaiFilhaNeta_dGridVendaItem').data('igGrid') === undefined) return '';
       var cols = $('#UIPaiFilhaNeta_dGridVendaItem').igGrid('option', 'columns');
       if (cols) {
         for (var idx = 0; idx < cols.length; idx++) {
             if (cols[idx].hidden !== true) visibleColumns += (visibleColumns === '' ? '' : ',') + cols[idx].key;
    ...
    var isElementHided = function (grid, forceCreating) {
      if (!grid) grid = $('#UIPaiFilhaNeta_dGridVendaItem');
      return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialogVendaItem').is(':visible'));
    }
    var refreshData = true;
    var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'UIPaiFilhaNeta_dGridVendaItem_container', dataBind: function (commitData, forceCreating) {
       var grid = $('#UIPaiFilhaNeta_dGridVendaItem');
       if (started && typeof grid.data('igGridUpdating') === 'undefined') { started = false; }
       if (commitData && started) {
           if (grid.igGridUpdating('isEditing')) {
               grid.igGrid('commit');
       ...
       if (!started) {
           ...
           started = true;
           commitData = false;
           $('#UIPaiFilhaNeta_dGridVendaItem_groupbyarea').addClass('hide');
       }
       if (grid.igGridUpdating('isEditing')) {
            grid.igGridUpdating('endEdit', true);
       }
    ...
    function createDataGrid(grid) {
        ...
        vm.gridSaveStates[gridId] = {
            ...
            closePopover: function () {
                $('#UIPaiFilhaNeta_dGridVendaItem_LayoutBtn').igPopover('hide');
            },
            applyLayout: function (layoutInfo) {
                var _this = this;
                if (isNull(layoutInfo) && (!_this.currentLayout() || _this.currentLayout().Id === 0)) {
        ...
        grid.igGrid({ height: (getGridHeightSuggested() * 0.75), width: '100%',
            ...
            cellClick: function(evt, ui) {
                 ...
                 if (typeof vm.OnGridClientClick === 'function') {
                     entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                     vm.OnGridClientClick('UIPaiFilhaNeta_dGridVendaItem', ui.colKey, entity);
                 }
                 var grid = $('#UIPaiFilhaNeta_dGridVendaItem');
                 var isEditing = grid.igGridUpdating('isEditing');
                 if (!isEditing)
            ...
            features: [
                        { name: 'Sorting', type: 'local', caseSensitive: true,
                          columnSorting: function (evt, ui) { 
                              $.grep(ui.owner.grid._visibleColumnsArray, function (e) { 
                                  if (e.key === ui.columnKey && e.dataType === 'string') 
                                      return $('#UIPaiFilhaNeta_dGridVendaItem').igGridSorting('option', 'caseSensitive', false); 
                                  else if (e.key === ui.columnKey) 
                                      return $('#UIPaiFilhaNeta_dGridVendaItem').igGridSorting('option', 'caseSensitive', true); 
                              }); 
                          } 
            },
                        { name: 'Filtering', mode: 'advanced', filterDropDownItemIcons: false, filterDropDownWidth: 200, allowFiltering: true, type: 'local', renderFC: false, renderFilterButton: true, 
                       ...
                       ,{ name: 'Updating', horizontalMoveOnEnter: true,
                          ...
                          showReadonlyEditors: false,
                          showDoneCancelButtons: false,
                          columnSettings: [{ columnKey: 'DatetimeVendaItem', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid === 'function') {vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVendaItem', 'DatetimeVendaItem', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: 'ComboboxVendaItem', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('ComboboxVendaItem', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('LX_VENDA_ITEM', ''),  textKey: 'name', valueKey: 'id', enableClearButton: false }}, { columnKey: "BigIntVendaItem" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVendaItem', 'BigIntVendaItem', ui.oldValue, ui.value);}},  maxLength: 24, maxValue: null, minValue: 0, dataMode: 'long' } }, { columnKey: "DecimalVendaItem" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVendaItem', 'DecimalVendaItem', ui.oldValue, ui.value);}},  maxLength: 14, maxValue: 99999999999.99, minValue: 0, dataMode: 'decimal', minDecimals: 2, maxDecimals: 2 } }, { columnKey: "IdVenda" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVendaItem', 'IdVenda', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "IdVendaItem" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVendaItem', 'IdVendaItem', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, dataMode: 'int' } }, { columnKey: "IntVendaItem" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVendaItem', 'IntVendaItem', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "SmallIntVendaItem" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVendaItem', 'SmallIntVendaItem', ui.oldValue, ui.value);}},  maxLength: 6, maxValue: null, minValue: 0, dataMode: 'short' } }, { columnKey: "StringVendaItem" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIPaiFilhaNeta_dGridVendaItem', 'StringVendaItem', ui.oldValue, ui.value);}}, maxLength: 50 } }],
                          rowDeleting: function (evt, ui) {
                              deletedIndex = ui.element.context.rowIndex;
                              var entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowID);
                              if (entity) {
                          ...
                          editCellStarted: function(evt, ui){
                              var lstRefreshDados = null;
                              var columns = $('#UIPaiFilhaNeta_dGridVendaItem').igGridUpdating('option', 'columnSettings');
                              var currentCol = null;
                              currentRow = ui.rowID;
                              columns.forEach(function (entry, index) {
                                 if (entry.columnKey === ui.columnKey) currentCol = entry;
            ...
            if ((typeof vm.OnDataGridCreated === 'function')){
                vm.OnDataGridCreated('UIPaiFilhaNeta_dGridVendaItem');
            }
            var selectionrowselectionchanged = null, selectedRowId = -1;
            selectionrowselectionchanged = function (evt, ui) {
                if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                    if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                        $(document).undelegate('#UIPaiFilhaNeta_dGridVendaItem', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        ui.owner.clearSelection();
                        ui.owner.selectRow(ui.row.index);
                        selectedRowId = ui.row.id;
                        $(document).delegate ('#UIPaiFilhaNeta_dGridVendaItem', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                    }
                    selectGridCurrentItem(vm.goToKey, 'RowDataId', ui, vm.currentDataItem().currentVenda().currentVendaItem, getDataSource()); 
                 } 
            };
            $(document).delegate('#UIPaiFilhaNeta_dGridVendaItem', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
            $('#UIPaiFilhaNeta_dGridVendaItem > tbody tr').live('focus', function(evt) {
                var grid = $('#UIPaiFilhaNeta_dGridVendaItem'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                var selectedRows = grid.igGridSelection('option', 'multipleSelection') ? grid.igGridSelection('selectedRows') : [grid.igGridSelection('selectedRow')];
                if (selectedRowId === id) return;
                selectedRowId = id;
                grid.igGridSelection('selectRowById', id);
        ...
        vm.addDataSource({ key: 'UIPaiFilhaNeta_dGridVendaItem', name: 'VendaItemList', itemsSource: itemsSource });
    }
...
, renderscyUIPaiFilhaNeta_dGrid: function(vm) {
    ...
    $('#scyUIPaiFilhaNeta_dGrid_headers').live('focus  keydown', function (evt) {
        ...
        if (keyCode === 9) {
            ...
            clear = vm.status() === 'C';
            if (vm.status() === 'C')
                $('#scyUIPaiFilhaNeta_dGrid').igGridUpdating('startEdit', firstRow, 0);
            else {
                var entity = findElementByKey(getDataSource(), 'RowDataId', firstRow);
                var indexColumn = 0;
                cols.some(function (entry) {
                    if (entry.key !== 'RowDataId' && !entry.hidden) {
                        if (verifyCanEditCol(entry.key, clear, entity)) {
                            $('#scyUIPaiFilhaNeta_dGrid').igGridUpdating('startEdit', firstRow, indexColumn);
                            return true;
                        }
                        indexColumn++;
                    }
    ...
    function createDataGrid(grid) {
        ...
        grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
            ...
            columns: [
                ...
                { key: 'BigIntCliente', headerText: 'Big Int Cliente', width: '250px', dataType: 'number', columnCssClass: 'ellipsis', hidden: false, formatter: function(val) { return (val == null ? '' : val.toString()).mask('##.###.###/####-##') } },
                ...
                { key: 'StringPais', headerText: 'String Pais', width: '421px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                { key: 'StringEstado', headerText: 'String Estado', width: '421px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
            ...
            features: [
                        { name: 'Sorting', type: 'local', caseSensitive: true,
                          columnSorting: function (evt, ui) { 
                              $.grep(ui.owner.grid._visibleColumnsArray, function (e) { 
                                  if (e.key === ui.columnKey && e.dataType === 'string') 
                                      return $('#scyUIPaiFilhaNeta_dGrid').igGridSorting('option', 'caseSensitive', false); 
                                  else if (e.key === ui.columnKey) 
                                      return $('#scyUIPaiFilhaNeta_dGrid').igGridSorting('option', 'caseSensitive', true); 
                              }); 
                          } 
                        ...
                        { name: 'Selection', mode: 'row'},
                        { name: 'Tooltips', columnSettings:[{ columnKey: "StringCliente", allowTooltips: true },{ columnKey: "BigIntCliente", allowTooltips: true },{ columnKey: "ComboboxCliente", allowTooltips: true },{ columnKey: "IdEstado", allowTooltips: false },{ columnKey: "IdPais", allowTooltips: false },{ columnKey: "IdCliente", allowTooltips: true },{ columnKey: "DatetimeCliente", allowTooltips: true },{ columnKey: "DecimalCliente", allowTooltips: true },{ columnKey: "BitCliente", allowTooltips: true },{ columnKey: "IntCliente", allowTooltips: true },{ columnKey: "StringPais", allowTooltips: false },{ columnKey: "StringEstado", allowTooltips: false },{ columnKey: "SmallIntCliente", allowTooltips: true }] },
                        { name: 'Resizing' }, 
                        { name: 'Hiding', 
                            columnHidden: function (evt, ui) {
                               showMultimidiaLazy('#scyUIPaiFilhaNeta_dGrid');
                            },
                            columnShown: function (evt, ui) {
                               showMultimidiaLazy('#scyUIPaiFilhaNeta_dGrid');
                            }
                       ...
                       ,{ name: 'Updating', horizontalMoveOnEnter: true,
                          ...
                          showReadonlyEditors: false,
                          showDoneCancelButtons: false,
                          columnSettings: [{ columnKey: "IdEstado", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpEstado", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, validateOnClearState:false, maxValue:2147483647, maxLength: 12, defaultValue: null } }, { columnKey: "IdPais", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpEstado", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, validateOnClearState:false, maxValue:2147483647, maxLength: 12, defaultValue: null } }, { columnKey: "StringPais", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpEstado", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, validateOnClearState:false, maxValue:null, maxLength: 50, defaultValue: '' } }, { columnKey: "StringEstado", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpEstado", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, validateOnClearState:false, maxValue:null, maxLength: 50, defaultValue: '' } }, { columnKey: 'DatetimeCliente', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid === 'function') {vm.OnPropertyChangeDataGrid('scyUIPaiFilhaNeta_dGrid', 'DatetimeCliente', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: 'ComboboxCliente', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('ComboboxCliente', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('LX_CLIENTE', ''),  textKey: 'name', valueKey: 'id', enableClearButton: false }}, { columnKey: "StringCliente" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIPaiFilhaNeta_dGrid', 'StringCliente', ui.oldValue, ui.value);}}, maxLength: 50 } }, { columnKey: "BigIntCliente", editorOptions: { type: "mask", inputMask: "00.000.000/0000-00", dataMode: 0 } }, { columnKey: "IdCliente" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIPaiFilhaNeta_dGrid', 'IdCliente', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, dataMode: 'int' } }, { columnKey: "DecimalCliente" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIPaiFilhaNeta_dGrid', 'DecimalCliente', ui.oldValue, ui.value);}},  maxLength: 14, maxValue: 99999999999.99, minValue: 0, dataMode: 'decimal', minDecimals: 2, maxDecimals: 2 } }, { columnKey: "IntCliente" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIPaiFilhaNeta_dGrid', 'IntCliente', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "SmallIntCliente" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIPaiFilhaNeta_dGrid', 'SmallIntCliente', ui.oldValue, ui.value);}},  maxLength: 6, maxValue: null, minValue: 0, dataMode: 'short' } }],
                          rowDeleting: function (evt, ui) {
                              deletedIndex = ui.element.context.rowIndex;
                              var entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowID);
                              if (entity) {
