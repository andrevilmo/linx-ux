namespace VAREJO.BV.Reports {
    ...
    using VAREJO.BV.OlapGraficosGauge;
    using System.Net.Http;
    using System.Net.Http.Headers;
    
    
    public partial class OlapGraficosGaugeDataSource {
        
        private string url = "VarejoOlapGraficosGauge/";
        
        public VAREJO.BV.OlapGraficosGauge.OlapGraficosGaugeDomainService _olapGraficosGaugeContext;
        ...
        private IEnumerable<Venda> GetLocalVenda(Telerik.Reporting.Processing.Report report) {
            ...
            if (_olapGraficosGaugeContext == null) _olapGraficosGaugeContext = new VAREJO.BV.OlapGraficosGauge.OlapGraficosGaugeDomainService(headers) { IsSecure = true };
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._olapGraficosGaugeContext.GetVendaByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._olapGraficosGaugeContext.GetVendaByEntitySearchNoAssociations(null).ToList();
            if (this.DetailsForLoading != null && this.DetailsForLoading.Length > 0)
            {
               foreach (var entity in result)
               {
                   entity.FillDetails(this._olapGraficosGaugeContext, null, null, this.DetailsForLoading);
               }
            }
            return result;
        }
        ...
        private IEnumerable<VendaItem> GetLocalVendaItem(Telerik.Reporting.Processing.Report report) {
            ...
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_olapGraficosGaugeContext == null) _olapGraficosGaugeContext = new VAREJO.BV.OlapGraficosGauge.OlapGraficosGaugeDomainService(headers) { IsSecure = true };
            result = this._olapGraficosGaugeContext.GetVendaItemByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<VendaItemParentComposition> GetLocalVendaItemParentComposition(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._olapGraficosGaugeContext.GetVendaItemParentCompositionByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._olapGraficosGaugeContext.GetVendaItemParentCompositionByEntitySearchNoAssociations(null).ToList();
            return result;
        }
