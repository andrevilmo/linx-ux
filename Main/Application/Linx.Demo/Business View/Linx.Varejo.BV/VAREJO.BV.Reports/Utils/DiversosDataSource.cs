namespace VAREJO.BV.Reports {
    ...
    using VAREJO.BV.Diversos;
    using System.Net.Http;
    using System.Net.Http.Headers;
    
    
    public partial class DiversosDataSource {
        
        private string url = "VarejoDiversos/";
        
        public VAREJO.BV.Diversos.DiversosDomainService _diversosContext;
        ...
        private IEnumerable<Venda> GetLocalVenda(Telerik.Reporting.Processing.Report report) {
            ...
            if (_diversosContext == null) _diversosContext = new VAREJO.BV.Diversos.DiversosDomainService(headers) { IsSecure = true };
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._diversosContext.GetVendaByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._diversosContext.GetVendaByEntitySearchNoAssociations(null).ToList();
            if (this.DetailsForLoading != null && this.DetailsForLoading.Length > 0)
            {
               foreach (var entity in result)
               {
                   entity.FillDetails(this._diversosContext, null, null, this.DetailsForLoading);
               }
            }
            return result;
        }
        ...
        private IEnumerable<VendaItem> GetLocalVendaItem(Telerik.Reporting.Processing.Report report) {
            ...
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_diversosContext == null) _diversosContext = new VAREJO.BV.Diversos.DiversosDomainService(headers) { IsSecure = true };
            result = this._diversosContext.GetVendaItemByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<VendaItemParentComposition> GetLocalVendaItemParentComposition(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._diversosContext.GetVendaItemParentCompositionByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._diversosContext.GetVendaItemParentCompositionByEntitySearchNoAssociations(null).ToList();
            return result;
        }
