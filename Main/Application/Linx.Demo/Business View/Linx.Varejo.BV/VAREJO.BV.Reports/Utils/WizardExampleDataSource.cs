namespace VAREJO.BV.Reports {
    ...
    using VAREJO.BV.WizardExample;
    using System.Net.Http;
    using System.Net.Http.Headers;
    
    
    public partial class WizardExampleDataSource {
        
        private string url = "VarejoWizardExample/";
        
        public VAREJO.BV.WizardExample.WizardExampleDomainService _wizardExampleContext;
        ...
        private IEnumerable<Cliente> GetLocalCliente(Telerik.Reporting.Processing.Report report) {
            ...
            if (_wizardExampleContext == null) _wizardExampleContext = new VAREJO.BV.WizardExample.WizardExampleDomainService(headers) { IsSecure = true };
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._wizardExampleContext.GetClienteByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._wizardExampleContext.GetClienteByEntitySearchNoAssociations(null).ToList();
            if (this.DetailsForLoading != null && this.DetailsForLoading.Length > 0)
            {
               foreach (var entity in result)
               {
                   entity.FillDetails(this._wizardExampleContext, null, null, this.DetailsForLoading);
               }
            }
            return result;
        }
        ...
        private IEnumerable<Venda> GetLocalVenda(Telerik.Reporting.Processing.Report report) {
            ...
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_wizardExampleContext == null) _wizardExampleContext = new VAREJO.BV.WizardExample.WizardExampleDomainService(headers) { IsSecure = true };
            result = this._wizardExampleContext.GetVendaByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<VendaParentComposition> GetLocalVendaParentComposition(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._wizardExampleContext.GetVendaParentCompositionByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._wizardExampleContext.GetVendaParentCompositionByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<VendaAtacado> GetLocalVendaAtacado(Telerik.Reporting.Processing.Report report) {
            ...
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_wizardExampleContext == null) _wizardExampleContext = new VAREJO.BV.WizardExample.WizardExampleDomainService(headers) { IsSecure = true };
            result = this._wizardExampleContext.GetVendaAtacadoByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<VendaAtacadoParentComposition> GetLocalVendaAtacadoParentComposition(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._wizardExampleContext.GetVendaAtacadoParentCompositionByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._wizardExampleContext.GetVendaAtacadoParentCompositionByEntitySearchNoAssociations(null).ToList();
            return result;
        }
