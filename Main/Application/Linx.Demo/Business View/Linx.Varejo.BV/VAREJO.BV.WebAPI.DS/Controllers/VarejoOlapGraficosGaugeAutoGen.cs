...
namespace VAREJO.BV.WebAPI.DS.Controllers
{
    ...
    //Examples:
    // Default Call: http://localhost:1710/VarejoOlapGraficosGauge/[ActionName]
    // Security Information Call: http://localhost:1710/VarejoOlapGraficosGauge/GetSecurityInfo
    // Entities Catalog Call: http://localhost:1710/VarejoOlapGraficosGauge/GetEntities
    // Entity MetaData Call: http://localhost:1710/VarejoOlapGraficosGauge/GetMetaData?entityName=[EntityName]&allComposition=false
    // Client Domains Call: http://localhost:1710/VarejoOlapGraficosGauge/GetClientDomains?erp=true
    // Client Service Call: http://localhost:1710/VarejoOlapGraficosGauge/GetClientService?erp=true
    // Client Factory Call: http://localhost:1710/VarejoOlapGraficosGauge/GetClientFactory?entityName=[EntityName]&erp=true
    // Client Factory Custom Events Call: http://localhost:1710/VarejoOlapGraficosGauge/GetClientFactoryCustomEvents?entityName=[EntityName]&erp=true
    // Help Call: http://localhost:1710/HelpController/VarejoOlapGraficosGauge
    // Feed OData Call: http://localhost:1710/VarejoOlapGraficosGaugeOData
    [RoutePrefix("VarejoOlapGraficosGauge")]
    [Breeze.WebApi2.BreezeController]
    [ODataBasicAuthenticationFilter]
    public partial class VarejoOlapGraficosGaugeController : ApiController
    {
        private const int maxObjectExcelReturned = 2300000;
        private DataServiceRepository<BusinessNS.OlapGraficosGaugeDomainService> _repository = null;
        private DataServiceRepository<BusinessNS.OlapGraficosGaugeDomainService> repository { get {  if (_repository == null) { _repository = new DataServiceRepository<BusinessNS.OlapGraficosGaugeDomainService>(typeof(BusinessNS.Venda), typeof(BusinessNS.VendaItem)); _repository.Context.IsSecure = true; } return _repository; } }
        public VarejoOlapGraficosGaugeController()
        { }
        ...
        public object AssemblyInfo()
        {
            return new
            {
                ApiAssemblyName = typeof(VarejoOlapGraficosGaugeController).Assembly.FullName,
                BusinessAssemblyName = typeof(BusinessNS.OlapGraficosGaugeDomainService).Assembly.FullName,
        ...
        {
            var result = repository.Context.GetMetaDataObject("VAREJO.BV.OlapGraficosGauge." + entityName, false, true);
        ...
        {
           return new string[] { "VAREJO.BV", "VarejoOlapGraficosGauge", "VarejoOlapGraficosGauge/ActionName" };
        }
        ...
        [VarejoOlapGraficosGaugeControllerAuthorize]
        [Route("GetGaugesFormularioRanges"), System.Web.Http.HttpGet()]
        public IEnumerable<KpiRangeItem> GetGaugesFormularioRanges()
        {
            var kpi = new VAREJO.BV.KPIs.GaugesFormulario();
        ...
        [VarejoOlapGraficosGaugeControllerAuthorize]
        [Route("GetKPIGridRanges"), System.Web.Http.HttpGet()]
        public IEnumerable<KpiRangeItem> GetKPIGridRanges()
        {
            var kpi = new VAREJO.BV.KPIs.KPIGrid();
            Linx.Business.Tools.KpiManager.UpdateKpiInfo(kpi);
            return kpi.Ranges.Values.ToArray();
        }
        ...
        public string GetVendaToExcel(string[] parameters)
        {
            ...
            var metadata = repository.Context.GetMetaDataObject("VAREJO.BV.OlapGraficosGauge.Venda");
            if (columns.Count > 0)
            {
        ...
        public string GetVendaToReportXml(string[] parameters)
        {
            ...
            var metadata = repository.Context.GetMetaDataObject("VAREJO.BV.OlapGraficosGauge.Venda", true);
            if (columns.Count > 0)
            {
                foreach (var item in metadata[0].Properties)
                {
            ...
            var zip = new LinxZip();
            zip.AddStringContent(reportName + ".trdx", ReportExport.CreateXmlReport(new ReportExport.EntitiesToExport{ ReportName = reportName, Metadata = metadata.First(), DataSourceAssembly = "VAREJO.BV.Reports", DataSourceFullName = "VAREJO.BV.Reports.OlapGraficosGaugeDataSource", DataSourceObject = "GetVenda", JExpressionTranslated = translatedJEntitySearch, JQueryExpression = jEntitySearch, ServiceBusUrl = serviceBusUrl, HasMedia = exportMedia }));
            zip.AddFile(System.Web.HttpContext.Current.Server.MapPath("~/bin/VAREJO.BV.Reports.dll"));
            zip.AddFile(System.Web.HttpContext.Current.Server.MapPath("~/bin/VAREJO.BV.dll"));
            return Convert.ToBase64String(zip.GetZipBytes());
        }
        ...
        public string GetVendaItemToExcel(string[] parameters)
        {
            ...
            var metadata = repository.Context.GetMetaDataObject("VAREJO.BV.OlapGraficosGauge.VendaItem");
            if (columns.Count > 0)
            {
                foreach (var item in metadata[0].Properties)
                {
        ...
        public string GetVendaItemToReportXml(string[] parameters)
        {
            ...
            var metadata = repository.Context.GetMetaDataObject("VAREJO.BV.OlapGraficosGauge.VendaItem", true);
            if (columns.Count > 0)
            {
                foreach (var item in metadata[0].Properties)
                {
            ...
            var zip = new LinxZip();
            zip.AddStringContent(reportName + ".trdx", ReportExport.CreateXmlReport(new ReportExport.EntitiesToExport{ ReportName = reportName, Metadata = metadata.First(), DataSourceAssembly = "VAREJO.BV.Reports", DataSourceFullName = "VAREJO.BV.Reports.OlapGraficosGaugeDataSource", DataSourceObject = "GetVendaItem", JExpressionTranslated = translatedJEntitySearch, JQueryExpression = jEntitySearch, ServiceBusUrl = serviceBusUrl, HasMedia = exportMedia }));
            zip.AddFile(System.Web.HttpContext.Current.Server.MapPath("~/bin/VAREJO.BV.Reports.dll"));
            zip.AddFile(System.Web.HttpContext.Current.Server.MapPath("~/bin/VAREJO.BV.dll"));
            return Convert.ToBase64String(zip.GetZipBytes());
        }
        ...
        public string GetVendaItemParentCompositionToReportXml(string[] parameters)
        {
            ...
            var zip = new LinxZip();
            zip.AddStringContent(reportName + ".trdx", ReportExport.CreateXmlReport(new ReportExport.EntitiesToExport{ ReportName = reportName, Metadata = metadata.First(), DataSourceAssembly = "VAREJO.BV.Reports", DataSourceFullName = "VAREJO.BV.Reports.OlapGraficosGaugeDataSource", DataSourceObject = "GetVendaItemParentComposition", JExpressionTranslated = translatedJEntitySearch, JQueryExpression = jEntitySearch, ServiceBusUrl = serviceBusUrl, HasMedia = exportMedia }));
            zip.AddFile(System.Web.HttpContext.Current.Server.MapPath("~/bin/VAREJO.BV.Reports.dll"));
            zip.AddFile(System.Web.HttpContext.Current.Server.MapPath("~/bin/VAREJO.BV.dll"));
            return Convert.ToBase64String(zip.GetZipBytes());
        }
    ...
    [ODataBasicAuthenticationFilter]
    public partial class VarejoOlapGraficosGaugeFeedController : ODataController
    {
        private BusinessNS.OlapGraficosGaugeDomainService _context;
        public BusinessNS.OlapGraficosGaugeDomainService Context { get {  if (_context == null) { _context = new BusinessNS.OlapGraficosGaugeDomainService(); _context.IsSecure = true; } return _context; }  }
    ...
    public partial class VarejoOlapGraficosGaugeControllerAuthorizeAttribute : System.Web.Http.AuthorizeAttribute
    {
        protected override bool IsAuthorized(System.Web.Http.Controllers.HttpActionContext actionContext)
        {
            return LinxAutorization.CheckAuthorization(actionContext, string.Format("{0}#{1}#{1}/{2}", "VAREJO.BV", "VarejoOlapGraficosGauge", actionContext.ActionDescriptor.ActionName));
        }
    }
}
