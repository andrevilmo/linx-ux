namespace Linx.Demo.BV.Reports {
    ...
    using Linx.Demo.BV.UIExterna;
    using System.Net.Http;
    using System.Net.Http.Headers;
    
    
    public partial class UIExternaDataSource {
        
        private string url = "LinxDemoUIExterna/";
        
        public Linx.Demo.BV.UIExterna.UIExternaDomainService _uIExternaContext;
        ...
        private IEnumerable<Cliente> GetLocalCliente(Telerik.Reporting.Processing.Report report) {
            ...
            if (_uIExternaContext == null) _uIExternaContext = new Linx.Demo.BV.UIExterna.UIExternaDomainService(headers) { IsSecure = true };
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._uIExternaContext.GetClienteByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._uIExternaContext.GetClienteByEntitySearchNoAssociations(null).ToList();
            return result;
        }
