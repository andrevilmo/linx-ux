namespace Linx.Demo.BV.Reports {
    ...
    using Linx.Demo.BV.Recuros;
    using System.Net.Http;
    using System.Net.Http.Headers;
    
    
    public partial class RecurosDataSource {
        
        private string url = "LinxDemoRecuros/";
        
        public Linx.Demo.BV.Recuros.RecurosDomainService _recurosContext;
        ...
        private IEnumerable<Cliente> GetLocalCliente(Telerik.Reporting.Processing.Report report) {
            ...
            if (_recurosContext == null) _recurosContext = new Linx.Demo.BV.Recuros.RecurosDomainService(headers) { IsSecure = true };
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._recurosContext.GetClienteByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._recurosContext.GetClienteByEntitySearchNoAssociations(null).ToList();
            if (this.DetailsForLoading != null && this.DetailsForLoading.Length > 0)
            {
               foreach (var entity in result)
               {
                   entity.FillDetails(this._recurosContext, null, null, this.DetailsForLoading);
               }
            }
            return result;
        }
        ...
        private IEnumerable<Venda> GetLocalVenda(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._recurosContext.GetVendaByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._recurosContext.GetVendaByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<VendaParentComposition> GetLocalVendaParentComposition(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._recurosContext.GetVendaParentCompositionByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._recurosContext.GetVendaParentCompositionByEntitySearchNoAssociations(null).ToList();
            return result;
        }
