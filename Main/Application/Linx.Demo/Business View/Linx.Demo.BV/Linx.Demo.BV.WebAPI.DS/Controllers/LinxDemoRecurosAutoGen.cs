...
using BusinessNS = Linx.Demo.BV.Recuros;

namespace Linx.Demo.BV.WebAPI.DS.Controllers
{
    
    //Examples:
    // Default Call: http://localhost:1710/LinxDemoRecuros/[ActionName]
    // Security Information Call: http://localhost:1710/LinxDemoRecuros/GetSecurityInfo
    // Entities Catalog Call: http://localhost:1710/LinxDemoRecuros/GetEntities
    // Entity MetaData Call: http://localhost:1710/LinxDemoRecuros/GetMetaData?entityName=[EntityName]&allComposition=false
    // Client Domains Call: http://localhost:1710/LinxDemoRecuros/GetClientDomains?erp=true
    // Client Service Call: http://localhost:1710/LinxDemoRecuros/GetClientService?erp=true
    // Client Factory Call: http://localhost:1710/LinxDemoRecuros/GetClientFactory?entityName=[EntityName]&erp=true
    // Client Factory Custom Events Call: http://localhost:1710/LinxDemoRecuros/GetClientFactoryCustomEvents?entityName=[EntityName]&erp=true
    // Help Call: http://localhost:1710/HelpController/LinxDemoRecuros
    // Feed OData Call: http://localhost:1710/LinxDemoRecurosOData
    [RoutePrefix("LinxDemoRecuros")]
    [Breeze.WebApi2.BreezeController]
    [ODataBasicAuthenticationFilter]
    public partial class LinxDemoRecurosController : ApiController
    {
        private const int maxObjectExcelReturned = 2300000;
        private DataServiceRepository<BusinessNS.RecurosDomainService> _repository = null;
        private DataServiceRepository<BusinessNS.RecurosDomainService> repository { get {  if (_repository == null) { _repository = new DataServiceRepository<BusinessNS.RecurosDomainService>(typeof(BusinessNS.Cliente), typeof(BusinessNS.Venda)); _repository.Context.IsSecure = true; } return _repository; } }
        public LinxDemoRecurosController()
        { }
        ...
        public object AssemblyInfo()
        {
            return new
            {
                ApiAssemblyName = typeof(LinxDemoRecurosController).Assembly.FullName,
                BusinessAssemblyName = typeof(BusinessNS.RecurosDomainService).Assembly.FullName,
        ...
        {
            var result = repository.Context.GetMetaDataObject("Linx.Demo.BV.Recuros." + entityName, false, true);
        ...
        {
           return new string[] { "Linx.Demo.BV", "LinxDemoRecuros", "LinxDemoRecuros/ActionName" };
        }
        ...
        [LinxDemoRecurosControllerAuthorize]
        ...
        public string GetClienteToExcel(string[] parameters)
        {
            ...
            var metadata = repository.Context.GetMetaDataObject("Linx.Demo.BV.Recuros.Cliente");
            if (columns.Count > 0)
            {
        ...
        public string GetClienteToReportXml(string[] parameters)
        {
            ...
            var metadata = repository.Context.GetMetaDataObject("Linx.Demo.BV.Recuros.Cliente", true);
            if (columns.Count > 0)
            {
                foreach (var item in metadata[0].Properties)
                {
            ...
            var zip = new LinxZip();
            zip.AddStringContent(reportName + ".trdx", ReportExport.CreateXmlReport(new ReportExport.EntitiesToExport{ ReportName = reportName, Metadata = metadata.First(), DataSourceAssembly = "Linx.Demo.BV.Reports", DataSourceFullName = "Linx.Demo.BV.Reports.RecurosDataSource", DataSourceObject = "GetCliente", JExpressionTranslated = translatedJEntitySearch, JQueryExpression = jEntitySearch, ServiceBusUrl = serviceBusUrl, HasMedia = exportMedia }));
            zip.AddFile(System.Web.HttpContext.Current.Server.MapPath("~/bin/Linx.Demo.BV.Reports.dll"));
            zip.AddFile(System.Web.HttpContext.Current.Server.MapPath("~/bin/Linx.Demo.BV.dll"));
            return Convert.ToBase64String(zip.GetZipBytes());
        }
        ...
        public string GetVendaToExcel(string[] parameters)
        {
            ...
            var metadata = repository.Context.GetMetaDataObject("Linx.Demo.BV.Recuros.Venda");
            if (columns.Count > 0)
            {
                foreach (var item in metadata[0].Properties)
                {
        ...
        public string GetVendaToReportXml(string[] parameters)
        {
            ...
            var metadata = repository.Context.GetMetaDataObject("Linx.Demo.BV.Recuros.Venda", true);
            if (columns.Count > 0)
            {
                foreach (var item in metadata[0].Properties)
                {
            ...
            var zip = new LinxZip();
            zip.AddStringContent(reportName + ".trdx", ReportExport.CreateXmlReport(new ReportExport.EntitiesToExport{ ReportName = reportName, Metadata = metadata.First(), DataSourceAssembly = "Linx.Demo.BV.Reports", DataSourceFullName = "Linx.Demo.BV.Reports.RecurosDataSource", DataSourceObject = "GetVenda", JExpressionTranslated = translatedJEntitySearch, JQueryExpression = jEntitySearch, ServiceBusUrl = serviceBusUrl, HasMedia = exportMedia }));
            zip.AddFile(System.Web.HttpContext.Current.Server.MapPath("~/bin/Linx.Demo.BV.Reports.dll"));
            zip.AddFile(System.Web.HttpContext.Current.Server.MapPath("~/bin/Linx.Demo.BV.dll"));
            return Convert.ToBase64String(zip.GetZipBytes());
        }
        ...
        public string GetVendaParentCompositionToReportXml(string[] parameters)
        {
            ...
            var zip = new LinxZip();
            zip.AddStringContent(reportName + ".trdx", ReportExport.CreateXmlReport(new ReportExport.EntitiesToExport{ ReportName = reportName, Metadata = metadata.First(), DataSourceAssembly = "Linx.Demo.BV.Reports", DataSourceFullName = "Linx.Demo.BV.Reports.RecurosDataSource", DataSourceObject = "GetVendaParentComposition", JExpressionTranslated = translatedJEntitySearch, JQueryExpression = jEntitySearch, ServiceBusUrl = serviceBusUrl, HasMedia = exportMedia }));
            zip.AddFile(System.Web.HttpContext.Current.Server.MapPath("~/bin/Linx.Demo.BV.Reports.dll"));
            zip.AddFile(System.Web.HttpContext.Current.Server.MapPath("~/bin/Linx.Demo.BV.dll"));
            return Convert.ToBase64String(zip.GetZipBytes());
        }
    ...
    [ODataBasicAuthenticationFilter]
    public partial class LinxDemoRecurosFeedController : ODataController
    {
        private BusinessNS.RecurosDomainService _context;
        public BusinessNS.RecurosDomainService Context { get {  if (_context == null) { _context = new BusinessNS.RecurosDomainService(); _context.IsSecure = true; } return _context; }  }
    ...
    public partial class LinxDemoRecurosControllerAuthorizeAttribute : System.Web.Http.AuthorizeAttribute
    {
        protected override bool IsAuthorized(System.Web.Http.Controllers.HttpActionContext actionContext)
        {
            return LinxAutorization.CheckAuthorization(actionContext, string.Format("{0}#{1}#{1}/{2}", "Linx.Demo.BV", "LinxDemoRecuros", actionContext.ActionDescriptor.ActionName));
        }
    }
}
