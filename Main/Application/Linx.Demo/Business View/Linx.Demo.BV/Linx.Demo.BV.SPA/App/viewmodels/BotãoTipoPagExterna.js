...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'BotãoTipoPagExterna', Items: [
{Name: 'BotãoTipoPagExterna_gbFormaPagamento', DisplayName: 'FormaPagamento', ColumnSpan: 12, Visible: true, Items: [{Name: 'BotãoTipoPagExterna_ntxBigIntFormaPagamento', DisplayName: 'Big Int Forma Pagamento', ColumnSpan: 8, Visible: true},{Name: 'BotãoTipoPagExterna_ckBitFormaPagamento', DisplayName: 'Bit Forma Pagamento', ColumnSpan: 6, Visible: true},{Name: 'BotãoTipoPagExterna_cmbComboboxFormaPagamento', DisplayName: 'Combobox Forma Pagamento', ColumnSpan: 6, Visible: true},{Name: 'BotãoTipoPagExterna_dtDatetimeFormaPagamento', DisplayName: 'Datetime Forma Pagamento', ColumnSpan: 3, Visible: true},{Name: 'BotãoTipoPagExterna_ntxDecimalFormaPagamento', DisplayName: 'Decimal Forma Pagamento', ColumnSpan: 5, Visible: true},{Name: 'BotãoTipoPagExterna_ntxIdFormaPagamento', DisplayName: 'Id Forma Pagamento', ColumnSpan: 5, Visible: true},{Name: 'BotãoTipoPagExterna_lUpIdVenda', DisplayName: 'Id Venda', ColumnSpan: 5, Visible: true},{Name: 'BotãoTipoPagExterna_ntxIntFormaPagamento', DisplayName: 'Int Forma Pagamento', ColumnSpan: 5, Visible: true},{Name: 'BotãoTipoPagExterna_ntxSmallIntFormaPagamento', DisplayName: 'Small Int Forma Pagamento', ColumnSpan: 2, Visible: true},{Name: 'BotãoTipoPagExterna_tbStringFormaPagamento', DisplayName: 'String Forma Pagamento', ColumnSpan: 9, Visible: true},]},       ]};
    };
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'BotãoTipoPagExterna' ]; settings.parentVM.BotãoTipoPagExterna = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.BotãoTipoPagExterna = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('BotãoTipoPagExterna', parentVM.internalUIs) === -1) parentVM.internalUIs.push('BotãoTipoPagExterna');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-demo-bv-spa-BotãoTipoPagExterna', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: BotãoTipoPagExterna', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
                   $('#BotãoTipoPagExterna_gbFormaPagamento').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

                   complement.renderscyBotãoTipoPagExterna_dGrid(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'FormaPagamento';
    ...
    var visibleColumns = 'BigIntFormaPagamento,BitFormaPagamento,ComboboxFormaPagamento,DatetimeFormaPagamento,DecimalFormaPagamento,IdFormaPagamento,IdVenda,IntFormaPagamento,SmallIntFormaPagamento,StringFormaPagamento';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'FormaPagamento', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'FormaPagamento', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('FormaPagamento');
        ...
        return dataContext.getFormaPagamentoByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), true, sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'FormaPagamento'); }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getFormaPagamentoByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'FormaPagamento'); } }
            dataView(data.results);
            currentPage(curPage);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearFormaPagamento(getBandeiraRede(), clearComplete);
        ...
        function saveSucceeded(saveResult) {
            ...
            for (var idxElem = toList.length - 1; idxElem >= 0; idxElem--) {
                    if (toList[idxElem].ChangeState !== 'N') {
                               var fromObj = _.where(fromList, { IdFormaPagamento: toList[idxElem]['IdFormaPagamento'] });
    ...
    var createFormaPagamento = function() {
        dataBind('dataView', true);
        var entity = dataContext.createFormaPagamento();
    ...
    var createAndNotifyFormaPagamento = function() {
        var entity = createFormaPagamento();
        notifyPresentation('');
        return entity;
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#BotãoTipoPagExterna_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#BotãoTipoPagExterna_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#BotãoTipoPagExterna_formViewer')[0];
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            FormaPagamentoDatetimeFormaPagamento_typeRange: ko.observable('R'), FormaPagamentoDatetimeFormaPagamento_begin: ko.observable(null), FormaPagamentoDatetimeFormaPagamento_end: ko.observable(null), FormaPagamentoDatetimeFormaPagamento_predefFilter: ko.observableArray([]), FormaPagamentoDatetimeFormaPagamento_predefValue: ko.observable(null),
        FormaPagamentoBigIntFormaPagamento_begin: ko.observable(null), FormaPagamentoBigIntFormaPagamento_end: ko.observable(null),
        FormaPagamentoDecimalFormaPagamento_begin: ko.observable(null), FormaPagamentoDecimalFormaPagamento_end: ko.observable(null),
        FormaPagamentoIdFormaPagamento_begin: ko.observable(null), FormaPagamentoIdFormaPagamento_end: ko.observable(null),
        FormaPagamentoIntFormaPagamento_begin: ko.observable(null), FormaPagamentoIntFormaPagamento_end: ko.observable(null),
        FormaPagamentoSmallIntFormaPagamento_begin: ko.observable(null), FormaPagamentoSmallIntFormaPagamento_end: ko.observable(null),
        FormaPagamentoIdVenda: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.FormaPagamentoDatetimeFormaPagamento_typeRange('R'); entitySearchRange.FormaPagamentoDatetimeFormaPagamento_begin(null); entitySearchRange.FormaPagamentoDatetimeFormaPagamento_end(null); entitySearchRange.FormaPagamentoDatetimeFormaPagamento_predefFilter([]); entitySearchRange.FormaPagamentoDatetimeFormaPagamento_predefValue(null);
        entitySearchRange.FormaPagamentoBigIntFormaPagamento_begin(null); entitySearchRange.FormaPagamentoBigIntFormaPagamento_end(null);
        entitySearchRange.FormaPagamentoDecimalFormaPagamento_begin(null); entitySearchRange.FormaPagamentoDecimalFormaPagamento_end(null);
        entitySearchRange.FormaPagamentoIdFormaPagamento_begin(null); entitySearchRange.FormaPagamentoIdFormaPagamento_end(null);
        entitySearchRange.FormaPagamentoIntFormaPagamento_begin(null); entitySearchRange.FormaPagamentoIntFormaPagamento_end(null);
        entitySearchRange.FormaPagamentoSmallIntFormaPagamento_begin(null); entitySearchRange.FormaPagamentoSmallIntFormaPagamento_end(null);
        entitySearchRange.FormaPagamentoIdVenda(null);
    };
    entitySearchRange.has_FormaPagamentoBigIntFormaPagamento = ko.computed(function(){ return (entitySearchRange.FormaPagamentoBigIntFormaPagamento_begin() != null || entitySearchRange.FormaPagamentoBigIntFormaPagamento_end() != null); });
    entitySearchRange.has_FormaPagamentoDecimalFormaPagamento = ko.computed(function(){ return (entitySearchRange.FormaPagamentoDecimalFormaPagamento_begin() != null || entitySearchRange.FormaPagamentoDecimalFormaPagamento_end() != null); });
    entitySearchRange.has_FormaPagamentoIdFormaPagamento = ko.computed(function(){ return (entitySearchRange.FormaPagamentoIdFormaPagamento_begin() != null || entitySearchRange.FormaPagamentoIdFormaPagamento_end() != null); });
    entitySearchRange.has_FormaPagamentoIntFormaPagamento = ko.computed(function(){ return (entitySearchRange.FormaPagamentoIntFormaPagamento_begin() != null || entitySearchRange.FormaPagamentoIntFormaPagamento_end() != null); });
    entitySearchRange.has_FormaPagamentoSmallIntFormaPagamento = ko.computed(function(){ return (entitySearchRange.FormaPagamentoSmallIntFormaPagamento_begin() != null || entitySearchRange.FormaPagamentoSmallIntFormaPagamento_end() != null); });
    entitySearchRange.has_FormaPagamentoDatetimeFormaPagamento = ko.computed(function(){ return (entitySearchRange.FormaPagamentoDatetimeFormaPagamento_typeRange() == 'R' && (entitySearchRange.FormaPagamentoDatetimeFormaPagamento_begin() != null || entitySearchRange.FormaPagamentoDatetimeFormaPagamento_end() != null) || (entitySearchRange.FormaPagamentoDatetimeFormaPagamento_typeRange() == 'P' && entitySearchRange.FormaPagamentoDatetimeFormaPagamento_predefFilter().length > 0)); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#BotãoTipoPagExterna_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: 'FORMA_PAGAMENTO',
            rootDataTypeName: 'FormaPagamento',
            ...
            createAndNotifyFormaPagamento: createAndNotifyFormaPagamento,
            ...
            __moduleId__: 'pkg_linx-demo-bv-spa/viewmodels/BotãoTipoPagExterna'
        };
