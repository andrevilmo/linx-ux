...
var complementCtor = function() {
    var complement = {
    isAutomatic: true
                   , ChangedBrandscyUILookUpDentroOutraUI_dGrid: function(vm, decimals, reset) {
                   var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#scyUILookUpDentroOutraUI_dGrid').data('igGrid'),
                       grdUpd = $('#scyUILookUpDentroOutraUI_dGrid').data('igGridUpdating');
               ...
               , renderscyUILookUpDentroOutraUI_dGrid: function(vm) {
                   ...
                   $('#scyUILookUpDentroOutraUI_dGrid_headers').live('focus  keydown', function (evt) {
                       var keyCode = window.event ? evt.which : evt.keyCode;
                       if (keyCode === 9) {
                           var cols = $('#scyUILookUpDentroOutraUI_dGrid').igGrid('option', 'columns');
                           var dataView = $('#scyUILookUpDentroOutraUI_dGrid').data('igGrid').dataSource._dataView
                           if (dataView.length === 0) return;
                           var firstRow = dataView[0].RowDataId;
                           clear = vm.status() === 'C';
                           if (vm.status() === 'C')
                               $('#scyUILookUpDentroOutraUI_dGrid').igGridUpdating('startEdit', firstRow, 0);
                           else {
                               ...
                               cols.some(function (entry) {
                                   if (entry.key !== 'RowDataId' && !entry.hidden) {
                                       if (verifyCanEditCol(entry.key, clear, entity)) {
                                           $('#scyUILookUpDentroOutraUI_dGrid').igGridUpdating('startEdit', firstRow, indexColumn);
                                           return true;
                                       }
                                       indexColumn++;
                                   }
                   ...
                   var getVisibleColumns = function(metaDataControl) {
                      ...
                      var visibleColumns = '';
                      if($('#scyUILookUpDentroOutraUI_dGrid').data('igGrid') === undefined) return '';
                      var cols = $('#scyUILookUpDentroOutraUI_dGrid').igGrid('option', 'columns');
                      if (cols) {
                   ...
                   var isElementHided = function (grid, forceCreating) {
                     if (!grid) grid = $('#scyUILookUpDentroOutraUI_dGrid');
                     return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialog').is(':visible'));
                   }
                   var refreshData = true;
                   var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'scyUILookUpDentroOutraUI_dGrid_container', dataBind: function (commitData, forceCreating) {
                      var grid = $('#scyUILookUpDentroOutraUI_dGrid');
                      ...
                      if (!started) {
                          ...
                          started = true;
                          commitData = false;
                          $('#scyUILookUpDentroOutraUI_dGrid_groupbyarea').addClass('hide');
                      }
                      if (grid.igGridUpdating('isEditing')) {
                   ...
                   function createDataGrid(grid) {
                       ...
                       vm.gridSaveStates[gridId] = {
                           ...
                           closePopover: function () {
                               $('#scyUILookUpDentroOutraUI_dGrid_LayoutBtn').igPopover('hide');
                           },
                       ...
                       grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
                           ...
                           cellClick: function(evt, ui) {
                                if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                                    ...
                                    if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                                    {
                                         ...
                                         showMultimidia(entity, e, table, key, vm.UILookUpDentroOutraUI());
                                    }
                                }
                                if (typeof vm.OnGridClientClick === 'function') {
                                    entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                    vm.OnGridClientClick('scyUILookUpDentroOutraUI_dGrid', ui.colKey, entity);
                                }
                                var grid = $('#scyUILookUpDentroOutraUI_dGrid');
                           ...
                           columns: [
                               ...
                               { key: 'ComboboxLoja', headerText: 'Combobox Loja', width: '205px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null  , formatter: function (val, record) { return  vm.dataDomains.getName('LX_LOJA', val);} },
                           ...
                           features: [
                                       ...
                                       filterDialogOpening: function (evt, ui) {
                                           ...
                                           var dataView = $('#scyUILookUpDentroOutraUI_dGrid').data('igGrid').dataSource;
                                           ...
                                           scriptHtml += '    var newGrid = $("#scyUILookUpDentroOutraUI_dGrid");';
                                           ...
                                           scriptHtml += '            $("#scyUILookUpDentroOutraUI_dGrid_container_dialog_list").html(newList)';
                                           scriptHtml += '         }});';
                                      ...
                                      ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                         ...
                                         columnSettings: [{ columnKey: 'DatetimeLoja', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid === 'function') {vm.OnPropertyChangeDataGrid('scyUILookUpDentroOutraUI_dGrid', 'DatetimeLoja', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: 'ComboboxLoja', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('ComboboxLoja', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('LX_LOJA', ''),  textKey: 'name', valueKey: 'id', enableClearButton: false }}, { columnKey: "BigIntLoja" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUILookUpDentroOutraUI_dGrid', 'BigIntLoja', ui.oldValue, ui.value);}},  maxLength: 24, maxValue: 9007199254740992, minValue: 0, dataMode: 'long' } }, { columnKey: "DecimalLoja" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUILookUpDentroOutraUI_dGrid', 'DecimalLoja', ui.oldValue, ui.value);}},  maxLength: 14, maxValue: 99999999999.99, minValue: 0, dataMode: 'decimal', minDecimals: 2, maxDecimals: 2 } }, { columnKey: "IdLoja" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUILookUpDentroOutraUI_dGrid', 'IdLoja', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, dataMode: 'int' } }, { columnKey: "IntLoja" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUILookUpDentroOutraUI_dGrid', 'IntLoja', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "SmallIntLoja" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUILookUpDentroOutraUI_dGrid', 'SmallIntLoja', ui.oldValue, ui.value);}},  maxLength: 6, maxValue: null, minValue: 0, dataMode: 'short' } }, { columnKey: "StringLoja" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUILookUpDentroOutraUI_dGrid', 'StringLoja', ui.oldValue, ui.value);}}, maxLength: 50 } }],
                                         ...
                                         editCellStarted: function(evt, ui){
                                             ...
                                             var columns = $('#scyUILookUpDentroOutraUI_dGrid').igGridUpdating('option', 'columnSettings');
                                             var currentCol = null;
                                             currentRow = ui.rowID;
                           ...
                           if ((typeof vm.OnDataGridCreated === 'function')){
                               vm.OnDataGridCreated('scyUILookUpDentroOutraUI_dGrid');
                           }
                           ...
                           selectionrowselectionchanged = function (evt, ui) {
                               if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                   if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                       $(document).undelegate('#scyUILookUpDentroOutraUI_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                       ...
                                       $(document).delegate ('#scyUILookUpDentroOutraUI_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                   }
                                   selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                                } 
                           };
                           $(document).delegate('#scyUILookUpDentroOutraUI_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                           $('#scyUILookUpDentroOutraUI_dGrid > tbody tr').live('focus', function(evt) {
                               var grid = $('#scyUILookUpDentroOutraUI_dGrid'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                           ...
                           grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                               if (vm.UILookUpDentroOutraUI().status() === 'Q') vm.UILookUpDentroOutraUI().dataToolbar.viewInfo();
                           });
                       }
                       vm.addDataSource({ key: 'scyUILookUpDentroOutraUI_dGrid', name: 'dataView', itemsSource: itemsSource });
                   }
