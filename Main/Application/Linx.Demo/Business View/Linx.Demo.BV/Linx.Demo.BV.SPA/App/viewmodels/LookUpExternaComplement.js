...
var complementCtor = function() {
    var complement = {
    isAutomatic: true
                   , ChangedBrandscyLookUpExterna_dGrid: function(vm, decimals, reset) {
                   var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#scyLookUpExterna_dGrid').data('igGrid'),
                       grdUpd = $('#scyLookUpExterna_dGrid').data('igGridUpdating');
               ...
               , renderscyLookUpExterna_dGrid: function(vm) {
                   ...
                   $('#scyLookUpExterna_dGrid_headers').live('focus  keydown', function (evt) {
                       var keyCode = window.event ? evt.which : evt.keyCode;
                       if (keyCode === 9) {
                           var cols = $('#scyLookUpExterna_dGrid').igGrid('option', 'columns');
                           var dataView = $('#scyLookUpExterna_dGrid').data('igGrid').dataSource._dataView
                           if (dataView.length === 0) return;
                           var firstRow = dataView[0].RowDataId;
                           clear = vm.status() === 'C';
                           if (vm.status() === 'C')
                               $('#scyLookUpExterna_dGrid').igGridUpdating('startEdit', firstRow, 0);
                           else {
                               ...
                               cols.some(function (entry) {
                                   if (entry.key !== 'RowDataId' && !entry.hidden) {
                                       if (verifyCanEditCol(entry.key, clear, entity)) {
                                           $('#scyLookUpExterna_dGrid').igGridUpdating('startEdit', firstRow, indexColumn);
                                           return true;
                                       }
                                       indexColumn++;
                                   }
                   ...
                   var getVisibleColumns = function(metaDataControl) {
                      ...
                      var visibleColumns = '';
                      if($('#scyLookUpExterna_dGrid').data('igGrid') === undefined) return '';
                      var cols = $('#scyLookUpExterna_dGrid').igGrid('option', 'columns');
                      if (cols) {
                   ...
                   var isElementHided = function (grid, forceCreating) {
                     if (!grid) grid = $('#scyLookUpExterna_dGrid');
                     return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialog').is(':visible'));
                   }
                   var refreshData = true;
                   var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'scyLookUpExterna_dGrid_container', dataBind: function (commitData, forceCreating) {
                      var grid = $('#scyLookUpExterna_dGrid');
                      ...
                      if (!started) {
                          ...
                          started = true;
                          commitData = false;
                          $('#scyLookUpExterna_dGrid_groupbyarea').addClass('hide');
                      }
                      if (grid.igGridUpdating('isEditing')) {
                   ...
                   function verifyCanEditCol(column, clear, entity){
                       switch(column){
                           case 'BigIntEstado': { canEditing = clear; break;}
                           case 'BitEstado': { canEditing = clear; break;}
                           case 'ComboboxEstado': { canEditing = clear; break;}
                           case 'DatetimeEstado': { canEditing = clear; break;}
                           case 'DecimalEstado': { canEditing = clear; break;}
                           case 'IdEstado': { canEditing = clear; break;}
                           case 'IdPais': { canEditing = clear; break;}
                           case 'IntEstado': { canEditing = clear; break;}
                           case 'SmallIntEstado': { canEditing = clear; break;}
                   ...
                   function createDataGrid(grid) {
                       ...
                       vm.gridSaveStates[gridId] = {
                           ...
                           closePopover: function () {
                               $('#scyLookUpExterna_dGrid_LayoutBtn').igPopover('hide');
                           },
                       ...
                       grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
                           ...
                           cellClick: function(evt, ui) {
                                if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                                    ...
                                    if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                                    {
                                         ...
                                         showMultimidia(entity, e, table, key, vm.LookUpExterna());
                                    }
                                }
                                if (typeof vm.OnGridClientClick === 'function') {
                                    entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                    vm.OnGridClientClick('scyLookUpExterna_dGrid', ui.colKey, entity);
                                }
                                var grid = $('#scyLookUpExterna_dGrid');
                           ...
                           columns: [
                               ...
                               { key: 'ComboboxEstado', headerText: 'Combobox Estado', width: '231px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null  , formatter: function (val, record) { return  vm.dataDomains.getName('LX_ESTADO', val);} },
                           ...
                           features: [
                                       ...
                                       filterDialogOpening: function (evt, ui) {
                                           ...
                                           var dataView = $('#scyLookUpExterna_dGrid').data('igGrid').dataSource;
                                           ...
                                           scriptHtml += '    var newGrid = $("#scyLookUpExterna_dGrid");';
                                           ...
                                           scriptHtml += '            $("#scyLookUpExterna_dGrid_container_dialog_list").html(newList)';
                                           scriptHtml += '         }});';
                                       ...
                                       { name: 'Tooltips', columnSettings:[{ columnKey: "BigIntEstado", allowTooltips: true },{ columnKey: "BitEstado", allowTooltips: true },{ columnKey: "ComboboxEstado", allowTooltips: true },{ columnKey: "DatetimeEstado", allowTooltips: true },{ columnKey: "DecimalEstado", allowTooltips: true },{ columnKey: "IdEstado", allowTooltips: true },{ columnKey: "IdPais", allowTooltips: false },{ columnKey: "IntEstado", allowTooltips: true },{ columnKey: "SmallIntEstado", allowTooltips: true },{ columnKey: "StringEstado", allowTooltips: false },{ columnKey: "StringPais", allowTooltips: false }] },
                                       { name: 'Resizing' }, 
                                       { name: 'Hiding', 
                                       },
                                      ...
                                      ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                         ...
                                         columnSettings: [{ columnKey: "IdPais", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpPais", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, validateOnClearState:false, maxValue:2147483647, maxLength: 12, defaultValue: null } }, { columnKey: "StringEstado", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, validateOnClearState:false, maxValue:null, maxLength: 50, defaultValue: '' } }, { columnKey: "StringPais", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpPais", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, validateOnClearState:false, maxValue:null, maxLength: 50, defaultValue: null } }, { columnKey: 'DatetimeEstado', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid === 'function') {vm.OnPropertyChangeDataGrid('scyLookUpExterna_dGrid', 'DatetimeEstado', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: 'ComboboxEstado', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('ComboboxEstado', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('LX_ESTADO', ''),  textKey: 'name', valueKey: 'id', enableClearButton: false }}, { columnKey: "BigIntEstado" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyLookUpExterna_dGrid', 'BigIntEstado', ui.oldValue, ui.value);}},  maxLength: 24, maxValue: 9007199254740992, minValue: 0, dataMode: 'long' } }, { columnKey: "DecimalEstado" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyLookUpExterna_dGrid', 'DecimalEstado', ui.oldValue, ui.value);}},  maxLength: 14, maxValue: 99999999999.99, minValue: 0, dataMode: 'decimal', minDecimals: 2, maxDecimals: 2 } }, { columnKey: "IdEstado" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyLookUpExterna_dGrid', 'IdEstado', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, dataMode: 'int' } }, { columnKey: "IntEstado" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyLookUpExterna_dGrid', 'IntEstado', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "SmallIntEstado" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyLookUpExterna_dGrid', 'SmallIntEstado', ui.oldValue, ui.value);}},  maxLength: 6, maxValue: null, minValue: 0, dataMode: 'short' } }],
                                         ...
                                         editCellStarted: function(evt, ui){
                                             ...
                                             var columns = $('#scyLookUpExterna_dGrid').igGridUpdating('option', 'columnSettings');
                                             var currentCol = null;
                                             currentRow = ui.rowID;
                           ...
                           if ((typeof vm.OnDataGridCreated === 'function')){
                               vm.OnDataGridCreated('scyLookUpExterna_dGrid');
                           }
                           ...
                           selectionrowselectionchanged = function (evt, ui) {
                               if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                   if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                       $(document).undelegate('#scyLookUpExterna_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                       ...
                                       $(document).delegate ('#scyLookUpExterna_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                   }
                                   selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                                } 
                           };
                           $(document).delegate('#scyLookUpExterna_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                           $('#scyLookUpExterna_dGrid > tbody tr').live('focus', function(evt) {
                               var grid = $('#scyLookUpExterna_dGrid'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                           ...
                           grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                               if (vm.LookUpExterna().status() === 'Q') vm.LookUpExterna().dataToolbar.viewInfo();
                           });
                       }
                       vm.addDataSource({ key: 'scyLookUpExterna_dGrid', name: 'dataView', itemsSource: itemsSource });
                   }
