...
var complementCtor = function() {
    var complement = {
    isAutomatic: true
                   , ChangedBrandscyBotãoTipoPagExterna_dGrid: function(vm, decimals, reset) {
                   var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#scyBotãoTipoPagExterna_dGrid').data('igGrid'),
                       grdUpd = $('#scyBotãoTipoPagExterna_dGrid').data('igGridUpdating');
               ...
               , renderscyBotãoTipoPagExterna_dGrid: function(vm) {
                   ...
                   $('#scyBotãoTipoPagExterna_dGrid_headers').live('focus  keydown', function (evt) {
                       var keyCode = window.event ? evt.which : evt.keyCode;
                       if (keyCode === 9) {
                           var cols = $('#scyBotãoTipoPagExterna_dGrid').igGrid('option', 'columns');
                           var dataView = $('#scyBotãoTipoPagExterna_dGrid').data('igGrid').dataSource._dataView
                           if (dataView.length === 0) return;
                           var firstRow = dataView[0].RowDataId;
                           clear = vm.status() === 'C';
                           if (vm.status() === 'C')
                               $('#scyBotãoTipoPagExterna_dGrid').igGridUpdating('startEdit', firstRow, 0);
                           else {
                               ...
                               cols.some(function (entry) {
                                   if (entry.key !== 'RowDataId' && !entry.hidden) {
                                       if (verifyCanEditCol(entry.key, clear, entity)) {
                                           $('#scyBotãoTipoPagExterna_dGrid').igGridUpdating('startEdit', firstRow, indexColumn);
                                           return true;
                                       }
                                       indexColumn++;
                                   }
                   ...
                   var getVisibleColumns = function(metaDataControl) {
                      ...
                      var visibleColumns = '';
                      if($('#scyBotãoTipoPagExterna_dGrid').data('igGrid') === undefined) return '';
                      var cols = $('#scyBotãoTipoPagExterna_dGrid').igGrid('option', 'columns');
                      if (cols) {
                   ...
                   var isElementHided = function (grid, forceCreating) {
                     if (!grid) grid = $('#scyBotãoTipoPagExterna_dGrid');
                     return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialog').is(':visible'));
                   }
                   var refreshData = true;
                   var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'scyBotãoTipoPagExterna_dGrid_container', dataBind: function (commitData, forceCreating) {
                      var grid = $('#scyBotãoTipoPagExterna_dGrid');
                      ...
                      if (!started) {
                          ...
                          started = true;
                          commitData = false;
                          $('#scyBotãoTipoPagExterna_dGrid_groupbyarea').addClass('hide');
                      }
                      if (grid.igGridUpdating('isEditing')) {
                   ...
                   function verifyCanEditCol(column, clear, entity){
                       switch(column){
                           case 'BigIntFormaPagamento': { canEditing = clear; break;}
                           case 'BitFormaPagamento': { canEditing = clear; break;}
                           case 'ComboboxFormaPagamento': { canEditing = clear; break;}
                           case 'DatetimeFormaPagamento': { canEditing = clear; break;}
                           case 'DecimalFormaPagamento': { canEditing = clear; break;}
                           case 'IdFormaPagamento': { canEditing = clear; break;}
                           case 'IdVenda': { canEditing = clear; break;}
                           case 'IntFormaPagamento': { canEditing = clear; break;}
                           case 'SmallIntFormaPagamento': { canEditing = clear; break;}
                           case 'StringFormaPagamento': { canEditing = clear; break;}
                       }
                       return canEditing;
                   };
                   function createDataGrid(grid) {
                       ...
                       vm.gridSaveStates[gridId] = {
                           ...
                           closePopover: function () {
                               $('#scyBotãoTipoPagExterna_dGrid_LayoutBtn').igPopover('hide');
                           },
                       ...
                       grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
                           ...
                           cellClick: function(evt, ui) {
                                if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                                    ...
                                    if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                                    {
                                         ...
                                         showMultimidia(entity, e, table, key, vm.BotãoTipoPagExterna());
                                    }
                                }
                                if (typeof vm.OnGridClientClick === 'function') {
                                    entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                    vm.OnGridClientClick('scyBotãoTipoPagExterna_dGrid', ui.colKey, entity);
                                }
                                var grid = $('#scyBotãoTipoPagExterna_dGrid');
                           ...
                           columns: [
                               ...
                               { key: 'BigIntFormaPagamento', headerText: 'Big Int Forma Pagamento', width: '335px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                               { key: 'BitFormaPagamento', headerText: 'Bit Forma Pagamento', width: '283px', dataType: 'bool', columnCssClass: 'ellipsis', format: 'checkbox', hidden: false, unbound: false, group: null   },
                               { key: 'ComboboxFormaPagamento', headerText: 'Combobox Forma Pagamento', width: '348px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null  , formatter: function (val, record) { return  vm.dataDomains.getName('LX_FORMA_PAGAMENTO', val);} },
                               { key: 'DatetimeFormaPagamento', headerText: 'Datetime Forma Pagamento', width: '348px', dataType: 'date', columnCssClass: 'ellipsis', format: 'dd/MM/yyyy', hidden: false, unbound: false, group: null   },
                               { key: 'DecimalFormaPagamento', headerText: 'Decimal Forma Pagamento', width: '335px', dataType: 'number', columnCssClass: 'ellipsis', format: '0.00', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                               { key: 'IdFormaPagamento', headerText: 'Id Forma Pagamento', width: '270px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                               ...
                               { key: 'IntFormaPagamento', headerText: 'Int Forma Pagamento', width: '283px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                               { key: 'SmallIntFormaPagamento', headerText: 'Small Int Forma Pagamento', width: '361px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                               { key: 'StringFormaPagamento', headerText: 'String Forma Pagamento', width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   }
                           ],
                           features: [
                                       ...
                                       filterDialogOpening: function (evt, ui) {
                                           ...
                                           var dataView = $('#scyBotãoTipoPagExterna_dGrid').data('igGrid').dataSource;
                                           ...
                                           scriptHtml += '    var newGrid = $("#scyBotãoTipoPagExterna_dGrid");';
                                           ...
                                           scriptHtml += '            $("#scyBotãoTipoPagExterna_dGrid_container_dialog_list").html(newList)';
                                           scriptHtml += '         }});';
                                       ...
                                       { name: 'Tooltips', columnSettings:[{ columnKey: "BigIntFormaPagamento", allowTooltips: true },{ columnKey: "BitFormaPagamento", allowTooltips: true },{ columnKey: "ComboboxFormaPagamento", allowTooltips: true },{ columnKey: "DatetimeFormaPagamento", allowTooltips: true },{ columnKey: "DecimalFormaPagamento", allowTooltips: true },{ columnKey: "IdFormaPagamento", allowTooltips: true },{ columnKey: "IdVenda", allowTooltips: false },{ columnKey: "IntFormaPagamento", allowTooltips: true },{ columnKey: "SmallIntFormaPagamento", allowTooltips: true },{ columnKey: "StringFormaPagamento", allowTooltips: true }] },
                                       { name: 'Resizing' }, 
                                       { name: 'Hiding', 
                                       },
                                      ...
                                      ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                         ...
                                         columnSettings: [{ columnKey: "IdVenda", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpVenda", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, validateOnClearState:false, maxValue:2147483647, maxLength: 12, defaultValue: null } }, { columnKey: 'DatetimeFormaPagamento', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid === 'function') {vm.OnPropertyChangeDataGrid('scyBotãoTipoPagExterna_dGrid', 'DatetimeFormaPagamento', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: 'ComboboxFormaPagamento', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('ComboboxFormaPagamento', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('LX_FORMA_PAGAMENTO', ''),  textKey: 'name', valueKey: 'id', enableClearButton: false }}, { columnKey: "BigIntFormaPagamento" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyBotãoTipoPagExterna_dGrid', 'BigIntFormaPagamento', ui.oldValue, ui.value);}},  maxLength: 24, maxValue: 9007199254740992, minValue: 0, dataMode: 'long' } }, { columnKey: "DecimalFormaPagamento" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyBotãoTipoPagExterna_dGrid', 'DecimalFormaPagamento', ui.oldValue, ui.value);}},  maxLength: 14, maxValue: 99999999999.99, minValue: 0, dataMode: 'decimal', minDecimals: 2, maxDecimals: 2 } }, { columnKey: "IdFormaPagamento" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyBotãoTipoPagExterna_dGrid', 'IdFormaPagamento', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, dataMode: 'int' } }, { columnKey: "IntFormaPagamento" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyBotãoTipoPagExterna_dGrid', 'IntFormaPagamento', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "SmallIntFormaPagamento" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyBotãoTipoPagExterna_dGrid', 'SmallIntFormaPagamento', ui.oldValue, ui.value);}},  maxLength: 6, maxValue: null, minValue: 0, dataMode: 'short' } }, { columnKey: "StringFormaPagamento" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyBotãoTipoPagExterna_dGrid', 'StringFormaPagamento', ui.oldValue, ui.value);}}, maxLength: 50 } }],
                                         ...
                                         editCellStarted: function(evt, ui){
                                             ...
                                             var columns = $('#scyBotãoTipoPagExterna_dGrid').igGridUpdating('option', 'columnSettings');
                                             var currentCol = null;
                                             currentRow = ui.rowID;
                           ...
                           if ((typeof vm.OnDataGridCreated === 'function')){
                               vm.OnDataGridCreated('scyBotãoTipoPagExterna_dGrid');
                           }
                           ...
                           selectionrowselectionchanged = function (evt, ui) {
                               if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                   if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                       $(document).undelegate('#scyBotãoTipoPagExterna_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                       ...
                                       $(document).delegate ('#scyBotãoTipoPagExterna_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                   }
                                   selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                                } 
                           };
                           $(document).delegate('#scyBotãoTipoPagExterna_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                           $('#scyBotãoTipoPagExterna_dGrid > tbody tr').live('focus', function(evt) {
                               var grid = $('#scyBotãoTipoPagExterna_dGrid'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                           ...
                           grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                               if (vm.BotãoTipoPagExterna().status() === 'Q') vm.BotãoTipoPagExterna().dataToolbar.viewInfo();
                           });
                       }
                       vm.addDataSource({ key: 'scyBotãoTipoPagExterna_dGrid', name: 'dataView', itemsSource: itemsSource });
                   }
