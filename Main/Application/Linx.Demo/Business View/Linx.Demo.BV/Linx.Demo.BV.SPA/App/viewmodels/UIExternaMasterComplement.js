...
var complementCtor = function() {
    var complement = {
    isAutomatic: true
                   , ChangedBrandscyUIExternaMaster_dGrid: function(vm, decimals, reset) {
                   var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#scyUIExternaMaster_dGrid').data('igGrid'),
                       grdUpd = $('#scyUIExternaMaster_dGrid').data('igGridUpdating');
               ...
               , renderscyUIExternaMaster_dGrid: function(vm) {
                   ...
                   $('#scyUIExternaMaster_dGrid_headers').live('focus  keydown', function (evt) {
                       var keyCode = window.event ? evt.which : evt.keyCode;
                       if (keyCode === 9) {
                           var cols = $('#scyUIExternaMaster_dGrid').igGrid('option', 'columns');
                           var dataView = $('#scyUIExternaMaster_dGrid').data('igGrid').dataSource._dataView
                           if (dataView.length === 0) return;
                           var firstRow = dataView[0].RowDataId;
                           clear = vm.status() === 'C';
                           if (vm.status() === 'C')
                               $('#scyUIExternaMaster_dGrid').igGridUpdating('startEdit', firstRow, 0);
                           else {
                               ...
                               cols.some(function (entry) {
                                   if (entry.key !== 'RowDataId' && !entry.hidden) {
                                       if (verifyCanEditCol(entry.key, clear, entity)) {
                                           $('#scyUIExternaMaster_dGrid').igGridUpdating('startEdit', firstRow, indexColumn);
                                           return true;
                                       }
                                       indexColumn++;
                                   }
                   ...
                   var getVisibleColumns = function(metaDataControl) {
                      ...
                      var visibleColumns = '';
                      if($('#scyUIExternaMaster_dGrid').data('igGrid') === undefined) return '';
                      var cols = $('#scyUIExternaMaster_dGrid').igGrid('option', 'columns');
                      if (cols) {
                   ...
                   var isElementHided = function (grid, forceCreating) {
                     if (!grid) grid = $('#scyUIExternaMaster_dGrid');
                     return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialog').is(':visible'));
                   }
                   var refreshData = true;
                   var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'scyUIExternaMaster_dGrid_container', dataBind: function (commitData, forceCreating) {
                      var grid = $('#scyUIExternaMaster_dGrid');
                      ...
                      if (!started) {
                          ...
                          started = true;
                          commitData = false;
                          $('#scyUIExternaMaster_dGrid_groupbyarea').addClass('hide');
                      }
                      if (grid.igGridUpdating('isEditing')) {
                   ...
                   function createDataGrid(grid) {
                       ...
                       vm.gridSaveStates[gridId] = {
                           ...
                           closePopover: function () {
                               $('#scyUIExternaMaster_dGrid_LayoutBtn').igPopover('hide');
                           },
                       ...
                       grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
                           ...
                           cellClick: function(evt, ui) {
                                if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                                    ...
                                    if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                                    {
                                         ...
                                         showMultimidia(entity, e, table, key, vm.UIExternaMaster());
                                    }
                                }
                                if (typeof vm.OnGridClientClick === 'function') {
                                    entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                    vm.OnGridClientClick('scyUIExternaMaster_dGrid', ui.colKey, entity);
                                }
                                var grid = $('#scyUIExternaMaster_dGrid');
                           ...
                           columns: [
                               ...
                               { key: 'DecimalCliente', headerText: 'Decimal Cliente', width: '231px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                           ...
                           features: [
                                       ...
                                       filterDialogOpening: function (evt, ui) {
                                           ...
                                           var dataView = $('#scyUIExternaMaster_dGrid').data('igGrid').dataSource;
                                           ...
                                           scriptHtml += '    var newGrid = $("#scyUIExternaMaster_dGrid");';
                                           ...
                                           scriptHtml += '            $("#scyUIExternaMaster_dGrid_container_dialog_list").html(newList)';
                                           scriptHtml += '         }});';
                                       ...
                                       { name: 'Tooltips', columnSettings:[{ columnKey: "ComboboxCliente", allowTooltips: true },{ columnKey: "IdPais", allowTooltips: false },{ columnKey: "IdCliente", allowTooltips: true },{ columnKey: "DatetimeCliente", allowTooltips: true },{ columnKey: "DecimalCliente", allowTooltips: true },{ columnKey: "IntCliente", allowTooltips: true },{ columnKey: "StringCliente", allowTooltips: true },{ columnKey: "BigIntCliente", allowTooltips: true },{ columnKey: "BitCliente", allowTooltips: true },{ columnKey: "SmallIntCliente", allowTooltips: true },{ columnKey: "StringEstado", allowTooltips: false },{ columnKey: "StringPais", allowTooltips: false },{ columnKey: "IdEstado", allowTooltips: false }] },
                                       { name: 'Resizing' }, 
                                       { name: 'Hiding', 
                                       },
                                      ...
                                      ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                         ...
                                         columnSettings: [{ columnKey: "IdPais", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpEstado", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, validateOnClearState:false, maxValue:2147483647, maxLength: 12, defaultValue: null } }, { columnKey: "StringEstado", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpEstado", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, validateOnClearState:false, maxValue:null, maxLength: 50, defaultValue: null } }, { columnKey: "StringPais", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpEstado", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, validateOnClearState:false, maxValue:null, maxLength: 50, defaultValue: null } }, { columnKey: "IdEstado", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpEstado", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, validateOnClearState:false, maxValue:2147483647, maxLength: 12, defaultValue: null } }, { columnKey: 'DatetimeCliente', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid === 'function') {vm.OnPropertyChangeDataGrid('scyUIExternaMaster_dGrid', 'DatetimeCliente', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: 'ComboboxCliente', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('ComboboxCliente', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('LX_CLIENTE', ''),  textKey: 'name', valueKey: 'id', enableClearButton: false }}, { columnKey: "IdCliente" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIExternaMaster_dGrid', 'IdCliente', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, dataMode: 'int' } }, { columnKey: "DecimalCliente" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIExternaMaster_dGrid', 'DecimalCliente', ui.oldValue, ui.value);}},  maxLength: 18, maxValue: null, minValue: 0, dataMode: 'decimal' } }, { columnKey: "IntCliente" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIExternaMaster_dGrid', 'IntCliente', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "StringCliente" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIExternaMaster_dGrid', 'StringCliente', ui.oldValue, ui.value);}}, maxLength: 50 } }, { columnKey: "BigIntCliente" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIExternaMaster_dGrid', 'BigIntCliente', ui.oldValue, ui.value);}},  maxLength: 24, maxValue: 9007199254740992, minValue: 0, dataMode: 'long' } }, { columnKey: "SmallIntCliente" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIExternaMaster_dGrid', 'SmallIntCliente', ui.oldValue, ui.value);}},  maxLength: 6, maxValue: null, minValue: 0, dataMode: 'short' } }],
                                         ...
                                         editCellStarted: function(evt, ui){
                                             ...
                                             var columns = $('#scyUIExternaMaster_dGrid').igGridUpdating('option', 'columnSettings');
                                             var currentCol = null;
                                             currentRow = ui.rowID;
                           ...
                           if ((typeof vm.OnDataGridCreated === 'function')){
                               vm.OnDataGridCreated('scyUIExternaMaster_dGrid');
                           }
                           ...
                           selectionrowselectionchanged = function (evt, ui) {
                               if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                   if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                       $(document).undelegate('#scyUIExternaMaster_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                       ...
                                       $(document).delegate ('#scyUIExternaMaster_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                   }
                                   selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                                } 
                           };
                           $(document).delegate('#scyUIExternaMaster_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                           $('#scyUIExternaMaster_dGrid > tbody tr').live('focus', function(evt) {
                               var grid = $('#scyUIExternaMaster_dGrid'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                           ...
                           grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                               if (vm.UIExternaMaster().status() === 'Q') vm.UIExternaMaster().dataToolbar.viewInfo();
                           });
                       }
                       vm.addDataSource({ key: 'scyUIExternaMaster_dGrid', name: 'dataView', itemsSource: itemsSource });
                   }
