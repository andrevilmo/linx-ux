...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'LookUpExterna', Items: [
{Name: 'LookUpExterna_gbEstado', DisplayName: 'Estado', ColumnSpan: 12, Visible: true, Items: [{Name: 'LookUpExterna_ntxBigIntEstado', DisplayName: 'Big Int Estado', ColumnSpan: 8, Visible: true},{Name: 'LookUpExterna_ckBitEstado', DisplayName: 'Bit Estado', ColumnSpan: 5, Visible: true},{Name: 'LookUpExterna_cmbComboboxEstado', DisplayName: 'Combobox Estado', ColumnSpan: 6, Visible: true},{Name: 'LookUpExterna_dtDatetimeEstado', DisplayName: 'Datetime Estado', ColumnSpan: 3, Visible: true},{Name: 'LookUpExterna_ntxDecimalEstado', DisplayName: 'Decimal Estado', ColumnSpan: 5, Visible: true},{Name: 'LookUpExterna_ntxIdEstado', DisplayName: 'Id Estado', ColumnSpan: 5, Visible: true},{Name: 'LookUpExterna_lUpIdPais', DisplayName: 'Id Pais', ColumnSpan: 5, Visible: true},{Name: 'LookUpExterna_ntxIntEstado', DisplayName: 'Int Estado', ColumnSpan: 5, Visible: true},{Name: 'LookUpExterna_ntxSmallIntEstado', DisplayName: 'Small Int Estado', ColumnSpan: 2, Visible: true},{Name: 'LookUpExterna_lUpStringEstado', DisplayName: 'String Estado', ColumnSpan: 9, Visible: true},{Name: 'LookUpExterna_lUpStringPais', DisplayName: 'String Pais', ColumnSpan: 9, Visible: true},]},       ]};
    };
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'LookUpExterna' ]; settings.parentVM.LookUpExterna = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('LookUpExterna', parentVM.internalUIs) === -1) parentVM.internalUIs.push('LookUpExterna');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-demo-bv-spa-LookUpExterna', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: LookUpExterna', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
                   $('#LookUpExterna_gbEstado').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

                   complement.renderscyLookUpExterna_dGrid(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'Estado';
    ...
    var visibleColumns = 'BigIntEstado,BitEstado,ComboboxEstado,DatetimeEstado,DecimalEstado,IdEstado,IdPais,IntEstado,SmallIntEstado,StringEstado,StringPais';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'Estado', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'Estado', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('Estado');
        ...
        return dataContext.getEstadoByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), true, sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'Estado'); }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getEstadoByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'Estado'); } }
            dataView(data.results);
            currentPage(curPage);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearEstado(getBandeiraRede(), clearComplete);
    ...
    var createEstado = function() {
        dataBind('dataView', true);
        var entity = dataContext.createEstado();
    ...
    var createAndNotifyEstado = function() {
        var entity = createEstado();
        notifyPresentation('');
        return entity;
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#LookUpExterna_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#LookUpExterna_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#LookUpExterna_formViewer')[0];
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            EstadoDatetimeEstado_typeRange: ko.observable('R'), EstadoDatetimeEstado_begin: ko.observable(null), EstadoDatetimeEstado_end: ko.observable(null), EstadoDatetimeEstado_predefFilter: ko.observableArray([]), EstadoDatetimeEstado_predefValue: ko.observable(null),
        EstadoBigIntEstado_begin: ko.observable(null), EstadoBigIntEstado_end: ko.observable(null),
        EstadoDecimalEstado_begin: ko.observable(null), EstadoDecimalEstado_end: ko.observable(null),
        EstadoIdEstado_begin: ko.observable(null), EstadoIdEstado_end: ko.observable(null),
        EstadoIntEstado_begin: ko.observable(null), EstadoIntEstado_end: ko.observable(null),
        EstadoSmallIntEstado_begin: ko.observable(null), EstadoSmallIntEstado_end: ko.observable(null),
        EstadoIdPais: ko.observable(null),
        EstadoStringEstado: ko.observable(null),
        EstadoStringPais: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.EstadoDatetimeEstado_typeRange('R'); entitySearchRange.EstadoDatetimeEstado_begin(null); entitySearchRange.EstadoDatetimeEstado_end(null); entitySearchRange.EstadoDatetimeEstado_predefFilter([]); entitySearchRange.EstadoDatetimeEstado_predefValue(null);
        entitySearchRange.EstadoBigIntEstado_begin(null); entitySearchRange.EstadoBigIntEstado_end(null);
        entitySearchRange.EstadoDecimalEstado_begin(null); entitySearchRange.EstadoDecimalEstado_end(null);
        entitySearchRange.EstadoIdEstado_begin(null); entitySearchRange.EstadoIdEstado_end(null);
        entitySearchRange.EstadoIntEstado_begin(null); entitySearchRange.EstadoIntEstado_end(null);
        entitySearchRange.EstadoSmallIntEstado_begin(null); entitySearchRange.EstadoSmallIntEstado_end(null);
        entitySearchRange.EstadoIdPais(null);
        entitySearchRange.EstadoStringEstado(null);
        entitySearchRange.EstadoStringPais(null);
    };
    entitySearchRange.has_EstadoBigIntEstado = ko.computed(function(){ return (entitySearchRange.EstadoBigIntEstado_begin() != null || entitySearchRange.EstadoBigIntEstado_end() != null); });
    entitySearchRange.has_EstadoDecimalEstado = ko.computed(function(){ return (entitySearchRange.EstadoDecimalEstado_begin() != null || entitySearchRange.EstadoDecimalEstado_end() != null); });
    entitySearchRange.has_EstadoIdEstado = ko.computed(function(){ return (entitySearchRange.EstadoIdEstado_begin() != null || entitySearchRange.EstadoIdEstado_end() != null); });
    entitySearchRange.has_EstadoIntEstado = ko.computed(function(){ return (entitySearchRange.EstadoIntEstado_begin() != null || entitySearchRange.EstadoIntEstado_end() != null); });
    entitySearchRange.has_EstadoSmallIntEstado = ko.computed(function(){ return (entitySearchRange.EstadoSmallIntEstado_begin() != null || entitySearchRange.EstadoSmallIntEstado_end() != null); });
    entitySearchRange.has_EstadoDatetimeEstado = ko.computed(function(){ return (entitySearchRange.EstadoDatetimeEstado_typeRange() == 'R' && (entitySearchRange.EstadoDatetimeEstado_begin() != null || entitySearchRange.EstadoDatetimeEstado_end() != null) || (entitySearchRange.EstadoDatetimeEstado_typeRange() == 'P' && entitySearchRange.EstadoDatetimeEstado_predefFilter().length > 0)); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#LookUpExterna_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: 'ESTADO',
            rootDataTypeName: 'Estado',
            ...
            createAndNotifyEstado: createAndNotifyEstado,
            ...
            __moduleId__: 'pkg_linx-demo-bv-spa/viewmodels/LookUpExterna'
        };
