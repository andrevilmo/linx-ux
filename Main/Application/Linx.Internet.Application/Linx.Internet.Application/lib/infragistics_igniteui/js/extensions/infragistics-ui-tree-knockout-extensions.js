/*!@license
 * Infragistics.Web.ClientUI igTree KnockoutJS extension 15.1.20151.2300
 *
		...
		addHandler = function (event, ui) {
			if (ui.element.attr('data-path').split(ui.owner.options.pathSeparator).length <= 1) {
				...
				$(element).find('.ui-igtree-noderoot').each(function (index, node) {
					ko.applyBindingsToNode(node, { igTreeNode: { value: ds()[index], bindings: options.bindings } }, ds()[index]);
				});
			}
		};
...
ko.bindingHandlers.igTreeNodeCollection = {
	init: function (element, valueAccessor) {
		var options = valueAccessor(), bindings = options.bindings, ds = options.value, children = $(element).children('li'), addHandler, deleteHandler;
		addHandler = function (event, ui) {
			if (ui.element.attr('data-path').split(ui.owner.options.pathSeparator).length > 1) {
				$(element).children('li').each(function (index, node) {
					ko.applyBindingsToNode(node, { igTreeNode: { value: ds()[index], bindings: bindings.bindings || bindings } }, ds()[index]);
				});
			}
		};
		deleteHandler = function () {
...
ko.bindingHandlers.igTreeNode = {
	init: function (element, valueAccessor) {
		var options = valueAccessor(), bindings = options.bindings, ds = options.value, children = $(element).children('ul');
		if (children.length > 0) {
			ko.applyBindingsToNode(children[0], { igTreeNodeCollection: { value: ds[bindings.childDataProperty], bindings: bindings } }, ds[bindings.childDataProperty]);
		}
    },
    update: function (element, valueAccessor) {
