/*!@license
* Infragistics.Web.ClientUI igEditors KnockoutJS extension 15.1.20151.2300
*
	...
	ko.bindingHandlers.igEditor = {
		init: function (element, valueAccessor, allBindingsAccessor, viewModel) {
			...
			if (options.updateMode.toLowerCase() === "onchange") {
				...
				//In that case the model is updated on keyup event
				...
				//Check if listItems are defined
		...
		update: function (element, valueAccessor) {
			var value, current, editor = $(element), options, parsedDate, val;
			value = ko.utils.unwrapObservable(valueAccessor().value);
			options = valueAccessor();
			//Check if editor has value set
			if (options.type !== undefined) {
				switch (options.type) {
					//Editor with type set to 'date', or 1 (DateEditor)
					case "date":
					case 2:
					case "datepicker":
					case 3:
					...
					break;
					default:
						//In Case of MaskEditor, TextEditor 
						if (current !== value) {
							editor.igEditor("value", value);
						}
				}
			} else {
			//No specific type is set. Proceed with the normal update of the value
				current = editor.igEditor("value");
				if (current !== value) {
					editor.igEditor("value", value);
				}
	...
	ko.bindingHandlers.igTextEditor = {
		init: function (element, valueAccessor, allBindingsAccessor, viewModel) {
			...
			if (options.updateMode.toLowerCase() === "onchange") {
				...
				if (options.listItems !== undefined) {
					//In that case the model is updated listselected event
			...
			if (options.updateMode.toLowerCase() === "onchange") {
				...
				editor.bind("igdatepickertextchanged", function (event, args) {
					if (ko.isObservable(valueAccessor().value) && args.text !== args.oldText) {
						//T.P. #153479 This allows immediate change only when the current value is a valid date in non en-US date regional setting. In non en-US formats immediate change is not supported, because of the limitation into the javascript Date object
