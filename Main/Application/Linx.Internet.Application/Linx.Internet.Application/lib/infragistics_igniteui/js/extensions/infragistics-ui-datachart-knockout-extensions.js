/*!@license
	* Infragistics.Web.ClientUI igDataChart KnockoutJS extension 15.1.20151.2300
	*
...
(function ($) {
	ko.bindingHandlers.igDataChart = {
		init: function (element, valueAccessor) {
			var options = $.extend(true, {}, ko.utils.unwrapObservable(valueAccessor())), ds, i, addHandler, deleteHandler, rootItem, current;
			addHandler = function(data, index) {
				...
				ko.applyBindingsToNode(current[0],{ igDataChartRecord : {value : ds[index], chartInstance: $(element), dataSourceInstance: ds}}, ds[i]);
			};
			...
			for(i= 0; i< ds.length ;i ++ ) {
				...
				ko.applyBindingsToNode(current[0],{ igDataChartRecord : {value : ds[i], chartInstance: $(element), dataSourceInstance: ds}}, ds[i]);
			}
