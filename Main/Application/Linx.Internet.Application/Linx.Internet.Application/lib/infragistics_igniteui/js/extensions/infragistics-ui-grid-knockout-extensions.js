/*!@license
* Infragistics.Web.ClientUI igGrid KnockoutJS extension 15.1.20151.2300
*
...
(function ($) {
	$.widget("ui.igGridKnockoutBridge", {
		...
		_gridRendered: function (evt, ui) {
			...
			var grb = $(ui.owner.element).data("igGridGroupBy") || $("#" + ui.owner.id()).data("igGridGroupBy");
		...
		_rebindCells: function () {
			...
			owner.element.children("tbody").children("tr[data-grouprow!=true][data-container!=true]")
				.filter(function () {
						...
						for (k = 0; k < opts.columns.length; k++) {
							...
							if (ko.isObservable(dataVal)) {
								...
								if (ko.hasOwnProperty("applyBindingAccessorsToNode")) {
									ko.applyBindingAccessorsToNode(cell, { igCell: { value: opts.columns[k] } }, { $data: dataVal });
								} else {
									ko.applyBindingsToNode(cell, { igCell: { value: opts.columns[k] } }, dataVal);
								}
							}
						}
					} else {
		...
		recordsUpdated: function (valueAccessor) {
			...
			updating = $(this.options.parent.element).data("igGridUpdating") || $("#" + grid.id()).data("igGridUpdating"),
