 define(['plugins/dialog', 'durandal/app', 'knockout', 'services/logger', 'managers/__auth', 'managers/user', 'common', 'viewmodels/shared/addCustomExport', 'plugins/router'],
    function (dialog, app, ko, logger, managerAuth, managerUser, common, addCustomExport, router) {
        ...
        var vm = {
            ...
            isUserLayout: ko.observable(true),
                    ...
                    .then(function (selectedOption) {
                        if (selectedOption === 'Yes') {
                            ...
                            $.ajax({
                                ...
                                url: managerAuth.getServiceAddress('linxframeworkobjeto', 'Linx.Framework.BV') + '/DeleteConfiguracaoExportacao' + '?idConfiguracaoExportacao=' + _this.currentDataItem().Id,
                                async: true,
                                cache: false,
                                globalError: true,
            ...
            simpleExport_Click: function () {
                ...
                var _this = this;
                //getFilter
                var lastFilter = _this.currentDataItem().JEntitySearch;
                var lastFilterTranslated = this.currentDataItem().TranslatedJEntitySearch;
            ...
            customUrl_Click: function () {
                ...
                var jEntitySearch = '?jEntitySearch=\'' + Linx.Base64.encode((isNullOrEmpty(this.currentDataItem().JEntitySearch) ? 'null' : this.currentDataItem().JEntitySearch)) + '\'';

                var action = metadata[0].actionFeed.replace(_this.sourceVM.controllerName, '');
                app.showMessage(_this.sourceVM.getDataContext().getServiceAddress(_this.sourceVM.controllerName) + action + jEntitySearch + selectProjection, 'Endereço do serviço', ['Ok']);
            },
            ...
            getData: function () {
                ...
                $.ajax({
                    ...
                    contentType: 'application/json; charset=UTF-8',
                    headers: managerAuth.getHeaders(),
                    url: managerAuth.getServiceAddress('linxframeworkobjeto', 'Linx.Framework.BV') + '/GetConfiguracaoExportacao' + '?isExcel=' + (this.isExcelDataSource() ? 'true' : 'false') + '&parentFullName=' + this.getParentFullName(),
                    async: true,
                    cache: false,
                    globalError: true,
                    error: function (jqXHR, textStatus, errorThrown) {
            ...
            createGrid: function () {
                ...
                _this.grid().igGrid({
                    ...
                    dataSource: [],
                    columns: [
                        { headerText: 'Id', key: 'Id', dataType: 'string', width: '1px', hidden: true },
                        ...
                        { headerText: 'Responsável', key: 'UserName', dataType: 'string', width: '200px' }
                    ],
                    features: [
                        {
                                ...
                                if (typeof id !== 'undefined') {
                                    $.each(_this.dataView, function (i, item) {
                                        if (item.Id.toString() === id) {
                                            ...
                                            _this.isUserLayout(item.IsUserLayout);
                                        }
                                    });
