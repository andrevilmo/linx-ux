    ...
    function (system, router, dialog, ko, logger, managerAuth, common, app) {
        ...
        return {
            ...
            showModal: function (moduleId, parentDataContext, title, options, modalType) {
                ...
                if (parentDataContext && parentDataContext.lookupInfo && parentDataContext.lookupInfo.vm && parentDataContext.lookupInfo.vm.__moduleId__)
                    this.moduleInfo(parentDataContext.lookupInfo.vm.__moduleId__);
            ...
            UI_selectOption: function (dialogResult) {
                var showWait = false;
                if (dialogResult == "Ok") {
                    if (this.parentDataContext && typeof this[this.internalUIs[0]] == 'function') {
                        var innerVM = this[this.internalUIs[0]]();
                        if (innerVM) {
                            if (innerVM.status() == 'E' && !(innerVM.hasInternalUIsValidationErrors() || innerVM.hasValidationErrors())) {
                                app.showMessage('Não é possível fechar com a tela em modo de edição!', 'Alerta', ['Ok']);
                                return;
                            }
                            if (innerVM.status() == 'E' && (innerVM.hasInternalUIsValidationErrors() || innerVM.hasValidationErrors()))
                                return;
                            //Lookup checking
                            if (this.parentDataContext.lookupInfo && this.parentDataContext.lookupInfo.vm && (typeof innerVM.getSpecializedLookupItems === 'function')) {
                ...
                if (showWait) {
                    var lVM = this.parentDataContext.lookupInfo.vm;
                    lVM.showProcessing();
                    var self = this;
                    setTimeout(function () {
