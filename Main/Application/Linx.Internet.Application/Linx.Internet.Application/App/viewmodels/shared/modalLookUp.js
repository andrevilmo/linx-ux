    ...
    function (dialog, ko, logger, managerAuth, common, breeze, app) {
        ...
        var vm = {
            ...
            getSelection: function () {
                var result = [];
                vm.saveSelection();
                for (var propName in vm.selectedItems)
                {                    
                    result = result.concat(vm.selectedItems[propName]);                    
                }
                return result;
            },
            saveSelection: function () {
                ...
                var multiSelect = grid.igGridSelection("option", "multipleSelection");
                if (multiSelect) {                    
                    var pageProp = "Page" + vm.lookupInfo.pageSkip.toString();
                    vm.selectedItems[pageProp] = [];
                    var rows = grid.igGridSelection("selectedRows");
                    if (rows && rows.length > 0) {
                        var ds = grid.data().igGrid.dataSource.dataView();
                        $.each(rows, function (index, value) {
                            var element = ds[value.index];
                            if (element)
                                vm.selectedItems[pageProp].push(element);
                        });
                    }
                }
            },
