define(['durandal/app', 'durandal/system', 'knockout', 'plugins/router', 'common', 'managers/__auth', 'managers/user'],
    function (app, system, ko, router, common, managerAuth, managerUser) {
        ...
        var QueryStringItem = function (p) {
            ...
            self.environmentId = p.environmentId;
            self.serviceAddress = p.serviceAddress;
        };
        ...
        var VM = function () {
            ...
            this.activate = function (context) {
                if (isNullOrEmpty(context)) {
                    queryString = new QueryStringItem({ filter: '', translation: '', environmentId: managerAuth.loginInfo.IdTcsAmbienteDefault, serviceAddress: '' });
                    this.back = '#';
                }
                else {
                    queryString = new QueryStringItem({ filter: context.filter, translation: context.translation, environmentId: context.environmentId, serviceAddress: context.serviceAddress });
            ...
            this.compositionComplete = function () {
                ...
                var serviceUrl = queryString.serviceAddress;

                if (isNullOrEmpty(serviceUrl)) {
                    serviceUrl = managerAuth.getServiceAddress('LinxReportAccessBVTelerikReport', 'Linx.Framework.BV', queryString.environmentId).replace('LinxReportAccessBVTelerikReport', 'api/LinxReportAccessBVTelerikReport')
                }
                else {
                    serviceUrl = serviceUrl + 'api/LinxReportAccessBVTelerikReport';
                }
                ...
                var environmentInfo = managerAuth.getEnvironmentInfo(queryString.environmentId);

                var idRelatorio = managerUser.searchReports(router.activeInstruction().config.route);

                var companyLogo = managerAuth.getParameter("TCS_LOGO_PADRAO", queryString.environmentId);

                if (!companyLogo) {
                    companyLogo = '';
                }

                var companyName = managerAuth.getParameter("TCS_NOME_EMPRESA", queryString.environmentId);
                if (!companyName) {
                    companyName = '';
                }
                ...
                viewer.reportSource({
                    ...
                    parameters: {
                        ...
                        CurrentUserName: managerAuth.loginInfo.NomeUsuario,
                        EconomicGroup: managerAuth.loginInfo.UidGrupoEconomico,
                        AccessGroup: '00000000-0000-0000-0000-000000000000',
                        CurrentCompany: environmentInfo.UidEmpresa,
                        AuthorizationToken: environmentInfo.Token,
                        Environment: environmentInfo.IdTcsAmbiente,
                        Application: environmentInfo.UidAplicacao,
                        ...
                        Username: 'abc',
                        Password: 'abc',
                        Branch: managerAuth.idFilialPfj,
