        ...
        var VM = function () {
            ...
            self.exportPDF = function () {               
                setTimeout(function () {
                    ...
                    var dataView = self.currentSettings.vm.__moduleId__.replace('viewmodels', 'views');

                    setTimeout(function () {
                        self.currentSettings.vm.dataToolbar.isBusy(true);
                    }, 10);

                    html2canvas($('body'), {
                        onclone: function(clone) {
                            $(clone).find('#sideMenu').css('display', 'none');
                            $(clone).find('header').css('padding-left', '35px');
                            $(clone).find('#main').css('margin-left', '0px');
                            $(clone).find('#tab').css('width', '100%');  
                            return true;
                        },
                        onrendered: function (canvas) {
                            self.currentSettings.vm.dataToolbar.isBusy(false)
                            var data = canvas.toDataURL();         
                            var docDefinition = { pageSize: 'A4', pageOrientation: 'landscape', content: [{ image: data, width: 750}] };
                            pdfMake.createPdf(docDefinition).download(self.currentSettings.vm.viewName + ".pdf");
                        }
                    });
                }, 100);
            ...
            self.UI_btnReport_Click = function (currentReportVM) {
                ...
                var idTcsAmbiente = (managerAuth.shellMode == "DEV" || managerAuth.shellMode == "SETUP" ? managerAuth.loginInfo.Ambientes[0].IdTcsAmbiente : router.activeInstruction().config.currentData.IdTcsAmbiente)
                var serviceAddress = self.currentSettings.vm.getServiceAddress(self.currentSettings.vm.controllerName);
                serviceAddress = strLeft(serviceAddress, serviceAddress.indexOf(self.currentSettings.vm.controllerName))

                router.navigate(currentReportVM.urlLink + '?filter=' + queryString + '&translation=' + queryTranslation + '&serviceAddress=' + serviceAddress + '&environmentId=' + idTcsAmbiente + '&back=' + router.activeInstruction().config.hash);
            };
