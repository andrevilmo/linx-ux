define(['durandal/app', 'pkg_linx-framework-bv-spa/services/AuditoriaContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-framework-bv-spa/viewmodels/ConsultaAuditoriaComplement', 'viewmodels/shared/modalCustomSearch', 'managers/user'],
...
var vmConstructor = function () {
    ...
    var getLayoutHeaderGrid = function(name) {
        try {
            ...
            Object.keys(flattenLayout()).filter(function(elem) {
                if (flattenLayout()[elem].Id !== undefined)
                    if (flattenLayout()[elem].Id === name)
    ...
    var objectLayout = function () {
       return {Name: 'ConsultaAuditoria', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_ConsultaAuditoria(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'ConsultaAuditoria' ]; settings.parentVM.ConsultaAuditoria = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.ConsultaAuditoria = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('ConsultaAuditoria', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('ConsultaAuditoria');
                           }
                           else {
                               parentVM.internalUIs = ['ConsultaAuditoria'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-ConsultaAuditoria', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: ConsultaAuditoria', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#ConsultaAuditoria_gbAdtAuditoria').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    $('#ConsultaAuditoria_gbExpander_8e2773cc416f400cbcc3ebd4b2206627').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    complement.renderConsultaAuditoria_dGridAdtAuditoriaItem(vm);

    complement.renderscyConsultaAuditoria_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                       complement.ChangedBrandConsultaAuditoria_dGridAdtAuditoriaItem(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'AdtAuditoria';
    ...
    var visibleColumns = 'IdAdtAuditoria,DataHora,AssemblyName,ConnectionString,IdUsuario,NomeUsuario';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'AdtAuditoria', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'AdtAuditoria', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('AdtAuditoria', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('AdtAuditoria');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('AdtAuditoria');
        ...
        return dataContext.getAdtAuditoriaByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'AdtAuditoria'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getAdtAuditoriaByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearAdtAuditoria(getBandeiraRede(), clearComplete);
    ...
    var createAdtAuditoria = function() {
        dataBind('dataView', true);
        var entity = dataContext.createAdtAuditoria();
    ...
    var createAndNotifyAdtAuditoria = function() {
        var entity = createAdtAuditoria();
        notifyPresentation('');
        return entity;
    };
    
    var createAdtAuditoriaItem = function(parent, noCurrent) {
        dataBind('AdtAuditoriaItemList', true);
        var entity = dataContext.createAdtAuditoriaItem(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentAdtAuditoriaItem(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyAdtAuditoriaItem = function(parent) {
        var entity = createAdtAuditoriaItem(parent);
    ...
    var removeFormViewControl = function () {
        var front = $('#ConsultaAuditoria_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#ConsultaAuditoria_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#ConsultaAuditoria_formViewer')[0];
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            AdtAuditoriaDataHora_typeRange: ko.observable('R'), AdtAuditoriaDataHora_begin: ko.observable(null), AdtAuditoriaDataHora_end: ko.observable(null), AdtAuditoriaDataHora_predefFilter: ko.observableArray([]), AdtAuditoriaDataHora_predefValue: ko.observable(null),
        AdtAuditoriaEmail: ko.observable(null),
        AdtAuditoriaIdUsuario: ko.observable(null),
        AdtAuditoriaNomeUsuario: ko.observable(null),
        AdtAuditoriaItemIdAdtAuditoria: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.AdtAuditoriaDataHora_typeRange('R'); entitySearchRange.AdtAuditoriaDataHora_begin(null); entitySearchRange.AdtAuditoriaDataHora_end(null); entitySearchRange.AdtAuditoriaDataHora_predefFilter([]); entitySearchRange.AdtAuditoriaDataHora_predefValue(null);
        entitySearchRange.AdtAuditoriaEmail(null);
        entitySearchRange.AdtAuditoriaIdUsuario(null);
        entitySearchRange.AdtAuditoriaNomeUsuario(null);
        entitySearchRange.AdtAuditoriaItemIdAdtAuditoria(null);
    };
    entitySearchRange.has_AdtAuditoriaDataHora = ko.computed(function(){ return (entitySearchRange.AdtAuditoriaDataHora_typeRange() == 'R' && (entitySearchRange.AdtAuditoriaDataHora_begin() != null || entitySearchRange.AdtAuditoriaDataHora_end() != null) || (entitySearchRange.AdtAuditoriaDataHora_typeRange() == 'P' && entitySearchRange.AdtAuditoriaDataHora_predefFilter().length > 0)); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#ConsultaAuditoria_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: 'ADT_AUDITORIA',
            rootDataTypeName: 'AdtAuditoria',
            rootNamespace: 'Linx.Framework.BV.Auditoria',
            ...
            createAndNotifyAdtAuditoria: createAndNotifyAdtAuditoria,
            createAdtAuditoriaItem: createAdtAuditoriaItem,
            createAndNotifyAdtAuditoriaItem: createAndNotifyAdtAuditoriaItem,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/ConsultaAuditoria'
        };
