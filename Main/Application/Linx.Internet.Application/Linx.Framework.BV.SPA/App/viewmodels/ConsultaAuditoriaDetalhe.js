...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'ConsultaAuditoriaDetalhe', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_ConsultaAuditoriaDetalhe(), 'Name'))());
        }, function (err) {
    ...
    var activate = function (settings, querystring) {
      ...
      vm.ConsultaAuditoriaDetalhe = getVM;
      //loadLanguage();
      changeLanguage();
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'ConsultaAuditoriaDetalhe' ]; settings.parentVM.ConsultaAuditoriaDetalhe = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.ConsultaAuditoriaDetalhe = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('ConsultaAuditoriaDetalhe', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('ConsultaAuditoriaDetalhe');
                           }
                           else {
                               parentVM.internalUIs = ['ConsultaAuditoriaDetalhe'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-ConsultaAuditoriaDetalhe', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: ConsultaAuditoriaDetalhe', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        complement.renderscyConsultaAuditoriaDetalhe_dGrid(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'AdtAuditoriaItemDetalhe';
    ...
    var visibleColumns = 'Propriedade,ValorAntigo,ValorNovo';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'AdtAuditoriaItemDetalhe', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'AdtAuditoriaItemDetalhe', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('AdtAuditoriaItemDetalhe', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('AdtAuditoriaItemDetalhe');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('AdtAuditoriaItemDetalhe');
        ...
        return dataContext.getAdtAuditoriaItemDetalheByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'AdtAuditoriaItemDetalhe'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getAdtAuditoriaItemDetalheByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearAdtAuditoriaItemDetalhe(getBandeiraRede(), clearComplete);
    ...
    var createAdtAuditoriaItemDetalhe = function() {
        dataBind('dataView', true);
        var entity = dataContext.createAdtAuditoriaItemDetalhe();
    ...
    var createAndNotifyAdtAuditoriaItemDetalhe = function() {
        var entity = createAdtAuditoriaItemDetalhe();
        notifyPresentation('');
        return entity;
    };
    ...
    var addNew = function (parentEntity) {
        ...
        goToItem(createAdtAuditoriaItemDetalhe());
        editInnerUIs();
        showButtonsEditorTemplate();
        dataBind();
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#ConsultaAuditoriaDetalhe_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#ConsultaAuditoriaDetalhe_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#ConsultaAuditoriaDetalhe_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#ConsultaAuditoriaDetalhe_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            dataView: dataView,
            viewName: 'ConsultaAuditoriaDetalhe',
            ...
            rootBmTypeName: 'ADT_AUDITORIA_ITEM_DETALHE',
            rootDataTypeName: 'AdtAuditoriaItemDetalhe',
            ...
            createAndNotifyAdtAuditoriaItemDetalhe: createAndNotifyAdtAuditoriaItemDetalhe,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/ConsultaAuditoriaDetalhe'
        };
