...
{
    public class DbSetView<TEntity> : IQueryable<TEntity>, IEnumerable<TEntity>, IEnumerable, IQueryable, IAsyncEnumerableAccessor<TEntity>, IInfrastructure<IServiceProvider>, IContext
        where TEntity : class
    {
        ...
        public IQueryable<TEntity> Where(string predicate, Dictionary<string, object> parameters)
        {
            return this.Where<TEntity>(predicate, parameters);
        }

        public IQueryable<TEntity> Where<TEntityBase>(string predicate, System.Data.Entity.Core.Objects.ObjectParameter[] parameters)
                ...
                foreach (var innerType in _innerDataSets.OfType<Type>())
                {
                    ...
                    switch (actionName)
                    {
                        ...
                        case "AttachUpdated":
                            ...
                            {
                                var property = originalEntry.Property(prop.Name); //.CurrentValues.SetValues(innerInstance);
                                if (property != null)
                                    property.CurrentValue = innerInstance.GetPropertyValue(prop.Name);
                            }
        ...
        public TEntity GetObjectByKey(EntityKey entityKey)
        {
            ...
            foreach (var eKey in entityKey.EntityKeyValues)
            {
                ...
                parameters.Add("p" + eKey.Key, eKey.Value);
            }
