...
namespace Microsoft.AspNetCore.OData.Query
{
    ...
    public class FilterQueryOption
    {
        ...
        public IQueryable ApplyTo(IQueryable query, ODataQuerySettings querySettings, IAssemblyProvider assemblyProvider)
        {
            ...
            var result = ExpressionHelpers.Where(query, filter, Context.ElementClrType);
            
            return result;
        }

        public Func<long> ApplyCount(IQueryable query, ODataQuerySettings querySettings, IAssemblyProvider assemblyProvider)
        {
            if (query == null)
            {
                throw Error.ArgumentNull("query");
            ...
            if (Context.ElementClrType == null)
            {
                throw Error.NotSupported(SRResources.ApplyToOnUntypedQueryOption, "ApplyCount");
            }

            var filter = FilterBinder.Bind(FilterClause, Context.ElementClrType, Context.Model, assemblyProvider, querySettings);
            var result = ExpressionHelpers.Where(query, filter, Context.ElementClrType);

            return ExpressionHelpers.Count(result, Context.ElementClrType);
        }
    }
}