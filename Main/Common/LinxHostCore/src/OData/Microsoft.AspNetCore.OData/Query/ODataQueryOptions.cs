...
namespace Microsoft.AspNetCore.OData.Query
{
    ...
    public class ODataQueryOptions
    {
        ...
        public virtual Func<long> ApplyForCount(IQueryable query, ODataQuerySettings querySettings)
        {
            ...
            if (Filter != null)
            {
                result = Filter.ApplyCount(query, querySettings, _assemblyProvider);
            }
            else
            {
                result = ExpressionHelpers.Count(query, Context.ElementClrType);
            }
        ...
        public virtual IQueryable ApplyTo(IQueryable query, ODataQuerySettings querySettings)
        {
            ...
            if (OrderBy != null)
            {
                query = OrderBy.ApplyTo(query, querySettings, _assemblyProvider);
            }
        ...
        private void BuildQueryOptions(IDictionary<string, string> queryParameters)
        {
            foreach (var kvp in queryParameters)
            {
                switch (kvp.Key.ToLowerInvariant())
                {
                    ...
                    case "$count":
                        ...
                        RawValues.Count = (kvp.Value == "true" || kvp.Value == "allpages" ? "true" : "false");
                        if (RawValues.Count == "true")
                        {
                            bool count;
                            if (bool.TryParse(RawValues.Count, out count))
                            {
                                Count = count;
                            }
                            else
            ...
            if (!String.IsNullOrWhiteSpace(RawValues.Select) || !String.IsNullOrWhiteSpace(RawValues.Expand))
            {
                ...
                SelectExpand = new SelectExpandQueryOption(RawValues.Select, RawValues.Expand, Context, _queryOptionParser, Request);
            }
