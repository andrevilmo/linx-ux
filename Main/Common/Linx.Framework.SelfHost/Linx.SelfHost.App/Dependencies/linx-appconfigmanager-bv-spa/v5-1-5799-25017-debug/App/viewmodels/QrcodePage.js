define(['durandal/app', 'pkg_linx-appconfigmanager-bv-spa/services/QrcodePageContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'viewmodels/shared/modalCustomSearch'],
function (app, dataContextFn, router, widget, managerAuth, modal, modal2, managerBrand, managerPredefined, logger, modalMultimidia, common, modalCustomSearch) {
var vms = [];
...
var vmConstructor = function () {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('QrcodePage', parentVM.internalUIs) === -1) parentVM.internalUIs.push('QrcodePage');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        if (managerAuth.shellMode == 'PROD') {
           managerAuth.getFormAccess('linx-appconfigmanager-bv-spa-QrcodePage', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: QrcodePage', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'Qrcode';
    ...
    var replaceKeyFromParent = function (parentPropertyName, oldValue, newValue) {
        if (parentEntityRelated != null && isChildVM() && uiSettings.detailFieldsRelation.length == 1 && uiSettings.parentFieldsRelation.length == 1 && uiSettings.parentFieldsRelation[0] === parentPropertyName) {
            ...
            var cacheElements = dataContext.getEntities('Qrcode', [dataContext.breeze.EntityState.Added]);
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'Qrcode', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'Qrcode', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && pageSize() === 0 && isChildVM()) {
           var cacheElements = dataContext.getEntities('Qrcode');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('Qrcode');
        ...
        return dataContext.getQrcodeByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), (status() !== 'C' && pageSize() === 0 && isChildVM()), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'Qrcode'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        vm.showProcessing('Pesquisando informações...');
        return dataContext.getQrcodeByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    $.ajax({
	url: vm.getServiceAddress('LinxAppConfigManagerQrcodePage/GetUrlLocal'),
	dataType: 'json',
	cache: false,
	...
	success: function (data) {
			$lx(vm.viewModel, "#QrcodePage_lblqrcode").html("<div align='center'><img src='" + data + "'/></div>");
	}
});
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearQrcode(getBandeiraRede(), clearComplete);
    ...
    var createQrcode = function() {
        dataBind('dataView', true);
        var entity = dataContext.createQrcode();
    ...
    var createAndNotifyQrcode = function() {
    ...
    var removeFormViewControl = function () {
        var front = $('#QrcodePage_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#QrcodePage_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#QrcodePage_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#QrcodePage_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: '',
            rootDataTypeName: 'Qrcode',
            rootNamespace: 'Linx.AppConfigManager.BV.QrcodePage',
            ...
            createAndNotifyQrcode: createAndNotifyQrcode,
            ...
            __moduleId__: 'pkg_linx-appconfigmanager-bv-spa/viewmodels/QrcodePage'
        };
