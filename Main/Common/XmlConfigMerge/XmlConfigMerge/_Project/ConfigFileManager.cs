...
namespace Tools.XmlConfigMerge
{
	...
	/// </summary>
	[Serializable]
#if PUBLIC_CONFIGFILEMANAGER
	public class ConfigFileManager
#else	
	internal class ConfigFileManager
#endif	
	{
		public ConfigFileManager(string masterConfigPath) : this(masterConfigPath, null) {
		}

		public ConfigFileManager(string masterConfigPath, string mergeFromConfigPath)
				: this(masterConfigPath, mergeFromConfigPath, false)  { //makeMergeFromConfigPathTheSavePath
		}
		...
		private static void UpdateExistingElementsAndAttribsRecurse(XmlNodeList fromNodes, XmlNode toParentNode) {
			...
			foreach (XmlNode node in fromNodes) {
                ...
                } else if (node.Attributes["Ref"] != null)  {
                    toNode = SelectSingleNodeMatchingNamespaceURI(toParentNode, node, node.Attributes["Ref"]);
                } else if (node.Attributes["ModuleName"] != null) {
                    toNode = SelectSingleNodeMatchingNamespaceURI(toParentNode, node, node.Attributes["ModuleName"]);
                } else
                {
