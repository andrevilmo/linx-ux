    ...
    public class EntitySearch
    {
                    ...
                    case "Value":
                        ...
                        else if (value == null || (value.ToString().ToLower() == "null"))
                        {
                            if (lastOperator == "==" || lastOperator == "=")
                                query = query.Left(query.Length - 1) + "is";
                            else if (lastOperator == "!=" || lastOperator == "<>")
                                query = query.Left(query.Length - 2) + "is not";
                            query += " null";
                        }
                        else
                        {
                            paramName = (this.EntityName.IsNullOrEmpty() ? alias : this.EntityName) + "_P" + (startParamIndex + parameters.Count).ToString().PadLeft(2, '0') + (paramSuffix.IsNullOrEmpty() ? string.Empty : paramSuffix);
        ...
        private static Type GetInternalType(Type topType, string internalTypeName)
        {
            ...
            else
            {
                ...
                {
                    customAttributes = detailMember.GetCustomAttributes(typeof(CompositionAttribute), false);
