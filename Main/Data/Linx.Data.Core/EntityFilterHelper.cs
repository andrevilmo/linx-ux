...
namespace Linx.Data
{
    public static class EntityFilterHelper
    {
        ...
        public static List<BmMetaDataProperty> GetBmEntityProperties(this DbContext context, string entityName, string parentDataPath)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    var entity = contextType.GetTypeInfo().Assembly.GetType(contextType.Namespace + "." + entityName);
                    var tableAttr = entity.GetTypeInfo().GetCustomAttribute<System.ComponentModel.DataAnnotations.Schema.TableAttribute>();
                    string displayName = "";
                    foreach (var propInfo in entity.GetProperties())
                    {
                        ...
                        var eName = (propInfo.GetMethod.IsVirtual ? (propInfo.PropertyType.GetTypeInfo().IsGenericType ? propInfo.PropertyType.GetElement().Name : propInfo.PropertyType.Name) : "");
                        properties.Add(new BmMetaDataProperty() { id = parentDataPath + "." + propInfo.Name, parent = parentDataPath, text = displayName, dataType = Linx.Tools.EntitySearch.ParseJDataType((propInfo.PropertyType.GetTypeInfo().IsGenericType ? propInfo.PropertyType.GetElement().Name : propInfo.PropertyType.Name)), entityName = eName, enabled = !propInfo.GetMethod.IsVirtual, children = propInfo.GetMethod.IsVirtual });
                    }
        ...
        public static string JExpressionToEntitySql(this DbContext context, string jExpression, List<ObjectParameter> parameters)
        {
        ...
        private static string GetFilterPart(this DbContext context, DbFilterExpression dbFilter, List<ObjectParameter> parameters, ref int aliasCount)
        {
            ...
            if (context != null && !entityDataPath.IsNullOrEmpty() && entityDataPath.Occurs(".") > 0)
            {
                ...
                var currentLevelEntity = contextType.GetTypeInfo().Assembly.GetType(contextType.Namespace + "." + topEntityName);
                ...
                if (parts.Length > 1)
                {
                    for (int idx = 1; idx < parts.Length; idx++)
                    {
                        ...
                        currentLevelEntity = (propertyNavigation.PropertyType.GetTypeInfo().IsGenericType ? propertyNavigation.PropertyType.GetElement() : propertyNavigation.PropertyType);
        ...
        private static List<DbFilterElement> GetPredefinedExpression(string propName, object propValue, char dataType, string oprt)
        {
            ...
            if (propValue is string && propValue.ToString().Occurs("$") == 2)
            {
                ...
                switch (predefinedValue)
                {
                    ...
                    case "Param":
                        hasExpression = false;
                        IParametersHelper parametersHelper = null;
