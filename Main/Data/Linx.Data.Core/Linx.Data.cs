    ...
    public class Entity : INotifyPropertyChanged
    {
        ...
        public void RefreshKeys()
        {
            ...
            if (!this.MetaDataMaps.IsNull())
            {
                ...
                {
                    if (!metaData.IsNull() && !metaData.DbEntity.IsNull())
                    {
        ...
        private void AdjustParentAssociation()
        {
            ...
            {
                isForeignKey = Linx.Tools.ObjectExtension.GetPropertyOfAttributeType(property, typeof(Linx.DS.Core.Data.AssociationAttribute), "IsForeignKey");
                if (!isForeignKey.IsNull() && ((bool)isForeignKey))
                {
                    thisKeyMembers = Linx.Tools.ObjectExtension.GetPropertyOfAttributeType(property, typeof(Linx.DS.Core.Data.AssociationAttribute), "ThisKeyMembers") as string[];
                    otherKeyMembers = Linx.Tools.ObjectExtension.GetPropertyOfAttributeType(property, typeof(Linx.DS.Core.Data.AssociationAttribute), "OtherKeyMembers") as string[];
                    ...
                    foreach (string relationPropertyName in metaData.PropertiesMap.Where(e => e.IsFK || (e.IsKey && e.EdmKey.Occurs(".") == 2)).Select(e => e.RelationPropertyName).Distinct())
                    {
                        ...
                        if (keysMap.Count() > 0)
                        {
                            ...
                            foreach (var parent in parents)
                            {
                                ...
                                {
                                    if (context.ChangeTracker.Entries().Any(c => c.Entity == parentMetaData.DbEntity && c.State == EntityState.Added))
                                    {
    ...
    public static class DbContextExtensions
    {
        ...
        private static void GetQueryExpression(List<EntitySearch> entitySearchList, ref string query, List<ObjectParameter> queryParameters, string edmParentEntityName, string alias, int level, List<EntitySearch> analyzedList, List<string> closeParenthesesControl = null, int queryGroup = 0)
        {
            ...
            if (entitySearchList != null && entitySearchList.Count > 0)
            {
                ...
                foreach (var searchElement in entitySearchList.Where(e => !analyzedList.Contains(e) && ((edmParentEntityName.IsNullOrEmpty() && e.EdmParentEntityName.IsNullOrEmpty()) || (e.EdmParentEntityName == edmParentEntityName && e.QueryGroup == queryGroup))).ToList())
                {
        ...
        public static bool TryGetObjectByKey(this DbContext context, Type type, EntityKey entityKey, out object objectReference)
        {
            ...
            if (dbSet == null) dbSet = context.GetPropertyValue(type.GetTypeInfo().BaseType.Name);
            ...
            else
            {
                IEnumerable<object> values =
                    entityKey.EntityKeyValues.Count > 1
                        ? ReorderValuesByColumnOrder(entityKey.EntityKeyValues.ToArray(), type)
        ...
        public static void DeleteObject(this DbContext context, object entityObject)
        {
            ...
            if (dbSet == null) dbSet = context.GetPropertyValue(entityObject.GetType().GetTypeInfo().BaseType.Name);
            var dbSetType = dbSet.GetType();
            if (dbSetType.Name == "DbSetView`1")
            {
        ...
        public static void AttachUpdated(this DbContext context, object entity, object originalEntity)
        {
            if (entity != null && originalEntity != null)
            {
                ...
                if (dbSet == null)
                    dbSet = context.GetPropertyValue(entity.GetType().GetTypeInfo().BaseType.Name);
                var dbSetType = dbSet.GetType();
                if (dbSetType.Name == "DbSetView`1")
                {
                ...
                else
                {
                    ...
                    context.Entry(originalEntity).UpdateProperties(entity);
                }
            }
        }
        ...
        private static bool TryGetObjectByKey(this DbContext context, Type type, IEnumerable<object> values, out object objectReference)
        {
            ...
            MethodInfo method = ctxTYpe.GetMethod("Set");
            MethodInfo generic = method.MakeGenericMethod(type);
            var dbSet = generic.Invoke(context, null) as IQueryable;

            objectReference = dbSet.Find(type, values.ToArray());
            return objectReference != null;
        }
        
        private static bool TryGetObjectByKey<TEntity>(this DbContext context, IEnumerable<object> values, out object objectReference) where TEntity: class
        {
            objectReference = context.Set<TEntity>().Find(values.ToArray());
            return objectReference != null;
        }
