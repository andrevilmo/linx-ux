    ...
    public class EfMapping
    {
        ...
        public EfMapping(DbContext db)
        {
            ...
            foreach (var set in conceptualContainer.BaseEntitySets.OfType<EntitySet>())
            {
                ...
                {
                    var keyProp = (mappingToLookAt.EntityType ?? mappingToLookAt.IsOfEntityTypes[0]).KeyProperties.FirstOrDefault(p => p.Name == item.PropertyName);
                    if (keyProp != null)
                    {
                        item.IsPrimaryKey = true;
                        var config = keyProp.MetadataProperties["Configuration"];
                        if (config != null)
                        {

                            PropertyInfo property = config.Value.GetType().GetProperty("DatabaseGeneratedOption");
                            if (property != null)
                            {
                                var propertyValue = property.GetValue(config.Value, null);
                                item.IsStoreGeneratedIdentity = (propertyValue != null && propertyValue.ToString() == "Identity");
                            }
                        }                        
                    }                    
                }
        ...
        {
            if (scalar.Column.TypeName == "nvarchar" || scalar.Column.TypeName == "varchar" || scalar.Column.TypeName == "char")
            {
