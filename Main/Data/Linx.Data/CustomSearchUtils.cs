    ...
    public class CustomSearchUtils
    {
        public static List<Table> GetEntities(string entityNames, Assembly assembly, DbContext context)
        {
            ...
            foreach (string entity in entities)
            {
                ...
                string edmEntityName = Linx.Tools.ObjectExtension.GetFunctionalPointOfType(types.UnderlyingSystemType, "EdmEntityName");
                string relations = ObjectExtension.GetFunctionalPointOfType(types.UnderlyingSystemType, "EntityRelations");
                ...
                foreach (PropertyDefinitions propertyDefinition in ObjectExtension.GetFunctionalPoints(types.UnderlyingSystemType))
                {
            ...
            if (cache.IsNull())
            {
                foreach (var item in type)
                {
                    if (!item.PropertyType.FullName.StartsWith("System.Nullable`1[[System.Int32") && item.PropertyType.GenericTypeArguments.Count() > 0)
                    {
                        ...
                        TableAttribute tableAttribute = entity.GetCustomAttributes(typeof(TableAttribute), true).FirstOrDefault() as TableAttribute;
