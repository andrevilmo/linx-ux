...
namespace Linx.DS.Core.Data
{
    ...
    internal static class KnownTypeUtilities
    {
        ...
        internal static IEnumerable<Type> ImportKnownTypes(Type type, bool inherit)
        {
            ...
            IEnumerable<KnownTypeAttribute> knownTypeAttributes = type.GetTypeInfo().GetCustomAttributes(typeof(KnownTypeAttribute), inherit).Cast<KnownTypeAttribute>();
            ...
            foreach (KnownTypeAttribute knownTypeAttribute in knownTypeAttributes)
            {
                ...
                if (!string.IsNullOrEmpty(methodName))
                {
                    ...
                    MethodInfo methodInfo = type.GetTypeInfo().GetMethod(methodName, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.DeclaredOnly);
                    if (methodInfo != null && typeOfIEnumerableOfType.GetTypeInfo().IsAssignableFrom(methodInfo.ReturnType))
                    {
