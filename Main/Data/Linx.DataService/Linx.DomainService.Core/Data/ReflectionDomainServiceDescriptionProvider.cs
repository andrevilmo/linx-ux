    ...
    internal class ReflectionDomainServiceDescriptionProvider : DomainServiceDescriptionProvider
    {
        ...
        public override DomainServiceDescription GetDescription()
        {
            ...
            IEnumerable<MethodInfo> methodsToInspect =
                description.DomainServiceType.GetTypeInfo().GetMethods(BindingFlags.Instance | BindingFlags.Public)
            ...
            foreach (MethodInfo method in methodsToInspect)
            {
                ...
                if (method.GetCustomAttributes(typeof(IgnoreAttribute), false).Count() > 0)
                {
                    continue;
                }
...
#pragma warning restore 0618
        ...
        private static AttributeCollection GetDomainServiceTypeAttributes(Type domainServiceType)
        {
            ...
            List<Attribute> attributes = new List<Attribute>(domainServiceType.GetTypeInfo().GetCustomAttributes().Cast<Attribute>());

            // All evaluate attributes that exist in interface implementations.
            Type[] interfaces = domainServiceType.GetTypeInfo().GetInterfaces();
            foreach (Type interfaceType in interfaces)
            {
                // Merge attributes together
                ReflectionDomainServiceDescriptionProvider.MergeAttributes(
                    attributes,
                    interfaceType.GetTypeInfo().GetCustomAttributes().Cast<Attribute>());
            }
        ...
        internal class ReflectionDomainOperationEntry : DomainOperationEntry
        {
            ...
            {
                var method = domainService.GetType().GetMethod(this.Name);
                if (method != null)
                    return method.Invoke(domainService, parameters);
                else
                    return null;
            }
            ...
            private static AttributeCollection GetAttributeCollection(MethodInfo methodInfo)
            {
                ...
                foreach (Type interfaceType in methodInfo.DeclaringType.GetTypeInfo().GetInterfaces())
                {
                    InterfaceMapping interfaceMapping = methodInfo.DeclaringType.GetTypeInfo().GetRuntimeInterfaceMap(interfaceType);
