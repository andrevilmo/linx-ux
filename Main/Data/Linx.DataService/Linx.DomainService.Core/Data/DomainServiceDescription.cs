...
namespace Linx.DS.Core.Data
{
    ...
    public sealed class DomainServiceDescription
    {
        ...
        public DomainOperationEntry GetCustomMethod(Type entityType, string methodName)
        {
            ...
            DomainOperationEntry method = null;
            for (Type baseType = entityType; baseType != null; baseType = baseType.GetTypeInfo().BaseType)
            {
        ...
        public DomainOperationEntry GetSubmitMethod(Type entityType, DomainOperation operation)
        {
            ...
            for (Type baseType = entityType; baseType != typeof(object) && baseType != null; baseType = baseType.GetTypeInfo().BaseType)
            {
        ...
        public Type GetSerializationType(Type type)
        {
            ...
            return this._exposedTypeMap.GetOrAdd(type, t =>
            {
                ...
                Type baseType;
                for (baseType = t; baseType != null; baseType = baseType.GetTypeInfo().BaseType)
                {
        ...
        public static DomainServiceDescription GetDescription(Type domainServiceType)
        {
            ...
            if (domainServiceType.GetTypeInfo().IsAbstract
                || domainServiceType.GetTypeInfo().IsGenericType
                || domainServiceType.GetTypeInfo().IsGenericTypeDefinition
                || !typeof(DomainService).GetTypeInfo().IsAssignableFrom(domainServiceType))
            {
        ...
        internal static DomainServiceDescriptionProvider CreateDescriptionProvider(Type domainServiceType)
        {
            ...
            for (int i = 0; i < baseTypes.Count; i++)
            {
                ...
                foreach (DomainServiceDescriptionProviderAttribute providerAttribute in
                    currType.GetTypeInfo().GetCustomAttributes(typeof(DomainServiceDescriptionProviderAttribute), false))
                {
        ...
        private static bool IsValidEntityType(Type type, out string errorMessage)
        {
            ...
            else if (!type.GetTypeInfo().IsAbstract && type.GetTypeInfo().GetConstructor(Type.EmptyTypes) == null)
            {
        ...
        private static Type GetQueryEntityReturnType(DomainOperationEntry method, out bool isSingleton, out Exception error)
        {
            ...
            if (enumerableOfT != null)
            {
                ...
                entityType = enumerableOfT.GetTypeInfo().GetGenericArguments()[0];
            }
            else if (!typeof(IEnumerable).GetTypeInfo().IsAssignableFrom(returnType) && returnType != typeof(void))
            {
        ...
        private void ValidateComplexTypes()
        {
            foreach (Type complexType in this.ComplexTypes)
            {
                ...
                Type childComplexType = this.ComplexTypes.Where(ct => (ct != complexType) && complexType.GetTypeInfo().IsAssignableFrom(ct)).FirstOrDefault();
                ...
                Type childEntityType = this.EntityTypes.Where(e => complexType.GetTypeInfo().IsAssignableFrom(e)).FirstOrDefault();
        ...
        internal static void ValidateComplexType(Type complexType)
        {
            ...
            if (KnownTypeUtilities.ImportKnownTypes(complexType, /* inherit */ false).Where(t => complexType.GetTypeInfo().IsAssignableFrom(t)).Any())
            {
        ...
        private void AddEntityType(Type entityType)
        {
            ...
            {
                if (entityType.GetTypeInfo().IsAssignableFrom(t))
                {
        ...
        private static void ValidatePropertyPolymorphism(Type entityType)
        {
            ...
            foreach (PropertyInfo propertyInfo in entityType.GetTypeInfo().GetProperties(BindingFlags.Public | BindingFlags.Instance))
            {
                ...
                if (methodInfo.DeclaringType.GetTypeInfo().BaseType != null && !methodInfo.IsVirtual)
                {
                    Type[] parameterTypes = methodInfo.GetParameters().Select<ParameterInfo, Type>(p => p.ParameterType).ToArray();
                    MethodInfo baseMethod = methodInfo.DeclaringType.GetTypeInfo().BaseType.GetTypeInfo().GetMethod(methodInfo.Name, parameterTypes);
        ...
        private void ValidateKnownTypeAttribute(Type entityType, bool mustSpecifyAll)
        {
            ...
            {
                hasConcreteDerivedType |= !derivedType.GetTypeInfo().IsAbstract;
            ...
            if (entityType.GetTypeInfo().IsAbstract && !hasConcreteDerivedType)
            {
        ...
        {
            if (entityType.GetTypeInfo().GetCustomAttributes<DataContractAttribute>() != null)
            {
                return;
            }
            ...
            while (baseType != null)
            {
                if (baseType.GetTypeInfo().GetCustomAttributes<DataContractAttribute>() != null)
                {
        ...
        private void ValidateEntityAssociations(Type entityType, PropertyDescriptorCollection entityProperties)
        {
            foreach (PropertyDescriptor pd in entityProperties)
            {
                ...
                bool isSelfReference = otherEntityType.GetTypeInfo().IsAssignableFrom(entityType);
                ...
                if (this._entityTypes.Contains(otherEntityType))
                {
                    ...
                    if (otherMember != null)
                    {
                        ...
                        if (!otherMemberEntityType.GetTypeInfo().IsAssignableFrom(entityType))
                        {
                            throw new InvalidOperationException(string.Format(
                                CultureInfo.CurrentCulture,
        ...
        private void ValidateDefaultQuery(DomainOperationEntry method)
        {
            ...
            foreach (DomainOperationEntry existingQuery in this._queryMethods.Values)
            {
                ...
                if (queryAttribute != null && queryAttribute.IsDefault)
                {
                    ...
                    if (existingEntityType.GetTypeInfo().IsAssignableFrom(entityType))
                    {
                    ...
                    if (entityType.GetTypeInfo().IsAssignableFrom(existingEntityType))
                    {
        ...
        private Dictionary<Type, HashSet<Type>> ComputeEntityKnownTypes()
        {
            ...
            foreach (Type entityType in this.EntityTypes)
            {
                ...
                IEnumerable<Type> knownTypes = KnownTypeUtilities.ImportKnownTypes(entityType, /* inherit */ true).Where(t => entityType.GetTypeInfo().IsAssignableFrom(t));
        ...
        internal IEnumerable<Type> GetEntityDerivedTypes(Type entityType)
        {
            ...
            return this.EntityTypes.Where(et => et != entityType && entityType.GetTypeInfo().IsAssignableFrom(et));
        }
    }
}
