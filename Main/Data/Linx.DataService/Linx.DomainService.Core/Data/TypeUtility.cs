    ...
    internal static class TypeUtility
    {
        ...
        public static bool IsSupportedCollectionType(Type type)
        {
            if (type.IsArray ||
               ...
               (typeof(IList).GetTypeInfo().IsAssignableFrom(type) && type.GetTypeInfo().GetConstructor(Type.EmptyTypes) != null))
            {
                return true;
            }
        ...
        public static bool IsPredefinedDictionaryType(Type type)
        {
            ...
            {
                return genericType.GetTypeInfo().GetGenericArguments().All(t => IsPredefinedSimpleType(t));
            }
        ...
        public static bool IsComplexType(Type type)
        {
            ...
            if (!type.GetTypeInfo().IsVisible || type.GetTypeInfo().IsGenericType || type.GetTypeInfo().IsAbstract)
            {
                return false;
            }
            ...
            if (IsSystemAssembly(type.GetTypeInfo().Assembly))
            {
                return false;
            }
        ...
        public static Type GetElementType(Type type)
        {
            ...
            if (ienum != null)
            {
                Type genericArg = ienum.GetTypeInfo().GetGenericArguments()[0];
                return genericArg;
            }
        ...
        internal static bool DefinitionIsAssignableFrom(this Type genericTypeDefinition, Type derivedType, out Type genericType)
        {
            ...
            while (genericType != null)
            {
                if (genericTypeDefinition.GetTypeInfo().IsInterface)
                {
                    ...
                    foreach (Type interfaceType in genericType.GetTypeInfo().GetInterfaces().Concat(new[] { derivedType }))
                    {
