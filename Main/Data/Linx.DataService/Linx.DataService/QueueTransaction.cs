...
namespace Linx.DataService
{
    ...
    public class QueueTransaction
    {
        ...
        public static QueueTransaction SaveTransaction<T>(SaveInformation<T> saveInfo, string assemblyName, HttpControllerContext controller, string actionName)
        {
            ...
            obj.AddChange(new ChangeTracker
            {
                ...
                Controller = controller.ControllerDescriptor.ControllerType.FullName,
        ...
        public ChangeTracker[] SubmitTansaction()
        {
            ...
            using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = System.Transactions.IsolationLevel.ReadCommitted }))
            {
                while (this.Changes.Count > 0)
                {
                    ...
                    var assem = System.Reflection.Assembly.Load(change.AssemblyName);
                    var controllerInstance = assem.CreateInstance(change.Controller);

                    dynamic retListObject = controllerInstance.GetType().GetMethod(change.ActionName + "__ForMEF").Invoke(controllerInstance, new object[] { change.DataObjects, change.RelationInfo, executedChanges });
    ...
    public class ViewMapHelper
    {
        ...
        private List<object> GetEntitiesInListByTypeName(List<object> parent)
        {
            ...
            ac = (obj) =>
            {
                ...
                foreach (var nav in t.GetProperties().Where(p => typeof(IEnumerable).IsAssignableFrom(p.PropertyType) && p.PropertyType.IsGenericType))
