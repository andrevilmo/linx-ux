...
(function () {
    ...
    function TestClientViewModel(data) {
        ...
        self.RequestHeaders = ko.observableArray(data.RequestHeaders);
...
function SendRequest(httpMethod, url, requestHeaders, requestBody, handleResponse) {
    ...
    try {
        //put the value for automatic authorization
        for (var i in requestHeaders) {
            var header = requestHeaders[i];
            if (header.name === 'Application' && header.value === '')
                header.value = 'A9B8C7D6-E5F4-F4E6-D6C7-B8A9A9B8C7D6';
            if ((header.name === 'CurrentCompany' || header.name === 'EconomicGroup') && header.value === '')
                header.value = 'F27FFC4F-EB6E-4484-91ED-A318A4A394B0';
            if (header.name === 'Environment' && header.value === '')
                header.value = '0';
