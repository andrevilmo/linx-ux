...
var result = function () {
    ...
    var getDataFeedUrl = function() {
       return getServiceAddress('LinxFaqOData');
    };
    ...
    var lookUpNames = [];
    var entitylookUps = [];
    entityNames.push('Form_BV');
    metadataInfo['Form_BV'] = [
        ...
        { key: 'Titulo', isQbeZero: false, isDomain: false, domainName: '', lookupPropertyName: '', lookupVisibleColumns: '', isRequired: true, maxLength: 0, isPartOfKey: false, headerText: 'Título', width: '250px', dataType: 'string', format: '', hidden: false, unbound: false, group: null },
        { key: 'Duvida', isQbeZero: false, isDomain: false, domainName: '', lookupPropertyName: '', lookupVisibleColumns: '', isRequired: true, maxLength: 0, isPartOfKey: false, headerText: 'Dúvida', width: '250px', dataType: 'string', format: '', hidden: false, unbound: false, group: null },
        { key: 'Empresa', isQbeZero: false, isDomain: false, domainName: '', lookupPropertyName: '', lookupVisibleColumns: '', isRequired: true, maxLength: 0, isPartOfKey: false, headerText: 'Empresa', width: '250px', dataType: 'string', format: '', hidden: false, unbound: false, group: null },
        { key: 'Nome', isQbeZero: false, isDomain: false, domainName: '', lookupPropertyName: '', lookupVisibleColumns: '', isRequired: true, maxLength: 0, isPartOfKey: false, headerText: 'Nome', width: '250px', dataType: 'string', format: '', hidden: false, unbound: false, group: null },
        { key: 'Email', isQbeZero: false, isDomain: false, domainName: '', lookupPropertyName: '', lookupVisibleColumns: '', isRequired: true, maxLength: 0, isPartOfKey: false, headerText: 'Email', width: '250px', dataType: 'string', format: '', hidden: false, unbound: false, group: null }
    ];
    dataExportInfo['Form_BV'] = [ 
        { name: 'Form_BV', canExportMedia: false , canExportReport: true, actionExport: 'LinxFaq/GetForm_BVToExcel', actionReport: 'LinxFaq/GetForm_BVToReportXml', actionFeed: 'LinxFaqOData/Form_BV', actionName: 'LinxFaq/GetForm_BVByEntitySearchNoAssociations', display: 'Form_BV',  metaData: function() { return metadataInfo['Form_BV']; } }
    ];
    entitylookUps.push('Form_BV');
    entitylookUps['Form_BV'] = [];
    ...
    ,Titulo: { dataType: DataType.String, isNullable: false, isPartOfKey: false, validators: [ Validator.hasValueValidator]  }
    ,Duvida: { dataType: DataType.String, isNullable: false, isPartOfKey: false, validators: [ Validator.hasValueValidator]  }
    ,Empresa: { dataType: DataType.String, isNullable: false, isPartOfKey: false, validators: [ Validator.hasValueValidator]  }
    ,Nome: { dataType: DataType.String, isNullable: false, isPartOfKey: false, validators: [ Validator.hasValueValidator]  }
    ,Email: { dataType: DataType.String, isNullable: false, isPartOfKey: false, validators: [ Validator.hasValueValidator]  }
                    },
    navigationProperties: {
    // Returns collections of details and associates with Parent
                          }
    });
    lookUpProperties['Form_BV'] = {};
    var Form_BVInitializer = function (ownerReference, isPOCO) {
       ownerReference.RowDataId = (isPOCO === true ? getNextSequence('Form_BV') : ko.observable(getNextSequence('Form_BV')));
       ...
       ownerReference.UpdateIndependentRelation = function(detailName) {
           ...
           for (var idxR = 0; idxR < cacheElements.length; idxR++) {
               if (typeof cacheElements[idxR].Form_BV !== 'function') { return; }
               else  if (cacheElements[idxR].Form_BV() != ownerReference) { cacheElements[idxR].Form_BV(ownerReference); }
           }
       }
       ...
       ownerReference.namespace = 'Linx.Dashboard.Faq';
       ownerReference.myProperties = [ ,'Titulo','Duvida','Empresa','Nome','Email' ];
    ...
    metadataStore.registerEntityTypeCtor("Form_BV", null, Form_BVInitializer);
    ...
    var clearForm_BV = function (idBandeiraRede, complete) {
        clearAll();
        resetSequence('Form_BV');
        var refForm_BV = manager.createEntity('Form_BV', {}, breeze.EntityState.Unchanged);
        if (complete) complete({ results: [ refForm_BV ] });
        return true;
    };
    
    var getForm_BV = function (predicate, preserveCurrentState, noTracking) {
        if (!preserveCurrentState) clearAll();
        var query = EntityQuery.from('GetForm_BV').noTracking(noTracking)
        .orderBy('Titulo asc')
        ;
    ...
    var getForm_BVByEntitySearchNoAssociations = function (jEntitySearch, skip, take, returnInlineCount, preserveCurrentState, noTracking, orderByDef) {
        if (!preserveCurrentState) clearAll();
        var query = EntityQuery.from('GetForm_BVByEntitySearchNoAssociations').noTracking(noTracking)
        .orderBy((isNullOrEmpty(orderByDef) ? 'Titulo asc' : orderByDef))
        ;
    ...
    // Define that the event name is 'FaqContext_DataUpdate'.
    var contextUpdtEvt = 'FaqContext_DataUpdate_' + getNewGuid();
    ...
    var saveChanges = function(saveSucceeded, saveFailed, fin, saveNoTRack) {
        if (saveNoTRack === true) {
            ...
            return $.ajax({
               type: 'POST',
               crossDomain: true,
               url: getServiceAddress('LinxFaq/Save' + vm.rootDataTypeName),
               globalError: false,
    ...
    var createForm_BV = function() {
        ...
        enableChangeTrack = false;
        var entity = createEntity('Form_BV', { EntityUniqueKey: getNewGuid() });
        entity.setDefaults();
    ...
    var exportReportDataSource = function(complete) {
        $.ajax({
           ...
           crossDomain: true,
           headers: managerAuth.getHeaders(),
           url: getServiceAddress("LinxFaq/GetReportDataSource"),
           success: function (response) {
    ...
    var exportTemplateReport = function(reportPath, complete) {
        $.ajax({
           ...
           crossDomain: true,
           headers: managerAuth.getHeaders(),
           url: getServiceAddress("LinxFaq/GetTemplateReport"),
    ...
    //#endregion getWithBinding
        ...
        var dataContext = {
            ...
            clearForm_BV: clearForm_BV,
            getForm_BV: getForm_BV,
            getForm_BVByEntitySearchNoAssociations: getForm_BVByEntitySearchNoAssociations
        };
    loadParameters();
    return dataContext;
