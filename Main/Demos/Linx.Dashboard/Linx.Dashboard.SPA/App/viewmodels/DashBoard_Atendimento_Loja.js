define(['durandal/app', 'pkg_linx-dashboard-spa/services/DashboardFinanceiroContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-dashboard-spa/viewmodels/DashBoard_Atendimento_LojaComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    var activate = function (settings, querystring) {
      ...
      vm.DashBoard_Atendimento_Loja = getVM;
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'DashBoard_Atendimento_Loja' ]; settings.parentVM.DashBoard_Atendimento_Loja = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.DashBoard_Atendimento_Loja = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('DashBoard_Atendimento_Loja', parentVM.internalUIs) === -1) parentVM.internalUIs.push('DashBoard_Atendimento_Loja');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-dashboard-spa-DashBoard_Atendimento_Loja', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: DashBoard_Atendimento_Loja', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#DashBoard_Atendimento_Loja_gbCustomContainer_6466f42689d8401abee603c6d41f7e37').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    complement.renderpivot70a64183c732442293877bd0a3e412b0DashBoard_Atendimento_Loja_pivotLjvAtendimento(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'LjvAtendimento';
    ...
    var visibleColumns = 'CodLoja,DescLoja,CodBandeiraRede,DescBandeiraRede,DataAtendimento,CodigoFilial,ValorCupomFiscal,ValorDescontoSubtotal,ValorTotal,TicketMedio,QtdeAtendimento';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'LjvAtendimento', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'LjvAtendimento', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getQueryFilter = function (currentDI) {
        ...
        if (eSearch === 'Error')
           return 'Error';
        if (vm.getBandeiraRede() === 0) {
           eSearch += 'LjvAtendimento{IdBandeiraRede#' + (!isNullOrEmpty(vm.currentBrands) ? 'In#S' : '==#I') + vm.getCurrentBrands() + '}';
        }
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('LjvAtendimento');
        ...
        return dataContext.getLjvAtendimentoByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'LjvAtendimento'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getLjvAtendimentoByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    vm.entitySearchRange.LjvAtendimentoDataAtendimento_begin(new Date(2015, 11, 15, 0, 0,0));
vm.entitySearchRange.LjvAtendimentoDataAtendimento_end(new Date(2015, 11, 31, 0, 0, 0));
    }
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearLjvAtendimento(getBandeiraRede(), clearComplete);
    ...
    var setBandeiraRede = function () {
       if (!queryLoaded) { loadDataView(); }
       if (getBandeiraRede() > 0) dataContext.loadParameters();
    };
    
    var createLjvAtendimento = function() {
        dataBind('dataView', true);
        var entity = dataContext.createLjvAtendimento();
    ...
    var createAndNotifyLjvAtendimento = function() {
        var entity = createLjvAtendimento();
        notifyPresentation('');
        return entity;
    };
    ...
    var canAddChangeEntity = function () {
       if (getBandeiraRede() === 0) {
           app.showMessage('Bandeira/Rede precisa ser selecionada.', 'Alerta', ['Ok']);
           return false;
       } else {
           return true;
       }
    ...
    var removeFormViewControl = function () {
        var front = $('#DashBoard_Atendimento_Loja_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#DashBoard_Atendimento_Loja_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#DashBoard_Atendimento_Loja_formViewer')[0];
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            LjvAtendimentoDataAtendimento_typeRange: ko.observable('R'), LjvAtendimentoDataAtendimento_begin: ko.observable(null), LjvAtendimentoDataAtendimento_end: ko.observable(null), LjvAtendimentoDataAtendimento_predefFilter: ko.observableArray([]), LjvAtendimentoDataAtendimento_predefValue: ko.observable(null),
        LjvAtendimentoValorCupomFiscal_begin: ko.observable(null), LjvAtendimentoValorCupomFiscal_end: ko.observable(null),
        LjvAtendimentoValorDescontoSubtotal_begin: ko.observable(null), LjvAtendimentoValorDescontoSubtotal_end: ko.observable(null),
        LjvAtendimentoValorTotal_begin: ko.observable(null), LjvAtendimentoValorTotal_end: ko.observable(null),
        LjvAtendimentoQtdeAtendimento_begin: ko.observable(null), LjvAtendimentoQtdeAtendimento_end: ko.observable(null),
        LjvAtendimentoTicketMedio_begin: ko.observable(null), LjvAtendimentoTicketMedio_end: ko.observable(null),
        LjvAtendimentoCodBandeiraRede: ko.observable(null),
        LjvAtendimentoCodLoja: ko.observable(null),
        LjvAtendimentoDescBandeiraRede: ko.observable(null),
        LjvAtendimentoDescLoja: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.LjvAtendimentoDataAtendimento_typeRange('R'); entitySearchRange.LjvAtendimentoDataAtendimento_begin(null); entitySearchRange.LjvAtendimentoDataAtendimento_end(null); entitySearchRange.LjvAtendimentoDataAtendimento_predefFilter([]); entitySearchRange.LjvAtendimentoDataAtendimento_predefValue(null);
        entitySearchRange.LjvAtendimentoValorCupomFiscal_begin(null); entitySearchRange.LjvAtendimentoValorCupomFiscal_end(null);
        entitySearchRange.LjvAtendimentoValorDescontoSubtotal_begin(null); entitySearchRange.LjvAtendimentoValorDescontoSubtotal_end(null);
        entitySearchRange.LjvAtendimentoValorTotal_begin(null); entitySearchRange.LjvAtendimentoValorTotal_end(null);
        entitySearchRange.LjvAtendimentoQtdeAtendimento_begin(null); entitySearchRange.LjvAtendimentoQtdeAtendimento_end(null);
        entitySearchRange.LjvAtendimentoTicketMedio_begin(null); entitySearchRange.LjvAtendimentoTicketMedio_end(null);
        entitySearchRange.LjvAtendimentoCodBandeiraRede(null);
        entitySearchRange.LjvAtendimentoCodLoja(null);
        entitySearchRange.LjvAtendimentoDescBandeiraRede(null);
        entitySearchRange.LjvAtendimentoDescLoja(null);
    };
    entitySearchRange.has_LjvAtendimentoValorCupomFiscal = ko.computed(function(){ return (entitySearchRange.LjvAtendimentoValorCupomFiscal_begin() != null || entitySearchRange.LjvAtendimentoValorCupomFiscal_end() != null); });
    entitySearchRange.has_LjvAtendimentoValorDescontoSubtotal = ko.computed(function(){ return (entitySearchRange.LjvAtendimentoValorDescontoSubtotal_begin() != null || entitySearchRange.LjvAtendimentoValorDescontoSubtotal_end() != null); });
    entitySearchRange.has_LjvAtendimentoValorTotal = ko.computed(function(){ return (entitySearchRange.LjvAtendimentoValorTotal_begin() != null || entitySearchRange.LjvAtendimentoValorTotal_end() != null); });
    entitySearchRange.has_LjvAtendimentoQtdeAtendimento = ko.computed(function(){ return (entitySearchRange.LjvAtendimentoQtdeAtendimento_begin() != null || entitySearchRange.LjvAtendimentoQtdeAtendimento_end() != null); });
    entitySearchRange.has_LjvAtendimentoTicketMedio = ko.computed(function(){ return (entitySearchRange.LjvAtendimentoTicketMedio_begin() != null || entitySearchRange.LjvAtendimentoTicketMedio_end() != null); });
    entitySearchRange.has_LjvAtendimentoDataAtendimento = ko.computed(function(){ return (entitySearchRange.LjvAtendimentoDataAtendimento_typeRange() == 'R' && (entitySearchRange.LjvAtendimentoDataAtendimento_begin() != null || entitySearchRange.LjvAtendimentoDataAtendimento_end() != null) || (entitySearchRange.LjvAtendimentoDataAtendimento_typeRange() == 'P' && entitySearchRange.LjvAtendimentoDataAtendimento_predefFilter().length > 0)); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#DashBoard_Atendimento_Loja_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var getSPAVersion = function(){
       ...
       var packageName = 'pkg_linx-dashboard-spa';
    ...
    var projectTemplateFiles = [
            {
                pivotName: 'LjvAtendimento',
                layoutFullName: getSPAVersion() + '/pivotTableLayouts/',
                name: '(Nenhum)',
                viewName: 'DashBoard_Atendimento_Loja',
                projectName: 'Linx.Dashboard.SPA',
                content: '',
                id: '',
                selected: true
            }
    ...
    var getLayoutsFiles = function () {
        ...
        {
            rootNamespace: 'Linx.Dashboard.SPA',
            viewName: 'DashBoard_Atendimento_Loja',
            pivotName: 'LjvAtendimento',
            pivotDataSource: 'LjvAtendimento'
        },
            function (data) {
    ...
    var vm = {
            ...
            rootBmTypeName: 'LJV_ATENDIMENTO',
            rootDataTypeName: 'LjvAtendimento',
            rootNamespace: 'Linx.Dashboard.DashboardFinanceiro',
            ...
            createLjvAtendimento: createLjvAtendimento,
            createAndNotifyLjvAtendimento: createAndNotifyLjvAtendimento,
            ...
            __moduleId__: 'pkg_linx-dashboard-spa/viewmodels/DashBoard_Atendimento_Loja'
        };
