define(['durandal/app', 'pkg_linx-dashboard-spa/services/FaqContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-dashboard-spa/viewmodels/FaqForm_UIComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'FaqForm_UI' ]; settings.parentVM.FaqForm_UI = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('FaqForm_UI', parentVM.internalUIs) === -1) parentVM.internalUIs.push('FaqForm_UI');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-dashboard-spa-FaqForm_UI', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: FaqForm_UI', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        complement.renderscyFaqForm_UI_dGrid(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'Form_BV';
    ...
    var visibleColumns = 'Titulo,Duvida,Empresa,Nome,Email';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'Form_BV', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'Form_BV', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('Form_BV', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('Form_BV');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('Form_BV');
        ...
        return dataContext.getForm_BVByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'Form_BV'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getForm_BVByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    var CustomControl608209812_Click = function () {
    app.showMessage(('Mensagem enviada com sucesso!').toString(), 'Alerta', ['Ok'])
    }
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearForm_BV(getBandeiraRede(), clearComplete);
    ...
    var createForm_BV = function() {
        dataBind('dataView', true);
        var entity = dataContext.createForm_BV();
    ...
    var createAndNotifyForm_BV = function() {
        var entity = createForm_BV();
        notifyPresentation('');
        return entity;
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#FaqForm_UI_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#FaqForm_UI_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#FaqForm_UI_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#FaqForm_UI_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: '',
            rootDataTypeName: 'Form_BV',
            rootNamespace: 'Linx.Dashboard.Faq',
            ...
            CustomControl608209812_Click: CustomControl608209812_Click,
            createForm_BV: createForm_BV,
            createAndNotifyForm_BV: createAndNotifyForm_BV,
            ...
            __moduleId__: 'pkg_linx-dashboard-spa/viewmodels/FaqForm_UI'
        };
