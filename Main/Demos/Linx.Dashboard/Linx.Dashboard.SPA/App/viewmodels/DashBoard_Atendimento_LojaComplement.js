...
var complementCtor = function() {
    var complement = {
    isAutomatic: true
    , renderpivot70a64183c732442293877bd0a3e412b0DashBoard_Atendimento_Loja_pivotLjvAtendimento: function(vm) {
    ...
    var itemsSource = { getVisibleColumns: getVisibleColumns, dataBind: function (commitData) {
        if ($('#pivot70a64183-c732-4422-9387-7bd0a3e412b0DashBoard_Atendimento_Loja_pivotLjvAtendimento').is(':visible') && (currentStatus != vm.status() || currentPage != vm.dataToolbar.currentPage() || !commitData)) {
        ...
        if(pivot == null) {
        $('#pivot70a64183-c732-4422-9387-7bd0a3e412b0DashBoard_Atendimento_Loja_pivotLjvAtendimento #fm-fields-view .fm-ui-btn:contains(\'OK\')')
             .live('mouseup', function () {
         });
        
        $('#pivot70a64183-c732-4422-9387-7bd0a3e412b0DashBoard_Atendimento_Loja_pivotLjvAtendimento #fm-toolbar-row .fm-ui-btn:contains(\'OK\')')
           .live('mouseup', function () {
               setTimeout(function () { filterPivotRelationship() }, 1);
        });
             $('#pivot70a64183-c732-4422-9387-7bd0a3e412b0DashBoard_Atendimento_Loja_pivotLjvAtendimento .btn-toggle-toolbar').die('click');
             $('#pivot70a64183-c732-4422-9387-7bd0a3e412b0DashBoard_Atendimento_Loja_pivotLjvAtendimento .btn-toggle-toolbar').live('click', function(){ 
                  var toolbar = pivot.getToolbarInstanceByPivotName('pivot70a64183-c732-4422-9387-7bd0a3e412b0DashBoard_Atendimento_Loja_pivotLjvAtendimento')
        ...
        var setReport = function() { 
            var data = arrayData.map(function(item){
                return {'CodBandeiraRede': (isNullOrEmpty(item.CodBandeiraRede) ? '' : item.CodBandeiraRede.toString()), 'CodigoFilial': (isNullOrEmpty(item.CodigoFilial) ? '' : item.CodigoFilial.toString()), 'CodLoja': (isNullOrEmpty(item.CodLoja) ? '' : item.CodLoja.toString()), 'DataAtendimento': (isNullOrEmpty(item.DataAtendimento) ? '' : Globalize.format(getUTCDate(item.DataAtendimento), 'MM/dd/yyyy')), 'DescBandeiraRede': (isNullOrEmpty(item.DescBandeiraRede) ? '' : item.DescBandeiraRede.toString()), 'DescLoja': (isNullOrEmpty(item.DescLoja) ? '' : item.DescLoja.toString()), 'ValorCupomFiscal': isNullOrEmpty(item.ValorCupomFiscal) ? 0 : item.ValorCupomFiscal, 'ValorDescontoSubtotal': isNullOrEmpty(item.ValorDescontoSubtotal) ? 0 : item.ValorDescontoSubtotal, 'ValorTotal': isNullOrEmpty(item.ValorTotal) ? 0 : item.ValorTotal, 'TicketMedio': isNullOrEmpty(item.TicketMedio) ? 0 : item.TicketMedio, 'QtdeAtendimento': isNullOrEmpty(item.QtdeAtendimento) ? 0 : item.QtdeAtendimento};
            });
        
            var structure = {};
            structure.CodBandeiraRede = { type:'string', caption: 'Cod. Bandeira Rede', dimensionUniqueName: 'ctrlLjvAtendimento', dimensionCaption: ''  };
            structure.CodigoFilial = { type:'string', caption: 'Codigo Filial', dimensionUniqueName: 'ctrlLjvAtendimento', dimensionCaption: ''  };
            structure.CodLoja = { type:'string', caption: 'Cod. Loja', dimensionUniqueName: 'ctrlLjvAtendimento', dimensionCaption: ''  };
            structure.DataAtendimento = { type:'date string', caption: 'Data Atendimento', dimensionUniqueName: 'ctrlLjvAtendimento', dimensionCaption: ''  };
            structure.DescBandeiraRede = { type:'string', caption: 'Bandeira Rede', dimensionUniqueName: 'ctrlLjvAtendimento', dimensionCaption: ''  };
            structure.DescLoja = { type:'string', caption: 'Loja', dimensionUniqueName: 'ctrlLjvAtendimento', dimensionCaption: ''  };
        ...
        if (pivotContext.report) {
            ...
            var report = {
                ...
                columns: [{ uniqueName: 'DataAtendimento' }],
                measures: [{ uniqueName: 'ValorCupomFiscal' }],
            };
               report.columns.push({uniqueName: '[Measures]'})
        
            pivot.setReport(report);
            pivot.setTopX("DataAtendimento", 5, "ValorCupomFiscal");
        }
        };
        ...
        var toolbarInstance = pivot.getToolbarInstanceByPivotName('pivot70a64183-c732-4422-9387-7bd0a3e412b0DashBoard_Atendimento_Loja_pivotLjvAtendimento');
        ...
        var addToolbarFooter = function () {
             ...
             $('#pivot70a64183-c732-4422-9387-7bd0a3e412b0DashBoard_Atendimento_Loja_pivotLjvAtendimento').append('<div class="plus-actions"><ul>'+ listItems +'</ul></div>');
        }
        ...
        var createInstance = function () {
             ...
             var containerId = 'pivot70a64183-c732-4422-9387-7bd0a3e412b0DashBoard_Atendimento_Loja_pivotLjvAtendimento';
    ...
    vm.addDataSource({ key: 'pivot70a64183-c732-4422-9387-7bd0a3e412b0DashBoard_Atendimento_Loja_pivotLjvAtendimento', name: 'dataView', itemsSource: itemsSource });
}
