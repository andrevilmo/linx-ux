...
var vmConstructor = function () {
    ...
    var activate = function (settings, querystring) {
      ...
      vm.DashBoard_Atendimento_Vendedor = getVM;
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'DashBoard_Atendimento_Vendedor' ]; settings.parentVM.DashBoard_Atendimento_Vendedor = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.DashBoard_Atendimento_Vendedor = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('DashBoard_Atendimento_Vendedor', parentVM.internalUIs) === -1) parentVM.internalUIs.push('DashBoard_Atendimento_Vendedor');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-dashboard-spa-DashBoard_Atendimento_Vendedor', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: DashBoard_Atendimento_Vendedor', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#DashBoard_Atendimento_Vendedor_gbExpander_89e73120b40a40668d3741064b7de5e5').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    complement.renderpivota0dbc8b2328b4924b8df255a721b6bedDashBoard_Atendimento_Vendedor_pivotLjvAtendimentoVendedor(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'LjvAtendimentoVendedor';
    ...
    var visibleColumns = 'DataAtendimento,IdAtendimento,NomeVendedor,VendedorApelido,ValorCupomFiscal,ValorComissao';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'LjvAtendimentoVendedor', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'LjvAtendimentoVendedor', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getQueryFilter = function (currentDI) {
        ...
        if (vm.getBandeiraRede() === 0) {
           eSearch += 'LjvAtendimentoVendedor{IdBandeiraRede#' + (!isNullOrEmpty(vm.currentBrands) ? 'In#S' : '==#I') + vm.getCurrentBrands() + '}';
        }
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('LjvAtendimentoVendedor');
        ...
        return dataContext.getLjvAtendimentoVendedorByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'LjvAtendimentoVendedor'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getLjvAtendimentoVendedorByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearLjvAtendimentoVendedor(getBandeiraRede(), clearComplete);
    ...
    var createLjvAtendimentoVendedor = function() {
        dataBind('dataView', true);
        var entity = dataContext.createLjvAtendimentoVendedor();
    ...
    var createAndNotifyLjvAtendimentoVendedor = function() {
        var entity = createLjvAtendimentoVendedor();
        notifyPresentation('');
        return entity;
    };
    ...
    var addNew = function (parentEntity) {
        ...
        goToItem(createLjvAtendimentoVendedor());
        editInnerUIs();
        showButtonsEditorTemplate();
        dataBind();
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#DashBoard_Atendimento_Vendedor_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#DashBoard_Atendimento_Vendedor_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#DashBoard_Atendimento_Vendedor_formViewer')[0];
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            LjvAtendimentoVendedorDataAtendimento_typeRange: ko.observable('R'), LjvAtendimentoVendedorDataAtendimento_begin: ko.observable(null), LjvAtendimentoVendedorDataAtendimento_end: ko.observable(null), LjvAtendimentoVendedorDataAtendimento_predefFilter: ko.observableArray([]), LjvAtendimentoVendedorDataAtendimento_predefValue: ko.observable(null),
        LjvAtendimentoVendedorIdAtendimento_begin: ko.observable(null), LjvAtendimentoVendedorIdAtendimento_end: ko.observable(null),
        LjvAtendimentoVendedorValorComissao_begin: ko.observable(null), LjvAtendimentoVendedorValorComissao_end: ko.observable(null),
        LjvAtendimentoVendedorValorCupomFiscal_begin: ko.observable(null), LjvAtendimentoVendedorValorCupomFiscal_end: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.LjvAtendimentoVendedorDataAtendimento_typeRange('R'); entitySearchRange.LjvAtendimentoVendedorDataAtendimento_begin(null); entitySearchRange.LjvAtendimentoVendedorDataAtendimento_end(null); entitySearchRange.LjvAtendimentoVendedorDataAtendimento_predefFilter([]); entitySearchRange.LjvAtendimentoVendedorDataAtendimento_predefValue(null);
        entitySearchRange.LjvAtendimentoVendedorIdAtendimento_begin(null); entitySearchRange.LjvAtendimentoVendedorIdAtendimento_end(null);
        entitySearchRange.LjvAtendimentoVendedorValorComissao_begin(null); entitySearchRange.LjvAtendimentoVendedorValorComissao_end(null);
        entitySearchRange.LjvAtendimentoVendedorValorCupomFiscal_begin(null); entitySearchRange.LjvAtendimentoVendedorValorCupomFiscal_end(null);
    };
    entitySearchRange.has_LjvAtendimentoVendedorIdAtendimento = ko.computed(function(){ return (entitySearchRange.LjvAtendimentoVendedorIdAtendimento_begin() != null || entitySearchRange.LjvAtendimentoVendedorIdAtendimento_end() != null); });
    entitySearchRange.has_LjvAtendimentoVendedorValorComissao = ko.computed(function(){ return (entitySearchRange.LjvAtendimentoVendedorValorComissao_begin() != null || entitySearchRange.LjvAtendimentoVendedorValorComissao_end() != null); });
    entitySearchRange.has_LjvAtendimentoVendedorValorCupomFiscal = ko.computed(function(){ return (entitySearchRange.LjvAtendimentoVendedorValorCupomFiscal_begin() != null || entitySearchRange.LjvAtendimentoVendedorValorCupomFiscal_end() != null); });
    entitySearchRange.has_LjvAtendimentoVendedorDataAtendimento = ko.computed(function(){ return (entitySearchRange.LjvAtendimentoVendedorDataAtendimento_typeRange() == 'R' && (entitySearchRange.LjvAtendimentoVendedorDataAtendimento_begin() != null || entitySearchRange.LjvAtendimentoVendedorDataAtendimento_end() != null) || (entitySearchRange.LjvAtendimentoVendedorDataAtendimento_typeRange() == 'P' && entitySearchRange.LjvAtendimentoVendedorDataAtendimento_predefFilter().length > 0)); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#DashBoard_Atendimento_Vendedor_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var projectTemplateFiles = [
            {
                pivotName: 'LjvAtendimentoVendedor',
                layoutFullName: getSPAVersion() + '/pivotTableLayouts/',
                name: '(Nenhum)',
                viewName: 'DashBoard_Atendimento_Vendedor',
    ...
    var getLayoutsFiles = function () {
        ...
        dataContext.getPivotLayouts(
        {
            ...
            viewName: 'DashBoard_Atendimento_Vendedor',
            pivotName: 'LjvAtendimentoVendedor',
            pivotDataSource: 'LjvAtendimentoVendedor'
        },
            function (data) {
    ...
    var vm = {
            ...
            rootBmTypeName: 'LJV_ATENDIMENTO_VENDEDOR',
            rootDataTypeName: 'LjvAtendimentoVendedor',
            ...
            createLjvAtendimentoVendedor: createLjvAtendimentoVendedor,
            createAndNotifyLjvAtendimentoVendedor: createAndNotifyLjvAtendimentoVendedor,
            ...
            __moduleId__: 'pkg_linx-dashboard-spa/viewmodels/DashBoard_Atendimento_Vendedor'
        };
