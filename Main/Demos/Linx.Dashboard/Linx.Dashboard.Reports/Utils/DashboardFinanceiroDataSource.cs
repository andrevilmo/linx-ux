namespace Linx.Dashboard.Reports {
    using System;
    using System.Linq;
    using System.Text;
    ...
    using Linx.Dashboard.DashboardFinanceiro;
    using System.Net.Http;
    using System.Net.Http.Headers;
    
    
    public partial class DashboardFinanceiroDataSource {
        
        private string url = "LinxDashboardFinanceiro/";
        
        public Linx.Dashboard.DashboardFinanceiro.DashboardFinanceiroDomainService _dashboardFinanceiroContext;
        ...
        private IEnumerable<LjvAtendimento> GetLocalLjvAtendimento(Telerik.Reporting.Processing.Report report) {
            IEnumerable<LjvAtendimento> result = default(IEnumerable<LjvAtendimento>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_dashboardFinanceiroContext == null) _dashboardFinanceiroContext = new Linx.Dashboard.DashboardFinanceiro.DashboardFinanceiroDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(LjvAtendimento), new string[] { });
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(LjvAtendimento), jEntitySearch, false, false, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._dashboardFinanceiroContext.GetLjvAtendimentoByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._dashboardFinanceiroContext.GetLjvAtendimentoByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<LjvAtendimento> GetLjvAtendimento(object reportItem) {
            ...
            {
                return GetLocalLjvAtendimento(report);
            }
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceAddress = "GetSampleLjvAtendimento?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    ...
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(LjvAtendimento), new string[] { });
                        serviceAddress = "GetLjvAtendimentoByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                    ...
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<LjvAtendimento>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
        ...
        private IEnumerable<LjvAtendimentoVendedor> GetLocalLjvAtendimentoVendedor(Telerik.Reporting.Processing.Report report) {
            IEnumerable<LjvAtendimentoVendedor> result = default(IEnumerable<LjvAtendimentoVendedor>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_dashboardFinanceiroContext == null) _dashboardFinanceiroContext = new Linx.Dashboard.DashboardFinanceiro.DashboardFinanceiroDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(LjvAtendimentoVendedor), new string[] { });
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(LjvAtendimentoVendedor), jEntitySearch, false, false, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._dashboardFinanceiroContext.GetLjvAtendimentoVendedorByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._dashboardFinanceiroContext.GetLjvAtendimentoVendedorByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<LjvAtendimentoVendedor> GetLjvAtendimentoVendedor(object reportItem) {
            var report = (reportItem is Telerik.Reporting.Processing.Report ? reportItem : ((Telerik.Reporting.Processing.ReportItem)(reportItem)).Report) as Telerik.Reporting.Processing.Report;
            IEnumerable<LjvAtendimentoVendedor> result = default(IEnumerable<LjvAtendimentoVendedor>);
            if (report != null && report.Parameters.ContainsKey("CurrentUser") && !report.Parameters["CurrentUser"].Value.IsNullOrEmpty())
            {
                return GetLocalLjvAtendimentoVendedor(report);
            }
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceBus = (report != null && report.Parameters.ContainsKey("ServiceBusUrl") && !report.Parameters["ServiceBusUrl"].Value.IsNullOrEmpty() ? report.Parameters["ServiceBusUrl"].Value.ToString() : "http://localhost:1710/");
                    string serviceAddress = "GetSampleLjvAtendimentoVendedor?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    client.BaseAddress = new Uri(serviceBus + (serviceBus.Right(1) == "/" ? "" : "/") +  url);
                    client.DefaultRequestHeaders.Accept.Clear();
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(LjvAtendimentoVendedor), new string[] { });
                        serviceAddress = "GetLjvAtendimentoVendedorByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", Convert.ToBase64String(System.Text.ASCIIEncoding.ASCII.GetBytes(string.Format("{0}:{1}", userName.Value.Value, password.Value.Value))));
                    }
                    else
                    {
                    ...
                    HttpResponseMessage response = client.GetAsync(serviceAddress).Result;
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<LjvAtendimentoVendedor>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
                        var responseContent = response.Content.ReadAsStringAsync();
                        responseContent.Wait();
