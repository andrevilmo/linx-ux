namespace LinxTraining001.BV.Reports {
    ...
    public partial class tstTableMediasDataSource {
        
        private string url = "LinxTraining001tstTableMedias/";
        
        public LinxTraining001.BV.tstTableMedias.tstTableMediasDomainService _tstTableMediasContext;
        ...
        private IEnumerable<ClientesView> GetLocalClientesView(Telerik.Reporting.Processing.Report report) {
            ...
            if (_tstTableMediasContext == null) _tstTableMediasContext = new LinxTraining001.BV.tstTableMedias.tstTableMediasDomainService(headers) { IsSecure = true };
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._tstTableMediasContext.GetClientesViewByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._tstTableMediasContext.GetClientesViewByEntitySearchNoAssociations(null).ToList();
            if (this.DetailsForLoading != null && this.DetailsForLoading.Length > 0)
            {
               foreach (var entity in result)
               {
                   entity.FillDetails(this._tstTableMediasContext, null, null, this.DetailsForLoading);
               }
            }
            return result;
        }
        ...
        private IEnumerable<VendasView> GetLocalVendasView(Telerik.Reporting.Processing.Report report) {
            ...
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_tstTableMediasContext == null) _tstTableMediasContext = new LinxTraining001.BV.tstTableMedias.tstTableMediasDomainService(headers) { IsSecure = true };
            result = this._tstTableMediasContext.GetVendasViewByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<VendasViewParentComposition> GetLocalVendasViewParentComposition(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._tstTableMediasContext.GetVendasViewParentCompositionByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._tstTableMediasContext.GetVendasViewParentCompositionByEntitySearchNoAssociations(null).ToList();
            return result;
        }
