namespace LinxTraining001.BV.Reports {
    ...
    using LinxTraining001.BV.CadastroVendas;
    using System.Net.Http;
    using System.Net.Http.Headers;
    
    
    public partial class CadastroVendasDataSource {
        
        private string url = "LinxTraining001CadastroVendas/";
        
        public LinxTraining001.BV.CadastroVendas.CadastroVendasDomainService _cadastroVendasContext;
        ...
        private IEnumerable<VendasView> GetLocalVendasView(Telerik.Reporting.Processing.Report report) {
            ...
            if (_cadastroVendasContext == null) _cadastroVendasContext = new LinxTraining001.BV.CadastroVendas.CadastroVendasDomainService(headers) { IsSecure = true };
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._cadastroVendasContext.GetVendasViewByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._cadastroVendasContext.GetVendasViewByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<TestePIVOTView> GetLocalTestePIVOTView(Telerik.Reporting.Processing.Report report) {
            IEnumerable<TestePIVOTView> result = default(IEnumerable<TestePIVOTView>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_cadastroVendasContext == null) _cadastroVendasContext = new LinxTraining001.BV.CadastroVendas.CadastroVendasDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(TestePIVOTView), new string[] { });
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(TestePIVOTView), jEntitySearch, false, false, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._cadastroVendasContext.GetTestePIVOTViewByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._cadastroVendasContext.GetTestePIVOTViewByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<TestePIVOTView> GetTestePIVOTView(object reportItem) {
            var report = (reportItem is Telerik.Reporting.Processing.Report ? reportItem : ((Telerik.Reporting.Processing.ReportItem)(reportItem)).Report) as Telerik.Reporting.Processing.Report;
            IEnumerable<TestePIVOTView> result = default(IEnumerable<TestePIVOTView>);
            if (report != null && report.Parameters.ContainsKey("CurrentUser") && !report.Parameters["CurrentUser"].Value.IsNullOrEmpty())
            {
                return GetLocalTestePIVOTView(report);
            }
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceBus = (report != null && report.Parameters.ContainsKey("ServiceBusUrl") && !report.Parameters["ServiceBusUrl"].Value.IsNullOrEmpty() ? report.Parameters["ServiceBusUrl"].Value.ToString() : "http://localhost:1710/");
                    string serviceAddress = "GetSampleTestePIVOTView?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    client.BaseAddress = new Uri(serviceBus + (serviceBus.Right(1) == "/" ? "" : "/") +  url);
                    client.DefaultRequestHeaders.Accept.Clear();
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(TestePIVOTView), new string[] { });
                        serviceAddress = "GetTestePIVOTViewByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", Convert.ToBase64String(System.Text.ASCIIEncoding.ASCII.GetBytes(string.Format("{0}:{1}", userName.Value.Value, password.Value.Value))));
                    }
                    else
                    {
                    ...
                    HttpResponseMessage response = client.GetAsync(serviceAddress).Result;
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<TestePIVOTView>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
                        var responseContent = response.Content.ReadAsStringAsync();
                        responseContent.Wait();
