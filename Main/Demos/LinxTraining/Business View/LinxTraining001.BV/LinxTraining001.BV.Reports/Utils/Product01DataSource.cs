namespace LinxTraining001.BV.Reports {
    ...
    using LinxTraining001.BV.Product01;
    using System.Net.Http;
    using System.Net.Http.Headers;
    
    
    public partial class Product01DataSource {
        
        private string url = "LinxTraining001Product01/";
        
        public LinxTraining001.BV.Product01.Product01DomainService _product01Context;
        ...
        private IEnumerable<ProductView> GetLocalProductView(Telerik.Reporting.Processing.Report report) {
            IEnumerable<ProductView> result = default(IEnumerable<ProductView>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_product01Context == null) _product01Context = new LinxTraining001.BV.Product01.Product01DomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(ProductView), new string[] { });
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(ProductView), jEntitySearch, false, false, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._product01Context.GetProductViewByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._product01Context.GetProductViewByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<ProductView> GetProductView(object reportItem) {
            ...
            {
                return GetLocalProductView(report);
            }
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceAddress = "GetSampleProductView?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    ...
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(ProductView), new string[] { });
                        serviceAddress = "GetProductViewByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                    ...
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<ProductView>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
