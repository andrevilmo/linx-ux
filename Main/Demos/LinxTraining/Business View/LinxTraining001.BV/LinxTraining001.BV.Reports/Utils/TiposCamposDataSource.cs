namespace LinxTraining001.BV.Reports {
    ...
    using LinxTraining001.BV.TiposCampos;
    using System.Net.Http;
    using System.Net.Http.Headers;
    
    
    public partial class TiposCamposDataSource {
        
        private string url = "LinxTraining001TiposCampos/";
        
        public LinxTraining001.BV.TiposCampos.TiposCamposDomainService _tiposCamposContext;
        ...
        private IEnumerable<TiposCamposView> GetLocalTiposCamposView(Telerik.Reporting.Processing.Report report) {
            ...
            if (_tiposCamposContext == null) _tiposCamposContext = new LinxTraining001.BV.TiposCampos.TiposCamposDomainService(headers) { IsSecure = true };
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._tiposCamposContext.GetTiposCamposViewByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._tiposCamposContext.GetTiposCamposViewByEntitySearchNoAssociations(null).ToList();
            if (this.DetailsForLoading != null && this.DetailsForLoading.Length > 0)
            {
               foreach (var entity in result)
               {
                   entity.FillDetails(this._tiposCamposContext, null, null, this.DetailsForLoading);
               }
            }
            return result;
        }
        ...
        private IEnumerable<TiposCamposFilhaView> GetLocalTiposCamposFilhaView(Telerik.Reporting.Processing.Report report) {
            ...
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_tiposCamposContext == null) _tiposCamposContext = new LinxTraining001.BV.TiposCampos.TiposCamposDomainService(headers) { IsSecure = true };
            result = this._tiposCamposContext.GetTiposCamposFilhaViewByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<TiposCamposFilhaViewParentComposition> GetLocalTiposCamposFilhaViewParentComposition(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._tiposCamposContext.GetTiposCamposFilhaViewParentCompositionByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._tiposCamposContext.GetTiposCamposFilhaViewParentCompositionByEntitySearchNoAssociations(null).ToList();
            return result;
        }
