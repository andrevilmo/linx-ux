namespace LinxTraining001.BV.Reports {
    ...
    using LinxTraining001.BV.TestePivotGrid;
    using System.Net.Http;
    using System.Net.Http.Headers;
    
    
    public partial class TestePivotGridDataSource {
        
        private string url = "LinxTraining001TestePivotGrid/";
        
        public LinxTraining001.BV.TestePivotGrid.TestePivotGridDomainService _testePivotGridContext;
        ...
        private IEnumerable<PivoGridOlap> GetLocalPivoGridOlap(Telerik.Reporting.Processing.Report report) {
            IEnumerable<PivoGridOlap> result = default(IEnumerable<PivoGridOlap>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_testePivotGridContext == null) _testePivotGridContext = new LinxTraining001.BV.TestePivotGrid.TestePivotGridDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(PivoGridOlap), new string[] { });
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(PivoGridOlap), jEntitySearch, false, false, true);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._testePivotGridContext.GetPivoGridOlapByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._testePivotGridContext.GetPivoGridOlapByEntitySearchNoAssociations(null).ToList();
            if (this.DetailsForLoading != null && this.DetailsForLoading.Length > 0)
            {
               foreach (var entity in result)
               {
                   entity.FillDetails(this._testePivotGridContext, null, null, this.DetailsForLoading);
               }
            }
            return result;
        }
        
        public virtual IEnumerable<PivoGridOlap> GetPivoGridOlap(object reportItem) {
            ...
            {
                return GetLocalPivoGridOlap(report);
            }
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceAddress = "GetSamplePivoGridOlap?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    ...
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(PivoGridOlap), new string[] { });
                        serviceAddress = "GetPivoGridOlapByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                    ...
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<PivoGridOlap>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
        ...
        private IEnumerable<PivotGridOlapFilha> GetLocalPivotGridOlapFilha(Telerik.Reporting.Processing.Report report) {
            IEnumerable<PivotGridOlapFilha> result = default(IEnumerable<PivotGridOlapFilha>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_testePivotGridContext == null) _testePivotGridContext = new LinxTraining001.BV.TestePivotGrid.TestePivotGridDomainService(headers) { IsSecure = true };
            result = this._testePivotGridContext.GetPivotGridOlapFilhaByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<PivotGridOlapFilha> GetPivotGridOlapFilha(object reportItem) {
            var report = (reportItem is Telerik.Reporting.Processing.Report ? reportItem : ((Telerik.Reporting.Processing.ReportItem)(reportItem)).Report) as Telerik.Reporting.Processing.Report;
            IEnumerable<PivotGridOlapFilha> result = default(IEnumerable<PivotGridOlapFilha>);
            if (report != null && report.Parameters.ContainsKey("CurrentUser") && !report.Parameters["CurrentUser"].Value.IsNullOrEmpty())
            {
                return GetLocalPivotGridOlapFilha(report);
            }
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceBus = (report != null && report.Parameters.ContainsKey("ServiceBusUrl") && !report.Parameters["ServiceBusUrl"].Value.IsNullOrEmpty() ? report.Parameters["ServiceBusUrl"].Value.ToString() : "http://localhost:1710/");
                    string serviceAddress = "GetSamplePivotGridOlapFilha?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    client.BaseAddress = new Uri(serviceBus + (serviceBus.Right(1) == "/" ? "" : "/") +  url);
                    client.DefaultRequestHeaders.Accept.Clear();
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(PivotGridOlapFilha), new string[] { });
                        serviceAddress = "GetPivotGridOlapFilhaByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", Convert.ToBase64String(System.Text.ASCIIEncoding.ASCII.GetBytes(string.Format("{0}:{1}", userName.Value.Value, password.Value.Value))));
                    }
                    else
                    {
                    ...
                    HttpResponseMessage response = client.GetAsync(serviceAddress).Result;
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<PivotGridOlapFilha>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
                        var responseContent = response.Content.ReadAsStringAsync();
                        responseContent.Wait();
