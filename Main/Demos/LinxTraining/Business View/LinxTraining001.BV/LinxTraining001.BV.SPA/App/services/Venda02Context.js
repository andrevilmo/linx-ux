...
var result = function () {
    ...
    var getDataFeedUrl = function() {
       return getServiceAddress('LinxTraining001Venda02OData');
    };
    ...
    metadataInfo['ClientesView'] = [
        ...
        { key: 'IDClientes', isDomain: false, domainName: '', lookupPropertyName: '', lookupVisibleColumns: '', isRequired: true, maxLength: 36, isPartOfKey: false, headerText: 'ID Clientes', width: '250px', dataType: 'string', format: '', hidden: false, unbound: false, group: null },
    ...
    var ClientesViewInitializer = function (ownerReference, isPOCO) {
       ...
       ownerReference.setDefaults = function () {
           setAbsoluteValue(ownerReference, 'IDClientes', getNewGuid());
       };
          ...
          if (!ownerReference.VendasViewIsLoaded) {
            ...
            if (isNullOrEmpty(detailName) || detailName === 'VendasView') {
              ...
              if ((force || !ownerReference.VendasViewList || ownerReference.VendasViewList().length === 0) && (!isNullOrEmpty(getAbsoluteValue(ownerReference.IDClientes)))) {
                ...
                manager.executeQuery(navQuery).then(function (data) { if (ownerReference.isPOCO) { for (var idx = 0; idx < data.results.length; idx++) { initializePOCO(data.results[idx], 'VendasView'); data.results[idx].ClientesView = ko.observable(ownerReference); } ownerReference.VendasViewList(data.results); }  for (var idx = 0; idx < data.results.length; idx++) { setAbsoluteValue(data.results[idx], 'EntityParentUniqueKey', getAbsoluteValue(ownerReference.EntityUniqueKey)); if (!ownerReference.isPOCO) { data.results[idx].ClientesView(ownerReference); data.results[idx].entityAspect.entityState === breeze.EntityState.Unchanged; } }
    ...
    var VendasViewInitializer = function (ownerReference, isPOCO) {
       ...
       ownerReference.setDefaults = function () {
           setAbsoluteValue(ownerReference, 'IDClientes', getNewGuid());
           ownerReference.executeLookUp('','IDClientes', null, vm, getNewGuid(), null);
           setAbsoluteValue(ownerReference, 'IDClientes', getAbsoluteValue(getAbsoluteValue(ownerReference.ClientesView).IDClientes));
       };
       ownerReference.delete = function() {
           if (ownerReference.isDetached()) {
               app.showMessage('A informação selecionada não pode ser excluída!', 'Alerta', ['Ok']);
    ...
    var getVendasViewByEntitySearchNoAssociations = function (jEntitySearch, skip, take, returnInlineCount, preserveCurrentState, noTracking, orderByDef) {
        ...
        .orderBy((isNullOrEmpty(orderByDef) ? 'Data asc' : orderByDef))
        ;
    ...
    // Define that the event name is 'Venda02Context_DataUpdate'.
    var contextUpdtEvt = 'Venda02Context_DataUpdate_' + getNewGuid();
    ...
    var saveChanges = function(saveSucceeded, saveFailed, fin, saveNoTRack) {
        if (saveNoTRack === true) {
            ...
            return $.ajax({
               type: 'POST',
               crossDomain: true,
               url: getServiceAddress('LinxTraining001Venda02/Save' + vm.rootDataTypeName),
               globalError: true,
