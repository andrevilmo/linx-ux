...
var complementCtor = function() {
    var complement = {
    isAutomatic: true
    , renderPaiFilhoGrade_dGridVendasView: function(vm) {
    ...
    $('#PaiFilhoGrade_dGridVendasView_headers').live('focus  keydown', function (evt) {
        var keyCode = (window.event) ? evt.which : evt.keyCode;
        if (keyCode == 9) {
            var cols = $('#PaiFilhoGrade_dGridVendasView').igGrid('option', 'columns');
            var firstRow = $('#PaiFilhoGrade_dGridVendasView').data('igGrid').dataSource._dataView[0].RowDataId;
            clear = vm.status() === 'C';
            if (vm.status() == 'C')
                $('#PaiFilhoGrade_dGridVendasView').igGridUpdating('startEdit', firstRow, 0);
            else {
                ...
                cols.some(function (entry) {
                    if (entry.key != 'RowDataId' && !entry.hidden) {
                        if (verifyCanEditCol(entry.key, clear, entity)) {
                            $('#PaiFilhoGrade_dGridVendasView').igGridUpdating('startEdit', firstRow, indexColumn);
                            return true;
                        }
                        indexColumn++;
                    }
    ...
    var getVisibleColumns = function(metaDataControl) {
       ...
       var visibleColumns = '';
       if($('#PaiFilhoGrade_dGridVendasView').data('igGrid') == undefined) return '';
       var cols = $('#PaiFilhoGrade_dGridVendasView').igGrid('option', 'columns');
       if (cols) {
    ...
    var itemsSource = { getVisibleColumns: getVisibleColumns, containerId: 'PaiFilhoGrade_dGridVendasView_container', dataBind: function (commitData, force) {
       var grid = $('#PaiFilhoGrade_dGridVendasView');
       ...
       if (!started) {
           ...
           started = true;
           commitData = false;
           $('#PaiFilhoGrade_dGridVendasView_groupbyarea').addClass('hide');
       }
       if (grid.igGridUpdating('isEditing')) {
    ...
    function createDataGrid(grid) {
        grid.igGrid({ height: (getGridHeightSuggested() * 0.7), width: '100%',
            ...
            cellClick: function(evt, ui) {
                 if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                     ...
                     if (e && e.tagName == 'IMG' && vm.status() != 'C')
                     {
                          ...
                          showMultimidia(entity, e, table, key, vm.PaiFilhoGrade());
                     }
                 }
                 if (typeof vm.OnGridClientClick === 'function') {
                     var entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                     vm.OnGridClientClick('PaiFilhoGrade_dGridVendasView', ui.colKey, entity);
                 }
            },
            enableUTCDates: true,
            ...
            features: [
                        { name: 'Paging', type: 'local', pageSizeDropDownLocation: 'inpager', pageSize: 50, pageIndexChanged: function (evt, ui) { if (!$('#PaiFilhoGrade_dGridVendasView').igGridSelection('option', 'multipleSelection')) $('#PaiFilhoGrade_dGridVendasView').igGridSelection('selectRow', 0); selectGridCurrentItem(vm.goToKey, 'RowDataId', ui, vm.currentDataItem().currentVendasView, getDataSource()); } },
                          ...
                          columnSorting: function (evt, ui) { 
                              $.grep(ui.owner.grid._visibleColumnsArray, function (e) { 
                                  if (e.key == ui.columnKey && e.dataType == 'string') 
                                      return $('#PaiFilhoGrade_dGridVendasView').igGridSorting('option', 'caseSensitive', false); 
                                  else if (e.key == ui.columnKey) 
                                      return $('#PaiFilhoGrade_dGridVendasView').igGridSorting('option', 'caseSensitive', true); 
                              }); 
                          } 
            },
                        ...
                        { name: 'Tooltips', columnSettings:[{ columnKey: "Data", allowTooltips: true },{ columnKey: "IDVendas", allowTooltips: true },{ columnKey: "Origem", allowTooltips: true },{ columnKey: "ValorTotal", allowTooltips: true },{ columnKey: "VendaVip", allowTooltips: true },{ columnKey: "IDClientes", allowTooltips: true }] },
                        { name: 'Resizing' }, 
                        { name: 'Hiding', 
                            columnHidden: function (evt, ui) {
                               showMultimidiaLazy('#PaiFilhoGrade_dGridVendasView');
                            },
                       ...
                       ,{ name: 'Updating', horizontalMoveOnEnter: true,
                          ...
                          editMode:'none', /*cell(atual) ou rowedittemplate(template)*/
            ...
            if ((typeof vm.OnDataGridCreated == 'function')){
                vm.OnDataGridCreated('PaiFilhoGrade_dGridVendasView');
            }
        }
        vm.addDataSource({ key: 'PaiFilhoGrade_dGridVendasView', name: 'VendasViewList', itemsSource: itemsSource });
    }


, renderscyPaiFilhoGrade_dGrid: function(vm) {
    var getDataSource = function() {
        var source = null;
        try {
            source = vm.dataView;
    ...
    $('#scyPaiFilhoGrade_dGrid_headers').live('focus  keydown', function (evt) {
        var keyCode = (window.event) ? evt.which : evt.keyCode;
        if (keyCode == 9) {
            var cols = $('#scyPaiFilhoGrade_dGrid').igGrid('option', 'columns');
            var firstRow = $('#scyPaiFilhoGrade_dGrid').data('igGrid').dataSource._dataView[0].RowDataId;
            clear = vm.status() === 'C';
            if (vm.status() == 'C')
                $('#scyPaiFilhoGrade_dGrid').igGridUpdating('startEdit', firstRow, 0);
            else {
                var entity = findElementByKey(getDataSource(), 'RowDataId', firstRow);
                var indexColumn = 0;
                cols.some(function (entry) {
                    if (entry.key != 'RowDataId' && !entry.hidden) {
                        if (verifyCanEditCol(entry.key, clear, entity)) {
                            $('#scyPaiFilhoGrade_dGrid').igGridUpdating('startEdit', firstRow, indexColumn);
                            return true;
                        }
                        indexColumn++;
                    }
    ...
    var getVisibleColumns = function(metaDataControl) {
       if (metaDataControl) return '';
       var visibleColumns = '';
       if($('#scyPaiFilhoGrade_dGrid').data('igGrid') == undefined) return '';
       var cols = $('#scyPaiFilhoGrade_dGrid').igGrid('option', 'columns');
       if (cols) {
         for (var idx = 0; idx < cols.length; idx++) {
             if (cols[idx].hidden != true) visibleColumns += (visibleColumns == '' ? '' : ',') + cols[idx].key;
    ...
    var itemsSource = { getVisibleColumns: getVisibleColumns, containerId: 'scyPaiFilhoGrade_dGrid_container', dataBind: function (commitData, force) {
       var grid = $('#scyPaiFilhoGrade_dGrid');
       if (started && (typeof grid.data('igGridUpdating') === 'undefined')) { started = false; }
       if (commitData && started) {
           if (grid.igGridUpdating('isEditing')) {
               grid.igGrid('commit');
       ...
       if (!started) {
           ...
           started = true;
           commitData = false;
           $('#scyPaiFilhoGrade_dGrid_groupbyarea').addClass('hide');
       }
       if (grid.igGridUpdating('isEditing')) {
            grid.igGridUpdating('endEdit', true);
       }
    ...
    function createDataGrid(grid) {
        grid.igGrid({ height: (vm.isDependentVM() ? (getGridHeightSuggested() * 0.7) : $(window).height() * 0.85), width: '100%',
            ...
            cellClick: function(evt, ui) {
                 ...
                 if (typeof vm.OnGridClientClick === 'function') {
                     var entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                     vm.OnGridClientClick('scyPaiFilhoGrade_dGrid', ui.colKey, entity);
                 }
            },
            enableUTCDates: true,
            featureChooserIconDisplay: 'always',
            ...
            features: [
                        { name: 'Sorting', type: 'local', caseSensitive: true,
                          columnSorting: function (evt, ui) { 
                              $.grep(ui.owner.grid._visibleColumnsArray, function (e) { 
                                  if (e.key == ui.columnKey && e.dataType == 'string') 
                                      return $('#scyPaiFilhoGrade_dGrid').igGridSorting('option', 'caseSensitive', false); 
                                  else if (e.key == ui.columnKey) 
                                      return $('#scyPaiFilhoGrade_dGrid').igGridSorting('option', 'caseSensitive', true); 
                              }); 
                          } 
                        ...
                        { name: 'Tooltips', columnSettings:[{ columnKey: "Nome", allowTooltips: true },{ columnKey: "Tipo", allowTooltips: true },{ columnKey: "IDClientes", allowTooltips: true }] },
                        { name: 'Resizing' }, 
                        { name: 'Hiding', 
                            columnHidden: function (evt, ui) {
                               showMultimidiaLazy('#scyPaiFilhoGrade_dGrid');
                            },
                            columnShown: function (evt, ui) {
                               showMultimidiaLazy('#scyPaiFilhoGrade_dGrid');
                            }
                       ...
                       ,{ name: 'Updating', horizontalMoveOnEnter: true,
                          ...
                          showReadonlyEditors: false,
                          showDoneCancelButtons: false,
                          columnSettings: [{ columnKey: 'Tipo', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('Tipo', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('LXTipoClientes', ''),  textKey: 'name', valueKey: 'id', enableClearButton: true }}, { columnKey: "Nome" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyPaiFilhoGrade_dGrid', 'Nome', ui.oldValue, ui.value);}}, maxLength: 40 } }, { columnKey: "IDClientes" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyPaiFilhoGrade_dGrid', 'IDClientes', ui.oldValue, ui.value);}}, maxLength: 36 } }],
                          ...
                          editCellStarted: function(evt, ui){
                              ...
                              var columns = $('#scyPaiFilhoGrade_dGrid').igGridUpdating('option', 'columnSettings');
                              var currentCol = null;
                              currentRow = ui.rowID;
            ...
            if ((typeof vm.OnDataGridCreated == 'function')){
                vm.OnDataGridCreated('scyPaiFilhoGrade_dGrid');
            }
            grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                if (vm.PaiFilhoGrade().status() === 'Q') vm.PaiFilhoGrade().dataToolbar.viewInfo();
            });
        }
        vm.addDataSource({ key: 'scyPaiFilhoGrade_dGrid', name: 'dataView', itemsSource: itemsSource });
    }
