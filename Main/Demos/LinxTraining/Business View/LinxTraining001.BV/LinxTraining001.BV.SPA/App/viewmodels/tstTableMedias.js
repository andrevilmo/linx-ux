define(['durandal/app', 'pkg_linxtraining001-bv-spa/services/tstTableMediasContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linxtraining001-bv-spa/viewmodels/tstTableMediasComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'tstTableMedias' ]; settings.parentVM.tstTableMedias = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('tstTableMedias', parentVM.internalUIs) === -1) parentVM.internalUIs.push('tstTableMedias');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        if (managerAuth.shellMode == 'PROD') {
           managerAuth.getFormAccess('linxtraining001-bv-spa-tstTableMedias', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: tstTableMedias', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#tstTableMedias_tcClientesViewTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#tstTableMedias_tcClientesViewTabControl');

    complement.rendertstTableMedias_dGridVendasView(vm);

    complement.renderscytstTableMedias_dGrid(vm);
    ...
    var removeFormViewControl = function () {
        var front = $('#tstTableMedias_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#tstTableMedias_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#tstTableMedias_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#tstTableMedias_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootNamespace: 'LinxTraining001.BV.tstTableMedias',
            ...
            __moduleId__: 'pkg_linxtraining001-bv-spa/viewmodels/tstTableMedias'
        };
