...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'TelaNullBanco' ]; settings.parentVM.TelaNullBanco = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('TelaNullBanco', parentVM.internalUIs) === -1) parentVM.internalUIs.push('TelaNullBanco');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        if (managerAuth.shellMode == 'PROD') {
           managerAuth.getFormAccess('linxtraining001-bv-spa-TelaNullBanco', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: TelaNullBanco', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#TelaNullBanco_tcPaiNotNullViewTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#TelaNullBanco_tcPaiNotNullViewTabControl');

    complement.renderTelaNullBanco_dGridFilhaNotNullView(vm);

    complement.renderscyTelaNullBanco_dGrid(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'PaiNotNullView';
    ...
    var visibleColumns = 'Boolean,Byte,DateTime,Decimal,IDPaiNotNull,Int,Long,Short,String,StringChar,StringText';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'PaiNotNullView', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'PaiNotNullView', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('PaiNotNullView', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('PaiNotNullView');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('PaiNotNullView');
        ...
        return dataContext.getPaiNotNullViewByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'PaiNotNullView'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        vm.showProcessing('Pesquisando informações...');
        return dataContext.getPaiNotNullViewByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearPaiNotNullView(getBandeiraRede(), clearComplete);
    ...
    var createPaiNotNullView = function() {
        dataBind('dataView', true);
        var entity = dataContext.createPaiNotNullView();
    ...
    var createAndNotifyPaiNotNullView = function() {
        var entity = createPaiNotNullView();
        notifyPresentation('');
        return entity;
    };
    
    var createFilhaNotNullView = function(parent, noCurrent) {
        dataBind('FilhaNotNullViewList', true);
        var entity = dataContext.createFilhaNotNullView(parent, noCurrent);
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentFilhaNotNullView(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyFilhaNotNullView = function(parent) {
        var entity = createFilhaNotNullView(parent);
        notifyPresentation('FilhaNotNullViewList');
        return entity;
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#TelaNullBanco_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#TelaNullBanco_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#TelaNullBanco_formViewer')[0];
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            PaiNotNullViewDateTime_typeRange: ko.observable('R'), PaiNotNullViewDateTime_begin: ko.observable(null), PaiNotNullViewDateTime_end: ko.observable(null), PaiNotNullViewDateTime_predefFilter: ko.observableArray([]), PaiNotNullViewDateTime_predefValue: ko.observable(null),
        FilhaNotNullViewDateTime_typeRange: ko.observable('R'), FilhaNotNullViewDateTime_begin: ko.observable(null), FilhaNotNullViewDateTime_end: ko.observable(null), FilhaNotNullViewDateTime_predefFilter: ko.observableArray([]), FilhaNotNullViewDateTime_predefValue: ko.observable(null),
        PaiNotNullViewByte_begin: ko.observable(null), PaiNotNullViewByte_end: ko.observable(null),
        PaiNotNullViewDecimal_begin: ko.observable(null), PaiNotNullViewDecimal_end: ko.observable(null),
        PaiNotNullViewIDPaiNotNull_begin: ko.observable(null), PaiNotNullViewIDPaiNotNull_end: ko.observable(null),
        PaiNotNullViewInt_begin: ko.observable(null), PaiNotNullViewInt_end: ko.observable(null),
        PaiNotNullViewLong_begin: ko.observable(null), PaiNotNullViewLong_end: ko.observable(null),
        PaiNotNullViewShort_begin: ko.observable(null), PaiNotNullViewShort_end: ko.observable(null),
        FilhaNotNullViewByte_begin: ko.observable(null), FilhaNotNullViewByte_end: ko.observable(null),
        FilhaNotNullViewDecimal_begin: ko.observable(null), FilhaNotNullViewDecimal_end: ko.observable(null),
        FilhaNotNullViewIDFilhaNotNull_begin: ko.observable(null), FilhaNotNullViewIDFilhaNotNull_end: ko.observable(null),
        FilhaNotNullViewIDPaiNotNull_begin: ko.observable(null), FilhaNotNullViewIDPaiNotNull_end: ko.observable(null),
        FilhaNotNullViewInt_begin: ko.observable(null), FilhaNotNullViewInt_end: ko.observable(null),
        FilhaNotNullViewLong_begin: ko.observable(null), FilhaNotNullViewLong_end: ko.observable(null),
        FilhaNotNullViewShort_begin: ko.observable(null), FilhaNotNullViewShort_end: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.PaiNotNullViewDateTime_typeRange('R'); entitySearchRange.PaiNotNullViewDateTime_begin(null); entitySearchRange.PaiNotNullViewDateTime_end(null); entitySearchRange.PaiNotNullViewDateTime_predefFilter([]); entitySearchRange.PaiNotNullViewDateTime_predefValue(null);
        entitySearchRange.FilhaNotNullViewDateTime_typeRange('R'); entitySearchRange.FilhaNotNullViewDateTime_begin(null); entitySearchRange.FilhaNotNullViewDateTime_end(null); entitySearchRange.FilhaNotNullViewDateTime_predefFilter([]); entitySearchRange.FilhaNotNullViewDateTime_predefValue(null);
        entitySearchRange.PaiNotNullViewByte_begin(null); entitySearchRange.PaiNotNullViewByte_end(null);
        entitySearchRange.PaiNotNullViewDecimal_begin(null); entitySearchRange.PaiNotNullViewDecimal_end(null);
        entitySearchRange.PaiNotNullViewIDPaiNotNull_begin(null); entitySearchRange.PaiNotNullViewIDPaiNotNull_end(null);
        entitySearchRange.PaiNotNullViewInt_begin(null); entitySearchRange.PaiNotNullViewInt_end(null);
        entitySearchRange.PaiNotNullViewLong_begin(null); entitySearchRange.PaiNotNullViewLong_end(null);
        entitySearchRange.PaiNotNullViewShort_begin(null); entitySearchRange.PaiNotNullViewShort_end(null);
        entitySearchRange.FilhaNotNullViewByte_begin(null); entitySearchRange.FilhaNotNullViewByte_end(null);
        entitySearchRange.FilhaNotNullViewDecimal_begin(null); entitySearchRange.FilhaNotNullViewDecimal_end(null);
        entitySearchRange.FilhaNotNullViewIDFilhaNotNull_begin(null); entitySearchRange.FilhaNotNullViewIDFilhaNotNull_end(null);
        entitySearchRange.FilhaNotNullViewIDPaiNotNull_begin(null); entitySearchRange.FilhaNotNullViewIDPaiNotNull_end(null);
        entitySearchRange.FilhaNotNullViewInt_begin(null); entitySearchRange.FilhaNotNullViewInt_end(null);
        entitySearchRange.FilhaNotNullViewLong_begin(null); entitySearchRange.FilhaNotNullViewLong_end(null);
        entitySearchRange.FilhaNotNullViewShort_begin(null); entitySearchRange.FilhaNotNullViewShort_end(null);
    };
    entitySearchRange.has_PaiNotNullViewByte = ko.computed(function(){ return (entitySearchRange.PaiNotNullViewByte_begin() != null || entitySearchRange.PaiNotNullViewByte_end() != null); });
    entitySearchRange.has_PaiNotNullViewDecimal = ko.computed(function(){ return (entitySearchRange.PaiNotNullViewDecimal_begin() != null || entitySearchRange.PaiNotNullViewDecimal_end() != null); });
    entitySearchRange.has_PaiNotNullViewIDPaiNotNull = ko.computed(function(){ return (entitySearchRange.PaiNotNullViewIDPaiNotNull_begin() != null || entitySearchRange.PaiNotNullViewIDPaiNotNull_end() != null); });
    entitySearchRange.has_PaiNotNullViewInt = ko.computed(function(){ return (entitySearchRange.PaiNotNullViewInt_begin() != null || entitySearchRange.PaiNotNullViewInt_end() != null); });
    entitySearchRange.has_PaiNotNullViewLong = ko.computed(function(){ return (entitySearchRange.PaiNotNullViewLong_begin() != null || entitySearchRange.PaiNotNullViewLong_end() != null); });
    entitySearchRange.has_PaiNotNullViewShort = ko.computed(function(){ return (entitySearchRange.PaiNotNullViewShort_begin() != null || entitySearchRange.PaiNotNullViewShort_end() != null); });
    entitySearchRange.has_FilhaNotNullViewByte = ko.computed(function(){ return (entitySearchRange.FilhaNotNullViewByte_begin() != null || entitySearchRange.FilhaNotNullViewByte_end() != null); });
    entitySearchRange.has_FilhaNotNullViewDecimal = ko.computed(function(){ return (entitySearchRange.FilhaNotNullViewDecimal_begin() != null || entitySearchRange.FilhaNotNullViewDecimal_end() != null); });
    entitySearchRange.has_FilhaNotNullViewIDFilhaNotNull = ko.computed(function(){ return (entitySearchRange.FilhaNotNullViewIDFilhaNotNull_begin() != null || entitySearchRange.FilhaNotNullViewIDFilhaNotNull_end() != null); });
    entitySearchRange.has_FilhaNotNullViewIDPaiNotNull = ko.computed(function(){ return (entitySearchRange.FilhaNotNullViewIDPaiNotNull_begin() != null || entitySearchRange.FilhaNotNullViewIDPaiNotNull_end() != null); });
    entitySearchRange.has_FilhaNotNullViewInt = ko.computed(function(){ return (entitySearchRange.FilhaNotNullViewInt_begin() != null || entitySearchRange.FilhaNotNullViewInt_end() != null); });
    entitySearchRange.has_FilhaNotNullViewLong = ko.computed(function(){ return (entitySearchRange.FilhaNotNullViewLong_begin() != null || entitySearchRange.FilhaNotNullViewLong_end() != null); });
    entitySearchRange.has_FilhaNotNullViewShort = ko.computed(function(){ return (entitySearchRange.FilhaNotNullViewShort_begin() != null || entitySearchRange.FilhaNotNullViewShort_end() != null); });
    entitySearchRange.has_PaiNotNullViewDateTime = ko.computed(function(){ return (entitySearchRange.PaiNotNullViewDateTime_typeRange() == 'R' && (entitySearchRange.PaiNotNullViewDateTime_begin() != null || entitySearchRange.PaiNotNullViewDateTime_end() != null) || (entitySearchRange.PaiNotNullViewDateTime_typeRange() == 'P' && entitySearchRange.PaiNotNullViewDateTime_predefFilter().length > 0)); });
    entitySearchRange.has_FilhaNotNullViewDateTime = ko.computed(function(){ return (entitySearchRange.FilhaNotNullViewDateTime_typeRange() == 'R' && (entitySearchRange.FilhaNotNullViewDateTime_begin() != null || entitySearchRange.FilhaNotNullViewDateTime_end() != null) || (entitySearchRange.FilhaNotNullViewDateTime_typeRange() == 'P' && entitySearchRange.FilhaNotNullViewDateTime_predefFilter().length > 0)); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#TelaNullBanco_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: 'PaiNotNull',
            rootDataTypeName: 'PaiNotNullView',
            ...
            createPaiNotNullView: createPaiNotNullView,
            createAndNotifyPaiNotNullView: createAndNotifyPaiNotNullView,
            createFilhaNotNullView: createFilhaNotNullView,
            createAndNotifyFilhaNotNullView: createAndNotifyFilhaNotNullView,
            ...
            __moduleId__: 'pkg_linxtraining001-bv-spa/viewmodels/TelaNullBanco'
        };
