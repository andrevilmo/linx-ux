...
var complementCtor = function() {
    var complement = {
    isAutomatic: true
    , renderTiposCampos_dGridTiposCamposFilhaView: function(vm) {
    ...
    $('#TiposCampos_dGridTiposCamposFilhaView_headers').live('focus  keydown', function (evt) {
        var keyCode = (window.event) ? evt.which : evt.keyCode;
        if (keyCode == 9) {
            var cols = $('#TiposCampos_dGridTiposCamposFilhaView').igGrid('option', 'columns');
            var firstRow = $('#TiposCampos_dGridTiposCamposFilhaView').data('igGrid').dataSource._dataView[0].RowDataId;
            clear = vm.status() === 'C';
            if (vm.status() == 'C')
                $('#TiposCampos_dGridTiposCamposFilhaView').igGridUpdating('startEdit', firstRow, 0);
            else {
                ...
                cols.some(function (entry) {
                    if (entry.key != 'RowDataId' && !entry.hidden) {
                        if (verifyCanEditCol(entry.key, clear, entity)) {
                            $('#TiposCampos_dGridTiposCamposFilhaView').igGridUpdating('startEdit', firstRow, indexColumn);
                            return true;
                        }
                        indexColumn++;
                    }
    ...
    var getVisibleColumns = function(metaDataControl) {
       ...
       var visibleColumns = '';
       if($('#TiposCampos_dGridTiposCamposFilhaView').data('igGrid') == undefined) return '';
       var cols = $('#TiposCampos_dGridTiposCamposFilhaView').igGrid('option', 'columns');
       if (cols) {
    ...
    var itemsSource = { getVisibleColumns: getVisibleColumns, containerId: 'TiposCampos_dGridTiposCamposFilhaView_container', dataBind: function (commitData, force) {
       var grid = $('#TiposCampos_dGridTiposCamposFilhaView');
       ...
       if (!started) {
           ...
           started = true;
           commitData = false;
           $('#TiposCampos_dGridTiposCamposFilhaView_groupbyarea').addClass('hide');
       }
       if (grid.igGridUpdating('isEditing')) {
       ...
       if (vm.status() === 'E') {
          grid.igGridSorting("sortColumn", "IDTiposCamposFilha", "ascending");
       }
    ...
    function createDataGrid(grid) {
        grid.igGrid({ height: (getGridHeightSuggested() * 0.7), width: '100%',
            ...
            cellClick: function(evt, ui) {
                 if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                     ...
                     if (e && e.tagName == 'IMG' && vm.status() != 'C')
                     {
                          ...
                          showMultimidia(entity, e, table, key, vm.TiposCampos());
                     }
                 }
                 if (typeof vm.OnGridClientClick === 'function') {
                     var entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                     vm.OnGridClientClick('TiposCampos_dGridTiposCamposFilhaView', ui.colKey, entity);
                 }
            },
            enableUTCDates: true,
            ...
            columns: [
                ...
                { key: 'String', headerText: 'String', width: '200px', dataType: 'string', format: '', hidden: false, unbound: false, group: null  , formatter: function (val, record) { return  vm.dataDomains.getName('DomainString', val);} },
            ...
            features: [
                        { name: 'Paging', type: 'local', pageSizeDropDownLocation: 'inpager', pageSize: 50, pageIndexChanged: function (evt, ui) { if (!$('#TiposCampos_dGridTiposCamposFilhaView').igGridSelection('option', 'multipleSelection')) $('#TiposCampos_dGridTiposCamposFilhaView').igGridSelection('selectRow', 0); selectGridCurrentItem(vm.goToKey, 'RowDataId', ui, vm.currentDataItem().currentTiposCamposFilhaView, getDataSource()); } },
                          ...
                          columnSorting: function (evt, ui) { 
                              $.grep(ui.owner.grid._visibleColumnsArray, function (e) { 
                                  if (e.key == ui.columnKey && e.dataType == 'string') 
                                      return $('#TiposCampos_dGridTiposCamposFilhaView').igGridSorting('option', 'caseSensitive', false); 
                                  else if (e.key == ui.columnKey) 
                                      return $('#TiposCampos_dGridTiposCamposFilhaView').igGridSorting('option', 'caseSensitive', true); 
                              }); 
                          } 
            },
                        ...
                        { name: 'Tooltips', columnSettings:[{ columnKey: "Boolean", allowTooltips: true },{ columnKey: "DateTime", allowTooltips: true },{ columnKey: "Decimal", allowTooltips: true },{ columnKey: "IDTiposCampos", allowTooltips: true },{ columnKey: "IDTiposCamposFilha", allowTooltips: true },{ columnKey: "Int", allowTooltips: true },{ columnKey: "Long", allowTooltips: true },{ columnKey: "Short", allowTooltips: true },{ columnKey: "String", allowTooltips: true },{ columnKey: "StringChar", allowTooltips: true },{ columnKey: "StringText", allowTooltips: true }] },
                        { name: 'Resizing' }, 
                        { name: 'Hiding', 
                            columnHidden: function (evt, ui) {
                               showMultimidiaLazy('#TiposCampos_dGridTiposCamposFilhaView');
                            },
                       ...
                       ,{ name: 'Updating', horizontalMoveOnEnter: true,
                          ...
                          columnSettings: [{ columnKey: 'DateTime', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid == 'function') {vm.OnPropertyChangeDataGrid('TiposCampos_dGridTiposCamposFilhaView', 'DateTime', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: 'String', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('String', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('DomainString', ''),  textKey: 'name', valueKey: 'id', enableClearButton: true }}, { columnKey: "Decimal" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('TiposCampos_dGridTiposCamposFilhaView', 'Decimal', ui.oldValue, ui.value);}},  maxLength: 11, maxValue: 99999999.99, minValue: 0, dataMode: 'decimal', minDecimals: 2, maxDecimals: 2 } }, { columnKey: "IDTiposCampos" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('TiposCampos_dGridTiposCamposFilhaView', 'IDTiposCampos', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "IDTiposCamposFilha" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('TiposCampos_dGridTiposCamposFilhaView', 'IDTiposCamposFilha', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "Int" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('TiposCampos_dGridTiposCamposFilhaView', 'Int', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "Long" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('TiposCampos_dGridTiposCamposFilhaView', 'Long', ui.oldValue, ui.value);}},  maxLength: 24, maxValue: null, minValue: 0, dataMode: 'long' } }, { columnKey: "Short" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('TiposCampos_dGridTiposCamposFilhaView', 'Short', ui.oldValue, ui.value);}},  maxLength: 6, maxValue: null, minValue: 0, dataMode: 'short' } }, { columnKey: "StringChar" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('TiposCampos_dGridTiposCamposFilhaView', 'StringChar', ui.oldValue, ui.value);}}, maxLength: 10 } }, { columnKey: "StringText" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('TiposCampos_dGridTiposCamposFilhaView', 'StringText', ui.oldValue, ui.value);}}, maxLength: 0 } }],
                          ...
                          editCellStarted: function(evt, ui){
                              ...
                              var columns = $('#TiposCampos_dGridTiposCamposFilhaView').igGridUpdating('option', 'columnSettings');
                              var currentCol = null;
                              currentRow = ui.rowID;
            ...
            if ((typeof vm.OnDataGridCreated == 'function')){
                vm.OnDataGridCreated('TiposCampos_dGridTiposCamposFilhaView');
            }
        }
        vm.addDataSource({ key: 'TiposCampos_dGridTiposCamposFilhaView', name: 'TiposCamposFilhaViewList', itemsSource: itemsSource });
    }


, renderscyTiposCampos_dGrid: function(vm) {
    var getDataSource = function() {
        var source = null;
        try {
            source = vm.dataView;
    ...
    $('#scyTiposCampos_dGrid_headers').live('focus  keydown', function (evt) {
        var keyCode = (window.event) ? evt.which : evt.keyCode;
        if (keyCode == 9) {
            var cols = $('#scyTiposCampos_dGrid').igGrid('option', 'columns');
            var firstRow = $('#scyTiposCampos_dGrid').data('igGrid').dataSource._dataView[0].RowDataId;
            clear = vm.status() === 'C';
            if (vm.status() == 'C')
                $('#scyTiposCampos_dGrid').igGridUpdating('startEdit', firstRow, 0);
            else {
                var entity = findElementByKey(getDataSource(), 'RowDataId', firstRow);
                var indexColumn = 0;
                cols.some(function (entry) {
                    if (entry.key != 'RowDataId' && !entry.hidden) {
                        if (verifyCanEditCol(entry.key, clear, entity)) {
                            $('#scyTiposCampos_dGrid').igGridUpdating('startEdit', firstRow, indexColumn);
                            return true;
                        }
                        indexColumn++;
                    }
    ...
    var getVisibleColumns = function(metaDataControl) {
       if (metaDataControl) return '';
       var visibleColumns = '';
       if($('#scyTiposCampos_dGrid').data('igGrid') == undefined) return '';
       var cols = $('#scyTiposCampos_dGrid').igGrid('option', 'columns');
       if (cols) {
         for (var idx = 0; idx < cols.length; idx++) {
             if (cols[idx].hidden != true) visibleColumns += (visibleColumns == '' ? '' : ',') + cols[idx].key;
    ...
    var itemsSource = { getVisibleColumns: getVisibleColumns, containerId: 'scyTiposCampos_dGrid_container', dataBind: function (commitData, force) {
       var grid = $('#scyTiposCampos_dGrid');
       if (started && (typeof grid.data('igGridUpdating') === 'undefined')) { started = false; }
       if (commitData && started) {
           if (grid.igGridUpdating('isEditing')) {
               grid.igGrid('commit');
       ...
       if (!started) {
           ...
           started = true;
           commitData = false;
           $('#scyTiposCampos_dGrid_groupbyarea').addClass('hide');
       }
       if (grid.igGridUpdating('isEditing')) {
            grid.igGridUpdating('endEdit', true);
       }
    ...
    function makeGauge(val, record, field) {
        var row = 0, value = 0;
        if (record.RowDataId > 0) {
            row = record.RowDataId;
            value = record[field];
            return "<div id='" + row + "' class='gauge' style='width:400px;height:20px;'></div> <script id='script" + row + "'>$('#" + row + "').kendoLinearGauge( {gaugeArea: {background: 'transparent', width:230}, pointer: { value: " + value + ", color: '#8B8386', shape: 'arrow' }, scale: { vertical: false ,line:{visible: false}, labels: {visible: false}, min: " + dadosGaugeQtdeTotal.min + ", max: " + dadosGaugeQtdeTotal.max + ", ranges: " + dadosGaugeQtdeTotal.ranges + " } });</script>"
        }
        return '';
    };
    function createDataGrid(grid) {
        grid.igGrid({ height: (vm.isDependentVM() ? (getGridHeightSuggested() * 0.7) : $(window).height() * 0.85), width: '100%',
            ...
            cellClick: function(evt, ui) {
                 ...
                 if (typeof vm.OnGridClientClick === 'function') {
                     var entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                     vm.OnGridClientClick('scyTiposCampos_dGrid', ui.colKey, entity);
                 }
            },
            enableUTCDates: true,
            featureChooserIconDisplay: 'always',
            ...
            columns: [
                ...
                { key: 'DateTime', headerText: 'DateTime', width: '140px', dataType: 'date', format: 'dd/MM/yyyy', hidden: false, unbound: false, group: null   },
                { key: 'Decimal', headerText: 'Decimal', width: '127px', dataType: 'number', format: 'percent', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                { key: 'IDTiposCampos', headerText: 'ID TiposCampos', width: '218px', dataType: 'number', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                { key: 'Int', headerText: 'Int', width: '130px', dataType: 'number', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                { key: 'Long', headerText: 'Long', width: '250px', dataType: 'number', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                { key: 'Short', headerText: 'Short', width: '101px', dataType: 'number', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
            ...
            features: [
                        { name: 'Sorting', type: 'local', caseSensitive: true,
                          columnSorting: function (evt, ui) { 
                              $.grep(ui.owner.grid._visibleColumnsArray, function (e) { 
                                  if (e.key == ui.columnKey && e.dataType == 'string') 
                                      return $('#scyTiposCampos_dGrid').igGridSorting('option', 'caseSensitive', false); 
                                  else if (e.key == ui.columnKey) 
                                      return $('#scyTiposCampos_dGrid').igGridSorting('option', 'caseSensitive', true); 
                              }); 
                          } 
                        ...
                        { name: 'Tooltips', columnSettings:[{ columnKey: "Boolean", allowTooltips: true },{ columnKey: "Byte", allowTooltips: true },{ columnKey: "DateTime", allowTooltips: true },{ columnKey: "Decimal", allowTooltips: true },{ columnKey: "IDTiposCampos", allowTooltips: true },{ columnKey: "Int", allowTooltips: true },{ columnKey: "Long", allowTooltips: true },{ columnKey: "Short", allowTooltips: true },{ columnKey: "String", allowTooltips: true },{ columnKey: "StringChar", allowTooltips: true },{ columnKey: "StringText", allowTooltips: true }] },
                        { name: 'Resizing' }, 
                        { name: 'Hiding', 
                            columnHidden: function (evt, ui) {
                               showMultimidiaLazy('#scyTiposCampos_dGrid');
                            },
                            columnShown: function (evt, ui) {
                               showMultimidiaLazy('#scyTiposCampos_dGrid');
                            }
                       ...
                       ,{ name: 'Updating', horizontalMoveOnEnter: true,
                          ...
                          showReadonlyEditors: false,
                          showDoneCancelButtons: false,
                          columnSettings: [{ columnKey: 'DateTime', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid == 'function') {vm.OnPropertyChangeDataGrid('scyTiposCampos_dGrid', 'DateTime', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: 'String', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('String', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('TstDomainString', ''),  textKey: 'name', valueKey: 'id', enableClearButton: true }}, { columnKey: "Byte" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTiposCampos_dGrid', 'Byte', ui.oldValue, ui.value);}},  maxLength: 3, maxValue: null, minValue: 0, dataMode: 'byte' } }, { columnKey: "Decimal" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTiposCampos_dGrid', 'Decimal', ui.oldValue, ui.value);}},  maxLength: 11, maxValue: 99999999.99, minValue: 0, dataMode: 'decimal', minDecimals: 2, maxDecimals: 2 } }, { columnKey: "IDTiposCampos" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTiposCampos_dGrid', 'IDTiposCampos', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "Int" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTiposCampos_dGrid', 'Int', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "Long" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTiposCampos_dGrid', 'Long', ui.oldValue, ui.value);}},  maxLength: 24, maxValue: null, minValue: 0, dataMode: 'long' } }, { columnKey: "Short" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTiposCampos_dGrid', 'Short', ui.oldValue, ui.value);}},  maxLength: 6, maxValue: null, minValue: 0, dataMode: 'short' } }, { columnKey: "StringChar" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTiposCampos_dGrid', 'StringChar', ui.oldValue, ui.value);}}, maxLength: 10 } }, { columnKey: "StringText" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTiposCampos_dGrid', 'StringText', ui.oldValue, ui.value);}}, maxLength: 0 } }],
                          rowDeleting: function (evt, ui) {
                              deletedIndex = ui.element.context.rowIndex;
                              var entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowID);
                              if (entity) {
                          ...
                          editCellStarted: function(evt, ui){
                              var lstRefreshDados = null;
                              var columns = $('#scyTiposCampos_dGrid').igGridUpdating('option', 'columnSettings');
                              var currentCol = null;
                              currentRow = ui.rowID;
                              columns.forEach(function (entry, index) {
                                 if (entry.columnKey == ui.columnKey) currentCol = entry;
            ...
            if ((typeof vm.OnDataGridCreated == 'function')){
                vm.OnDataGridCreated('scyTiposCampos_dGrid');
            }
            grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                if (vm.TiposCampos().status() === 'Q') vm.TiposCampos().dataToolbar.viewInfo();
            });
        }
        vm.addDataSource({ key: 'scyTiposCampos_dGrid', name: 'dataView', itemsSource: itemsSource });
    }
