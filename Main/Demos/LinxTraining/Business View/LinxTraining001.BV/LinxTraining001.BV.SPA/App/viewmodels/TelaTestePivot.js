...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'TelaTestePivot' ]; settings.parentVM.TelaTestePivot = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('TelaTestePivot', parentVM.internalUIs) === -1) parentVM.internalUIs.push('TelaTestePivot');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        if (managerAuth.shellMode == 'PROD') {
           managerAuth.getFormAccess('linxtraining001-bv-spa-TelaTestePivot', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: TelaTestePivot', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        complement.renderTelaTestePivot_pivotTestePIVOTView(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TestePIVOTView';
    ...
    var visibleColumns = 'Inteiro,String,Data,Bolean,Decimal';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TestePIVOTView', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TestePIVOTView', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TestePIVOTView', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TestePIVOTView');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TestePIVOTView');
        ...
        return dataContext.getTestePIVOTViewByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TestePIVOTView'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        vm.showProcessing('Pesquisando informações...');
        return dataContext.getTestePIVOTViewByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTestePIVOTView(getBandeiraRede(), clearComplete);
    ...
    var createTestePIVOTView = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTestePIVOTView();
    ...
    var createAndNotifyTestePIVOTView = function() {
        var entity = createTestePIVOTView();
        notifyPresentation('');
        return entity;
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#TelaTestePivot_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#TelaTestePivot_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#TelaTestePivot_formViewer')[0];
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TestePIVOTViewData_typeRange: ko.observable('R'), TestePIVOTViewData_begin: ko.observable(null), TestePIVOTViewData_end: ko.observable(null), TestePIVOTViewData_predefFilter: ko.observableArray([]), TestePIVOTViewData_predefValue: ko.observable(null),
        TestePIVOTViewDecimal_begin: ko.observable(null), TestePIVOTViewDecimal_end: ko.observable(null),
        TestePIVOTViewInteiro_begin: ko.observable(null), TestePIVOTViewInteiro_end: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TestePIVOTViewData_typeRange('R'); entitySearchRange.TestePIVOTViewData_begin(null); entitySearchRange.TestePIVOTViewData_end(null); entitySearchRange.TestePIVOTViewData_predefFilter([]); entitySearchRange.TestePIVOTViewData_predefValue(null);
        entitySearchRange.TestePIVOTViewDecimal_begin(null); entitySearchRange.TestePIVOTViewDecimal_end(null);
        entitySearchRange.TestePIVOTViewInteiro_begin(null); entitySearchRange.TestePIVOTViewInteiro_end(null);
    };
    entitySearchRange.has_TestePIVOTViewDecimal = ko.computed(function(){ return (entitySearchRange.TestePIVOTViewDecimal_begin() != null || entitySearchRange.TestePIVOTViewDecimal_end() != null); });
    entitySearchRange.has_TestePIVOTViewInteiro = ko.computed(function(){ return (entitySearchRange.TestePIVOTViewInteiro_begin() != null || entitySearchRange.TestePIVOTViewInteiro_end() != null); });
    entitySearchRange.has_TestePIVOTViewData = ko.computed(function(){ return (entitySearchRange.TestePIVOTViewData_typeRange() == 'R' && (entitySearchRange.TestePIVOTViewData_begin() != null || entitySearchRange.TestePIVOTViewData_end() != null) || (entitySearchRange.TestePIVOTViewData_typeRange() == 'P' && entitySearchRange.TestePIVOTViewData_predefFilter().length > 0)); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#TelaTestePivot_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var projectTemplateFiles = [
            {
                pivotName: 'TestePIVOTView',
                layoutFullName: getSPAVersion() + '/pivotTableLayouts/',
                name: '(Nenhum)',
                viewName: 'TelaTestePivot',
    ...
    var getLayoutsFiles = function () {
        ...
        dataContext.getPivotLayouts(
        {
            ...
            viewName: 'TelaTestePivot',
            pivotName: 'TestePIVOTView',
            pivotDataSource: 'TestePIVOTView'
        },
            function (data) {
    ...
    var vm = {
            ...
            rootBmTypeName: 'TestePIVOT',
            rootDataTypeName: 'TestePIVOTView',
            rootNamespace: 'LinxTraining001.BV.CadastroVendas',
            ...
            createAndNotifyTestePIVOTView: createAndNotifyTestePIVOTView,
            ...
            __moduleId__: 'pkg_linxtraining001-bv-spa/viewmodels/TelaTestePivot'
        };
