...
var complementCtor = function() {
    var complement = {
    isAutomatic: true
    , renderscyTiposCamposUIExterna_dGrid: function(vm) {
    ...
    $('#scyTiposCamposUIExterna_dGrid_headers').live('focus  keydown', function (evt) {
        var keyCode = (window.event) ? evt.which : evt.keyCode;
        if (keyCode == 9) {
            var cols = $('#scyTiposCamposUIExterna_dGrid').igGrid('option', 'columns');
            var firstRow = $('#scyTiposCamposUIExterna_dGrid').data('igGrid').dataSource._dataView[0].RowDataId;
            clear = vm.status() === 'C';
            if (vm.status() == 'C')
                $('#scyTiposCamposUIExterna_dGrid').igGridUpdating('startEdit', firstRow, 0);
            else {
                ...
                cols.some(function (entry) {
                    if (entry.key != 'RowDataId' && !entry.hidden) {
                        if (verifyCanEditCol(entry.key, clear, entity)) {
                            $('#scyTiposCamposUIExterna_dGrid').igGridUpdating('startEdit', firstRow, indexColumn);
                            return true;
                        }
                        indexColumn++;
                    }
    ...
    var getVisibleColumns = function(metaDataControl) {
       ...
       var visibleColumns = '';
       if($('#scyTiposCamposUIExterna_dGrid').data('igGrid') == undefined) return '';
       var cols = $('#scyTiposCamposUIExterna_dGrid').igGrid('option', 'columns');
       if (cols) {
    ...
    var itemsSource = { getVisibleColumns: getVisibleColumns, containerId: 'scyTiposCamposUIExterna_dGrid_container', dataBind: function (commitData, force) {
       var grid = $('#scyTiposCamposUIExterna_dGrid');
       ...
       if (!started) {
           ...
           started = true;
           commitData = false;
           $('#scyTiposCamposUIExterna_dGrid_groupbyarea').addClass('hide');
       }
       if (grid.igGridUpdating('isEditing')) {
    ...
    function createDataGrid(grid) {
        grid.igGrid({ height: (vm.isDependentVM() ? (getGridHeightSuggested() * 0.7) : $(window).height() * 0.85), width: '100%',
            ...
            cellClick: function(evt, ui) {
                 if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                     ...
                     if (e && e.tagName == 'IMG' && vm.status() != 'C')
                     {
                          ...
                          showMultimidia(entity, e, table, key, vm.TiposCamposUIExterna());
                     }
                 }
                 if (typeof vm.OnGridClientClick === 'function') {
                     var entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                     vm.OnGridClientClick('scyTiposCamposUIExterna_dGrid', ui.colKey, entity);
                 }
            },
            enableUTCDates: true,
            ...
            features: [
                          ...
                          columnSorting: function (evt, ui) { 
                              $.grep(ui.owner.grid._visibleColumnsArray, function (e) { 
                                  if (e.key == ui.columnKey && e.dataType == 'string') 
                                      return $('#scyTiposCamposUIExterna_dGrid').igGridSorting('option', 'caseSensitive', false); 
                                  else if (e.key == ui.columnKey) 
                                      return $('#scyTiposCamposUIExterna_dGrid').igGridSorting('option', 'caseSensitive', true); 
                              }); 
                          } 
                        ...
                        { name: 'Tooltips', columnSettings:[{ columnKey: "Boolean", allowTooltips: true },{ columnKey: "Byte", allowTooltips: true },{ columnKey: "DateTime", allowTooltips: true },{ columnKey: "Decimal", allowTooltips: true },{ columnKey: "Guid", allowTooltips: true },{ columnKey: "IDTiposCampos", allowTooltips: true },{ columnKey: "Int", allowTooltips: true },{ columnKey: "Long", allowTooltips: true },{ columnKey: "Short", allowTooltips: true },{ columnKey: "String", allowTooltips: true },{ columnKey: "StringChar", allowTooltips: true },{ columnKey: "StringText", allowTooltips: true }] },
                        { name: 'Resizing' }, 
                        { name: 'Hiding', 
                            columnHidden: function (evt, ui) {
                               showMultimidiaLazy('#scyTiposCamposUIExterna_dGrid');
                            },
                       ...
                       ,{ name: 'Updating', horizontalMoveOnEnter: true,
                          ...
                          columnSettings: [{ columnKey: 'DateTime', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid == 'function') {vm.OnPropertyChangeDataGrid('scyTiposCamposUIExterna_dGrid', 'DateTime', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: "Byte" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTiposCamposUIExterna_dGrid', 'Byte', ui.oldValue, ui.value);}},  maxLength: 3, maxValue: null, minValue: 0, dataMode: 'byte' } }, { columnKey: "Decimal" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTiposCamposUIExterna_dGrid', 'Decimal', ui.oldValue, ui.value);}},  maxLength: 11, maxValue: 99999999.99, minValue: 0, dataMode: 'decimal', minDecimals: 2, maxDecimals: 2 } }, { columnKey: "Guid" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTiposCamposUIExterna_dGrid', 'Guid', ui.oldValue, ui.value);}}, maxLength: 36 } }, { columnKey: "IDTiposCampos" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTiposCamposUIExterna_dGrid', 'IDTiposCampos', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "Int" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTiposCamposUIExterna_dGrid', 'Int', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "Long" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTiposCamposUIExterna_dGrid', 'Long', ui.oldValue, ui.value);}},  maxLength: 24, maxValue: null, minValue: 0, dataMode: 'long' } }, { columnKey: "Short" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTiposCamposUIExterna_dGrid', 'Short', ui.oldValue, ui.value);}},  maxLength: 6, maxValue: null, minValue: 0, dataMode: 'short' } }, { columnKey: "String" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTiposCamposUIExterna_dGrid', 'String', ui.oldValue, ui.value);}}, maxLength: 10 } }, { columnKey: "StringChar" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTiposCamposUIExterna_dGrid', 'StringChar', ui.oldValue, ui.value);}}, maxLength: 10 } }, { columnKey: "StringText" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTiposCamposUIExterna_dGrid', 'StringText', ui.oldValue, ui.value);}}, maxLength: 0 } }],
                          ...
                          editCellStarted: function(evt, ui){
                              ...
                              var columns = $('#scyTiposCamposUIExterna_dGrid').igGridUpdating('option', 'columnSettings');
                              var currentCol = null;
                              currentRow = ui.rowID;
            ...
            if ((typeof vm.OnDataGridCreated == 'function')){
                vm.OnDataGridCreated('scyTiposCamposUIExterna_dGrid');
            }
            grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                if (vm.TiposCamposUIExterna().status() === 'Q') vm.TiposCamposUIExterna().dataToolbar.viewInfo();
            });
        }
        vm.addDataSource({ key: 'scyTiposCamposUIExterna_dGrid', name: 'dataView', itemsSource: itemsSource });
    }
