...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'Sofilterrange' ]; settings.parentVM.Sofilterrange = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('Sofilterrange', parentVM.internalUIs) === -1) parentVM.internalUIs.push('Sofilterrange');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        if (managerAuth.shellMode == 'PROD') {
           managerAuth.getFormAccess('linxtraining001-bv-spa-Sofilterrange', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: Sofilterrange', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        complement.renderscySofilterrange_dGrid(vm);
    ...
    var visibleColumns = 'Data,ValorTotal';
    ...
    var removeFormViewControl = function () {
        var front = $('#Sofilterrange_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#Sofilterrange_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#Sofilterrange_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#Sofilterrange_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            __moduleId__: 'pkg_linxtraining001-bv-spa/viewmodels/Sofilterrange'
        };
