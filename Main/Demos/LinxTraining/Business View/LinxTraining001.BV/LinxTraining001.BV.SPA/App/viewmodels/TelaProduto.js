define(['durandal/app', 'pkg_linxtraining001-bv-spa/services/Product01Context', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linxtraining001-bv-spa/viewmodels/TelaProdutoComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'TelaProduto' ]; settings.parentVM.TelaProduto = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('TelaProduto', parentVM.internalUIs) === -1) parentVM.internalUIs.push('TelaProduto');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        if (managerAuth.shellMode == 'PROD') {
           managerAuth.getFormAccess('linxtraining001-bv-spa-TelaProduto', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: TelaProduto', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        complement.renderscyTelaProduto_dGrid(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'ProductView';
    ...
    var visibleColumns = 'Class,Color,DaysToManufacture,ProductSubcategoryID,UnitMeasureCode,UnitMeasureCode1,DiscontinuedDate,FinishedGoodsFlag,ProductModelID,ListPrice,MakeFlag,ModifiedDate,Name,ProductID,ProductLine,ProductNumber,ReorderPoint,SafetyStockLevel,SellEndDate,SellStartDate,Size,StandardCost,Style,Weight';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'ProductView', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'ProductView', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('ProductView', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('ProductView');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('ProductView');
        ...
        return dataContext.getProductViewByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'ProductView'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        vm.showProcessing('Pesquisando informações...');
        return dataContext.getProductViewByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearProductView(getBandeiraRede(), clearComplete);
    ...
    var createProductView = function() {
        dataBind('dataView', true);
        var entity = dataContext.createProductView();
    ...
    var createAndNotifyProductView = function() {
        var entity = createProductView();
        notifyPresentation('');
        return entity;
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#TelaProduto_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#TelaProduto_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#TelaProduto_formViewer')[0];
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            ProductViewDiscontinuedDate_typeRange: ko.observable('R'), ProductViewDiscontinuedDate_begin: ko.observable(null), ProductViewDiscontinuedDate_end: ko.observable(null), ProductViewDiscontinuedDate_predefFilter: ko.observableArray([]), ProductViewDiscontinuedDate_predefValue: ko.observable(null),
        ProductViewModifiedDate_typeRange: ko.observable('R'), ProductViewModifiedDate_begin: ko.observable(null), ProductViewModifiedDate_end: ko.observable(null), ProductViewModifiedDate_predefFilter: ko.observableArray([]), ProductViewModifiedDate_predefValue: ko.observable(null),
        ProductViewSellEndDate_typeRange: ko.observable('R'), ProductViewSellEndDate_begin: ko.observable(null), ProductViewSellEndDate_end: ko.observable(null), ProductViewSellEndDate_predefFilter: ko.observableArray([]), ProductViewSellEndDate_predefValue: ko.observable(null),
        ProductViewSellStartDate_typeRange: ko.observable('R'), ProductViewSellStartDate_begin: ko.observable(null), ProductViewSellStartDate_end: ko.observable(null), ProductViewSellStartDate_predefFilter: ko.observableArray([]), ProductViewSellStartDate_predefValue: ko.observable(null),
        ProductViewDaysToManufacture_begin: ko.observable(null), ProductViewDaysToManufacture_end: ko.observable(null),
        ProductViewListPrice_begin: ko.observable(null), ProductViewListPrice_end: ko.observable(null),
        ProductViewProductID_begin: ko.observable(null), ProductViewProductID_end: ko.observable(null),
        ProductViewReorderPoint_begin: ko.observable(null), ProductViewReorderPoint_end: ko.observable(null),
        ProductViewSafetyStockLevel_begin: ko.observable(null), ProductViewSafetyStockLevel_end: ko.observable(null),
        ProductViewStandardCost_begin: ko.observable(null), ProductViewStandardCost_end: ko.observable(null),
        ProductViewWeight_begin: ko.observable(null), ProductViewWeight_end: ko.observable(null),
        ProductViewProductModelID: ko.observable(null),
        ProductViewProductSubcategoryID: ko.observable(null),
        ProductViewUnitMeasureCode: ko.observable(null),
        ProductViewUnitMeasureCode1: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.ProductViewDiscontinuedDate_typeRange('R'); entitySearchRange.ProductViewDiscontinuedDate_begin(null); entitySearchRange.ProductViewDiscontinuedDate_end(null); entitySearchRange.ProductViewDiscontinuedDate_predefFilter([]); entitySearchRange.ProductViewDiscontinuedDate_predefValue(null);
        entitySearchRange.ProductViewModifiedDate_typeRange('R'); entitySearchRange.ProductViewModifiedDate_begin(null); entitySearchRange.ProductViewModifiedDate_end(null); entitySearchRange.ProductViewModifiedDate_predefFilter([]); entitySearchRange.ProductViewModifiedDate_predefValue(null);
        entitySearchRange.ProductViewSellEndDate_typeRange('R'); entitySearchRange.ProductViewSellEndDate_begin(null); entitySearchRange.ProductViewSellEndDate_end(null); entitySearchRange.ProductViewSellEndDate_predefFilter([]); entitySearchRange.ProductViewSellEndDate_predefValue(null);
        entitySearchRange.ProductViewSellStartDate_typeRange('R'); entitySearchRange.ProductViewSellStartDate_begin(null); entitySearchRange.ProductViewSellStartDate_end(null); entitySearchRange.ProductViewSellStartDate_predefFilter([]); entitySearchRange.ProductViewSellStartDate_predefValue(null);
        entitySearchRange.ProductViewDaysToManufacture_begin(null); entitySearchRange.ProductViewDaysToManufacture_end(null);
        entitySearchRange.ProductViewListPrice_begin(null); entitySearchRange.ProductViewListPrice_end(null);
        entitySearchRange.ProductViewProductID_begin(null); entitySearchRange.ProductViewProductID_end(null);
        entitySearchRange.ProductViewReorderPoint_begin(null); entitySearchRange.ProductViewReorderPoint_end(null);
        entitySearchRange.ProductViewSafetyStockLevel_begin(null); entitySearchRange.ProductViewSafetyStockLevel_end(null);
        entitySearchRange.ProductViewStandardCost_begin(null); entitySearchRange.ProductViewStandardCost_end(null);
        entitySearchRange.ProductViewWeight_begin(null); entitySearchRange.ProductViewWeight_end(null);
        entitySearchRange.ProductViewProductModelID(null);
        entitySearchRange.ProductViewProductSubcategoryID(null);
        entitySearchRange.ProductViewUnitMeasureCode(null);
        entitySearchRange.ProductViewUnitMeasureCode1(null);
    };
    entitySearchRange.has_ProductViewDaysToManufacture = ko.computed(function(){ return (entitySearchRange.ProductViewDaysToManufacture_begin() != null || entitySearchRange.ProductViewDaysToManufacture_end() != null); });
    entitySearchRange.has_ProductViewListPrice = ko.computed(function(){ return (entitySearchRange.ProductViewListPrice_begin() != null || entitySearchRange.ProductViewListPrice_end() != null); });
    entitySearchRange.has_ProductViewProductID = ko.computed(function(){ return (entitySearchRange.ProductViewProductID_begin() != null || entitySearchRange.ProductViewProductID_end() != null); });
    entitySearchRange.has_ProductViewReorderPoint = ko.computed(function(){ return (entitySearchRange.ProductViewReorderPoint_begin() != null || entitySearchRange.ProductViewReorderPoint_end() != null); });
    entitySearchRange.has_ProductViewSafetyStockLevel = ko.computed(function(){ return (entitySearchRange.ProductViewSafetyStockLevel_begin() != null || entitySearchRange.ProductViewSafetyStockLevel_end() != null); });
    entitySearchRange.has_ProductViewStandardCost = ko.computed(function(){ return (entitySearchRange.ProductViewStandardCost_begin() != null || entitySearchRange.ProductViewStandardCost_end() != null); });
    entitySearchRange.has_ProductViewWeight = ko.computed(function(){ return (entitySearchRange.ProductViewWeight_begin() != null || entitySearchRange.ProductViewWeight_end() != null); });
    entitySearchRange.has_ProductViewDiscontinuedDate = ko.computed(function(){ return (entitySearchRange.ProductViewDiscontinuedDate_typeRange() == 'R' && (entitySearchRange.ProductViewDiscontinuedDate_begin() != null || entitySearchRange.ProductViewDiscontinuedDate_end() != null) || (entitySearchRange.ProductViewDiscontinuedDate_typeRange() == 'P' && entitySearchRange.ProductViewDiscontinuedDate_predefFilter().length > 0)); });
    entitySearchRange.has_ProductViewModifiedDate = ko.computed(function(){ return (entitySearchRange.ProductViewModifiedDate_typeRange() == 'R' && (entitySearchRange.ProductViewModifiedDate_begin() != null || entitySearchRange.ProductViewModifiedDate_end() != null) || (entitySearchRange.ProductViewModifiedDate_typeRange() == 'P' && entitySearchRange.ProductViewModifiedDate_predefFilter().length > 0)); });
    entitySearchRange.has_ProductViewSellEndDate = ko.computed(function(){ return (entitySearchRange.ProductViewSellEndDate_typeRange() == 'R' && (entitySearchRange.ProductViewSellEndDate_begin() != null || entitySearchRange.ProductViewSellEndDate_end() != null) || (entitySearchRange.ProductViewSellEndDate_typeRange() == 'P' && entitySearchRange.ProductViewSellEndDate_predefFilter().length > 0)); });
    entitySearchRange.has_ProductViewSellStartDate = ko.computed(function(){ return (entitySearchRange.ProductViewSellStartDate_typeRange() == 'R' && (entitySearchRange.ProductViewSellStartDate_begin() != null || entitySearchRange.ProductViewSellStartDate_end() != null) || (entitySearchRange.ProductViewSellStartDate_typeRange() == 'P' && entitySearchRange.ProductViewSellStartDate_predefFilter().length > 0)); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#TelaProduto_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: 'Product',
            rootDataTypeName: 'ProductView',
            rootNamespace: 'LinxTraining001.BV.Product01',
            ...
            createAndNotifyProductView: createAndNotifyProductView,
            ...
            __moduleId__: 'pkg_linxtraining001-bv-spa/viewmodels/TelaProduto'
        };
