...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'VendasDetalhes' ]; settings.parentVM.VendasDetalhes = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('VendasDetalhes', parentVM.internalUIs) === -1) parentVM.internalUIs.push('VendasDetalhes');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        if (managerAuth.shellMode == 'PROD') {
           managerAuth.getFormAccess('linxtraining001-bv-spa-VendasDetalhes', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: VendasDetalhes', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#VendasDetalhes_tcVendasViewTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#VendasDetalhes_tcVendasViewTabControl');

    complement.renderVendasDetalhes_dGridVendaDetalheView(vm);

    complement.renderscyVendasDetalhes_dGrid(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'VendasView';
    ...
    var visibleColumns = 'Data,IDClientes,Nome,Origem,ValorTotal,VendaVip';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'VendasView', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'VendasView', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('VendasView', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('VendasView');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('VendasView');
        ...
        return dataContext.getVendasViewByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'VendasView'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        vm.showProcessing('Pesquisando informações...');
        return dataContext.getVendasViewByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearVendasView(getBandeiraRede(), clearComplete);
    ...
    var createVendasView = function() {
        dataBind('dataView', true);
        var entity = dataContext.createVendasView();
    ...
    var createAndNotifyVendasView = function() {
        var entity = createVendasView();
        notifyPresentation('');
        return entity;
    };
    
    var createVendaDetalheView = function(parent, noCurrent) {
        dataBind('VendaDetalheViewList', true);
        var entity = dataContext.createVendaDetalheView(parent, noCurrent);
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentVendaDetalheView(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyVendaDetalheView = function(parent) {
        var entity = createVendaDetalheView(parent);
    ...
    var removeFormViewControl = function () {
        var front = $('#VendasDetalhes_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#VendasDetalhes_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#VendasDetalhes_formViewer')[0];
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            VendasViewData_typeRange: ko.observable('R'), VendasViewData_begin: ko.observable(null), VendasViewData_end: ko.observable(null), VendasViewData_predefFilter: ko.observableArray([]), VendasViewData_predefValue: ko.observable(null),
        VendasViewValorTotal_begin: ko.observable(null), VendasViewValorTotal_end: ko.observable(null),
        VendaDetalheViewPreco_begin: ko.observable(null), VendaDetalheViewPreco_end: ko.observable(null),
        VendaDetalheViewQuantidade_begin: ko.observable(null), VendaDetalheViewQuantidade_end: ko.observable(null),
        VendasViewIDClientes: ko.observable(null),
        VendasViewNome: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.VendasViewData_typeRange('R'); entitySearchRange.VendasViewData_begin(null); entitySearchRange.VendasViewData_end(null); entitySearchRange.VendasViewData_predefFilter([]); entitySearchRange.VendasViewData_predefValue(null);
        entitySearchRange.VendasViewValorTotal_begin(null); entitySearchRange.VendasViewValorTotal_end(null);
        entitySearchRange.VendaDetalheViewPreco_begin(null); entitySearchRange.VendaDetalheViewPreco_end(null);
        entitySearchRange.VendaDetalheViewQuantidade_begin(null); entitySearchRange.VendaDetalheViewQuantidade_end(null);
        entitySearchRange.VendasViewIDClientes(null);
        entitySearchRange.VendasViewNome(null);
    };
    entitySearchRange.has_VendasViewValorTotal = ko.computed(function(){ return (entitySearchRange.VendasViewValorTotal_begin() != null || entitySearchRange.VendasViewValorTotal_end() != null); });
    entitySearchRange.has_VendaDetalheViewPreco = ko.computed(function(){ return (entitySearchRange.VendaDetalheViewPreco_begin() != null || entitySearchRange.VendaDetalheViewPreco_end() != null); });
    entitySearchRange.has_VendaDetalheViewQuantidade = ko.computed(function(){ return (entitySearchRange.VendaDetalheViewQuantidade_begin() != null || entitySearchRange.VendaDetalheViewQuantidade_end() != null); });
    entitySearchRange.has_VendasViewData = ko.computed(function(){ return (entitySearchRange.VendasViewData_typeRange() == 'R' && (entitySearchRange.VendasViewData_begin() != null || entitySearchRange.VendasViewData_end() != null) || (entitySearchRange.VendasViewData_typeRange() == 'P' && entitySearchRange.VendasViewData_predefFilter().length > 0)); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#VendasDetalhes_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: 'Vendas',
            rootDataTypeName: 'VendasView',
            rootNamespace: 'LinxTraining001.BV.DetalhamentoVenda',
            ...
            createAndNotifyVendaDetalheView: createAndNotifyVendaDetalheView,
            ...
            __moduleId__: 'pkg_linxtraining001-bv-spa/viewmodels/VendasDetalhes'
        };
