define(['durandal/app', 'pkg_linxtraining001-bv-spa/services/TiposCamposFilhaSingleContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linxtraining001-bv-spa/viewmodels/TiposCamposFilhaSingleUIComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    var activate = function (settings, querystring) {
      ...
      vm.TiposCamposFilhaSingleUI = getVM;
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'TiposCamposFilhaSingleUI' ]; settings.parentVM.TiposCamposFilhaSingleUI = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.TiposCamposFilhaSingleUI = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('TiposCamposFilhaSingleUI', parentVM.internalUIs) === -1) parentVM.internalUIs.push('TiposCamposFilhaSingleUI');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        if (managerAuth.shellMode == 'PROD') {
           managerAuth.getFormAccess('linxtraining001-bv-spa-TiposCamposFilhaSingleUI', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: TiposCamposFilhaSingleUI', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        complement.renderscyTiposCamposFilhaSingleUI_dGrid(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TiposCamposFilhaView';
    ...
    var visibleColumns = 'String,StringText,StringChar,Short,Long,Int,IDTiposCamposFilha,IDTiposCampos,Guid,Decimal,DateTime,Byte,Boolean';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TiposCamposFilhaView', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TiposCamposFilhaView', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TiposCamposFilhaView', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TiposCamposFilhaView');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TiposCamposFilhaView');
        ...
        return dataContext.getTiposCamposFilhaViewByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TiposCamposFilhaView'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        vm.showProcessing('Pesquisando informações...');
        return dataContext.getTiposCamposFilhaViewByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTiposCamposFilhaView(getBandeiraRede(), clearComplete);
    ...
    var createTiposCamposFilhaView = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTiposCamposFilhaView();
    ...
    var createAndNotifyTiposCamposFilhaView = function() {
        var entity = createTiposCamposFilhaView();
        notifyPresentation('');
        return entity;
    };
    ...
    var addNew = function (parentEntity) {
        ...
        goToItem(createTiposCamposFilhaView());
        editInnerUIs();
        dataBind();
    };
    var remove = function () {
    ...
    var removeFormViewControl = function () {
        var front = $('#TiposCamposFilhaSingleUI_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#TiposCamposFilhaSingleUI_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#TiposCamposFilhaSingleUI_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#TiposCamposFilhaSingleUI_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            dataView: dataView,
            viewName: 'TiposCamposFilhaSingleUI',
            ...
            rootBmTypeName: 'TiposCamposFilha',
            rootDataTypeName: 'TiposCamposFilhaView',
            rootNamespace: 'LinxTraining001.BV.TiposCamposFilhaSingle',
            ...
            __moduleId__: 'pkg_linxtraining001-bv-spa/viewmodels/TiposCamposFilhaSingleUI'
        };
