...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'TiposCampos' ]; settings.parentVM.TiposCampos = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('TiposCampos', parentVM.internalUIs) === -1) parentVM.internalUIs.push('TiposCampos');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        uiSettings = null;
        isDependentVM(false);
        setSecurity(true, true, true, true, true, true, true, true, true, false);
        if (managerAuth.shellMode == 'PROD') {
           managerAuth.getFormAccess('linxtraining001-bv-spa-TiposCampos', function (data) {
              if (data && !data.AcessoTotal) {
                  setSecurity(data.Incluir, true, data.PesquisaEspecial, data.Excluir, data.Alterar, data.Layout, true, data.Imprimir, data.Pesquisar, false);
              }
           }, logger);
        }
        else if (managerAuth.shellMode == 'DEV') {
              if (managerAuth.formAccess && !managerAuth.formAccess.AcessoTotal) {
                  setSecurity(managerAuth.formAccess.canAddNew, true, managerAuth.formAccess.canCustomSearch, managerAuth.formAccess.canDelete, managerAuth.formAccess.canEdit, managerAuth.formAccess.canLayout, true, managerAuth.formAccess.canPrint, managerAuth.formAccess.canSearch, false);
              }
        }
    };
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: TiposCampos', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#TiposCampos_tcTiposCamposViewTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#TiposCampos_tcTiposCamposViewTabControl');

    complement.renderTiposCampos_dGridTiposCamposFilhaView(vm);

    complement.renderscyTiposCampos_dGrid(vm);
    ...
    var _canRefreshData = true, _canQuickSearch = true, _canAddNew = true, _canClear = true, _canCustomSearch = true, _canDelete = true, _canEdit = true, _canLayout = true, _canNavigate = true, _canPrint = true, _canSearch = true, _canExport = false;
    ...
    var removeFormViewControl = function () {
        var front = $('#TiposCampos_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#TiposCampos_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#TiposCampos_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#TiposCampos_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            __moduleId__: 'pkg_linxtraining001-bv-spa/viewmodels/TiposCampos'
        };
