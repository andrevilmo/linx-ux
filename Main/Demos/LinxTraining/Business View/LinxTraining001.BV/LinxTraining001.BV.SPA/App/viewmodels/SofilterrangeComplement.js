...
var complementCtor = function() {
    var complement = {
    isAutomatic: true
    , renderscySofilterrange_dGrid: function(vm) {
    ...
    $('#scySofilterrange_dGrid_headers').live('focus  keydown', function (evt) {
        var keyCode = (window.event) ? evt.which : evt.keyCode;
        if (keyCode == 9) {
            var cols = $('#scySofilterrange_dGrid').igGrid('option', 'columns');
            var firstRow = $('#scySofilterrange_dGrid').data('igGrid').dataSource._dataView[0].RowDataId;
            clear = vm.status() === 'C';
            if (vm.status() == 'C')
                $('#scySofilterrange_dGrid').igGridUpdating('startEdit', firstRow, 0);
            else {
                ...
                cols.some(function (entry) {
                    if (entry.key != 'RowDataId' && !entry.hidden) {
                        if (verifyCanEditCol(entry.key, clear, entity)) {
                            $('#scySofilterrange_dGrid').igGridUpdating('startEdit', firstRow, indexColumn);
                            return true;
                        }
                        indexColumn++;
                    }
    ...
    var getVisibleColumns = function(metaDataControl) {
       ...
       var visibleColumns = '';
       if($('#scySofilterrange_dGrid').data('igGrid') == undefined) return '';
       var cols = $('#scySofilterrange_dGrid').igGrid('option', 'columns');
       if (cols) {
    ...
    var itemsSource = { getVisibleColumns: getVisibleColumns, containerId: 'scySofilterrange_dGrid_container', dataBind: function (commitData, force) {
       var grid = $('#scySofilterrange_dGrid');
       ...
       if (!started) {
           ...
           started = true;
           commitData = false;
           $('#scySofilterrange_dGrid_groupbyarea').addClass('hide');
       }
       if (grid.igGridUpdating('isEditing')) {
    ...
    function createDataGrid(grid) {
        grid.igGrid({ height: (vm.isDependentVM() ? (getGridHeightSuggested() * 0.7) : $(window).height() * 0.85), width: '100%',
            ...
            cellClick: function(evt, ui) {
                 if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                     ...
                     if (e && e.tagName == 'IMG' && vm.status() != 'C')
                     {
                          ...
                          showMultimidia(entity, e, table, key, vm.Sofilterrange());
                     }
                 }
                 if (typeof vm.OnGridClientClick === 'function') {
                     var entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                     vm.OnGridClientClick('scySofilterrange_dGrid', ui.colKey, entity);
                 }
            },
            enableUTCDates: true,
            ...
            features: [
                          ...
                          columnSorting: function (evt, ui) { 
                              $.grep(ui.owner.grid._visibleColumnsArray, function (e) { 
                                  if (e.key == ui.columnKey && e.dataType == 'string') 
                                      return $('#scySofilterrange_dGrid').igGridSorting('option', 'caseSensitive', false); 
                                  else if (e.key == ui.columnKey) 
                                      return $('#scySofilterrange_dGrid').igGridSorting('option', 'caseSensitive', true); 
                              }); 
                          } 
                        ...
                        { name: 'Tooltips', columnSettings:[{ columnKey: "Data", allowTooltips: true },{ columnKey: "ValorTotal", allowTooltips: true }] },
                        { name: 'Resizing' }, 
                        { name: 'Hiding', 
                            columnHidden: function (evt, ui) {
                               showMultimidiaLazy('#scySofilterrange_dGrid');
                            },
                       ...
                       ,{ name: 'Updating', horizontalMoveOnEnter: true,
                          ...
                          columnSettings: [{ columnKey: 'Data', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid == 'function') {vm.OnPropertyChangeDataGrid('scySofilterrange_dGrid', 'Data', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: "ValorTotal" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scySofilterrange_dGrid', 'ValorTotal', ui.oldValue, ui.value);}},  maxLength: 11, maxValue: 99999999.99, minValue: 0, dataMode: 'decimal', minDecimals: 2, maxDecimals: 2 } }],
                          ...
                          editCellStarted: function(evt, ui){
                              ...
                              var columns = $('#scySofilterrange_dGrid').igGridUpdating('option', 'columnSettings');
                              var currentCol = null;
                              currentRow = ui.rowID;
            ...
            if ((typeof vm.OnDataGridCreated == 'function')){
                vm.OnDataGridCreated('scySofilterrange_dGrid');
            }
            grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                if (vm.Sofilterrange().status() === 'Q') vm.Sofilterrange().dataToolbar.viewInfo();
            });
        }
        vm.addDataSource({ key: 'scySofilterrange_dGrid', name: 'dataView', itemsSource: itemsSource });
    }
