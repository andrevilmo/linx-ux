define(['durandal/app', 'pkg_linxtraining001-bv-spa/services/TiposCamposContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linxtraining001-bv-spa/viewmodels/PivotTiposCamposComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    isBusy.subscribe(function (newValue) {
        if ($(".page-container").html() == undefined || $(".page-container").html().length == 0)
            return;
        if (typeof newValue === 'undefined') newValue = isBusy();
        if (newValue) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'PivotTiposCampos' ]; settings.parentVM.PivotTiposCampos = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.PivotTiposCampos = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('PivotTiposCampos', parentVM.internalUIs) === -1) parentVM.internalUIs.push('PivotTiposCampos');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        if (managerAuth.shellMode == 'PROD') {
           managerAuth.getFormAccess('linxtraining001-bv-spa-PivotTiposCampos', function (data) {
              ...
              if (managerAuth.formAccess && !managerAuth.formAccess.AcessoTotal) {
                  setSecurity(managerAuth.formAccess.canAddNew, true, managerAuth.formAccess.canCustomSearch, managerAuth.formAccess.canDelete, managerAuth.formAccess.canEdit, managerAuth.formAccess.canLayout, true, managerAuth.formAccess.canPrint, managerAuth.formAccess.canSearch, managerAuth.formAccess.canExport);
              }
        }
    };
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: PivotTiposCampos', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        complement.renderPivotTiposCampos_pivotTiposCamposView(vm);

    $('#PivotTiposCampos_tcTiposCamposViewTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#PivotTiposCampos_tcTiposCamposViewTabControl');

    complement.renderPivotTiposCampos_pivotTiposCamposFilhaView(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TiposCamposView';
    ...
    var visibleColumns = 'Boolean,Byte,DateTime,Decimal,IDTiposCampos,Int,Long,Short,String,StringChar,StringText';
    ...
    var getKpiTesteRanges = function (succeeded) {
        if (vm.kpiKpiTeste == null) dataContext.getKpiTesteRanges().then(querySucceeded);
        else if (succeeded) succeeded(vm.kpiKpiTeste.ranges, vm.kpiKpiTeste.min, vm.kpiKpiTeste.max);
        return true;
    
        function querySucceeded(data) {
            vm.kpiKpiTeste = { ranges: [], min: 0, max: 0 };
            for (var r in data.results) {
                vm.kpiKpiTeste.ranges.push({ from: data.results[r].StartValue, to: data.results[r].EndValue, color: data.results[r].Color });
                if (vm.kpiKpiTeste.min > data.results[r].StartValue) vm.kpiKpiTeste.min = data.results[r].StartValue;
                if (vm.kpiKpiTeste.max < data.results[r].EndValue) vm.kpiKpiTeste.max = data.results[r].EndValue;
            }
            if (succeeded) succeeded(vm.kpiKpiTeste.ranges, vm.kpiKpiTeste.min, vm.kpiKpiTeste.max);
        }
    };
    var getKpiTesteGaugeGrid = function (succeeded) {
       if (vm.kpiKpiTeste == null) {
           getKpiTesteRanges(succeeded);
       }
       if (vm.kpiKpiTeste != null) {
           var ranges = '';
           for (var i in vm.kpiKpiTeste.ranges) {
               ranges += '{"from" : "' + vm.kpiKpiTeste.ranges[i].from + '", "to" : "' + vm.kpiKpiTeste.ranges[i].to + '" , "color" : "' + vm.kpiKpiTeste.ranges[i].color + '" }';
               if (i < vm.kpiKpiTeste.ranges.length - 1)
                   ranges += ",";
           }
           ranges = "[" + ranges + "]";
           var obj = {min: vm.kpiKpiTeste.min, max: vm.kpiKpiTeste.max, ranges: ranges };
           return obj;
        }
       else {
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TiposCamposView', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TiposCamposView', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TiposCamposView', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TiposCamposView');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TiposCamposView');
        ...
        return dataContext.getTiposCamposViewByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TiposCamposView'); } }
            hasError = false;
    ...
    function goToKey(primaryKey, value, currentElement, viewSource) {
        ...
        for (var idx = 0; idx < viewSource().length; idx++) {
            ...
            if (dataValue == value) {
                if (currentElement) { currentElement().commitDetailsVisualPendings(); currentElement(viewSource()[idx]); currentElement().fillDetails(); } else { if (currentDataItem()) { currentDataItem().commitDetailsVisualPendings(); } goToIndex(idx); }
                break;
            }
        }
    }
    ...
    var refresh = function (curPage, goLast) {
        vm.showProcessing('Pesquisando informações...');
        return dataContext.getTiposCamposViewByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTiposCamposView(getBandeiraRede(), clearComplete);
    ...
    var save = function (isExclusion, externalSaveSucceeded, transactionId, saveCompleteCallback) {
    ...
    var createTiposCamposView = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTiposCamposView();
    ...
    var createAndNotifyTiposCamposView = function() {
        var entity = createTiposCamposView();
        notifyPresentation('');
        return entity;
    };
    
    var createTiposCamposFilhaView = function(parent, noCurrent) {
        dataBind('TiposCamposFilhaViewList', true);
        var entity = dataContext.createTiposCamposFilhaView(parent, noCurrent);
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTiposCamposFilhaView(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTiposCamposFilhaView = function(parent) {
        var entity = createTiposCamposFilhaView(parent);
    ...
    var addNew = function (parentEntity) {
        ...
        goToItem(createTiposCamposView());
        editInnerUIs();
        dataBind();
    };
    var remove = function () {
    ...
    var removeFormViewControl = function () {
        var front = $('#PivotTiposCampos_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#PivotTiposCampos_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#PivotTiposCampos_formViewer')[0];
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TiposCamposViewDateTime_typeRange: ko.observable('R'), TiposCamposViewDateTime_begin: ko.observable(null), TiposCamposViewDateTime_end: ko.observable(null), TiposCamposViewDateTime_predefFilter: ko.observableArray([]), TiposCamposViewDateTime_predefValue: ko.observable(null),
        TiposCamposFilhaViewDateTime_typeRange: ko.observable('R'), TiposCamposFilhaViewDateTime_begin: ko.observable(null), TiposCamposFilhaViewDateTime_end: ko.observable(null), TiposCamposFilhaViewDateTime_predefFilter: ko.observableArray([]), TiposCamposFilhaViewDateTime_predefValue: ko.observable(null),
        TiposCamposViewByte_begin: ko.observable(null), TiposCamposViewByte_end: ko.observable(null),
        TiposCamposViewDecimal_begin: ko.observable(null), TiposCamposViewDecimal_end: ko.observable(null),
        TiposCamposViewIDTiposCampos_begin: ko.observable(null), TiposCamposViewIDTiposCampos_end: ko.observable(null),
        TiposCamposViewInt_begin: ko.observable(null), TiposCamposViewInt_end: ko.observable(null),
        TiposCamposViewLong_begin: ko.observable(null), TiposCamposViewLong_end: ko.observable(null),
        TiposCamposViewShort_begin: ko.observable(null), TiposCamposViewShort_end: ko.observable(null),
        TiposCamposFilhaViewDecimal_begin: ko.observable(null), TiposCamposFilhaViewDecimal_end: ko.observable(null),
        TiposCamposFilhaViewIDTiposCampos_begin: ko.observable(null), TiposCamposFilhaViewIDTiposCampos_end: ko.observable(null),
        TiposCamposFilhaViewIDTiposCamposFilha_begin: ko.observable(null), TiposCamposFilhaViewIDTiposCamposFilha_end: ko.observable(null),
        TiposCamposFilhaViewInt_begin: ko.observable(null), TiposCamposFilhaViewInt_end: ko.observable(null),
        TiposCamposFilhaViewLong_begin: ko.observable(null), TiposCamposFilhaViewLong_end: ko.observable(null),
        TiposCamposFilhaViewShort_begin: ko.observable(null), TiposCamposFilhaViewShort_end: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TiposCamposViewDateTime_typeRange('R'); entitySearchRange.TiposCamposViewDateTime_begin(null); entitySearchRange.TiposCamposViewDateTime_end(null); entitySearchRange.TiposCamposViewDateTime_predefFilter([]); entitySearchRange.TiposCamposViewDateTime_predefValue(null);
        entitySearchRange.TiposCamposFilhaViewDateTime_typeRange('R'); entitySearchRange.TiposCamposFilhaViewDateTime_begin(null); entitySearchRange.TiposCamposFilhaViewDateTime_end(null); entitySearchRange.TiposCamposFilhaViewDateTime_predefFilter([]); entitySearchRange.TiposCamposFilhaViewDateTime_predefValue(null);
        entitySearchRange.TiposCamposViewByte_begin(null); entitySearchRange.TiposCamposViewByte_end(null);
        entitySearchRange.TiposCamposViewDecimal_begin(null); entitySearchRange.TiposCamposViewDecimal_end(null);
        entitySearchRange.TiposCamposViewIDTiposCampos_begin(null); entitySearchRange.TiposCamposViewIDTiposCampos_end(null);
        entitySearchRange.TiposCamposViewInt_begin(null); entitySearchRange.TiposCamposViewInt_end(null);
        entitySearchRange.TiposCamposViewLong_begin(null); entitySearchRange.TiposCamposViewLong_end(null);
        entitySearchRange.TiposCamposViewShort_begin(null); entitySearchRange.TiposCamposViewShort_end(null);
        entitySearchRange.TiposCamposFilhaViewDecimal_begin(null); entitySearchRange.TiposCamposFilhaViewDecimal_end(null);
        entitySearchRange.TiposCamposFilhaViewIDTiposCampos_begin(null); entitySearchRange.TiposCamposFilhaViewIDTiposCampos_end(null);
        entitySearchRange.TiposCamposFilhaViewIDTiposCamposFilha_begin(null); entitySearchRange.TiposCamposFilhaViewIDTiposCamposFilha_end(null);
        entitySearchRange.TiposCamposFilhaViewInt_begin(null); entitySearchRange.TiposCamposFilhaViewInt_end(null);
        entitySearchRange.TiposCamposFilhaViewLong_begin(null); entitySearchRange.TiposCamposFilhaViewLong_end(null);
        entitySearchRange.TiposCamposFilhaViewShort_begin(null); entitySearchRange.TiposCamposFilhaViewShort_end(null);
    };
    entitySearchRange.has_TiposCamposViewByte = ko.computed(function(){ return (entitySearchRange.TiposCamposViewByte_begin() != null || entitySearchRange.TiposCamposViewByte_end() != null); });
    entitySearchRange.has_TiposCamposViewDecimal = ko.computed(function(){ return (entitySearchRange.TiposCamposViewDecimal_begin() != null || entitySearchRange.TiposCamposViewDecimal_end() != null); });
    entitySearchRange.has_TiposCamposViewIDTiposCampos = ko.computed(function(){ return (entitySearchRange.TiposCamposViewIDTiposCampos_begin() != null || entitySearchRange.TiposCamposViewIDTiposCampos_end() != null); });
    entitySearchRange.has_TiposCamposViewInt = ko.computed(function(){ return (entitySearchRange.TiposCamposViewInt_begin() != null || entitySearchRange.TiposCamposViewInt_end() != null); });
    entitySearchRange.has_TiposCamposViewLong = ko.computed(function(){ return (entitySearchRange.TiposCamposViewLong_begin() != null || entitySearchRange.TiposCamposViewLong_end() != null); });
    entitySearchRange.has_TiposCamposViewShort = ko.computed(function(){ return (entitySearchRange.TiposCamposViewShort_begin() != null || entitySearchRange.TiposCamposViewShort_end() != null); });
    entitySearchRange.has_TiposCamposFilhaViewDecimal = ko.computed(function(){ return (entitySearchRange.TiposCamposFilhaViewDecimal_begin() != null || entitySearchRange.TiposCamposFilhaViewDecimal_end() != null); });
    entitySearchRange.has_TiposCamposFilhaViewIDTiposCampos = ko.computed(function(){ return (entitySearchRange.TiposCamposFilhaViewIDTiposCampos_begin() != null || entitySearchRange.TiposCamposFilhaViewIDTiposCampos_end() != null); });
    entitySearchRange.has_TiposCamposFilhaViewIDTiposCamposFilha = ko.computed(function(){ return (entitySearchRange.TiposCamposFilhaViewIDTiposCamposFilha_begin() != null || entitySearchRange.TiposCamposFilhaViewIDTiposCamposFilha_end() != null); });
    entitySearchRange.has_TiposCamposFilhaViewInt = ko.computed(function(){ return (entitySearchRange.TiposCamposFilhaViewInt_begin() != null || entitySearchRange.TiposCamposFilhaViewInt_end() != null); });
    entitySearchRange.has_TiposCamposFilhaViewLong = ko.computed(function(){ return (entitySearchRange.TiposCamposFilhaViewLong_begin() != null || entitySearchRange.TiposCamposFilhaViewLong_end() != null); });
    entitySearchRange.has_TiposCamposFilhaViewShort = ko.computed(function(){ return (entitySearchRange.TiposCamposFilhaViewShort_begin() != null || entitySearchRange.TiposCamposFilhaViewShort_end() != null); });
    entitySearchRange.has_TiposCamposViewDateTime = ko.computed(function(){ return (entitySearchRange.TiposCamposViewDateTime_typeRange() == 'R' && (entitySearchRange.TiposCamposViewDateTime_begin() != null || entitySearchRange.TiposCamposViewDateTime_end() != null) || (entitySearchRange.TiposCamposViewDateTime_typeRange() == 'P' && entitySearchRange.TiposCamposViewDateTime_predefFilter().length > 0)); });
    entitySearchRange.has_TiposCamposFilhaViewDateTime = ko.computed(function(){ return (entitySearchRange.TiposCamposFilhaViewDateTime_typeRange() == 'R' && (entitySearchRange.TiposCamposFilhaViewDateTime_begin() != null || entitySearchRange.TiposCamposFilhaViewDateTime_end() != null) || (entitySearchRange.TiposCamposFilhaViewDateTime_typeRange() == 'P' && entitySearchRange.TiposCamposFilhaViewDateTime_predefFilter().length > 0)); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#PivotTiposCampos_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var getSPAVersion = function(){
       var currentSPAVersion = '';
       var packageName = 'pkg_linxtraining001-bv-spa';
       var currentSpa = _.find(require.s.contexts._.config.packages, function (item) { return item.name == packageName });
       if (currentSpa && currentSpa.location){ 
          currentSPAVersion = currentSpa.location;
          while (currentSPAVersion.startsWith('.') || currentSPAVersion.startsWith('/'))
              currentSPAVersion = currentSPAVersion.substring(1);
       }
       return currentSPAVersion;
    }
    var projectTemplateFiles = [
            {
                pivotName: 'TiposCamposView',
                layoutFullName: getSPAVersion() + '/pivotTableLayouts/',
                name: '(Nenhum)',
                viewName: 'PivotTiposCampos',
                projectName: 'LinxTraining001.BV.SPA',
                content: '',
                id: '',
                selected: true
            }
            ,
            {
                pivotName: 'TiposCamposFilhaView',
                layoutFullName: getSPAVersion() + '/pivotTableLayouts/',
                name: '(Nenhum)',
                viewName: 'PivotTiposCampos',
                projectName: 'LinxTraining001.BV.SPA',
    ...
    var getLayoutsFiles = function () {
    
        showProcessing('Buscando Layouts...');
        
        //Ação realizada para captura de versão disponível da SPA.
        layoutFiles.clear();
        
        projectTemplateFiles.forEach(function(item){layoutFiles.push(item);});
    
        dataContext.getPivotLayouts(
        {
            rootNamespace: 'LinxTraining001.BV.SPA',
            viewName: 'PivotTiposCampos',
            pivotName: 'TiposCamposView',
            pivotDataSource: 'TiposCamposView'
        },
            function (data) {
        ...
        dataContext.getPivotLayouts(
        {
            ...
            viewName: 'PivotTiposCampos',
            pivotName: 'TiposCamposFilhaView',
            pivotDataSource: 'TiposCamposFilhaView'
        },
            function (data) {
                data.forEach(function (item) {
                    var content = '';
    ...
    var vm = {
            ...
            selectedLayoutFile: selectedLayoutFile,
            getLayoutsFiles: getLayoutsFiles,
            ...
            rootBmTypeName: 'TiposCampos',
            rootDataTypeName: 'TiposCamposView',
            rootNamespace: 'LinxTraining001.BV.TiposCampos',
            ...
            createAndNotifyTiposCamposView: createAndNotifyTiposCamposView,
            createTiposCamposFilhaView: createTiposCamposFilhaView,
            createAndNotifyTiposCamposFilhaView: createAndNotifyTiposCamposFilhaView,
            getKpiTesteRanges: getKpiTesteRanges,
            kpiKpiTeste: null,
            getKpiTesteGaugeGrid: getKpiTesteGaugeGrid,
            ...
            __moduleId__: 'pkg_linxtraining001-bv-spa/viewmodels/PivotTiposCampos'
        };
