...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'WizardNRede' ]; settings.parentVM.WizardNRede = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('WizardNRede', parentVM.internalUIs) === -1) parentVM.internalUIs.push('WizardNRede');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        if (managerAuth.shellMode == 'PROD') {
           managerAuth.getFormAccess('linxtraining001-bv-spa-WizardNRede', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: WizardNRede', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#WizardNRede_wizWizard_d71b4906824a4788a656b686296fe25d').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });

    complement.renderWizardNRede_wizWizard_d71b4906824a4788a656b686296fe25d(vm);
    ...
    var visibleColumns = 'Data,IDClientes,IDVendas,Nome,Origem,ValorTotal,VendaVip';
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#WizardNRede_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            __moduleId__: 'pkg_linxtraining001-bv-spa/viewmodels/WizardNRede'
        };
