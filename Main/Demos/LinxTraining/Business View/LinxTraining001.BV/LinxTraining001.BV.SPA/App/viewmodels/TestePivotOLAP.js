define(['durandal/app', 'pkg_linxtraining001-bv-spa/services/TestePivotGridContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linxtraining001-bv-spa/viewmodels/TestePivotOLAPComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'TestePivotOLAP' ]; settings.parentVM.TestePivotOLAP = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('TestePivotOLAP', parentVM.internalUIs) === -1) parentVM.internalUIs.push('TestePivotOLAP');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        if (managerAuth.shellMode == 'PROD') {
           managerAuth.getFormAccess('linxtraining001-bv-spa-TestePivotOLAP', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: TestePivotOLAP', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#TestePivotOLAP_tcPivoGridOlapTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#TestePivotOLAP_tcPivoGridOlapTabControl');

    complement.renderTestePivotOLAP_dGridPivotGridOlapFilha(vm);

    complement.renderscyTestePivotOLAP_dGrid(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'PivoGridOlap';
    ...
    var visibleColumns = 'Ano,VlrBruto';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'PivoGridOlap', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'PivoGridOlap', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getQueryFilter = function (currentDI) {
       ...
       translatedJEntitySearch = common.translateSearch(dataContext, eSearch);
        if (!isNullOrEmpty(eSearch) && eSearch.indexOf('{LinqValidProperties#') === -1) eSearch += getVisibleProperties('dataView');
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('PivoGridOlap', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('PivoGridOlap');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('PivoGridOlap');
        ...
        return dataContext.getPivoGridOlapByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'PivoGridOlap'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        vm.showProcessing('Pesquisando informações...');
        return dataContext.getPivoGridOlapByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearPivoGridOlap(getBandeiraRede(), clearComplete);
    ...
    var createPivoGridOlap = function() {
        dataBind('dataView', true);
        var entity = dataContext.createPivoGridOlap();
    ...
    var createAndNotifyPivoGridOlap = function() {
        var entity = createPivoGridOlap();
        notifyPresentation('');
        return entity;
    };
    
    var createPivotGridOlapFilha = function(parent, noCurrent) {
        dataBind('PivotGridOlapFilhaList', true);
        var entity = dataContext.createPivotGridOlapFilha(parent, noCurrent);
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentPivotGridOlapFilha(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyPivotGridOlapFilha = function(parent) {
        var entity = createPivotGridOlapFilha(parent);
    ...
    var canAddNew = ko.computed(function () { return false; });
    var canRemove = ko.computed(function () { return false; });
    var canEdit = ko.computed(function () { return false; });
    ...
    var removeFormViewControl = function () {
        var front = $('#TestePivotOLAP_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#TestePivotOLAP_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#TestePivotOLAP_formViewer')[0];
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            PivoGridOlapVlrBruto_begin: ko.observable(null), PivoGridOlapVlrBruto_end: ko.observable(null),
        PivotGridOlapFilhaVlrBruto_begin: ko.observable(null), PivotGridOlapFilhaVlrBruto_end: ko.observable(null),
        PivotGridOlapFilhaVlrPago_begin: ko.observable(null), PivotGridOlapFilhaVlrPago_end: ko.observable(null),
        PivoGridOlapAno: ko.observable(null),
        PivotGridOlapFilhaAno: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.PivoGridOlapVlrBruto_begin(null); entitySearchRange.PivoGridOlapVlrBruto_end(null);
        entitySearchRange.PivotGridOlapFilhaVlrBruto_begin(null); entitySearchRange.PivotGridOlapFilhaVlrBruto_end(null);
        entitySearchRange.PivotGridOlapFilhaVlrPago_begin(null); entitySearchRange.PivotGridOlapFilhaVlrPago_end(null);
        entitySearchRange.PivoGridOlapAno(null);
        entitySearchRange.PivotGridOlapFilhaAno(null);
    };
    entitySearchRange.has_PivoGridOlapVlrBruto = ko.computed(function(){ return (entitySearchRange.PivoGridOlapVlrBruto_begin() != null || entitySearchRange.PivoGridOlapVlrBruto_end() != null); });
    entitySearchRange.has_PivotGridOlapFilhaVlrBruto = ko.computed(function(){ return (entitySearchRange.PivotGridOlapFilhaVlrBruto_begin() != null || entitySearchRange.PivotGridOlapFilhaVlrBruto_end() != null); });
    entitySearchRange.has_PivotGridOlapFilhaVlrPago = ko.computed(function(){ return (entitySearchRange.PivotGridOlapFilhaVlrPago_begin() != null || entitySearchRange.PivotGridOlapFilhaVlrPago_end() != null); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#TestePivotOLAP_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: '',
            rootDataTypeName: 'PivoGridOlap',
            rootNamespace: 'LinxTraining001.BV.TestePivotGrid',
            ...
            createAndNotifyPivoGridOlap: createAndNotifyPivoGridOlap,
            createPivotGridOlapFilha: createPivotGridOlapFilha,
            createAndNotifyPivotGridOlapFilha: createAndNotifyPivotGridOlapFilha,
            ...
            __moduleId__: 'pkg_linxtraining001-bv-spa/viewmodels/TestePivotOLAP'
        };
