...
var vmConstructor = function () {
    ...
    var activate = function (settings, querystring) {
      ...
      vm.TelaClienteVenda123456789012345678901234567890 = getVM;
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'TelaClienteVenda123456789012345678901234567890' ]; settings.parentVM.TelaClienteVenda123456789012345678901234567890 = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.TelaClienteVenda123456789012345678901234567890 = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('TelaClienteVenda123456789012345678901234567890', parentVM.internalUIs) === -1) parentVM.internalUIs.push('TelaClienteVenda123456789012345678901234567890');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        if (managerAuth.shellMode == 'PROD') {
           managerAuth.getFormAccess('linxtraining001-bv-spa-TelaClienteVenda123456789012345678901234567890', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: TelaClienteVenda123456789012345678901234567890', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#TelaClienteVenda123456789012345678901234567890_tcClientesViewTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#TelaClienteVenda123456789012345678901234567890_tcClientesViewTabControl');

    complement.renderTelaClienteVenda123456789012345678901234567890_dGridVendasView(vm);

    complement.renderscyTelaClienteVenda123456789012345678901234567890_dGrid(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'ClientesView';
    ...
    var visibleColumns = 'IDClientes,Nome,Tipo';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'ClientesView', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'ClientesView', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('ClientesView', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('ClientesView');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('ClientesView');
        ...
        return dataContext.getClientesViewByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'ClientesView'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        vm.showProcessing('Pesquisando informações...');
        return dataContext.getClientesViewByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    var createVendasView = function(parent, noCurrent) {
        ...
        var entity = dataContext.createVendasView(parent, noCurrent);
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentVendasView(entity); entity.fillDetails(); } 
        return entity;
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#TelaClienteVenda123456789012345678901234567890_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#TelaClienteVenda123456789012345678901234567890_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#TelaClienteVenda123456789012345678901234567890_formViewer')[0];
    ...
    var entitySearchRange = {
        ...
        VendasViewIDVendas_begin: ko.observable(null), VendasViewIDVendas_end: ko.observable(null),
    ...
    entitySearchRange.has_VendasViewIDVendas = ko.computed(function(){ return (entitySearchRange.VendasViewIDVendas_begin() != null || entitySearchRange.VendasViewIDVendas_end() != null); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#TelaClienteVenda123456789012345678901234567890_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            dataView: dataView,
            viewName: 'TelaClienteVenda123456789012345678901234567890',
            ...
            __moduleId__: 'pkg_linxtraining001-bv-spa/viewmodels/TelaClienteVenda123456789012345678901234567890'
        };
