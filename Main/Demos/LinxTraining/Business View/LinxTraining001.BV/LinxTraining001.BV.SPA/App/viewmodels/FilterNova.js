...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'FilterNova' ]; settings.parentVM.FilterNova = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('FilterNova', parentVM.internalUIs) === -1) parentVM.internalUIs.push('FilterNova');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        if (managerAuth.shellMode == 'PROD') {
           managerAuth.getFormAccess('linxtraining001-bv-spa-FilterNova', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: FilterNova', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        complement.renderscyFilterNova_dGrid(vm);
    ...
    var removeFormViewControl = function () {
        var front = $('#FilterNova_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#FilterNova_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#FilterNova_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#FilterNova_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            __moduleId__: 'pkg_linxtraining001-bv-spa/viewmodels/FilterNova'
        };
