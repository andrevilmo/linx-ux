...
var complementCtor = function() {
    var complement = {
    isAutomatic: true
    , renderscyTesteOLAP_dGrid: function(vm) {
    ...
    $('#scyTesteOLAP_dGrid_headers').live('focus  keydown', function (evt) {
        var keyCode = (window.event) ? evt.which : evt.keyCode;
        if (keyCode == 9) {
            var cols = $('#scyTesteOLAP_dGrid').igGrid('option', 'columns');
            var firstRow = $('#scyTesteOLAP_dGrid').data('igGrid').dataSource._dataView[0].RowDataId;
            clear = vm.status() === 'C';
            if (vm.status() == 'C')
                $('#scyTesteOLAP_dGrid').igGridUpdating('startEdit', firstRow, 0);
            else {
                ...
                cols.some(function (entry) {
                    if (entry.key != 'RowDataId' && !entry.hidden) {
                        if (verifyCanEditCol(entry.key, clear, entity)) {
                            $('#scyTesteOLAP_dGrid').igGridUpdating('startEdit', firstRow, indexColumn);
                            return true;
                        }
                        indexColumn++;
                    }
    ...
    var getVisibleColumns = function(metaDataControl) {
       ...
       var visibleColumns = '';
       if($('#scyTesteOLAP_dGrid').data('igGrid') == undefined) return '';
       var cols = $('#scyTesteOLAP_dGrid').igGrid('option', 'columns');
       if (cols) {
    ...
    var itemsSource = { getVisibleColumns: getVisibleColumns, containerId: 'scyTesteOLAP_dGrid_container', dataBind: function (commitData, force) {
       var grid = $('#scyTesteOLAP_dGrid');
       ...
       if (!started) {
           ...
           started = true;
           commitData = false;
           $('#scyTesteOLAP_dGrid_groupbyarea').addClass('hide');
       }
       if (grid.igGridUpdating('isEditing')) {
    ...
    function createDataGrid(grid) {
        grid.igGrid({ height: (vm.isDependentVM() ? (getGridHeightSuggested() * 0.7) : $(window).height() * 0.85), width: '100%',
            ...
            cellClick: function(evt, ui) {
                 if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                     ...
                     if (e && e.tagName == 'IMG' && vm.status() != 'C')
                     {
                          ...
                          showMultimidia(entity, e, table, key, vm.TesteOLAP());
                     }
                 }
                 if (typeof vm.OnGridClientClick === 'function') {
                     var entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                     vm.OnGridClientClick('scyTesteOLAP_dGrid', ui.colKey, entity);
                 }
            },
            enableUTCDates: true,
            ...
            features: [
                          ...
                          columnSorting: function (evt, ui) { 
                              $.grep(ui.owner.grid._visibleColumnsArray, function (e) { 
                                  if (e.key == ui.columnKey && e.dataType == 'string') 
                                      return $('#scyTesteOLAP_dGrid').igGridSorting('option', 'caseSensitive', false); 
                                  else if (e.key == ui.columnKey) 
                                      return $('#scyTesteOLAP_dGrid').igGridSorting('option', 'caseSensitive', true); 
                              }); 
                          } 
                        ...
                        { name: 'Hiding', 
                            columnHidden: function (evt, ui) {
                               showMultimidiaLazy('#scyTesteOLAP_dGrid');
                            },
                       ...
                       ,{ name: 'Updating', horizontalMoveOnEnter: true,
                          ...
                          editCellStarted: function(evt, ui){
                              ...
                              var columns = $('#scyTesteOLAP_dGrid').igGridUpdating('option', 'columnSettings');
                              var currentCol = null;
                              currentRow = ui.rowID;
            ...
            if ((typeof vm.OnDataGridCreated == 'function')){
                vm.OnDataGridCreated('scyTesteOLAP_dGrid');
            }
            grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                if (vm.TesteOLAP().status() === 'Q') vm.TesteOLAP().dataToolbar.viewInfo();
            });
        }
        vm.addDataSource({ key: 'scyTesteOLAP_dGrid', name: 'dataView', itemsSource: itemsSource });
    }
