...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'TiposCamposUIExterna' ]; settings.parentVM.TiposCamposUIExterna = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.TiposCamposUIExterna = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('TiposCamposUIExterna', parentVM.internalUIs) === -1) parentVM.internalUIs.push('TiposCamposUIExterna');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        if (managerAuth.shellMode == 'PROD') {
           managerAuth.getFormAccess('linxtraining001-bv-spa-TiposCamposUIExterna', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: TiposCamposUIExterna', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        complement.renderscyTiposCamposUIExterna_dGrid(vm);
    ...
    var visibleColumns = 'Boolean,Byte,DateTime,Decimal,Guid,IDTiposCampos,Int,Long,Short,String,StringChar,StringText';
    ...
    var saveFakeInnerUIs = function (transactionID, saveCompleteCallback) {
        ...
        var saveFakeInnerUI = function (currentIndex) {
            if (currentIndex < vmsForSaving.length)
                vmsForSaving[currentIndex].dataToolbar.save(false, function () { saveFakeInnerUI(currentIndex + 1); }, transactionID, saveCompleteCallback);
    ...
    var saveFake = function (transactionID, externalSaveSucceeded, saveCompleteCallback) {
    ...
    var save = function (isExclusion, externalSaveSucceeded, transactionId, saveCompleteCallback) {
        ...
        if (!isNullOrEmpty(transactionId)){
            try{ dataToolbar.saveFake(transactionId, externalSaveSucceeded, saveCompleteCallback); }
            ...
            for (var idxElem = toList.length - 1; idxElem >= 0; idxElem--) {
                    if (toList[idxElem].ChangeState !== 'N') {
                       ...
                       if (toList[idxElem].ChangeState == 'I') {
                           fromObj = _.where(fromList, { TemporaryIDTiposCampos: toList[idxElem]['IDTiposCampos'] });
                       } else {
                           fromObj = _.where(fromList, { IDTiposCampos: toList[idxElem]['IDTiposCampos'] });
                       }
    ...
    var deleteEntity = function (entity, isMultiSelection) {
        ...
        if (selectedEntities.length > 0) {
           ...
           else if (typeof isMultiSelection !== 'undefined') {
    ...
    var removeFormViewControl = function () {
        var front = $('#TiposCamposUIExterna_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#TiposCamposUIExterna_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#TiposCamposUIExterna_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#TiposCamposUIExterna_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootNamespace: 'LinxTraining001.BV.TiposCamposUIExterna',
            ...
            __moduleId__: 'pkg_linxtraining001-bv-spa/viewmodels/TiposCamposUIExterna'
        };
