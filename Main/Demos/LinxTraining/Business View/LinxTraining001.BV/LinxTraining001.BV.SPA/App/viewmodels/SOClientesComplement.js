...
var complementCtor = function() {
    var complement = {
    isAutomatic: true
    , renderscySOClientes_dGrid: function(vm) {
    ...
    $('#scySOClientes_dGrid_headers').live('focus  keydown', function (evt) {
        var keyCode = (window.event) ? evt.which : evt.keyCode;
        if (keyCode == 9) {
            var cols = $('#scySOClientes_dGrid').igGrid('option', 'columns');
            var firstRow = $('#scySOClientes_dGrid').data('igGrid').dataSource._dataView[0].RowDataId;
            clear = vm.status() === 'C';
            if (vm.status() == 'C')
                $('#scySOClientes_dGrid').igGridUpdating('startEdit', firstRow, 0);
            else {
                ...
                cols.some(function (entry) {
                    if (entry.key != 'RowDataId' && !entry.hidden) {
                        if (verifyCanEditCol(entry.key, clear, entity)) {
                            $('#scySOClientes_dGrid').igGridUpdating('startEdit', firstRow, indexColumn);
                            return true;
                        }
                        indexColumn++;
                    }
    ...
    var getVisibleColumns = function(metaDataControl) {
       ...
       var visibleColumns = '';
       if($('#scySOClientes_dGrid').data('igGrid') == undefined) return '';
       var cols = $('#scySOClientes_dGrid').igGrid('option', 'columns');
       if (cols) {
    ...
    var itemsSource = { getVisibleColumns: getVisibleColumns, containerId: 'scySOClientes_dGrid_container', dataBind: function (commitData, force) {
       var grid = $('#scySOClientes_dGrid');
       ...
       if (!started) {
           ...
           started = true;
           commitData = false;
           $('#scySOClientes_dGrid_groupbyarea').addClass('hide');
       }
       if (grid.igGridUpdating('isEditing')) {
    ...
    function createDataGrid(grid) {
        grid.igGrid({ height: (vm.isDependentVM() ? (getGridHeightSuggested() * 0.7) : $(window).height() * 0.85), width: '100%',
            ...
            cellClick: function(evt, ui) {
                 if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                     ...
                     if (e && e.tagName == 'IMG' && vm.status() != 'C')
                     {
                          ...
                          showMultimidia(entity, e, table, key, vm.SOClientes());
                     }
                 }
                 if (typeof vm.OnGridClientClick === 'function') {
                     var entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                     vm.OnGridClientClick('scySOClientes_dGrid', ui.colKey, entity);
                 }
            },
            enableUTCDates: true,
            ...
            features: [
                          ...
                          columnSorting: function (evt, ui) { 
                              $.grep(ui.owner.grid._visibleColumnsArray, function (e) { 
                                  if (e.key == ui.columnKey && e.dataType == 'string') 
                                      return $('#scySOClientes_dGrid').igGridSorting('option', 'caseSensitive', false); 
                                  else if (e.key == ui.columnKey) 
                                      return $('#scySOClientes_dGrid').igGridSorting('option', 'caseSensitive', true); 
                              }); 
                          } 
                        ...
                        { name: 'Tooltips', columnSettings:[{ columnKey: "IDClientes", allowTooltips: true },{ columnKey: "Nome", allowTooltips: true },{ columnKey: "Tipo", allowTooltips: true }] },
                        { name: 'Resizing' }, 
                        { name: 'Hiding', 
                            columnHidden: function (evt, ui) {
                               showMultimidiaLazy('#scySOClientes_dGrid');
                            },
                       ...
                       ,{ name: 'Updating', horizontalMoveOnEnter: true,
                          ...
                          columnSettings: [{ columnKey: 'Tipo', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('Tipo', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('LXTipoClientes', ''),  textKey: 'name', valueKey: 'id', enableClearButton: true }}, { columnKey: "IDClientes" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scySOClientes_dGrid', 'IDClientes', ui.oldValue, ui.value);}}, maxLength: 36 } }, { columnKey: "Nome" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scySOClientes_dGrid', 'Nome', ui.oldValue, ui.value);}}, maxLength: 40 } }],
                          ...
                          editCellStarted: function(evt, ui){
                              ...
                              var columns = $('#scySOClientes_dGrid').igGridUpdating('option', 'columnSettings');
                              var currentCol = null;
                              currentRow = ui.rowID;
            ...
            if ((typeof vm.OnDataGridCreated == 'function')){
                vm.OnDataGridCreated('scySOClientes_dGrid');
            }
            grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                if (vm.SOClientes().status() === 'Q') vm.SOClientes().dataToolbar.viewInfo();
            });
        }
        vm.addDataSource({ key: 'scySOClientes_dGrid', name: 'dataView', itemsSource: itemsSource });
    }
