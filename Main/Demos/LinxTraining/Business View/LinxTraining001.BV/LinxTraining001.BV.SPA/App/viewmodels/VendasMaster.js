...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'VendasMaster' ]; settings.parentVM.VendasMaster = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('VendasMaster', parentVM.internalUIs) === -1) parentVM.internalUIs.push('VendasMaster');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        if (managerAuth.shellMode == 'PROD') {
           managerAuth.getFormAccess('linxtraining001-bv-spa-VendasMaster', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: VendasMaster', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        complement.renderVendasMaster_pChartPivotDrillDownChart_292335414f8f4609bb6587a1a58d8f8f(vm);

    complement.renderscyVendasMaster_dGrid(vm);
    ...
    var visibleColumns = 'ValorTotal,Data,IDVendas,IDClientes,Nome,Origem,VendaVip';
    ...
    var removeFormViewControl = function () {
        var front = $('#VendasMaster_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#VendasMaster_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#VendasMaster_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#VendasMaster_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            __moduleId__: 'pkg_linxtraining001-bv-spa/viewmodels/VendasMaster'
        };
