...
var vmConstructor = function () {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'PaiFilhoGrade' ]; settings.parentVM.PaiFilhoGrade = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('PaiFilhoGrade', parentVM.internalUIs) === -1) parentVM.internalUIs.push('PaiFilhoGrade');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        if (managerAuth.shellMode == 'PROD') {
           managerAuth.getFormAccess('linxtraining001-bv-spa-PaiFilhoGrade', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: PaiFilhoGrade', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        $('#PaiFilhoGrade_tcClientesViewTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#PaiFilhoGrade_tcClientesViewTabControl');

    complement.renderPaiFilhoGrade_dGridVendasView(vm);

    complement.renderscyPaiFilhoGrade_dGrid(vm);
    ...
    var visibleColumns = 'Nome,Tipo,IDClientes';
    ...
    var removeFormViewControl = function () {
        var front = $('#PaiFilhoGrade_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#PaiFilhoGrade_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#PaiFilhoGrade_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#PaiFilhoGrade_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            __moduleId__: 'pkg_linxtraining001-bv-spa/viewmodels/PaiFilhoGrade'
        };
