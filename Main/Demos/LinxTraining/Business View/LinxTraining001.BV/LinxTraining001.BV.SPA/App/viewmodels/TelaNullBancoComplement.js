...
var complementCtor = function() {
    var complement = {
    isAutomatic: true
    , renderTelaNullBanco_dGridFilhaNotNullView: function(vm) {
    var getDataSource = function() {
        var source = null;
        try {
            source = vm.currentDataItem().FilhaNotNullViewList;
        }
        catch (e) { }
    ...
    var dataSourceIsLoaded = function() {
        var isLoaded = false;
        try {
            isLoaded = (vm.currentDataItem().FilhaNotNullViewIsLoaded === true || vm.currentDataItem().FilhaNotNullViewList().length > 0);
        }
        catch (e) {
            isLoaded = true;
        }
    ...
    $('#TelaNullBanco_dGridFilhaNotNullView_headers').live('focus  keydown', function (evt) {
        var keyCode = (window.event) ? evt.which : evt.keyCode;
        if (keyCode == 9) {
            var cols = $('#TelaNullBanco_dGridFilhaNotNullView').igGrid('option', 'columns');
            var firstRow = $('#TelaNullBanco_dGridFilhaNotNullView').data('igGrid').dataSource._dataView[0].RowDataId;
            clear = vm.status() === 'C';
            if (vm.status() == 'C')
                $('#TelaNullBanco_dGridFilhaNotNullView').igGridUpdating('startEdit', firstRow, 0);
            else {
                ...
                cols.some(function (entry) {
                    if (entry.key != 'RowDataId' && !entry.hidden) {
                        if (verifyCanEditCol(entry.key, clear, entity)) {
                            $('#TelaNullBanco_dGridFilhaNotNullView').igGridUpdating('startEdit', firstRow, indexColumn);
                            return true;
                        }
                        indexColumn++;
                    }
    ...
    var getVisibleColumns = function(metaDataControl) {
       ...
       var visibleColumns = '';
       if($('#TelaNullBanco_dGridFilhaNotNullView').data('igGrid') == undefined) return '';
       var cols = $('#TelaNullBanco_dGridFilhaNotNullView').igGrid('option', 'columns');
       if (cols) {
    ...
    var itemsSource = { getVisibleColumns: getVisibleColumns, containerId: 'TelaNullBanco_dGridFilhaNotNullView_container', dataBind: function (commitData, force) {
       var grid = $('#TelaNullBanco_dGridFilhaNotNullView');
       ...
       if ((!grid[0] || (!force && grid.parent().width() <= 0)) && !$('#dialogFilhaNotNullView').is(':visible') ) return;
       if (!started) {
           createDataGrid(grid);
           started = true;
           commitData = false;
           $('#TelaNullBanco_dGridFilhaNotNullView_groupbyarea').addClass('hide');
       }
       if (grid.igGridUpdating('isEditing')) {
       ...
       if ((vm.status() != 'C' && vm.status() != 'I') && !dataSourceIsLoaded()) {
         ...
         vm.currentDataItem().fillDetails(false, 'FilhaNotNullView');
         return;
       }
       ...
       if (rows.length > 0) {
         ...
         if (vm.currentDataItem().currentFilhaNotNullView() != null)
         {
           for(var idx = 0; idx < rows.length; idx++)
           {
             if (rows[idx].dataset.id == getAbsoluteValue(vm.currentDataItem().currentFilhaNotNullView().RowDataId))
             {
         ...
         if ($('#dialogFilhaNotNullView').is(':visible')) {
           var hasPaging = $.grep(grid.igGrid('option', 'features'), function (e) {
         	...
         	if (hasPaging.length > 0) {
         		...
         		$('label#currentNumberFilhaNotNullView').html(current + ' - ' + totalCurrentPage);
         	}
         	else
         	    $('label#currentNumberFilhaNotNullView').html(1);
         	$('label#totalNumberFilhaNotNullView').html(totalGrid);
         }
       }
    }};
    var valueGrouBy = -1;
    ...
    function verifyCanEditCol(column, clear, entity){
        switch(column){
            ...
            case 'IDFilhaNotNull': { canEditing = clear || (entity && entity.isAdded()); break;}
            case 'IDPaiNotNull': { canEditing = clear || vm.enabledForEditing(); break;}
    ...
    function createDataGrid(grid) {
        grid.igGrid({ height: (getGridHeightSuggested() * 0.7), width: '100%',
            ...
            cellClick: function(evt, ui) {
                 if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                     ...
                     if (e && e.tagName == 'IMG' && vm.status() != 'C')
                     {
                          ...
                          showMultimidia(entity, e, table, key, vm.TelaNullBanco());
                     }
                 }
                 if (typeof vm.OnGridClientClick === 'function') {
                     var entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                     vm.OnGridClientClick('TelaNullBanco_dGridFilhaNotNullView', ui.colKey, entity);
                 }
            },
            enableUTCDates: true,
            ...
            columns: [
                ...
                { key: 'IDFilhaNotNull', headerText: 'ID FilhaNotNull', width: '231px', dataType: 'number', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                { key: 'IDPaiNotNull', headerText: 'ID PaiNotNull', width: '205px', dataType: 'number', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
            ...
            features: [
                          ...
                          columnSorting: function (evt, ui) { 
                              $.grep(ui.owner.grid._visibleColumnsArray, function (e) { 
                                  if (e.key == ui.columnKey && e.dataType == 'string') 
                                      return $('#TelaNullBanco_dGridFilhaNotNullView').igGridSorting('option', 'caseSensitive', false); 
                                  else if (e.key == ui.columnKey) 
                                      return $('#TelaNullBanco_dGridFilhaNotNullView').igGridSorting('option', 'caseSensitive', true); 
                              }); 
                          } 
            },
                        ...
                        { name: 'Selection', mode: 'row',
                          rowSelectionChanged: function(evt, ui) {
                             if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                 ...
                                 selectGridCurrentItem(vm.goToKey, 'RowDataId', ui, vm.currentDataItem().currentFilhaNotNullView, getDataSource()); } 
                          },
                        },
                        { name: 'Tooltips', columnSettings:[{ columnKey: "Boolean", allowTooltips: true },{ columnKey: "Byte", allowTooltips: true },{ columnKey: "DateTime", allowTooltips: true },{ columnKey: "Decimal", allowTooltips: true },{ columnKey: "IDFilhaNotNull", allowTooltips: true },{ columnKey: "IDPaiNotNull", allowTooltips: true },{ columnKey: "Int", allowTooltips: true },{ columnKey: "Long", allowTooltips: true },{ columnKey: "Short", allowTooltips: true },{ columnKey: "String", allowTooltips: true },{ columnKey: "StringChar", allowTooltips: true },{ columnKey: "StringText", allowTooltips: true }] },
                        { name: 'Resizing' }, 
                        { name: 'Hiding', 
                            columnHidden: function (evt, ui) {
                               showMultimidiaLazy('#TelaNullBanco_dGridFilhaNotNullView');
                            },
                       ...
                       ,{ name: 'Updating', horizontalMoveOnEnter: true,
                          ...
                          columnSettings: [{ columnKey: 'DateTime', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid == 'function') {vm.OnPropertyChangeDataGrid('TelaNullBanco_dGridFilhaNotNullView', 'DateTime', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: "Byte" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('TelaNullBanco_dGridFilhaNotNullView', 'Byte', ui.oldValue, ui.value);}},  maxLength: 3, maxValue: null, minValue: 0, dataMode: 'byte' } }, { columnKey: "Decimal" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('TelaNullBanco_dGridFilhaNotNullView', 'Decimal', ui.oldValue, ui.value);}},  maxLength: 11, maxValue: 99999999.99, minValue: 0, dataMode: 'decimal', minDecimals: 2, maxDecimals: 2 } }, { columnKey: "IDFilhaNotNull" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('TelaNullBanco_dGridFilhaNotNullView', 'IDFilhaNotNull', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "IDPaiNotNull" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('TelaNullBanco_dGridFilhaNotNullView', 'IDPaiNotNull', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "Int" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('TelaNullBanco_dGridFilhaNotNullView', 'Int', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "Long" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('TelaNullBanco_dGridFilhaNotNullView', 'Long', ui.oldValue, ui.value);}},  maxLength: 24, maxValue: null, minValue: 0, dataMode: 'long' } }, { columnKey: "Short" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('TelaNullBanco_dGridFilhaNotNullView', 'Short', ui.oldValue, ui.value);}},  maxLength: 6, maxValue: null, minValue: 0, dataMode: 'short' } }, { columnKey: "String" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('TelaNullBanco_dGridFilhaNotNullView', 'String', ui.oldValue, ui.value);}}, maxLength: 10 } }, { columnKey: "StringChar" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('TelaNullBanco_dGridFilhaNotNullView', 'StringChar', ui.oldValue, ui.value);}}, maxLength: 10 } }, { columnKey: "StringText" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('TelaNullBanco_dGridFilhaNotNullView', 'StringText', ui.oldValue, ui.value);}}, maxLength: 0 } }],
                          ...
                          editCellStarted: function(evt, ui){
                              ...
                              var columns = $('#TelaNullBanco_dGridFilhaNotNullView').igGridUpdating('option', 'columnSettings');
                              var currentCol = null;
                              currentRow = ui.rowID;
                              ...
                              if (currentCol !== null && currentCol.hasOwnProperty('editorType') && currentCol.editorType === 'combo') {
                                 ...
                                 if (lookUpName !== null) {
                                     ...
                                     if (lstRefreshDados.length == 0)
                                         vm.dataCombo.fillDataCombos(lookUpName, ui.columnKey, vm.currentDataItem().currentFilhaNotNullView(), function (result) {
            ...
            if ((typeof vm.OnDataGridCreated == 'function')){
                vm.OnDataGridCreated('TelaNullBanco_dGridFilhaNotNullView');
            }
        }
        vm.addDataSource({ key: 'TelaNullBanco_dGridFilhaNotNullView', name: 'FilhaNotNullViewList', itemsSource: itemsSource });
    }


, renderscyTelaNullBanco_dGrid: function(vm) {
    var getDataSource = function() {
        var source = null;
        try {
            source = vm.dataView;
    ...
    $('#scyTelaNullBanco_dGrid_headers').live('focus  keydown', function (evt) {
        var keyCode = (window.event) ? evt.which : evt.keyCode;
        if (keyCode == 9) {
            var cols = $('#scyTelaNullBanco_dGrid').igGrid('option', 'columns');
            var firstRow = $('#scyTelaNullBanco_dGrid').data('igGrid').dataSource._dataView[0].RowDataId;
            clear = vm.status() === 'C';
            if (vm.status() == 'C')
                $('#scyTelaNullBanco_dGrid').igGridUpdating('startEdit', firstRow, 0);
            else {
                var entity = findElementByKey(getDataSource(), 'RowDataId', firstRow);
                var indexColumn = 0;
                cols.some(function (entry) {
                    if (entry.key != 'RowDataId' && !entry.hidden) {
                        if (verifyCanEditCol(entry.key, clear, entity)) {
                            $('#scyTelaNullBanco_dGrid').igGridUpdating('startEdit', firstRow, indexColumn);
                            return true;
                        }
                        indexColumn++;
                    }
    ...
    var getVisibleColumns = function(metaDataControl) {
       if (metaDataControl) return '';
       var visibleColumns = '';
       if($('#scyTelaNullBanco_dGrid').data('igGrid') == undefined) return '';
       var cols = $('#scyTelaNullBanco_dGrid').igGrid('option', 'columns');
       if (cols) {
         for (var idx = 0; idx < cols.length; idx++) {
             if (cols[idx].hidden != true) visibleColumns += (visibleColumns == '' ? '' : ',') + cols[idx].key;
    ...
    var itemsSource = { getVisibleColumns: getVisibleColumns, containerId: 'scyTelaNullBanco_dGrid_container', dataBind: function (commitData, force) {
       var grid = $('#scyTelaNullBanco_dGrid');
       if (started && (typeof grid.data('igGridUpdating') === 'undefined')) { started = false; }
       if (commitData && started) {
           if (grid.igGridUpdating('isEditing')) {
               grid.igGrid('commit');
       ...
       if (!started) {
           ...
           started = true;
           commitData = false;
           $('#scyTelaNullBanco_dGrid_groupbyarea').addClass('hide');
       }
       if (grid.igGridUpdating('isEditing')) {
            grid.igGridUpdating('endEdit', true);
       }
    ...
    function verifyCanEditCol(column, clear, entity){
        switch(column){
            ...
            case 'IDPaiNotNull': { canEditing = clear; break;}
    ...
    function createDataGrid(grid) {
        grid.igGrid({ height: (vm.isDependentVM() ? (getGridHeightSuggested() * 0.7) : $(window).height() * 0.85), width: '100%',
            ...
            cellClick: function(evt, ui) {
                 ...
                 if (typeof vm.OnGridClientClick === 'function') {
                     var entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                     vm.OnGridClientClick('scyTelaNullBanco_dGrid', ui.colKey, entity);
                 }
            },
            enableUTCDates: true,
            featureChooserIconDisplay: 'always',
            ...
            features: [
                        { name: 'Sorting', type: 'local', caseSensitive: true,
                          columnSorting: function (evt, ui) { 
                              $.grep(ui.owner.grid._visibleColumnsArray, function (e) { 
                                  if (e.key == ui.columnKey && e.dataType == 'string') 
                                      return $('#scyTelaNullBanco_dGrid').igGridSorting('option', 'caseSensitive', false); 
                                  else if (e.key == ui.columnKey) 
                                      return $('#scyTelaNullBanco_dGrid').igGridSorting('option', 'caseSensitive', true); 
                              }); 
                          } 
                        ...
                        { name: 'Tooltips', columnSettings:[{ columnKey: "Boolean", allowTooltips: true },{ columnKey: "Byte", allowTooltips: true },{ columnKey: "DateTime", allowTooltips: true },{ columnKey: "Decimal", allowTooltips: true },{ columnKey: "IDPaiNotNull", allowTooltips: true },{ columnKey: "Int", allowTooltips: true },{ columnKey: "Long", allowTooltips: true },{ columnKey: "Short", allowTooltips: true },{ columnKey: "String", allowTooltips: true },{ columnKey: "StringChar", allowTooltips: true },{ columnKey: "StringText", allowTooltips: true }] },
                        { name: 'Resizing' }, 
                        { name: 'Hiding', 
                            columnHidden: function (evt, ui) {
                               showMultimidiaLazy('#scyTelaNullBanco_dGrid');
                            },
                            columnShown: function (evt, ui) {
                               showMultimidiaLazy('#scyTelaNullBanco_dGrid');
                            }
                       ...
                       ,{ name: 'Updating', horizontalMoveOnEnter: true,
                          ...
                          showReadonlyEditors: false,
                          showDoneCancelButtons: false,
                          columnSettings: [{ columnKey: 'DateTime', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid == 'function') {vm.OnPropertyChangeDataGrid('scyTelaNullBanco_dGrid', 'DateTime', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: "Byte" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTelaNullBanco_dGrid', 'Byte', ui.oldValue, ui.value);}},  maxLength: 3, maxValue: null, minValue: 0, dataMode: 'byte' } }, { columnKey: "Decimal" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTelaNullBanco_dGrid', 'Decimal', ui.oldValue, ui.value);}},  maxLength: 11, maxValue: 99999999.99, minValue: 0, dataMode: 'decimal', minDecimals: 2, maxDecimals: 2 } }, { columnKey: "IDPaiNotNull" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTelaNullBanco_dGrid', 'IDPaiNotNull', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "Int" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTelaNullBanco_dGrid', 'Int', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "Long" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTelaNullBanco_dGrid', 'Long', ui.oldValue, ui.value);}},  maxLength: 24, maxValue: null, minValue: 0, dataMode: 'long' } }, { columnKey: "Short" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTelaNullBanco_dGrid', 'Short', ui.oldValue, ui.value);}},  maxLength: 6, maxValue: null, minValue: 0, dataMode: 'short' } }, { columnKey: "String" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTelaNullBanco_dGrid', 'String', ui.oldValue, ui.value);}}, maxLength: 10 } }, { columnKey: "StringChar" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTelaNullBanco_dGrid', 'StringChar', ui.oldValue, ui.value);}}, maxLength: 10 } }, { columnKey: "StringText" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid == 'function'){vm.OnPropertyChangeDataGrid('scyTelaNullBanco_dGrid', 'StringText', ui.oldValue, ui.value);}}, maxLength: 0 } }],
                          rowDeleting: function (evt, ui) {
                              deletedIndex = ui.element.context.rowIndex;
                              var entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowID);
                              if (entity) {
                          ...
                          editCellStarted: function(evt, ui){
                              var lstRefreshDados = null;
                              var columns = $('#scyTelaNullBanco_dGrid').igGridUpdating('option', 'columnSettings');
                              var currentCol = null;
                              currentRow = ui.rowID;
                              columns.forEach(function (entry, index) {
                                 if (entry.columnKey == ui.columnKey) currentCol = entry;
            ...
            if ((typeof vm.OnDataGridCreated == 'function')){
                vm.OnDataGridCreated('scyTelaNullBanco_dGrid');
            }
            grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                if (vm.TelaNullBanco().status() === 'Q') vm.TelaNullBanco().dataToolbar.viewInfo();
            });
        }
        vm.addDataSource({ key: 'scyTelaNullBanco_dGrid', name: 'dataView', itemsSource: itemsSource });
    }
