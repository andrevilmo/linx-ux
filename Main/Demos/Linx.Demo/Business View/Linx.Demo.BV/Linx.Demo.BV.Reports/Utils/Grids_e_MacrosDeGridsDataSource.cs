namespace Linx.Demo.BV.Reports {
    ...
    using Linx.Demo.BV.Grids_e_MacrosDeGrids;
    using System.Net.Http;
    using System.Net.Http.Headers;
    
    
    public partial class Grids_e_MacrosDeGridsDataSource {
        
        private string url = "LinxDemoGrids_e_MacrosDeGrids/";
        
        public Linx.Demo.BV.Grids_e_MacrosDeGrids.Grids_e_MacrosDeGridsDomainService _grids_e_MacrosDeGridsContext;
        ...
        private IEnumerable<Venda> GetLocalVenda(Telerik.Reporting.Processing.Report report) {
            ...
            if (_grids_e_MacrosDeGridsContext == null) _grids_e_MacrosDeGridsContext = new Linx.Demo.BV.Grids_e_MacrosDeGrids.Grids_e_MacrosDeGridsDomainService(headers) { IsSecure = true };
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._grids_e_MacrosDeGridsContext.GetVendaByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._grids_e_MacrosDeGridsContext.GetVendaByEntitySearchNoAssociations(null).ToList();
            if (this.DetailsForLoading != null && this.DetailsForLoading.Length > 0)
            {
               foreach (var entity in result)
               {
                   entity.FillDetails(this._grids_e_MacrosDeGridsContext, null, null, this.DetailsForLoading);
               }
            }
            return result;
        }
        ...
        private IEnumerable<VendaItem> GetLocalVendaItem(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._grids_e_MacrosDeGridsContext.GetVendaItemByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._grids_e_MacrosDeGridsContext.GetVendaItemByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<VendaItemParentComposition> GetLocalVendaItemParentComposition(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._grids_e_MacrosDeGridsContext.GetVendaItemParentCompositionByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._grids_e_MacrosDeGridsContext.GetVendaItemParentCompositionByEntitySearchNoAssociations(null).ToList();
            return result;
        }
