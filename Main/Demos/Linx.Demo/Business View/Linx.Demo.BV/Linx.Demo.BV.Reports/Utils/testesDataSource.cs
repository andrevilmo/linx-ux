namespace Linx.Demo.BV.Reports {
    ...
    using Linx.Demo.BV.testes;
    using System.Net.Http;
    using System.Net.Http.Headers;
    
    
    public partial class testesDataSource {
        
        private string url = "LinxDemotestes/";
        
        public Linx.Demo.BV.testes.testesDomainService _testesContext;
        ...
        private IEnumerable<Produto> GetLocalProduto(Telerik.Reporting.Processing.Report report) {
            ...
            if (_testesContext == null) _testesContext = new Linx.Demo.BV.testes.testesDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(Produto), new string[] { });
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(Produto), jEntitySearch, false, true, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._testesContext.GetProdutoByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._testesContext.GetProdutoByEntitySearchNoAssociations(null).ToList();
            return result;
        }
