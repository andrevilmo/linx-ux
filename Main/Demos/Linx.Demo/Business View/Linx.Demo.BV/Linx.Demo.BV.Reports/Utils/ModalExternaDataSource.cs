namespace Linx.Demo.BV.Reports {
    ...
    public partial class ModalExternaDataSource {
        ...
        private IEnumerable<Pais> GetLocalPais(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._modalExternaContext.GetPaisByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._modalExternaContext.GetPaisByEntitySearchNoAssociations(null).ToList();
            return result;
        }
