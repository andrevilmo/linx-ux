namespace Linx.Demo.BV.Reports {
    ...
    public partial class Graf_dash_KPIDataSource {
        
        private string url = "LinxDemoGraf_dash_KPI/";
        
        public Linx.Demo.BV.Graf_dash_KPI.Graf_dash_KPIDomainService _graf_dash_KPIContext;
        ...
        private IEnumerable<Produto> GetLocalProduto(Telerik.Reporting.Processing.Report report) {
            IEnumerable<Produto> result = default(IEnumerable<Produto>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_graf_dash_KPIContext == null) _graf_dash_KPIContext = new Linx.Demo.BV.Graf_dash_KPI.Graf_dash_KPIDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(Produto), new string[] { });
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(Produto), jEntitySearch, false, false, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._graf_dash_KPIContext.GetProdutoByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._graf_dash_KPIContext.GetProdutoByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<Produto> GetProduto(object reportItem) {
            ...
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceAddress = "GetSampleProduto?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    ...
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(Produto), new string[] { });
                        serviceAddress = "GetProdutoByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                    ...
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<Produto>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
