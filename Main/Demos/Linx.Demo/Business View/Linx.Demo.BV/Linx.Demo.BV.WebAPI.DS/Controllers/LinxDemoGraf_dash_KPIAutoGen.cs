...
namespace Linx.Demo.BV.WebAPI.DS.Controllers
{
    ...
    //Examples:
    // Default Call: http://localhost:1710/LinxDemoGraf_dash_KPI/[ActionName]
    // Security Information Call: http://localhost:1710/LinxDemoGraf_dash_KPI/GetSecurityInfo
    // Entities Catalog Call: http://localhost:1710/LinxDemoGraf_dash_KPI/GetEntities
    // Entity MetaData Call: http://localhost:1710/LinxDemoGraf_dash_KPI/GetMetaData?entityName=[EntityName]&allComposition=false
    // Client Domains Call: http://localhost:1710/LinxDemoGraf_dash_KPI/GetClientDomains?erp=true
    // Client Service Call: http://localhost:1710/LinxDemoGraf_dash_KPI/GetClientService?erp=true
    // Client Factory Call: http://localhost:1710/LinxDemoGraf_dash_KPI/GetClientFactory?entityName=[EntityName]&erp=true
    // Client Factory Custom Events Call: http://localhost:1710/LinxDemoGraf_dash_KPI/GetClientFactoryCustomEvents?entityName=[EntityName]&erp=true
    // Help Call: http://localhost:1710/HelpController/LinxDemoGraf_dash_KPI
    // Feed OData Call: http://localhost:1710/LinxDemoGraf_dash_KPIOData
    [RoutePrefix("LinxDemoGraf_dash_KPI")]
    [Breeze.WebApi2.BreezeController]
    [ODataBasicAuthenticationFilter]
    public partial class LinxDemoGraf_dash_KPIController : ApiController
    {
        private const int maxObjectExcelReturned = 2300000;
        private DataServiceRepository<BusinessNS.Graf_dash_KPIDomainService> _repository = null;
        private DataServiceRepository<BusinessNS.Graf_dash_KPIDomainService> repository { get {  if (_repository == null) { _repository = new DataServiceRepository<BusinessNS.Graf_dash_KPIDomainService>(typeof(BusinessNS.Produto)); _repository.Context.IsSecure = true; } return _repository; } }
        public LinxDemoGraf_dash_KPIController()
        { }
        ...
        public object AssemblyInfo()
        {
            return new
            {
                ApiAssemblyName = typeof(LinxDemoGraf_dash_KPIController).Assembly.FullName,
                BusinessAssemblyName = typeof(BusinessNS.Graf_dash_KPIDomainService).Assembly.FullName,
        ...
        {
            var result = repository.Context.GetMetaDataObject("Linx.Demo.BV.Graf_dash_KPI." + entityName, false, true);
        ...
        {
           return new string[] { "Linx.Demo.BV", "LinxDemoGraf_dash_KPI", "LinxDemoGraf_dash_KPI/ActionName" };
        }
        ...
        public string GetProdutoToExcel(string[] parameters)
        {
            ...
            var metadata = repository.Context.GetMetaDataObject("Linx.Demo.BV.Graf_dash_KPI.Produto");
            if (columns.Count > 0)
            {
        ...
        public string GetProdutoToReportXml(string[] parameters)
        {
            ...
            var metadata = repository.Context.GetMetaDataObject("Linx.Demo.BV.Graf_dash_KPI.Produto", true);
            if (columns.Count > 0)
            {
                foreach (var item in metadata[0].Properties)
                {
            ...
            var zip = new LinxZip();
            zip.AddStringContent(reportName + ".trdx", ReportExport.CreateXmlReport(new ReportExport.EntitiesToExport{ ReportName = reportName, Metadata = metadata.First(), DataSourceAssembly = "Linx.Demo.BV.Reports", DataSourceFullName = "Linx.Demo.BV.Reports.Graf_dash_KPIDataSource", DataSourceObject = "GetProduto", JExpressionTranslated = translatedJEntitySearch, JQueryExpression = jEntitySearch, ServiceBusUrl = serviceBusUrl, HasMedia = exportMedia }));
            zip.AddFile(System.Web.HttpContext.Current.Server.MapPath("~/bin/Linx.Demo.BV.Reports.dll"));
            zip.AddFile(System.Web.HttpContext.Current.Server.MapPath("~/bin/Linx.Demo.BV.dll"));
            return Convert.ToBase64String(zip.GetZipBytes());
        }
    ...
    [ODataBasicAuthenticationFilter]
    public partial class LinxDemoGraf_dash_KPIFeedController : ODataController
    {
        private BusinessNS.Graf_dash_KPIDomainService _context;
        public BusinessNS.Graf_dash_KPIDomainService Context { get {  if (_context == null) { _context = new BusinessNS.Graf_dash_KPIDomainService(); _context.IsSecure = true; } return _context; }  }
    ...
    public partial class LinxDemoGraf_dash_KPIControllerAuthorizeAttribute : System.Web.Http.AuthorizeAttribute
    {
        protected override bool IsAuthorized(System.Web.Http.Controllers.HttpActionContext actionContext)
        {
            return LinxAutorization.CheckAuthorization(actionContext, string.Format("{0}#{1}#{1}/{2}", "Linx.Demo.BV", "LinxDemoGraf_dash_KPI", actionContext.ActionDescriptor.ActionName));
        }
    }
}
