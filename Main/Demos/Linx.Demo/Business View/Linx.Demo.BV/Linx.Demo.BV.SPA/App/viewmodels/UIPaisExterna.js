...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'UIPaisExterna', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_UIPaisExterna(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'UIPaisExterna' ]; settings.parentVM.UIPaisExterna = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('UIPaisExterna', parentVM.internalUIs) === -1) parentVM.internalUIs.push('UIPaisExterna');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-demo-bv-spa-UIPaisExterna', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: UIPaisExterna', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#UIPaisExterna_gbPais').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    complement.renderscyUIPaisExterna_dGrid(vm);
    ...
    var removeFormViewControl = function () {
        var front = $('#UIPaisExterna_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#UIPaisExterna_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#UIPaisExterna_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#UIPaisExterna_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            __moduleId__: 'pkg_linx-demo-bv-spa/viewmodels/UIPaisExterna'
        };
