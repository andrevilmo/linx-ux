...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'UIPaiFilhaLeft', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_UIPaiFilhaLeft(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'UIPaiFilhaLeft' ]; settings.parentVM.UIPaiFilhaLeft = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('UIPaiFilhaLeft', parentVM.internalUIs) === -1) parentVM.internalUIs.push('UIPaiFilhaLeft');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-demo-bv-spa-UIPaiFilhaLeft', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: UIPaiFilhaLeft', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#UIPaiFilhaLeft_gbLoja').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    $('#UIPaiFilhaLeft_tcLojaTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#UIPaiFilhaLeft_tcLojaTabControl');

    complement.renderUIPaiFilhaLeft_dGridVenda(vm);

    complement.renderUIPaiFilhaLeft_dGridVendaItem(vm);

    complement.renderscyUIPaiFilhaLeft_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                           complement.ChangedBrandUIPaiFilhaLeft_dGridVenda(vm, decimals, reset);
                       complement.ChangedBrandUIPaiFilhaLeft_dGridVendaItem(vm, decimals, reset);
    ...
    var visibleColumns = 'BigIntLoja,BitLoja,ComboboxLoja,DatetimeLoja,IdCidade,IdEstado,IdPais,DecimalLoja,NomeCidade,IdLoja,IntLoja,SmallIntLoja,GuidLoja,StringEstado,StringLoja,StringPais';
    ...
    var removeFormViewControl = function () {
        var front = $('#UIPaiFilhaLeft_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#UIPaiFilhaLeft_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#UIPaiFilhaLeft_formViewer')[0];
    ...
    var entitySearchRange = {
        ...
        VendaIdLoja_begin: ko.observable(null), VendaIdLoja_end: ko.observable(null),
    ...
    entitySearchRange.clear = function(){
        ...
        entitySearchRange.VendaIdLoja_begin(null); entitySearchRange.VendaIdLoja_end(null);
    ...
    entitySearchRange.has_VendaIdLoja = ko.computed(function(){ return (entitySearchRange.VendaIdLoja_begin() != null || entitySearchRange.VendaIdLoja_end() != null); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#UIPaiFilhaLeft_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            __moduleId__: 'pkg_linx-demo-bv-spa/viewmodels/UIPaiFilhaLeft'
        };
