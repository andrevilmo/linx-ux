define(['managers/__auth', 'managers/user'], function (managerAuth, managerUser) {
    var complementCtor = function() {
        var complement = {
            isAutomatic: true
            , ChangedBrandscyUIExterna_dGrid: function(vm, decimals, reset) {
                var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#scyUIExterna_dGrid').data('igGrid'),
                    grdUpd = $('#scyUIExterna_dGrid').data('igGridUpdating');
            ...
            , renderscyUIExterna_dGrid: function(vm) {
                ...
                $('#scyUIExterna_dGrid_headers').live('focus  keydown', function (evt) {
                    var keyCode = window.event ? evt.which : evt.keyCode;
                    if (keyCode === 9) {
                        var cols = $('#scyUIExterna_dGrid').igGrid('option', 'columns');
                        var dataView = $('#scyUIExterna_dGrid').data('igGrid').dataSource._dataView
                        if (dataView.length === 0) return;
                        var firstRow = dataView[0].RowDataId;
                        clear = vm.status() === 'C';
                        if (vm.status() === 'C')
                            $('#scyUIExterna_dGrid').igGridUpdating('startEdit', firstRow, 0, true);
                        else {
                            ...
                            cols.some(function (entry) {
                                if (entry.key !== 'RowDataId' && !entry.hidden) {
                                    if (verifyCanEditCol(entry.key, clear, entity)) {
                                        $('#scyUIExterna_dGrid').igGridUpdating('startEdit', firstRow, indexColumn, true);
                                        return true;
                                    }
                                    indexColumn++;
                                }
                ...
                var getVisibleColumns = function(metaDataControl) {
                   ...
                   var visibleColumns = '';
                   if($('#scyUIExterna_dGrid').data('igGrid') === undefined) return '';
                   var cols = $('#scyUIExterna_dGrid').igGrid('option', 'columns');
                   if (cols) {
                ...
                var isElementHided = function (grid, forceCreating) {
                  if (!grid) grid = $('#scyUIExterna_dGrid');
                  return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialog').is(':visible'));
                }
                var refreshData = true;
                var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'scyUIExterna_dGrid_container', dataBind: function (commitData, forceCreating) {
                   var grid = $('#scyUIExterna_dGrid');
                   ...
                   if (!started) {
                       ...
                       started = true;
                       commitData = false;
                       $('#scyUIExterna_dGrid_groupbyarea').addClass('hide');
                   }
                   if (grid.igGridUpdating('isEditing')) {
                ...
                function verifyCanEditCol(column, clear, entity){
                    switch(column){
                        ...
                        case 'NomeLoja': { canEditing = clear; break;}
                ...
                function createDataGrid(grid) {
                    ...
                    vm.gridSaveStates[gridId] = {
                        ...
                        closePopover: function () {
                            $('#scyUIExterna_dGrid_LayoutBtn').igPopover('hide');
                        },
                    ...
                    grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
                        ...
                        cellClick: function(evt, ui) {
                             if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                                 ...
                                 if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                                 {
                                      ...
                                      showMultimidia(entity, e, table, key, vm.UIExterna());
                                 }
                             }
                             if (typeof vm.OnGridClientClick === 'function') {
                                 entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                 vm.OnGridClientClick('scyUIExterna_dGrid', ui.colKey, entity);
                             }
                             if (vm.status() != 'Q') {
                                 var grid = $('#scyUIExterna_dGrid');
                        ...
                        columns: [
                            ...
                            { key: 'BigIntLoja', headerText: vm.getLayoutHeaderGrid('BigIntLoja'), width: '0px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                            { key: 'BitLoja', headerText: vm.getLayoutHeaderGrid('BitLoja'), width: '0px', dataType: 'bool', columnCssClass: 'ellipsis', format: 'checkbox', hidden: false, unbound: false, group: null   },
                            { key: 'ComboboxLoja', headerText: vm.getLayoutHeaderGrid('ComboboxLoja'), width: '0px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null  , formatter: function (val, record) { return  vm.dataDomains.getName('LX_COMBOBOX_LOJA', val);} },
                            { key: 'DatetimeLoja', headerText: vm.getLayoutHeaderGrid('DatetimeLoja'), width: '0px', dataType: 'date', columnCssClass: 'ellipsis', format: 'dd/MM/yyyy', hidden: false, unbound: false, group: null   },
                            { key: 'IdCidade', headerText: vm.getLayoutHeaderGrid('IdCidade'), width: '0px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            ...
                            { key: 'DecimalLoja', headerText: vm.getLayoutHeaderGrid('DecimalLoja'), width: '0px', dataType: 'number', columnCssClass: 'ellipsis', format: '0.00', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                            { key: 'IdLoja', headerText: vm.getLayoutHeaderGrid('IdLoja'), width: '0px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                            { key: 'IntLoja', headerText: vm.getLayoutHeaderGrid('IntLoja'), width: '0px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                            { key: 'NomeLoja', headerText: vm.getLayoutHeaderGrid('NomeLoja'), width: '0px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'SmallIntLoja', headerText: vm.getLayoutHeaderGrid('SmallIntLoja'), width: '0px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                            { key: 'IdEstado', headerText: vm.getLayoutHeaderGrid('IdEstado'), width: '0px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   }
                        ],
                        features: [
                                    ...
                                    filterDialogOpening: function (evt, ui) {
                                        ...
                                        var dataView = $('#scyUIExterna_dGrid').data('igGrid').dataSource;
                                        ...
                                        scriptHtml += '    var newGrid = $("#scyUIExterna_dGrid");';
                                        ...
                                        scriptHtml += '            $("#scyUIExterna_dGrid_container_dialog_list").html(newList)';
                                        scriptHtml += '         }});';
                                    ...
                                    { name: 'Tooltips', columnSettings:[{ columnKey: "BigIntLoja", allowTooltips: true },{ columnKey: "BitLoja", allowTooltips: true },{ columnKey: "ComboboxLoja", allowTooltips: true },{ columnKey: "DatetimeLoja", allowTooltips: true },{ columnKey: "IdCidade", allowTooltips: false },{ columnKey: "IdPais", allowTooltips: false },{ columnKey: "DecimalLoja", allowTooltips: true },{ columnKey: "IdLoja", allowTooltips: true },{ columnKey: "IntLoja", allowTooltips: true },{ columnKey: "NomeLoja", allowTooltips: true },{ columnKey: "SmallIntLoja", allowTooltips: true },{ columnKey: "IdEstado", allowTooltips: false }] },
                                    { name: 'Resizing' }, 
                                    { name: 'Hiding', 
                                    },
                                   ...
                                   ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                      ...
                                      columnSettings: [{ columnKey: "IdCidade", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpCidade", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, validateOnClearState:false, maxValue:2147483647, maxLength: 12, defaultValue: null } }, { columnKey: "IdPais", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpPais", isNullable: false, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, validateOnClearState:false, maxValue:2147483647, maxLength: 12, defaultValue: 0 } }, { columnKey: "IdEstado", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpEstado", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:false, validateOnClearState:false, maxValue:2147483647, maxLength: 12, defaultValue: null } }, { columnKey: 'DatetimeLoja', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid === 'function') {vm.OnPropertyChangeDataGrid('scyUIExterna_dGrid', 'DatetimeLoja', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: 'ComboboxLoja', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('ComboboxLoja', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('LX_COMBOBOX_LOJA', ''),  textKey: 'name', valueKey: 'id', enableClearButton: false }}, { columnKey: "BigIntLoja" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIExterna_dGrid', 'BigIntLoja', ui.oldValue, ui.value);}},  maxLength: 24, maxValue: null, minValue: 0, dataMode: 'long' } }, { columnKey: "DecimalLoja" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIExterna_dGrid', 'DecimalLoja', ui.oldValue, ui.value);}},  maxLength: 14, maxValue: 99999999999.99, minValue: 0, dataMode: 'decimal', minDecimals: 2, maxDecimals: 2 } }, { columnKey: "IdLoja" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIExterna_dGrid', 'IdLoja', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "IntLoja" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIExterna_dGrid', 'IntLoja', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "NomeLoja" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIExterna_dGrid', 'NomeLoja', ui.oldValue, ui.value);}}, maxLength: 50 } }, { columnKey: "SmallIntLoja" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIExterna_dGrid', 'SmallIntLoja', ui.oldValue, ui.value);}},  maxLength: 6, maxValue: null, minValue: 0, dataMode: 'short' } }],
                                      ...
                                      editCellStarted: function(evt, ui){
                                          ...
                                          var columns = $('#scyUIExterna_dGrid').igGridUpdating('option', 'columnSettings');
                                          var currentCol = null;
                                          currentRow = ui.rowID;
                        ...
                        if ((typeof vm.OnDataGridCreated === 'function')){
                            vm.OnDataGridCreated('scyUIExterna_dGrid');
                        }
                        ...
                        selectionrowselectionchanged = function (evt, ui) {
                            if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                    $(document).undelegate('#scyUIExterna_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                    ...
                                    $(document).delegate ('#scyUIExterna_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                }
                                selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                             } 
                        };
                        $(document).delegate('#scyUIExterna_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        $('#scyUIExterna_dGrid > tbody tr').live('focus', function(evt) {
                            var grid = $('#scyUIExterna_dGrid'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                        ...
                        grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                            if (vm.UIExterna().status() === 'Q') vm.UIExterna().dataToolbar.viewInfo();
                        });
                    }
                    vm.addDataSource({ key: 'scyUIExterna_dGrid', name: 'dataView', itemsSource: itemsSource });
                }
