define(['durandal/app', 'pkg_linx-demo-bv-spa/services/Grids_e_MacrosDeGridsContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-demo-bv-spa/viewmodels/Exemplos_GRIDs_com_macroComplement', 'viewmodels/shared/modalCustomSearch', 'managers/user'],
...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'Exemplos_GRIDs_com_macro', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_Exemplos_GRIDs_com_macro(), 'Name'))());
        }, function (err) {
    ...
    var activate = function (settings, querystring) {
      ...
      vm.Exemplos_GRIDs_com_macro = getVM;
      //loadLanguage();
      changeLanguage();
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'Exemplos_GRIDs_com_macro' ]; settings.parentVM.Exemplos_GRIDs_com_macro = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.Exemplos_GRIDs_com_macro = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('Exemplos_GRIDs_com_macro', parentVM.internalUIs) === -1) parentVM.internalUIs.push('Exemplos_GRIDs_com_macro');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-demo-bv-spa-Exemplos_GRIDs_com_macro', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: Exemplos_GRIDs_com_macro', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        complement.renderExemplos_GRIDs_com_macro_dGridvendas(vm);

    $('#Exemplos_GRIDs_com_macro_tcVendaTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#Exemplos_GRIDs_com_macro_tcVendaTabControl');

    complement.renderExemplos_GRIDs_com_macro_dGridVendaItens(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                   complement.ChangedBrandExemplos_GRIDs_com_macro_dGridvendas(vm, decimals, reset);
    ...
    var visibleColumns = 'IdCliente,IdVenda,StringVenda,ComboboxVenda,IntVenda,SmallIntVenda,DatetimeVenda,DecimalVenda,StringLoja,StringVendedor,IdCidade,NomeCidade';
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('Venda', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('Venda');
           var result = [];
           var relationExpr = '';
    ...
    var DesabilitaGrid_Click = function () {
    /*
{
  dataContext.sharedData['EnableGrid_dGridVendaItens'] = true;

  var handle = function(){ return dataContext.sharedData['EnableGrid_dGridVendaItens']; };      
       
  if('dGridVendaItens'.indexOf('dGrid') > -1)
    var gridSelector = $lx(vm, '#dGridVendaItens');
  else
    var gridSelector = $lx(vm, '#dGriddGridVendaItens');
  //event
  $(document).delegate(gridSelector, 'iggridupdatingeditcellstarting', handle);	
};
{
  if(isNull(dataContext.sharedData['EnableGrid_dGridVendaItens'])){
    console.error("Não foi executada a macro @DisableGridLoaded(dGridVendaItens), no evento loaded da tela.")
    return;
  }
  if('dGridVendaItens'.indexOf('dGrid') > -1)
    var gridSelector = $lx(vm, '#dGridVendaItens');
  ...
  dataContext.sharedData['EnableGrid_dGridVendaItens'] = !true;
  if(!dataContext.sharedData['EnableGrid_dGridVendaItens'] && $(gridSelector).igGridUpdating('isEditing')){
    $(gridSelector).igGridUpdating('endEdit');
  }
  console.log(dataContext.sharedData['EnableGrid_dGridVendaItens'] ? 'Enable Grid [dGridVendaItens]': 'Disable Grid [dGridVendaItens]');

};
*/
    }
    var DesabilitaColuna_Click = function () {
    /*

      {
      if('dGridVendaItens'.indexOf('dGrid') > -1)
      var controlGrid = $lx(vm, '#dGridVendaItens');
      else
      var controlGrid = $lx(vm, '#dGriddGridVendaItens');

      if (controlGrid.length && controlGrid.data('igGrid') != undefined) {
      var columns = controlGrid.igGridUpdating('option', 'columnSettings');
      columns.forEach(function (entry, index) {
      if (entry.columnKey == 'dtDatetimeVendaItem') {
      columns[index]['readOnly'] = true;
      columns[index]['fieldTplDisabled'] = true;
      }
      });
      controlGrid.igGridUpdating('option', 'columnSettings', columns);
      }
      else if (managerAuth.shellMode == 'DEV')
      console.warn("Controle(dGridVendaItens) não encontrado, verifique o nome do mesmo/ou não inicializado.");
      }
    ;


      {
      if('dGridTeste'.indexOf('dGrid') > -1)
      var controlGrid = $lx(vm, '#dGridTeste');
      else
      var controlGrid = $lx(vm, '#dGriddGridTeste');

      if (controlGrid.length && controlGrid.data('igGrid') != undefined) {
      var columns = controlGrid.igGridUpdating('option', 'columnSettings');
      columns.forEach(function (entry, index) {
      if (entry.columnKey == 'NomeCliente') {
      columns[index]['readOnly'] = true;
      columns[index]['fieldTplDisabled'] = true;
      }
      });
      ...
      console.warn("Controle(dGridTeste) não encontrado, verifique o nome do mesmo/ou não inicializado.");
      }
    ;
 Param 1: Prefix + Nome da grid.
 Param 2: Nome do controle.
 Param 3: booleano para desabilitar ou habilitar a coluna na grid.
 Param 4: booleano para desabilitar ou habilitar o campo no editor template.
      ...
      var columns = controlGrid.igGridUpdating('option', 'columnSettings');
      columns.forEach(function (entry, index) {
      if (entry.columnKey == ''Nome'') {
      columns[index]['readOnly'] = getAbsoluteValue(ownerReference.Id) > 10;
      columns[index]['fieldTplDisabled'] =  getAbsoluteValue(ownerReference.Id) > 20;
      }
      });
      controlGrid.igGridUpdating('option', 'columnSettings', columns);
      }
    ...
    ; 
//Disable por nome com fórmula (Id maior que 10);;Disable por nome com fórmula (Id maior que 20)
*/
    }
    ...
    var removeFormViewControl = function () {
        var front = $('#Exemplos_GRIDs_com_macro_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#Exemplos_GRIDs_com_macro_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#Exemplos_GRIDs_com_macro_formViewer')[0];
    ...
    var entitySearchRange = {
        ...
        VendaIdCidade: ko.observable(null),
        VendaNomeCidade: ko.observable(null),
        VendaStringLoja: ko.observable(null),
        VendaStringVendedor: ko.observable(null),
    ...
    entitySearchRange.clear = function(){
        ...
        entitySearchRange.VendaIdCidade(null);
        entitySearchRange.VendaNomeCidade(null);
        entitySearchRange.VendaStringLoja(null);
        entitySearchRange.VendaStringVendedor(null);
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#Exemplos_GRIDs_com_macro_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            dataView: dataView,
            viewName: 'Exemplos_GRIDs_com_macro',
            ...
            rootNamespace: 'Linx.Demo.BV.Grids_e_MacrosDeGrids',
            ...
            DesabilitaGrid_Click: DesabilitaGrid_Click,
            DesabilitaColuna_Click: DesabilitaColuna_Click,
            ...
            __moduleId__: 'pkg_linx-demo-bv-spa/viewmodels/Exemplos_GRIDs_com_macro'
        };
