define(['managers/__auth', 'managers/user'], function (managerAuth, managerUser) {
    var complementCtor = function() {
        var complement = {
            isAutomatic: true
            , ChangedBrandscyLkpEspecializada_dGrid: function(vm, decimals, reset) {
                var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#scyLkpEspecializada_dGrid').data('igGrid'),
                    grdUpd = $('#scyLkpEspecializada_dGrid').data('igGridUpdating');
            ...
            , renderscyLkpEspecializada_dGrid: function(vm) {
                ...
                $('#scyLkpEspecializada_dGrid_headers').live('focus  keydown', function (evt) {
                    var keyCode = window.event ? evt.which : evt.keyCode;
                    if (keyCode === 9) {
                        var cols = $('#scyLkpEspecializada_dGrid').igGrid('option', 'columns');
                        var dataView = $('#scyLkpEspecializada_dGrid').data('igGrid').dataSource._dataView
                        if (dataView.length === 0) return;
                        var firstRow = dataView[0].RowDataId;
                        clear = vm.status() === 'C';
                        if (vm.status() === 'C')
                            $('#scyLkpEspecializada_dGrid').igGridUpdating('startEdit', firstRow, 0, true);
                        else {
                            ...
                            cols.some(function (entry) {
                                if (entry.key !== 'RowDataId' && !entry.hidden) {
                                    if (verifyCanEditCol(entry.key, clear, entity)) {
                                        $('#scyLkpEspecializada_dGrid').igGridUpdating('startEdit', firstRow, indexColumn, true);
                                        return true;
                                    }
                                    indexColumn++;
                                }
                ...
                var getVisibleColumns = function(metaDataControl) {
                   ...
                   var visibleColumns = '';
                   if($('#scyLkpEspecializada_dGrid').data('igGrid') === undefined) return '';
                   var cols = $('#scyLkpEspecializada_dGrid').igGrid('option', 'columns');
                   if (cols) {
                ...
                var isElementHided = function (grid, forceCreating) {
                  if (!grid) grid = $('#scyLkpEspecializada_dGrid');
                  return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialog').is(':visible'));
                }
                var refreshData = true;
                var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'scyLkpEspecializada_dGrid_container', dataBind: function (commitData, forceCreating) {
                   var grid = $('#scyLkpEspecializada_dGrid');
                   ...
                   if (!started) {
                       ...
                       started = true;
                       commitData = false;
                       $('#scyLkpEspecializada_dGrid_groupbyarea').addClass('hide');
                   }
                   if (grid.igGridUpdating('isEditing')) {
                ...
                function verifyCanEditCol(column, clear, entity){
                    switch(column){
                        ...
                        case 'DatetimeCliente': { canEditing = true; break;}
                        ...
                        case 'IdNmCompleto': { canEditing = clear; break;}
                ...
                function createDataGrid(grid) {
                    ...
                    vm.gridSaveStates[gridId] = {
                        ...
                        closePopover: function () {
                            $('#scyLkpEspecializada_dGrid_LayoutBtn').igPopover('hide');
                        },
                    ...
                    grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
                        ...
                        cellClick: function(evt, ui) {
                             if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                                 ...
                                 if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                                 {
                                      ...
                                      showMultimidia(entity, e, table, key, vm.LkpEspecializada());
                                 }
                             }
                             if (typeof vm.OnGridClientClick === 'function') {
                                 entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                 vm.OnGridClientClick('scyLkpEspecializada_dGrid', ui.colKey, entity);
                             }
                             if (vm.status() != 'Q') {
                                 var grid = $('#scyLkpEspecializada_dGrid');
                        ...
                        columns: [
                            ...
                            { key: 'ComboboxCliente', headerText: vm.getLayoutHeaderGrid('ComboboxCliente'), width: '244px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                            { key: 'DatetimeCliente', headerText: vm.getLayoutHeaderGrid('DatetimeCliente'), width: '244px', dataType: 'date', columnCssClass: 'ellipsis', format: 'dd/MM/yyyy', hidden: false, unbound: false, group: null   },
                            { key: 'DecimalCliente', headerText: vm.getLayoutHeaderGrid('DecimalCliente'), width: '231px', dataType: 'number', columnCssClass: 'ellipsis', format: '0.00', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                            ...
                            { key: 'IntCliente', headerText: vm.getLayoutHeaderGrid('IntCliente'), width: '250px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                            { key: 'SmallIntCliente', headerText: vm.getLayoutHeaderGrid('SmallIntCliente'), width: '257px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null   },
                            { key: 'IdNmCompleto', headerText: vm.getLayoutHeaderGrid('IdNmCompleto'), width: '250px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null   },
                            { key: 'StringCliente', headerText: vm.getLayoutHeaderGrid('StringCliente'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'StringEstado', headerText: vm.getLayoutHeaderGrid('StringEstado'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   }
                        ],
                        features: [
                                    ...
                                    filterDialogOpening: function (evt, ui) {
                                        ...
                                        var dataView = $('#scyLkpEspecializada_dGrid').data('igGrid').dataSource;
                                        ...
                                        scriptHtml += '    var newGrid = $("#scyLkpEspecializada_dGrid");';
                                        ...
                                        scriptHtml += '            $("#scyLkpEspecializada_dGrid_container_dialog_list").html(newList)';
                                        scriptHtml += '         }});';
                                    ...
                                    { name: 'Tooltips', columnSettings:[{ columnKey: "ComboboxCliente", allowTooltips: true },{ columnKey: "DatetimeCliente", allowTooltips: true },{ columnKey: "DecimalCliente", allowTooltips: true },{ columnKey: "IdCliente", allowTooltips: true },{ columnKey: "IdEstado", allowTooltips: false },{ columnKey: "IntCliente", allowTooltips: true },{ columnKey: "SmallIntCliente", allowTooltips: true },{ columnKey: "IdNmCompleto", allowTooltips: true },{ columnKey: "StringCliente", allowTooltips: false },{ columnKey: "StringEstado", allowTooltips: true }] },
                                    { name: 'Resizing' }, 
                                    { name: 'Hiding', 
                                    },
                                   ...
                                   ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                      ...
                                      columnSettings: [{ columnKey: "IdEstado", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpEstado", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, activateAutoComplete: false, autoCompleteMaxResults: 7, validateOnClearState:false, maxValue:2147483647, maxLength: 10, defaultValue: null } }, { columnKey: "StringCliente", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LkpTbnmcompleto", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, activateAutoComplete: false, autoCompleteMaxResults: 7, validateOnClearState:false, maxValue:null, maxLength: 50, defaultValue: '' } }, { columnKey: 'DatetimeCliente', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid === 'function') {vm.OnPropertyChangeDataGrid('scyLkpEspecializada_dGrid', 'DatetimeCliente', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: "ComboboxCliente" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyLkpEspecializada_dGrid', 'ComboboxCliente', ui.oldValue, ui.value);}},  maxLength: 3, maxValue: null, minValue: 0, dataMode: 'byte' } }, { columnKey: "DecimalCliente" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyLkpEspecializada_dGrid', 'DecimalCliente', ui.oldValue, ui.value);}},  maxLength: 14, maxValue: 99999999999.99, minValue: 0, dataMode: 'decimal', minDecimals: 2, maxDecimals: 2 } }, { columnKey: "IdCliente" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyLkpEspecializada_dGrid', 'IdCliente', ui.oldValue, ui.value);}},  maxLength: 10, maxValue: null, dataMode: 'int' } }, { columnKey: "IntCliente" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyLkpEspecializada_dGrid', 'IntCliente', ui.oldValue, ui.value);}},  maxLength: 10, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "SmallIntCliente" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyLkpEspecializada_dGrid', 'SmallIntCliente', ui.oldValue, ui.value);}}, maxLength: 5 } }, { columnKey: "IdNmCompleto" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyLkpEspecializada_dGrid', 'IdNmCompleto', ui.oldValue, ui.value);}}, maxLength: 0 } }, { columnKey: "StringEstado" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyLkpEspecializada_dGrid', 'StringEstado', ui.oldValue, ui.value);}}, maxLength: 50 } }],
                                      ...
                                      editCellStarted: function(evt, ui){
                                          ...
                                          var columns = $('#scyLkpEspecializada_dGrid').igGridUpdating('option', 'columnSettings');
                                          var currentCol = null;
                                          currentRow = ui.rowID;
                        ...
                        if ((typeof vm.OnDataGridCreated === 'function')){
                            vm.OnDataGridCreated('scyLkpEspecializada_dGrid');
                        }
                        ...
                        selectionrowselectionchanged = function (evt, ui) {
                            if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                    $(document).undelegate('#scyLkpEspecializada_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                    ...
                                    $(document).delegate ('#scyLkpEspecializada_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                }
                                selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                             } 
                        };
                        $(document).delegate('#scyLkpEspecializada_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        $('#scyLkpEspecializada_dGrid > tbody tr').live('focus', function(evt) {
                            var grid = $('#scyLkpEspecializada_dGrid'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                        ...
                        grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                            if (vm.LkpEspecializada().status() === 'Q') vm.LkpEspecializada().dataToolbar.viewInfo();
                        });
                    }
                    vm.addDataSource({ key: 'scyLkpEspecializada_dGrid', name: 'dataView', itemsSource: itemsSource });
                }
