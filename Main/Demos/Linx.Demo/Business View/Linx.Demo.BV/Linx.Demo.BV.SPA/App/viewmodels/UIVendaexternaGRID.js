...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'UIVendaexternaGRID', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_UIVendaexternaGRID(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'UIVendaexternaGRID' ]; settings.parentVM.UIVendaexternaGRID = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.UIVendaexternaGRID = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('UIVendaexternaGRID', parentVM.internalUIs) === -1) parentVM.internalUIs.push('UIVendaexternaGRID');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-demo-bv-spa-UIVendaexternaGRID', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: UIVendaexternaGRID', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        complement.renderUIVendaexternaGRID_dGridVenda(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                   complement.ChangedBrandUIVendaexternaGRID_dGridVenda(vm, decimals, reset);
    ...
    var visibleColumns = 'BigIntVenda,BitVenda,ComboboxVenda,DatetimeVenda,IdVendedor,DecimalVenda,IdFormaPagamento,IdLoja,IdVenda,IntVenda,SmallIntVenda,ValorTotal';
    ...
    var removeFormViewControl = function () {
        var front = $('#UIVendaexternaGRID_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#UIVendaexternaGRID_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#UIVendaexternaGRID_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#UIVendaexternaGRID_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            __moduleId__: 'pkg_linx-demo-bv-spa/viewmodels/UIVendaexternaGRID'
        };
