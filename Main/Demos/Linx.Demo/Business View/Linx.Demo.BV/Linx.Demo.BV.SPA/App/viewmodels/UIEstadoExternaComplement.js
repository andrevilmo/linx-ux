define(['managers/__auth', 'managers/user'], function (managerAuth, managerUser) {
    var complementCtor = function() {
        var complement = {
            isAutomatic: true
            , ChangedBrandUIEstadoExterna_dGridEstado: function(vm, decimals, reset) {
                var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#UIEstadoExterna_dGridEstado').data('igGrid'),
                    grdUpd = $('#UIEstadoExterna_dGridEstado').data('igGridUpdating');
            ...
            , selectedCurrentItems: function (firstIfNoItem, isSavingData) {
                  var grid = $('#UIEstadoExterna_dGridEstado');
                  var selectedItems = [];
            ...
            , renderUIEstadoExterna_dGridEstado: function(vm) {
                var self = this;
                self.vm = vm;
                ...
                $('#UIEstadoExterna_dGridEstado_headers').live('focus  keydown', function (evt) {
                    var keyCode = window.event ? evt.which : evt.keyCode;
                    if (keyCode === 9) {
                        var cols = $('#UIEstadoExterna_dGridEstado').igGrid('option', 'columns');
                        var dataView = $('#UIEstadoExterna_dGridEstado').data('igGrid').dataSource._dataView
                        if (dataView.length === 0) return;
                        var firstRow = dataView[0].RowDataId;
                        clear = vm.status() === 'C';
                        if (vm.status() === 'C')
                            $('#UIEstadoExterna_dGridEstado').igGridUpdating('startEdit', firstRow, 0, true);
                        else {
                            ...
                            cols.some(function (entry) {
                                if (entry.key !== 'RowDataId' && !entry.hidden) {
                                    if (verifyCanEditCol(entry.key, clear, entity)) {
                                        $('#UIEstadoExterna_dGridEstado').igGridUpdating('startEdit', firstRow, indexColumn, true);
                                        return true;
                                    }
                                    indexColumn++;
                                }
                ...
                var getVisibleColumns = function(metaDataControl) {
                   ...
                   var visibleColumns = '';
                   if($('#UIEstadoExterna_dGridEstado').data('igGrid') === undefined) return '';
                   var cols = $('#UIEstadoExterna_dGridEstado').igGrid('option', 'columns');
                   if (cols) {
                ...
                var isElementHided = function (grid, forceCreating) {
                  if (!grid) grid = $('#UIEstadoExterna_dGridEstado');
                  return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialogEstado').is(':visible'));
                }
                var refreshData = true;
                var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'UIEstadoExterna_dGridEstado_container', dataBind: function (commitData, forceCreating) {
                   var grid = $('#UIEstadoExterna_dGridEstado');
                   ...
                   if (!started) {
                       ...
                       started = true;
                       commitData = false;
                       $('#UIEstadoExterna_dGridEstado_groupbyarea').addClass('hide');
                   }
                   if (grid.igGridUpdating('isEditing')) {
                ...
                function createDataGrid(grid) {
                    ...
                    vm.gridSaveStates[gridId] = {
                        ...
                        closePopover: function () {
                            $('#UIEstadoExterna_dGridEstado_LayoutBtn').igPopover('hide');
                        },
                    ...
                    grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
                        ...
                        cellClick: function(evt, ui) {
                             if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                                 ...
                                 if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                                 {
                                      ...
                                      showMultimidia(entity, e, table, key, vm.UIEstadoExterna());
                                 }
                             }
                             if (typeof vm.OnGridClientClick === 'function') {
                                 entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                 vm.OnGridClientClick('UIEstadoExterna_dGridEstado', ui.colKey, entity);
                             }
                             if (vm.status() != 'Q') {
                                 var grid = $('#UIEstadoExterna_dGridEstado');
                        ...
                        features: [
                                    ...
                                    filterDialogOpening: function (evt, ui) {
                                        ...
                                        var dataView = $('#UIEstadoExterna_dGridEstado').data('igGrid').dataSource;
                                        ...
                                        scriptHtml += '    var newGrid = $("#UIEstadoExterna_dGridEstado");';
                                        ...
                                        scriptHtml += '            $("#UIEstadoExterna_dGridEstado_container_dialog_list").html(newList)';
                                        scriptHtml += '         }});';
                                    ...
                                    { name: 'RowSelectors', enableCheckBoxes: vm.allowMultiSelectionInSearch(), enableRowNumbering: false,  checkBoxStateChanged: function(evt, ui){ 
                                       if ((typeof vm.OnDataGridRowChecked === 'function')){
                                           vm.OnDataGridRowChecked('UIEstadoExterna_dGridEstado', self.selectedItems());
                                       }
                                    ...
                                    { name: 'Tooltips', columnSettings:[{ columnKey: "ComboboxEstado", allowTooltips: true },{ columnKey: "DateTimeEstado", allowTooltips: true },{ columnKey: "DecimalEstado", allowTooltips: true },{ columnKey: "IdEstado", allowTooltips: true },{ columnKey: "IdPais", allowTooltips: false },{ columnKey: "NomeEstado", allowTooltips: true }] },
                                    { name: 'Resizing' }, 
                                    { name: 'Hiding', 
                                    },
                                   ...
                                   ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                      ...
                                      columnSettings: [{ columnKey: "IdPais", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, validateOnClearState:false, maxValue:2147483647, maxLength: 12, defaultValue: null } }, { columnKey: 'DateTimeEstado', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid === 'function') {vm.OnPropertyChangeDataGrid('UIEstadoExterna_dGridEstado', 'DateTimeEstado', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: 'ComboboxEstado', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('ComboboxEstado', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('LX_COMBOBOX_ESTADO', ''),  textKey: 'name', valueKey: 'id', enableClearButton: false }}, { columnKey: "DecimalEstado" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIEstadoExterna_dGridEstado', 'DecimalEstado', ui.oldValue, ui.value);}},  maxLength: 14, maxValue: 99999999999.99, minValue: 0, dataMode: 'decimal', minDecimals: 2, maxDecimals: 2 } }, { columnKey: "IdEstado" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIEstadoExterna_dGridEstado', 'IdEstado', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "NomeEstado" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIEstadoExterna_dGridEstado', 'NomeEstado', ui.oldValue, ui.value);}}, maxLength: 50 } }],
                                      ...
                                      editCellStarted: function(evt, ui){
                                          ...
                                          var columns = $('#UIEstadoExterna_dGridEstado').igGridUpdating('option', 'columnSettings');
                                          var currentCol = null;
                                          currentRow = ui.rowID;
                        ...
                        if ((typeof vm.OnDataGridCreated === 'function')){
                            vm.OnDataGridCreated('UIEstadoExterna_dGridEstado');
                        }
                        ...
                        selectionrowselectionchanged = function (evt, ui) {
                            if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                             ...
                             if ((typeof vm.OnDataGridRowChecked === 'function')){ vm.OnDataGridRowChecked('UIEstadoExterna_dGridEstado', self.selectedItems()); }
                        };
                        $(document).delegate('#UIEstadoExterna_dGridEstado', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                    }
                    vm.addDataSource({ key: 'UIEstadoExterna_dGridEstado', name: 'dataView', itemsSource: itemsSource });
                }
