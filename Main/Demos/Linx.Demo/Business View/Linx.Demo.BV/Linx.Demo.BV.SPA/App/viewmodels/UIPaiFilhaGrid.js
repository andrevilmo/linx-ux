...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'UIPaiFilhaGrid', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_UIPaiFilhaGrid(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'UIPaiFilhaGrid' ]; settings.parentVM.UIPaiFilhaGrid = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('UIPaiFilhaGrid', parentVM.internalUIs) === -1) parentVM.internalUIs.push('UIPaiFilhaGrid');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-demo-bv-spa-UIPaiFilhaGrid', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: UIPaiFilhaGrid', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        complement.renderUIPaiFilhaGrid_dGridLoja(vm);

    $('#UIPaiFilhaGrid_tcLojaTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#UIPaiFilhaGrid_tcLojaTabControl');

    complement.renderUIPaiFilhaGrid_dGridVenda(vm);

    complement.renderUIPaiFilhaGrid_dGridVendaItem(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                   complement.ChangedBrandUIPaiFilhaGrid_dGridLoja(vm, decimals, reset);
                       complement.ChangedBrandUIPaiFilhaGrid_dGridVenda(vm, decimals, reset);

                       complement.ChangedBrandUIPaiFilhaGrid_dGridVendaItem(vm, decimals, reset);
    ...
    var removeFormViewControl = function () {
        var front = $('#UIPaiFilhaGrid_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#UIPaiFilhaGrid_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#UIPaiFilhaGrid_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#UIPaiFilhaGrid_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            __moduleId__: 'pkg_linx-demo-bv-spa/viewmodels/UIPaiFilhaGrid'
        };
