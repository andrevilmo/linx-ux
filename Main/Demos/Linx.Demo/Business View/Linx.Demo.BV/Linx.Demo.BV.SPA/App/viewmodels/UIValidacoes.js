...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'UIValidacoes', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_UIValidacoes(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'UIValidacoes' ]; settings.parentVM.UIValidacoes = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('UIValidacoes', parentVM.internalUIs) === -1) parentVM.internalUIs.push('UIValidacoes');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-demo-bv-spa-UIValidacoes', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: UIValidacoes', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#UIValidacoes_gbPais').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    $('#UIValidacoes_tcPaisTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#UIValidacoes_tcPaisTabControl');

    complement.renderUIValidacoes_dGridEstado(vm);

    complement.renderscyUIValidacoes_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                           complement.ChangedBrandUIValidacoes_dGridEstado(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'Pais';
    ...
    var visibleColumns = 'ComboboxPais,DateTimePais,DecimalPais,IdPais,NomePais';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'Pais', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'Pais', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('Pais', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('Pais');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('Pais');
        ...
        return dataContext.getPaisByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'Pais'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getPaisByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearPais(getBandeiraRede(), clearComplete);
    ...
    var createPais = function() {
        dataBind('dataView', true);
        var entity = dataContext.createPais();
    ...
    var createAndNotifyPais = function() {
        var entity = createPais();
        notifyPresentation('');
        return entity;
    };
    
    var createEstado = function(parent, noCurrent) {
        dataBind('EstadoList', true);
        var entity = dataContext.createEstado(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentEstado(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyEstado = function(parent) {
        var entity = createEstado(parent);
    ...
    var removeFormViewControl = function () {
        var front = $('#UIValidacoes_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#UIValidacoes_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#UIValidacoes_formViewer')[0];
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            PaisDecimalPais_begin: ko.observable(null), PaisDecimalPais_end: ko.observable(null),
        PaisIdPais_begin: ko.observable(null), PaisIdPais_end: ko.observable(null),
    ...
    entitySearchRange.clear = function(){
            entitySearchRange.PaisDecimalPais_begin(null); entitySearchRange.PaisDecimalPais_end(null);
        entitySearchRange.PaisIdPais_begin(null); entitySearchRange.PaisIdPais_end(null);
    ...
    entitySearchRange.has_PaisDecimalPais = ko.computed(function(){ return (entitySearchRange.PaisDecimalPais_begin() != null || entitySearchRange.PaisDecimalPais_end() != null); });
    entitySearchRange.has_PaisIdPais = ko.computed(function(){ return (entitySearchRange.PaisIdPais_begin() != null || entitySearchRange.PaisIdPais_end() != null); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#UIValidacoes_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            createAndNotifyPais: createAndNotifyPais,
            ...
            __moduleId__: 'pkg_linx-demo-bv-spa/viewmodels/UIValidacoes'
        };
