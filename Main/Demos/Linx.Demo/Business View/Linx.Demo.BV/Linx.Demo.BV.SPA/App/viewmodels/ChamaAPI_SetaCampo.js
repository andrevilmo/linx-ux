...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'ChamaAPI_SetaCampo', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_ChamaAPI_SetaCampo(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'ChamaAPI_SetaCampo' ]; settings.parentVM.ChamaAPI_SetaCampo = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.ChamaAPI_SetaCampo = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('ChamaAPI_SetaCampo', parentVM.internalUIs) === -1) parentVM.internalUIs.push('ChamaAPI_SetaCampo');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-demo-bv-spa-ChamaAPI_SetaCampo', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: ChamaAPI_SetaCampo', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#ChamaAPI_SetaCampo_gbProduto').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    complement.renderscyChamaAPI_SetaCampo_dGrid(vm);
    ...
    var produto = getAbsoluteValue(currentDataItem().IdProduto) 
      ...
      (function(){setAbsoluteValue(currentDataItem(), 'DescEstoque', result[0].DESCRICAO); if(currentDataItem().typeName === vm.rootDataTypeName) vm.lazyRefreshBinding();})()
    ;
			  
      (function(){setAbsoluteValue(currentDataItem(), 'QtdeEstoque', result[0].QTDE_PRODUTO); if(currentDataItem().typeName === vm.rootDataTypeName) vm.lazyRefreshBinding();})()
    ;
			  
      (function(){setAbsoluteValue(currentDataItem(), 'DtEntrada', result[0].DATA_ENTRADA); if(currentDataItem().typeName === vm.rootDataTypeName) vm.lazyRefreshBinding();})()
    ;
            }  
    ...
    var removeFormViewControl = function () {
        var front = $('#ChamaAPI_SetaCampo_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#ChamaAPI_SetaCampo_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#ChamaAPI_SetaCampo_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#ChamaAPI_SetaCampo_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            GetProduto_Click: GetProduto_Click,
            ...
            __moduleId__: 'pkg_linx-demo-bv-spa/viewmodels/ChamaAPI_SetaCampo'
        };
