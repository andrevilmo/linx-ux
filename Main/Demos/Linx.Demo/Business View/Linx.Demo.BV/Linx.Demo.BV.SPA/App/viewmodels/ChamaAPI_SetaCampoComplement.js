define(['managers/__auth', 'managers/user'], function (managerAuth, managerUser) {
    var complementCtor = function() {
        var complement = {
            isAutomatic: true
            , ChangedBrandscyChamaAPI_SetaCampo_dGrid: function(vm, decimals, reset) {
                var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#scyChamaAPI_SetaCampo_dGrid').data('igGrid'),
                    grdUpd = $('#scyChamaAPI_SetaCampo_dGrid').data('igGridUpdating');
            ...
            , renderscyChamaAPI_SetaCampo_dGrid: function(vm) {
                ...
                $('#scyChamaAPI_SetaCampo_dGrid_headers').live('focus  keydown', function (evt) {
                    var keyCode = window.event ? evt.which : evt.keyCode;
                    if (keyCode === 9) {
                        var cols = $('#scyChamaAPI_SetaCampo_dGrid').igGrid('option', 'columns');
                        var dataView = $('#scyChamaAPI_SetaCampo_dGrid').data('igGrid').dataSource._dataView
                        if (dataView.length === 0) return;
                        var firstRow = dataView[0].RowDataId;
                        clear = vm.status() === 'C';
                        if (vm.status() === 'C')
                            $('#scyChamaAPI_SetaCampo_dGrid').igGridUpdating('startEdit', firstRow, 0, true);
                        else {
                            ...
                            cols.some(function (entry) {
                                if (entry.key !== 'RowDataId' && !entry.hidden) {
                                    if (verifyCanEditCol(entry.key, clear, entity)) {
                                        $('#scyChamaAPI_SetaCampo_dGrid').igGridUpdating('startEdit', firstRow, indexColumn, true);
                                        return true;
                                    }
                                    indexColumn++;
                                }
                ...
                var getVisibleColumns = function(metaDataControl) {
                   ...
                   var visibleColumns = '';
                   if($('#scyChamaAPI_SetaCampo_dGrid').data('igGrid') === undefined) return '';
                   var cols = $('#scyChamaAPI_SetaCampo_dGrid').igGrid('option', 'columns');
                   if (cols) {
                ...
                var isElementHided = function (grid, forceCreating) {
                  if (!grid) grid = $('#scyChamaAPI_SetaCampo_dGrid');
                  return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialog').is(':visible'));
                }
                var refreshData = true;
                var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'scyChamaAPI_SetaCampo_dGrid_container', dataBind: function (commitData, forceCreating) {
                   var grid = $('#scyChamaAPI_SetaCampo_dGrid');
                   ...
                   if (!started) {
                       ...
                       started = true;
                       commitData = false;
                       $('#scyChamaAPI_SetaCampo_dGrid_groupbyarea').addClass('hide');
                   }
                   if (grid.igGridUpdating('isEditing')) {
                ...
                function verifyCanEditCol(column, clear, entity){
                    switch(column){
                        case 'IdProduto': { canEditing = clear; break;}
                        case 'NmProduto': { canEditing = clear; break;}
                        case 'DescProduto': { canEditing = clear; break;}
                        case 'DataCadastro': { canEditing = clear; break;}
                        case 'TipoUnidade': { canEditing = clear; break;}
                        case 'DescEstoque': { canEditing = clear; break;}
                        case 'QtdeEstoque': { canEditing = clear; break;}
                        case 'DtEntrada': { canEditing = clear; break;}
                    }
                    return canEditing;
                };
                function createDataGrid(grid) {
                    ...
                    vm.gridSaveStates[gridId] = {
                        ...
                        closePopover: function () {
                            $('#scyChamaAPI_SetaCampo_dGrid_LayoutBtn').igPopover('hide');
                        },
                    ...
                    grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
                        ...
                        cellClick: function(evt, ui) {
                             if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                                 ...
                                 if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                                 {
                                      ...
                                      showMultimidia(entity, e, table, key, vm.ChamaAPI_SetaCampo());
                                 }
                             }
                             if (typeof vm.OnGridClientClick === 'function') {
                                 entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                 vm.OnGridClientClick('scyChamaAPI_SetaCampo_dGrid', ui.colKey, entity);
                             }
                             if (vm.status() != 'Q') {
                                 var grid = $('#scyChamaAPI_SetaCampo_dGrid');
                        ...
                        columns: [
                            ...
                            { key: 'IdProduto', headerText: vm.getLayoutHeaderGrid('IdProduto'), width: '250px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                            { key: 'NmProduto', headerText: vm.getLayoutHeaderGrid('NmProduto'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'DescProduto', headerText: vm.getLayoutHeaderGrid('DescProduto'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'DataCadastro', headerText: vm.getLayoutHeaderGrid('DataCadastro'), width: '205px', dataType: 'date', columnCssClass: 'ellipsis', format: 'dd/MM/yyyy', hidden: false, unbound: false, group: null   },
                            { key: 'TipoUnidade', headerText: vm.getLayoutHeaderGrid('TipoUnidade'), width: '192px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                            { key: 'DescEstoque', headerText: vm.getLayoutHeaderGrid('DescEstoque'), width: '250px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'QtdeEstoque', headerText: vm.getLayoutHeaderGrid('QtdeEstoque'), width: '250px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null   },
                            { key: 'DtEntrada', headerText: vm.getLayoutHeaderGrid('DtEntrada'), width: '120px', dataType: 'date', columnCssClass: 'ellipsis', format: 'dd/MM/yyyy', hidden: false, unbound: false, group: null   }
                        ],
                        features: [
                                    ...
                                    filterDialogOpening: function (evt, ui) {
                                        ...
                                        var dataView = $('#scyChamaAPI_SetaCampo_dGrid').data('igGrid').dataSource;
                                        ...
                                        scriptHtml += '    var newGrid = $("#scyChamaAPI_SetaCampo_dGrid");';
                                        ...
                                        scriptHtml += '            $("#scyChamaAPI_SetaCampo_dGrid_container_dialog_list").html(newList)';
                                        scriptHtml += '         }});';
                                    ...
                                    { name: 'Tooltips', columnSettings:[{ columnKey: "IdProduto", allowTooltips: true },{ columnKey: "NmProduto", allowTooltips: true },{ columnKey: "DescProduto", allowTooltips: true },{ columnKey: "DataCadastro", allowTooltips: true },{ columnKey: "TipoUnidade", allowTooltips: true },{ columnKey: "DescEstoque", allowTooltips: true },{ columnKey: "QtdeEstoque", allowTooltips: true },{ columnKey: "DtEntrada", allowTooltips: true }] },
                                    { name: 'Resizing' }, 
                                    { name: 'Hiding', 
                                    },
                                   ...
                                   ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                      ...
                                      columnSettings: [{ columnKey: 'DataCadastro', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid === 'function') {vm.OnPropertyChangeDataGrid('scyChamaAPI_SetaCampo_dGrid', 'DataCadastro', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: "IdProduto" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyChamaAPI_SetaCampo_dGrid', 'IdProduto', ui.oldValue, ui.value);}},  maxLength: 10, maxValue: null, dataMode: 'int' } }, { columnKey: "NmProduto" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyChamaAPI_SetaCampo_dGrid', 'NmProduto', ui.oldValue, ui.value);}}, maxLength: 50 } }, { columnKey: "DescProduto" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyChamaAPI_SetaCampo_dGrid', 'DescProduto', ui.oldValue, ui.value);}}, maxLength: 100 } }, { columnKey: "TipoUnidade" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyChamaAPI_SetaCampo_dGrid', 'TipoUnidade', ui.oldValue, ui.value);}},  maxLength: 3, maxValue: null, minValue: 0, dataMode: 'byte' } }, { columnKey: "DescEstoque" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyChamaAPI_SetaCampo_dGrid', 'DescEstoque', ui.oldValue, ui.value);}}, maxLength: 0 } }, { columnKey: "QtdeEstoque" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyChamaAPI_SetaCampo_dGrid', 'QtdeEstoque', ui.oldValue, ui.value);}}, maxLength: 0 } }, { columnKey: "DtEntrada" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyChamaAPI_SetaCampo_dGrid', 'DtEntrada', ui.oldValue, ui.value);}}, maxLength: 0 } }],
                                      ...
                                      editCellStarted: function(evt, ui){
                                          ...
                                          var columns = $('#scyChamaAPI_SetaCampo_dGrid').igGridUpdating('option', 'columnSettings');
                                          var currentCol = null;
                                          currentRow = ui.rowID;
                        ...
                        if ((typeof vm.OnDataGridCreated === 'function')){
                            vm.OnDataGridCreated('scyChamaAPI_SetaCampo_dGrid');
                        }
                        ...
                        selectionrowselectionchanged = function (evt, ui) {
                            if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                    $(document).undelegate('#scyChamaAPI_SetaCampo_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                    ...
                                    $(document).delegate ('#scyChamaAPI_SetaCampo_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                }
                                selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                             } 
                        };
                        $(document).delegate('#scyChamaAPI_SetaCampo_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        $('#scyChamaAPI_SetaCampo_dGrid > tbody tr').live('focus', function(evt) {
                            var grid = $('#scyChamaAPI_SetaCampo_dGrid'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                        ...
                        grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                            if (vm.ChamaAPI_SetaCampo().status() === 'Q') vm.ChamaAPI_SetaCampo().dataToolbar.viewInfo();
                        });
                    }
                    vm.addDataSource({ key: 'scyChamaAPI_SetaCampo_dGrid', name: 'dataView', itemsSource: itemsSource });
                }
