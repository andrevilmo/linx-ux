...
var vmConstructor = function () {
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_ExAutoComp(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'ExAutoComp' ]; settings.parentVM.ExAutoComp = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('ExAutoComp', parentVM.internalUIs) === -1) parentVM.internalUIs.push('ExAutoComp');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-demo-bv-spa-ExAutoComp', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: ExAutoComp', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#ExAutoComp_gbTbnmcompleto').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    complement.renderscyExAutoComp_dGrid(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'Tbnmcompleto';
    ...
    var visibleColumns = 'idNomeCompleto,NomeCompleto,IdNome,idnomeMeio,IdSobrenome,Nome,Nomedomeio,SobreNome,IdCliente';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'Tbnmcompleto', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'Tbnmcompleto', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('Tbnmcompleto', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('Tbnmcompleto');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('Tbnmcompleto');
        ...
        return dataContext.getTbnmcompletoByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'Tbnmcompleto'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTbnmcompletoByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTbnmcompleto(getBandeiraRede(), clearComplete);
    ...
    var createTbnmcompleto = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTbnmcompleto();
    ...
    var createAndNotifyTbnmcompleto = function() {
        var entity = createTbnmcompleto();
        notifyPresentation('');
        return entity;
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#ExAutoComp_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#ExAutoComp_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#ExAutoComp_formViewer')[0];
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TbnmcompletoIdCliente_begin: ko.observable(null), TbnmcompletoIdCliente_end: ko.observable(null),
        TbnmcompletoidNomeCompleto_begin: ko.observable(null), TbnmcompletoidNomeCompleto_end: ko.observable(null),
        TbnmcompletoIdNome: ko.observable(null),
        TbnmcompletoidnomeMeio: ko.observable(null),
        TbnmcompletoIdSobrenome: ko.observable(null),
        TbnmcompletoNome: ko.observable(null),
        TbnmcompletoNomedomeio: ko.observable(null),
        TbnmcompletoSobreNome: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TbnmcompletoIdCliente_begin(null); entitySearchRange.TbnmcompletoIdCliente_end(null);
        entitySearchRange.TbnmcompletoidNomeCompleto_begin(null); entitySearchRange.TbnmcompletoidNomeCompleto_end(null);
        entitySearchRange.TbnmcompletoIdNome(null);
        entitySearchRange.TbnmcompletoidnomeMeio(null);
        entitySearchRange.TbnmcompletoIdSobrenome(null);
        entitySearchRange.TbnmcompletoNome(null);
        entitySearchRange.TbnmcompletoNomedomeio(null);
        entitySearchRange.TbnmcompletoSobreNome(null);
    };
    entitySearchRange.has_TbnmcompletoIdCliente = ko.computed(function(){ return (entitySearchRange.TbnmcompletoIdCliente_begin() != null || entitySearchRange.TbnmcompletoIdCliente_end() != null); });
    entitySearchRange.has_TbnmcompletoidNomeCompleto = ko.computed(function(){ return (entitySearchRange.TbnmcompletoidNomeCompleto_begin() != null || entitySearchRange.TbnmcompletoidNomeCompleto_end() != null); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#ExAutoComp_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: 'TBNMCOMPLETO',
            rootDataTypeName: 'Tbnmcompleto',
            ...
            createAndNotifyTbnmcompleto: createAndNotifyTbnmcompleto,
            ...
            __moduleId__: 'pkg_linx-demo-bv-spa/viewmodels/ExAutoComp'
        };
