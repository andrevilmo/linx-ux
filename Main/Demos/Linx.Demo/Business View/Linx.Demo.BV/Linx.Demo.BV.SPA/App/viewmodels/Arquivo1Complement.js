define(['managers/__auth', 'managers/user'], function (managerAuth, managerUser) {
    var complementCtor = function() {
        var complement = {
            isAutomatic: true
            , ChangedBrandArquivo1_dGridArquivo: function(vm, decimals, reset) {
                var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#Arquivo1_dGridArquivo').data('igGrid'),
                    grdUpd = $('#Arquivo1_dGridArquivo').data('igGridUpdating');
            ...
            , renderArquivo1_dGridArquivo: function(vm) {
                ...
                $('#Arquivo1_dGridArquivo_headers').live('focus  keydown', function (evt) {
                    var keyCode = window.event ? evt.which : evt.keyCode;
                    if (keyCode === 9) {
                        var cols = $('#Arquivo1_dGridArquivo').igGrid('option', 'columns');
                        var dataView = $('#Arquivo1_dGridArquivo').data('igGrid').dataSource._dataView
                        if (dataView.length === 0) return;
                        var firstRow = dataView[0].RowDataId;
                        clear = vm.status() === 'C';
                        if (vm.status() === 'C')
                            $('#Arquivo1_dGridArquivo').igGridUpdating('startEdit', firstRow, 0, true);
                        else {
                            ...
                            cols.some(function (entry) {
                                if (entry.key !== 'RowDataId' && !entry.hidden) {
                                    if (verifyCanEditCol(entry.key, clear, entity)) {
                                        $('#Arquivo1_dGridArquivo').igGridUpdating('startEdit', firstRow, indexColumn, true);
                                        return true;
                                    }
                                    indexColumn++;
                                }
                ...
                var getVisibleColumns = function(metaDataControl) {
                   ...
                   var visibleColumns = '';
                   if($('#Arquivo1_dGridArquivo').data('igGrid') === undefined) return '';
                   var cols = $('#Arquivo1_dGridArquivo').igGrid('option', 'columns');
                   if (cols) {
                ...
                var isElementHided = function (grid, forceCreating) {
                  if (!grid) grid = $('#Arquivo1_dGridArquivo');
                  return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialogArquivo').is(':visible'));
                }
                var refreshData = true;
                var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'Arquivo1_dGridArquivo_container', dataBind: function (commitData, forceCreating) {
                   var grid = $('#Arquivo1_dGridArquivo');
                   ...
                   if (!started) {
                       ...
                       started = true;
                       commitData = false;
                       $('#Arquivo1_dGridArquivo_groupbyarea').addClass('hide');
                   }
                   if (grid.igGridUpdating('isEditing')) {
                   ...
                   if (rows.length > 0) {
                     ...
                     if ($('#dialogArquivo').is(':visible')) {
                        ...
                        if (hasPaging.length > 0) {
                           ...
                           $('label#currentNumberArquivo').html(current + ' - ' + totalCurrentPage);
                        }
                        else
                           $('label#currentNumberArquivo').html(1);
                        $('label#totalNumberArquivo').html(totalGrid);
                    }
                   } else {
                       $('label#currentNumberArquivo').html(0);
                       $('label#totalNumberArquivo').html(0);
                   }
                }};
                var valueGrouBy = -1;
                var deletedIndex = -1;
                function verifyCanEditCol(column, clear, entity){
                    switch(column){
                        case 'NomeArquivo': { canEditing = clear; break;}
                    }
                    return canEditing;
                };
                function createDataGrid(grid) {
                    ...
                    vm.gridSaveStates[gridId] = {
                        ...
                        closePopover: function () {
                            $('#Arquivo1_dGridArquivo_LayoutBtn').igPopover('hide');
                        },
                    ...
                    grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
                        ...
                        cellClick: function(evt, ui) {
                             if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                                 ...
                                 if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                                 {
                                      ...
                                      showMultimidia(entity, e, table, key, vm.Arquivo1());
                                 }
                             }
                             if (typeof vm.OnGridClientClick === 'function') {
                                 entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                 vm.OnGridClientClick('Arquivo1_dGridArquivo', ui.colKey, entity);
                             }
                             if (vm.status() != 'Q') {
                                 var grid = $('#Arquivo1_dGridArquivo');
                        ...
                        columns: [
                            ...
                            { key: 'NomeArquivo', headerText: vm.getLayoutHeaderGrid('NomeArquivo'), width: '0px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   }
                        ],
                        features: [
                                    ...
                                    filterDialogOpening: function (evt, ui) {
                                        ...
                                        var dataView = $('#Arquivo1_dGridArquivo').data('igGrid').dataSource;
                                        ...
                                        scriptHtml += '    var newGrid = $("#Arquivo1_dGridArquivo");';
                                        ...
                                        scriptHtml += '            $("#Arquivo1_dGridArquivo_container_dialog_list").html(newList)';
                                        scriptHtml += '         }});';
                                    ...
                                    { name: 'Tooltips', columnSettings:[{ columnKey: "NomeArquivo", allowTooltips: true }] },
                                    { name: 'Resizing' }, 
                                    { name: 'Hiding', 
                                    },
                                   ...
                                   ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                      ...
                                      columnSettings: [{ columnKey: "NomeArquivo" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('Arquivo1_dGridArquivo', 'NomeArquivo', ui.oldValue, ui.value);}}, maxLength: 0 } }],
                                      ...
                                      editCellStarted: function(evt, ui){
                                          ...
                                          var columns = $('#Arquivo1_dGridArquivo').igGridUpdating('option', 'columnSettings');
                                          var currentCol = null;
                                          currentRow = ui.rowID;
                        ...
                        if ((typeof vm.OnDataGridCreated === 'function')){
                            vm.OnDataGridCreated('Arquivo1_dGridArquivo');
                        }
                        ...
                        selectionrowselectionchanged = function (evt, ui) {
                            if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                    $(document).undelegate('#Arquivo1_dGridArquivo', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                    ...
                                    $(document).delegate ('#Arquivo1_dGridArquivo', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                }
                                selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                             } 
                        };
                        $(document).delegate('#Arquivo1_dGridArquivo', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        $('#Arquivo1_dGridArquivo > tbody tr').live('focus', function(evt) {
                            var grid = $('#Arquivo1_dGridArquivo'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                    ...
                    vm.addDataSource({ key: 'Arquivo1_dGridArquivo', name: 'dataView', itemsSource: itemsSource });
                }
