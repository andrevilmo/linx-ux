...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'ChamaAPI_SetaGrid', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_ChamaAPI_SetaGrid(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'ChamaAPI_SetaGrid' ]; settings.parentVM.ChamaAPI_SetaGrid = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.ChamaAPI_SetaGrid = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('ChamaAPI_SetaGrid', parentVM.internalUIs) === -1) parentVM.internalUIs.push('ChamaAPI_SetaGrid');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-demo-bv-spa-ChamaAPI_SetaGrid', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: ChamaAPI_SetaGrid', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#ChamaAPI_SetaGrid_gbProduto').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    $('#ChamaAPI_SetaGrid_gbExpander_f524ac80d5064adfaef08a710b431310').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    complement.renderChamaAPI_SetaGrid_dGridestoque(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                       complement.ChangedBrandChamaAPI_SetaGrid_dGridestoque(vm, decimals, reset);
    ...
    var visibleColumns = 'IdProduto,NmProduto,DescProduto,DataCadastro,TipoUnidade,DescEstoque,QtdeEstoque,DtEntrada';
    ...
    var getQueryFilter = function (currentDI) {
        ...
        if (eSearch === 'Error')
           return 'Error';
        var extraFilter = OnSearching();
        if (extraFilter === 'Error')
           return 'Error';
        if (!isNullOrEmpty(extraFilter)) eSearch += extraFilter;
    ...
    var CarregaEstoque_Click = function () {
    
var produto = getAbsoluteValue(currentDataItem().IdProduto) 

      {
      var coluna = ['DescEstoque','QtdeEstoque','DtEntrada'];
      var valor = ['Descrição','Quantidade','Data'];

      var grid = $lx(vm, '#ChamaAPI_SetaGrid_dGridestoque');
      ...
      var selectedColumn = gridColumns.filter(function (ele) { if (coluna.contains(ele.key)) return ele; });

      $.each(selectedColumn, function (i, e){
      e.headerText = valor[i];
      grid.igGrid("headersTable").find('th[id="' + gridId + '_' + e.key + '"] > a > span.ui-iggrid-headertext').text(valor[i]);
      grid.igGrid("headersTable").find('th[id="' + gridId + '_' + e.key + '"] > span.ui-iggrid-headertext').text(valor[i]);
      $lx(vm, '#lbl' + e.key).text(valor[i]);
      });
	...
	if (result.length > 0)
		{
			var tabela = $lx(vm, '#ChamaAPI_SetaGrid_dGridestoque');
			var resultado=[];
			debugger
			for (var i=0; i<result.length;i++)
			{
				resultado.push({ "RowDataId":result[i].ID_ESTOQUE,
								"DescEstoque":result[i].DESCRICAO,
								"QtdeEstoque": result[i].QTDE_PRODUTO,
								"DtEntrada": result[i].DATA_ENTRADA })
			}
			$(tabela).igGrid({dataSource: resultado});
		}
    ...
    var CarregaEstoque2_Click = function () {
...
/*
    ...
    ;*/
      ...
      var urlDef = '';
      if (!isNullOrEmpty(controllerName)) { urlDef = vm.getDataContext().getServiceAddress(controllerName) + '/'; }
      urlDef += "BuscaEstoquePorProduto2?idproduto="+produto;
	...
	if (result.length > 0)
		{
			...
			for (var i=0; i<result.length;i++)
			{
				resultado.push({ "RowDataId":result[i].idestoque,
								"DescEstoque":result[i].descricao,
								"QtdeEstoque": result[i].qtda,
								"DtEntrada": result[i].dtentrada })								
			}
			$(tabela).igGrid({dataSource: resultado});
		}
    ...
    var CarregaEstoque3_Click = function () {
...
var produto = getAbsoluteValue(currentDataItem().IdProduto) 
      ...
      var urlDef = '';
      if (!isNullOrEmpty(controllerName)) { urlDef = vm.getDataContext().getServiceAddress(controllerName) + '/'; }
      urlDef += "BuscaEstoquePorProduto?idproduto="+produto;
...
//var produto = getAbsoluteValue(currentDataItem().IdProduto);
/*BuscaEstoquePorIdProduto(produto);*/
...
/*
    ...
    var removeFormViewControl = function () {
        var front = $('#ChamaAPI_SetaGrid_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#ChamaAPI_SetaGrid_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#ChamaAPI_SetaGrid_formViewer')[0];
    ...
    var entitySearchRange = {
        ...
        ProdutoDtEntrada_typeRange: ko.observable('R'), ProdutoDtEntrada_begin: ko.observable(null), ProdutoDtEntrada_end: ko.observable(null), ProdutoDtEntrada_predefFilter: ko.observableArray([]), ProdutoDtEntrada_predefValue: ko.observable(null),
    ...
    entitySearchRange.clear = function(){
        ...
        entitySearchRange.ProdutoDtEntrada_typeRange('R'); entitySearchRange.ProdutoDtEntrada_begin(null); entitySearchRange.ProdutoDtEntrada_end(null); entitySearchRange.ProdutoDtEntrada_predefFilter([]); entitySearchRange.ProdutoDtEntrada_predefValue(null);
    ...
    entitySearchRange.has_ProdutoDtEntrada = ko.computed(function(){ return (entitySearchRange.ProdutoDtEntrada_typeRange() == 'R' && (entitySearchRange.ProdutoDtEntrada_begin() != null || entitySearchRange.ProdutoDtEntrada_end() != null) || (entitySearchRange.ProdutoDtEntrada_typeRange() == 'P' && entitySearchRange.ProdutoDtEntrada_predefFilter().length > 0)); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#ChamaAPI_SetaGrid_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootNamespace: 'Linx.Demo.BV.Graf_dash_KPI',
            ...
            CarregaEstoque_Click: CarregaEstoque_Click,
            CarregaEstoque2_Click: CarregaEstoque2_Click,
            CarregaEstoque3_Click: CarregaEstoque3_Click,
            ...
            __moduleId__: 'pkg_linx-demo-bv-spa/viewmodels/ChamaAPI_SetaGrid'
        };
