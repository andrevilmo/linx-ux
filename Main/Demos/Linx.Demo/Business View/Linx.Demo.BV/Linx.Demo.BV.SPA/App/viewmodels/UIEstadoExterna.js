...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'UIEstadoExterna', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_UIEstadoExterna(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'UIEstadoExterna' ]; settings.parentVM.UIEstadoExterna = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.UIEstadoExterna = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('UIEstadoExterna', parentVM.internalUIs) === -1) parentVM.internalUIs.push('UIEstadoExterna');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-demo-bv-spa-UIEstadoExterna', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: UIEstadoExterna', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        complement.renderUIEstadoExterna_dGridEstado(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                   complement.ChangedBrandUIEstadoExterna_dGridEstado(vm, decimals, reset);
    ...
    var visibleColumns = 'ComboboxEstado,DateTimeEstado,DecimalEstado,IdEstado,IdPais,NomeEstado';
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('Estado', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('Estado');
           var result = [];
           var relationExpr = '';
    ...
    var removeFormViewControl = function () {
        var front = $('#UIEstadoExterna_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#UIEstadoExterna_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#UIEstadoExterna_formViewer')[0];
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#UIEstadoExterna_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            __moduleId__: 'pkg_linx-demo-bv-spa/viewmodels/UIEstadoExterna'
        };
