...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'MarcasFamosas', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_MarcasFamosas(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'MarcasFamosas' ]; settings.parentVM.MarcasFamosas = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('MarcasFamosas', parentVM.internalUIs) === -1) parentVM.internalUIs.push('MarcasFamosas');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-demo-bv-spa-MarcasFamosas', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: MarcasFamosas', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#MarcasFamosas_gbTesteCkbView').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    complement.renderscyMarcasFamosas_dGrid(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TesteCkbView';
    ...
    var visibleColumns = 'IdQualquer,NaoObrigatorio,Obrigatorio';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TesteCkbView', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TesteCkbView', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TesteCkbView', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TesteCkbView');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TesteCkbView');
        ...
        return dataContext.getTesteCkbViewByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TesteCkbView'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTesteCkbViewByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTesteCkbView(getBandeiraRede(), clearComplete);
    ...
    var createTesteCkbView = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTesteCkbView();
    ...
    var createAndNotifyTesteCkbView = function() {
        var entity = createTesteCkbView();
        notifyPresentation('');
        return entity;
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#MarcasFamosas_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#MarcasFamosas_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#MarcasFamosas_formViewer')[0];
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TesteCkbViewIdQualquer_begin: ko.observable(null), TesteCkbViewIdQualquer_end: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TesteCkbViewIdQualquer_begin(null); entitySearchRange.TesteCkbViewIdQualquer_end(null);
    };
    entitySearchRange.has_TesteCkbViewIdQualquer = ko.computed(function(){ return (entitySearchRange.TesteCkbViewIdQualquer_begin() != null || entitySearchRange.TesteCkbViewIdQualquer_end() != null); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#MarcasFamosas_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: 'TesteCkb',
            rootDataTypeName: 'TesteCkbView',
            ...
            createAndNotifyTesteCkbView: createAndNotifyTesteCkbView,
            ...
            __moduleId__: 'pkg_linx-demo-bv-spa/viewmodels/MarcasFamosas'
        };
