...
var vmConstructor = function () {
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_UIExterna(), 'Name'))());
        }, function (err) {
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#UIExterna_gbCustomContainer_6857ba8d00974468904b3b916a35109c').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    $('#UIExterna_gbLoja').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    $('#UIExterna_tcTabControl_0c64bcba91b84ea4a75423aa1e8b4d5f').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#UIExterna_tcTabControl_0c64bcba91b84ea4a75423aa1e8b4d5f');
    ...
    var visibleColumns = 'BigIntLoja,BitLoja,ComboboxLoja,DatetimeLoja,IdCidade,IdPais,DecimalLoja,IdLoja,IntLoja,NomeLoja,SmallIntLoja,IdEstado';
    ...
    var CustomControl3975155_Click = function () {
    
      var str = 'Linx.Demo.BV.SPA/UIPaisExterna'; while (str.indexOf('.') !== -1) { str = str.replace('.', '-'); };
      var currentModal = (modal.inUse ? modal2 : modal);
      currentModal.showModal('pkg_' + strExtract('#' + str.toLowerCase(), '#', '/') + '/viewmodels/' + strExtract(str + '#', '/', '#'), {  executeQuery: true, parentUI: {vm: vm}, querySetters: {}, toolbarSettings: {removeDataToolbar:false,canAddNew:true,canClear:true,canCustomSearch:true,canDelete:true,canEdit:true,canLayout:true,canNavigate:true,canPrint:true,canSearch:true} }, 'Title', ['Ok'], 'large')
    
    }
    var OpenView_Click = function () {
    
      var str = 'Linx.Demo.BV.SPA/UIPaisExterna'.toLowerCase();
      while (str.indexOf('.') !== -1) { str = str.replace('.', '-').replace('/', '-'); };
      vm.common.go('#' + str, 'objectQuery=IdPais:1&executeQuery=true')
    ...
    var onSavingValidation = function (changes) {
        ...
        if (changes.length === 0) { undo(changes); return true; }
    ...
    var vm = {
            ...
            CustomControl3975155_Click: CustomControl3975155_Click,
            OpenView_Click: OpenView_Click,
