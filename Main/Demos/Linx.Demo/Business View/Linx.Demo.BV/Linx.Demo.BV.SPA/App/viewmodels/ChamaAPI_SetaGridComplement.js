define(['managers/__auth', 'managers/user'], function (managerAuth, managerUser) {
    var complementCtor = function() {
        var complement = {
            isAutomatic: true
            , ChangedBrandChamaAPI_SetaGrid_dGridestoque: function(vm, decimals, reset) {
                var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#ChamaAPI_SetaGrid_dGridestoque').data('igGrid'),
                    grdUpd = $('#ChamaAPI_SetaGrid_dGridestoque').data('igGridUpdating');
            ...
            , renderChamaAPI_SetaGrid_dGridestoque: function(vm) {
                ...
                $('#ChamaAPI_SetaGrid_dGridestoque_headers').live('focus  keydown', function (evt) {
                    var keyCode = window.event ? evt.which : evt.keyCode;
                    if (keyCode === 9) {
                        var cols = $('#ChamaAPI_SetaGrid_dGridestoque').igGrid('option', 'columns');
                        var dataView = $('#ChamaAPI_SetaGrid_dGridestoque').data('igGrid').dataSource._dataView
                        if (dataView.length === 0) return;
                        var firstRow = dataView[0].RowDataId;
                        clear = vm.status() === 'C';
                        if (vm.status() === 'C')
                            $('#ChamaAPI_SetaGrid_dGridestoque').igGridUpdating('startEdit', firstRow, 0, true);
                        else {
                            ...
                            cols.some(function (entry) {
                                if (entry.key !== 'RowDataId' && !entry.hidden) {
                                    if (verifyCanEditCol(entry.key, clear, entity)) {
                                        $('#ChamaAPI_SetaGrid_dGridestoque').igGridUpdating('startEdit', firstRow, indexColumn, true);
                                        return true;
                                    }
                                    indexColumn++;
                                }
                ...
                var getVisibleColumns = function(metaDataControl) {
                   ...
                   var visibleColumns = '';
                   if($('#ChamaAPI_SetaGrid_dGridestoque').data('igGrid') === undefined) return '';
                   var cols = $('#ChamaAPI_SetaGrid_dGridestoque').igGrid('option', 'columns');
                   if (cols) {
                ...
                var isElementHided = function (grid, forceCreating) {
                  if (!grid) grid = $('#ChamaAPI_SetaGrid_dGridestoque');
                  return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialogGroupBox_ae6b4967089b44a2a5eaeb60b3b9c6e5').is(':visible'));
                }
                var refreshData = true;
                var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'ChamaAPI_SetaGrid_dGridestoque_container', dataBind: function (commitData, forceCreating) {
                   var grid = $('#ChamaAPI_SetaGrid_dGridestoque');
                   ...
                   if (!started) {
                       ...
                       started = true;
                       commitData = false;
                       $('#ChamaAPI_SetaGrid_dGridestoque_groupbyarea').addClass('hide');
                   }
                   if (grid.igGridUpdating('isEditing')) {
                   ...
                   if (rows.length > 0) {
                     ...
                     if ($('#dialogGroupBox_ae6b4967089b44a2a5eaeb60b3b9c6e5').is(':visible')) {
                        ...
                        if (hasPaging.length > 0) {
                           ...
                           $('label#currentNumberGroupBox_ae6b4967089b44a2a5eaeb60b3b9c6e5').html(current + ' - ' + totalCurrentPage);
                        }
                        else
                           $('label#currentNumberGroupBox_ae6b4967089b44a2a5eaeb60b3b9c6e5').html(1);
                        $('label#totalNumberGroupBox_ae6b4967089b44a2a5eaeb60b3b9c6e5').html(totalGrid);
                    }
                   } else {
                       $('label#currentNumberGroupBox_ae6b4967089b44a2a5eaeb60b3b9c6e5').html(0);
                       $('label#totalNumberGroupBox_ae6b4967089b44a2a5eaeb60b3b9c6e5').html(0);
                   }
                }};
                var valueGrouBy = -1;
                var deletedIndex = -1;
                ...
                function createDataGrid(grid) {
                    ...
                    vm.gridSaveStates[gridId] = {
                        ...
                        closePopover: function () {
                            $('#ChamaAPI_SetaGrid_dGridestoque_LayoutBtn').igPopover('hide');
                        },
                    ...
                    grid.igGrid({ height: (getGridHeightSuggested() * 0.75), width: '100%',
                        ...
                        cellClick: function(evt, ui) {
                             if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                                 ...
                                 if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                                 {
                                      ...
                                      showMultimidia(entity, e, table, key, vm.ChamaAPI_SetaGrid());
                                 }
                             }
                             if (typeof vm.OnGridClientClick === 'function') {
                                 entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                 vm.OnGridClientClick('ChamaAPI_SetaGrid_dGridestoque', ui.colKey, entity);
                             }
                             if (vm.status() != 'Q') {
                                 var grid = $('#ChamaAPI_SetaGrid_dGridestoque');
                        ...
                        columns: [
                            ...
                            { key: 'DescEstoque', headerText: vm.getLayoutHeaderGrid('DescEstoque'), width: '257px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                        ...
                        features: [
                                    ...
                                    filterDialogOpening: function (evt, ui) {
                                        ...
                                        var dataView = $('#ChamaAPI_SetaGrid_dGridestoque').data('igGrid').dataSource;
                                        ...
                                        scriptHtml += '    var newGrid = $("#ChamaAPI_SetaGrid_dGridestoque");';
                                        ...
                                        scriptHtml += '            $("#ChamaAPI_SetaGrid_dGridestoque_container_dialog_list").html(newList)';
                                        scriptHtml += '         }});';
                                    ...
                                    { name: 'Tooltips', columnSettings:[{ columnKey: "DescEstoque", allowTooltips: true },{ columnKey: "QtdeEstoque", allowTooltips: true },{ columnKey: "DtEntrada", allowTooltips: true }] },
                                    { name: 'Resizing' }, 
                                    { name: 'Hiding', 
                                    },
                                   ...
                                   ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                      ...
                                      columnSettings: [{ columnKey: 'DtEntrada', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid === 'function') {vm.OnPropertyChangeDataGrid('ChamaAPI_SetaGrid_dGridestoque', 'DtEntrada', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: "DescEstoque" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('ChamaAPI_SetaGrid_dGridestoque', 'DescEstoque', ui.oldValue, ui.value);}}, maxLength: 0 } }, { columnKey: "QtdeEstoque" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('ChamaAPI_SetaGrid_dGridestoque', 'QtdeEstoque', ui.oldValue, ui.value);}}, maxLength: 0 } }],
                                      ...
                                      editCellStarted: function(evt, ui){
                                          ...
                                          var columns = $('#ChamaAPI_SetaGrid_dGridestoque').igGridUpdating('option', 'columnSettings');
                                          var currentCol = null;
                                          currentRow = ui.rowID;
                        ...
                        if ((typeof vm.OnDataGridCreated === 'function')){
                            vm.OnDataGridCreated('ChamaAPI_SetaGrid_dGridestoque');
                        }
                        ...
                        selectionrowselectionchanged = function (evt, ui) {
                            if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                    $(document).undelegate('#ChamaAPI_SetaGrid_dGridestoque', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                    ...
                                    $(document).delegate ('#ChamaAPI_SetaGrid_dGridestoque', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                }
                                selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                             } 
                        };
                        $(document).delegate('#ChamaAPI_SetaGrid_dGridestoque', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        $('#ChamaAPI_SetaGrid_dGridestoque > tbody tr').live('focus', function(evt) {
                            var grid = $('#ChamaAPI_SetaGrid_dGridestoque'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                    ...
                    vm.addDataSource({ key: 'ChamaAPI_SetaGrid_dGridestoque', name: 'dataView', itemsSource: itemsSource });
                }
