define(['managers/__auth', 'managers/user'], function (managerAuth, managerUser) {
    var complementCtor = function() {
        var complement = {
            isAutomatic: true
            , ChangedBrandUIValidacoes_dGridEstado: function(vm, decimals, reset) {
                var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#UIValidacoes_dGridEstado').data('igGrid'),
                    grdUpd = $('#UIValidacoes_dGridEstado').data('igGridUpdating');
            ...
            , renderUIValidacoes_dGridEstado: function(vm) {
                var getDataSource = function() {
                    var source = null;
                    try {
                        source = vm.currentDataItem().EstadoList;
                    }
                    catch (e) { }
                ...
                var dataSourceIsLoaded = function() {
                    var isLoaded = false;
                    try {
                        isLoaded = (vm.currentDataItem().EstadoIsLoaded === true || vm.currentDataItem().EstadoList().length > 0);
                    }
                    catch (e) {
                        isLoaded = true;
                    }
                ...
                $('#UIValidacoes_dGridEstado_headers').live('focus  keydown', function (evt) {
                    var keyCode = window.event ? evt.which : evt.keyCode;
                    if (keyCode === 9) {
                        var cols = $('#UIValidacoes_dGridEstado').igGrid('option', 'columns');
                        var dataView = $('#UIValidacoes_dGridEstado').data('igGrid').dataSource._dataView
                        if (dataView.length === 0) return;
                        var firstRow = dataView[0].RowDataId;
                        clear = vm.status() === 'C';
                        if (vm.status() === 'C')
                            $('#UIValidacoes_dGridEstado').igGridUpdating('startEdit', firstRow, 0, true);
                        else {
                            ...
                            cols.some(function (entry) {
                                if (entry.key !== 'RowDataId' && !entry.hidden) {
                                    if (verifyCanEditCol(entry.key, clear, entity)) {
                                        $('#UIValidacoes_dGridEstado').igGridUpdating('startEdit', firstRow, indexColumn, true);
                                        return true;
                                    }
                                    indexColumn++;
                                }
                ...
                var getVisibleColumns = function(metaDataControl) {
                   ...
                   var visibleColumns = '';
                   if($('#UIValidacoes_dGridEstado').data('igGrid') === undefined) return '';
                   var cols = $('#UIValidacoes_dGridEstado').igGrid('option', 'columns');
                   if (cols) {
                ...
                var isElementHided = function (grid, forceCreating) {
                  if (!grid) grid = $('#UIValidacoes_dGridEstado');
                  return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialogEstado').is(':visible'));
                }
                var refreshData = true;
                var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'UIValidacoes_dGridEstado_container', dataBind: function (commitData, forceCreating) {
                   var grid = $('#UIValidacoes_dGridEstado');
                   ...
                   if (!started) {
                       ...
                       started = true;
                       commitData = false;
                       $('#UIValidacoes_dGridEstado_groupbyarea').addClass('hide');
                   }
                   if (grid.igGridUpdating('isEditing')) {
                   ...
                   if (execFillDetais) {
                     ...
                     vm.currentDataItem().fillDetails(false, 'Estado');
                     return;
                   }
                   ...
                   if (rows.length > 0) {
                     ...
                     if (vm.currentDataItem().currentEstado() != null)
                     {
                       for(var idx = 0; idx < rows.length; idx++)
                       {
                         if (rows[idx].dataset.id == getAbsoluteValue(vm.currentDataItem().currentEstado().RowDataId))
                         {
                ...
                function verifyCanEditCol(column, clear, entity){
                    switch(column){
                        ...
                        case 'NomeEstado': { canEditing = clear || vm.enabledForEditing(); break;}
                    }
                    return canEditing;
                };
                function createDataGrid(grid) {
                    ...
                    vm.gridSaveStates[gridId] = {
                        ...
                        closePopover: function () {
                            $('#UIValidacoes_dGridEstado_LayoutBtn').igPopover('hide');
                        },
                    ...
                    grid.igGrid({ height: (getGridHeightSuggested() * 0.75), width: '100%',
                        ...
                        cellClick: function(evt, ui) {
                             if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                                 ...
                                 if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                                 {
                                      ...
                                      showMultimidia(entity, e, table, key, vm.UIValidacoes());
                                 }
                             }
                             if (typeof vm.OnGridClientClick === 'function') {
                                 entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                 vm.OnGridClientClick('UIValidacoes_dGridEstado', ui.colKey, entity);
                             }
                             if (vm.status() != 'Q') {
                                 var grid = $('#UIValidacoes_dGridEstado');
                        ...
                        dataRendered: function(evt, ui) { 
                           showMultimidiaLazy('#UIValidacoes_dGridEstado');
                        },
                        columns: [
                            { key: 'RowDataId', headerText: 'RowDataId', width: '50px', dataType: 'number', hidden: true },
                            { key: 'ComboboxEstado', headerText: vm.getLayoutHeaderGrid('ComboboxEstado'), width: '0px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null  , formatter: function (val, record) { return  vm.dataDomains.getName('LX_COMBOBOX_ESTADO', val);} },
                            { key: 'DateTimeEstado', headerText: vm.getLayoutHeaderGrid('DateTimeEstado'), width: '0px', dataType: 'date', columnCssClass: 'ellipsis', format: 'dd/MM/yyyy', hidden: false, unbound: false, group: null   },
                            { key: 'DecimalEstado', headerText: vm.getLayoutHeaderGrid('DecimalEstado'), width: '0px', dataType: 'number', columnCssClass: 'ellipsis', format: '0.00', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                            { key: 'IdEstado', headerText: vm.getLayoutHeaderGrid('IdEstado'), width: '0px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                            { key: 'IdPais', headerText: vm.getLayoutHeaderGrid('IdPais'), width: '0px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'NomeEstado', headerText: vm.getLayoutHeaderGrid('NomeEstado'), width: '0px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'IdEstadoMulti', headerText: '', width: '0px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: true, group: null, formula: function(data, grid) { var templateName = getTemplateImageName(vm.status(), 'grid'); var entity = findElementByKey(getDataSource(), 'RowDataId', data.RowDataId); var url = loadMultimidiaUrl('ESTADO', entity.IdEstado, vm.UIValidacoes()); return "<div class='media-small'>" + ko.renderTemplateX(templateName, vm, { tableName: 'ESTADO', key: getAbsoluteValue(entity.IdEstado), vm: vm.UIValidacoes(), url: url })+ "</div>";}}
                        ],
                        features: [
                                    ...
                                    filterDialogOpening: function (evt, ui) {
                                        ...
                                        var dataView = $('#UIValidacoes_dGridEstado').data('igGrid').dataSource;
                                        ...
                                        scriptHtml += '    var newGrid = $("#UIValidacoes_dGridEstado");';
                                        ...
                                        scriptHtml += '            $("#UIValidacoes_dGridEstado_container_dialog_list").html(newList)';
                                        scriptHtml += '         }});';
                                    ...
                                    { name: 'Tooltips', columnSettings:[{ columnKey: "ComboboxEstado", allowTooltips: true },{ columnKey: "DateTimeEstado", allowTooltips: true },{ columnKey: "DecimalEstado", allowTooltips: true },{ columnKey: "IdEstado", allowTooltips: true },{ columnKey: "IdPais", allowTooltips: false },{ columnKey: "NomeEstado", allowTooltips: true },{ columnKey: "IdEstado", allowTooltips: true }] },
                                    { name: 'Resizing' }, 
                                    { name: 'Hiding', 
                                    },
                                   ...
                                   ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                      ...
                                      columnSettings: [{ columnKey: "IdPais", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, validateOnClearState:false, maxValue:2147483647, maxLength: 12, defaultValue: null } }, { columnKey: 'DateTimeEstado', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid === 'function') {vm.OnPropertyChangeDataGrid('UIValidacoes_dGridEstado', 'DateTimeEstado', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: 'ComboboxEstado', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('ComboboxEstado', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('LX_COMBOBOX_ESTADO', ''),  textKey: 'name', valueKey: 'id', enableClearButton: false }}, { columnKey: "DecimalEstado" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIValidacoes_dGridEstado', 'DecimalEstado', ui.oldValue, ui.value);}},  maxLength: 14, maxValue: 99999999999.99, minValue: 0, dataMode: 'decimal', minDecimals: 2, maxDecimals: 2 } }, { columnKey: "IdEstado" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIValidacoes_dGridEstado', 'IdEstado', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "NomeEstado" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('UIValidacoes_dGridEstado', 'NomeEstado', ui.oldValue, ui.value);}}, maxLength: 50 } }],
                                      ...
                                      editCellStarted: function(evt, ui){
                                          ...
                                          var columns = $('#UIValidacoes_dGridEstado').igGridUpdating('option', 'columnSettings');
                                          var currentCol = null;
                                          currentRow = ui.rowID;
                                          ...
                                          if (currentCol != null && currentCol.hasOwnProperty('editorType') && currentCol.editorType === 'combo') {
                                             ...
                                             if (lookUpName != null) {
                                                 ...
                                                 if (lstRefreshDados.length === 0)
                                                     vm.dataCombo.fillDataCombos(lookUpName, ui.columnKey, vm.currentDataItem().currentEstado(), function (result) {
                        ...
                        if ((typeof vm.OnDataGridCreated === 'function')){
                            vm.OnDataGridCreated('UIValidacoes_dGridEstado');
                        }
                        ...
                        selectionrowselectionchanged = function (evt, ui) {
                            if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                    $(document).undelegate('#UIValidacoes_dGridEstado', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                    ...
                                    $(document).delegate ('#UIValidacoes_dGridEstado', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                }
                                selectGridCurrentItem(vm.goToKey, 'RowDataId', ui, vm.currentDataItem().currentEstado, getDataSource()); 
                             } 
                        };
                        $(document).delegate('#UIValidacoes_dGridEstado', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        $('#UIValidacoes_dGridEstado > tbody tr').live('focus', function(evt) {
                            var grid = $('#UIValidacoes_dGridEstado'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                    ...
                    vm.addDataSource({ key: 'UIValidacoes_dGridEstado', name: 'EstadoList', itemsSource: itemsSource });
                }
            
            
            , ChangedBrandscyUIValidacoes_dGrid: function(vm, decimals, reset) {
                var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#scyUIValidacoes_dGrid').data('igGrid'),
                    grdUpd = $('#scyUIValidacoes_dGrid').data('igGridUpdating');
                if(isNull(grd) || isNull(grdUpd)) return;
                for (i = 0; i < grd.options.columns.length; i++) {
                }
                for (i = 0; i < grdUpd.options.columnSettings.length; i++) {
            ...
            , renderscyUIValidacoes_dGrid: function(vm) {
                var getDataSource = function() {
                    var source = null;
                    try {
                        source = vm.dataView;
                ...
                $('#scyUIValidacoes_dGrid_headers').live('focus  keydown', function (evt) {
                    var keyCode = window.event ? evt.which : evt.keyCode;
                    if (keyCode === 9) {
                        var cols = $('#scyUIValidacoes_dGrid').igGrid('option', 'columns');
                        var dataView = $('#scyUIValidacoes_dGrid').data('igGrid').dataSource._dataView
                        if (dataView.length === 0) return;
                        var firstRow = dataView[0].RowDataId;
                        clear = vm.status() === 'C';
                        if (vm.status() === 'C')
                            $('#scyUIValidacoes_dGrid').igGridUpdating('startEdit', firstRow, 0, true);
                        else {
                            var entity = findElementByKey(getDataSource(), 'RowDataId', firstRow);
                            var indexColumn = 0;
                            cols.some(function (entry) {
                                if (entry.key !== 'RowDataId' && !entry.hidden) {
                                    if (verifyCanEditCol(entry.key, clear, entity)) {
                                        $('#scyUIValidacoes_dGrid').igGridUpdating('startEdit', firstRow, indexColumn, true);
                                        return true;
                                    }
                                    indexColumn++;
                                }
                ...
                var getVisibleColumns = function(metaDataControl) {
                   if (metaDataControl) return '';
                   var visibleColumns = '';
                   if($('#scyUIValidacoes_dGrid').data('igGrid') === undefined) return '';
                   var cols = $('#scyUIValidacoes_dGrid').igGrid('option', 'columns');
                   if (cols) {
                     for (var idx = 0; idx < cols.length; idx++) {
                         if (cols[idx].hidden !== true) visibleColumns += (visibleColumns === '' ? '' : ',') + cols[idx].key;
                ...
                var isElementHided = function (grid, forceCreating) {
                  if (!grid) grid = $('#scyUIValidacoes_dGrid');
                  return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialog').is(':visible'));
                }
                var refreshData = true;
                var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'scyUIValidacoes_dGrid_container', dataBind: function (commitData, forceCreating) {
                   var grid = $('#scyUIValidacoes_dGrid');
                   if (started && typeof grid.data('igGridUpdating') === 'undefined') { started = false; }
                   if (commitData && started) {
                       if (grid.igGridUpdating('isEditing')) {
                           grid.igGrid('commit');
                   ...
                   if (!started) {
                       ...
                       started = true;
                       commitData = false;
                       $('#scyUIValidacoes_dGrid_groupbyarea').addClass('hide');
                   }
                   if (grid.igGridUpdating('isEditing')) {
                        grid.igGridUpdating('endEdit', true);
                   }
                ...
                function verifyCanEditCol(column, clear, entity){
                    switch(column){
                        case 'ComboboxPais': { canEditing = clear; break;}
                        case 'DateTimePais': { canEditing = clear; break;}
                        case 'DecimalPais': { canEditing = clear; break;}
                        case 'IdPais': { canEditing = clear; break;}
                        case 'NomePais': { canEditing = clear; break;}
                    }
                    return canEditing;
                };
                function createDataGrid(grid) {
                    ...
                    vm.gridSaveStates[gridId] = {
                        ...
                        closePopover: function () {
                            $('#scyUIValidacoes_dGrid_LayoutBtn').igPopover('hide');
                        },
                        applyLayout: function (layoutInfo) {
                            var _this = this;
                            if (isNull(layoutInfo) && (!_this.currentLayout() || _this.currentLayout().Id === 0)) {
                    ...
                    grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
                        ...
                        cellClick: function(evt, ui) {
                             ...
                             if (typeof vm.OnGridClientClick === 'function') {
                                 entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                 vm.OnGridClientClick('scyUIValidacoes_dGrid', ui.colKey, entity);
                             }
                             if (vm.status() != 'Q') {
                                 var grid = $('#scyUIValidacoes_dGrid');
                                 var isEditing = grid.igGridUpdating('isEditing');
                        ...
                        dataRendered: function(evt, ui) { 
                           showMultimidiaLazy('#scyUIValidacoes_dGrid');
                        },
                        columns: [
                            { key: 'RowDataId', headerText: 'RowDataId', width: '50px', dataType: '', hidden: true },
                            { key: 'ComboboxPais', headerText: vm.getLayoutHeaderGrid('ComboboxPais'), width: '0px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null  , formatter: function (val, record) { return  vm.dataDomains.getName('LX_COMBOBOX_PAIS', val);} },
                            { key: 'DateTimePais', headerText: vm.getLayoutHeaderGrid('DateTimePais'), width: '0px', dataType: 'date', columnCssClass: 'ellipsis', format: 'dd/MM/yyyy', hidden: false, unbound: false, group: null   },
                            { key: 'DecimalPais', headerText: vm.getLayoutHeaderGrid('DecimalPais'), width: '0px', dataType: 'number', columnCssClass: 'ellipsis', format: '0.00', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                            { key: 'IdPais', headerText: vm.getLayoutHeaderGrid('IdPais'), width: '0px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                            { key: 'NomePais', headerText: vm.getLayoutHeaderGrid('NomePais'), width: '0px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'IdPaisMulti', headerText: '', width: '0px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: true, group: null, formula: function(data, grid) { var templateName = getTemplateImageName(vm.status(), 'grid'); var entity = findElementByKey(getDataSource(), 'RowDataId', data.RowDataId); var url = loadMultimidiaUrl('PAIS', entity.IdPais, vm.UIValidacoes()); return "<div class='media-small'>" + ko.renderTemplateX(templateName, vm, { tableName: 'PAIS', key: getAbsoluteValue(entity.IdPais), vm: vm.UIValidacoes(), url: url })+ "</div>";}}
                        ],
                        features: [
                                    { name: 'Paging', type: 'local', pageSizeDropDownLocation: 'inpager', pageSize: 100, pageIndexChanged: function (evt, ui) { if (!$('#scyUIValidacoes_dGrid').igGridSelection('option', 'multipleSelection')) $('#scyUIValidacoes_dGrid').igGridSelection('selectRow', 0); selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); } },
                                    { name: 'Sorting', type: 'local', caseSensitive: true, unsortedColumnTooltip: '', sortedColumnTooltip: '',
                                      columnSorting: function (evt, ui) { 
                                          return false;
                                      } 
                                    ...
                                    filterDialogOpening: function (evt, ui) {
                                        ...
                                        var dataView = $('#scyUIValidacoes_dGrid').data('igGrid').dataSource;
                                        if (dataView.settings.filtering.expressions.length <= 0)
                                            dataView._filteredData = [];
                                        ...
                                        scriptHtml += '  <script>';
                                        scriptHtml += '    var newCol = "' + col + '";';
                                        scriptHtml += '    var newGrid = $("#scyUIValidacoes_dGrid");';
                                        scriptHtml += '    var listFilter = [];'
                                        scriptHtml += '    var reloadList = ' + reloadList + ';';
                                        scriptHtml += '    function hideColumn(){';
                                        scriptHtml += '     if ($("#showHideColumn")["0"].innerHTML.indexOf("Ocultar") >= 0) {';
                                        ...
                                        scriptHtml += '            newCol = ui.items["0"].data.key;';
                                        scriptHtml += '            var newList = reloadList(newCol);';
                                        scriptHtml += '            $("#scyUIValidacoes_dGrid_container_dialog_list").html(newList)';
                                        scriptHtml += '         }});';
                                        scriptHtml += '         $("#comboFields").igCombo("value", newCol);';
                                        scriptHtml += '      </script>';
                                        scriptHtml += '  </div>';
                                    ...
                                    { name: 'Tooltips', columnSettings:[{ columnKey: "ComboboxPais", allowTooltips: true },{ columnKey: "DateTimePais", allowTooltips: true },{ columnKey: "DecimalPais", allowTooltips: true },{ columnKey: "IdPais", allowTooltips: true },{ columnKey: "NomePais", allowTooltips: true },{ columnKey: "IdPais", allowTooltips: true }] },
                                    { name: 'Resizing' }, 
                                    { name: 'Hiding', 
                                    },
                                    { name: 'MultiColumnHeaders' }
                                   ...
                                   ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                      ...
                                      showReadonlyEditors: false,
                                      showDoneCancelButtons: false,
                                      columnSettings: [{ columnKey: 'DateTimePais', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid === 'function') {vm.OnPropertyChangeDataGrid('scyUIValidacoes_dGrid', 'DateTimePais', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: 'ComboboxPais', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('ComboboxPais', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('LX_COMBOBOX_PAIS', ''),  textKey: 'name', valueKey: 'id', enableClearButton: false }}, { columnKey: "DecimalPais" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIValidacoes_dGrid', 'DecimalPais', ui.oldValue, ui.value);}},  maxLength: 14, maxValue: 99999999999.99, minValue: 0, dataMode: 'decimal', minDecimals: 2, maxDecimals: 2 } }, { columnKey: "IdPais" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIValidacoes_dGrid', 'IdPais', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "NomePais" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyUIValidacoes_dGrid', 'NomePais', ui.oldValue, ui.value);}}, maxLength: 50 } }],
                                      rowDeleting: function (evt, ui) {
                                          deletedIndex = ui.element.context.rowIndex;
                                          var entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowID);
                                          if (entity) {
                                      ...
                                      editCellStarted: function(evt, ui){
                                          var lstRefreshDados = null;
                                          var columns = $('#scyUIValidacoes_dGrid').igGridUpdating('option', 'columnSettings');
                                          var currentCol = null;
                                          currentRow = ui.rowID;
                                          columns.forEach(function (entry, index) {
                                             if (entry.columnKey === ui.columnKey) currentCol = entry;
                        ...
                        if ((typeof vm.OnDataGridCreated === 'function')){
                            vm.OnDataGridCreated('scyUIValidacoes_dGrid');
                        }
                        var selectionrowselectionchanged = null, selectedRowId = -1;
                        selectionrowselectionchanged = function (evt, ui) {
                            if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                    $(document).undelegate('#scyUIValidacoes_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                    ui.owner.clearSelection();
                                    ui.owner.selectRow(ui.row.index);
                                    if (vm.status() === 'Q'){
                                        var gridCell = ui.owner.grid;
                                    ...
                                    selectedRowId = ui.row.id;
                                    $(document).delegate ('#scyUIValidacoes_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                }
                                selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                             } 
                        };
                        $(document).delegate('#scyUIValidacoes_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        $('#scyUIValidacoes_dGrid > tbody tr').live('focus', function(evt) {
                            var grid = $('#scyUIValidacoes_dGrid'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                            var selectedRows = grid.igGridSelection('option', 'multipleSelection') ? grid.igGridSelection('selectedRows') : [grid.igGridSelection('selectedRow')];
                            if (selectedRowId === id) return;
                            selectedRowId = id;
                            grid.igGridSelection('selectRowById', id);
                        ...
                        grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                            if (vm.UIValidacoes().status() === 'Q') vm.UIValidacoes().dataToolbar.viewInfo();
                        });
                    }
                    vm.addDataSource({ key: 'scyUIValidacoes_dGrid', name: 'dataView', itemsSource: itemsSource });
                }
