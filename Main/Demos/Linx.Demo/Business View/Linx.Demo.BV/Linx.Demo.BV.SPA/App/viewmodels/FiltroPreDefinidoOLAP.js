define(['durandal/app', 'pkg_linx-demo-bv-spa/services/MacrosEventosValidacoesContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-demo-bv-spa/viewmodels/FiltroPreDefinidoOLAPComplement', 'viewmodels/shared/modalCustomSearch', 'managers/user'],
...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'FiltroPreDefinidoOLAP', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_FiltroPreDefinidoOLAP(), 'Name'))());
        }, function (err) {
    ...
    var activate = function (settings, querystring) {
      ...
      vm.FiltroPreDefinidoOLAP = getVM;
      //loadLanguage();
      changeLanguage();
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'FiltroPreDefinidoOLAP' ]; settings.parentVM.FiltroPreDefinidoOLAP = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.FiltroPreDefinidoOLAP = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('FiltroPreDefinidoOLAP', parentVM.internalUIs) === -1) parentVM.internalUIs.push('FiltroPreDefinidoOLAP');
                  }
              }
          }
          else {
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-demo-bv-spa-FiltroPreDefinidoOLAP', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: FiltroPreDefinidoOLAP', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#FiltroPreDefinidoOLAP_gbEntityAdapter1').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    complement.renderscyFiltroPreDefinidoOLAP_dGrid(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'ValorVendas';
    ...
    var visibleColumns = 'IdBandeiraRede,Loja,CodLoja,Cliente,Data,QtdItemBruto,VlrItemPago';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'ValorVendas', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'ValorVendas', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getQueryFilter = function (currentDI) {
        ...
        if (vm.getBandeiraRede() === 0) {
           eSearch += 'ValorVendas{IdBandeiraRede#' + (!isNullOrEmpty(vm.currentBrands()) ? 'In#S' : '==#I') + vm.getCurrentBrands() + '}';
        }
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('ValorVendas', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('ValorVendas');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('ValorVendas');
        ...
        return dataContext.getValorVendasByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'ValorVendas'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getValorVendasByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo).then(querySucceeded).fin(complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearValorVendas(getBandeiraRede(), clearComplete);
    ...
    var createValorVendas = function() {
        dataBind('dataView', true);
        var entity = dataContext.createValorVendas();
    ...
    var createAndNotifyValorVendas = function() {
        var entity = createValorVendas();
        notifyPresentation('');
        return entity;
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#FiltroPreDefinidoOLAP_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#FiltroPreDefinidoOLAP_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#FiltroPreDefinidoOLAP_formViewer')[0];
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            ValorVendasData_typeRange: ko.observable('R'), ValorVendasData_begin: ko.observable(null), ValorVendasData_end: ko.observable(null), ValorVendasData_predefFilter: ko.observableArray([]), ValorVendasData_predefValue: ko.observable(null),
        ValorVendasQtdItemBruto_begin: ko.observable(null), ValorVendasQtdItemBruto_end: ko.observable(null),
        ValorVendasVlrItemPago_begin: ko.observable(null), ValorVendasVlrItemPago_end: ko.observable(null),
        ValorVendasCliente: ko.observable(null),
        ValorVendasCodLoja: ko.observable(null),
        ValorVendasIdBandeiraRede: ko.observable(null),
        ValorVendasLoja: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.ValorVendasData_typeRange('R'); entitySearchRange.ValorVendasData_begin(null); entitySearchRange.ValorVendasData_end(null); entitySearchRange.ValorVendasData_predefFilter([]); entitySearchRange.ValorVendasData_predefValue(null);
        entitySearchRange.ValorVendasQtdItemBruto_begin(null); entitySearchRange.ValorVendasQtdItemBruto_end(null);
        entitySearchRange.ValorVendasVlrItemPago_begin(null); entitySearchRange.ValorVendasVlrItemPago_end(null);
        entitySearchRange.ValorVendasCliente(null);
        entitySearchRange.ValorVendasCodLoja(null);
        entitySearchRange.ValorVendasIdBandeiraRede(null);
        entitySearchRange.ValorVendasLoja(null);
    };
    entitySearchRange.has_ValorVendasQtdItemBruto = ko.computed(function(){ return (entitySearchRange.ValorVendasQtdItemBruto_begin() != null || entitySearchRange.ValorVendasQtdItemBruto_end() != null); });
    entitySearchRange.has_ValorVendasVlrItemPago = ko.computed(function(){ return (entitySearchRange.ValorVendasVlrItemPago_begin() != null || entitySearchRange.ValorVendasVlrItemPago_end() != null); });
    entitySearchRange.has_ValorVendasData = ko.computed(function(){ return (entitySearchRange.ValorVendasData_typeRange() == 'R' && (entitySearchRange.ValorVendasData_begin() != null || entitySearchRange.ValorVendasData_end() != null) || (entitySearchRange.ValorVendasData_typeRange() == 'P' && entitySearchRange.ValorVendasData_predefFilter().length > 0)); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#FiltroPreDefinidoOLAP_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            dataView: dataView,
            viewName: 'FiltroPreDefinidoOLAP',
            ...
            rootBmTypeName: '',
            rootDataTypeName: 'ValorVendas',
            rootNamespace: 'Linx.Demo.BV.MacrosEventosValidacoes',
            ...
            createAndNotifyValorVendas: createAndNotifyValorVendas,
            ...
            __moduleId__: 'pkg_linx-demo-bv-spa/viewmodels/FiltroPreDefinidoOLAP'
        };
