define(['managers/__auth', 'managers/user'], function (managerAuth, managerUser) {
    var complementCtor = function() {
        var complement = {
            isAutomatic: true
            , ChangedBrandscyExAutoComp_dGrid: function(vm, decimals, reset) {
                var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#scyExAutoComp_dGrid').data('igGrid'),
                    grdUpd = $('#scyExAutoComp_dGrid').data('igGridUpdating');
            ...
            , renderscyExAutoComp_dGrid: function(vm) {
                ...
                $('#scyExAutoComp_dGrid_headers').live('focus  keydown', function (evt) {
                    var keyCode = window.event ? evt.which : evt.keyCode;
                    if (keyCode === 9) {
                        var cols = $('#scyExAutoComp_dGrid').igGrid('option', 'columns');
                        var dataView = $('#scyExAutoComp_dGrid').data('igGrid').dataSource._dataView
                        if (dataView.length === 0) return;
                        var firstRow = dataView[0].RowDataId;
                        clear = vm.status() === 'C';
                        if (vm.status() === 'C')
                            $('#scyExAutoComp_dGrid').igGridUpdating('startEdit', firstRow, 0, true);
                        else {
                            ...
                            cols.some(function (entry) {
                                if (entry.key !== 'RowDataId' && !entry.hidden) {
                                    if (verifyCanEditCol(entry.key, clear, entity)) {
                                        $('#scyExAutoComp_dGrid').igGridUpdating('startEdit', firstRow, indexColumn, true);
                                        return true;
                                    }
                                    indexColumn++;
                                }
                ...
                var getVisibleColumns = function(metaDataControl) {
                   ...
                   var visibleColumns = '';
                   if($('#scyExAutoComp_dGrid').data('igGrid') === undefined) return '';
                   var cols = $('#scyExAutoComp_dGrid').igGrid('option', 'columns');
                   if (cols) {
                ...
                var isElementHided = function (grid, forceCreating) {
                  if (!grid) grid = $('#scyExAutoComp_dGrid');
                  return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialog').is(':visible'));
                }
                var refreshData = true;
                var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'scyExAutoComp_dGrid_container', dataBind: function (commitData, forceCreating) {
                   var grid = $('#scyExAutoComp_dGrid');
                   ...
                   if (!started) {
                       ...
                       started = true;
                       commitData = false;
                       $('#scyExAutoComp_dGrid_groupbyarea').addClass('hide');
                   }
                   if (grid.igGridUpdating('isEditing')) {
                ...
                function verifyCanEditCol(column, clear, entity){
                    switch(column){
                        case 'idNomeCompleto': { canEditing = clear; break;}
                        case 'NomeCompleto': { canEditing = clear; break;}
                        case 'IdNome': { canEditing = clear; break;}
                        case 'idnomeMeio': { canEditing = clear; break;}
                        case 'IdSobrenome': { canEditing = clear; break;}
                        case 'Nome': { canEditing = clear; break;}
                        case 'Nomedomeio': { canEditing = clear; break;}
                        case 'SobreNome': { canEditing = clear; break;}
                ...
                function createDataGrid(grid) {
                    ...
                    vm.gridSaveStates[gridId] = {
                        ...
                        closePopover: function () {
                            $('#scyExAutoComp_dGrid_LayoutBtn').igPopover('hide');
                        },
                    ...
                    grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
                        ...
                        cellClick: function(evt, ui) {
                             if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                                 ...
                                 if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                                 {
                                      ...
                                      showMultimidia(entity, e, table, key, vm.ExAutoComp());
                                 }
                             }
                             if (typeof vm.OnGridClientClick === 'function') {
                                 entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                 vm.OnGridClientClick('scyExAutoComp_dGrid', ui.colKey, entity);
                             }
                             if (vm.status() != 'Q') {
                                 var grid = $('#scyExAutoComp_dGrid');
                        ...
                        columns: [
                            ...
                            { key: 'idNomeCompleto', headerText: vm.getLayoutHeaderGrid('idNomeCompleto'), width: '250px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                            { key: 'NomeCompleto', headerText: vm.getLayoutHeaderGrid('NomeCompleto'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'IdNome', headerText: vm.getLayoutHeaderGrid('IdNome'), width: '271px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'idnomeMeio', headerText: vm.getLayoutHeaderGrid('idnomeMeio'), width: '271px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'IdSobrenome', headerText: vm.getLayoutHeaderGrid('IdSobrenome'), width: '271px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'Nome', headerText: vm.getLayoutHeaderGrid('Nome'), width: '421px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'Nomedomeio', headerText: vm.getLayoutHeaderGrid('Nomedomeio'), width: '421px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'SobreNome', headerText: vm.getLayoutHeaderGrid('SobreNome'), width: '421px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'IdCliente', headerText: vm.getLayoutHeaderGrid('IdCliente'), width: '271px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  }
                        ],
                        features: [
                                    ...
                                    filterDialogOpening: function (evt, ui) {
                                        ...
                                        var dataView = $('#scyExAutoComp_dGrid').data('igGrid').dataSource;
                                        ...
                                        scriptHtml += '    var newGrid = $("#scyExAutoComp_dGrid");';
                                        ...
                                        scriptHtml += '            $("#scyExAutoComp_dGrid_container_dialog_list").html(newList)';
                                        scriptHtml += '         }});';
                                    ...
                                    { name: 'Tooltips', columnSettings:[{ columnKey: "idNomeCompleto", allowTooltips: true },{ columnKey: "NomeCompleto", allowTooltips: true },{ columnKey: "IdNome", allowTooltips: false },{ columnKey: "idnomeMeio", allowTooltips: false },{ columnKey: "IdSobrenome", allowTooltips: false },{ columnKey: "Nome", allowTooltips: false },{ columnKey: "Nomedomeio", allowTooltips: false },{ columnKey: "SobreNome", allowTooltips: false },{ columnKey: "IdCliente", allowTooltips: true }] },
                                    { name: 'Resizing' }, 
                                    { name: 'Hiding', 
                                    },
                                   ...
                                   ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                      ...
                                      columnSettings: [{ columnKey: "IdNome", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpTbnome", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, activateAutoComplete: false, autoCompleteMaxResults: 7, validateOnClearState:false, maxValue:2147483647, maxLength: 10, defaultValue: null } }, { columnKey: "idnomeMeio", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpTbnmmeio", isNullable: false, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, activateAutoComplete: false, autoCompleteMaxResults: 7, validateOnClearState:false, maxValue:2147483647, maxLength: 10, defaultValue: 0 } }, { columnKey: "IdSobrenome", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpTbsobrenm", isNullable: false, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, activateAutoComplete: false, autoCompleteMaxResults: 7, validateOnClearState:false, maxValue:2147483647, maxLength: 10, defaultValue: 0 } }, { columnKey: "Nome", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpTbnome", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, activateAutoComplete: false, autoCompleteMaxResults: 7, validateOnClearState:false, maxValue:null, maxLength: 100, defaultValue: '' } }, { columnKey: "Nomedomeio", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpTbnmmeio", isNullable: false, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, activateAutoComplete: false, autoCompleteMaxResults: 7, validateOnClearState:false, maxValue:null, maxLength: 100, defaultValue: '' } }, { columnKey: "SobreNome", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpTbsobrenm", isNullable: false, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, activateAutoComplete: false, autoCompleteMaxResults: 7, validateOnClearState:false, maxValue:null, maxLength: 100, defaultValue: '' } }, { columnKey: "idNomeCompleto" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyExAutoComp_dGrid', 'idNomeCompleto', ui.oldValue, ui.value);}},  maxLength: 10, maxValue: null, dataMode: 'int' } }, { columnKey: "NomeCompleto" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyExAutoComp_dGrid', 'NomeCompleto', ui.oldValue, ui.value);}}, maxLength: 100 } }, { columnKey: "IdCliente" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyExAutoComp_dGrid', 'IdCliente', ui.oldValue, ui.value);}},  maxLength: 10, maxValue: null, minValue: 0, dataMode: 'int' } }],
                                      ...
                                      editCellStarted: function(evt, ui){
                                          ...
                                          var columns = $('#scyExAutoComp_dGrid').igGridUpdating('option', 'columnSettings');
                                          var currentCol = null;
                                          currentRow = ui.rowID;
                        ...
                        if ((typeof vm.OnDataGridCreated === 'function')){
                            vm.OnDataGridCreated('scyExAutoComp_dGrid');
                        }
                        ...
                        selectionrowselectionchanged = function (evt, ui) {
                            if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                    $(document).undelegate('#scyExAutoComp_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                    ...
                                    $(document).delegate ('#scyExAutoComp_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                }
                                selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                             } 
                        };
                        $(document).delegate('#scyExAutoComp_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        $('#scyExAutoComp_dGrid > tbody tr').live('focus', function(evt) {
                            var grid = $('#scyExAutoComp_dGrid'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                        ...
                        grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                            if (vm.ExAutoComp().status() === 'Q') vm.ExAutoComp().dataToolbar.viewInfo();
                        });
                    }
                    vm.addDataSource({ key: 'scyExAutoComp_dGrid', name: 'dataView', itemsSource: itemsSource });
                }
