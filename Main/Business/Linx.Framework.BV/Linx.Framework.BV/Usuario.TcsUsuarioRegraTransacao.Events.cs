...
namespace Linx.Framework.BV.Usuario
{
    ...
    public partial class TcsUsuarioRegraTransacao
    {
        ...
        public static void OnLookUpingLookUpTcsUsuarioRegraTransacao(ref IQueryable<LookUpTcsUsuarioRegraTransacao> searchDefinition, string propertyName, EntitySearch entitySearch)
        {
            searchDefinition =
                        from result in Utils.GetLookUpTransacaoModulo(entitySearch)
                        select new LookUpTcsUsuarioRegraTransacao()
                        {
        ...
        {
            entities.Where(i => i.Entity is TcsUsuarioRegraTransacao && i.Operation != DomainOperation.None).Select(i => ((TcsUsuarioRegraTransacao)i.Entity).UidUsuario).Distinct().ToList().ForEach(uidUsuario =>
                {
