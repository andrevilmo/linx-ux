...
namespace Linx.Framework.BV.Modulo
{
    ...
    public partial class TcsModuloGrupo
    {
        ...
        {
            if (entities.Where(i => i.Entity is TcsModuloGrupo && i.Operation != DomainOperation.None).Count() > 0)
        ...
        {
            searchDefinition = from result in Utils.GetLookupTcsAplicativo(entitySearch) select new LookUpTcsAplicativo { IdTcsAplicativo = result.IdTcsAplicativo, DescricaoAplicativo = result.DescricaoAplicativo };
        }
    }
}
