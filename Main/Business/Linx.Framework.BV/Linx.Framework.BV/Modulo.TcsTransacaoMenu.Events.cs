...
namespace Linx.Framework.BV.Modulo
{
    ...
    public partial class TcsTransacaoMenu
    {
        ...
        public static void OnLookUpingLookUpTcsTransacaoMenu(ref IQueryable<LookUpTcsTransacaoMenu> searchDefinition, string propertyName, EntitySearch entitySearch)
        {
            searchDefinition =
                (from result in Utils.GetLookupTransacao(entitySearch)
                 select new LookUpTcsTransacaoMenu()
                 {
        ...
        {
            if (entities.Where(i => i.Entity is TcsTransacaoMenu && i.Operation != DomainOperation.None).Count() > 0)
