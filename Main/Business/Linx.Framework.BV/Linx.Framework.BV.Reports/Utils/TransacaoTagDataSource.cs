namespace Linx.Framework.BV.Reports {
    ...
    public partial class TransacaoTagDataSource {
        
        private string url = "LinxFrameworkTransacaoTag/";
        
        public Linx.Framework.BV.TransacaoTag.TransacaoTagDomainService _transacaoTagContext;
        ...
        private IEnumerable<TcsTransacaoTag> GetLocalTcsTransacaoTag(Telerik.Reporting.Processing.Report report) {
            IEnumerable<TcsTransacaoTag> result = default(IEnumerable<TcsTransacaoTag>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_transacaoTagContext == null) _transacaoTagContext = new Linx.Framework.BV.TransacaoTag.TransacaoTagDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(TcsTransacaoTag), new string[] { });
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(TcsTransacaoTag), jEntitySearch, false, false, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._transacaoTagContext.GetTcsTransacaoTagByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._transacaoTagContext.GetTcsTransacaoTagByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<TcsTransacaoTag> GetTcsTransacaoTag(object reportItem) {
            ...
            {
                return GetLocalTcsTransacaoTag(report);
            }
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceAddress = "GetSampleTcsTransacaoTag?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    ...
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(TcsTransacaoTag), new string[] { });
                        serviceAddress = "GetTcsTransacaoTagByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                    ...
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<TcsTransacaoTag>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
