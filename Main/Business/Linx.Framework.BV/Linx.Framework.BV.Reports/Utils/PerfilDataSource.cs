namespace Linx.Framework.BV.Reports {
    ...
    using Linx.Framework.BV.Perfil;
    using System.Net.Http;
    using System.Net.Http.Headers;
    
    
    public partial class PerfilDataSource {
        
        private string url = "LinxFrameworkPerfil/";
        
        public Linx.Framework.BV.Perfil.PerfilDomainService _perfilContext;
        ...
        private IEnumerable<TcsPerfil> GetLocalTcsPerfil(Telerik.Reporting.Processing.Report report) {
            ...
            if (_perfilContext == null) _perfilContext = new Linx.Framework.BV.Perfil.PerfilDomainService(headers) { IsSecure = true };
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._perfilContext.GetTcsPerfilByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._perfilContext.GetTcsPerfilByEntitySearchNoAssociations(null).ToList();
            if (this.DetailsForLoading != null && this.DetailsForLoading.Length > 0)
            {
               foreach (var entity in result)
               {
                   entity.FillDetails(this._perfilContext, null, null, this.DetailsForLoading);
               }
            }
            return result;
        }
        ...
        private IEnumerable<TcsUsuarioPerfil> GetLocalTcsUsuarioPerfil(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._perfilContext.GetTcsUsuarioPerfilByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._perfilContext.GetTcsUsuarioPerfilByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<TcsUsuarioPerfilParentComposition> GetLocalTcsUsuarioPerfilParentComposition(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._perfilContext.GetTcsUsuarioPerfilParentCompositionByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._perfilContext.GetTcsUsuarioPerfilParentCompositionByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<TcsPerfilRegraModulo> GetLocalTcsPerfilRegraModulo(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._perfilContext.GetTcsPerfilRegraModuloByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._perfilContext.GetTcsPerfilRegraModuloByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<TcsPerfilRegraModuloParentComposition> GetLocalTcsPerfilRegraModuloParentComposition(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._perfilContext.GetTcsPerfilRegraModuloParentCompositionByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._perfilContext.GetTcsPerfilRegraModuloParentCompositionByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<TcsPerfilRegraColuna> GetLocalTcsPerfilRegraColuna(Telerik.Reporting.Processing.Report report) {
            IEnumerable<TcsPerfilRegraColuna> result = default(IEnumerable<TcsPerfilRegraColuna>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_perfilContext == null) _perfilContext = new Linx.Framework.BV.Perfil.PerfilDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(TcsPerfilRegraColuna), new string[] { });
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(TcsPerfilRegraColuna), jEntitySearch, false, false, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._perfilContext.GetTcsPerfilRegraColunaByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._perfilContext.GetTcsPerfilRegraColunaByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<TcsPerfilRegraColuna> GetTcsPerfilRegraColuna(object reportItem) {
            var report = (reportItem is Telerik.Reporting.Processing.Report ? reportItem : ((Telerik.Reporting.Processing.ReportItem)(reportItem)).Report) as Telerik.Reporting.Processing.Report;
            IEnumerable<TcsPerfilRegraColuna> result = default(IEnumerable<TcsPerfilRegraColuna>);
            if (report != null && report.Parameters.ContainsKey("CurrentUser") && !report.Parameters["CurrentUser"].Value.IsNullOrEmpty())
            {
                return GetLocalTcsPerfilRegraColuna(report);
            }
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceBus = (report != null && report.Parameters.ContainsKey("ServiceBusUrl") && !report.Parameters["ServiceBusUrl"].Value.IsNullOrEmpty() ? report.Parameters["ServiceBusUrl"].Value.ToString() : "http://localhost:1710/");
                    string serviceAddress = "GetSampleTcsPerfilRegraColuna?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    client.BaseAddress = new Uri(serviceBus + (serviceBus.Right(1) == "/" ? "" : "/") +  url);
                    client.DefaultRequestHeaders.Accept.Clear();
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(TcsPerfilRegraColuna), new string[] { });
                        serviceAddress = "GetTcsPerfilRegraColunaByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", Convert.ToBase64String(System.Text.ASCIIEncoding.ASCII.GetBytes(string.Format("{0}:{1}", userName.Value.Value, password.Value.Value))));
                    }
                    else
                    {
                    ...
                    HttpResponseMessage response = client.GetAsync(serviceAddress).Result;
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<TcsPerfilRegraColuna>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
                        var responseContent = response.Content.ReadAsStringAsync();
                        responseContent.Wait();
        ...
        private IEnumerable<TcsPerfilRegraColunaParentComposition> GetLocalTcsPerfilRegraColunaParentComposition(Telerik.Reporting.Processing.Report report) {
            IEnumerable<TcsPerfilRegraColunaParentComposition> result = default(IEnumerable<TcsPerfilRegraColunaParentComposition>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_perfilContext == null) _perfilContext = new Linx.Framework.BV.Perfil.PerfilDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(TcsPerfilRegraColunaParentComposition), new string[] {"TcsPerfil"});
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(TcsPerfilRegraColunaParentComposition), jEntitySearch, false, false, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._perfilContext.GetTcsPerfilRegraColunaParentCompositionByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._perfilContext.GetTcsPerfilRegraColunaParentCompositionByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<TcsPerfilRegraColunaParentComposition> GetTcsPerfilRegraColunaParentComposition(object reportItem) {
            var report = (reportItem is Telerik.Reporting.Processing.Report ? reportItem : ((Telerik.Reporting.Processing.ReportItem)(reportItem)).Report) as Telerik.Reporting.Processing.Report;
            IEnumerable<TcsPerfilRegraColunaParentComposition> result = default(IEnumerable<TcsPerfilRegraColunaParentComposition>);
            if (report != null && report.Parameters.ContainsKey("CurrentUser") && !report.Parameters["CurrentUser"].Value.IsNullOrEmpty())
            {
                return GetLocalTcsPerfilRegraColunaParentComposition(report);
            }
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceBus = (report != null && report.Parameters.ContainsKey("ServiceBusUrl") && !report.Parameters["ServiceBusUrl"].Value.IsNullOrEmpty() ? report.Parameters["ServiceBusUrl"].Value.ToString() : "http://localhost:1710/");
                    string serviceAddress = "GetSampleTcsPerfilRegraColunaParentComposition?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    client.BaseAddress = new Uri(serviceBus + (serviceBus.Right(1) == "/" ? "" : "/") +  url);
                    client.DefaultRequestHeaders.Accept.Clear();
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(TcsPerfilRegraColunaParentComposition), new string[] {"TcsPerfil"});
                        serviceAddress = "GetTcsPerfilRegraColunaParentCompositionByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", Convert.ToBase64String(System.Text.ASCIIEncoding.ASCII.GetBytes(string.Format("{0}:{1}", userName.Value.Value, password.Value.Value))));
                    }
                    else
                    {
                    ...
                    HttpResponseMessage response = client.GetAsync(serviceAddress).Result;
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<TcsPerfilRegraColunaParentComposition>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
                        var responseContent = response.Content.ReadAsStringAsync();
                        responseContent.Wait();
        ...
        private IEnumerable<TcsPerfilRegraTransacao> GetLocalTcsPerfilRegraTransacao(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._perfilContext.GetTcsPerfilRegraTransacaoByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._perfilContext.GetTcsPerfilRegraTransacaoByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<TcsPerfilRegraTransacaoParentComposition> GetLocalTcsPerfilRegraTransacaoParentComposition(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._perfilContext.GetTcsPerfilRegraTransacaoParentCompositionByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._perfilContext.GetTcsPerfilRegraTransacaoParentCompositionByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<TcsPerfilBandeiraRede> GetLocalTcsPerfilBandeiraRede(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._perfilContext.GetTcsPerfilBandeiraRedeByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._perfilContext.GetTcsPerfilBandeiraRedeByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<TcsPerfilBandeiraRedeParentComposition> GetLocalTcsPerfilBandeiraRedeParentComposition(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._perfilContext.GetTcsPerfilBandeiraRedeParentCompositionByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._perfilContext.GetTcsPerfilBandeiraRedeParentCompositionByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<TcsPerfilLayout> GetLocalTcsPerfilLayout(Telerik.Reporting.Processing.Report report) {
            IEnumerable<TcsPerfilLayout> result = default(IEnumerable<TcsPerfilLayout>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_perfilContext == null) _perfilContext = new Linx.Framework.BV.Perfil.PerfilDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(TcsPerfilLayout), new string[] { });
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(TcsPerfilLayout), jEntitySearch, false, false, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._perfilContext.GetTcsPerfilLayoutByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._perfilContext.GetTcsPerfilLayoutByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<TcsPerfilLayout> GetTcsPerfilLayout(object reportItem) {
            var report = (reportItem is Telerik.Reporting.Processing.Report ? reportItem : ((Telerik.Reporting.Processing.ReportItem)(reportItem)).Report) as Telerik.Reporting.Processing.Report;
            IEnumerable<TcsPerfilLayout> result = default(IEnumerable<TcsPerfilLayout>);
            if (report != null && report.Parameters.ContainsKey("CurrentUser") && !report.Parameters["CurrentUser"].Value.IsNullOrEmpty())
            {
                return GetLocalTcsPerfilLayout(report);
            }
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceBus = (report != null && report.Parameters.ContainsKey("ServiceBusUrl") && !report.Parameters["ServiceBusUrl"].Value.IsNullOrEmpty() ? report.Parameters["ServiceBusUrl"].Value.ToString() : "http://localhost:1710/");
                    string serviceAddress = "GetSampleTcsPerfilLayout?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    client.BaseAddress = new Uri(serviceBus + (serviceBus.Right(1) == "/" ? "" : "/") +  url);
                    client.DefaultRequestHeaders.Accept.Clear();
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(TcsPerfilLayout), new string[] { });
                        serviceAddress = "GetTcsPerfilLayoutByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", Convert.ToBase64String(System.Text.ASCIIEncoding.ASCII.GetBytes(string.Format("{0}:{1}", userName.Value.Value, password.Value.Value))));
                    }
                    else
                    {
                    ...
                    HttpResponseMessage response = client.GetAsync(serviceAddress).Result;
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<TcsPerfilLayout>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
                        var responseContent = response.Content.ReadAsStringAsync();
                        responseContent.Wait();
        ...
        private IEnumerable<TcsPerfilLayoutParentComposition> GetLocalTcsPerfilLayoutParentComposition(Telerik.Reporting.Processing.Report report) {
            IEnumerable<TcsPerfilLayoutParentComposition> result = default(IEnumerable<TcsPerfilLayoutParentComposition>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_perfilContext == null) _perfilContext = new Linx.Framework.BV.Perfil.PerfilDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(TcsPerfilLayoutParentComposition), new string[] {"TcsPerfil"});
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(TcsPerfilLayoutParentComposition), jEntitySearch, false, false, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._perfilContext.GetTcsPerfilLayoutParentCompositionByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._perfilContext.GetTcsPerfilLayoutParentCompositionByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<TcsPerfilLayoutParentComposition> GetTcsPerfilLayoutParentComposition(object reportItem) {
            var report = (reportItem is Telerik.Reporting.Processing.Report ? reportItem : ((Telerik.Reporting.Processing.ReportItem)(reportItem)).Report) as Telerik.Reporting.Processing.Report;
            IEnumerable<TcsPerfilLayoutParentComposition> result = default(IEnumerable<TcsPerfilLayoutParentComposition>);
            if (report != null && report.Parameters.ContainsKey("CurrentUser") && !report.Parameters["CurrentUser"].Value.IsNullOrEmpty())
            {
                return GetLocalTcsPerfilLayoutParentComposition(report);
            }
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceBus = (report != null && report.Parameters.ContainsKey("ServiceBusUrl") && !report.Parameters["ServiceBusUrl"].Value.IsNullOrEmpty() ? report.Parameters["ServiceBusUrl"].Value.ToString() : "http://localhost:1710/");
                    string serviceAddress = "GetSampleTcsPerfilLayoutParentComposition?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    client.BaseAddress = new Uri(serviceBus + (serviceBus.Right(1) == "/" ? "" : "/") +  url);
                    client.DefaultRequestHeaders.Accept.Clear();
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(TcsPerfilLayoutParentComposition), new string[] {"TcsPerfil"});
                        serviceAddress = "GetTcsPerfilLayoutParentCompositionByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", Convert.ToBase64String(System.Text.ASCIIEncoding.ASCII.GetBytes(string.Format("{0}:{1}", userName.Value.Value, password.Value.Value))));
                    }
                    else
                    {
                    ...
                    HttpResponseMessage response = client.GetAsync(serviceAddress).Result;
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<TcsPerfilLayoutParentComposition>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
                        var responseContent = response.Content.ReadAsStringAsync();
                        responseContent.Wait();
        ...
        private IEnumerable<TcsPerfilFilial> GetLocalTcsPerfilFilial(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._perfilContext.GetTcsPerfilFilialByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._perfilContext.GetTcsPerfilFilialByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<TcsPerfilFilialParentComposition> GetLocalTcsPerfilFilialParentComposition(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._perfilContext.GetTcsPerfilFilialParentCompositionByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._perfilContext.GetTcsPerfilFilialParentCompositionByEntitySearchNoAssociations(null).ToList();
            return result;
        }
