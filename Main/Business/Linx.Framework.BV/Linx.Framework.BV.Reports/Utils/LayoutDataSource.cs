namespace Linx.Framework.BV.Reports {
    ...
    using Linx.Framework.BV.Layout;
    using System.Net.Http;
    using System.Net.Http.Headers;
    
    
    public partial class LayoutDataSource {
        
        private string url = "LinxFrameworkLayout/";
        
        public Linx.Framework.BV.Layout.LayoutDomainService _layoutContext;
        ...
        private IEnumerable<TcsLayout> GetLocalTcsLayout(Telerik.Reporting.Processing.Report report) {
            IEnumerable<TcsLayout> result = default(IEnumerable<TcsLayout>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_layoutContext == null) _layoutContext = new Linx.Framework.BV.Layout.LayoutDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(TcsLayout), new string[] { });
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(TcsLayout), jEntitySearch, false, false, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._layoutContext.GetTcsLayoutByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._layoutContext.GetTcsLayoutByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<TcsLayout> GetTcsLayout(object reportItem) {
            ...
            {
                return GetLocalTcsLayout(report);
            }
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceAddress = "GetSampleTcsLayout?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    ...
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(TcsLayout), new string[] { });
                        serviceAddress = "GetTcsLayoutByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                    ...
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<TcsLayout>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
