namespace Linx.Framework.BV.Reports {
    ...
    public partial class RedeDataSource {
        
        private string url = "LinxFrameworkRede/";
        
        public Linx.Framework.BV.Rede.RedeDomainService _redeContext;
        ...
        private IEnumerable<TbcBandeiraRede> GetLocalTbcBandeiraRede(Telerik.Reporting.Processing.Report report) {
            ...
            if (_redeContext == null) _redeContext = new Linx.Framework.BV.Rede.RedeDomainService(headers) { IsSecure = true };
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._redeContext.GetTbcBandeiraRedeByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._redeContext.GetTbcBandeiraRedeByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<BandeiraRedeCache> GetLocalBandeiraRedeCache(Telerik.Reporting.Processing.Report report) {
            IEnumerable<BandeiraRedeCache> result = default(IEnumerable<BandeiraRedeCache>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_redeContext == null) _redeContext = new Linx.Framework.BV.Rede.RedeDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(BandeiraRedeCache), new string[] { });
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(BandeiraRedeCache), jEntitySearch, false, false, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._redeContext.GetBandeiraRedeCacheByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._redeContext.GetBandeiraRedeCacheByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<BandeiraRedeCache> GetBandeiraRedeCache(object reportItem) {
            var report = (reportItem is Telerik.Reporting.Processing.Report ? reportItem : ((Telerik.Reporting.Processing.ReportItem)(reportItem)).Report) as Telerik.Reporting.Processing.Report;
            IEnumerable<BandeiraRedeCache> result = default(IEnumerable<BandeiraRedeCache>);
            if (report != null && report.Parameters.ContainsKey("CurrentUser") && !report.Parameters["CurrentUser"].Value.IsNullOrEmpty())
            {
                return GetLocalBandeiraRedeCache(report);
            }
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceBus = (report != null && report.Parameters.ContainsKey("ServiceBusUrl") && !report.Parameters["ServiceBusUrl"].Value.IsNullOrEmpty() ? report.Parameters["ServiceBusUrl"].Value.ToString() : "http://localhost:1710/");
                    string serviceAddress = "GetSampleBandeiraRedeCache?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    client.BaseAddress = new Uri(serviceBus + (serviceBus.Right(1) == "/" ? "" : "/") +  url);
                    client.DefaultRequestHeaders.Accept.Clear();
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(BandeiraRedeCache), new string[] { });
                        serviceAddress = "GetBandeiraRedeCacheByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", Convert.ToBase64String(System.Text.ASCIIEncoding.ASCII.GetBytes(string.Format("{0}:{1}", userName.Value.Value, password.Value.Value))));
                    }
                    else
                    {
                    ...
                    HttpResponseMessage response = client.GetAsync(serviceAddress).Result;
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<BandeiraRedeCache>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
                        var responseContent = response.Content.ReadAsStringAsync();
                        responseContent.Wait();
