namespace Linx.Framework.BV.Reports {
    ...
    public partial class UsuarioExternoDataSource {
        
        private string url = "LinxFrameworkUsuarioExterno/";
        
        public Linx.Framework.BV.UsuarioExterno.UsuarioExternoDomainService _usuarioExternoContext;
        ...
        private IEnumerable<TcsUsuarioExterno> GetLocalTcsUsuarioExterno(Telerik.Reporting.Processing.Report report) {
            IEnumerable<TcsUsuarioExterno> result = default(IEnumerable<TcsUsuarioExterno>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_usuarioExternoContext == null) _usuarioExternoContext = new Linx.Framework.BV.UsuarioExterno.UsuarioExternoDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(TcsUsuarioExterno), new string[] { });
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(TcsUsuarioExterno), jEntitySearch, false, false, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._usuarioExternoContext.GetTcsUsuarioExternoByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._usuarioExternoContext.GetTcsUsuarioExternoByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<TcsUsuarioExterno> GetTcsUsuarioExterno(object reportItem) {
            ...
            {
                return GetLocalTcsUsuarioExterno(report);
            }
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceAddress = "GetSampleTcsUsuarioExterno?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    ...
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(TcsUsuarioExterno), new string[] { });
                        serviceAddress = "GetTcsUsuarioExternoByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                    ...
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<TcsUsuarioExterno>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
