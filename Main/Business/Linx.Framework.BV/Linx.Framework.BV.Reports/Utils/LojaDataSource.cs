namespace Linx.Framework.BV.Reports {
    ...
    using Linx.Framework.BV.Loja;
    using System.Net.Http;
    using System.Net.Http.Headers;
    
    
    public partial class LojaDataSource {
        
        private string url = "LinxFrameworkLoja/";
        
        public Linx.Framework.BV.Loja.LojaDomainService _lojaContext;
        ...
        private IEnumerable<LjvLoja> GetLocalLjvLoja(Telerik.Reporting.Processing.Report report) {
            IEnumerable<LjvLoja> result = default(IEnumerable<LjvLoja>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_lojaContext == null) _lojaContext = new Linx.Framework.BV.Loja.LojaDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(LjvLoja), new string[] { });
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(LjvLoja), jEntitySearch, false, false, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._lojaContext.GetLjvLojaByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._lojaContext.GetLjvLojaByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<LjvLoja> GetLjvLoja(object reportItem) {
            ...
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceAddress = "GetSampleLjvLoja?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    ...
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(LjvLoja), new string[] { });
                        serviceAddress = "GetLjvLojaByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                    ...
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<LjvLoja>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
