namespace Linx.Framework.BV.Reports {
    ...
    using Linx.Framework.BV.TabelaAutorizacao;
    using System.Net.Http;
    using System.Net.Http.Headers;
    
    
    public partial class TabelaAutorizacaoDataSource {
        
        private string url = "LinxFrameworkTabelaAutorizacao/";
        
        public Linx.Framework.BV.TabelaAutorizacao.TabelaAutorizacaoDomainService _tabelaAutorizacaoContext;
        ...
        private IEnumerable<TcsTabelaAutorizacao> GetLocalTcsTabelaAutorizacao(Telerik.Reporting.Processing.Report report) {
            IEnumerable<TcsTabelaAutorizacao> result = default(IEnumerable<TcsTabelaAutorizacao>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_tabelaAutorizacaoContext == null) _tabelaAutorizacaoContext = new Linx.Framework.BV.TabelaAutorizacao.TabelaAutorizacaoDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(TcsTabelaAutorizacao), new string[] { });
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(TcsTabelaAutorizacao), jEntitySearch, false, false, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._tabelaAutorizacaoContext.GetTcsTabelaAutorizacaoByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._tabelaAutorizacaoContext.GetTcsTabelaAutorizacaoByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<TcsTabelaAutorizacao> GetTcsTabelaAutorizacao(object reportItem) {
            ...
            {
                return GetLocalTcsTabelaAutorizacao(report);
            }
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceAddress = "GetSampleTcsTabelaAutorizacao?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    ...
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(TcsTabelaAutorizacao), new string[] { });
                        serviceAddress = "GetTcsTabelaAutorizacaoByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                    ...
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<TcsTabelaAutorizacao>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
