namespace Linx.Framework.BV.Reports {
    ...
    public partial class UtilitariosDataSource {
        
        private string url = "LinxFrameworkUtilitarios/";
        
        public Linx.Framework.BV.Utilitarios.UtilitariosDomainService _utilitariosContext;
        ...
        private IEnumerable<TcsUsuarioAutenticacao> GetLocalTcsUsuarioAutenticacao(Telerik.Reporting.Processing.Report report) {
            ...
            if (_utilitariosContext == null) _utilitariosContext = new Linx.Framework.BV.Utilitarios.UtilitariosDomainService(headers) { IsSecure = true };
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._utilitariosContext.GetTcsUsuarioAutenticacaoByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._utilitariosContext.GetTcsUsuarioAutenticacaoByEntitySearchNoAssociations(null).ToList();
            return result;
        }
