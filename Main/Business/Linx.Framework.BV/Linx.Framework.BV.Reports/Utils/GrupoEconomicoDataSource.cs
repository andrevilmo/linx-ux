namespace Linx.Framework.BV.Reports {
    ...
    public partial class GrupoEconomicoDataSource {
        
        private string url = "LinxFrameworkGrupoEconomico/";
        
        public Linx.Framework.BV.GrupoEconomico.GrupoEconomicoDomainService _grupoEconomicoContext;
        ...
        private IEnumerable<TbcGrupoEconomico> GetLocalTbcGrupoEconomico(Telerik.Reporting.Processing.Report report) {
            ...
            if (_grupoEconomicoContext == null) _grupoEconomicoContext = new Linx.Framework.BV.GrupoEconomico.GrupoEconomicoDomainService(headers) { IsSecure = true };
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._grupoEconomicoContext.GetTbcGrupoEconomicoByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._grupoEconomicoContext.GetTbcGrupoEconomicoByEntitySearchNoAssociations(null).ToList();
            if (this.DetailsForLoading != null && this.DetailsForLoading.Length > 0)
            {
               foreach (var entity in result)
               {
                   entity.FillDetails(this._grupoEconomicoContext, null, null, this.DetailsForLoading);
               }
            }
            return result;
        }
        ...
        private IEnumerable<TcsUsuarioGpecon> GetLocalTcsUsuarioGpecon(Telerik.Reporting.Processing.Report report) {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._grupoEconomicoContext.GetTcsUsuarioGpeconByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._grupoEconomicoContext.GetTcsUsuarioGpeconByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<TcsUsuarioGpeconParentComposition> GetLocalTcsUsuarioGpeconParentComposition(Telerik.Reporting.Processing.Report report) {
            ...
            if (_grupoEconomicoContext == null) _grupoEconomicoContext = new Linx.Framework.BV.GrupoEconomico.GrupoEconomicoDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(TcsUsuarioGpeconParentComposition), new string[] {"TbcGrupoEconomico"});
            if (!jEntitySearch.IsNullOrEmpty())
            {
            ...
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._grupoEconomicoContext.GetTcsUsuarioGpeconParentCompositionByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._grupoEconomicoContext.GetTcsUsuarioGpeconParentCompositionByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        ...
        private IEnumerable<EconomicGroupView> GetLocalEconomicGroupView(Telerik.Reporting.Processing.Report report) {
            IEnumerable<EconomicGroupView> result = default(IEnumerable<EconomicGroupView>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_grupoEconomicoContext == null) _grupoEconomicoContext = new Linx.Framework.BV.GrupoEconomico.GrupoEconomicoDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(EconomicGroupView), new string[] { });
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(EconomicGroupView), jEntitySearch, false, false, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._grupoEconomicoContext.GetEconomicGroupViewByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._grupoEconomicoContext.GetEconomicGroupViewByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<EconomicGroupView> GetEconomicGroupView(object reportItem) {
            var report = (reportItem is Telerik.Reporting.Processing.Report ? reportItem : ((Telerik.Reporting.Processing.ReportItem)(reportItem)).Report) as Telerik.Reporting.Processing.Report;
            IEnumerable<EconomicGroupView> result = default(IEnumerable<EconomicGroupView>);
            if (report != null && report.Parameters.ContainsKey("CurrentUser") && !report.Parameters["CurrentUser"].Value.IsNullOrEmpty())
            {
                return GetLocalEconomicGroupView(report);
            }
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceBus = (report != null && report.Parameters.ContainsKey("ServiceBusUrl") && !report.Parameters["ServiceBusUrl"].Value.IsNullOrEmpty() ? report.Parameters["ServiceBusUrl"].Value.ToString() : "http://localhost:1710/");
                    string serviceAddress = "GetSampleEconomicGroupView?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    client.BaseAddress = new Uri(serviceBus + (serviceBus.Right(1) == "/" ? "" : "/") +  url);
                    client.DefaultRequestHeaders.Accept.Clear();
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(EconomicGroupView), new string[] { });
                        serviceAddress = "GetEconomicGroupViewByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", Convert.ToBase64String(System.Text.ASCIIEncoding.ASCII.GetBytes(string.Format("{0}:{1}", userName.Value.Value, password.Value.Value))));
                    }
                    else
                    {
                    ...
                    HttpResponseMessage response = client.GetAsync(serviceAddress).Result;
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<EconomicGroupView>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
                        var responseContent = response.Content.ReadAsStringAsync();
                        responseContent.Wait();
