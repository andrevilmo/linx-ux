namespace Linx.Framework.BV.Reports {
    ...
    using Linx.Framework.BV.LojaVendedor;
    using System.Net.Http;
    using System.Net.Http.Headers;
    
    
    public partial class LojaVendedorDataSource {
        
        private string url = "LinxFrameworkLojaVendedor/";
        
        public Linx.Framework.BV.LojaVendedor.LojaVendedorDomainService _lojaVendedorContext;
        ...
        private IEnumerable<LjvVendedor> GetLocalLjvVendedor(Telerik.Reporting.Processing.Report report) {
            IEnumerable<LjvVendedor> result = default(IEnumerable<LjvVendedor>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_lojaVendedorContext == null) _lojaVendedorContext = new Linx.Framework.BV.LojaVendedor.LojaVendedorDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(LjvVendedor), new string[] { });
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(LjvVendedor), jEntitySearch, false, false, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._lojaVendedorContext.GetLjvVendedorByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._lojaVendedorContext.GetLjvVendedorByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<LjvVendedor> GetLjvVendedor(object reportItem) {
            ...
            {
                return GetLocalLjvVendedor(report);
            }
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceAddress = "GetSampleLjvVendedor?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    ...
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(LjvVendedor), new string[] { });
                        serviceAddress = "GetLjvVendedorByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                    ...
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<LjvVendedor>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
