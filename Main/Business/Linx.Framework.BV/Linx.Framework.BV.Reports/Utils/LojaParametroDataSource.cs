namespace Linx.Framework.BV.Reports {
    ...
    using Linx.Framework.BV.LojaParametro;
    using System.Net.Http;
    using System.Net.Http.Headers;
    
    
    public partial class LojaParametroDataSource {
        
        private string url = "LinxFrameworkLojaParametro/";
        
        public Linx.Framework.BV.LojaParametro.LojaParametroDomainService _lojaParametroContext;
        ...
        private IEnumerable<LjvParametro> GetLocalLjvParametro(Telerik.Reporting.Processing.Report report) {
            IEnumerable<LjvParametro> result = default(IEnumerable<LjvParametro>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_lojaParametroContext == null) _lojaParametroContext = new Linx.Framework.BV.LojaParametro.LojaParametroDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(LjvParametro), new string[] { });
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(LjvParametro), jEntitySearch, false, false, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._lojaParametroContext.GetLjvParametroByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._lojaParametroContext.GetLjvParametroByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<LjvParametro> GetLjvParametro(object reportItem) {
            ...
            {
                return GetLocalLjvParametro(report);
            }
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceAddress = "GetSampleLjvParametro?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    ...
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(LjvParametro), new string[] { });
                        serviceAddress = "GetLjvParametroByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                    ...
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<LjvParametro>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
