define(['durandal/app', 'pkg_linx-framework-custom-bv-spa/services/PerfilFranquiaContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-framework-custom-bv-spa/viewmodels/PerfilFranquiaComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'PerfilFranquia', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_PerfilFranquia(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'PerfilFranquia' ]; settings.parentVM.PerfilFranquia = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('PerfilFranquia', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('PerfilFranquia');
                           }
                           else {
                               parentVM.internalUIs = ['PerfilFranquia'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-custom-bv-spa-PerfilFranquia', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: PerfilFranquia', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        complement.renderPerfilFranquia_dGridTcsPerfilBandeiraRede(vm);

    $('#PerfilFranquia_tcTcsPerfilTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#PerfilFranquia_tcTcsPerfilTabControl');

    complement.renderPerfilFranquia_dGridTcsPerfilRegraModulo(vm);

    complement.renderPerfilFranquia_dGridTcsPerfilRegraTransacao(vm);

    complement.renderPerfilFranquia_dGridTcsPerfilFilial(vm);

    complement.renderPerfilFranquia_dGridTcsUsuarioPerfil(vm);

    complement.renderscyPerfilFranquia_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                           complement.ChangedBrandPerfilFranquia_dGridTcsPerfilBandeiraRede(vm, decimals, reset);
                           complement.ChangedBrandPerfilFranquia_dGridTcsPerfilRegraModulo(vm, decimals, reset);

                           complement.ChangedBrandPerfilFranquia_dGridTcsPerfilRegraTransacao(vm, decimals, reset);

                           complement.ChangedBrandPerfilFranquia_dGridTcsPerfilFilial(vm, decimals, reset);

                           complement.ChangedBrandPerfilFranquia_dGridTcsUsuarioPerfil(vm, decimals, reset);
    ...
    var visibleColumns = 'IdPerfil,DescPerfil,Inativo';
...
if (dataGridName !== "PerfilFranquia_dGridTcsPerfilRegraModulo" || !control.length || !control.data('igGrid')){
	return;
}

$(document).delegate("#PerfilFranquia_dGridTcsPerfilRegraModulo", "iggridupdatingeditcellstarting", function (evt, ui) { 
    if (status() === "C"){
		return true;
	}
/*
	//se coluna igual a LxRegraAcessoModulo e acesso diferente de 2 (Acesso Total) - Bloqueia a alteração
	var acesso = getAbsoluteValue(currentDataItem().currentTcsPerfilRegraModulo().LxRegraAcessoModulo);
	if (ui.columnKey == "LxRegraAcessoModulo" && acesso != 2){
		return false;
	}
	*/
});
    ...
    if (action == "Delete" && status() == "Q"){
	var table = currentDataItem();
	if (table.TcsUsuarioPerfilList().count() > 0){
		app.showMessage(("Não será possível excluir, existem usuários atrelados a este perfil.").toString(), 'Alerta', ['Ok']);
		return false;
	}
}
return true;
    ...
    var removeFormViewControl = function () {
        var front = $('#PerfilFranquia_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#PerfilFranquia_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#PerfilFranquia_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        ...
        TcsPerfilRegraModuloAcesso: ko.observable(null),
        TcsPerfilRegraModuloDescModulo: ko.observable(null),
        TcsPerfilRegraTransacaoAcesso: ko.observable(null),
    ...
    entitySearchRange.clear = function(){
        ...
        entitySearchRange.TcsPerfilRegraModuloAcesso(null);
        entitySearchRange.TcsPerfilRegraModuloDescModulo(null);
        entitySearchRange.TcsPerfilRegraTransacaoAcesso(null);
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#PerfilFranquia_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootNamespace: 'Linx.Framework.Custom.BV.PerfilFranquia',
            ...
            __moduleId__: 'pkg_linx-framework-custom-bv-spa/viewmodels/PerfilFranquia',
            pivots : pivots
        };
