...
namespace Linx.Framework.Custom.BV.PerfilFranquia
{
    ...
    public partial class TcsPerfilRegraModulo
    {
        ...
        private string Values(int valueOption)
        {
            ...
            if (this._DescModulo.IsNullOrEmpty() && !this.IdModulo.IsNullOrEmpty())
            {
                string descriptions = Linx.Framework.BV.Utils.GetDescModulo(this.IdModulo);
                this._DescModulo = descriptions.Extract("[", "]");
                this._DescAplicativo = descriptions.Extract(",[", "]");
                this._Origem = descriptions.Extract(",[", "]", 2);
            }
        ...
        {
            if (!this._Origem.IsNullOrEmpty() || this.IdModulo.IsNullOrEmpty())
                return this._Origem;
