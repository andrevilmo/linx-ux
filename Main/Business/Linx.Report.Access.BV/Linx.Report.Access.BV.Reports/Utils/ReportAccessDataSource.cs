namespace Linx.Report.Access.BV.Reports {
    using System;
    using System.Linq;
    using System.Text;
    ...
    public partial class ReportAccessDataSource {
        
        private string url = "LinxReportAccessReportAccess/";
        
        public Linx.Report.Access.BV.ReportAccess.ReportAccessDomainService _reportAccessContext;
        ...
        private IEnumerable<TelerikReportsList> GetLocalTelerikReportsList(Telerik.Reporting.Processing.Report report) {
            IEnumerable<TelerikReportsList> result = default(IEnumerable<TelerikReportsList>);
            Dictionary<string, string> headers = LinxReportHelper.GetReportHeaders(report.Parameters);
            if (_reportAccessContext == null) _reportAccessContext = new Linx.Report.Access.BV.ReportAccess.ReportAccessDomainService(headers) { IsSecure = true };
            string entitySearchExpression = String.Empty;
            var jEntitySearch = GetFilterExpression(report, typeof(TelerikReportsList), new string[] { });
            if (!jEntitySearch.IsNullOrEmpty())
            {
                entitySearchExpression = Linx.Tools.EntitySearch.ParseFromJEntitySearch(typeof(TelerikReportsList), jEntitySearch, false, false, false);
            }
            if (!entitySearchExpression.IsNullOrEmpty() || !jEntitySearch.IsNullOrEmpty())
              result = this._reportAccessContext.GetTelerikReportsListByEntitySearchNoAssociations(entitySearchExpression, jEntitySearch).ToList();
            else
              result = this._reportAccessContext.GetTelerikReportsListByEntitySearchNoAssociations(null).ToList();
            return result;
        }
        
        public virtual IEnumerable<TelerikReportsList> GetTelerikReportsList(object reportItem) {
            ...
            {
                return GetLocalTelerikReportsList(report);
            }
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    string serviceAddress = "GetSampleTelerikReportsList?details=" + String.Join("-", (this.DetailsForLoading ?? new string[] {}));
                    ...
                    if (!userName.Key.IsNullOrEmpty() && !password.Key.IsNullOrEmpty())
                    {
                        ...
                        var jEntitySearch = GetFilterExpression(report, typeof(TelerikReportsList), new string[] { });
                        serviceAddress = "GetTelerikReportsListByEntitySearchNoAssociations?jEntitySearch=" + System.Web.HttpUtility.UrlEncode(jEntitySearch);
                    ...
                    if (response.IsSuccessStatusCode)
                        result = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<TelerikReportsList>>(response.Content.ReadAsStringAsync().Result);
                    else
                    {
