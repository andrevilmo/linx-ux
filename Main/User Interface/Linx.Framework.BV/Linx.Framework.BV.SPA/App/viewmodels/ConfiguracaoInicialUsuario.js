define(['durandal/app', 'pkg_linx-framework-bv-spa/services/ConfiguracaoContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-framework-bv-spa/viewmodels/ConfiguracaoInicialUsuarioComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'ConfiguracaoInicialUsuario', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_ConfiguracaoInicialUsuario(), 'Name'))());
        }, function (err) {
    ...
    var activate = function (settings, querystring) {
      ...
      vm.ConfiguracaoInicialUsuario = getVM;
      //loadLanguage();
      changeLanguage();
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'ConfiguracaoInicialUsuario' ]; settings.parentVM.ConfiguracaoInicialUsuario = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.ConfiguracaoInicialUsuario = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('ConfiguracaoInicialUsuario', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('ConfiguracaoInicialUsuario');
                           }
                           else {
                               parentVM.internalUIs = ['ConfiguracaoInicialUsuario'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        uiSettings = null;
        isDependentVM(false);
        setSecurity(false, true, false, false, false, false, false, false, false, false);
        managerAuth.getFormAccess('linx-framework-bv-spa-ConfiguracaoInicialUsuario', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: ConfiguracaoInicialUsuario', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#ConfiguracaoInicialUsuario_tcTcsUsuarioConfiguracaoTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#ConfiguracaoInicialUsuario_tcTcsUsuarioConfiguracaoTabControl');

    complement.renderConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso(vm);

    complement.renderscyConfiguracaoInicialUsuario_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                               complement.ChangedBrandConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsUsuarioConfiguracao';
    ...
    var visibleColumns = 'IdLinx,NomeEmpresa,NomeUsuario,NomeAutenticacao';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsUsuarioConfiguracao', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsUsuarioConfiguracao', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsUsuarioConfiguracao', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsUsuarioConfiguracao');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsUsuarioConfiguracao');
        ...
        return dataContext.getTcsUsuarioConfiguracaoByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsUsuarioConfiguracao'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsUsuarioConfiguracaoByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
...
if (dataGridName === "ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso"){
	$(document).delegate("#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso", "iggridupdatingeditcellstarting", function (evt, ui) { 
		return ui.columnKey === "Selecionado"
	});
}

    }
    var CustomControl167994734_Click = function () {
    //Força o commit para o caso de marcar o checkbox e clicar no botão sem mudar de célula
var grid = $lx(vm,"#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso");
if (grid.igGridUpdating('isEditing')) {
	grid.igGrid('commit');
}

var items = $.grep(vm.currentDataItem().TcsUsuarioConfiguracaoAcessoList(), function (element, index) { return getAbsoluteValue(element.Selecionado) === true });
var acesso = [];

for (var i = 0; i < items.length; i++) {
	var current = items[i];
	acesso.push({ 	IdLinx : getAbsoluteValue(current.IdLinx), 
					UidAplicacao : getAbsoluteValue(current.UidAplicacao), 
					IdTcsAmbiente : getAbsoluteValue(current.IdTcsAmbiente), 
					IdTcsAplicativo : getAbsoluteValue(current.IdTcsAplicativo), 
					IdUsuario : getAbsoluteValue(current.IdUsuario), 
					UidEmpresa : getAbsoluteValue(current.UidEmpresa)
				});
}

if (acesso.length == 0){
	app.showMessage(("Ambientes não informados.").toString(), 'Informação', ['Ok'])
	return;	
	}
	
app.showMessage(('Antes de prosseguir verifique se a Empresa possui Módulos cadastrados e se o Módulo "Utilitários" está habilitado.\n\nConfirma execução do processo ?'), 'Atenção', ['Yes', 'No'])
      .then(function (selectedOption) {
      if (selectedOption === 'Yes'){
		   vm.showProcessing('Atualizando informações.');
			...
			$.ajax({
					type: 'POST',
					message: "Gerando configuração inicial.",
					messageUser: "Gerando configuração inicial.",
					globalError: true,
					headers: managerAuth.getHeaders(),
					url: managerAuth.getServiceAddress('LinxFrameworkConfiguracao', 'Linx.Framework.Bv') + '/CriaConfiguracaoInicial' ,
					...
					success: function (data) {
						app.showMessage(("Processo finalizado com sucesso.").toString(), 'Informação', ['Ok']);
						vm.closeProcessing();
					}
			});	  
		}
    ...
    ctrl.setCustomEnable(vm, '#tbNomeAutenticacao', false); 
    }
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsUsuarioConfiguracao(getBandeiraRede(), clearComplete);
    ...
    var createTcsUsuarioConfiguracao = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsUsuarioConfiguracao();
    ...
    var createAndNotifyTcsUsuarioConfiguracao = function() {
        var entity = createTcsUsuarioConfiguracao();
        notifyPresentation('');
        return entity;
    };
    
    var createTcsUsuarioConfiguracaoAcesso = function(parent, noCurrent) {
        dataBind('TcsUsuarioConfiguracaoAcessoList', true);
        var entity = dataContext.createTcsUsuarioConfiguracaoAcesso(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsUsuarioConfiguracaoAcesso(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsUsuarioConfiguracaoAcesso = function(parent) {
        var entity = createTcsUsuarioConfiguracaoAcesso(parent);
    ...
    var addNew = function (parentEntity) {
        ...
        goToItem(createTcsUsuarioConfiguracao());
        editInnerUIs();
        showButtonsEditorTemplate();
        dataBind();
    };
    ...
    var _canRefreshData = true, _canQuickSearch = true, _canAddNew = false, _canClear = true, _canCustomSearch = false, _canDelete = false, _canEdit = false, _canLayout = false, _canNavigate = false, _canPrint = false, _canSearch = false, _canExport = false, _noBusyLoading = false;
    ...
    var removeFormViewControl = function () {
        var front = $('#ConfiguracaoInicialUsuario_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#ConfiguracaoInicialUsuario_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#ConfiguracaoInicialUsuario_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsUsuarioConfiguracaoIdLinx: ko.observable(null),
        TcsUsuarioConfiguracaoNomeEmpresa: ko.observable(null),
        TcsUsuarioConfiguracaoNomeUsuario: ko.observable(null),
        TcsUsuarioConfiguracaoAcessoDescricaoAmbiente: ko.observable(null),
        TcsUsuarioConfiguracaoAcessoDescricaoAplicacao: ko.observable(null),
        TcsUsuarioConfiguracaoAcessoDescricaoAplicativo: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsUsuarioConfiguracaoIdLinx(null);
        entitySearchRange.TcsUsuarioConfiguracaoNomeEmpresa(null);
        entitySearchRange.TcsUsuarioConfiguracaoNomeUsuario(null);
        entitySearchRange.TcsUsuarioConfiguracaoAcessoDescricaoAmbiente(null);
        entitySearchRange.TcsUsuarioConfiguracaoAcessoDescricaoAplicacao(null);
        entitySearchRange.TcsUsuarioConfiguracaoAcessoDescricaoAplicativo(null);
    };
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#ConfiguracaoInicialUsuario_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            dataView: dataView,
            viewName: 'ConfiguracaoInicialUsuario',
            ...
            rootDataTypeName: 'TcsUsuarioConfiguracao',
            rootNamespace: 'Linx.Framework.BV.Configuracao',
            ...
            CustomControl167994734_Click: CustomControl167994734_Click,
            createTcsUsuarioConfiguracao: createTcsUsuarioConfiguracao,
            createAndNotifyTcsUsuarioConfiguracao: createAndNotifyTcsUsuarioConfiguracao,
            createTcsUsuarioConfiguracaoAcesso: createTcsUsuarioConfiguracaoAcesso,
            createAndNotifyTcsUsuarioConfiguracaoAcesso: createAndNotifyTcsUsuarioConfiguracaoAcesso,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/ConfiguracaoInicialUsuario',
            pivots : pivots
        };
