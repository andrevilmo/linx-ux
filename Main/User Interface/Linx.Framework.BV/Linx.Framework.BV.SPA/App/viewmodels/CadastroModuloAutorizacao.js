...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroModuloAutorizacao', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroModuloAutorizacao(), 'Name'))());
        }, function (err) {
    ...
    var activate = function (settings, querystring) {
      ...
      vm.CadastroModuloAutorizacao = getVM;
      //loadLanguage();
      changeLanguage();
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroModuloAutorizacao' ]; settings.parentVM.CadastroModuloAutorizacao = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.CadastroModuloAutorizacao = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('CadastroModuloAutorizacao', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroModuloAutorizacao');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroModuloAutorizacao'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroModuloAutorizacao', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroModuloAutorizacao', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        complement.renderCadastroModuloAutorizacao_dGridTcsModuloMenuAutorizacao(vm);

    complement.renderCadastroModuloAutorizacao_dGridTcsTransacaoMenuAutorizacaoModulo(vm);

    complement.renderscyCadastroModuloAutorizacao_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                                   complement.ChangedBrandCadastroModuloAutorizacao_dGridTcsModuloMenuAutorizacao(vm, decimals, reset);
                               complement.ChangedBrandCadastroModuloAutorizacao_dGridTcsTransacaoMenuAutorizacaoModulo(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsModuloAutorizacao';
    ...
    var visibleColumns = 'NomeCurto,DescModulo,DescricaoAplicativo,OrdemNavegacao,Inativo,Icone';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsModuloAutorizacao', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsModuloAutorizacao', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsModuloAutorizacao', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsModuloAutorizacao');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsModuloAutorizacao');
        ...
        return dataContext.getTcsModuloAutorizacaoByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsModuloAutorizacao'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsModuloAutorizacaoByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsModuloAutorizacao(getBandeiraRede(), clearComplete);
    ...
    var createTcsModuloAutorizacao = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsModuloAutorizacao();
    ...
    var createAndNotifyTcsModuloAutorizacao = function() {
        var entity = createTcsModuloAutorizacao();
        notifyPresentation('');
        return entity;
    };
    
    var createTcsModuloMenuAutorizacao = function(parent, noCurrent) {
        dataBind('TcsModuloMenuAutorizacaoList', true);
        var entity = dataContext.createTcsModuloMenuAutorizacao(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsModuloMenuAutorizacao(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsModuloMenuAutorizacao = function(parent) {
        var entity = createTcsModuloMenuAutorizacao(parent);
        notifyPresentation('TcsModuloMenuAutorizacaoList');
        return entity;
    };
    
    var createTcsTransacaoMenuAutorizacaoModulo = function(parent, noCurrent) {
        dataBind('TcsTransacaoMenuAutorizacaoModuloList', true);
        var entity = dataContext.createTcsTransacaoMenuAutorizacaoModulo(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsTransacaoMenuAutorizacaoModulo(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsTransacaoMenuAutorizacaoModulo = function(parent) {
        var entity = createTcsTransacaoMenuAutorizacaoModulo(parent);
    ...
    var addNew = function (parentEntity) {
        ...
        goToItem(createTcsModuloAutorizacao());
        editInnerUIs();
        showButtonsEditorTemplate();
        dataBind();
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroModuloAutorizacao_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroModuloAutorizacao_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroModuloAutorizacao_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsModuloMenuAutorizacaoOrdemNavegacao_begin: ko.observable(null), TcsModuloMenuAutorizacaoOrdemNavegacao_end: ko.observable(null),
        TcsTransacaoMenuAutorizacaoModuloOrdemNavegacao_begin: ko.observable(null), TcsTransacaoMenuAutorizacaoModuloOrdemNavegacao_end: ko.observable(null),
        TcsModuloAutorizacaoDescricaoAplicativo: ko.observable(null),
        TcsModuloMenuAutorizacaoDescModuloMenuSuperior: ko.observable(null),
        TcsTransacaoMenuAutorizacaoModuloDescTransacao: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsModuloMenuAutorizacaoOrdemNavegacao_begin(null); entitySearchRange.TcsModuloMenuAutorizacaoOrdemNavegacao_end(null);
        entitySearchRange.TcsTransacaoMenuAutorizacaoModuloOrdemNavegacao_begin(null); entitySearchRange.TcsTransacaoMenuAutorizacaoModuloOrdemNavegacao_end(null);
        entitySearchRange.TcsModuloAutorizacaoDescricaoAplicativo(null);
        entitySearchRange.TcsModuloMenuAutorizacaoDescModuloMenuSuperior(null);
        entitySearchRange.TcsTransacaoMenuAutorizacaoModuloDescTransacao(null);
    };
    entitySearchRange.has_TcsModuloMenuAutorizacaoOrdemNavegacao = ko.computed(function(){ return (entitySearchRange.TcsModuloMenuAutorizacaoOrdemNavegacao_begin() != null || entitySearchRange.TcsModuloMenuAutorizacaoOrdemNavegacao_end() != null); });
    entitySearchRange.has_TcsTransacaoMenuAutorizacaoModuloOrdemNavegacao = ko.computed(function(){ return (entitySearchRange.TcsTransacaoMenuAutorizacaoModuloOrdemNavegacao_begin() != null || entitySearchRange.TcsTransacaoMenuAutorizacaoModuloOrdemNavegacao_end() != null); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroModuloAutorizacao_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            dataView: dataView,
            viewName: 'CadastroModuloAutorizacao',
            ...
            rootBmTypeName: 'TCS_MODULO_AUTORIZACAO',
            rootDataTypeName: 'TcsModuloAutorizacao',
            rootNamespace: 'Linx.Framework.BV.ModuloAutorizacao',
            ...
            createAndNotifyTcsModuloAutorizacao: createAndNotifyTcsModuloAutorizacao,
            createTcsModuloMenuAutorizacao: createTcsModuloMenuAutorizacao,
            createAndNotifyTcsModuloMenuAutorizacao: createAndNotifyTcsModuloMenuAutorizacao,
            createTcsTransacaoMenuAutorizacaoModulo: createTcsTransacaoMenuAutorizacaoModulo,
            createAndNotifyTcsTransacaoMenuAutorizacaoModulo: createAndNotifyTcsTransacaoMenuAutorizacaoModulo,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroModuloAutorizacao',
            pivots : pivots
        };
