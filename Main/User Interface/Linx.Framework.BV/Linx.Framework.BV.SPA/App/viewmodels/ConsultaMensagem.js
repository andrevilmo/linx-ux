define(['durandal/app', 'pkg_linx-framework-bv-spa/services/MensagemContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-framework-bv-spa/viewmodels/ConsultaMensagemComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'ConsultaMensagem', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_ConsultaMensagem(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'ConsultaMensagem' ]; settings.parentVM.ConsultaMensagem = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.ConsultaMensagem = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('ConsultaMensagem', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('ConsultaMensagem');
                           }
                           else {
                               parentVM.internalUIs = ['ConsultaMensagem'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-ConsultaMensagem', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: ConsultaMensagem', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#ConsultaMensagem_tcTabControl_eb3f696999dd454ba38a842c72390b0d').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#ConsultaMensagem_tcTabControl_eb3f696999dd454ba38a842c72390b0d');

    complement.renderConsultaMensagem_dGridTcsMensagemConsultaLog(vm);

    complement.renderscyConsultaMensagem_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                                   complement.ChangedBrandConsultaMensagem_dGridTcsMensagemConsultaLog(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsMensagemConsulta';
    ...
    var visibleColumns = 'Titulo,LxTipoMensagem,Criacao,Envio,NomeEmpresa,NomeUsuario';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsMensagemConsulta', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsMensagemConsulta', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsMensagemConsulta', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsMensagemConsulta');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsMensagemConsulta');
        ...
        return dataContext.getTcsMensagemConsultaByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsMensagemConsulta'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsMensagemConsultaByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
    ...
    var AdjustValue = function () {
    var control = $lx(vm, '#divCorpo');
if (control){
	control.html(getAbsoluteValue(vm.currentDataItem().Corpo))
}
    }
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsMensagemConsulta(getBandeiraRede(), clearComplete);
    ...
    var createTcsMensagemConsulta = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsMensagemConsulta();
    ...
    var createAndNotifyTcsMensagemConsulta = function() {
        var entity = createTcsMensagemConsulta();
        notifyPresentation('');
        return entity;
    };
    
    var createTcsMensagemConsultaLog = function(parent, noCurrent) {
        dataBind('TcsMensagemConsultaLogList', true);
        var entity = dataContext.createTcsMensagemConsultaLog(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsMensagemConsultaLog(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsMensagemConsultaLog = function(parent) {
        var entity = createTcsMensagemConsultaLog(parent);
    ...
    var addNew = function (parentEntity) {
        ...
        goToItem(createTcsMensagemConsulta());
        editInnerUIs();
        showButtonsEditorTemplate();
        dataBind();
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#ConsultaMensagem_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#ConsultaMensagem_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#ConsultaMensagem_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsMensagemConsultaCriacao_typeRange: ko.observable('R'), TcsMensagemConsultaCriacao_begin: ko.observable(null), TcsMensagemConsultaCriacao_end: ko.observable(null), TcsMensagemConsultaCriacao_predefFilter: ko.observableArray([]), TcsMensagemConsultaCriacao_predefValue: ko.observable(null),
        TcsMensagemConsultaEnvio_typeRange: ko.observable('R'), TcsMensagemConsultaEnvio_begin: ko.observable(null), TcsMensagemConsultaEnvio_end: ko.observable(null), TcsMensagemConsultaEnvio_predefFilter: ko.observableArray([]), TcsMensagemConsultaEnvio_predefValue: ko.observable(null),
        TcsMensagemConsultaLogDispensada_typeRange: ko.observable('R'), TcsMensagemConsultaLogDispensada_begin: ko.observable(null), TcsMensagemConsultaLogDispensada_end: ko.observable(null), TcsMensagemConsultaLogDispensada_predefFilter: ko.observableArray([]), TcsMensagemConsultaLogDispensada_predefValue: ko.observable(null),
        TcsMensagemConsultaLogEntregue_typeRange: ko.observable('R'), TcsMensagemConsultaLogEntregue_begin: ko.observable(null), TcsMensagemConsultaLogEntregue_end: ko.observable(null), TcsMensagemConsultaLogEntregue_predefFilter: ko.observableArray([]), TcsMensagemConsultaLogEntregue_predefValue: ko.observable(null),
        TcsMensagemConsultaLogLida_typeRange: ko.observable('R'), TcsMensagemConsultaLogLida_begin: ko.observable(null), TcsMensagemConsultaLogLida_end: ko.observable(null), TcsMensagemConsultaLogLida_predefFilter: ko.observableArray([]), TcsMensagemConsultaLogLida_predefValue: ko.observable(null),
        TcsMensagemConsultaNomeEmpresa: ko.observable(null),
        TcsMensagemConsultaNomeUsuario: ko.observable(null),
        TcsMensagemConsultaLogNomeUsuario: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsMensagemConsultaCriacao_typeRange('R'); entitySearchRange.TcsMensagemConsultaCriacao_begin(null); entitySearchRange.TcsMensagemConsultaCriacao_end(null); entitySearchRange.TcsMensagemConsultaCriacao_predefFilter([]); entitySearchRange.TcsMensagemConsultaCriacao_predefValue(null);
        entitySearchRange.TcsMensagemConsultaEnvio_typeRange('R'); entitySearchRange.TcsMensagemConsultaEnvio_begin(null); entitySearchRange.TcsMensagemConsultaEnvio_end(null); entitySearchRange.TcsMensagemConsultaEnvio_predefFilter([]); entitySearchRange.TcsMensagemConsultaEnvio_predefValue(null);
        entitySearchRange.TcsMensagemConsultaLogDispensada_typeRange('R'); entitySearchRange.TcsMensagemConsultaLogDispensada_begin(null); entitySearchRange.TcsMensagemConsultaLogDispensada_end(null); entitySearchRange.TcsMensagemConsultaLogDispensada_predefFilter([]); entitySearchRange.TcsMensagemConsultaLogDispensada_predefValue(null);
        entitySearchRange.TcsMensagemConsultaLogEntregue_typeRange('R'); entitySearchRange.TcsMensagemConsultaLogEntregue_begin(null); entitySearchRange.TcsMensagemConsultaLogEntregue_end(null); entitySearchRange.TcsMensagemConsultaLogEntregue_predefFilter([]); entitySearchRange.TcsMensagemConsultaLogEntregue_predefValue(null);
        entitySearchRange.TcsMensagemConsultaLogLida_typeRange('R'); entitySearchRange.TcsMensagemConsultaLogLida_begin(null); entitySearchRange.TcsMensagemConsultaLogLida_end(null); entitySearchRange.TcsMensagemConsultaLogLida_predefFilter([]); entitySearchRange.TcsMensagemConsultaLogLida_predefValue(null);
        entitySearchRange.TcsMensagemConsultaNomeEmpresa(null);
        entitySearchRange.TcsMensagemConsultaNomeUsuario(null);
        entitySearchRange.TcsMensagemConsultaLogNomeUsuario(null);
    };
    entitySearchRange.has_TcsMensagemConsultaCriacao = ko.computed(function(){ return (entitySearchRange.TcsMensagemConsultaCriacao_typeRange() == 'R' && (entitySearchRange.TcsMensagemConsultaCriacao_begin() != null || entitySearchRange.TcsMensagemConsultaCriacao_end() != null) || (entitySearchRange.TcsMensagemConsultaCriacao_typeRange() == 'P' && entitySearchRange.TcsMensagemConsultaCriacao_predefFilter().length > 0)); });
    entitySearchRange.has_TcsMensagemConsultaEnvio = ko.computed(function(){ return (entitySearchRange.TcsMensagemConsultaEnvio_typeRange() == 'R' && (entitySearchRange.TcsMensagemConsultaEnvio_begin() != null || entitySearchRange.TcsMensagemConsultaEnvio_end() != null) || (entitySearchRange.TcsMensagemConsultaEnvio_typeRange() == 'P' && entitySearchRange.TcsMensagemConsultaEnvio_predefFilter().length > 0)); });
    entitySearchRange.has_TcsMensagemConsultaLogDispensada = ko.computed(function(){ return (entitySearchRange.TcsMensagemConsultaLogDispensada_typeRange() == 'R' && (entitySearchRange.TcsMensagemConsultaLogDispensada_begin() != null || entitySearchRange.TcsMensagemConsultaLogDispensada_end() != null) || (entitySearchRange.TcsMensagemConsultaLogDispensada_typeRange() == 'P' && entitySearchRange.TcsMensagemConsultaLogDispensada_predefFilter().length > 0)); });
    entitySearchRange.has_TcsMensagemConsultaLogEntregue = ko.computed(function(){ return (entitySearchRange.TcsMensagemConsultaLogEntregue_typeRange() == 'R' && (entitySearchRange.TcsMensagemConsultaLogEntregue_begin() != null || entitySearchRange.TcsMensagemConsultaLogEntregue_end() != null) || (entitySearchRange.TcsMensagemConsultaLogEntregue_typeRange() == 'P' && entitySearchRange.TcsMensagemConsultaLogEntregue_predefFilter().length > 0)); });
    entitySearchRange.has_TcsMensagemConsultaLogLida = ko.computed(function(){ return (entitySearchRange.TcsMensagemConsultaLogLida_typeRange() == 'R' && (entitySearchRange.TcsMensagemConsultaLogLida_begin() != null || entitySearchRange.TcsMensagemConsultaLogLida_end() != null) || (entitySearchRange.TcsMensagemConsultaLogLida_typeRange() == 'P' && entitySearchRange.TcsMensagemConsultaLogLida_predefFilter().length > 0)); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#ConsultaMensagem_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: 'TCS_MENSAGEM',
            rootDataTypeName: 'TcsMensagemConsulta',
            rootNamespace: 'Linx.Framework.BV.Mensagem',
            ...
            createAndNotifyTcsMensagemConsulta: createAndNotifyTcsMensagemConsulta,
            createTcsMensagemConsultaLog: createTcsMensagemConsultaLog,
            createAndNotifyTcsMensagemConsultaLog: createAndNotifyTcsMensagemConsultaLog,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/ConsultaMensagem',
            pivots : pivots
        };
