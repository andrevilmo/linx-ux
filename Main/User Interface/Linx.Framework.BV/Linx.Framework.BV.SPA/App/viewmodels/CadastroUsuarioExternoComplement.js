define(['managers/__auth', 'managers/user'], function (managerAuth, managerUser) {
    var complementCtor = function() {
        var complement = {
            isAutomatic: true
            , ChangedBrandscyCadastroUsuarioExterno_dGrid: function(vm, decimals, reset) {
                var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#scyCadastroUsuarioExterno_dGrid').data('igGrid'),
                    grdUpd = $('#scyCadastroUsuarioExterno_dGrid').data('igGridUpdating');
            ...
            , renderscyCadastroUsuarioExterno_dGrid: function(vm) {
                ...
                $('#scyCadastroUsuarioExterno_dGrid_headers').live('focus  keydown', function (evt) {
                    var keyCode = window.event ? evt.which : evt.keyCode;
                    if (keyCode === 9) {
                        var cols = $('#scyCadastroUsuarioExterno_dGrid').igGrid('option', 'columns');
                        var dataView = $('#scyCadastroUsuarioExterno_dGrid').data('igGrid').dataSource._dataView
                        if (dataView.length === 0) return;
                        var firstRow = dataView[0].RowDataId;
                        clear = vm.status() === 'C';
                        if (vm.status() === 'C')
                            $('#scyCadastroUsuarioExterno_dGrid').igGridUpdating('startEdit', firstRow, 0, true);
                        else {
                            ...
                            cols.some(function (entry) {
                                if (entry.key !== 'RowDataId' && !entry.hidden) {
                                    if (verifyCanEditCol(entry.key, clear, entity)) {
                                        $('#scyCadastroUsuarioExterno_dGrid').igGridUpdating('startEdit', firstRow, indexColumn, true);
                                        return true;
                                    }
                                    indexColumn++;
                                }
                ...
                var getVisibleColumns = function(metaDataControl) {
                   ...
                   var visibleColumns = '';
                   if($('#scyCadastroUsuarioExterno_dGrid').data('igGrid') === undefined) return '';
                   var cols = $('#scyCadastroUsuarioExterno_dGrid').igGrid('option', 'columns');
                   if (cols) {
                ...
                var isElementHided = function (grid, forceCreating) {
                  if (!grid) grid = $('#scyCadastroUsuarioExterno_dGrid');
                  return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialog').is(':visible'));
                }
                var refreshData = true;
                var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'scyCadastroUsuarioExterno_dGrid_container', dataBind: function (commitData, forceCreating) {
                   var grid = $('#scyCadastroUsuarioExterno_dGrid');
                   ...
                   if (!started) {
                       ...
                       started = true;
                       commitData = false;
                       $('#scyCadastroUsuarioExterno_dGrid_groupbyarea').addClass('hide');
                   }
                   if (grid.igGridUpdating('isEditing')) {
                ...
                function verifyCanEditCol(column, clear, entity){
                    switch(column){
                        ...
                        case 'IdentidadeExterna': { canEditing = clear; break;}
                        case 'LxTipoAutenticador': { canEditing = clear; break;}
                        case 'IdDispositivo': { canEditing = clear; break;}
                ...
                function createDataGrid(grid) {
                    ...
                    vm.gridSaveStates[gridId] = {
                        ...
                        closePopover: function () {
                            $('#scyCadastroUsuarioExterno_dGrid_LayoutBtn').igPopover('hide');
                        },
                    ...
                    grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
                        ...
                        cellClick: function(evt, ui) {
                             if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                                 ...
                                 if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                                 {
                                      ...
                                      showMultimidia(entity, e, table, key, vm.CadastroUsuarioExterno());
                                 }
                             }
                             if (typeof vm.OnGridClientClick === 'function') {
                                 entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                 vm.OnGridClientClick('scyCadastroUsuarioExterno_dGrid', ui.colKey, entity);
                             }
                             if (vm.status() != 'Q') {
                                 var grid = $('#scyCadastroUsuarioExterno_dGrid');
                        ...
                        columns: [
                            ...
                            { key: 'NomeUsuario', headerText: vm.getLayoutDisplayName('CadastroUsuarioExterno_tbNomeUsuario'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'CnpjCpf', headerText: vm.getLayoutDisplayName('CadastroUsuarioExterno_tbCnpjCpf'), width: '200px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'Email', headerText: vm.getLayoutDisplayName('CadastroUsuarioExterno_tbEmail'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'FoneCelular', headerText: vm.getLayoutDisplayName('CadastroUsuarioExterno_tbFoneCelular'), width: '200px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'DataCadastro', headerText: vm.getLayoutDisplayName('CadastroUsuarioExterno_dtDataCadastro'), width: '205px', dataType: 'date', columnCssClass: 'ellipsis', format: 'dd/MM/yyyy', hidden: false, unbound: false, group: null   },
                            { key: 'IdentidadeExterna', headerText: vm.getLayoutDisplayName('CadastroUsuarioExterno_tbIdentidadeExterna'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'LxTipoAutenticador', headerText: vm.getLayoutDisplayName('CadastroUsuarioExterno_cmbLxTipoAutenticador'), width: '200px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null  , formatter: function (val, record) { return  vm.dataDomains.getName('TipoAutenticador', val);} },
                            { key: 'IdDispositivo', headerText: vm.getLayoutDisplayName('CadastroUsuarioExterno_tbIdDispositivo'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'Inativo', headerText: vm.getLayoutDisplayName('CadastroUsuarioExterno_ckInativo'), width: '127px', dataType: 'bool', columnCssClass: 'ellipsis', format: 'checkbox', hidden: false, unbound: false, group: null   }
                        ],
                        features: [
                                    ...
                                    filterDialogOpening: function (evt, ui) {
                                        ...
                                        var dataView = $('#scyCadastroUsuarioExterno_dGrid').data('igGrid').dataSource;
                                        ...
                                        scriptHtml += '    var newGrid = $("#scyCadastroUsuarioExterno_dGrid");';
                                        scriptHtml += '    var listFilter = [];';
                                        scriptHtml += '    function hideColumn(){';
                                        scriptHtml += '     if ($("#showHideColumn_scyCadastroUsuarioExterno_dGrid")["0"].innerHTML.indexOf("Ocultar") >= 0) {';
                                        scriptHtml += '        $("#showHideColumn_scyCadastroUsuarioExterno_dGrid")["0"].innerHTML = "Mostrar Coluna";';
                                        scriptHtml += '        newGrid.igGridHiding("hideColumn", newCol);';
                                        scriptHtml += '     }';
                                        scriptHtml += '     else{';
                                        scriptHtml += '        $("#showHideColumn_scyCadastroUsuarioExterno_dGrid")["0"].innerHTML = "Ocultar Coluna";';
                                        ...
                                        scriptHtml += '             $("#showHideColumn_scyCadastroUsuarioExterno_dGrid")["0"].innerHTML = column[0].hidden ? "Mostrar Coluna" : "Ocultar Coluna"';
                                        scriptHtml += '         }';
                                        scriptHtml += '    }';
                                        ...
                                        scriptHtml += '              <div id="comboFields_scyCadastroUsuarioExterno_dGrid"></div>';
                                        scriptHtml += '              <script>';
                                        scriptHtml += '                  var columns = newGrid.igGrid("option", "columns");';
                                        scriptHtml += '                  $("#comboFields_scyCadastroUsuarioExterno_dGrid").igCombo({ dataSource: columns, mode : "dropdown", valueKey: "key", textKey: "headerText", selectionChanging: function (evt, ui) {';
                                        scriptHtml += '                      newCol = ui.items["0"].data.key;';
                                        scriptHtml += '                      updateHideButton()';
                                        scriptHtml += '                  }});';
                                        scriptHtml += '                  $("#comboFields_scyCadastroUsuarioExterno_dGrid").igCombo("value", newCol);';
                                        scriptHtml += '              </script>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '     <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">';
                                        ...
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <div style="margin-left: 5px; margin-top: 5px">';
                                        scriptHtml += '              <i class="fa fa-eye-slash" aria-hidden="true" style="margin-right: 5px;"></i><a onclick="hideColumn()" style="cursor: pointer" id="showHideColumn_scyCadastroUsuarioExterno_dGrid">Ocultar Coluna</a>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <br>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '  </div>';
                                    ...
                                    { name: 'Tooltips', columnSettings:[{ columnKey: "NomeUsuario", allowTooltips: true },{ columnKey: "CnpjCpf", allowTooltips: true },{ columnKey: "Email", allowTooltips: true },{ columnKey: "FoneCelular", allowTooltips: true },{ columnKey: "DataCadastro", allowTooltips: true },{ columnKey: "IdentidadeExterna", allowTooltips: true },{ columnKey: "LxTipoAutenticador", allowTooltips: true },{ columnKey: "IdDispositivo", allowTooltips: true },{ columnKey: "Inativo", allowTooltips: true }] },
                                    { name: 'Resizing' }, 
                                    { name: 'Hiding', 
                                    },
                                   ...
                                   ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                      ...
                                      columnSettings: [{ columnKey: 'DataCadastro', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid === 'function') {vm.OnPropertyChangeDataGrid('scyCadastroUsuarioExterno_dGrid', 'DataCadastro', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: 'LxTipoAutenticador', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('LxTipoAutenticador', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('TipoAutenticador', ''),  textKey: 'name', valueKey: 'id', enableClearButton: true }}, { columnKey: "NomeUsuario" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyCadastroUsuarioExterno_dGrid', 'NomeUsuario', ui.oldValue, ui.value);}}, maxLength: 250 } }, { columnKey: "CnpjCpf" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyCadastroUsuarioExterno_dGrid', 'CnpjCpf', ui.oldValue, ui.value);}}, maxLength: 20 } }, { columnKey: "Email" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyCadastroUsuarioExterno_dGrid', 'Email', ui.oldValue, ui.value);}}, maxLength: 250 } }, { columnKey: "FoneCelular" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyCadastroUsuarioExterno_dGrid', 'FoneCelular', ui.oldValue, ui.value);}}, maxLength: 20 } }, { columnKey: "IdentidadeExterna" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyCadastroUsuarioExterno_dGrid', 'IdentidadeExterna', ui.oldValue, ui.value);}}, maxLength: 250 } }, { columnKey: "IdDispositivo" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyCadastroUsuarioExterno_dGrid', 'IdDispositivo', ui.oldValue, ui.value);}}, maxLength: 256 } }],
                                      ...
                                      editCellStarted: function(evt, ui){
                                          ...
                                          var columns = $('#scyCadastroUsuarioExterno_dGrid').igGridUpdating('option', 'columnSettings');
                                          var currentCol = null;
                                          currentRow = ui.rowID;
                        ...
                        if ((typeof vm.OnDataGridCreated === 'function')){
                            vm.OnDataGridCreated('scyCadastroUsuarioExterno_dGrid');
                        }
                        ...
                        selectionrowselectionchanged = function (evt, ui) {
                            if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                    $(document).undelegate('#scyCadastroUsuarioExterno_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                    ...
                                    $(document).delegate ('#scyCadastroUsuarioExterno_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                }
                                selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                             } 
                        };
                        $(document).delegate('#scyCadastroUsuarioExterno_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        $('#scyCadastroUsuarioExterno_dGrid > tbody tr').live('focus', function(evt) {
                            var grid = $('#scyCadastroUsuarioExterno_dGrid'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                        ...
                        grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                            if (vm.CadastroUsuarioExterno().status() === 'Q') vm.CadastroUsuarioExterno().dataToolbar.viewInfo();
                        });
                    }
                    vm.addDataSource({ key: 'scyCadastroUsuarioExterno_dGrid', name: 'dataView', itemsSource: itemsSource });
                }
