define(['durandal/app', 'pkg_linx-framework-bv-spa/services/AplicacaoContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-framework-bv-spa/viewmodels/CadastroAplicacaoComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroAplicacao', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroAplicacao(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroAplicacao' ]; settings.parentVM.CadastroAplicacao = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.CadastroAplicacao = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('CadastroAplicacao', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroAplicacao');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroAplicacao'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroAplicacao', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroAplicacao', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#CadastroAplicacao_tcTcsAplicacaoTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#CadastroAplicacao_tcTcsAplicacaoTabControl');

    complement.renderCadastroAplicacao_dGridTcsAplicacaoVersaoHistorico(vm);

    complement.renderCadastroAplicacao_dGridTcsAmbiente(vm);

    complement.renderscyCadastroAplicacao_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                               complement.ChangedBrandCadastroAplicacao_dGridTcsAplicacaoVersaoHistorico(vm, decimals, reset);
                           complement.ChangedBrandCadastroAplicacao_dGridTcsAmbiente(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsAplicacao';
    ...
    var visibleColumns = 'UidAplicacao,DescricaoAplicacao,DescricaoAplicativo,EmDesenvolvimento,UrlWorkArea,Url';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsAplicacao', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsAplicacao', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsAplicacao', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsAplicacao');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsAplicacao');
        ...
        return dataContext.getTcsAplicacaoByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsAplicacao'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsAplicacaoByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsAplicacao(getBandeiraRede(), clearComplete);
    ...
    var createTcsAplicacao = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsAplicacao();
    ...
    var createAndNotifyTcsAplicacao = function() {
        var entity = createTcsAplicacao();
        notifyPresentation('');
        return entity;
    };
    
    var createTcsAplicacaoVersaoHistorico = function(parent, noCurrent) {
        dataBind('TcsAplicacaoVersaoHistoricoList', true);
        var entity = dataContext.createTcsAplicacaoVersaoHistorico(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsAplicacaoVersaoHistorico(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsAplicacaoVersaoHistorico = function(parent) {
        var entity = createTcsAplicacaoVersaoHistorico(parent);
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroAplicacao_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroAplicacao_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroAplicacao_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsAplicacaoVersaoHistoricoDataAtualizacao_typeRange: ko.observable('R'), TcsAplicacaoVersaoHistoricoDataAtualizacao_begin: ko.observable(null), TcsAplicacaoVersaoHistoricoDataAtualizacao_end: ko.observable(null), TcsAplicacaoVersaoHistoricoDataAtualizacao_predefFilter: ko.observableArray([]), TcsAplicacaoVersaoHistoricoDataAtualizacao_predefValue: ko.observable(null),
        TcsAplicacaoDescricaoAplicativo: ko.observable(null),
        TcsAmbienteDescricaoAmbiente: ko.observable(null),
        TcsAmbienteNomeEmpresa: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsAplicacaoVersaoHistoricoDataAtualizacao_typeRange('R'); entitySearchRange.TcsAplicacaoVersaoHistoricoDataAtualizacao_begin(null); entitySearchRange.TcsAplicacaoVersaoHistoricoDataAtualizacao_end(null); entitySearchRange.TcsAplicacaoVersaoHistoricoDataAtualizacao_predefFilter([]); entitySearchRange.TcsAplicacaoVersaoHistoricoDataAtualizacao_predefValue(null);
        entitySearchRange.TcsAplicacaoDescricaoAplicativo(null);
        entitySearchRange.TcsAmbienteDescricaoAmbiente(null);
        entitySearchRange.TcsAmbienteNomeEmpresa(null);
    };
    entitySearchRange.has_TcsAplicacaoVersaoHistoricoDataAtualizacao = ko.computed(function(){ return (entitySearchRange.TcsAplicacaoVersaoHistoricoDataAtualizacao_typeRange() == 'R' && (entitySearchRange.TcsAplicacaoVersaoHistoricoDataAtualizacao_begin() != null || entitySearchRange.TcsAplicacaoVersaoHistoricoDataAtualizacao_end() != null) || (entitySearchRange.TcsAplicacaoVersaoHistoricoDataAtualizacao_typeRange() == 'P' && entitySearchRange.TcsAplicacaoVersaoHistoricoDataAtualizacao_predefFilter().length > 0)); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroAplicacao_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: 'TCS_APLICACAO',
            rootDataTypeName: 'TcsAplicacao',
            rootNamespace: 'Linx.Framework.BV.Aplicacao',
            ...
            createAndNotifyTcsAplicacao: createAndNotifyTcsAplicacao,
            createTcsAplicacaoVersaoHistorico: createTcsAplicacaoVersaoHistorico,
            createAndNotifyTcsAplicacaoVersaoHistorico: createAndNotifyTcsAplicacaoVersaoHistorico,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroAplicacao',
            pivots : pivots
        };
