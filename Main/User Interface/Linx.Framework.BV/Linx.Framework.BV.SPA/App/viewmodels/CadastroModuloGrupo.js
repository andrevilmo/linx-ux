...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroModuloGrupo', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroModuloGrupo(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroModuloGrupo' ]; settings.parentVM.CadastroModuloGrupo = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.CadastroModuloGrupo = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('CadastroModuloGrupo', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroModuloGrupo');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroModuloGrupo'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroModuloGrupo', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroModuloGrupo', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#CadastroModuloGrupo_tcTcsModuloGrupoTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#CadastroModuloGrupo_tcTcsModuloGrupoTabControl');

    complement.renderCadastroModuloGrupo_dGridTcsModuloDoGrupoDetalhe(vm);

    complement.renderscyCadastroModuloGrupo_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                               complement.ChangedBrandCadastroModuloGrupo_dGridTcsModuloDoGrupoDetalhe(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsModuloGrupo';
    ...
    var visibleColumns = 'IdGrupoModulo,DescGrupoModulo,DescricaoAplicativo';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsModuloGrupo', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsModuloGrupo', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsModuloGrupo', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsModuloGrupo');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsModuloGrupo');
        ...
        return dataContext.getTcsModuloGrupoByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsModuloGrupo'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsModuloGrupoByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsModuloGrupo(getBandeiraRede(), clearComplete);
    ...
    var createTcsModuloGrupo = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsModuloGrupo();
    ...
    var createAndNotifyTcsModuloGrupo = function() {
        var entity = createTcsModuloGrupo();
        notifyPresentation('');
        return entity;
    };
    
    var createTcsModuloDoGrupoDetalhe = function(parent, noCurrent) {
        dataBind('TcsModuloDoGrupoDetalheList', true);
        var entity = dataContext.createTcsModuloDoGrupoDetalhe(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsModuloDoGrupoDetalhe(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsModuloDoGrupoDetalhe = function(parent) {
        var entity = createTcsModuloDoGrupoDetalhe(parent);
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroModuloGrupo_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroModuloGrupo_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroModuloGrupo_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsModuloGrupoDescricaoAplicativo: ko.observable(null),
        TcsModuloDoGrupoDetalheDescModulo: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsModuloGrupoDescricaoAplicativo(null);
        entitySearchRange.TcsModuloDoGrupoDetalheDescModulo(null);
    };
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroModuloGrupo_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: 'TCS_MODULO_GRUPO',
            rootDataTypeName: 'TcsModuloGrupo',
            ...
            createAndNotifyTcsModuloGrupo: createAndNotifyTcsModuloGrupo,
            createTcsModuloDoGrupoDetalhe: createTcsModuloDoGrupoDetalhe,
            createAndNotifyTcsModuloDoGrupoDetalhe: createAndNotifyTcsModuloDoGrupoDetalhe,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroModuloGrupo',
            pivots : pivots
        };
