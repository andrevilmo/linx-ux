define(['durandal/app', 'pkg_linx-framework-bv-spa/services/ModuloContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-framework-bv-spa/viewmodels/CadastroModuloComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroModulo', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroModulo(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroModulo' ]; settings.parentVM.CadastroModulo = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('CadastroModulo', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroModulo');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroModulo'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroModulo', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroModulo', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#CadastroModulo_tcTcsModuloTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#CadastroModulo_tcTcsModuloTabControl');

    complement.renderCadastroModulo_dGridTcsModuloMenu(vm);

    complement.renderCadastroModulo_dGridTcsTransacaoMenu(vm);

    complement.renderCadastroModulo_dGridTcsModuloDoGrupo(vm);

    complement.renderscyCadastroModulo_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                                   complement.ChangedBrandCadastroModulo_dGridTcsModuloMenu(vm, decimals, reset);
                               complement.ChangedBrandCadastroModulo_dGridTcsTransacaoMenu(vm, decimals, reset);

                               complement.ChangedBrandCadastroModulo_dGridTcsModuloDoGrupo(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsModulo';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        ...
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsModulo', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsModulo', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsModulo', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsModulo');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsModulo');
        ...
        return dataContext.getTcsModuloByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsModulo'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsModuloByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
    ...
    var control = $lx(vm, '#gbTransacoes');

if(control.length){
	
      setVisibility(vm, 'gbTransacoes',action != "Add");
    ;
}
return true;
    }
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsModulo(getBandeiraRede(), clearComplete);
    ...
    var createTcsModulo = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsModulo();
    ...
    var createAndNotifyTcsModulo = function() {
        var entity = createTcsModulo();
        notifyPresentation('');
        return entity;
    };
    
    var createTcsModuloMenu = function(parent, noCurrent) {
        dataBind('TcsModuloMenuList', true);
        var entity = dataContext.createTcsModuloMenu(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
        if (typeof entity.OnAdded == 'function') {
       ...
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsModuloMenu(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsModuloMenu = function(parent) {
        var entity = createTcsModuloMenu(parent);
        notifyPresentation('TcsModuloMenuList');
        return entity;
    };
    
    var createTcsTransacaoMenu = function(parent, noCurrent) {
        dataBind('TcsTransacaoMenuList', true);
        var entity = dataContext.createTcsTransacaoMenu(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
        if (typeof entity.OnAdded == 'function') {
       ...
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsTransacaoMenu(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsTransacaoMenu = function(parent) {
        var entity = createTcsTransacaoMenu(parent);
        notifyPresentation('TcsTransacaoMenuList');
        return entity;
    };
    
    var createTcsModuloDoGrupo = function(parent, noCurrent) {
        dataBind('TcsModuloDoGrupoList', true);
        var entity = dataContext.createTcsModuloDoGrupo(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsModuloDoGrupo(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsModuloDoGrupo = function(parent) {
        var entity = createTcsModuloDoGrupo(parent);
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroModulo_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroModulo_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroModulo_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsModuloMenuDescModuloMenuSuperior: ko.observable(null),
        TcsTransacaoMenuDescTransacao: ko.observable(null),
        TcsModuloDoGrupoDescGrupoModulo: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsModuloMenuDescModuloMenuSuperior(null);
        entitySearchRange.TcsTransacaoMenuDescTransacao(null);
        entitySearchRange.TcsModuloDoGrupoDescGrupoModulo(null);
    };
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroModulo_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: 'TCS_MODULO',
            rootDataTypeName: 'TcsModulo',
            rootNamespace: 'Linx.Framework.BV.Modulo',
            ...
            createAndNotifyTcsModulo: createAndNotifyTcsModulo,
            createTcsModuloMenu: createTcsModuloMenu,
            createAndNotifyTcsModuloMenu: createAndNotifyTcsModuloMenu,
            createTcsTransacaoMenu: createTcsTransacaoMenu,
            createAndNotifyTcsTransacaoMenu: createAndNotifyTcsTransacaoMenu,
            createTcsModuloDoGrupo: createTcsModuloDoGrupo,
            createAndNotifyTcsModuloDoGrupo: createAndNotifyTcsModuloDoGrupo,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroModulo',
            pivots : pivots
        };
