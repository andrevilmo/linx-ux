...
var result = function () {
    ...
    var getDataFeedUrl = function() {
       return getServiceAddress('LinxFrameworkTabelaAutorizacaoOData');
    };
    ...
    var controllerName = 'LinxFrameworkTabelaAutorizacao';
    ...
    var lookUpNames = [];
    var entitylookUps = [];
    entityNames.push('TcsTabelaAutorizacao');
    metadataInfo['TcsTabelaAutorizacao'] = [
        { key: 'ClasseNome', isQbeZero: false, isDomain: false, domainName: '', lookupPropertyName: '', lookupVisibleColumns: '', maxLength: 400, validateMaxLength: true, isPartOfKey: false, headerText: 'Classe Nome', width: '400px', dataType: 'string', format: '', hidden: false, unbound: false, group: null },
        { key: 'DescTabela', isQbeZero: false, isDomain: false, domainName: '', lookupPropertyName: '', lookupVisibleColumns: '', isRequired: true, maxLength: 80, validateMaxLength: true, isPartOfKey: false, headerText: 'Descrição', width: '400px', dataType: 'string', format: '', hidden: false, unbound: false, group: null },
        ...
        { key: 'TabelaAutorizacao', isQbeZero: false, isDomain: false, domainName: '', lookupPropertyName: '', lookupVisibleColumns: '', maxLength: 0, isPartOfKey: false, headerText: 'Tabela Banco Autorização', width: '348px', dataType: 'bool', format: 'checkbox', hidden: false, unbound: false, group: null },
    ...
    dataExportInfo['TcsTabelaAutorizacao'] = [ 
        { name: 'TcsTabelaAutorizacao', canExportMedia: true , canExportReport: true, actionExport: 'LinxFrameworkTabelaAutorizacao/GetTcsTabelaAutorizacaoToExcel', actionReport: 'LinxFrameworkTabelaAutorizacao/GetTcsTabelaAutorizacaoToReportXml', actionFeed: 'LinxFrameworkTabelaAutorizacaoOData/TcsTabelaAutorizacao', actionName: 'LinxFrameworkTabelaAutorizacao/GetTcsTabelaAutorizacaoByEntitySearchNoAssociations', display: 'TcsTabelaAutorizacao',  metaData: function() { return metadataInfo['TcsTabelaAutorizacao']; } }
    ];
    entitylookUps.push('TcsTabelaAutorizacao');
    entitylookUps['TcsTabelaAutorizacao'] = [];
    ...
    // Configure TcsTabelaAutorizacao data type
    metadataStore.addEntityType({
    shortName: "TcsTabelaAutorizacao",
    ...
    ,TabelaAutorizacao: { dataType: DataType.Boolean, isNullable: true, isPartOfKey: false, validators: [ ]  }
    ...
    lookUpProperties['TcsTabelaAutorizacao'] = {};
    var TcsTabelaAutorizacaoInitializer = function (ownerReference, isPOCO) {
       ownerReference.RowDataId = (isPOCO === true ? getNextSequence('TcsTabelaAutorizacao') : ko.observable(getNextSequence('TcsTabelaAutorizacao')));
       ...
       enableChangeTrack = true;
          ...
          ownerReference.refreshData = function(noWait, succeeded) {
             var filterByKey = 'TcsTabelaAutorizacao{' + 'UidTabela#==#G' + getAbsoluteValue(ownerReference.UidTabela).toString() + '}';
             ...
             return dataContext.getTcsTabelaAutorizacaoByEntitySearchNoAssociations(filterByKey, 0, 0, false, true, ownerReference.isPOCO === true, '', querySucceeded);
       ...
       ownerReference.serverDataType['TabelaAutorizacao'] = 'B';
       ownerReference.serverDataType['UidTabela'] = 'G';
       ownerReference.typeName = 'TcsTabelaAutorizacao';
       ...
       ownerReference.UpdateIndependentRelation = function(detailName) {
           ...
           for (var idxR = 0; idxR < cacheElements.length; idxR++) {
               if (typeof cacheElements[idxR].TcsTabelaAutorizacao !== 'function') { return; }
               else  if (cacheElements[idxR].TcsTabelaAutorizacao() != ownerReference) { cacheElements[idxR].TcsTabelaAutorizacao(ownerReference); }
           }
       }
       ...
       ownerReference.namespace = 'Linx.Framework.BV.TabelaAutorizacao';
       ownerReference.myProperties = [ 'ClasseNome','DescTabela','NomeTabela','TabelaAutorizacao','UidTabela' ];
    ...
    metadataStore.registerEntityTypeCtor("TcsTabelaAutorizacao", null, TcsTabelaAutorizacaoInitializer);
    ...
    var clearTcsTabelaAutorizacao = function (idBandeiraRede, complete) {
        clearAll();
        resetSequence('TcsTabelaAutorizacao');
        var refTcsTabelaAutorizacao = manager.createEntity('TcsTabelaAutorizacao', {}, breeze.EntityState.Unchanged);
        if (complete) complete({ results: [ refTcsTabelaAutorizacao ] });
        return true;
    };
    
    var getTcsTabelaAutorizacao = function (predicate, preserveCurrentState, noTracking) {
        if (!preserveCurrentState) clearAll();
        var query = EntityQuery.from('GetTcsTabelaAutorizacao').noTracking(noTracking)
        .orderBy('UidTabela asc')
        ;
    ...
    var getTcsTabelaAutorizacaoByEntitySearchNoAssociations = function (jEntitySearch, skip, take, returnInlineCount, preserveCurrentState, noTracking, orderByDef, succeeded, complete) {
        if (!preserveCurrentState) clearAll();
        ...
        if (hasEntitySearch && jEntitySearch.length > 1000) {
           $.ajax({
               type: 'POST',
               crossDomain: true,
               url: getServiceAddress('LinxFrameworkTabelaAutorizacao/AddTcsTabelaAutorizacaoEntitySearchId'),
               globalError: true,
               headers: managerAuth.getHeaders(),
               ...
               success: function (response) {
                   var query = EntityQuery.from('GetTcsTabelaAutorizacaoByEntitySearchIdNoAssociations').noTracking(noTracking)
                           .orderBy((isNullOrEmpty(orderByDef) ? 'UidTabela asc' : orderByDef))
                               ;
           ...
           var query = EntityQuery.from('GetTcsTabelaAutorizacaoByEntitySearchNoAssociations').noTracking(noTracking)
           .orderBy((isNullOrEmpty(orderByDef) ? 'UidTabela asc' : orderByDef))
           ;
    ...
    // Define that the event name is 'TabelaAutorizacaoContext_DataUpdate'.
    var contextUpdtEvt = 'TabelaAutorizacaoContext_DataUpdate_' + getNewGuid();
    ...
    var saveChanges = function(saveSucceeded, saveFailed, fin, saveNoTRack) {
        if (saveNoTRack === true) {
            ...
            return $.ajax({
               type: 'POST',
               crossDomain: true,
               url: getServiceAddress('LinxFrameworkTabelaAutorizacao/Save' + vm.rootDataTypeName),
               globalError: false,
               contentType: 'application/json',
               async: true,
               cache: false,
    ...
    var createTcsTabelaAutorizacao = function() {
        ...
        enableChangeTrack = false;
        var entity = createEntity('TcsTabelaAutorizacao', { UidTabela: getNewGuid(), TabelaAutorizacao: false });
        entity.setDefaults();
    ...
    var exportReportDataSource = function(complete) {
        $.ajax({
           ...
           crossDomain: true,
           headers: managerAuth.getHeaders(),
           url: getServiceAddress("LinxFrameworkTabelaAutorizacao/GetReportDataSource"),
           success: function (response) {
    ...
    var exportTemplateReport = function(reportPath, complete) {
        $.ajax({
           ...
           crossDomain: true,
           headers: managerAuth.getHeaders(),
           url: getServiceAddress("LinxFrameworkTabelaAutorizacao/GetTemplateReport"),
    ...
    //#endregion getWithBinding
        ...
        var dataContext = {
            ...
            clearTcsTabelaAutorizacao: clearTcsTabelaAutorizacao,
            getTcsTabelaAutorizacao: getTcsTabelaAutorizacao,
            getTcsTabelaAutorizacaoByEntitySearchNoAssociations: getTcsTabelaAutorizacaoByEntitySearchNoAssociations
        };
    loadParameters();
    return dataContext;
