...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroEmpresaAutenticacao', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroEmpresaAutenticacao(), 'Name'))());
        }, function (err) {
    ...
    var activate = function (settings, querystring) {
      ...
      vm.CadastroEmpresaAutenticacao = getVM;
      //loadLanguage();
      changeLanguage();
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroEmpresaAutenticacao' ]; settings.parentVM.CadastroEmpresaAutenticacao = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.CadastroEmpresaAutenticacao = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('CadastroEmpresaAutenticacao', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroEmpresaAutenticacao');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroEmpresaAutenticacao'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroEmpresaAutenticacao', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroEmpresaAutenticacao', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#CadastroEmpresaAutenticacao_tcTcsEmpresaAutenticacaoTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#CadastroEmpresaAutenticacao_tcTcsEmpresaAutenticacaoTabControl');

    complement.renderCadastroEmpresaAutenticacao_dGridTcsEmpresaModulo(vm);

    complement.renderCadastroEmpresaAutenticacao_dGridTcsEmpresaGpecon(vm);

    complement.renderCadastroEmpresaAutenticacao_dGridTcsAmbiente(vm);

    complement.renderCadastroEmpresaAutenticacao_dGridTcsUsuarioAutenticacao(vm);

    complement.renderscyCadastroEmpresaAutenticacao_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                               complement.ChangedBrandCadastroEmpresaAutenticacao_dGridTcsEmpresaModulo(vm, decimals, reset);
                           complement.ChangedBrandCadastroEmpresaAutenticacao_dGridTcsEmpresaGpecon(vm, decimals, reset);

                           complement.ChangedBrandCadastroEmpresaAutenticacao_dGridTcsAmbiente(vm, decimals, reset);

                           complement.ChangedBrandCadastroEmpresaAutenticacao_dGridTcsUsuarioAutenticacao(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsEmpresaAutenticacao';
    ...
    var visibleColumns = 'IdLinx,UidEmpresa,CnpjCpf,NomeEmpresa';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsEmpresaAutenticacao', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsEmpresaAutenticacao', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsEmpresaAutenticacao', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsEmpresaAutenticacao');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsEmpresaAutenticacao');
        ...
        return dataContext.getTcsEmpresaAutenticacaoByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsEmpresaAutenticacao'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsEmpresaAutenticacaoByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsEmpresaAutenticacao(getBandeiraRede(), clearComplete);
    ...
    var createTcsEmpresaAutenticacao = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsEmpresaAutenticacao();
    ...
    var createAndNotifyTcsEmpresaAutenticacao = function() {
        var entity = createTcsEmpresaAutenticacao();
        notifyPresentation('');
        return entity;
    };
    
    var createTcsEmpresaGpecon = function(parent, noCurrent) {
        dataBind('TcsEmpresaGpeconList', true);
        var entity = dataContext.createTcsEmpresaGpecon(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsEmpresaGpecon(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsEmpresaGpecon = function(parent) {
        var entity = createTcsEmpresaGpecon(parent);
        notifyPresentation('TcsEmpresaGpeconList');
        return entity;
    };
    
    var createTcsAmbiente = function(parent, noCurrent) {
        dataBind('TcsAmbienteList', true);
        var entity = dataContext.createTcsAmbiente(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsAmbiente(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsAmbiente = function(parent) {
        var entity = createTcsAmbiente(parent);
        notifyPresentation('TcsAmbienteList');
        return entity;
    };
    
    var createTcsEmpresaModulo = function(parent, noCurrent) {
        dataBind('TcsEmpresaModuloList', true);
        var entity = dataContext.createTcsEmpresaModulo(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsEmpresaModulo(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsEmpresaModulo = function(parent) {
        var entity = createTcsEmpresaModulo(parent);
        notifyPresentation('TcsEmpresaModuloList');
        return entity;
    };
    
    var createTcsUsuarioAutenticacao = function(parent, noCurrent) {
        dataBind('TcsUsuarioAutenticacaoList', true);
        var entity = dataContext.createTcsUsuarioAutenticacao(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsUsuarioAutenticacao(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsUsuarioAutenticacao = function(parent) {
        var entity = createTcsUsuarioAutenticacao(parent);
    ...
    var addNew = function (parentEntity) {
        ...
        goToItem(createTcsEmpresaAutenticacao());
        editInnerUIs();
        showButtonsEditorTemplate();
        dataBind();
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroEmpresaAutenticacao_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroEmpresaAutenticacao_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroEmpresaAutenticacao_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsEmpresaGpeconGrupoEconomico: ko.observable(null),
        TcsEmpresaGpeconIdLinxGpecon: ko.observable(null),
        TcsAmbienteDescricaoAmbiente: ko.observable(null),
        TcsAmbienteDescricaoAplicacao: ko.observable(null),
        TcsEmpresaModuloDescModulo: ko.observable(null),
        TcsUsuarioAutenticacaoNomeUsuario: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsEmpresaGpeconGrupoEconomico(null);
        entitySearchRange.TcsEmpresaGpeconIdLinxGpecon(null);
        entitySearchRange.TcsAmbienteDescricaoAmbiente(null);
        entitySearchRange.TcsAmbienteDescricaoAplicacao(null);
        entitySearchRange.TcsEmpresaModuloDescModulo(null);
        entitySearchRange.TcsUsuarioAutenticacaoNomeUsuario(null);
    };
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroEmpresaAutenticacao_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            dataView: dataView,
            viewName: 'CadastroEmpresaAutenticacao',
            ...
            rootBmTypeName: 'TCS_EMPRESA_AUTENTICACAO',
            rootDataTypeName: 'TcsEmpresaAutenticacao',
            rootNamespace: 'Linx.Framework.BV.Empresa',
            ...
            createAndNotifyTcsEmpresaAutenticacao: createAndNotifyTcsEmpresaAutenticacao,
            createTcsEmpresaGpecon: createTcsEmpresaGpecon,
            createAndNotifyTcsEmpresaGpecon: createAndNotifyTcsEmpresaGpecon,
            ...
            createAndNotifyTcsEmpresaModulo: createAndNotifyTcsEmpresaModulo,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroEmpresaAutenticacao',
            pivots : pivots
        };
