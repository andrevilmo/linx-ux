...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroUsuarioLocalPerfil', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroUsuarioLocalPerfil(), 'Name'))());
        }, function (err) {
    ...
    var activate = function (settings, querystring) {
      ...
      vm.CadastroUsuarioLocalPerfil = getVM;
      //loadLanguage();
      changeLanguage();
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroUsuarioLocalPerfil' ]; settings.parentVM.CadastroUsuarioLocalPerfil = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.CadastroUsuarioLocalPerfil = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('CadastroUsuarioLocalPerfil', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroUsuarioLocalPerfil');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroUsuarioLocalPerfil'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroUsuarioLocalPerfil', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroUsuarioLocalPerfil', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        complement.renderCadastroUsuarioLocalPerfil_dGridExpander_e4f439559d84476db94e9b74fdcf41bd(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                           complement.ChangedBrandCadastroUsuarioLocalPerfil_dGridExpander_e4f439559d84476db94e9b74fdcf41bd(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsUsuarioAutenticacaoPerfil';
    ...
    var visibleColumns = 'NomeUsuario,NomeEmpresa,DescPerfil';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsUsuarioAutenticacaoPerfil', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsUsuarioAutenticacaoPerfil', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsUsuarioAutenticacaoPerfil', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsUsuarioAutenticacaoPerfil');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsUsuarioAutenticacaoPerfil');
        ...
        return dataContext.getTcsUsuarioAutenticacaoPerfilByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsUsuarioAutenticacaoPerfil'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsUsuarioAutenticacaoPerfilByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
    ...
    return {
	idLinx: uiSettings.toolbarSettings.idLinx, 
	idUsuario: uiSettings.toolbarSettings.idUsuario,
	nomeUsuario: uiSettings.toolbarSettings.nomeUsuario,
	nomeEmpresa: uiSettings.toolbarSettings.nomeEmpresa
	};
    }
...
var data = {
	IdLinx : getAbsoluteValue(vm.currentDataItem().IdLinx),
	IdUsuario : getAbsoluteValue(vm.currentDataItem().IdUsuario),
	PerfilList: []
	};

for (var i = 0; i < vm.dataView().count(); i++) {
	var currentItem = vm.dataView()[i];
	data.PerfilList.push ({	DescPerfil: 		getAbsoluteValue(currentItem.DescPerfil), 
							IdLinxPerfil:		getAbsoluteValue(currentItem.IdLinx),
							IdPerfil: 			getAbsoluteValue(currentItem.IdPerfil),
							IdTcsUsuarioPerfil:	getAbsoluteValue(currentItem.IdTcsUsuarioPerfil),
							IdUsuario: 			getAbsoluteValue(currentItem.IdUsuario),
							NomeUsuario:		getAbsoluteValue(currentItem.NomeUsuario)
				});
}
...
vm.showProcessing('Atualizando informações...');
	$.ajax({
		type: 'POST',
		messageUser: "Atualizando informações de Perfil",
		globalError: true,
		url: managerAuth.getServiceAddress('LinxFrameworkUsuarioFranquia/UsuarioPerfilSync'),
...
return true;
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsUsuarioAutenticacaoPerfil(getBandeiraRede(), clearComplete);
    ...
    var createTcsUsuarioAutenticacaoPerfil = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsUsuarioAutenticacaoPerfil();
    ...
    var createAndNotifyTcsUsuarioAutenticacaoPerfil = function() {
        var entity = createTcsUsuarioAutenticacaoPerfil();
        notifyPresentation('');
        return entity;
    };
    ...
    var addNew = function (parentEntity) {
        ...
        goToItem(createTcsUsuarioAutenticacaoPerfil());
        editInnerUIs();
        showButtonsEditorTemplate();
        dataBind();
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroUsuarioLocalPerfil_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroUsuarioLocalPerfil_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroUsuarioLocalPerfil_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsUsuarioAutenticacaoPerfilDescPerfil: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsUsuarioAutenticacaoPerfilDescPerfil(null);
    };
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroUsuarioLocalPerfil_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            dataView: dataView,
            viewName: 'CadastroUsuarioLocalPerfil',
            ...
            rootBmTypeName: '',
            rootDataTypeName: 'TcsUsuarioAutenticacaoPerfil',
            ...
            createAndNotifyTcsUsuarioAutenticacaoPerfil: createAndNotifyTcsUsuarioAutenticacaoPerfil,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroUsuarioLocalPerfil',
            pivots : pivots
        };
