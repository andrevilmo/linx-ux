...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroGrupoParametroAutorizacao', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroGrupoParametroAutorizacao(), 'Name'))());
        }, function (err) {
    ...
    var activate = function (settings, querystring) {
      ...
      vm.CadastroGrupoParametroAutorizacao = getVM;
      //loadLanguage();
      changeLanguage();
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroGrupoParametroAutorizacao' ]; settings.parentVM.CadastroGrupoParametroAutorizacao = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.CadastroGrupoParametroAutorizacao = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('CadastroGrupoParametroAutorizacao', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroGrupoParametroAutorizacao');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroGrupoParametroAutorizacao'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroGrupoParametroAutorizacao', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroGrupoParametroAutorizacao', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#CadastroGrupoParametroAutorizacao_tcTcsParametroGrupoAutorizacaoTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#CadastroGrupoParametroAutorizacao_tcTcsParametroGrupoAutorizacaoTabControl');

    complement.renderCadastroGrupoParametroAutorizacao_dGridTcsParametroAutorizacao(vm);

    complement.renderscyCadastroGrupoParametroAutorizacao_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                               complement.ChangedBrandCadastroGrupoParametroAutorizacao_dGridTcsParametroAutorizacao(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsParametroGrupoAutorizacao';
    ...
    var visibleColumns = 'DescGrupoParametro';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsParametroGrupoAutorizacao', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsParametroGrupoAutorizacao', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsParametroGrupoAutorizacao', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsParametroGrupoAutorizacao');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsParametroGrupoAutorizacao');
        ...
        return dataContext.getTcsParametroGrupoAutorizacaoByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsParametroGrupoAutorizacao'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsParametroGrupoAutorizacaoByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsParametroGrupoAutorizacao(getBandeiraRede(), clearComplete);
    ...
    var createTcsParametroGrupoAutorizacao = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsParametroGrupoAutorizacao();
    ...
    var createAndNotifyTcsParametroGrupoAutorizacao = function() {
        var entity = createTcsParametroGrupoAutorizacao();
        notifyPresentation('');
        return entity;
    };
    
    var createTcsParametroAutorizacaoGrupo = function(parent, noCurrent) {
        dataBind('TcsParametroAutorizacaoGrupoList', true);
        var entity = dataContext.createTcsParametroAutorizacaoGrupo(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsParametroAutorizacaoGrupo(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsParametroAutorizacaoGrupo = function(parent) {
        var entity = createTcsParametroAutorizacaoGrupo(parent);
    ...
    var addNew = function (parentEntity) {
        ...
        goToItem(createTcsParametroGrupoAutorizacao());
        editInnerUIs();
        showButtonsEditorTemplate();
        dataBind();
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroGrupoParametroAutorizacao_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroGrupoParametroAutorizacao_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroGrupoParametroAutorizacao_formViewer')[0];
        if (panel) {
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroGrupoParametroAutorizacao_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            dataView: dataView,
            viewName: 'CadastroGrupoParametroAutorizacao',
            ...
            rootBmTypeName: 'TCS_PARAMETRO_GRUPO_AUTORIZACAO',
            rootDataTypeName: 'TcsParametroGrupoAutorizacao',
            ...
            createAndNotifyTcsParametroGrupoAutorizacao: createAndNotifyTcsParametroGrupoAutorizacao,
            createTcsParametroAutorizacaoGrupo: createTcsParametroAutorizacaoGrupo,
            createAndNotifyTcsParametroAutorizacaoGrupo: createAndNotifyTcsParametroAutorizacaoGrupo,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroGrupoParametroAutorizacao',
            pivots : pivots
        };
