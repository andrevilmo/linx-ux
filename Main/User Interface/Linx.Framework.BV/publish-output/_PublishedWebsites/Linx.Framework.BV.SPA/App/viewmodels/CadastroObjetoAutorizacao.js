define(['durandal/app', 'pkg_linx-framework-bv-spa/services/ObjetoAutorizacaoContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-framework-bv-spa/viewmodels/CadastroObjetoAutorizacaoComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroObjetoAutorizacao', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroObjetoAutorizacao(), 'Name'))());
        }, function (err) {
    ...
    var activate = function (settings, querystring) {
      ...
      vm.CadastroObjetoAutorizacao = getVM;
      //loadLanguage();
      changeLanguage();
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroObjetoAutorizacao' ]; settings.parentVM.CadastroObjetoAutorizacao = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.CadastroObjetoAutorizacao = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('CadastroObjetoAutorizacao', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroObjetoAutorizacao');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroObjetoAutorizacao'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroObjetoAutorizacao', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroObjetoAutorizacao', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#CadastroObjetoAutorizacao_tcTcsObjetoAutorizacaoTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#CadastroObjetoAutorizacao_tcTcsObjetoAutorizacaoTabControl');

    complement.renderCadastroObjetoAutorizacao_dGridTcsTransacaoAutorizacaoChild(vm);

    complement.renderCadastroObjetoAutorizacao_dGridTcsObjetoConteudoAutorizacao(vm);

    complement.renderscyCadastroObjetoAutorizacao_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                               complement.ChangedBrandCadastroObjetoAutorizacao_dGridTcsTransacaoAutorizacaoChild(vm, decimals, reset);
                           complement.ChangedBrandCadastroObjetoAutorizacao_dGridTcsObjetoConteudoAutorizacao(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsObjetoAutorizacao';
    ...
    var visibleColumns = 'ClasseNome,DescObjeto,LxTipoObjeto';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsObjetoAutorizacao', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsObjetoAutorizacao', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsObjetoAutorizacao', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsObjetoAutorizacao');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsObjetoAutorizacao');
        ...
        return dataContext.getTcsObjetoAutorizacaoByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsObjetoAutorizacao'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsObjetoAutorizacaoByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsObjetoAutorizacao(getBandeiraRede(), clearComplete);
    ...
    var createTcsObjetoAutorizacao = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsObjetoAutorizacao();
    ...
    var createAndNotifyTcsObjetoAutorizacao = function() {
        var entity = createTcsObjetoAutorizacao();
        notifyPresentation('');
        return entity;
    };
    
    var createTcsTransacaoAutorizacaoChild = function(parent, noCurrent) {
        dataBind('TcsTransacaoAutorizacaoChildList', true);
        var entity = dataContext.createTcsTransacaoAutorizacaoChild(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsTransacaoAutorizacaoChild(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsTransacaoAutorizacaoChild = function(parent) {
        var entity = createTcsTransacaoAutorizacaoChild(parent);
        notifyPresentation('TcsTransacaoAutorizacaoChildList');
        return entity;
    };
    
    var createTcsObjetoConteudoAutorizacao = function(parent, noCurrent) {
        dataBind('TcsObjetoConteudoAutorizacaoList', true);
        var entity = dataContext.createTcsObjetoConteudoAutorizacao(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsObjetoConteudoAutorizacao(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsObjetoConteudoAutorizacao = function(parent) {
        var entity = createTcsObjetoConteudoAutorizacao(parent);
    ...
    var addNew = function (parentEntity) {
        ...
        goToItem(createTcsObjetoAutorizacao());
        editInnerUIs();
        showButtonsEditorTemplate();
        dataBind();
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroObjetoAutorizacao_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroObjetoAutorizacao_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroObjetoAutorizacao_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsObjetoConteudoAutorizacaoUltAtualizacao_typeRange: ko.observable('R'), TcsObjetoConteudoAutorizacaoUltAtualizacao_begin: ko.observable(null), TcsObjetoConteudoAutorizacaoUltAtualizacao_end: ko.observable(null), TcsObjetoConteudoAutorizacaoUltAtualizacao_predefFilter: ko.observableArray([]), TcsObjetoConteudoAutorizacaoUltAtualizacao_predefValue: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsObjetoConteudoAutorizacaoUltAtualizacao_typeRange('R'); entitySearchRange.TcsObjetoConteudoAutorizacaoUltAtualizacao_begin(null); entitySearchRange.TcsObjetoConteudoAutorizacaoUltAtualizacao_end(null); entitySearchRange.TcsObjetoConteudoAutorizacaoUltAtualizacao_predefFilter([]); entitySearchRange.TcsObjetoConteudoAutorizacaoUltAtualizacao_predefValue(null);
    };
    entitySearchRange.has_TcsObjetoConteudoAutorizacaoUltAtualizacao = ko.computed(function(){ return (entitySearchRange.TcsObjetoConteudoAutorizacaoUltAtualizacao_typeRange() == 'R' && (entitySearchRange.TcsObjetoConteudoAutorizacaoUltAtualizacao_begin() != null || entitySearchRange.TcsObjetoConteudoAutorizacaoUltAtualizacao_end() != null) || (entitySearchRange.TcsObjetoConteudoAutorizacaoUltAtualizacao_typeRange() == 'P' && entitySearchRange.TcsObjetoConteudoAutorizacaoUltAtualizacao_predefFilter().length > 0)); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroObjetoAutorizacao_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            dataView: dataView,
            viewName: 'CadastroObjetoAutorizacao',
            ...
            rootBmTypeName: 'TCS_OBJETO_AUTORIZACAO',
            rootDataTypeName: 'TcsObjetoAutorizacao',
            rootNamespace: 'Linx.Framework.BV.ObjetoAutorizacao',
            ...
            createAndNotifyTcsObjetoAutorizacao: createAndNotifyTcsObjetoAutorizacao,
            createTcsTransacaoAutorizacaoChild: createTcsTransacaoAutorizacaoChild,
            createAndNotifyTcsTransacaoAutorizacaoChild: createAndNotifyTcsTransacaoAutorizacaoChild,
            createTcsObjetoConteudoAutorizacao: createTcsObjetoConteudoAutorizacao,
            createAndNotifyTcsObjetoConteudoAutorizacao: createAndNotifyTcsObjetoConteudoAutorizacao,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroObjetoAutorizacao',
            pivots : pivots
        };
