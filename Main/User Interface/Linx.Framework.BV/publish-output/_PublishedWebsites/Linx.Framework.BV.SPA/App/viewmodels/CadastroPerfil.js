define(['durandal/app', 'pkg_linx-framework-bv-spa/services/PerfilContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-framework-bv-spa/viewmodels/CadastroPerfilComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroPerfil', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroPerfil(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroPerfil' ]; settings.parentVM.CadastroPerfil = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      if ($.inArray('CadastroPerfil', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroPerfil');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroPerfil'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroPerfil', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroPerfil', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#CadastroPerfil_tcTcsPerfilTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#CadastroPerfil_tcTcsPerfilTabControl');

    complement.renderCadastroPerfil_dGridTcsUsuarioPerfil(vm);

    complement.renderCadastroPerfil_dGridTcsPerfilRegraModulo(vm);

    complement.renderCadastroPerfil_dGridTcsPerfilRegraTransacao(vm);

    complement.renderCadastroPerfil_dGridTcsPerfilRegraColuna(vm);

    complement.renderCadastroPerfil_dGridTcsPerfilBandeiraRede(vm);

    complement.renderCadastroPerfil_dGridTcsPerfilLayout(vm);

    complement.renderCadastroPerfil_dGridTcsPerfilFilial(vm);

    complement.renderscyCadastroPerfil_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                               complement.ChangedBrandCadastroPerfil_dGridTcsUsuarioPerfil(vm, decimals, reset);
                           complement.ChangedBrandCadastroPerfil_dGridTcsPerfilRegraModulo(vm, decimals, reset);

                           complement.ChangedBrandCadastroPerfil_dGridTcsPerfilRegraTransacao(vm, decimals, reset);

                           complement.ChangedBrandCadastroPerfil_dGridTcsPerfilRegraColuna(vm, decimals, reset);

                           complement.ChangedBrandCadastroPerfil_dGridTcsPerfilBandeiraRede(vm, decimals, reset);

                           complement.ChangedBrandCadastroPerfil_dGridTcsPerfilLayout(vm, decimals, reset);

                           complement.ChangedBrandCadastroPerfil_dGridTcsPerfilFilial(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsPerfil';
    ...
    var visibleColumns = 'IdPerfil,DescPerfil,Inativo,IdGpeconP,DescGrupoEconomico';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsPerfil', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsPerfil', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsPerfil', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsPerfil');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsPerfil');
        ...
        return dataContext.getTcsPerfilByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsPerfil'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsPerfilByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsPerfil(getBandeiraRede(), clearComplete);
    ...
    var createTcsPerfil = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsPerfil();
    ...
    var createAndNotifyTcsPerfil = function() {
        var entity = createTcsPerfil();
        notifyPresentation('');
        return entity;
    };
    ...
    var createTcsPerfilRegraModulo = function(parent, noCurrent) {
        dataBind('TcsPerfilRegraModuloList', true);
        var entity = dataContext.createTcsPerfilRegraModulo(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsPerfilRegraModulo(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsPerfilRegraModulo = function(parent) {
        var entity = createTcsPerfilRegraModulo(parent);
        notifyPresentation('TcsPerfilRegraModuloList');
        return entity;
    };
    
    var createTcsPerfilRegraColuna = function(parent, noCurrent) {
        dataBind('TcsPerfilRegraColunaList', true);
        var entity = dataContext.createTcsPerfilRegraColuna(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsPerfilRegraColuna(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsPerfilRegraColuna = function(parent) {
        var entity = createTcsPerfilRegraColuna(parent);
        notifyPresentation('TcsPerfilRegraColunaList');
        return entity;
    };
    
    var createTcsPerfilRegraTransacao = function(parent, noCurrent) {
        dataBind('TcsPerfilRegraTransacaoList', true);
        var entity = dataContext.createTcsPerfilRegraTransacao(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsPerfilRegraTransacao(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsPerfilRegraTransacao = function(parent) {
        var entity = createTcsPerfilRegraTransacao(parent);
        notifyPresentation('TcsPerfilRegraTransacaoList');
        return entity;
    };
    
    var createTcsPerfilBandeiraRede = function(parent, noCurrent) {
        dataBind('TcsPerfilBandeiraRedeList', true);
        var entity = dataContext.createTcsPerfilBandeiraRede(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsPerfilBandeiraRede(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsPerfilBandeiraRede = function(parent) {
        var entity = createTcsPerfilBandeiraRede(parent);
        notifyPresentation('TcsPerfilBandeiraRedeList');
        return entity;
    };
    
    var createTcsPerfilLayout = function(parent, noCurrent) {
        dataBind('TcsPerfilLayoutList', true);
        var entity = dataContext.createTcsPerfilLayout(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsPerfilLayout(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsPerfilLayout = function(parent) {
        var entity = createTcsPerfilLayout(parent);
        notifyPresentation('TcsPerfilLayoutList');
        return entity;
    };
    
    var createTcsPerfilFilial = function(parent, noCurrent) {
        dataBind('TcsPerfilFilialList', true);
        var entity = dataContext.createTcsPerfilFilial(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsPerfilFilial(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsPerfilFilial = function(parent) {
        var entity = createTcsPerfilFilial(parent);
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroPerfil_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroPerfil_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroPerfil_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsPerfilDescGrupoEconomico: ko.observable(null),
        TcsPerfilIdGpeconP: ko.observable(null),
        TcsUsuarioPerfilNomeUsuario: ko.observable(null),
        TcsPerfilRegraModuloDescModulo: ko.observable(null),
        TcsPerfilRegraColunaDescTransacao: ko.observable(null),
        TcsPerfilRegraTransacaoDescTransacao: ko.observable(null),
        TcsPerfilBandeiraRedeDescBandeiraRede: ko.observable(null),
        TcsPerfilLayoutDescLayout: ko.observable(null),
        TcsPerfilFilialCodigoFilial: ko.observable(null),
        TcsPerfilFilialNomeFilial: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsPerfilDescGrupoEconomico(null);
        entitySearchRange.TcsPerfilIdGpeconP(null);
        entitySearchRange.TcsUsuarioPerfilNomeUsuario(null);
        entitySearchRange.TcsPerfilRegraModuloDescModulo(null);
        entitySearchRange.TcsPerfilRegraColunaDescTransacao(null);
        entitySearchRange.TcsPerfilRegraTransacaoDescTransacao(null);
        entitySearchRange.TcsPerfilBandeiraRedeDescBandeiraRede(null);
        entitySearchRange.TcsPerfilLayoutDescLayout(null);
        entitySearchRange.TcsPerfilFilialCodigoFilial(null);
        entitySearchRange.TcsPerfilFilialNomeFilial(null);
    };
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroPerfil_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: 'TCS_PERFIL',
            rootDataTypeName: 'TcsPerfil',
            rootNamespace: 'Linx.Framework.BV.Perfil',
            ...
            createAndNotifyTcsPerfil: createAndNotifyTcsPerfil,
            ...
            createAndNotifyTcsPerfilRegraModulo: createAndNotifyTcsPerfilRegraModulo,
            createTcsPerfilRegraColuna: createTcsPerfilRegraColuna,
            createAndNotifyTcsPerfilRegraColuna: createAndNotifyTcsPerfilRegraColuna,
            createTcsPerfilRegraTransacao: createTcsPerfilRegraTransacao,
            createAndNotifyTcsPerfilRegraTransacao: createAndNotifyTcsPerfilRegraTransacao,
            createTcsPerfilBandeiraRede: createTcsPerfilBandeiraRede,
            createAndNotifyTcsPerfilBandeiraRede: createAndNotifyTcsPerfilBandeiraRede,
            createTcsPerfilLayout: createTcsPerfilLayout,
            createAndNotifyTcsPerfilLayout: createAndNotifyTcsPerfilLayout,
            createTcsPerfilFilial: createTcsPerfilFilial,
            createAndNotifyTcsPerfilFilial: createAndNotifyTcsPerfilFilial,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroPerfil',
            pivots : pivots
        };
