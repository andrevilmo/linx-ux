define(['managers/__auth', 'managers/user'], function (managerAuth, managerUser) {
    var complementCtor = function() {
        var complement = {
            isAutomatic: true
            , ChangedBrandCadastroNomeConexao_dGridTcsConexaoDb: function(vm, decimals, reset) {
                var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#CadastroNomeConexao_dGridTcsConexaoDb').data('igGrid'),
                    grdUpd = $('#CadastroNomeConexao_dGridTcsConexaoDb').data('igGridUpdating');
            ...
            , renderCadastroNomeConexao_dGridTcsConexaoDb: function(vm) {
                ...
                $('#CadastroNomeConexao_dGridTcsConexaoDb_headers').live('focus  keydown', function (evt) {
                    var keyCode = window.event ? evt.which : evt.keyCode;
                    if (keyCode === 9) {
                        var cols = $('#CadastroNomeConexao_dGridTcsConexaoDb').igGrid('option', 'columns');
                        var dataView = $('#CadastroNomeConexao_dGridTcsConexaoDb').data('igGrid').dataSource._dataView
                        if (dataView.length === 0) return;
                        var firstRow = dataView[0].RowDataId;
                        clear = vm.status() === 'C';
                        if (vm.status() === 'C')
                            $('#CadastroNomeConexao_dGridTcsConexaoDb').igGridUpdating('startEdit', firstRow, 0, true);
                        else {
                            ...
                            cols.some(function (entry) {
                                if (entry.key !== 'RowDataId' && !entry.hidden) {
                                    if (verifyCanEditCol(entry.key, clear, entity)) {
                                        $('#CadastroNomeConexao_dGridTcsConexaoDb').igGridUpdating('startEdit', firstRow, indexColumn, true);
                                        return true;
                                    }
                                    indexColumn++;
                                }
                ...
                var getVisibleColumns = function(metaDataControl) {
                   ...
                   var visibleColumns = '';
                   if($('#CadastroNomeConexao_dGridTcsConexaoDb').data('igGrid') === undefined) return '';
                   var cols = $('#CadastroNomeConexao_dGridTcsConexaoDb').igGrid('option', 'columns');
                   if (cols) {
                ...
                var isElementHided = function (grid, forceCreating) {
                  if (!grid) grid = $('#CadastroNomeConexao_dGridTcsConexaoDb');
                  return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialogTcsConexaoDb').is(':visible'));
                }
                var refreshData = true;
                var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'CadastroNomeConexao_dGridTcsConexaoDb_container', dataBind: function (commitData, forceCreating) {
                   var grid = $('#CadastroNomeConexao_dGridTcsConexaoDb');
                   ...
                   if (!started) {
                       ...
                       started = true;
                       commitData = false;
                       $('#CadastroNomeConexao_dGridTcsConexaoDb_groupbyarea').addClass('hide');
                   }
                   if (grid.igGridUpdating('isEditing')) {
                   ...
                   if (rows.length > 0) {
                     ...
                     if ($('#dialogTcsConexaoDb').is(':visible')) {
                        ...
                        if (hasPaging.length > 0) {
                           ...
                           $('label#currentNumberTcsConexaoDb').html(current + ' - ' + totalCurrentPage);
                        }
                        else
                           $('label#currentNumberTcsConexaoDb').html(1);
                        $('label#totalNumberTcsConexaoDb').html(totalGrid);
                    }
                   } else {
                       $('label#currentNumberTcsConexaoDb').html(0);
                       $('label#totalNumberTcsConexaoDb').html(0);
                   }
                }};
                var valueGrouBy = -1;
                var deletedIndex = -1;
                function verifyCanEditCol(column, clear, entity){
                    switch(column){
                        case 'NomeConexao': { canEditing = clear || vm.enabledForEditing(); break;}
                    }
                    return canEditing;
                };
                function createDataGrid(grid) {
                    ...
                    vm.gridSaveStates[gridId] = {
                        ...
                        closePopover: function () {
                            $('#CadastroNomeConexao_dGridTcsConexaoDb_LayoutBtn').igPopover('hide');
                        },
                    ...
                    grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
                        ...
                        cellClick: function(evt, ui) {
                             if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                                 ...
                                 if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                                 {
                                      ...
                                      showMultimidia(entity, e, table, key, vm.CadastroNomeConexao());
                                 }
                             }
                             if (typeof vm.OnGridClientClick === 'function') {
                                 entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                 vm.OnGridClientClick('CadastroNomeConexao_dGridTcsConexaoDb', ui.colKey, entity);
                             }
                             if (vm.status() != 'Q') {
                                 var grid = $('#CadastroNomeConexao_dGridTcsConexaoDb');
                        ...
                        columns: [
                            ...
                            { key: 'NomeConexao', headerText: vm.getLayoutHeaderGrid('CadastroNomeConexao_tbNomeConexao'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   }
                        ],
                        features: [
                                    ...
                                    filterDialogOpening: function (evt, ui) {
                                        ...
                                        var dataView = $('#CadastroNomeConexao_dGridTcsConexaoDb').data('igGrid').dataSource;
                                        ...
                                        scriptHtml += '    var newGrid = $("#CadastroNomeConexao_dGridTcsConexaoDb");';
                                        scriptHtml += '    var listFilter = [];';
                                        scriptHtml += '    function hideColumn(){';
                                        scriptHtml += '     if ($("#showHideColumn_CadastroNomeConexao_dGridTcsConexaoDb")["0"].innerHTML.indexOf("Ocultar") >= 0) {';
                                        scriptHtml += '        $("#showHideColumn_CadastroNomeConexao_dGridTcsConexaoDb")["0"].innerHTML = "Mostrar Coluna";';
                                        scriptHtml += '        newGrid.igGridHiding("hideColumn", newCol);';
                                        scriptHtml += '     }';
                                        scriptHtml += '     else{';
                                        scriptHtml += '        $("#showHideColumn_CadastroNomeConexao_dGridTcsConexaoDb")["0"].innerHTML = "Ocultar Coluna";';
                                        ...
                                        scriptHtml += '             $("#showHideColumn_CadastroNomeConexao_dGridTcsConexaoDb")["0"].innerHTML = column[0].hidden ? "Mostrar Coluna" : "Ocultar Coluna"';
                                        scriptHtml += '         }';
                                        scriptHtml += '    }';
                                        ...
                                        scriptHtml += '              <div id="comboFields_CadastroNomeConexao_dGridTcsConexaoDb"></div>';
                                        scriptHtml += '              <script>';
                                        scriptHtml += '                  var columns = newGrid.igGrid("option", "columns");';
                                        scriptHtml += '                  $("#comboFields_CadastroNomeConexao_dGridTcsConexaoDb").igCombo({ dataSource: columns, mode : "dropdown", valueKey: "key", textKey: "headerText", selectionChanging: function (evt, ui) {';
                                        scriptHtml += '                      newCol = ui.items["0"].data.key;';
                                        scriptHtml += '                      updateHideButton()';
                                        scriptHtml += '                  }});';
                                        scriptHtml += '                  $("#comboFields_CadastroNomeConexao_dGridTcsConexaoDb").igCombo("value", newCol);';
                                        scriptHtml += '              </script>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '     <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">';
                                        ...
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <div style="margin-left: 5px; margin-top: 5px">';
                                        scriptHtml += '              <i class="fa fa-eye-slash" aria-hidden="true" style="margin-right: 5px;"></i><a onclick="hideColumn()" style="cursor: pointer" id="showHideColumn_CadastroNomeConexao_dGridTcsConexaoDb">Ocultar Coluna</a>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <br>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '  </div>';
                                    ...
                                    { name: 'Tooltips', columnSettings:[{ columnKey: "NomeConexao", allowTooltips: true }] },
                                    { name: 'Resizing' }, 
                                    { name: 'Hiding', 
                                    },
                                   ...
                                   ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                      ...
                                      columnSettings: [{ columnKey: "NomeConexao" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('CadastroNomeConexao_dGridTcsConexaoDb', 'NomeConexao', ui.oldValue, ui.value);}}, maxLength: 250 } }],
                                      ...
                                      editCellStarted: function(evt, ui){
                                          ...
                                          var columns = $('#CadastroNomeConexao_dGridTcsConexaoDb').igGridUpdating('option', 'columnSettings');
                                          var currentCol = null;
                                          currentRow = ui.rowID;
                        ...
                        if ((typeof vm.OnDataGridCreated === 'function')){
                            vm.OnDataGridCreated('CadastroNomeConexao_dGridTcsConexaoDb');
                        }
                        ...
                        selectionrowselectionchanged = function (evt, ui) {
                            if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                    $(document).undelegate('#CadastroNomeConexao_dGridTcsConexaoDb', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                    ...
                                    $(document).delegate ('#CadastroNomeConexao_dGridTcsConexaoDb', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                }
                                selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                             } 
                        };
                        $(document).delegate('#CadastroNomeConexao_dGridTcsConexaoDb', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        $('#CadastroNomeConexao_dGridTcsConexaoDb > tbody tr').live('focus', function(evt) {
                            var grid = $('#CadastroNomeConexao_dGridTcsConexaoDb'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                    ...
                    vm.addDataSource({ key: 'CadastroNomeConexao_dGridTcsConexaoDb', name: 'dataView', itemsSource: itemsSource });
                }
