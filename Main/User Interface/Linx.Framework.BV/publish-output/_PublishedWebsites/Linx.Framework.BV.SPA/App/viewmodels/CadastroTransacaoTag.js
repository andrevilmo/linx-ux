...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroTransacaoTag', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroTransacaoTag(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroTransacaoTag' ]; settings.parentVM.CadastroTransacaoTag = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.CadastroTransacaoTag = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('CadastroTransacaoTag', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroTransacaoTag');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroTransacaoTag'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroTransacaoTag', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroTransacaoTag', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        complement.renderscyCadastroTransacaoTag_dGrid(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsTransacaoTag';
    ...
    var visibleColumns = 'CodTransacao,DescTransacao';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsTransacaoTag', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsTransacaoTag', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsTransacaoTag', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsTransacaoTag');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsTransacaoTag');
        ...
        return dataContext.getTcsTransacaoTagByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsTransacaoTag'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsTransacaoTagByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
    ...
    $lx(vm,'#CadastroTransacaoTag_tbTagInput').on('itemAdded', function(event) {
	UpdateTagValue();
});

$lx(vm,'#CadastroTransacaoTag_tbTagInput').on('itemRemoved', function(event) {
	UpdateTagValue();
});
...
var tag = $lx(vm,'#CadastroTransacaoTag_tbTagInput').val();
...
$lx(vm,'#CadastroTransacaoTag_tbTagInput').tagsinput('removeAll');

	
if (status() === 'Q'){
	$lx(vm, '#CadastroTransacaoTag_tbTagInput').attr('disabled', 'disabled');
}
else{
	$lx(vm, '#CadastroTransacaoTag_tbTagInput').removeAttr('disabled');
}
    }
...
tag.split(',').forEach(function(e) {
	$lx(vm,'#CadastroTransacaoTag_tbTagInput').tagsinput('add', e);
})
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsTransacaoTag(getBandeiraRede(), clearComplete);
    ...
    var createTcsTransacaoTag = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsTransacaoTag();
    ...
    var createAndNotifyTcsTransacaoTag = function() {
        var entity = createTcsTransacaoTag();
        notifyPresentation('');
        return entity;
    };
    ...
    var addNew = function (parentEntity) {
        ...
        goToItem(createTcsTransacaoTag());
        editInnerUIs();
        showButtonsEditorTemplate();
        dataBind();
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroTransacaoTag_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroTransacaoTag_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroTransacaoTag_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsTransacaoTagDescTransacao: ko.observable(null),
        TcsTransacaoTagCodTransacao: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsTransacaoTagDescTransacao(null);
        entitySearchRange.TcsTransacaoTagCodTransacao(null);
    };
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroTransacaoTag_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: 'TCS_TRANSACAO_TAG',
            rootDataTypeName: 'TcsTransacaoTag',
            rootNamespace: 'Linx.Framework.BV.TransacaoTag',
            ...
            createAndNotifyTcsTransacaoTag: createAndNotifyTcsTransacaoTag,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroTransacaoTag',
            pivots : pivots
        };
