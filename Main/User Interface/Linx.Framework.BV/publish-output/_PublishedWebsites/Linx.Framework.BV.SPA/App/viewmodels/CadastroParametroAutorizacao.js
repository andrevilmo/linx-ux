define(['durandal/app', 'pkg_linx-framework-bv-spa/services/ParametroAutorizacaoContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-framework-bv-spa/viewmodels/CadastroParametroAutorizacaoComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroParametroAutorizacao', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroParametroAutorizacao(), 'Name'))());
        }, function (err) {
    ...
    var activate = function (settings, querystring) {
      ...
      vm.CadastroParametroAutorizacao = getVM;
      //loadLanguage();
      changeLanguage();
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroParametroAutorizacao' ]; settings.parentVM.CadastroParametroAutorizacao = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.CadastroParametroAutorizacao = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('CadastroParametroAutorizacao', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroParametroAutorizacao');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroParametroAutorizacao'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        uiSettings = null;
        isDependentVM(false);
        setSecurity(false, true, true, true, true, true, true, true, true, true);
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroParametroAutorizacao', function (data) {
           if (data && !data.AcessoTotal) {
              setSecurity(false, true, data.PesquisaEspecial, data.Excluir, data.Alterar, data.Layout, true, data.Imprimir, data.Pesquisar, data.Exportar);
           }
        }, logger);
    };
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroParametroAutorizacao', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        complement.renderCadastroParametroAutorizacao_dGridTcsParametroTabelaSelecaoAutorizacao(vm);

    complement.renderscyCadastroParametroAutorizacao_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                           complement.ChangedBrandCadastroParametroAutorizacao_dGridTcsParametroTabelaSelecaoAutorizacao(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsParametroAutorizacao';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsParametroAutorizacao', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsParametroAutorizacao', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsParametroAutorizacao', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsParametroAutorizacao');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsParametroAutorizacao');
        ...
        return dataContext.getTcsParametroAutorizacaoByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsParametroAutorizacao'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsParametroAutorizacaoByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
...
if(control.length){
    ...
    var entity = currentDataItem();

if (!entity){
	return true;
}
if (getAbsoluteValue(entity.NivelAcesso) < getAbsoluteValue(entity.NivelAcessoEdicao)){
	app.showMessage(("Nível de edição do parâmetro deve ser menor ou igual ao nível de visualização!").toString(), 'Alerta', ['Ok']);
	return false;
}
if (vm.currentDataItem().TcsParametroTabelaSelecaoAutorizacaoList().count() > 0 && !getAbsoluteValue(entity.PermiteVariacaoPorEntidade)){
	app.showMessage(("Parâmetro possui variações cadastradas e a opção 'Permite Variação Por Entidade' não está marcada!").toString(), 'Alerta', ['Ok']);
	return false;	
}

var checkHierarquia = $.grep(vm.currentDataItem().TcsParametroTabelaSelecaoAutorizacaoList(), function(element, index) { return isNullOrEmpty(getAbsoluteValue(element.LxParametroHierarquia)) });
if (checkHierarquia.count() > 0){
	app.showMessage(("Parâmetro possui variação sem Hierarquia definida!").toString(), 'Alerta', ['Ok']);
	return false;
}
...
return true;
    ...
    var control = $lx(vm, '#gbvariacoesPermitidas');
if(control.length){
	
      setVisibility(vm, 'gbvariacoesPermitidas',getAbsoluteValue(currentDataItem().PermiteVariacaoPorEntidade));
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsParametroAutorizacao(getBandeiraRede(), clearComplete);
    ...
    var createTcsParametroAutorizacao = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsParametroAutorizacao();
    ...
    var createAndNotifyTcsParametroAutorizacao = function() {
        var entity = createTcsParametroAutorizacao();
        notifyPresentation('');
        return entity;
    };
    
    var createTcsParametroTabelaSelecaoAutorizacao = function(parent, noCurrent) {
        dataBind('TcsParametroTabelaSelecaoAutorizacaoList', true);
        var entity = dataContext.createTcsParametroTabelaSelecaoAutorizacao(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       ...
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsParametroTabelaSelecaoAutorizacao(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsParametroTabelaSelecaoAutorizacao = function(parent) {
        var entity = createTcsParametroTabelaSelecaoAutorizacao(parent);
    ...
    var addNew = function (parentEntity) {
        ...
        goToItem(createTcsParametroAutorizacao());
        editInnerUIs();
        showButtonsEditorTemplate();
        dataBind();
    };
    ...
    var _canRefreshData = true, _canQuickSearch = true, _canAddNew = false, _canClear = true, _canCustomSearch = true, _canDelete = true, _canEdit = true, _canLayout = true, _canNavigate = true, _canPrint = true, _canSearch = true, _canExport = true, _noBusyLoading = false;
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroParametroAutorizacao_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroParametroAutorizacao_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroParametroAutorizacao_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsParametroAutorizacaoNivelAcesso_begin: ko.observable(null), TcsParametroAutorizacaoNivelAcesso_end: ko.observable(null),
        TcsParametroAutorizacaoNivelAcessoEdicao_begin: ko.observable(null), TcsParametroAutorizacaoNivelAcessoEdicao_end: ko.observable(null),
        TcsParametroAutorizacaoDescGrupoParametro: ko.observable(null),
        TcsParametroAutorizacaoDescricaoAplicativo: ko.observable(null),
        TcsParametroAutorizacaoIdTcsAplicativo: ko.observable(null),
        TcsParametroTabelaSelecaoAutorizacaoDescTabela: ko.observable(null),
        TcsParametroTabelaSelecaoAutorizacaoNomeTabela: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsParametroAutorizacaoNivelAcesso_begin(null); entitySearchRange.TcsParametroAutorizacaoNivelAcesso_end(null);
        entitySearchRange.TcsParametroAutorizacaoNivelAcessoEdicao_begin(null); entitySearchRange.TcsParametroAutorizacaoNivelAcessoEdicao_end(null);
        entitySearchRange.TcsParametroAutorizacaoDescGrupoParametro(null);
        entitySearchRange.TcsParametroAutorizacaoDescricaoAplicativo(null);
        entitySearchRange.TcsParametroAutorizacaoIdTcsAplicativo(null);
        entitySearchRange.TcsParametroTabelaSelecaoAutorizacaoDescTabela(null);
        entitySearchRange.TcsParametroTabelaSelecaoAutorizacaoNomeTabela(null);
    };
    entitySearchRange.has_TcsParametroAutorizacaoNivelAcesso = ko.computed(function(){ return (entitySearchRange.TcsParametroAutorizacaoNivelAcesso_begin() != null || entitySearchRange.TcsParametroAutorizacaoNivelAcesso_end() != null); });
    entitySearchRange.has_TcsParametroAutorizacaoNivelAcessoEdicao = ko.computed(function(){ return (entitySearchRange.TcsParametroAutorizacaoNivelAcessoEdicao_begin() != null || entitySearchRange.TcsParametroAutorizacaoNivelAcessoEdicao_end() != null); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroParametroAutorizacao_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            dataView: dataView,
            viewName: 'CadastroParametroAutorizacao',
            ...
            rootBmTypeName: 'TCS_PARAMETRO_AUTORIZACAO',
            rootDataTypeName: 'TcsParametroAutorizacao',
            rootNamespace: 'Linx.Framework.BV.ParametroAutorizacao',
            ...
            createAndNotifyTcsParametroAutorizacao: createAndNotifyTcsParametroAutorizacao,
            createTcsParametroTabelaSelecaoAutorizacao: createTcsParametroTabelaSelecaoAutorizacao,
            createAndNotifyTcsParametroTabelaSelecaoAutorizacao: createAndNotifyTcsParametroTabelaSelecaoAutorizacao,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroParametroAutorizacao',
            pivots : pivots
        };
