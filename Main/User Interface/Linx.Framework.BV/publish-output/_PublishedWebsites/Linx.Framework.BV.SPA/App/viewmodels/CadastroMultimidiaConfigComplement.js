define(['managers/__auth', 'managers/user'], function (managerAuth, managerUser) {
    var complementCtor = function() {
        var complement = {
            isAutomatic: true
            , ChangedBrandCadastroMultimidiaConfig_dGridDocMultimidiaConfig: function(vm, decimals, reset) {
                var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig').data('igGrid'),
                    grdUpd = $('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig').data('igGridUpdating');
            ...
            , renderCadastroMultimidiaConfig_dGridDocMultimidiaConfig: function(vm) {
                ...
                $('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig_headers').live('focus  keydown', function (evt) {
                    var keyCode = window.event ? evt.which : evt.keyCode;
                    if (keyCode === 9) {
                        var cols = $('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig').igGrid('option', 'columns');
                        var dataView = $('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig').data('igGrid').dataSource._dataView
                        if (dataView.length === 0) return;
                        var firstRow = dataView[0].RowDataId;
                        clear = vm.status() === 'C';
                        if (vm.status() === 'C')
                            $('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig').igGridUpdating('startEdit', firstRow, 0, true);
                        else {
                            ...
                            cols.some(function (entry) {
                                if (entry.key !== 'RowDataId' && !entry.hidden) {
                                    if (verifyCanEditCol(entry.key, clear, entity)) {
                                        $('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig').igGridUpdating('startEdit', firstRow, indexColumn, true);
                                        return true;
                                    }
                                    indexColumn++;
                                }
                ...
                var getVisibleColumns = function(metaDataControl) {
                   ...
                   var visibleColumns = '';
                   if($('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig').data('igGrid') === undefined) return '';
                   var cols = $('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig').igGrid('option', 'columns');
                   if (cols) {
                ...
                var isElementHided = function (grid, forceCreating) {
                  if (!grid) grid = $('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig');
                  return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialogDocMultimidiaConfig').is(':visible'));
                }
                var refreshData = true;
                var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'CadastroMultimidiaConfig_dGridDocMultimidiaConfig_container', dataBind: function (commitData, forceCreating) {
                   var grid = $('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig');
                   ...
                   if (!started) {
                       ...
                       started = true;
                       commitData = false;
                       $('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig_groupbyarea').addClass('hide');
                   }
                   if (grid.igGridUpdating('isEditing')) {
                   ...
                   if (rows.length > 0) {
                     ...
                     if ($('#dialogDocMultimidiaConfig').is(':visible')) {
                        ...
                        if (hasPaging.length > 0) {
                           ...
                           $('label#currentNumberDocMultimidiaConfig').html(current + ' - ' + totalCurrentPage);
                        }
                        else
                           $('label#currentNumberDocMultimidiaConfig').html(1);
                        $('label#totalNumberDocMultimidiaConfig').html(totalGrid);
                    }
                   } else {
                       $('label#currentNumberDocMultimidiaConfig').html(0);
                       $('label#totalNumberDocMultimidiaConfig').html(0);
                   }
                }};
                var valueGrouBy = -1;
                var deletedIndex = -1;
                function verifyCanEditCol(column, clear, entity){
                    switch(column){
                        case 'DescricaoAplicativo': { canEditing = clear || (entity && entity.isAdded()); break;}
                        case 'LxUsoMultimidia': { canEditing = clear || (entity && entity.isAdded()); break;}
                        case 'DocLargura': { canEditing = clear || vm.enabledForEditing(); break;}
                        case 'DocAltura': { canEditing = clear || vm.enabledForEditing(); break;}
                        case 'DocDuracao': { canEditing = clear || vm.enabledForEditing(); break;}
                        case 'DocFormatoVisualizacao': { canEditing = clear || vm.enabledForEditing(); break;}
                        case 'DocTamanho': { canEditing = clear || vm.enabledForEditing(); break;}
                    }
                    return canEditing;
                };
                function createDataGrid(grid) {
                    ...
                    vm.gridSaveStates[gridId] = {
                        ...
                        closePopover: function () {
                            $('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig_LayoutBtn').igPopover('hide');
                        },
                    ...
                    grid.igGrid({ height: (getGridHeightSuggested() * 0.75), width: '100%',
                        ...
                        cellClick: function(evt, ui) {
                             if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                                 ...
                                 if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                                 {
                                      ...
                                      showMultimidia(entity, e, table, key, vm.CadastroMultimidiaConfig());
                                 }
                             }
                             if (typeof vm.OnGridClientClick === 'function') {
                                 entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                 vm.OnGridClientClick('CadastroMultimidiaConfig_dGridDocMultimidiaConfig', ui.colKey, entity);
                             }
                             if (vm.status() != 'Q') {
                                 var grid = $('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig');
                        ...
                        columns: [
                            ...
                            { key: 'DescricaoAplicativo', headerText: vm.getLayoutHeaderGrid('CadastroMultimidiaConfig_lUpDescricaoAplicativo'), width: '271px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'LxUsoMultimidia', headerText: vm.getLayoutHeaderGrid('CadastroMultimidiaConfig_cmbLxUsoMultimidia'), width: '200px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null  , formatter: function (val, record) { return  vm.dataDomains.getName('UsoMultimidia', val);} },
                            { key: 'DocLargura', headerText: vm.getLayoutHeaderGrid('CadastroMultimidiaConfig_ntxDocLargura'), width: '130px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                            { key: 'DocAltura', headerText: vm.getLayoutHeaderGrid('CadastroMultimidiaConfig_ntxDocAltura'), width: '130px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                            { key: 'DocDuracao', headerText: vm.getLayoutHeaderGrid('CadastroMultimidiaConfig_tbDocDuracao'), width: '127px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'DocFormatoVisualizacao', headerText: vm.getLayoutHeaderGrid('CadastroMultimidiaConfig_tbDocFormatoVisualizacao'), width: '335px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'DocTamanho', headerText: vm.getLayoutHeaderGrid('CadastroMultimidiaConfig_tbDocTamanho'), width: '127px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   }
                        ],
                        features: [
                                    ...
                                    filterDialogOpening: function (evt, ui) {
                                        ...
                                        var dataView = $('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig').data('igGrid').dataSource;
                                        ...
                                        scriptHtml += '    var newGrid = $("#CadastroMultimidiaConfig_dGridDocMultimidiaConfig");';
                                        scriptHtml += '    var listFilter = [];';
                                        scriptHtml += '    function hideColumn(){';
                                        scriptHtml += '     if ($("#showHideColumn_CadastroMultimidiaConfig_dGridDocMultimidiaConfig")["0"].innerHTML.indexOf("Ocultar") >= 0) {';
                                        scriptHtml += '        $("#showHideColumn_CadastroMultimidiaConfig_dGridDocMultimidiaConfig")["0"].innerHTML = "Mostrar Coluna";';
                                        scriptHtml += '        newGrid.igGridHiding("hideColumn", newCol);';
                                        scriptHtml += '     }';
                                        scriptHtml += '     else{';
                                        scriptHtml += '        $("#showHideColumn_CadastroMultimidiaConfig_dGridDocMultimidiaConfig")["0"].innerHTML = "Ocultar Coluna";';
                                        ...
                                        scriptHtml += '             $("#showHideColumn_CadastroMultimidiaConfig_dGridDocMultimidiaConfig")["0"].innerHTML = column[0].hidden ? "Mostrar Coluna" : "Ocultar Coluna"';
                                        scriptHtml += '         }';
                                        scriptHtml += '    }';
                                        ...
                                        scriptHtml += '              <div id="comboFields_CadastroMultimidiaConfig_dGridDocMultimidiaConfig"></div>';
                                        scriptHtml += '              <script>';
                                        scriptHtml += '                  var columns = newGrid.igGrid("option", "columns");';
                                        scriptHtml += '                  $("#comboFields_CadastroMultimidiaConfig_dGridDocMultimidiaConfig").igCombo({ dataSource: columns, mode : "dropdown", valueKey: "key", textKey: "headerText", selectionChanging: function (evt, ui) {';
                                        scriptHtml += '                      newCol = ui.items["0"].data.key;';
                                        scriptHtml += '                      updateHideButton()';
                                        scriptHtml += '                  }});';
                                        scriptHtml += '                  $("#comboFields_CadastroMultimidiaConfig_dGridDocMultimidiaConfig").igCombo("value", newCol);';
                                        scriptHtml += '              </script>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '     <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">';
                                        ...
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <div style="margin-left: 5px; margin-top: 5px">';
                                        scriptHtml += '              <i class="fa fa-eye-slash" aria-hidden="true" style="margin-right: 5px;"></i><a onclick="hideColumn()" style="cursor: pointer" id="showHideColumn_CadastroMultimidiaConfig_dGridDocMultimidiaConfig">Ocultar Coluna</a>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <br>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '  </div>';
                                    ...
                                    { name: 'Tooltips', columnSettings:[{ columnKey: "DescricaoAplicativo", allowTooltips: false },{ columnKey: "LxUsoMultimidia", allowTooltips: true },{ columnKey: "DocLargura", allowTooltips: true },{ columnKey: "DocAltura", allowTooltips: true },{ columnKey: "DocDuracao", allowTooltips: true },{ columnKey: "DocFormatoVisualizacao", allowTooltips: true },{ columnKey: "DocTamanho", allowTooltips: true }] },
                                    { name: 'Resizing' }, 
                                    { name: 'Hiding', 
                                    },
                                   ...
                                   ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                      ...
                                      columnSettings: [{ columnKey: "DescricaoAplicativo", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpTcsAplicativo", isNullable: false, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, activateAutoComplete: false, autoCompleteMaxResults: 7, validateOnClearState:false, maxValue:null, maxLength: 0, defaultValue: '' } }, { columnKey: 'LxUsoMultimidia', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('LxUsoMultimidia', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('UsoMultimidia', ''),  textKey: 'name', valueKey: 'id', enableClearButton: false }}, { columnKey: "DocLargura" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('CadastroMultimidiaConfig_dGridDocMultimidiaConfig', 'DocLargura', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "DocAltura" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('CadastroMultimidiaConfig_dGridDocMultimidiaConfig', 'DocAltura', ui.oldValue, ui.value);}},  maxLength: 12, maxValue: null, minValue: 0, dataMode: 'int' } }, { columnKey: "DocDuracao" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('CadastroMultimidiaConfig_dGridDocMultimidiaConfig', 'DocDuracao', ui.oldValue, ui.value);}}, maxLength: 10 } }, { columnKey: "DocFormatoVisualizacao" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('CadastroMultimidiaConfig_dGridDocMultimidiaConfig', 'DocFormatoVisualizacao', ui.oldValue, ui.value);}}, maxLength: 10 } }, { columnKey: "DocTamanho" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('CadastroMultimidiaConfig_dGridDocMultimidiaConfig', 'DocTamanho', ui.oldValue, ui.value);}}, maxLength: 10 } }],
                                      ...
                                      editCellStarted: function(evt, ui){
                                          ...
                                          var columns = $('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig').igGridUpdating('option', 'columnSettings');
                                          var currentCol = null;
                                          currentRow = ui.rowID;
                        ...
                        if ((typeof vm.OnDataGridCreated === 'function')){
                            vm.OnDataGridCreated('CadastroMultimidiaConfig_dGridDocMultimidiaConfig');
                        }
                        ...
                        selectionrowselectionchanged = function (evt, ui) {
                            if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                    $(document).undelegate('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                    ...
                                    $(document).delegate ('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                }
                                selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                             } 
                        };
                        $(document).delegate('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        $('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig > tbody tr').live('focus', function(evt) {
                            var grid = $('#CadastroMultimidiaConfig_dGridDocMultimidiaConfig'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                    ...
                    vm.addDataSource({ key: 'CadastroMultimidiaConfig_dGridDocMultimidiaConfig', name: 'dataView', itemsSource: itemsSource });
                }
