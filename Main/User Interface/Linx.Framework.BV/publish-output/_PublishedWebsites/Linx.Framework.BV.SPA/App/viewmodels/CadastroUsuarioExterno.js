...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroUsuarioExterno', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroUsuarioExterno(), 'Name'))());
        }, function (err) {
    ...
    var activate = function (settings, querystring) {
      ...
      vm.CadastroUsuarioExterno = getVM;
      //loadLanguage();
      changeLanguage();
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroUsuarioExterno' ]; settings.parentVM.CadastroUsuarioExterno = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.CadastroUsuarioExterno = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('CadastroUsuarioExterno', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroUsuarioExterno');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroUsuarioExterno'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroUsuarioExterno', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroUsuarioExterno', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#CadastroUsuarioExterno_gbTcsUsuarioExterno').one('shown.bs.collapse', function (e) { vm.notifyInnerElements($(e.currentTarget), true); });

    complement.renderscyCadastroUsuarioExterno_dGrid(vm);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsUsuarioExterno';
    ...
    var visibleColumns = 'NomeUsuario,CnpjCpf,Email,FoneCelular,DataCadastro,IdentidadeExterna,LxTipoAutenticador,IdDispositivo,Inativo';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsUsuarioExterno', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsUsuarioExterno', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsUsuarioExterno', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsUsuarioExterno');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsUsuarioExterno');
        ...
        return dataContext.getTcsUsuarioExternoByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsUsuarioExterno'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsUsuarioExternoByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsUsuarioExterno(getBandeiraRede(), clearComplete);
    ...
    var createTcsUsuarioExterno = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsUsuarioExterno();
    ...
    var createAndNotifyTcsUsuarioExterno = function() {
        var entity = createTcsUsuarioExterno();
        notifyPresentation('');
        return entity;
    };
    ...
    var addNew = function (parentEntity) {
        ...
        goToItem(createTcsUsuarioExterno());
        editInnerUIs();
        showButtonsEditorTemplate();
        dataBind();
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroUsuarioExterno_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroUsuarioExterno_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroUsuarioExterno_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsUsuarioExternoDataCadastro_typeRange: ko.observable('R'), TcsUsuarioExternoDataCadastro_begin: ko.observable(null), TcsUsuarioExternoDataCadastro_end: ko.observable(null), TcsUsuarioExternoDataCadastro_predefFilter: ko.observableArray([]), TcsUsuarioExternoDataCadastro_predefValue: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsUsuarioExternoDataCadastro_typeRange('R'); entitySearchRange.TcsUsuarioExternoDataCadastro_begin(null); entitySearchRange.TcsUsuarioExternoDataCadastro_end(null); entitySearchRange.TcsUsuarioExternoDataCadastro_predefFilter([]); entitySearchRange.TcsUsuarioExternoDataCadastro_predefValue(null);
    };
    entitySearchRange.has_TcsUsuarioExternoDataCadastro = ko.computed(function(){ return (entitySearchRange.TcsUsuarioExternoDataCadastro_typeRange() == 'R' && (entitySearchRange.TcsUsuarioExternoDataCadastro_begin() != null || entitySearchRange.TcsUsuarioExternoDataCadastro_end() != null) || (entitySearchRange.TcsUsuarioExternoDataCadastro_typeRange() == 'P' && entitySearchRange.TcsUsuarioExternoDataCadastro_predefFilter().length > 0)); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroUsuarioExterno_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            dataView: dataView,
            viewName: 'CadastroUsuarioExterno',
            ...
            rootBmTypeName: 'TCS_USUARIO_EXTERNO',
            rootDataTypeName: 'TcsUsuarioExterno',
            rootNamespace: 'Linx.Framework.BV.UsuarioExterno',
            ...
            createTcsUsuarioExterno: createTcsUsuarioExterno,
            createAndNotifyTcsUsuarioExterno: createAndNotifyTcsUsuarioExterno,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroUsuarioExterno',
            pivots : pivots
        };
