define(['durandal/app', 'pkg_linx-framework-bv-spa/services/MultimidiaContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-framework-bv-spa/viewmodels/CadastroMultimidiaConfigComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroMultimidiaConfig', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroMultimidiaConfig(), 'Name'))());
        }, function (err) {
    ...
    var activate = function (settings, querystring) {
      ...
      vm.CadastroMultimidiaConfig = getVM;
      //loadLanguage();
      changeLanguage();
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroMultimidiaConfig' ]; settings.parentVM.CadastroMultimidiaConfig = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.CadastroMultimidiaConfig = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('CadastroMultimidiaConfig', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroMultimidiaConfig');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroMultimidiaConfig'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroMultimidiaConfig', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroMultimidiaConfig', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        complement.renderCadastroMultimidiaConfig_dGridDocMultimidiaConfig(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                       complement.ChangedBrandCadastroMultimidiaConfig_dGridDocMultimidiaConfig(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'DocMultimidiaConfig';
    ...
    var visibleColumns = 'DescricaoAplicativo,LxUsoMultimidia,DocLargura,DocAltura,DocDuracao,DocFormatoVisualizacao,DocTamanho';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'DocMultimidiaConfig', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'DocMultimidiaConfig', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('DocMultimidiaConfig', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('DocMultimidiaConfig');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('DocMultimidiaConfig');
        ...
        return dataContext.getDocMultimidiaConfigByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'DocMultimidiaConfig'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getDocMultimidiaConfigByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearDocMultimidiaConfig(getBandeiraRede(), clearComplete);
    ...
    var createDocMultimidiaConfig = function() {
        dataBind('dataView', true);
        var entity = dataContext.createDocMultimidiaConfig();
    ...
    var createAndNotifyDocMultimidiaConfig = function() {
        var entity = createDocMultimidiaConfig();
        notifyPresentation('');
        return entity;
    };
    ...
    var addNew = function (parentEntity) {
        ...
        goToItem(createDocMultimidiaConfig());
        editInnerUIs();
        showButtonsEditorTemplate();
        dataBind();
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroMultimidiaConfig_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroMultimidiaConfig_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroMultimidiaConfig_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            DocMultimidiaConfigDocAltura_begin: ko.observable(null), DocMultimidiaConfigDocAltura_end: ko.observable(null),
        DocMultimidiaConfigDocLargura_begin: ko.observable(null), DocMultimidiaConfigDocLargura_end: ko.observable(null),
        DocMultimidiaConfigDescricaoAplicativo: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.DocMultimidiaConfigDocAltura_begin(null); entitySearchRange.DocMultimidiaConfigDocAltura_end(null);
        entitySearchRange.DocMultimidiaConfigDocLargura_begin(null); entitySearchRange.DocMultimidiaConfigDocLargura_end(null);
        entitySearchRange.DocMultimidiaConfigDescricaoAplicativo(null);
    };
    entitySearchRange.has_DocMultimidiaConfigDocAltura = ko.computed(function(){ return (entitySearchRange.DocMultimidiaConfigDocAltura_begin() != null || entitySearchRange.DocMultimidiaConfigDocAltura_end() != null); });
    entitySearchRange.has_DocMultimidiaConfigDocLargura = ko.computed(function(){ return (entitySearchRange.DocMultimidiaConfigDocLargura_begin() != null || entitySearchRange.DocMultimidiaConfigDocLargura_end() != null); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroMultimidiaConfig_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            dataView: dataView,
            viewName: 'CadastroMultimidiaConfig',
            ...
            rootBmTypeName: 'DOC_MULTIMIDIA_CONFIG',
            rootDataTypeName: 'DocMultimidiaConfig',
            rootNamespace: 'Linx.Framework.BV.Multimidia',
            ...
            createAndNotifyDocMultimidiaConfig: createAndNotifyDocMultimidiaConfig,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroMultimidiaConfig',
            pivots : pivots
        };
