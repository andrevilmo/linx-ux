define(['durandal/app', 'pkg_linx-framework-bv-spa/services/IndicadorMedidaContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-framework-bv-spa/viewmodels/CadastroIndicadorMedidaComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroIndicadorMedida', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroIndicadorMedida(), 'Name'))());
        }, function (err) {
    ...
    var activate = function (settings, querystring) {
      ...
      vm.CadastroIndicadorMedida = getVM;
      //loadLanguage();
      changeLanguage();
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroIndicadorMedida' ]; settings.parentVM.CadastroIndicadorMedida = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.CadastroIndicadorMedida = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('CadastroIndicadorMedida', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroIndicadorMedida');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroIndicadorMedida'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroIndicadorMedida', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroIndicadorMedida', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#CadastroIndicadorMedida_tcTcsIndicadorMedidaTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#CadastroIndicadorMedida_tcTcsIndicadorMedidaTabControl');

    complement.renderCadastroIndicadorMedida_dGridTcsIndicadorIndice(vm);

    complement.renderscyCadastroIndicadorMedida_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                               complement.ChangedBrandCadastroIndicadorMedida_dGridTcsIndicadorIndice(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsIndicadorMedida';
    ...
    var visibleColumns = 'CodIndicadorMedida,DescIndicadorMedida';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsIndicadorMedida', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsIndicadorMedida', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsIndicadorMedida', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsIndicadorMedida');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsIndicadorMedida');
        ...
        return dataContext.getTcsIndicadorMedidaByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsIndicadorMedida'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsIndicadorMedidaByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsIndicadorMedida(getBandeiraRede(), clearComplete);
    ...
    var createTcsIndicadorMedida = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsIndicadorMedida();
    ...
    var createAndNotifyTcsIndicadorMedida = function() {
        var entity = createTcsIndicadorMedida();
        notifyPresentation('');
        return entity;
    };
    
    var createTcsIndicadorIndice = function(parent, noCurrent) {
        dataBind('TcsIndicadorIndiceList', true);
        var entity = dataContext.createTcsIndicadorIndice(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsIndicadorIndice(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsIndicadorIndice = function(parent) {
        var entity = createTcsIndicadorIndice(parent);
    ...
    var addNew = function (parentEntity) {
        ...
        goToItem(createTcsIndicadorMedida());
        editInnerUIs();
        showButtonsEditorTemplate();
        dataBind();
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroIndicadorMedida_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroIndicadorMedida_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroIndicadorMedida_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsIndicadorIndiceLimiteInferior_begin: ko.observable(null), TcsIndicadorIndiceLimiteInferior_end: ko.observable(null),
        TcsIndicadorIndiceLimiteSuperior_begin: ko.observable(null), TcsIndicadorIndiceLimiteSuperior_end: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsIndicadorIndiceLimiteInferior_begin(null); entitySearchRange.TcsIndicadorIndiceLimiteInferior_end(null);
        entitySearchRange.TcsIndicadorIndiceLimiteSuperior_begin(null); entitySearchRange.TcsIndicadorIndiceLimiteSuperior_end(null);
    };
    entitySearchRange.has_TcsIndicadorIndiceLimiteInferior = ko.computed(function(){ return (entitySearchRange.TcsIndicadorIndiceLimiteInferior_begin() != null || entitySearchRange.TcsIndicadorIndiceLimiteInferior_end() != null); });
    entitySearchRange.has_TcsIndicadorIndiceLimiteSuperior = ko.computed(function(){ return (entitySearchRange.TcsIndicadorIndiceLimiteSuperior_begin() != null || entitySearchRange.TcsIndicadorIndiceLimiteSuperior_end() != null); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroIndicadorMedida_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            dataView: dataView,
            viewName: 'CadastroIndicadorMedida',
            ...
            rootBmTypeName: 'TCS_INDICADOR_MEDIDA',
            rootDataTypeName: 'TcsIndicadorMedida',
            rootNamespace: 'Linx.Framework.BV.IndicadorMedida',
            ...
            createAndNotifyTcsIndicadorMedida: createAndNotifyTcsIndicadorMedida,
            createTcsIndicadorIndice: createTcsIndicadorIndice,
            createAndNotifyTcsIndicadorIndice: createAndNotifyTcsIndicadorIndice,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroIndicadorMedida',
            pivots : pivots
        };
