define(['managers/__auth', 'managers/user'], function (managerAuth, managerUser) {
    var complementCtor = function() {
        var complement = {
            isAutomatic: true
            , ChangedBrandscyConsultaAuditoriaDetalhe_dGrid: function(vm, decimals, reset) {
                var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#scyConsultaAuditoriaDetalhe_dGrid').data('igGrid'),
                    grdUpd = $('#scyConsultaAuditoriaDetalhe_dGrid').data('igGridUpdating');
            ...
            , renderscyConsultaAuditoriaDetalhe_dGrid: function(vm) {
                ...
                $('#scyConsultaAuditoriaDetalhe_dGrid_headers').live('focus  keydown', function (evt) {
                    var keyCode = window.event ? evt.which : evt.keyCode;
                    if (keyCode === 9) {
                        var cols = $('#scyConsultaAuditoriaDetalhe_dGrid').igGrid('option', 'columns');
                        var dataView = $('#scyConsultaAuditoriaDetalhe_dGrid').data('igGrid').dataSource._dataView
                        if (dataView.length === 0) return;
                        var firstRow = dataView[0].RowDataId;
                        clear = vm.status() === 'C';
                        if (vm.status() === 'C')
                            $('#scyConsultaAuditoriaDetalhe_dGrid').igGridUpdating('startEdit', firstRow, 0, true);
                        else {
                            ...
                            cols.some(function (entry) {
                                if (entry.key !== 'RowDataId' && !entry.hidden) {
                                    if (verifyCanEditCol(entry.key, clear, entity)) {
                                        $('#scyConsultaAuditoriaDetalhe_dGrid').igGridUpdating('startEdit', firstRow, indexColumn, true);
                                        return true;
                                    }
                                    indexColumn++;
                                }
                ...
                var getVisibleColumns = function(metaDataControl) {
                   ...
                   var visibleColumns = '';
                   if($('#scyConsultaAuditoriaDetalhe_dGrid').data('igGrid') === undefined) return '';
                   var cols = $('#scyConsultaAuditoriaDetalhe_dGrid').igGrid('option', 'columns');
                   if (cols) {
                ...
                var isElementHided = function (grid, forceCreating) {
                  if (!grid) grid = $('#scyConsultaAuditoriaDetalhe_dGrid');
                  return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialog').is(':visible'));
                }
                var refreshData = true;
                var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'scyConsultaAuditoriaDetalhe_dGrid_container', dataBind: function (commitData, forceCreating) {
                   var grid = $('#scyConsultaAuditoriaDetalhe_dGrid');
                   ...
                   if (!started) {
                       ...
                       started = true;
                       commitData = false;
                       $('#scyConsultaAuditoriaDetalhe_dGrid_groupbyarea').addClass('hide');
                   }
                   if (grid.igGridUpdating('isEditing')) {
                ...
                function verifyCanEditCol(column, clear, entity){
                    switch(column){
                        ...
                        case 'IdAdtAuditoriaItemDetalhe': { canEditing = clear; break;}
                        case 'Propriedade': { canEditing = clear; break;}
                        case 'ValorAntigo': { canEditing = clear; break;}
                        case 'ValorNovo': { canEditing = clear; break;}
                    }
                    return canEditing;
                };
                function createDataGrid(grid) {
                    ...
                    vm.gridSaveStates[gridId] = {
                        ...
                        closePopover: function () {
                            $('#scyConsultaAuditoriaDetalhe_dGrid_LayoutBtn').igPopover('hide');
                        },
                    ...
                    grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
                        ...
                        cellClick: function(evt, ui) {
                             if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                                 ...
                                 if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                                 {
                                      ...
                                      showMultimidia(entity, e, table, key, vm.ConsultaAuditoriaDetalhe());
                                 }
                             }
                             if (typeof vm.OnGridClientClick === 'function') {
                                 entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                 vm.OnGridClientClick('scyConsultaAuditoriaDetalhe_dGrid', ui.colKey, entity);
                             }
                             if (vm.status() != 'Q') {
                                 var grid = $('#scyConsultaAuditoriaDetalhe_dGrid');
                        ...
                        columns: [
                            ...
                            { key: 'IdAdtAuditoriaItem', headerText: vm.getLayoutDisplayName('ConsultaAuditoriaDetalhe_tbIdAdtAuditoriaItem'), width: '309px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: true, unbound: false, group: null   },
                            { key: 'IdAdtAuditoriaItemDetalhe', headerText: vm.getLayoutDisplayName('ConsultaAuditoriaDetalhe_tbIdAdtAuditoriaItemDetalhe'), width: '413px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: true, unbound: false, group: null   },
                            { key: 'Propriedade', headerText: vm.getLayoutDisplayName('ConsultaAuditoriaDetalhe_tbPropriedade'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'ValorAntigo', headerText: vm.getLayoutDisplayName('ConsultaAuditoriaDetalhe_edValorAntigo'), width: '250px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'ValorNovo', headerText: vm.getLayoutDisplayName('ConsultaAuditoriaDetalhe_edValorNovo'), width: '250px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   }
                        ],
                        features: [
                                    ...
                                    filterDialogOpening: function (evt, ui) {
                                        ...
                                        var dataView = $('#scyConsultaAuditoriaDetalhe_dGrid').data('igGrid').dataSource;
                                        ...
                                        scriptHtml += '    var newGrid = $("#scyConsultaAuditoriaDetalhe_dGrid");';
                                        scriptHtml += '    var listFilter = [];';
                                        scriptHtml += '    function hideColumn(){';
                                        scriptHtml += '     if ($("#showHideColumn_scyConsultaAuditoriaDetalhe_dGrid")["0"].innerHTML.indexOf("Ocultar") >= 0) {';
                                        scriptHtml += '        $("#showHideColumn_scyConsultaAuditoriaDetalhe_dGrid")["0"].innerHTML = "Mostrar Coluna";';
                                        scriptHtml += '        newGrid.igGridHiding("hideColumn", newCol);';
                                        scriptHtml += '     }';
                                        scriptHtml += '     else{';
                                        scriptHtml += '        $("#showHideColumn_scyConsultaAuditoriaDetalhe_dGrid")["0"].innerHTML = "Ocultar Coluna";';
                                        ...
                                        scriptHtml += '             $("#showHideColumn_scyConsultaAuditoriaDetalhe_dGrid")["0"].innerHTML = column[0].hidden ? "Mostrar Coluna" : "Ocultar Coluna"';
                                        scriptHtml += '         }';
                                        scriptHtml += '    }';
                                        ...
                                        scriptHtml += '              <div id="comboFields_scyConsultaAuditoriaDetalhe_dGrid"></div>';
                                        scriptHtml += '              <script>';
                                        scriptHtml += '                  var columns = newGrid.igGrid("option", "columns");';
                                        scriptHtml += '                  $("#comboFields_scyConsultaAuditoriaDetalhe_dGrid").igCombo({ dataSource: columns, mode : "dropdown", valueKey: "key", textKey: "headerText", selectionChanging: function (evt, ui) {';
                                        scriptHtml += '                      newCol = ui.items["0"].data.key;';
                                        scriptHtml += '                      updateHideButton()';
                                        scriptHtml += '                  }});';
                                        scriptHtml += '                  $("#comboFields_scyConsultaAuditoriaDetalhe_dGrid").igCombo("value", newCol);';
                                        scriptHtml += '              </script>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '     <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">';
                                        ...
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <div style="margin-left: 5px; margin-top: 5px">';
                                        scriptHtml += '              <i class="fa fa-eye-slash" aria-hidden="true" style="margin-right: 5px;"></i><a onclick="hideColumn()" style="cursor: pointer" id="showHideColumn_scyConsultaAuditoriaDetalhe_dGrid">Ocultar Coluna</a>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <br>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '  </div>';
                                    ...
                                    { name: 'Tooltips', columnSettings:[{ columnKey: "IdAdtAuditoriaItem", allowTooltips: true },{ columnKey: "IdAdtAuditoriaItemDetalhe", allowTooltips: true },{ columnKey: "Propriedade", allowTooltips: true },{ columnKey: "ValorAntigo", allowTooltips: true },{ columnKey: "ValorNovo", allowTooltips: true }] },
                                    { name: 'Resizing' }, 
                                    { name: 'Hiding', 
                                    },
                                   ...
                                   ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                      ...
                                      columnSettings: [{ columnKey: "IdAdtAuditoriaItem" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyConsultaAuditoriaDetalhe_dGrid', 'IdAdtAuditoriaItem', ui.oldValue, ui.value);}}, maxLength: 0 } }, { columnKey: "IdAdtAuditoriaItemDetalhe" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyConsultaAuditoriaDetalhe_dGrid', 'IdAdtAuditoriaItemDetalhe', ui.oldValue, ui.value);}}, maxLength: 0 } }, { columnKey: "Propriedade" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyConsultaAuditoriaDetalhe_dGrid', 'Propriedade', ui.oldValue, ui.value);}}, maxLength: 100 } }, { columnKey: "ValorAntigo" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyConsultaAuditoriaDetalhe_dGrid', 'ValorAntigo', ui.oldValue, ui.value);}}, maxLength: 0 } }, { columnKey: "ValorNovo" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyConsultaAuditoriaDetalhe_dGrid', 'ValorNovo', ui.oldValue, ui.value);}}, maxLength: 0 } }],
                                      ...
                                      editCellStarted: function(evt, ui){
                                          ...
                                          var columns = $('#scyConsultaAuditoriaDetalhe_dGrid').igGridUpdating('option', 'columnSettings');
                                          var currentCol = null;
                                          currentRow = ui.rowID;
                        ...
                        if ((typeof vm.OnDataGridCreated === 'function')){
                            vm.OnDataGridCreated('scyConsultaAuditoriaDetalhe_dGrid');
                        }
                        ...
                        selectionrowselectionchanged = function (evt, ui) {
                            if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                    $(document).undelegate('#scyConsultaAuditoriaDetalhe_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                    ...
                                    $(document).delegate ('#scyConsultaAuditoriaDetalhe_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                }
                                selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                             } 
                        };
                        $(document).delegate('#scyConsultaAuditoriaDetalhe_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        $('#scyConsultaAuditoriaDetalhe_dGrid > tbody tr').live('focus', function(evt) {
                            var grid = $('#scyConsultaAuditoriaDetalhe_dGrid'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                        ...
                        grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                            if (vm.ConsultaAuditoriaDetalhe().status() === 'Q') vm.ConsultaAuditoriaDetalhe().dataToolbar.viewInfo();
                        });
                    }
                    vm.addDataSource({ key: 'scyConsultaAuditoriaDetalhe_dGrid', name: 'dataView', itemsSource: itemsSource });
                }
