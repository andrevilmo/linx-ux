define(['durandal/app', 'pkg_linx-framework-bv-spa/services/AmbienteContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-framework-bv-spa/viewmodels/CadastroAmbienteComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroAmbiente', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroAmbiente(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroAmbiente' ]; settings.parentVM.CadastroAmbiente = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.CadastroAmbiente = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('CadastroAmbiente', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroAmbiente');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroAmbiente'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroAmbiente', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroAmbiente', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#CadastroAmbiente_tcTcsAmbienteTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#CadastroAmbiente_tcTcsAmbienteTabControl');

    complement.renderCadastroAmbiente_dGridTcsAmbienteConexao(vm);

    complement.renderCadastroAmbiente_dGridTcsAmbienteUsuarioAcesso(vm);

    complement.renderCadastroAmbiente_dGridTcsAmbienteServicoExcecao(vm);

    complement.renderscyCadastroAmbiente_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                               complement.ChangedBrandCadastroAmbiente_dGridTcsAmbienteConexao(vm, decimals, reset);
                           complement.ChangedBrandCadastroAmbiente_dGridTcsAmbienteUsuarioAcesso(vm, decimals, reset);

                           complement.ChangedBrandCadastroAmbiente_dGridTcsAmbienteServicoExcecao(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsAmbiente';
    ...
    var visibleColumns = 'IdTcsAmbiente,DescricaoAmbiente,NomeEmpresa,EmDesenvolvimento,DescricaoAplicacao,DescricaoAplicativo,UrlWorkArea';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsAmbiente', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsAmbiente', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsAmbiente', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsAmbiente');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsAmbiente');
        ...
        return dataContext.getTcsAmbienteByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsAmbiente'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsAmbienteByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
    ...
    var control = $lx(vm, '#dGridTcsAmbienteUsuarioAcesso');
...
var visible = (status() == 'C' || getAbsoluteValue(vm.currentDataItem().IdTcsAplicativo) == 3);


      setColGridVisibility(vm, 'dGridTcsAmbienteUsuarioAcesso', 'DescricaoAmbienteRelacionado', visible);
    

      setColGridVisibility(vm, 'dGridTcsAmbienteUsuarioAcesso', 'NomeEmpresaAmbienteRelacionado', visible);
    

      setColGridVisibility(vm, 'dGridTcsAmbienteUsuarioAcesso', 'DescricaoAplicacaoAmbienteRelacionado', visible);
    ...
    if (dataGridName == 'CadastroAmbiente_dGridTcsAmbienteUsuarioAcesso'){
	AdjustControls();
}
    }
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsAmbiente(getBandeiraRede(), clearComplete);
    ...
    var createTcsAmbiente = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsAmbiente();
    ...
    var createAndNotifyTcsAmbiente = function() {
        var entity = createTcsAmbiente();
        notifyPresentation('');
        return entity;
    };
    
    var createTcsAmbienteUsuarioAcesso = function(parent, noCurrent) {
        dataBind('TcsAmbienteUsuarioAcessoList', true);
        var entity = dataContext.createTcsAmbienteUsuarioAcesso(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsAmbienteUsuarioAcesso(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsAmbienteUsuarioAcesso = function(parent) {
        var entity = createTcsAmbienteUsuarioAcesso(parent);
        notifyPresentation('TcsAmbienteUsuarioAcessoList');
        return entity;
    };
    
    var createTcsAmbienteConexao = function(parent, noCurrent) {
        dataBind('TcsAmbienteConexaoList', true);
        var entity = dataContext.createTcsAmbienteConexao(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsAmbienteConexao(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsAmbienteConexao = function(parent) {
        var entity = createTcsAmbienteConexao(parent);
        notifyPresentation('TcsAmbienteConexaoList');
        return entity;
    };
    
    var createTcsAmbienteServicoExcecao = function(parent, noCurrent) {
        dataBind('TcsAmbienteServicoExcecaoList', true);
        var entity = dataContext.createTcsAmbienteServicoExcecao(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsAmbienteServicoExcecao(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsAmbienteServicoExcecao = function(parent) {
        var entity = createTcsAmbienteServicoExcecao(parent);
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroAmbiente_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroAmbiente_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroAmbiente_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsAmbienteDescricaoAplicacao: ko.observable(null),
        TcsAmbienteDescricaoAplicativo: ko.observable(null),
        TcsAmbienteNomeEmpresa: ko.observable(null),
        TcsAmbienteUsuarioAcessoDescricaoAmbienteRelacionado: ko.observable(null),
        TcsAmbienteUsuarioAcessoNomeEmpresa: ko.observable(null),
        TcsAmbienteUsuarioAcessoNomeUsuario: ko.observable(null),
        TcsAmbienteConexaoDescricaoBancoServidor: ko.observable(null),
        TcsAmbienteConexaoNomeConexao: ko.observable(null),
        TcsAmbienteServicoExcecaoNomeServico: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsAmbienteDescricaoAplicacao(null);
        entitySearchRange.TcsAmbienteDescricaoAplicativo(null);
        entitySearchRange.TcsAmbienteNomeEmpresa(null);
        entitySearchRange.TcsAmbienteUsuarioAcessoDescricaoAmbienteRelacionado(null);
        entitySearchRange.TcsAmbienteUsuarioAcessoNomeEmpresa(null);
        entitySearchRange.TcsAmbienteUsuarioAcessoNomeUsuario(null);
        entitySearchRange.TcsAmbienteConexaoDescricaoBancoServidor(null);
        entitySearchRange.TcsAmbienteConexaoNomeConexao(null);
        entitySearchRange.TcsAmbienteServicoExcecaoNomeServico(null);
    };
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroAmbiente_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: 'TCS_AMBIENTE',
            rootDataTypeName: 'TcsAmbiente',
            rootNamespace: 'Linx.Framework.BV.Ambiente',
            ...
            createAndNotifyTcsAmbiente: createAndNotifyTcsAmbiente,
            createTcsAmbienteUsuarioAcesso: createTcsAmbienteUsuarioAcesso,
            createAndNotifyTcsAmbienteUsuarioAcesso: createAndNotifyTcsAmbienteUsuarioAcesso,
            createTcsAmbienteConexao: createTcsAmbienteConexao,
            createAndNotifyTcsAmbienteConexao: createAndNotifyTcsAmbienteConexao,
            createTcsAmbienteServicoExcecao: createTcsAmbienteServicoExcecao,
            createAndNotifyTcsAmbienteServicoExcecao: createAndNotifyTcsAmbienteServicoExcecao,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroAmbiente',
            pivots : pivots
        };
