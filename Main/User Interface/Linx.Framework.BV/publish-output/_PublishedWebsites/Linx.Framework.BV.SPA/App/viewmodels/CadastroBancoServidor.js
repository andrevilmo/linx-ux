define(['durandal/app', 'pkg_linx-framework-bv-spa/services/ConexaoContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-framework-bv-spa/viewmodels/CadastroBancoServidorComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroBancoServidor', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroBancoServidor(), 'Name'))());
        }, function (err) {
    ...
    var activate = function (settings, querystring) {
      ...
      vm.CadastroBancoServidor = getVM;
      //loadLanguage();
      changeLanguage();
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroBancoServidor' ]; settings.parentVM.CadastroBancoServidor = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.CadastroBancoServidor = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('CadastroBancoServidor', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroBancoServidor');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroBancoServidor'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroBancoServidor', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroBancoServidor', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#CadastroBancoServidor_tcTcsBancoServidorTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#CadastroBancoServidor_tcTcsBancoServidorTabControl');

    complement.renderCadastroBancoServidor_dGridTcsAmbienteConexao(vm);

    complement.renderscyCadastroBancoServidor_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                               complement.ChangedBrandCadastroBancoServidor_dGridTcsAmbienteConexao(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsBancoServidor';
    ...
    var visibleColumns = 'DescricaoBancoServidor,NomeServidor,NomeBanco,LxTipoServidor,SequencialInicial,Incremento,StringConexao';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsBancoServidor', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsBancoServidor', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsBancoServidor', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsBancoServidor');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsBancoServidor');
        ...
        return dataContext.getTcsBancoServidorByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsBancoServidor'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsBancoServidorByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsBancoServidor(getBandeiraRede(), clearComplete);
    ...
    var createTcsBancoServidor = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsBancoServidor();
    ...
    var createAndNotifyTcsBancoServidor = function() {
        var entity = createTcsBancoServidor();
        notifyPresentation('');
        return entity;
    };
    ...
    var addNew = function (parentEntity) {
        ...
        goToItem(createTcsBancoServidor());
        editInnerUIs();
        showButtonsEditorTemplate();
        dataBind();
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroBancoServidor_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroBancoServidor_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroBancoServidor_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsBancoServidorIncremento_begin: ko.observable(null), TcsBancoServidorIncremento_end: ko.observable(null),
        TcsBancoServidorSequencialInicial_begin: ko.observable(null), TcsBancoServidorSequencialInicial_end: ko.observable(null),
        TcsAmbienteConexaoDescricaoAmbiente: ko.observable(null),
        TcsAmbienteConexaoIdLinx: ko.observable(null),
        TcsAmbienteConexaoNomeConexao: ko.observable(null),
        TcsAmbienteConexaoNomeEmpresa: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsBancoServidorIncremento_begin(null); entitySearchRange.TcsBancoServidorIncremento_end(null);
        entitySearchRange.TcsBancoServidorSequencialInicial_begin(null); entitySearchRange.TcsBancoServidorSequencialInicial_end(null);
        entitySearchRange.TcsAmbienteConexaoDescricaoAmbiente(null);
        entitySearchRange.TcsAmbienteConexaoIdLinx(null);
        entitySearchRange.TcsAmbienteConexaoNomeConexao(null);
        entitySearchRange.TcsAmbienteConexaoNomeEmpresa(null);
    };
    entitySearchRange.has_TcsBancoServidorIncremento = ko.computed(function(){ return (entitySearchRange.TcsBancoServidorIncremento_begin() != null || entitySearchRange.TcsBancoServidorIncremento_end() != null); });
    entitySearchRange.has_TcsBancoServidorSequencialInicial = ko.computed(function(){ return (entitySearchRange.TcsBancoServidorSequencialInicial_begin() != null || entitySearchRange.TcsBancoServidorSequencialInicial_end() != null); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroBancoServidor_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            dataView: dataView,
            viewName: 'CadastroBancoServidor',
            ...
            rootBmTypeName: 'TCS_BANCO_SERVIDOR',
            rootDataTypeName: 'TcsBancoServidor',
            rootNamespace: 'Linx.Framework.BV.Conexao',
            ...
            createAndNotifyTcsBancoServidor: createAndNotifyTcsBancoServidor,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroBancoServidor',
            pivots : pivots
        };
