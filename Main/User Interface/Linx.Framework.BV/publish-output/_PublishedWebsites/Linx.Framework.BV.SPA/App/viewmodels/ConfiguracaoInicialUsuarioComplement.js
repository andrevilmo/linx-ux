define(['managers/__auth', 'managers/user'], function (managerAuth, managerUser) {
    var complementCtor = function() {
        var complement = {
            isAutomatic: true
            , ChangedBrandConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso: function(vm, decimals, reset) {
                var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso').data('igGrid'),
                    grdUpd = $('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso').data('igGridUpdating');
            ...
            , renderConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso: function(vm) {
                var getDataSource = function() {
                    var source = null;
                    try {
                        source = vm.currentDataItem().TcsUsuarioConfiguracaoAcessoList;
                    }
                    catch (e) { }
                ...
                var dataSourceIsLoaded = function() {
                    var isLoaded = false;
                    try {
                        isLoaded = (vm.currentDataItem().TcsUsuarioConfiguracaoAcessoIsLoaded === true || vm.currentDataItem().TcsUsuarioConfiguracaoAcessoList().length > 0);
                    }
                    catch (e) {
                        isLoaded = true;
                    }
                ...
                $('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso_headers').live('focus  keydown', function (evt) {
                    var keyCode = window.event ? evt.which : evt.keyCode;
                    if (keyCode === 9) {
                        var cols = $('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso').igGrid('option', 'columns');
                        var dataView = $('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso').data('igGrid').dataSource._dataView
                        if (dataView.length === 0) return;
                        var firstRow = dataView[0].RowDataId;
                        clear = vm.status() === 'C';
                        if (vm.status() === 'C')
                            $('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso').igGridUpdating('startEdit', firstRow, 0, true);
                        else {
                            ...
                            cols.some(function (entry) {
                                if (entry.key !== 'RowDataId' && !entry.hidden) {
                                    if (verifyCanEditCol(entry.key, clear, entity)) {
                                        $('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso').igGridUpdating('startEdit', firstRow, indexColumn, true);
                                        return true;
                                    }
                                    indexColumn++;
                                }
                ...
                var getVisibleColumns = function(metaDataControl) {
                   ...
                   var visibleColumns = '';
                   if($('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso').data('igGrid') === undefined) return '';
                   var cols = $('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso').igGrid('option', 'columns');
                   if (cols) {
                ...
                var isElementHided = function (grid, forceCreating) {
                  if (!grid) grid = $('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso');
                  return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialogTcsUsuarioConfiguracaoAcesso').is(':visible'));
                }
                var refreshData = true;
                var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso_container', dataBind: function (commitData, forceCreating) {
                   var grid = $('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso');
                   ...
                   if (!started) {
                       ...
                       started = true;
                       commitData = false;
                       $('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso_groupbyarea').addClass('hide');
                   }
                   if (grid.igGridUpdating('isEditing')) {
                   ...
                   if (execFillDetais) {
                     ...
                     vm.currentDataItem().fillDetails(false, 'TcsUsuarioConfiguracaoAcesso');
                     return;
                   }
                   ...
                   if (rows.length > 0) {
                     ...
                     if (vm.currentDataItem().currentTcsUsuarioConfiguracaoAcesso() != null)
                     {
                       for(var idx = 0; idx < rows.length; idx++)
                       {
                         if (rows[idx].dataset.id == getAbsoluteValue(vm.currentDataItem().currentTcsUsuarioConfiguracaoAcesso().RowDataId))
                         {
                     ...
                     if ($('#dialogTcsUsuarioConfiguracaoAcesso').is(':visible')) {
                        ...
                        if (hasPaging.length > 0) {
                           ...
                           $('label#currentNumberTcsUsuarioConfiguracaoAcesso').html(current + ' - ' + totalCurrentPage);
                        }
                        else
                           $('label#currentNumberTcsUsuarioConfiguracaoAcesso').html(1);
                        $('label#totalNumberTcsUsuarioConfiguracaoAcesso').html(totalGrid);
                    }
                   } else {
                       $('label#currentNumberTcsUsuarioConfiguracaoAcesso').html(0);
                       $('label#totalNumberTcsUsuarioConfiguracaoAcesso').html(0);
                   }
                }};
                var valueGrouBy = -1;
                var deletedIndex = -1;
                function verifyCanEditCol(column, clear, entity){
                    switch(column){
                        case 'Selecionado': { canEditing = clear; break;}
                        ...
                        case 'IndicaAcessoPadrao': { canEditing = clear; break;}
                        case 'IndicaAdministrador': { canEditing = clear; break;}
                        case 'IndicaMultiGpecon': { canEditing = clear; break;}
                        case 'UidAplicacao': { canEditing = clear; break;}
                    }
                    return canEditing;
                };
                function createDataGrid(grid) {
                    ...
                    vm.gridSaveStates[gridId] = {
                        ...
                        closePopover: function () {
                            $('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso_LayoutBtn').igPopover('hide');
                        },
                    ...
                    grid.igGrid({ height: (getGridHeightSuggested() * 0.75), width: '100%',
                        ...
                        cellClick: function(evt, ui) {
                             if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                                 ...
                                 if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                                 {
                                      ...
                                      showMultimidia(entity, e, table, key, vm.ConfiguracaoInicialUsuario());
                                 }
                             }
                             if (typeof vm.OnGridClientClick === 'function') {
                                 entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                 vm.OnGridClientClick('ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso', ui.colKey, entity);
                             }
                             if (vm.status() != 'Q') {
                                 var grid = $('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso');
                        ...
                        columns: [
                            ...
                            { key: 'Selecionado', headerText: vm.getLayoutHeaderGrid('ConfiguracaoInicialUsuario_ckTcsUsuarioConfiguracaoAcesso_Selecionado'), width: '150px', dataType: 'bool', columnCssClass: 'ellipsis', format: 'checkbox', hidden: false, unbound: false, group: null   },
                            { key: 'DescricaoAmbiente', headerText: vm.getLayoutHeaderGrid('ConfiguracaoInicialUsuario_lUpTcsUsuarioConfiguracaoAcesso_DescricaoAmbiente'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'DescricaoAplicacao', headerText: vm.getLayoutHeaderGrid('ConfiguracaoInicialUsuario_lUpTcsUsuarioConfiguracaoAcesso_DescricaoAplicacao'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'DescricaoAplicativo', headerText: vm.getLayoutHeaderGrid('ConfiguracaoInicialUsuario_lUpTcsUsuarioConfiguracaoAcesso_DescricaoAplicativo'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'IndicaAcessoPadrao', headerText: vm.getLayoutHeaderGrid('ConfiguracaoInicialUsuario_ckTcsUsuarioConfiguracaoAcesso_IndicaAcessoPadrao'), width: '296px', dataType: 'bool', columnCssClass: 'ellipsis', format: 'checkbox', hidden: false, unbound: false, group: null   },
                            { key: 'IndicaAdministrador', headerText: vm.getLayoutHeaderGrid('ConfiguracaoInicialUsuario_ckTcsUsuarioConfiguracaoAcesso_IndicaAdministrador'), width: '296px', dataType: 'bool', columnCssClass: 'ellipsis', format: 'checkbox', hidden: false, unbound: false, group: null   },
                            { key: 'IndicaMultiGpecon', headerText: vm.getLayoutHeaderGrid('ConfiguracaoInicialUsuario_ckTcsUsuarioConfiguracaoAcesso_IndicaMultiGpecon'), width: '283px', dataType: 'bool', columnCssClass: 'ellipsis', format: 'checkbox', hidden: false, unbound: false, group: null   },
                            { key: 'UidAplicacao', headerText: vm.getLayoutHeaderGrid('ConfiguracaoInicialUsuario_tbTcsUsuarioConfiguracaoAcesso_UidAplicacao'), width: '250px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   }
                        ],
                        features: [
                                    ...
                                    filterDialogOpening: function (evt, ui) {
                                        ...
                                        var dataView = $('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso').data('igGrid').dataSource;
                                        ...
                                        scriptHtml += '    var newGrid = $("#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso");';
                                        scriptHtml += '    var listFilter = [];';
                                        scriptHtml += '    function hideColumn(){';
                                        scriptHtml += '     if ($("#showHideColumn_ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso")["0"].innerHTML.indexOf("Ocultar") >= 0) {';
                                        scriptHtml += '        $("#showHideColumn_ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso")["0"].innerHTML = "Mostrar Coluna";';
                                        scriptHtml += '        newGrid.igGridHiding("hideColumn", newCol);';
                                        scriptHtml += '     }';
                                        scriptHtml += '     else{';
                                        scriptHtml += '        $("#showHideColumn_ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso")["0"].innerHTML = "Ocultar Coluna";';
                                        ...
                                        scriptHtml += '             $("#showHideColumn_ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso")["0"].innerHTML = column[0].hidden ? "Mostrar Coluna" : "Ocultar Coluna"';
                                        scriptHtml += '         }';
                                        scriptHtml += '    }';
                                        ...
                                        scriptHtml += '              <div id="comboFields_ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso"></div>';
                                        scriptHtml += '              <script>';
                                        scriptHtml += '                  var columns = newGrid.igGrid("option", "columns");';
                                        scriptHtml += '                  $("#comboFields_ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso").igCombo({ dataSource: columns, mode : "dropdown", valueKey: "key", textKey: "headerText", selectionChanging: function (evt, ui) {';
                                        scriptHtml += '                      newCol = ui.items["0"].data.key;';
                                        scriptHtml += '                      updateHideButton()';
                                        scriptHtml += '                  }});';
                                        scriptHtml += '                  $("#comboFields_ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso").igCombo("value", newCol);';
                                        scriptHtml += '              </script>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '     <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">';
                                        ...
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <div style="margin-left: 5px; margin-top: 5px">';
                                        scriptHtml += '              <i class="fa fa-eye-slash" aria-hidden="true" style="margin-right: 5px;"></i><a onclick="hideColumn()" style="cursor: pointer" id="showHideColumn_ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso">Ocultar Coluna</a>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <br>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '  </div>';
                                    ...
                                    { name: 'Tooltips', columnSettings:[{ columnKey: "Selecionado", allowTooltips: true },{ columnKey: "DescricaoAmbiente", allowTooltips: false },{ columnKey: "DescricaoAplicacao", allowTooltips: false },{ columnKey: "DescricaoAplicativo", allowTooltips: false },{ columnKey: "IndicaAcessoPadrao", allowTooltips: true },{ columnKey: "IndicaAdministrador", allowTooltips: true },{ columnKey: "IndicaMultiGpecon", allowTooltips: true },{ columnKey: "UidAplicacao", allowTooltips: true }] },
                                    { name: 'Resizing' }, 
                                    { name: 'Hiding', 
                                    },
                                   ...
                                   ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                      ...
                                      columnSettings: [{ columnKey: "DescricaoAmbiente", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "", isNullable: false, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, activateAutoComplete: false, autoCompleteMaxResults: 7, validateOnClearState:false, maxValue:null, maxLength: 250, defaultValue: '' } }, { columnKey: "DescricaoAplicacao", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "", isNullable: false, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, activateAutoComplete: false, autoCompleteMaxResults: 7, validateOnClearState:false, maxValue:null, maxLength: 60, defaultValue: '' } }, { columnKey: "DescricaoAplicativo", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, activateAutoComplete: false, autoCompleteMaxResults: 7, validateOnClearState:false, maxValue:null, maxLength: 250, defaultValue: '' } }, { columnKey: "UidAplicacao" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso', 'UidAplicacao', ui.oldValue, ui.value);}}, maxLength: 36 } }],
                                      ...
                                      editCellStarted: function(evt, ui){
                                          ...
                                          var columns = $('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso').igGridUpdating('option', 'columnSettings');
                                          var currentCol = null;
                                          currentRow = ui.rowID;
                                          ...
                                          if (currentCol != null && currentCol.hasOwnProperty('editorType') && currentCol.editorType === 'combo') {
                                             ...
                                             if (lookUpName != null) {
                                                 ...
                                                 if (lstRefreshDados.length === 0)
                                                     vm.dataCombo.fillDataCombos(lookUpName, ui.columnKey, vm.currentDataItem().currentTcsUsuarioConfiguracaoAcesso(), function (result) {
                        ...
                        if ((typeof vm.OnDataGridCreated === 'function')){
                            vm.OnDataGridCreated('ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso');
                        }
                        ...
                        selectionrowselectionchanged = function (evt, ui) {
                            if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                    $(document).undelegate('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                    ...
                                    $(document).delegate ('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                }
                                selectGridCurrentItem(vm.goToKey, 'RowDataId', ui, vm.currentDataItem().currentTcsUsuarioConfiguracaoAcesso, getDataSource()); 
                             } 
                        };
                        $(document).delegate('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        $('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso > tbody tr').live('focus', function(evt) {
                            var grid = $('#ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                    ...
                    vm.addDataSource({ key: 'ConfiguracaoInicialUsuario_dGridTcsUsuarioConfiguracaoAcesso', name: 'TcsUsuarioConfiguracaoAcessoList', itemsSource: itemsSource });
                }
            
            
            , ChangedBrandscyConfiguracaoInicialUsuario_dGrid: function(vm, decimals, reset) {
                var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#scyConfiguracaoInicialUsuario_dGrid').data('igGrid'),
                    grdUpd = $('#scyConfiguracaoInicialUsuario_dGrid').data('igGridUpdating');
                if(isNull(grd) || isNull(grdUpd)) return;
                for (i = 0; i < grd.options.columns.length; i++) {
                }
                for (i = 0; i < grdUpd.options.columnSettings.length; i++) {
            ...
            , renderscyConfiguracaoInicialUsuario_dGrid: function(vm) {
                var getDataSource = function() {
                    var source = null;
                    try {
                        source = vm.dataView;
                ...
                $('#scyConfiguracaoInicialUsuario_dGrid_headers').live('focus  keydown', function (evt) {
                    var keyCode = window.event ? evt.which : evt.keyCode;
                    if (keyCode === 9) {
                        var cols = $('#scyConfiguracaoInicialUsuario_dGrid').igGrid('option', 'columns');
                        var dataView = $('#scyConfiguracaoInicialUsuario_dGrid').data('igGrid').dataSource._dataView
                        if (dataView.length === 0) return;
                        var firstRow = dataView[0].RowDataId;
                        clear = vm.status() === 'C';
                        if (vm.status() === 'C')
                            $('#scyConfiguracaoInicialUsuario_dGrid').igGridUpdating('startEdit', firstRow, 0, true);
                        else {
                            var entity = findElementByKey(getDataSource(), 'RowDataId', firstRow);
                            var indexColumn = 0;
                            cols.some(function (entry) {
                                if (entry.key !== 'RowDataId' && !entry.hidden) {
                                    if (verifyCanEditCol(entry.key, clear, entity)) {
                                        $('#scyConfiguracaoInicialUsuario_dGrid').igGridUpdating('startEdit', firstRow, indexColumn, true);
                                        return true;
                                    }
                                    indexColumn++;
                                }
                ...
                var getVisibleColumns = function(metaDataControl) {
                   if (metaDataControl) return '';
                   var visibleColumns = '';
                   if($('#scyConfiguracaoInicialUsuario_dGrid').data('igGrid') === undefined) return '';
                   var cols = $('#scyConfiguracaoInicialUsuario_dGrid').igGrid('option', 'columns');
                   if (cols) {
                     for (var idx = 0; idx < cols.length; idx++) {
                         if (cols[idx].hidden !== true) visibleColumns += (visibleColumns === '' ? '' : ',') + cols[idx].key;
                ...
                var isElementHided = function (grid, forceCreating) {
                  if (!grid) grid = $('#scyConfiguracaoInicialUsuario_dGrid');
                  return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialog').is(':visible'));
                }
                var refreshData = true;
                var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'scyConfiguracaoInicialUsuario_dGrid_container', dataBind: function (commitData, forceCreating) {
                   var grid = $('#scyConfiguracaoInicialUsuario_dGrid');
                   if (started && typeof grid.data('igGridUpdating') === 'undefined') { started = false; }
                   if (commitData && started) {
                       if (grid.igGridUpdating('isEditing')) {
                           grid.igGrid('commit');
                   ...
                   if (!started) {
                       ...
                       started = true;
                       commitData = false;
                       $('#scyConfiguracaoInicialUsuario_dGrid_groupbyarea').addClass('hide');
                   }
                   if (grid.igGridUpdating('isEditing')) {
                        grid.igGridUpdating('endEdit', true);
                   }
                ...
                function createDataGrid(grid) {
                    ...
                    vm.gridSaveStates[gridId] = {
                        ...
                        closePopover: function () {
                            $('#scyConfiguracaoInicialUsuario_dGrid_LayoutBtn').igPopover('hide');
                        },
                        applyLayout: function (layoutInfo) {
                            var _this = this;
                            if (isNull(layoutInfo) && (!_this.currentLayout() || _this.currentLayout().Id === 0)) {
                    ...
                    grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
                        ...
                        cellClick: function(evt, ui) {
                             ...
                             if (typeof vm.OnGridClientClick === 'function') {
                                 entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                 vm.OnGridClientClick('scyConfiguracaoInicialUsuario_dGrid', ui.colKey, entity);
                             }
                             if (vm.status() != 'Q') {
                                 var grid = $('#scyConfiguracaoInicialUsuario_dGrid');
                                 var isEditing = grid.igGridUpdating('isEditing');
                        ...
                        columns: [
                            ...
                            { key: 'IdLinx', headerText: vm.getLayoutDisplayName('ConfiguracaoInicialUsuario_lUpIdLinx'), width: '271px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'NomeEmpresa', headerText: vm.getLayoutDisplayName('ConfiguracaoInicialUsuario_lUpNomeEmpresa'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'NomeUsuario', headerText: vm.getLayoutDisplayName('ConfiguracaoInicialUsuario_lUpNomeUsuario'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'NomeAutenticacao', headerText: vm.getLayoutDisplayName('ConfiguracaoInicialUsuario_tbNomeAutenticacao'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   }
                        ],
                        features: [
                                    { name: 'Sorting', type: 'local', caseSensitive: false, unsortedColumnTooltip: '', sortedColumnTooltip: '',
                                      columnSorting: function (evt, ui) { }
                                    ...
                                    filterDialogOpening: function (evt, ui) {
                                        ...
                                        var dataView = $('#scyConfiguracaoInicialUsuario_dGrid').data('igGrid').dataSource;
                                        if (dataView.settings.filtering.expressions.length <= 0)
                                            dataView._filteredData = [];
                                        ...
                                        scriptHtml += '  <script>';
                                        scriptHtml += '    var newCol = "' + col + '";';
                                        scriptHtml += '    var newGrid = $("#scyConfiguracaoInicialUsuario_dGrid");';
                                        scriptHtml += '    var listFilter = [];';
                                        scriptHtml += '    function hideColumn(){';
                                        scriptHtml += '     if ($("#showHideColumn_scyConfiguracaoInicialUsuario_dGrid")["0"].innerHTML.indexOf("Ocultar") >= 0) {';
                                        scriptHtml += '        $("#showHideColumn_scyConfiguracaoInicialUsuario_dGrid")["0"].innerHTML = "Mostrar Coluna";';
                                        scriptHtml += '        newGrid.igGridHiding("hideColumn", newCol);';
                                        scriptHtml += '     }';
                                        scriptHtml += '     else{';
                                        scriptHtml += '        $("#showHideColumn_scyConfiguracaoInicialUsuario_dGrid")["0"].innerHTML = "Ocultar Coluna";';
                                        scriptHtml += '        newGrid.igGridHiding("showColumn", newCol);';
                                        scriptHtml += '     }';
                                        scriptHtml += '    }';
                                        scriptHtml += '    function updateHideButton(){';
                                        scriptHtml += '         var column = $.grep(newGrid.igGrid("option", "columns"), function (element, index) { return element.key == newCol });';
                                        scriptHtml += '         if (column.length > 0){';
                                        scriptHtml += '             $("#showHideColumn_scyConfiguracaoInicialUsuario_dGrid")["0"].innerHTML = column[0].hidden ? "Mostrar Coluna" : "Ocultar Coluna"';
                                        scriptHtml += '         }';
                                        scriptHtml += '    }';
                                        scriptHtml += '    function orderColumn(dir){';
                                        scriptHtml += '      if(dir == 1){dir = "asc"} else{dir = "desc"}';
                                        ...
                                        scriptHtml += '          <div  style="margin-left: 5px" >';
                                        scriptHtml += '              <div>Propriedade:</div>';
                                        scriptHtml += '              <div id="comboFields_scyConfiguracaoInicialUsuario_dGrid"></div>';
                                        scriptHtml += '              <script>';
                                        scriptHtml += '                  var columns = newGrid.igGrid("option", "columns");';
                                        scriptHtml += '                  $("#comboFields_scyConfiguracaoInicialUsuario_dGrid").igCombo({ dataSource: columns, mode : "dropdown", valueKey: "key", textKey: "headerText", selectionChanging: function (evt, ui) {';
                                        scriptHtml += '                      newCol = ui.items["0"].data.key;';
                                        scriptHtml += '                      updateHideButton()';
                                        scriptHtml += '                  }});';
                                        scriptHtml += '                  $("#comboFields_scyConfiguracaoInicialUsuario_dGrid").igCombo("value", newCol);';
                                        scriptHtml += '              </script>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '     <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">';
                                        ...
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <div style="margin-left: 5px; margin-top: 5px">';
                                        scriptHtml += '              <i class="fa fa-eye-slash" aria-hidden="true" style="margin-right: 5px;"></i><a onclick="hideColumn()" style="cursor: pointer" id="showHideColumn_scyConfiguracaoInicialUsuario_dGrid">Ocultar Coluna</a>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <br>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '  </div>';
                                    ...
                                    { name: 'Tooltips', columnSettings:[{ columnKey: "IdLinx", allowTooltips: false },{ columnKey: "NomeEmpresa", allowTooltips: false },{ columnKey: "NomeUsuario", allowTooltips: false },{ columnKey: "NomeAutenticacao", allowTooltips: true }] },
                                    { name: 'Resizing' }, 
                                    { name: 'Hiding', 
                                    },
                                    { name: 'MultiColumnHeaders' }
                                   ...
                                   ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                      ...
                                      showReadonlyEditors: false,
                                      showDoneCancelButtons: false,
                                      columnSettings: [{ columnKey: "IdLinx", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpTcsEmpresaAutenticacao", isNullable: false, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:false, activateAutoComplete: false, autoCompleteMaxResults: 7, validateOnClearState:false, maxValue:2147483647, maxLength: 0, defaultValue: 0 } }, { columnKey: "NomeEmpresa", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpTcsEmpresaAutenticacao", isNullable: false, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:false, activateAutoComplete: false, autoCompleteMaxResults: 7, validateOnClearState:false, maxValue:null, maxLength: 250, defaultValue: '' } }, { columnKey: "NomeUsuario", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpTcsUsuarioAutenticacao", isNullable: false, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:false, activateAutoComplete: false, autoCompleteMaxResults: 7, validateOnClearState:false, maxValue:null, maxLength: 250, defaultValue: '' } }, { columnKey: "NomeAutenticacao" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyConfiguracaoInicialUsuario_dGrid', 'NomeAutenticacao', ui.oldValue, ui.value);}}, maxLength: 250 } }],
                                      rowDeleting: function (evt, ui) {
                                          deletedIndex = ui.element.context.rowIndex;
                                          var entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowID);
                                          if (entity) {
                                      ...
                                      editCellStarted: function(evt, ui){
                                          var lstRefreshDados = null;
                                          var columns = $('#scyConfiguracaoInicialUsuario_dGrid').igGridUpdating('option', 'columnSettings');
                                          var currentCol = null;
                                          currentRow = ui.rowID;
                                          columns.forEach(function (entry, index) {
                                             if (entry.columnKey === ui.columnKey) currentCol = entry;
                        ...
                        if ((typeof vm.OnDataGridCreated === 'function')){
                            vm.OnDataGridCreated('scyConfiguracaoInicialUsuario_dGrid');
                        }
                        var selectionrowselectionchanged = null, selectedRowId = -1;
                        selectionrowselectionchanged = function (evt, ui) {
                            if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                    $(document).undelegate('#scyConfiguracaoInicialUsuario_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                    ui.owner.clearSelection();
                                    ui.owner.selectRow(ui.row.index);
                                    if (vm.status() === 'Q'){
                                        var gridCell = ui.owner.grid;
                                    ...
                                    selectedRowId = ui.row.id;
                                    $(document).delegate ('#scyConfiguracaoInicialUsuario_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                }
                                selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                             } 
                        };
                        $(document).delegate('#scyConfiguracaoInicialUsuario_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        $('#scyConfiguracaoInicialUsuario_dGrid > tbody tr').live('focus', function(evt) {
                            var grid = $('#scyConfiguracaoInicialUsuario_dGrid'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                            var selectedRows = grid.igGridSelection('option', 'multipleSelection') ? grid.igGridSelection('selectedRows') : [grid.igGridSelection('selectedRow')];
                            if (selectedRowId === id) return;
                            selectedRowId = id;
                            grid.igGridSelection('selectRowById', id);
                        ...
                        grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                            if (vm.ConfiguracaoInicialUsuario().status() === 'Q') vm.ConfiguracaoInicialUsuario().dataToolbar.viewInfo();
                        });
                    }
                    vm.addDataSource({ key: 'scyConfiguracaoInicialUsuario_dGrid', name: 'dataView', itemsSource: itemsSource });
                }
