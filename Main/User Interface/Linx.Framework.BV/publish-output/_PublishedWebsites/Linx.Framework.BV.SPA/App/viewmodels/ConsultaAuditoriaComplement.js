define(['managers/__auth', 'managers/user'], function (managerAuth, managerUser) {
    var complementCtor = function() {
        var complement = {
            isAutomatic: true
            , ChangedBrandConsultaAuditoria_dGridAdtAuditoriaItem: function(vm, decimals, reset) {
                var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#ConsultaAuditoria_dGridAdtAuditoriaItem').data('igGrid'),
                    grdUpd = $('#ConsultaAuditoria_dGridAdtAuditoriaItem').data('igGridUpdating');
            ...
            , renderConsultaAuditoria_dGridAdtAuditoriaItem: function(vm) {
                var getDataSource = function() {
                    var source = null;
                    try {
                        source = vm.currentDataItem().AdtAuditoriaItemList;
                    }
                    catch (e) { }
                ...
                var dataSourceIsLoaded = function() {
                    var isLoaded = false;
                    try {
                        isLoaded = (vm.currentDataItem().AdtAuditoriaItemIsLoaded === true || vm.currentDataItem().AdtAuditoriaItemList().length > 0);
                    }
                    catch (e) {
                        isLoaded = true;
                    }
                ...
                $('#ConsultaAuditoria_dGridAdtAuditoriaItem_headers').live('focus  keydown', function (evt) {
                    var keyCode = window.event ? evt.which : evt.keyCode;
                    if (keyCode === 9) {
                        var cols = $('#ConsultaAuditoria_dGridAdtAuditoriaItem').igGrid('option', 'columns');
                        var dataView = $('#ConsultaAuditoria_dGridAdtAuditoriaItem').data('igGrid').dataSource._dataView
                        if (dataView.length === 0) return;
                        var firstRow = dataView[0].RowDataId;
                        clear = vm.status() === 'C';
                        if (vm.status() === 'C')
                            $('#ConsultaAuditoria_dGridAdtAuditoriaItem').igGridUpdating('startEdit', firstRow, 0, true);
                        else {
                            ...
                            cols.some(function (entry) {
                                if (entry.key !== 'RowDataId' && !entry.hidden) {
                                    if (verifyCanEditCol(entry.key, clear, entity)) {
                                        $('#ConsultaAuditoria_dGridAdtAuditoriaItem').igGridUpdating('startEdit', firstRow, indexColumn, true);
                                        return true;
                                    }
                                    indexColumn++;
                                }
                ...
                var getVisibleColumns = function(metaDataControl) {
                   ...
                   var visibleColumns = '';
                   if($('#ConsultaAuditoria_dGridAdtAuditoriaItem').data('igGrid') === undefined) return '';
                   var cols = $('#ConsultaAuditoria_dGridAdtAuditoriaItem').igGrid('option', 'columns');
                   if (cols) {
                ...
                var isElementHided = function (grid, forceCreating) {
                  if (!grid) grid = $('#ConsultaAuditoria_dGridAdtAuditoriaItem');
                  return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialogAdtAuditoriaItem').is(':visible'));
                }
                var refreshData = true;
                var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'ConsultaAuditoria_dGridAdtAuditoriaItem_container', dataBind: function (commitData, forceCreating) {
                   var grid = $('#ConsultaAuditoria_dGridAdtAuditoriaItem');
                   ...
                   if (!started) {
                       ...
                       started = true;
                       commitData = false;
                       $('#ConsultaAuditoria_dGridAdtAuditoriaItem_groupbyarea').addClass('hide');
                   }
                   if (grid.igGridUpdating('isEditing')) {
                   ...
                   if (execFillDetais) {
                     ...
                     vm.currentDataItem().fillDetails(false, 'AdtAuditoriaItem');
                     return;
                   }
                   ...
                   if (rows.length > 0) {
                     ...
                     if (vm.currentDataItem().currentAdtAuditoriaItem() != null)
                     {
                       for(var idx = 0; idx < rows.length; idx++)
                       {
                         if (rows[idx].dataset.id == getAbsoluteValue(vm.currentDataItem().currentAdtAuditoriaItem().RowDataId))
                         {
                     ...
                     if ($('#dialogAdtAuditoriaItem').is(':visible')) {
                        ...
                        if (hasPaging.length > 0) {
                           ...
                           $('label#currentNumberAdtAuditoriaItem').html(current + ' - ' + totalCurrentPage);
                        }
                        else
                           $('label#currentNumberAdtAuditoriaItem').html(1);
                        $('label#totalNumberAdtAuditoriaItem').html(totalGrid);
                    }
                   } else {
                       $('label#currentNumberAdtAuditoriaItem').html(0);
                       $('label#totalNumberAdtAuditoriaItem').html(0);
                   }
                }};
                var valueGrouBy = -1;
                var deletedIndex = -1;
                function verifyCanEditCol(column, clear, entity){
                    switch(column){
                        case 'IdAdtAuditoriaItem': { canEditing = clear; break;}
                        case 'TipoOperacao': { canEditing = clear; break;}
                        case 'SchemaTabela': { canEditing = clear; break;}
                        case 'NomeTabela': { canEditing = clear; break;}
                        case 'IdAdtAuditoria': { canEditing = clear; break;}
                    }
                    return canEditing;
                };
                function createDataGrid(grid) {
                    ...
                    vm.gridSaveStates[gridId] = {
                        ...
                        closePopover: function () {
                            $('#ConsultaAuditoria_dGridAdtAuditoriaItem_LayoutBtn').igPopover('hide');
                        },
                    ...
                    grid.igGrid({ height: (getGridHeightSuggested() * 0.5), width: '100%',
                        dataSource: [],
                        ...
                        cellClick: function(evt, ui) {
                             if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                                 ...
                                 if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                                 {
                                      ...
                                      showMultimidia(entity, e, table, key, vm.ConsultaAuditoria());
                                 }
                             }
                             if (typeof vm.OnGridClientClick === 'function') {
                                 entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                 vm.OnGridClientClick('ConsultaAuditoria_dGridAdtAuditoriaItem', ui.colKey, entity);
                             }
                             if (vm.status() != 'Q') {
                                 var grid = $('#ConsultaAuditoria_dGridAdtAuditoriaItem');
                        ...
                        columns: [
                            ...
                            { key: 'IdAdtAuditoriaItem', headerText: vm.getLayoutHeaderGrid('ConsultaAuditoria_tbAdtAuditoriaItem_IdAdtAuditoriaItem'), width: '309px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: true, unbound: false, group: null   },
                            { key: 'TipoOperacao', headerText: vm.getLayoutHeaderGrid('ConsultaAuditoria_cmbAdtAuditoriaItem_TipoOperacao'), width: '100px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null  , formatter: function (val, record) { return  vm.dataDomains.getName('TIPO_OPERACAO', val);} },
                            { key: 'SchemaTabela', headerText: vm.getLayoutHeaderGrid('ConsultaAuditoria_tbAdtAuditoriaItem_SchemaTabela'), width: '205px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'NomeTabela', headerText: vm.getLayoutHeaderGrid('ConsultaAuditoria_tbAdtAuditoriaItem_NomeTabela'), width: '500px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'IdAdtAuditoria', headerText: vm.getLayoutHeaderGrid('ConsultaAuditoria_lUpAdtAuditoriaItem_IdAdtAuditoria'), width: '271px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: true, unbound: false, group: null   }
                        ],
                        features: [
                                    ...
                                    filterDialogOpening: function (evt, ui) {
                                        ...
                                        var dataView = $('#ConsultaAuditoria_dGridAdtAuditoriaItem').data('igGrid').dataSource;
                                        ...
                                        scriptHtml += '    var newGrid = $("#ConsultaAuditoria_dGridAdtAuditoriaItem");';
                                        scriptHtml += '    var listFilter = [];';
                                        scriptHtml += '    function hideColumn(){';
                                        scriptHtml += '     if ($("#showHideColumn_ConsultaAuditoria_dGridAdtAuditoriaItem")["0"].innerHTML.indexOf("Ocultar") >= 0) {';
                                        scriptHtml += '        $("#showHideColumn_ConsultaAuditoria_dGridAdtAuditoriaItem")["0"].innerHTML = "Mostrar Coluna";';
                                        scriptHtml += '        newGrid.igGridHiding("hideColumn", newCol);';
                                        scriptHtml += '     }';
                                        scriptHtml += '     else{';
                                        scriptHtml += '        $("#showHideColumn_ConsultaAuditoria_dGridAdtAuditoriaItem")["0"].innerHTML = "Ocultar Coluna";';
                                        ...
                                        scriptHtml += '             $("#showHideColumn_ConsultaAuditoria_dGridAdtAuditoriaItem")["0"].innerHTML = column[0].hidden ? "Mostrar Coluna" : "Ocultar Coluna"';
                                        scriptHtml += '         }';
                                        scriptHtml += '    }';
                                        ...
                                        scriptHtml += '              <div id="comboFields_ConsultaAuditoria_dGridAdtAuditoriaItem"></div>';
                                        scriptHtml += '              <script>';
                                        scriptHtml += '                  var columns = newGrid.igGrid("option", "columns");';
                                        scriptHtml += '                  $("#comboFields_ConsultaAuditoria_dGridAdtAuditoriaItem").igCombo({ dataSource: columns, mode : "dropdown", valueKey: "key", textKey: "headerText", selectionChanging: function (evt, ui) {';
                                        scriptHtml += '                      newCol = ui.items["0"].data.key;';
                                        scriptHtml += '                      updateHideButton()';
                                        scriptHtml += '                  }});';
                                        scriptHtml += '                  $("#comboFields_ConsultaAuditoria_dGridAdtAuditoriaItem").igCombo("value", newCol);';
                                        scriptHtml += '              </script>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '     <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">';
                                        ...
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <div style="margin-left: 5px; margin-top: 5px">';
                                        scriptHtml += '              <i class="fa fa-eye-slash" aria-hidden="true" style="margin-right: 5px;"></i><a onclick="hideColumn()" style="cursor: pointer" id="showHideColumn_ConsultaAuditoria_dGridAdtAuditoriaItem">Ocultar Coluna</a>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <br>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '  </div>';
                                    ...
                                    { name: 'Tooltips', columnSettings:[{ columnKey: "IdAdtAuditoriaItem", allowTooltips: true },{ columnKey: "TipoOperacao", allowTooltips: true },{ columnKey: "SchemaTabela", allowTooltips: true },{ columnKey: "NomeTabela", allowTooltips: true },{ columnKey: "IdAdtAuditoria", allowTooltips: false }] },
                                    { name: 'Resizing' }, 
                                    { name: 'Hiding', 
                                    },
                                   ...
                                   ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                      ...
                                      columnSettings: [{ columnKey: "IdAdtAuditoria", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "", isNullable: false, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, activateAutoComplete: false, autoCompleteMaxResults: 7, validateOnClearState:false, maxValue:9007199254740992, maxLength: 0, defaultValue: 0 } }, { columnKey: 'TipoOperacao', editorType: 'combo', editorOptions: {   selectionChanged: function (evt, ui) {   var val = null;   if (ui.items != null && ui.items.length > 0) { val = ui.items[0].data['id']; }  updateEntity('TipoOperacao', val, false); },  mode: 'dropdown', dropDownOnFocus: true,  dataSource: vm.dataDomains.getItems('TIPO_OPERACAO', ''),  textKey: 'name', valueKey: 'id', enableClearButton: false }}, { columnKey: "IdAdtAuditoriaItem" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('ConsultaAuditoria_dGridAdtAuditoriaItem', 'IdAdtAuditoriaItem', ui.oldValue, ui.value);}}, maxLength: 0 } }, { columnKey: "SchemaTabela" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('ConsultaAuditoria_dGridAdtAuditoriaItem', 'SchemaTabela', ui.oldValue, ui.value);}}, maxLength: 20 } }, { columnKey: "NomeTabela" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('ConsultaAuditoria_dGridAdtAuditoriaItem', 'NomeTabela', ui.oldValue, ui.value);}}, maxLength: 100 } }],
                                      ...
                                      editCellStarted: function(evt, ui){
                                          ...
                                          var columns = $('#ConsultaAuditoria_dGridAdtAuditoriaItem').igGridUpdating('option', 'columnSettings');
                                          var currentCol = null;
                                          currentRow = ui.rowID;
                                          ...
                                          if (currentCol != null && currentCol.hasOwnProperty('editorType') && currentCol.editorType === 'combo') {
                                             ...
                                             if (lookUpName != null) {
                                                 ...
                                                 if (lstRefreshDados.length === 0)
                                                     vm.dataCombo.fillDataCombos(lookUpName, ui.columnKey, vm.currentDataItem().currentAdtAuditoriaItem(), function (result) {
                        ...
                        if ((typeof vm.OnDataGridCreated === 'function')){
                            vm.OnDataGridCreated('ConsultaAuditoria_dGridAdtAuditoriaItem');
                        }
                        ...
                        selectionrowselectionchanged = function (evt, ui) {
                            if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                    $(document).undelegate('#ConsultaAuditoria_dGridAdtAuditoriaItem', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                    ...
                                    $(document).delegate ('#ConsultaAuditoria_dGridAdtAuditoriaItem', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                }
                                selectGridCurrentItem(vm.goToKey, 'RowDataId', ui, vm.currentDataItem().currentAdtAuditoriaItem, getDataSource()); 
                             } 
                        };
                        $(document).delegate('#ConsultaAuditoria_dGridAdtAuditoriaItem', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        $('#ConsultaAuditoria_dGridAdtAuditoriaItem > tbody tr').live('focus', function(evt) {
                            var grid = $('#ConsultaAuditoria_dGridAdtAuditoriaItem'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                    ...
                    vm.addDataSource({ key: 'ConsultaAuditoria_dGridAdtAuditoriaItem', name: 'AdtAuditoriaItemList', itemsSource: itemsSource });
                }
            
            
            , ChangedBrandscyConsultaAuditoria_dGrid: function(vm, decimals, reset) {
                var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#scyConsultaAuditoria_dGrid').data('igGrid'),
                    grdUpd = $('#scyConsultaAuditoria_dGrid').data('igGridUpdating');
                if(isNull(grd) || isNull(grdUpd)) return;
                for (i = 0; i < grd.options.columns.length; i++) {
                }
                for (i = 0; i < grdUpd.options.columnSettings.length; i++) {
            ...
            , renderscyConsultaAuditoria_dGrid: function(vm) {
                var getDataSource = function() {
                    var source = null;
                    try {
                        source = vm.dataView;
                ...
                $('#scyConsultaAuditoria_dGrid_headers').live('focus  keydown', function (evt) {
                    var keyCode = window.event ? evt.which : evt.keyCode;
                    if (keyCode === 9) {
                        var cols = $('#scyConsultaAuditoria_dGrid').igGrid('option', 'columns');
                        var dataView = $('#scyConsultaAuditoria_dGrid').data('igGrid').dataSource._dataView
                        if (dataView.length === 0) return;
                        var firstRow = dataView[0].RowDataId;
                        clear = vm.status() === 'C';
                        if (vm.status() === 'C')
                            $('#scyConsultaAuditoria_dGrid').igGridUpdating('startEdit', firstRow, 0, true);
                        else {
                            var entity = findElementByKey(getDataSource(), 'RowDataId', firstRow);
                            var indexColumn = 0;
                            cols.some(function (entry) {
                                if (entry.key !== 'RowDataId' && !entry.hidden) {
                                    if (verifyCanEditCol(entry.key, clear, entity)) {
                                        $('#scyConsultaAuditoria_dGrid').igGridUpdating('startEdit', firstRow, indexColumn, true);
                                        return true;
                                    }
                                    indexColumn++;
                                }
                ...
                var getVisibleColumns = function(metaDataControl) {
                   if (metaDataControl) return '';
                   var visibleColumns = '';
                   if($('#scyConsultaAuditoria_dGrid').data('igGrid') === undefined) return '';
                   var cols = $('#scyConsultaAuditoria_dGrid').igGrid('option', 'columns');
                   if (cols) {
                     for (var idx = 0; idx < cols.length; idx++) {
                         if (cols[idx].hidden !== true) visibleColumns += (visibleColumns === '' ? '' : ',') + cols[idx].key;
                ...
                var isElementHided = function (grid, forceCreating) {
                  if (!grid) grid = $('#scyConsultaAuditoria_dGrid');
                  return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialog').is(':visible'));
                }
                var refreshData = true;
                var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'scyConsultaAuditoria_dGrid_container', dataBind: function (commitData, forceCreating) {
                   var grid = $('#scyConsultaAuditoria_dGrid');
                   if (started && typeof grid.data('igGridUpdating') === 'undefined') { started = false; }
                   if (commitData && started) {
                       if (grid.igGridUpdating('isEditing')) {
                           grid.igGrid('commit');
                   ...
                   if (!started) {
                       ...
                       started = true;
                       commitData = false;
                       $('#scyConsultaAuditoria_dGrid_groupbyarea').addClass('hide');
                   }
                   if (grid.igGridUpdating('isEditing')) {
                        grid.igGridUpdating('endEdit', true);
                   }
                ...
                function verifyCanEditCol(column, clear, entity){
                    switch(column){
                        case 'IdAdtAuditoria': { canEditing = clear; break;}
                        case 'DataHora': { canEditing = clear; break;}
                        case 'AssemblyName': { canEditing = clear; break;}
                        case 'ConnectionString': { canEditing = clear; break;}
                        case 'IdUsuario': { canEditing = clear; break;}
                ...
                function createDataGrid(grid) {
                    ...
                    vm.gridSaveStates[gridId] = {
                        ...
                        closePopover: function () {
                            $('#scyConsultaAuditoria_dGrid_LayoutBtn').igPopover('hide');
                        },
                        applyLayout: function (layoutInfo) {
                            var _this = this;
                            if (isNull(layoutInfo) && (!_this.currentLayout() || _this.currentLayout().Id === 0)) {
                    ...
                    grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
                        ...
                        cellClick: function(evt, ui) {
                             ...
                             if (typeof vm.OnGridClientClick === 'function') {
                                 entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                 vm.OnGridClientClick('scyConsultaAuditoria_dGrid', ui.colKey, entity);
                             }
                             if (vm.status() != 'Q') {
                                 var grid = $('#scyConsultaAuditoria_dGrid');
                                 var isEditing = grid.igGridUpdating('isEditing');
                        ...
                        columns: [
                            ...
                            { key: 'IdAdtAuditoria', headerText: vm.getLayoutDisplayName('ConsultaAuditoria_tbIdAdtAuditoria'), width: '250px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null   },
                            { key: 'DataHora', headerText: vm.getLayoutDisplayName('ConsultaAuditoria_dtDataHora'), width: '153px', dataType: 'date', columnCssClass: 'ellipsis', format: 'dd/MM/yyyy', hidden: false, unbound: false, group: null   },
                            { key: 'AssemblyName', headerText: vm.getLayoutDisplayName('ConsultaAuditoria_edAssemblyName'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'ConnectionString', headerText: vm.getLayoutDisplayName('ConsultaAuditoria_edConnectionString'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'IdUsuario', headerText: vm.getLayoutDisplayName('ConsultaAuditoria_lUpIdUsuario'), width: '271px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'Email', headerText: vm.getLayoutDisplayName('ConsultaAuditoria_lUpEmail'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: true, unbound: false, group: null   },
                            { key: 'NomeUsuario', headerText: vm.getLayoutDisplayName('ConsultaAuditoria_lUpNomeUsuario'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   }
                        ],
                        features: [
                                    { name: 'Sorting', type: 'local', caseSensitive: false, unsortedColumnTooltip: '', sortedColumnTooltip: '',
                                      columnSorting: function (evt, ui) { }
                                    ...
                                    filterDialogOpening: function (evt, ui) {
                                        ...
                                        var dataView = $('#scyConsultaAuditoria_dGrid').data('igGrid').dataSource;
                                        if (dataView.settings.filtering.expressions.length <= 0)
                                            dataView._filteredData = [];
                                        ...
                                        scriptHtml += '  <script>';
                                        scriptHtml += '    var newCol = "' + col + '";';
                                        scriptHtml += '    var newGrid = $("#scyConsultaAuditoria_dGrid");';
                                        scriptHtml += '    var listFilter = [];';
                                        scriptHtml += '    function hideColumn(){';
                                        scriptHtml += '     if ($("#showHideColumn_scyConsultaAuditoria_dGrid")["0"].innerHTML.indexOf("Ocultar") >= 0) {';
                                        scriptHtml += '        $("#showHideColumn_scyConsultaAuditoria_dGrid")["0"].innerHTML = "Mostrar Coluna";';
                                        scriptHtml += '        newGrid.igGridHiding("hideColumn", newCol);';
                                        scriptHtml += '     }';
                                        scriptHtml += '     else{';
                                        scriptHtml += '        $("#showHideColumn_scyConsultaAuditoria_dGrid")["0"].innerHTML = "Ocultar Coluna";';
                                        scriptHtml += '        newGrid.igGridHiding("showColumn", newCol);';
                                        scriptHtml += '     }';
                                        scriptHtml += '    }';
                                        scriptHtml += '    function updateHideButton(){';
                                        scriptHtml += '         var column = $.grep(newGrid.igGrid("option", "columns"), function (element, index) { return element.key == newCol });';
                                        scriptHtml += '         if (column.length > 0){';
                                        scriptHtml += '             $("#showHideColumn_scyConsultaAuditoria_dGrid")["0"].innerHTML = column[0].hidden ? "Mostrar Coluna" : "Ocultar Coluna"';
                                        scriptHtml += '         }';
                                        scriptHtml += '    }';
                                        scriptHtml += '    function orderColumn(dir){';
                                        scriptHtml += '      if(dir == 1){dir = "asc"} else{dir = "desc"}';
                                        ...
                                        scriptHtml += '          <div  style="margin-left: 5px" >';
                                        scriptHtml += '              <div>Propriedade:</div>';
                                        scriptHtml += '              <div id="comboFields_scyConsultaAuditoria_dGrid"></div>';
                                        scriptHtml += '              <script>';
                                        scriptHtml += '                  var columns = newGrid.igGrid("option", "columns");';
                                        scriptHtml += '                  $("#comboFields_scyConsultaAuditoria_dGrid").igCombo({ dataSource: columns, mode : "dropdown", valueKey: "key", textKey: "headerText", selectionChanging: function (evt, ui) {';
                                        scriptHtml += '                      newCol = ui.items["0"].data.key;';
                                        scriptHtml += '                      updateHideButton()';
                                        scriptHtml += '                  }});';
                                        scriptHtml += '                  $("#comboFields_scyConsultaAuditoria_dGrid").igCombo("value", newCol);';
                                        scriptHtml += '              </script>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '     <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">';
                                        ...
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <div style="margin-left: 5px; margin-top: 5px">';
                                        scriptHtml += '              <i class="fa fa-eye-slash" aria-hidden="true" style="margin-right: 5px;"></i><a onclick="hideColumn()" style="cursor: pointer" id="showHideColumn_scyConsultaAuditoria_dGrid">Ocultar Coluna</a>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <br>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '  </div>';
                                    ...
                                    { name: 'Tooltips', columnSettings:[{ columnKey: "IdAdtAuditoria", allowTooltips: true },{ columnKey: "DataHora", allowTooltips: true },{ columnKey: "AssemblyName", allowTooltips: true },{ columnKey: "ConnectionString", allowTooltips: true },{ columnKey: "IdUsuario", allowTooltips: false },{ columnKey: "Email", allowTooltips: false },{ columnKey: "NomeUsuario", allowTooltips: false }] },
                                    { name: 'Resizing' }, 
                                    { name: 'Hiding', 
                                    },
                                    { name: 'MultiColumnHeaders' }
                                   ...
                                   ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                      ...
                                      showReadonlyEditors: false,
                                      showDoneCancelButtons: false,
                                      columnSettings: [{ columnKey: "IdUsuario", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpTcsUsuario", isNullable: false, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, activateAutoComplete: false, autoCompleteMaxResults: 7, validateOnClearState:false, maxValue:9007199254740992, maxLength: 0, defaultValue: 0 } }, { columnKey: "Email", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpTcsUsuario", isNullable: true, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, activateAutoComplete: false, autoCompleteMaxResults: 7, validateOnClearState:false, maxValue:null, maxLength: 250, defaultValue: '' } }, { columnKey: "NomeUsuario", editorProvider: new $.ig.EditorProviderLookUp(), editorOptions: { lookUpName: "LookUpTcsUsuario", isNullable: false, custom: vm.custom, vm: vm, verifyCanEditCol: verifyCanEditCol, allowMultiSelectionInSearch:true, activateAutoComplete: false, autoCompleteMaxResults: 7, validateOnClearState:false, maxValue:null, maxLength: 250, defaultValue: '' } }, { columnKey: 'DataHora', editorType: 'datepicker', editorOptions: {valueChanged: function(evt, ui){if (typeof vm.OnPropertyChangeDataGrid === 'function') {vm.OnPropertyChangeDataGrid('scyConsultaAuditoria_dGrid', 'DataHora', ui.oldValue, ui.value);}}, minValue: new Date(1900, 0, 1), datepickerOptions: { changeMonth: true, changeYear: true }  } }, { columnKey: "IdAdtAuditoria" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyConsultaAuditoria_dGrid', 'IdAdtAuditoria', ui.oldValue, ui.value);}}, maxLength: 0 } }, { columnKey: "AssemblyName" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyConsultaAuditoria_dGrid', 'AssemblyName', ui.oldValue, ui.value);}}, maxLength: 500 } }, { columnKey: "ConnectionString" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyConsultaAuditoria_dGrid', 'ConnectionString', ui.oldValue, ui.value);}}, maxLength: 500 } }],
                                      rowDeleting: function (evt, ui) {
                                          deletedIndex = ui.element.context.rowIndex;
                                          var entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowID);
                                          if (entity) {
                                      ...
                                      editCellStarted: function(evt, ui){
                                          var lstRefreshDados = null;
                                          var columns = $('#scyConsultaAuditoria_dGrid').igGridUpdating('option', 'columnSettings');
                                          var currentCol = null;
                                          currentRow = ui.rowID;
                                          columns.forEach(function (entry, index) {
                                             if (entry.columnKey === ui.columnKey) currentCol = entry;
                        ...
                        if ((typeof vm.OnDataGridCreated === 'function')){
                            vm.OnDataGridCreated('scyConsultaAuditoria_dGrid');
                        }
                        var selectionrowselectionchanged = null, selectedRowId = -1;
                        selectionrowselectionchanged = function (evt, ui) {
                            if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                    $(document).undelegate('#scyConsultaAuditoria_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                    ui.owner.clearSelection();
                                    ui.owner.selectRow(ui.row.index);
                                    if (vm.status() === 'Q'){
                                        var gridCell = ui.owner.grid;
                                    ...
                                    selectedRowId = ui.row.id;
                                    $(document).delegate ('#scyConsultaAuditoria_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                }
                                selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                             } 
                        };
                        $(document).delegate('#scyConsultaAuditoria_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        $('#scyConsultaAuditoria_dGrid > tbody tr').live('focus', function(evt) {
                            var grid = $('#scyConsultaAuditoria_dGrid'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                            var selectedRows = grid.igGridSelection('option', 'multipleSelection') ? grid.igGridSelection('selectedRows') : [grid.igGridSelection('selectedRow')];
                            if (selectedRowId === id) return;
                            selectedRowId = id;
                            grid.igGridSelection('selectRowById', id);
                        ...
                        grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                            if (vm.ConsultaAuditoria().status() === 'Q') vm.ConsultaAuditoria().dataToolbar.viewInfo();
                        });
                    }
                    vm.addDataSource({ key: 'scyConsultaAuditoria_dGrid', name: 'dataView', itemsSource: itemsSource });
                }
