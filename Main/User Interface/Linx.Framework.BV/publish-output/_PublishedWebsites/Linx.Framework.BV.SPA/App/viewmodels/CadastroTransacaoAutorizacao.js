define(['durandal/app', 'pkg_linx-framework-bv-spa/services/TransacaoAutorizacaoContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-framework-bv-spa/viewmodels/CadastroTransacaoAutorizacaoComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroTransacaoAutorizacao', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroTransacaoAutorizacao(), 'Name'))());
        }, function (err) {
    ...
    var activate = function (settings, querystring) {
      ...
      vm.CadastroTransacaoAutorizacao = getVM;
      //loadLanguage();
      changeLanguage();
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroTransacaoAutorizacao' ]; settings.parentVM.CadastroTransacaoAutorizacao = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.CadastroTransacaoAutorizacao = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('CadastroTransacaoAutorizacao', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroTransacaoAutorizacao');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroTransacaoAutorizacao'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroTransacaoAutorizacao', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroTransacaoAutorizacao', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#CadastroTransacaoAutorizacao_tcTcsTransacaoAutorizacaoTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#CadastroTransacaoAutorizacao_tcTcsTransacaoAutorizacaoTabControl');

    complement.renderCadastroTransacaoAutorizacao_dGridTcsTransacaoMenuAutorizacao(vm);

    complement.renderCadastroTransacaoAutorizacao_dGridTcsTransacaoDependenteAutorizacao(vm);

    complement.renderscyCadastroTransacaoAutorizacao_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                                   complement.ChangedBrandCadastroTransacaoAutorizacao_dGridTcsTransacaoMenuAutorizacao(vm, decimals, reset);
                                   complement.ChangedBrandCadastroTransacaoAutorizacao_dGridTcsTransacaoDependenteAutorizacao(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsTransacaoAutorizacao';
    ...
    var visibleColumns = 'CodTransacao,NomeCurto,DescTransacao,ClasseNome,Inativo,DescObjeto,LxTipoTransacao,DescModulo,Icone';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsTransacaoAutorizacao', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsTransacaoAutorizacao', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsTransacaoAutorizacao', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsTransacaoAutorizacao');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsTransacaoAutorizacao');
        ...
        return dataContext.getTcsTransacaoAutorizacaoByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsTransacaoAutorizacao'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsTransacaoAutorizacaoByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
    ...
    $lx(vm,'#CadastroTransacaoAutorizacao_tbTagInput').on('itemAdded', function(event) {
	UpdateTagValue();
});

$lx(vm,'#CadastroTransacaoAutorizacao_tbTagInput').on('itemRemoved', function(event) {
	UpdateTagValue();
});
...
var tag = $lx(vm,'#CadastroTransacaoAutorizacao_tbTagInput').val();
var value = status() === "E" ? tag :  '%' + tag + '%';
setAbsoluteValue(currentDataItem(), 'Tag', value)

    }
    var ClearTaginput = function () {
    $("input[data-role='tagsinput']").tagsinput();

$lx(vm,'#CadastroTransacaoAutorizacao_tbTagInput').tagsinput('removeAll');
	
if (status() === 'Q'){
	$lx(vm, '#CadastroTransacaoAutorizacao_tbTagInput').attr('disabled', 'disabled');
}
else{
	$lx(vm, '#CadastroTransacaoAutorizacao_tbTagInput').removeAttr('disabled');
}
    }
    var OnCleared = function () {
    ClearTaginput();
setAbsoluteValue(currentDataItem(), 'Tag', '')
    }
    var OnNavigated = function (currentIndex) {
    UpdateTaginputValue();
    }
    var UpdateTaginputValue = function () {
    ClearTaginput();
...
var tag = getAbsoluteValue(currentDataItem().Tag)
if (isNullOrEmpty(tag)){
	return;
}

tag.split(',').forEach(function(e) {
	$lx(vm,'#CadastroTransacaoAutorizacao_tbTagInput').tagsinput('add', e);
})
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsTransacaoAutorizacao(getBandeiraRede(), clearComplete);
    ...
    var createTcsTransacaoAutorizacao = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsTransacaoAutorizacao();
    ...
    var createAndNotifyTcsTransacaoAutorizacao = function() {
        var entity = createTcsTransacaoAutorizacao();
        notifyPresentation('');
        return entity;
    };
    
    var createTcsTransacaoMenuAutorizacao = function(parent, noCurrent) {
        dataBind('TcsTransacaoMenuAutorizacaoList', true);
        var entity = dataContext.createTcsTransacaoMenuAutorizacao(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsTransacaoMenuAutorizacao(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsTransacaoMenuAutorizacao = function(parent) {
        var entity = createTcsTransacaoMenuAutorizacao(parent);
        notifyPresentation('TcsTransacaoMenuAutorizacaoList');
        return entity;
    };
    
    var createTcsTransacaoDependenteAutorizacao = function(parent, noCurrent) {
        dataBind('TcsTransacaoDependenteAutorizacaoList', true);
        var entity = dataContext.createTcsTransacaoDependenteAutorizacao(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsTransacaoDependenteAutorizacao(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsTransacaoDependenteAutorizacao = function(parent) {
        var entity = createTcsTransacaoDependenteAutorizacao(parent);
    ...
    var addNew = function (parentEntity) {
        ...
        goToItem(createTcsTransacaoAutorizacao());
        editInnerUIs();
        showButtonsEditorTemplate();
        dataBind();
    };
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroTransacaoAutorizacao_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroTransacaoAutorizacao_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroTransacaoAutorizacao_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsTransacaoMenuAutorizacaoOrdemNavegacao_begin: ko.observable(null), TcsTransacaoMenuAutorizacaoOrdemNavegacao_end: ko.observable(null),
        TcsTransacaoAutorizacaoDescModulo: ko.observable(null),
        TcsTransacaoAutorizacaoDescObjeto: ko.observable(null),
        TcsTransacaoMenuAutorizacaoDescModulo: ko.observable(null),
        TcsTransacaoMenuAutorizacaoDescModuloMenu: ko.observable(null),
        TcsTransacaoDependenteAutorizacaoClasseNome: ko.observable(null),
        TcsTransacaoDependenteAutorizacaoDescTransacao: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsTransacaoMenuAutorizacaoOrdemNavegacao_begin(null); entitySearchRange.TcsTransacaoMenuAutorizacaoOrdemNavegacao_end(null);
        entitySearchRange.TcsTransacaoAutorizacaoDescModulo(null);
        entitySearchRange.TcsTransacaoAutorizacaoDescObjeto(null);
        entitySearchRange.TcsTransacaoMenuAutorizacaoDescModulo(null);
        entitySearchRange.TcsTransacaoMenuAutorizacaoDescModuloMenu(null);
        entitySearchRange.TcsTransacaoDependenteAutorizacaoClasseNome(null);
        entitySearchRange.TcsTransacaoDependenteAutorizacaoDescTransacao(null);
    };
    entitySearchRange.has_TcsTransacaoMenuAutorizacaoOrdemNavegacao = ko.computed(function(){ return (entitySearchRange.TcsTransacaoMenuAutorizacaoOrdemNavegacao_begin() != null || entitySearchRange.TcsTransacaoMenuAutorizacaoOrdemNavegacao_end() != null); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroTransacaoAutorizacao_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            dataView: dataView,
            viewName: 'CadastroTransacaoAutorizacao',
            ...
            rootBmTypeName: 'TCS_TRANSACAO_AUTORIZACAO',
            rootDataTypeName: 'TcsTransacaoAutorizacao',
            rootNamespace: 'Linx.Framework.BV.TransacaoAutorizacao',
            ...
            createAndNotifyTcsTransacaoAutorizacao: createAndNotifyTcsTransacaoAutorizacao,
            createTcsTransacaoMenuAutorizacao: createTcsTransacaoMenuAutorizacao,
            createAndNotifyTcsTransacaoMenuAutorizacao: createAndNotifyTcsTransacaoMenuAutorizacao,
            createTcsTransacaoDependenteAutorizacao: createTcsTransacaoDependenteAutorizacao,
            createAndNotifyTcsTransacaoDependenteAutorizacao: createAndNotifyTcsTransacaoDependenteAutorizacao,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroTransacaoAutorizacao',
            pivots : pivots
        };
