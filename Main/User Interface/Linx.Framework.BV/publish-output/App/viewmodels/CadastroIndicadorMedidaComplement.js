define(['managers/__auth', 'managers/user'], function (managerAuth, managerUser) {
    var complementCtor = function() {
        var complement = {
            isAutomatic: true
            , ChangedBrandCadastroIndicadorMedida_dGridTcsIndicadorIndice: function(vm, decimals, reset) {
                var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#CadastroIndicadorMedida_dGridTcsIndicadorIndice').data('igGrid'),
                    grdUpd = $('#CadastroIndicadorMedida_dGridTcsIndicadorIndice').data('igGridUpdating');
            ...
            , renderCadastroIndicadorMedida_dGridTcsIndicadorIndice: function(vm) {
                var getDataSource = function() {
                    var source = null;
                    try {
                        source = vm.currentDataItem().TcsIndicadorIndiceList;
                    }
                    catch (e) { }
                ...
                var dataSourceIsLoaded = function() {
                    var isLoaded = false;
                    try {
                        isLoaded = (vm.currentDataItem().TcsIndicadorIndiceIsLoaded === true || vm.currentDataItem().TcsIndicadorIndiceList().length > 0);
                    }
                    catch (e) {
                        isLoaded = true;
                    }
                ...
                $('#CadastroIndicadorMedida_dGridTcsIndicadorIndice_headers').live('focus  keydown', function (evt) {
                    var keyCode = window.event ? evt.which : evt.keyCode;
                    if (keyCode === 9) {
                        var cols = $('#CadastroIndicadorMedida_dGridTcsIndicadorIndice').igGrid('option', 'columns');
                        var dataView = $('#CadastroIndicadorMedida_dGridTcsIndicadorIndice').data('igGrid').dataSource._dataView
                        if (dataView.length === 0) return;
                        var firstRow = dataView[0].RowDataId;
                        clear = vm.status() === 'C';
                        if (vm.status() === 'C')
                            $('#CadastroIndicadorMedida_dGridTcsIndicadorIndice').igGridUpdating('startEdit', firstRow, 0, true);
                        else {
                            ...
                            cols.some(function (entry) {
                                if (entry.key !== 'RowDataId' && !entry.hidden) {
                                    if (verifyCanEditCol(entry.key, clear, entity)) {
                                        $('#CadastroIndicadorMedida_dGridTcsIndicadorIndice').igGridUpdating('startEdit', firstRow, indexColumn, true);
                                        return true;
                                    }
                                    indexColumn++;
                                }
                ...
                var getVisibleColumns = function(metaDataControl) {
                   ...
                   var visibleColumns = '';
                   if($('#CadastroIndicadorMedida_dGridTcsIndicadorIndice').data('igGrid') === undefined) return '';
                   var cols = $('#CadastroIndicadorMedida_dGridTcsIndicadorIndice').igGrid('option', 'columns');
                   if (cols) {
                ...
                var isElementHided = function (grid, forceCreating) {
                  if (!grid) grid = $('#CadastroIndicadorMedida_dGridTcsIndicadorIndice');
                  return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialogTcsIndicadorIndice').is(':visible'));
                }
                var refreshData = true;
                var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'CadastroIndicadorMedida_dGridTcsIndicadorIndice_container', dataBind: function (commitData, forceCreating) {
                   var grid = $('#CadastroIndicadorMedida_dGridTcsIndicadorIndice');
                   ...
                   if (!started) {
                       ...
                       started = true;
                       commitData = false;
                       $('#CadastroIndicadorMedida_dGridTcsIndicadorIndice_groupbyarea').addClass('hide');
                   }
                   if (grid.igGridUpdating('isEditing')) {
                   ...
                   if (execFillDetais) {
                     ...
                     vm.currentDataItem().fillDetails(false, 'TcsIndicadorIndice');
                     return;
                   }
                   ...
                   if (rows.length > 0) {
                     ...
                     if (vm.currentDataItem().currentTcsIndicadorIndice() != null)
                     {
                       for(var idx = 0; idx < rows.length; idx++)
                       {
                         if (rows[idx].dataset.id == getAbsoluteValue(vm.currentDataItem().currentTcsIndicadorIndice().RowDataId))
                         {
                     ...
                     if ($('#dialogTcsIndicadorIndice').is(':visible')) {
                        ...
                        if (hasPaging.length > 0) {
                           ...
                           $('label#currentNumberTcsIndicadorIndice').html(current + ' - ' + totalCurrentPage);
                        }
                        else
                           $('label#currentNumberTcsIndicadorIndice').html(1);
                        $('label#totalNumberTcsIndicadorIndice').html(totalGrid);
                    }
                   } else {
                       $('label#currentNumberTcsIndicadorIndice').html(0);
                       $('label#totalNumberTcsIndicadorIndice').html(0);
                   }
                }};
                var valueGrouBy = -1;
                var deletedIndex = -1;
                function verifyCanEditCol(column, clear, entity){
                    switch(column){
                        case 'CodIndiceMedida': { canEditing = clear || vm.enabledForEditing(); break;}
                        case 'DescIndiceMedida': { canEditing = clear || vm.enabledForEditing(); break;}
                        case 'LimiteInferior': { canEditing = clear || vm.enabledForEditing(); break;}
                        case 'LimiteSuperior': { canEditing = clear || vm.enabledForEditing(); break;}
                        case 'Rgb': { canEditing = clear || vm.enabledForEditing(); break;}
                        case 'IndicaMedidaAlvo': { canEditing = clear || vm.enabledForEditing(); break;}
                    }
                    return canEditing;
                };
                function createDataGrid(grid) {
                    ...
                    vm.gridSaveStates[gridId] = {
                        ...
                        closePopover: function () {
                            $('#CadastroIndicadorMedida_dGridTcsIndicadorIndice_LayoutBtn').igPopover('hide');
                        },
                    ...
                    grid.igGrid({ height: (getGridHeightSuggested() * 0.75), width: '100%',
                        ...
                        cellClick: function(evt, ui) {
                             if (ui.cellElement && ui.cellElement.childNodes[0] && ui.cellElement.childNodes[0].childNodes[1]) {
                                 ...
                                 if (e && e.tagName === 'IMG' && vm.status() !== 'C')
                                 {
                                      ...
                                      showMultimidia(entity, e, table, key, vm.CadastroIndicadorMedida());
                                 }
                             }
                             if (typeof vm.OnGridClientClick === 'function') {
                                 entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                 vm.OnGridClientClick('CadastroIndicadorMedida_dGridTcsIndicadorIndice', ui.colKey, entity);
                             }
                             if (vm.status() != 'Q') {
                                 var grid = $('#CadastroIndicadorMedida_dGridTcsIndicadorIndice');
                        ...
                        columns: [
                            ...
                            { key: 'CodIndiceMedida', headerText: vm.getLayoutHeaderGrid('CadastroIndicadorMedida_tbTcsIndicadorIndice_CodIndiceMedida'), width: '200px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'DescIndiceMedida', headerText: vm.getLayoutHeaderGrid('CadastroIndicadorMedida_tbTcsIndicadorIndice_DescIndiceMedida'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'LimiteInferior', headerText: vm.getLayoutHeaderGrid('CadastroIndicadorMedida_ntxTcsIndicadorIndice_LimiteInferior'), width: '231px', dataType: 'number', columnCssClass: 'ellipsis', format: '0.00000', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                            { key: 'LimiteSuperior', headerText: vm.getLayoutHeaderGrid('CadastroIndicadorMedida_ntxTcsIndicadorIndice_LimiteSuperior'), width: '231px', dataType: 'number', columnCssClass: 'ellipsis', format: '0.00000', hidden: false, unbound: false, group: null , formatter: function (val, record) { return formatAndAlignNumber(grid, val, record, this.dataType, this.format); }  },
                            { key: 'Rgb', headerText: vm.getLayoutHeaderGrid('CadastroIndicadorMedida_TcsIndicadorIndice_Rgb'), width: '130px', dataType: 'number', columnCssClass: 'ellipsis', format: 'int', hidden: false, unbound: false, group: null   },
                            { key: 'IndicaMedidaAlvo', headerText: vm.getLayoutHeaderGrid('CadastroIndicadorMedida_ckTcsIndicadorIndice_IndicaMedidaAlvo'), width: '101px', dataType: 'bool', columnCssClass: 'ellipsis', format: 'checkbox', hidden: false, unbound: false, group: null   }
                        ],
                        features: [
                                    ...
                                    filterDialogOpening: function (evt, ui) {
                                        ...
                                        var dataView = $('#CadastroIndicadorMedida_dGridTcsIndicadorIndice').data('igGrid').dataSource;
                                        ...
                                        scriptHtml += '    var newGrid = $("#CadastroIndicadorMedida_dGridTcsIndicadorIndice");';
                                        scriptHtml += '    var listFilter = [];';
                                        scriptHtml += '    function hideColumn(){';
                                        scriptHtml += '     if ($("#showHideColumn_CadastroIndicadorMedida_dGridTcsIndicadorIndice")["0"].innerHTML.indexOf("Ocultar") >= 0) {';
                                        scriptHtml += '        $("#showHideColumn_CadastroIndicadorMedida_dGridTcsIndicadorIndice")["0"].innerHTML = "Mostrar Coluna";';
                                        scriptHtml += '        newGrid.igGridHiding("hideColumn", newCol);';
                                        scriptHtml += '     }';
                                        scriptHtml += '     else{';
                                        scriptHtml += '        $("#showHideColumn_CadastroIndicadorMedida_dGridTcsIndicadorIndice")["0"].innerHTML = "Ocultar Coluna";';
                                        ...
                                        scriptHtml += '             $("#showHideColumn_CadastroIndicadorMedida_dGridTcsIndicadorIndice")["0"].innerHTML = column[0].hidden ? "Mostrar Coluna" : "Ocultar Coluna"';
                                        scriptHtml += '         }';
                                        scriptHtml += '    }';
                                        ...
                                        scriptHtml += '              <div id="comboFields_CadastroIndicadorMedida_dGridTcsIndicadorIndice"></div>';
                                        scriptHtml += '              <script>';
                                        scriptHtml += '                  var columns = newGrid.igGrid("option", "columns");';
                                        scriptHtml += '                  $("#comboFields_CadastroIndicadorMedida_dGridTcsIndicadorIndice").igCombo({ dataSource: columns, mode : "dropdown", valueKey: "key", textKey: "headerText", selectionChanging: function (evt, ui) {';
                                        scriptHtml += '                      newCol = ui.items["0"].data.key;';
                                        scriptHtml += '                      updateHideButton()';
                                        scriptHtml += '                  }});';
                                        scriptHtml += '                  $("#comboFields_CadastroIndicadorMedida_dGridTcsIndicadorIndice").igCombo("value", newCol);';
                                        scriptHtml += '              </script>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '     <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">';
                                        ...
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <div style="margin-left: 5px; margin-top: 5px">';
                                        scriptHtml += '              <i class="fa fa-eye-slash" aria-hidden="true" style="margin-right: 5px;"></i><a onclick="hideColumn()" style="cursor: pointer" id="showHideColumn_CadastroIndicadorMedida_dGridTcsIndicadorIndice">Ocultar Coluna</a>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <br>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '  </div>';
                                    ...
                                    { name: 'Tooltips', columnSettings:[{ columnKey: "CodIndiceMedida", allowTooltips: true },{ columnKey: "DescIndiceMedida", allowTooltips: true },{ columnKey: "LimiteInferior", allowTooltips: true },{ columnKey: "LimiteSuperior", allowTooltips: true },{ columnKey: "Rgb", allowTooltips: true },{ columnKey: "IndicaMedidaAlvo", allowTooltips: true }] },
                                    { name: 'Resizing' }, 
                                    { name: 'Hiding', 
                                    },
                                   ...
                                   ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                      ...
                                      columnSettings: [{ columnKey: "CodIndiceMedida" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('CadastroIndicadorMedida_dGridTcsIndicadorIndice', 'CodIndiceMedida', ui.oldValue, ui.value);}}, maxLength: 20 } }, { columnKey: "DescIndiceMedida" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('CadastroIndicadorMedida_dGridTcsIndicadorIndice', 'DescIndiceMedida', ui.oldValue, ui.value);}}, maxLength: 60 } }, { columnKey: "LimiteInferior" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('CadastroIndicadorMedida_dGridTcsIndicadorIndice', 'LimiteInferior', ui.oldValue, ui.value);}},  maxLength: 21, maxValue: 999999999999999.99999, minValue: 0, dataMode: 'decimal', minDecimals: 5, maxDecimals: 5 } }, { columnKey: "LimiteSuperior" , editorType: 'numeric', editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('CadastroIndicadorMedida_dGridTcsIndicadorIndice', 'LimiteSuperior', ui.oldValue, ui.value);}},  maxLength: 21, maxValue: 999999999999999.99999, minValue: 0, dataMode: 'decimal', minDecimals: 5, maxDecimals: 5 } }, { columnKey: "Rgb" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('CadastroIndicadorMedida_dGridTcsIndicadorIndice', 'Rgb', ui.oldValue, ui.value);}}, maxLength: 12 } }],
                                      ...
                                      editCellStarted: function(evt, ui){
                                          ...
                                          var columns = $('#CadastroIndicadorMedida_dGridTcsIndicadorIndice').igGridUpdating('option', 'columnSettings');
                                          var currentCol = null;
                                          currentRow = ui.rowID;
                                          ...
                                          if (currentCol != null && currentCol.hasOwnProperty('editorType') && currentCol.editorType === 'combo') {
                                             ...
                                             if (lookUpName != null) {
                                                 ...
                                                 if (lstRefreshDados.length === 0)
                                                     vm.dataCombo.fillDataCombos(lookUpName, ui.columnKey, vm.currentDataItem().currentTcsIndicadorIndice(), function (result) {
                        ...
                        if ((typeof vm.OnDataGridCreated === 'function')){
                            vm.OnDataGridCreated('CadastroIndicadorMedida_dGridTcsIndicadorIndice');
                        }
                        ...
                        selectionrowselectionchanged = function (evt, ui) {
                            if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                    $(document).undelegate('#CadastroIndicadorMedida_dGridTcsIndicadorIndice', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                    ...
                                    $(document).delegate ('#CadastroIndicadorMedida_dGridTcsIndicadorIndice', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                }
                                selectGridCurrentItem(vm.goToKey, 'RowDataId', ui, vm.currentDataItem().currentTcsIndicadorIndice, getDataSource()); 
                             } 
                        };
                        $(document).delegate('#CadastroIndicadorMedida_dGridTcsIndicadorIndice', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        $('#CadastroIndicadorMedida_dGridTcsIndicadorIndice > tbody tr').live('focus', function(evt) {
                            var grid = $('#CadastroIndicadorMedida_dGridTcsIndicadorIndice'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                    ...
                    vm.addDataSource({ key: 'CadastroIndicadorMedida_dGridTcsIndicadorIndice', name: 'TcsIndicadorIndiceList', itemsSource: itemsSource });
                }
            
            
            , ChangedBrandscyCadastroIndicadorMedida_dGrid: function(vm, decimals, reset) {
                var i, format = '0.'.concat('0'.repeat(decimals)), grd =$('#scyCadastroIndicadorMedida_dGrid').data('igGrid'),
                    grdUpd = $('#scyCadastroIndicadorMedida_dGrid').data('igGridUpdating');
                if(isNull(grd) || isNull(grdUpd)) return;
                for (i = 0; i < grd.options.columns.length; i++) {
                }
                for (i = 0; i < grdUpd.options.columnSettings.length; i++) {
            ...
            , renderscyCadastroIndicadorMedida_dGrid: function(vm) {
                var getDataSource = function() {
                    var source = null;
                    try {
                        source = vm.dataView;
                ...
                $('#scyCadastroIndicadorMedida_dGrid_headers').live('focus  keydown', function (evt) {
                    var keyCode = window.event ? evt.which : evt.keyCode;
                    if (keyCode === 9) {
                        var cols = $('#scyCadastroIndicadorMedida_dGrid').igGrid('option', 'columns');
                        var dataView = $('#scyCadastroIndicadorMedida_dGrid').data('igGrid').dataSource._dataView
                        if (dataView.length === 0) return;
                        var firstRow = dataView[0].RowDataId;
                        clear = vm.status() === 'C';
                        if (vm.status() === 'C')
                            $('#scyCadastroIndicadorMedida_dGrid').igGridUpdating('startEdit', firstRow, 0, true);
                        else {
                            var entity = findElementByKey(getDataSource(), 'RowDataId', firstRow);
                            var indexColumn = 0;
                            cols.some(function (entry) {
                                if (entry.key !== 'RowDataId' && !entry.hidden) {
                                    if (verifyCanEditCol(entry.key, clear, entity)) {
                                        $('#scyCadastroIndicadorMedida_dGrid').igGridUpdating('startEdit', firstRow, indexColumn, true);
                                        return true;
                                    }
                                    indexColumn++;
                                }
                ...
                var getVisibleColumns = function(metaDataControl) {
                   if (metaDataControl) return '';
                   var visibleColumns = '';
                   if($('#scyCadastroIndicadorMedida_dGrid').data('igGrid') === undefined) return '';
                   var cols = $('#scyCadastroIndicadorMedida_dGrid').igGrid('option', 'columns');
                   if (cols) {
                     for (var idx = 0; idx < cols.length; idx++) {
                         if (cols[idx].hidden !== true) visibleColumns += (visibleColumns === '' ? '' : ',') + cols[idx].key;
                ...
                var isElementHided = function (grid, forceCreating) {
                  if (!grid) grid = $('#scyCadastroIndicadorMedida_dGrid');
                  return ((!grid[0] || (!forceCreating && grid.parent().width() <= 0)) && !$('#dialog').is(':visible'));
                }
                var refreshData = true;
                var itemsSource = { isElementHided: isElementHided, getVisibleColumns: getVisibleColumns, containerId: 'scyCadastroIndicadorMedida_dGrid_container', dataBind: function (commitData, forceCreating) {
                   var grid = $('#scyCadastroIndicadorMedida_dGrid');
                   if (started && typeof grid.data('igGridUpdating') === 'undefined') { started = false; }
                   if (commitData && started) {
                       if (grid.igGridUpdating('isEditing')) {
                           grid.igGrid('commit');
                   ...
                   if (!started) {
                       ...
                       started = true;
                       commitData = false;
                       $('#scyCadastroIndicadorMedida_dGrid_groupbyarea').addClass('hide');
                   }
                   if (grid.igGridUpdating('isEditing')) {
                        grid.igGridUpdating('endEdit', true);
                   }
                ...
                function verifyCanEditCol(column, clear, entity){
                    switch(column){
                        case 'CodIndicadorMedida': { canEditing = clear; break;}
                        case 'DescIndicadorMedida': { canEditing = clear; break;}
                    }
                    return canEditing;
                };
                function createDataGrid(grid) {
                    ...
                    vm.gridSaveStates[gridId] = {
                        ...
                        closePopover: function () {
                            $('#scyCadastroIndicadorMedida_dGrid_LayoutBtn').igPopover('hide');
                        },
                        applyLayout: function (layoutInfo) {
                            var _this = this;
                            if (isNull(layoutInfo) && (!_this.currentLayout() || _this.currentLayout().Id === 0)) {
                    ...
                    grid.igGrid({ height: (vm.isDependentVM() ? getGridHeightSuggested() * 0.7 : $(window).height() * 0.85), width: '100%',
                        ...
                        cellClick: function(evt, ui) {
                             ...
                             if (typeof vm.OnGridClientClick === 'function') {
                                 entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowKey);
                                 vm.OnGridClientClick('scyCadastroIndicadorMedida_dGrid', ui.colKey, entity);
                             }
                             if (vm.status() != 'Q') {
                                 var grid = $('#scyCadastroIndicadorMedida_dGrid');
                                 var isEditing = grid.igGridUpdating('isEditing');
                        ...
                        columns: [
                            ...
                            { key: 'CodIndicadorMedida', headerText: vm.getLayoutDisplayName('CadastroIndicadorMedida_tbCodIndicadorMedida'), width: '200px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   },
                            { key: 'DescIndicadorMedida', headerText: vm.getLayoutDisplayName('CadastroIndicadorMedida_tbDescIndicadorMedida'), width: '400px', dataType: 'string', columnCssClass: 'ellipsis', format: '', hidden: false, unbound: false, group: null   }
                        ],
                        features: [
                                    { name: 'Sorting', type: 'local', caseSensitive: false, unsortedColumnTooltip: '', sortedColumnTooltip: '',
                                      columnSorting: function (evt, ui) { }
                                    ...
                                    filterDialogOpening: function (evt, ui) {
                                        ...
                                        var dataView = $('#scyCadastroIndicadorMedida_dGrid').data('igGrid').dataSource;
                                        if (dataView.settings.filtering.expressions.length <= 0)
                                            dataView._filteredData = [];
                                        ...
                                        scriptHtml += '  <script>';
                                        scriptHtml += '    var newCol = "' + col + '";';
                                        scriptHtml += '    var newGrid = $("#scyCadastroIndicadorMedida_dGrid");';
                                        scriptHtml += '    var listFilter = [];';
                                        scriptHtml += '    function hideColumn(){';
                                        scriptHtml += '     if ($("#showHideColumn_scyCadastroIndicadorMedida_dGrid")["0"].innerHTML.indexOf("Ocultar") >= 0) {';
                                        scriptHtml += '        $("#showHideColumn_scyCadastroIndicadorMedida_dGrid")["0"].innerHTML = "Mostrar Coluna";';
                                        scriptHtml += '        newGrid.igGridHiding("hideColumn", newCol);';
                                        scriptHtml += '     }';
                                        scriptHtml += '     else{';
                                        scriptHtml += '        $("#showHideColumn_scyCadastroIndicadorMedida_dGrid")["0"].innerHTML = "Ocultar Coluna";';
                                        scriptHtml += '        newGrid.igGridHiding("showColumn", newCol);';
                                        scriptHtml += '     }';
                                        scriptHtml += '    }';
                                        scriptHtml += '    function updateHideButton(){';
                                        scriptHtml += '         var column = $.grep(newGrid.igGrid("option", "columns"), function (element, index) { return element.key == newCol });';
                                        scriptHtml += '         if (column.length > 0){';
                                        scriptHtml += '             $("#showHideColumn_scyCadastroIndicadorMedida_dGrid")["0"].innerHTML = column[0].hidden ? "Mostrar Coluna" : "Ocultar Coluna"';
                                        scriptHtml += '         }';
                                        scriptHtml += '    }';
                                        scriptHtml += '    function orderColumn(dir){';
                                        scriptHtml += '      if(dir == 1){dir = "asc"} else{dir = "desc"}';
                                        ...
                                        scriptHtml += '          <div  style="margin-left: 5px" >';
                                        scriptHtml += '              <div>Propriedade:</div>';
                                        scriptHtml += '              <div id="comboFields_scyCadastroIndicadorMedida_dGrid"></div>';
                                        scriptHtml += '              <script>';
                                        scriptHtml += '                  var columns = newGrid.igGrid("option", "columns");';
                                        scriptHtml += '                  $("#comboFields_scyCadastroIndicadorMedida_dGrid").igCombo({ dataSource: columns, mode : "dropdown", valueKey: "key", textKey: "headerText", selectionChanging: function (evt, ui) {';
                                        scriptHtml += '                      newCol = ui.items["0"].data.key;';
                                        scriptHtml += '                      updateHideButton()';
                                        scriptHtml += '                  }});';
                                        scriptHtml += '                  $("#comboFields_scyCadastroIndicadorMedida_dGrid").igCombo("value", newCol);';
                                        scriptHtml += '              </script>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '     <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">';
                                        ...
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <div style="margin-left: 5px; margin-top: 5px">';
                                        scriptHtml += '              <i class="fa fa-eye-slash" aria-hidden="true" style="margin-right: 5px;"></i><a onclick="hideColumn()" style="cursor: pointer" id="showHideColumn_scyCadastroIndicadorMedida_dGrid">Ocultar Coluna</a>';
                                        scriptHtml += '          </div>';
                                        scriptHtml += '          <br>';
                                        scriptHtml += '      </div>';
                                        scriptHtml += '  </div>';
                                    ...
                                    { name: 'Tooltips', columnSettings:[{ columnKey: "CodIndicadorMedida", allowTooltips: true },{ columnKey: "DescIndicadorMedida", allowTooltips: true }] },
                                    { name: 'Resizing' }, 
                                    { name: 'Hiding', 
                                    },
                                    { name: 'MultiColumnHeaders' }
                                   ...
                                   ,{ name: 'Updating', horizontalMoveOnEnter: true,
                                      ...
                                      showReadonlyEditors: false,
                                      showDoneCancelButtons: false,
                                      columnSettings: [{ columnKey: "CodIndicadorMedida" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyCadastroIndicadorMedida_dGrid', 'CodIndicadorMedida', ui.oldValue, ui.value);}}, maxLength: 20 } }, { columnKey: "DescIndicadorMedida" , editorOptions: {valueChanged: function(evt, ui){if(typeof vm.OnPropertyChangeDataGrid === 'function'){vm.OnPropertyChangeDataGrid('scyCadastroIndicadorMedida_dGrid', 'DescIndicadorMedida', ui.oldValue, ui.value);}}, maxLength: 60 } }],
                                      rowDeleting: function (evt, ui) {
                                          deletedIndex = ui.element.context.rowIndex;
                                          var entity = findElementByKey(getDataSource(), 'RowDataId', ui.rowID);
                                          if (entity) {
                                      ...
                                      editCellStarted: function(evt, ui){
                                          var lstRefreshDados = null;
                                          var columns = $('#scyCadastroIndicadorMedida_dGrid').igGridUpdating('option', 'columnSettings');
                                          var currentCol = null;
                                          currentRow = ui.rowID;
                                          columns.forEach(function (entry, index) {
                                             if (entry.columnKey === ui.columnKey) currentCol = entry;
                        ...
                        if ((typeof vm.OnDataGridCreated === 'function')){
                            vm.OnDataGridCreated('scyCadastroIndicadorMedida_dGrid');
                        }
                        var selectionrowselectionchanged = null, selectedRowId = -1;
                        selectionrowselectionchanged = function (evt, ui) {
                            if ((ui.owner.grid.selectedRow() && typeof ui.owner.grid.selectedRow().id !== 'undefined') || (ui.owner.grid.selectedRows() && ui.owner.grid.selectedRows().length > 0)) { 
                                if (isNullOrEmpty(ui.owner.selectedRows())|| ui.selectedRows.length <= 1) {
                                    $(document).undelegate('#scyCadastroIndicadorMedida_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                    ui.owner.clearSelection();
                                    ui.owner.selectRow(ui.row.index);
                                    if (vm.status() === 'Q'){
                                        var gridCell = ui.owner.grid;
                                    ...
                                    selectedRowId = ui.row.id;
                                    $(document).delegate ('#scyCadastroIndicadorMedida_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                                }
                                selectGridCurrentItem(vm.goToKey, 'RowDataId', ui); 
                             } 
                        };
                        $(document).delegate('#scyCadastroIndicadorMedida_dGrid', 'iggridselectionrowselectionchanged', selectionrowselectionchanged);
                        $('#scyCadastroIndicadorMedida_dGrid > tbody tr').live('focus', function(evt) {
                            var grid = $('#scyCadastroIndicadorMedida_dGrid'), row = $(this).closest('tr'), id = parseInt(row.attr('data-id'), 10);
                            var selectedRows = grid.igGridSelection('option', 'multipleSelection') ? grid.igGridSelection('selectedRows') : [grid.igGridSelection('selectedRow')];
                            if (selectedRowId === id) return;
                            selectedRowId = id;
                            grid.igGridSelection('selectRowById', id);
                        ...
                        grid.delegate('.ui-iggrid-activerow', 'dblclick', function (e) {
                            if (vm.CadastroIndicadorMedida().status() === 'Q') vm.CadastroIndicadorMedida().dataToolbar.viewInfo();
                        });
                    }
                    vm.addDataSource({ key: 'scyCadastroIndicadorMedida_dGrid', name: 'dataView', itemsSource: itemsSource });
                }
