define(['durandal/app', 'pkg_linx-framework-bv-spa/services/AplicativoContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-framework-bv-spa/viewmodels/CadastroAplicativoComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroAplicativo', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroAplicativo(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroAplicativo' ]; settings.parentVM.CadastroAplicativo = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.CadastroAplicativo = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('CadastroAplicativo', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroAplicativo');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroAplicativo'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroAplicativo', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroAplicativo', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#CadastroAplicativo_tcTcsAplicativoTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#CadastroAplicativo_tcTcsAplicativoTabControl');

    complement.renderCadastroAplicativo_dGridTcsAplicativoConexao(vm);

    complement.renderCadastroAplicativo_dGridTcsAplicacao(vm);

    complement.renderscyCadastroAplicativo_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                               complement.ChangedBrandCadastroAplicativo_dGridTcsAplicativoConexao(vm, decimals, reset);
                           complement.ChangedBrandCadastroAplicativo_dGridTcsAplicacao(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsAplicativo';
    ...
    var visibleColumns = 'IdTcsAplicativo,DescricaoAplicativo';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsAplicativo', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsAplicativo', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsAplicativo', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsAplicativo');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsAplicativo');
        ...
        return dataContext.getTcsAplicativoByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsAplicativo'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsAplicativoByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsAplicativo(getBandeiraRede(), clearComplete);
    ...
    var createTcsAplicativo = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsAplicativo();
    ...
    var createAndNotifyTcsAplicativo = function() {
        var entity = createTcsAplicativo();
        notifyPresentation('');
        return entity;
    };
    
    var createTcsAplicativoConexao = function(parent, noCurrent) {
        dataBind('TcsAplicativoConexaoList', true);
        var entity = dataContext.createTcsAplicativoConexao(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsAplicativoConexao(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsAplicativoConexao = function(parent) {
        var entity = createTcsAplicativoConexao(parent);
        notifyPresentation('TcsAplicativoConexaoList');
        return entity;
    };
    
    var createTcsAplicacao = function(parent, noCurrent) {
        dataBind('TcsAplicacaoList', true);
        var entity = dataContext.createTcsAplicacao(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsAplicacao(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsAplicacao = function(parent) {
        var entity = createTcsAplicacao(parent);
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroAplicativo_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroAplicativo_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroAplicativo_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsAplicativoConexaoNomeConexao: ko.observable(null),
        TcsAplicacaoDescricaoAplicacao: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsAplicativoConexaoNomeConexao(null);
        entitySearchRange.TcsAplicacaoDescricaoAplicacao(null);
    };
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroAplicativo_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: 'TCS_APLICATIVO',
            rootDataTypeName: 'TcsAplicativo',
            rootNamespace: 'Linx.Framework.BV.Aplicativo',
            ...
            createAndNotifyTcsAplicativo: createAndNotifyTcsAplicativo,
            createTcsAplicativoConexao: createTcsAplicativoConexao,
            createAndNotifyTcsAplicativoConexao: createAndNotifyTcsAplicativoConexao,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroAplicativo',
            pivots : pivots
        };
