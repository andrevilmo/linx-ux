define(['durandal/app', 'pkg_linx-framework-bv-spa/services/TransacaoContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-framework-bv-spa/viewmodels/CadastroTransacaoComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroTransacao', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroTransacao(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroTransacao' ]; settings.parentVM.CadastroTransacao = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.CadastroTransacao = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('CadastroTransacao', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroTransacao');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroTransacao'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroTransacao', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroTransacao', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#CadastroTransacao_tcTcsTransacaoTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#CadastroTransacao_tcTcsTransacaoTabControl');

    complement.renderCadastroTransacao_dGridTcsTransacaoMenuChild(vm);

    complement.renderCadastroTransacao_dGridTcsTransacaoDependente(vm);

    complement.renderscyCadastroTransacao_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                                   complement.ChangedBrandCadastroTransacao_dGridTcsTransacaoMenuChild(vm, decimals, reset);
                                   complement.ChangedBrandCadastroTransacao_dGridTcsTransacaoDependente(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsTransacao';
    ...
    var visibleColumns = 'CodTransacao,NomeCurto,DescTransacao,ClasseNome,Inativo,DescObjeto,LxTipoTransacao,Icone';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        ...
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsTransacao', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsTransacao', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsTransacao', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsTransacao');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsTransacao');
        ...
        return dataContext.getTcsTransacaoByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsTransacao'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsTransacaoByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
    ...
    var control = $lx(vm, '#gbTransacaoMenu');

if(control.length){
	
      setVisibility(vm, 'gbTransacaoMenu',action != "Add");
    ;
}
...
return true;
    ...
    $lx(vm,'#CadastroTransacao_tbTagInput').on('itemAdded', function(event) {
	UpdateTagValue();
});

$lx(vm,'#CadastroTransacao_tbTagInput').on('itemRemoved', function(event) {
	UpdateTagValue();
});
...
var tag = $lx(vm,'#CadastroTransacao_tbTagInput').val();
...
$lx(vm,'#CadastroTransacao_tbTagInput').tagsinput('removeAll');
	
if (status() === 'Q'){
	$lx(vm, '#CadastroTransacao_tbTagInput').attr('disabled', 'disabled');
}
else{
	$lx(vm, '#CadastroTransacao_tbTagInput').removeAttr('disabled');
}
    }
...
tag.split(',').forEach(function(e) {
	$lx(vm,'#CadastroTransacao_tbTagInput').tagsinput('add', e);
})
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsTransacao(getBandeiraRede(), clearComplete);
    ...
    var createTcsTransacao = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsTransacao();
    ...
    var createAndNotifyTcsTransacao = function() {
        var entity = createTcsTransacao();
        notifyPresentation('');
        return entity;
    };
    
    var createTcsTransacaoMenuChild = function(parent, noCurrent) {
        dataBind('TcsTransacaoMenuChildList', true);
        var entity = dataContext.createTcsTransacaoMenuChild(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       ...
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsTransacaoMenuChild(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsTransacaoMenuChild = function(parent) {
        var entity = createTcsTransacaoMenuChild(parent);
        notifyPresentation('TcsTransacaoMenuChildList');
        return entity;
    };
    
    var createTcsTransacaoDependente = function(parent, noCurrent) {
        dataBind('TcsTransacaoDependenteList', true);
        var entity = dataContext.createTcsTransacaoDependente(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsTransacaoDependente(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsTransacaoDependente = function(parent) {
        var entity = createTcsTransacaoDependente(parent);
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroTransacao_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroTransacao_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroTransacao_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsTransacaoDescObjeto: ko.observable(null),
        TcsTransacaoMenuChildDescModuloMenu: ko.observable(null),
        TcsTransacaoMenuChildDescModulo: ko.observable(null),
        TcsTransacaoDependenteClasseNome: ko.observable(null),
        TcsTransacaoDependenteDescTransacao: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsTransacaoDescObjeto(null);
        entitySearchRange.TcsTransacaoMenuChildDescModuloMenu(null);
        entitySearchRange.TcsTransacaoMenuChildDescModulo(null);
        entitySearchRange.TcsTransacaoDependenteClasseNome(null);
        entitySearchRange.TcsTransacaoDependenteDescTransacao(null);
    };
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroTransacao_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: 'TCS_TRANSACAO',
            rootDataTypeName: 'TcsTransacao',
            rootNamespace: 'Linx.Framework.BV.Transacao',
            ...
            createAndNotifyTcsTransacao: createAndNotifyTcsTransacao,
            createTcsTransacaoMenuChild: createTcsTransacaoMenuChild,
            createAndNotifyTcsTransacaoMenuChild: createAndNotifyTcsTransacaoMenuChild,
            createTcsTransacaoDependente: createTcsTransacaoDependente,
            createAndNotifyTcsTransacaoDependente: createAndNotifyTcsTransacaoDependente,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroTransacao',
            pivots : pivots
        };
