define(['durandal/app', 'pkg_linx-framework-bv-spa/services/UtilitariosContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-framework-bv-spa/viewmodels/AdministracaoCacheComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'AdministracaoCache', Items: [
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_AdministracaoCache(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'AdministracaoCache' ]; settings.parentVM.AdministracaoCache = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.AdministracaoCache = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('AdministracaoCache', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('AdministracaoCache');
                           }
                           else {
                               parentVM.internalUIs = ['AdministracaoCache'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-AdministracaoCache', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: AdministracaoCache', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        complement.renderscyAdministracaoCache_dGrid(vm);
    ...
    var visibleColumns = 'Geral,Conexao,Relatorio,Modulo,BandeiraRede,NomeUsuario';
...
vm.showProcessing('Limpando Cache...');
	...
	$.ajax({
		...
		success: function (data) {
			vm.closeProcessing();
			app.showMessage(("Processo concluído com sucesso.").toString(), 'Informação', ['Ok']);
		}
	});
    ...
    var _canRefreshData = true, _canQuickSearch = true, _canAddNew = false, _canClear = false, _canCustomSearch = false, _canDelete = false, _canEdit = false, _canLayout = false, _canNavigate = false, _canPrint = false, _canSearch = false, _canExport = false, _noBusyLoading = false;
    ...
    var removeFormViewControl = function () {
        var front = $('#AdministracaoCache_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#AdministracaoCache_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#AdministracaoCache_formViewer')[0];
        if (panel) {
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#AdministracaoCache_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootNamespace: 'Linx.Framework.BV.Utilitarios',
            ...
            ClearCache_Click: ClearCache_Click,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/AdministracaoCache',
            pivots : pivots
        };
