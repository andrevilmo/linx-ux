...
namespace Linx.EntityAdapterDesigner
{
    public partial class EntityAdapter
    {
        ...
        public void UpdateEntitySets()
        {
            ...
            bool isObjectContext = (model.EdmInfo.EdmType == EdmReader.EdmTypeEnum.ObjectContext);
            ...
            foreach (string entityRef in entities)
            {
                ...
                memberType = model.EdmInfo.GetType(model.TargetNamespace + "." + typeName);
                ...
                if (isObjectContext)
                {
                    foreach (PropertyInfo propInfo in members.Where(m => m.MemberType == MemberTypes.Property).Select(m => (PropertyInfo)m))
                    {
                        ...
                        {
                            foreach (EdmScalarPropertyAttribute attrib in propInfo.GetCustomAttributes<EdmScalarPropertyAttribute>(true))
                            {
                                if (attrib.EntityKeyProperty)
                                {
                ...
                {
                    foreach (PropertyInfo pKey in members.Where(m => !m.GetCustomAttribute<KeyAttribute>().IsNull() && m is PropertyInfo).Select(m => (PropertyInfo)m))
                        ...
                        if (this.IsAggregationView)
                        {
                            ...
                            else
                            {
                                ...
                                if (entityAttribute.AggregationFunction == UIAggregationFunctions.CountDistinct)
                                    selectProperties.Add(prop.Name + " = rg0.Select(e => e.q." + entityAttribute.Name + ").Distinct().Count()");
                                else
                    ...
                    switch (eq.RelationType)
                    {
                        case QueryNodeType.Entity:
                            ...
                            foreach (var prop in eq.Properties.Where(e => e.Selected).OrderBy(e => e.Name))
                            {
                                ...
                                {
                                    selectProperties.Add(prop.Name + " = " + prop.Formula.Replace("this.", eq.Alias + "."));
                                }
                            }
            ...
            if (!this.PrimaryEntity.IsNullOrEmpty())
            {
                ...
                if (defUI != null)
                {
                    if (!defUI.LayoutContent.IsNullOrEmpty())
                    {
                        ...
                        if (layout != null)
                            return layout.EnableMedias && (this.GetAllInheritanceProperties().Count(e => (e.Datatype.ToLower().Contains("guid") || e.Datatype.ToLower().InList("int", "int32", "system.int32")) && this.IsPrimaryKey(e)) == 1);
                    }
                }
            }
            return false;
        ...
        {
            return this.TargetEntityAdapter != null && !this.TargetEntityAdapter.GetDetailRelationBySetName(this.PrimaryEntity).IsNullOrEmpty();
        }

        public string GetEntitiesSelectionForLinq(bool isDbContext, string alias, bool byEntitySearch, string indent, bool isDetail = false, string parentAlias = "")
        {
            string result = String.Empty;
            ...
            if (this.EntityAdapterRepresentation == null)
            {
                ...
                if (parentAlias.IsNullOrEmpty() || this.TargetEntityAdapter == null)
                    ...
                    return "from " + alias + " in " + parentAlias + "." + this.TargetEntityAdapter.GetDetailRelationBySetName(this.PrimaryEntity) + result;
            }
            else
            {
        ...
        public string GetDetailRelationBySetName(string setName)
        {
                            ...
                            foreach (CustomizedCode.PublicationProperty member in pubEntity.Properties)
                            {
                                if (!usedFields.Contains(member.Name))
                                {
                                    ...
                                    result.Add(new CommonExtendedFilterProperty()
                                    {
                                        ...
                                        EdmKey = MacroEngine.ReplaceMacros(member.EdmKey, true)
                                    });
                                }
                            }
                        }
        ...
        public string GetRepresentationDomainServiceSubmitCommands(string indent, string contentTester = "")
        {
            ...
            foreach (var ds in GetRepresentationDomainServices(true))
            {
                commandLine = "\r\n " + indent + ds.Value + ".SubmitData(this.ServiceContext, entityChanges.Where(e => e.Mark == \"" + ds.Value + "\").ToList());";
                if (contentTester.IsNullOrEmpty() || !contentTester.Contains(commandLine))
                    result += commandLine;
            }
            return result;
        ...
        public Dictionary<string, string> GetEdmEntities(string alias)
        {
            ...
            foreach (var prop in this.GetAllInheritanceAttributes())
            {
                ...
                if (MacroEngine.HasMacro(reference))
                    continue;
        ...
        public List<EntityAdapterAttribute> GetAllAttributes()
        {
            ...
            return attributes; //.OrderBy(e => e.DisplayOrder).ToList();
        }
        ...
        public List<EntityAdapterAttribute> GetAllInheritanceAttributes(bool byParentComposition = false)
        {
            ...
            return (byParentComposition ? result.Where(e => !e.IsCustomized && !(e is EntityAdapterFormula)).ToList() : result);
            ...
            if (link != null)
            {
                ...
                if (!parentKeyFields.IsNullOrEmpty() && !detailKeyFields.IsNullOrEmpty())
                {
                    if (isJS)
                    {
                        ...
                        association = @"[Association(""FK_" + this.TargetEntityAdapter.Name + @"_" + this.Name + @""", """ + (isForeignKey ? detailKeyFields : parentKeyFields) + @""", """ + (!isForeignKey ? detailKeyFields : parentKeyFields) + @""", IsForeignKey=" + isForeignKey.ToString().ToLower() + @")]";
                }
            }
            return association;
        }
        ...
        public string GetEnumDefinitions(string indent, bool byParentComposition)
        {
            ...
            {
                if (!byParentComposition || !fieldDef.IsCustomized)
        ...
        public string GetSaveMedia(string indent)
        {
            ...
            if (this.HasEnabledMedias())
            {
                var pk = this.GetAllInheritanceProperties().FirstOrDefault(e => (e.Datatype.ToLower().Contains("guid") || e.Datatype.ToLower().InList("int", "int32", "system.int32")) && this.IsPrimaryKey(e));
                if (pk != null)
                {
        ...
        public void GetJsWhereDetailRelation(Linx.Tools.CodeBuilder codeBuilder, string parentAlias)
        {
            EntityAdapterReferencesTargetEntityAdapter link = this.GetParentLinkRelation();
            if (link != null)
            {
                codeBuilder.Add("            .withParameters({ jEntitySearch: ownerReference.GetJsWhereDetailRelationFor" + this.Name + "() })");
            }
        }

        public void GenerateJsWhereDetailRelationMethod(Linx.Tools.CodeBuilder codeBuilder, string parentAlias)
        {
            codeBuilder.AddLine("   ownerReference.GetJsWhereDetailRelationFor" + this.Name + " = function() {");
            ...
            if (link != null)
            {
                ...
                if (!this.TargetEntityAdapter.IsDashboardFilter && parentKeys.Length > 0 && parentKeys.Length == detailKeys.Length)
                {
                    ...
                    for (int idx = 0; idx < detailKeys.Length; idx++)
                    {
                        ...
                        codeBuilder.Add((link.RemoveFieldIfEmpty ? "(isNullOrEmpty(getAbsoluteValue(" + parentAlias + "." + parentKeys[idx] + ")) ? '' : " : "") + "'" + (this.EnableQueryByParent ? parentKeys[idx] : detailKeys[idx]) + "#==#' + " + parentAlias + ".serverDataType['" + parentKeys[idx] + "'] + " + ((parentAttr != null && parentAttr.Datatype.ToLower().Contains("date")) ? "convertDateToString(getAbsoluteValue(" + parentAlias + "." + parentKeys[idx] + "))" : "getAbsoluteValue(" + parentAlias + "." + parentKeys[idx] + ").toString()") + (link.RemoveFieldIfEmpty ? ")" : "") + (idx < detailKeys.Length - 1 ? " + ';' + " : ""));
                    }
                }

                if (this.TargetEntityAdapter.ExistsClientEvent("OnDetailSearching"))
                {
                    codeBuilder.Add(" + function() { var filter = ownerReference.OnDetailSearching('" + this.Name + "');" + (this.EnableMetaDataFilter && !this.EnableQueryByParent ? " if (!isNullOrEmpty(filter) && filter.indexOf('{LinqValidProperties#') === -1) filter += vm.getVisibleProperties('" + this.Name + "List');" : "") + " return (!isNullOrEmpty(filter) && filter.indexOf('{') == -1 ? ';' + filter + '}' : '}' + filter); }()");
                }
                else
                {
                    codeBuilder.Add(" + '}'");

                    if (this.EnableMetaDataFilter && !this.EnableQueryByParent)
        ...
        public string GetJsTestDetailRelation(string parentAlias)
        {
            ...
            if (link != null)
            {
                if (!link.ParentKeyFields.IsNullOrEmpty() && !link.DetailKeyFields.IsNullOrEmpty())
                {
                    ...
                    if (parentKeys.Length == detailKeys.Length)
                    {
                        for (int idx = 0; idx < detailKeys.Length; idx++)
                        {
                            associationTest += (associationTest.IsNullOrEmpty() ? "" : " && ") + "!isNullOrEmpty(getAbsoluteValue(" + parentAlias + "." + parentKeys[idx] + "))";
                        }
                    }
                }
            }
        ...
        {
            return GetAssociation(isForeignKey, false);
        }
        ...
        public string GetRelationFieldsToLinq(string alias, string indent, bool byParentComposition, bool checkValidProps)
        {
            ...
            foreach (EntityAdapterAttribute attribute in (byParentComposition ? this.GetAllInheritanceAttributes().Where(e => !(e is EntityAdapterFormula) && !e.IsCustomized) : this.GetAllInheritanceAttributes()))
            {
                if (attribute is EntityAdapterProperty)
                {
                    ...
                    relationFieldsToLinq += "\r\n" + indent + (relationFieldsToLinq.IsNullOrEmpty() ? "" : ", ") + property.Name + " = " + (this.EnableMetaDataFilter && checkValidProps ? "(has" + attribute.Name + ".Contains(true) || !hasNo" + attribute.Name + ".Contains(true) ? " : "") + (prefixLinqMethod.IsNullOrEmpty() ? "" : prefixLinqMethod + "(") + (isRepresentation ? (!property.DataRelationKey.Contains("#") ? property.DataRelationKey : property.DataRelationKey.Left("#") + "." + property.DataRelationKey.Right(".")) : (property.DenormalizedDataInfo.IsNullOrEmpty() ? ReplaceEdmPath(property.EdmKey, alias) : "normalizedEntity" + property.Name + ".Value")) + (prefixLinqMethod.IsNullOrEmpty() ? "" : ")") + (suffixLinqMethod.IsNullOrEmpty() ? "" : "." + suffixLinqMethod) + (this.EnableMetaDataFilter && checkValidProps ? " : " + this.GetDefaultByDataType(attribute.Datatype) + ")" : "");
                }
                else if (attribute is EntityAdapterPublicationProperty)
                {
                    pProperty = (EntityAdapterPublicationProperty)attribute;
                    relationFieldsToLinq += "\r\n" + indent + (relationFieldsToLinq.IsNullOrEmpty() ? "" : ", ") + pProperty.Name + " = " + (this.EnableMetaDataFilter && checkValidProps ? "(has" + attribute.Name + ".Contains(true) || !hasNo" + attribute.Name + ".Contains(true) ? " : "") + (isRepresentation ? (!pProperty.DataRelationKey.Contains("#") ? pProperty.DataRelationKey : pProperty.DataRelationKey.Left("#") + "." + pProperty.DataRelationKey.Right(".")) : ReplaceEdmPath(pProperty.EdmKey, alias)) + (this.EnableMetaDataFilter && checkValidProps ? " : " + this.GetDefaultByDataType(attribute.Datatype) + ")" : "");
                }
                else if (attribute is EntityAdapterFormula)
                {
                    ...
                    {
                        relationFieldsToLinq += "\r\n" + indent + (relationFieldsToLinq.IsNullOrEmpty() ? "" : ", ") + formula.Name + " = " + (this.EnableMetaDataFilter && checkValidProps ? "(has" + attribute.Name + ".Contains(true) || !hasNo" + attribute.Name + ".Contains(true) ? " : "") + (isRepresentation ? (!formula.DataRelationKey.Contains("#") ? formula.DataRelationKey : formula.DataRelationKey.Left("#") + "." + formula.DataRelationKey.Right(".")) : ReplaceEdmPath(formula.LinqDefinition, alias)) + (this.EnableMetaDataFilter && checkValidProps ? " : " + this.GetDefaultByDataType(attribute.Datatype) + ")" : "");
                    }
                }
            }
        ...
        public string GetGroupingFieldsToLinq(string alias, string indent, bool checkValidProps)
        {
            ...
            foreach (EntityAdapterAttribute attribute in this.GetAllInheritanceAttributes().Where(e => e.AggregationFunction == UIAggregationFunctions.None))
            {
                if (attribute is EntityAdapterProperty)
                {
                    property = (EntityAdapterProperty)attribute;
                    string prefixLinqMethod = property.GetPrefixLinqMethod(), suffixLinqMethod = property.GetSuffixLinqMethod();
                    relationFieldsToLinq += "\r\n" + indent + (relationFieldsToLinq.IsNullOrEmpty() ? "" : ", ") + attribute.Name + " = " + (this.EnableMetaDataFilter && checkValidProps ? "(has" + attribute.Name + ".Contains(true) || !hasNo" + attribute.Name + ".Contains(true) ? " : "") + (prefixLinqMethod.IsNullOrEmpty() ? "" : prefixLinqMethod + "(") + (isRepresentation ? (!property.DataRelationKey.Contains("#") ? property.DataRelationKey : property.DataRelationKey.Left("#") + "." + property.DataRelationKey.Right(".")) : ReplaceEdmPath(property.EdmKey, alias)) + (prefixLinqMethod.IsNullOrEmpty() ? "" : ")") + (suffixLinqMethod.IsNullOrEmpty() ? "" : "." + suffixLinqMethod) + (this.EnableMetaDataFilter && checkValidProps ? " : " + this.GetDefaultByDataType(attribute.Datatype) + ")" : "");
                }
                else if (attribute is EntityAdapterPublicationProperty)
                {
                    pProperty = (EntityAdapterPublicationProperty)attribute;
                    relationFieldsToLinq += "\r\n" + indent + (relationFieldsToLinq.IsNullOrEmpty() ? "" : ", ") + attribute.Name + " = " + (this.EnableMetaDataFilter && checkValidProps ? "(has" + attribute.Name + ".Contains(true) || !hasNo" + attribute.Name + ".Contains(true) ? " : "") + (isRepresentation ? (!pProperty.DataRelationKey.Contains("#") ? pProperty.DataRelationKey : pProperty.DataRelationKey.Left("#") + "." + pProperty.DataRelationKey.Right(".")) : ReplaceEdmPath(pProperty.EdmKey, alias)) + (this.EnableMetaDataFilter && checkValidProps ? " : " + this.GetDefaultByDataType(attribute.Datatype) + ")" : "");
                }
                else if (attribute is EntityAdapterFormula)
                {
                    formula = (EntityAdapterFormula)attribute;
        ...
        public string GetAggregationFieldsToLinq(string groupAlias, string alias, string ident, bool checkValidProps)
        {
            string relationFieldsToLinq = String.Empty;
            EntityAdapterProperty property;
            EntityAdapterPublicationProperty pProperty;
        ...
        public EntityAdapterProperty GetTemporaryKey()
        {
            if (!this.IsAggregationView)
            {
                ...
                {
                    //Verify Primary key            
                    List<EntityAdapterProperty> details = keys.Where(e => e.Datatype.InList("System.Int32", "Int32")).ToList();
                    if (details.Count == 1)
        ...
        public string GetEntitiesDescription()
        {
            ...
            descriptionReturn += ";SubQueryInfo[" + (this.TargetEntityAdapter == null ? "" : "Select 1 From #ParentAlias#." + this.TargetEntityAdapter.GetDetailRelationBySetName(this.PrimaryEntity) + " as #Alias#") + "]";
        ...
        public bool IsPrimaryKey(EntityAdapterProperty property)
        {
            ...
            if (this.IsAggregationView || this.GetCurrentDataModel() == null || this.EntitySets.IsNullOrEmpty())
                return property.IsPK;
            else
            {
                string keyFields = "," + this.EntitySets.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries)[0].Extract("[", "]") + ",";
        ...
        private string GetAttributeDefinitions(EntityAdapterAttribute fieldDef, string contextName, Dictionary<string, string> lookUpsPropInfo, string indent, int memberOrder, bool isPoco, bool hasDynamicPK, bool isFreeProperty, bool byParentComposition = false)
        {
            bool isRequired = (!isPoco && !isFreeProperty && !fieldDef.RemoveValidations && (!fieldDef.IsNull || fieldDef.IsCompulsory));
            string body = "\r\n\r\n" + indent + (fieldDef.IgnoreDataMember ? "[Exclude()]" : "[DataMember(" + (isRequired ? "IsRequired = true, " : String.Empty) + "Name = \"" + (fieldDef.DataMemberName.IsNullOrEmpty() ? fieldDef.Name : fieldDef.DataMemberName) + "\", EmitDefaultValue = " + this.DataMemberEmitDefaultValue.ToString().ToLower() + (memberOrder > 0 ? ", Order = " + memberOrder : "") + ")]");
            body += "\r\n" + indent + "[XmlAttribute()]";
            body += "\r\n" + indent + "[Editable(true)]";
            body += "\r\n" + indent + @"[Display(Name = """ + fieldDef.DisplayName + @""", Description=""" + fieldDef.Description + @""", Order = " + fieldDef.DisplayOrder.ToString() + @", AutoGenerateField = " + fieldDef.IsBrowsable.ToString().ToLower() + @", GroupName="""", ResourceType= null)]";
            ...
            if (!isPoco)
            {
                ...
                if (byParentComposition && this.IsParentCompositionAllowed() && !edmKey.IsNullOrEmpty())
                {
                    if ((fieldDef is EntityAdapterProperty && ((EntityAdapterProperty)fieldDef).EntityAdapter != this))
                    {
                        ...
                        {
                            relation += (relation.IsNullOrEmpty() ? "" : ".") + this.GetParentRelationName();
                            parent = parent.TargetEntityAdapter;
                        }
        ...
        public bool ExcludeAsFilter(EntityAdapterAttribute fieldDef)
        {
            ...
            if (MacroEngine.HasMacro(fieldDef.GetEdmPath(true)))
                return false;
                ...
                if (fields.Length > 0)
                {
                    ...
                    if (propertyRelation != null)
                    {
                        ...
                        if (!bmParentRelation.IsNullOrEmpty() && !bmParentName.IsNullOrEmpty())
                        {
                            ...
                            foreach (EntityAdapterAttribute fieldDef in properties.Where(e => !(e is EntityAdapterFormula) && !e.IsCustomized && !attributes.Contains(e.Name)))
                            {
                                attributes.Add(fieldDef.Name);
                                string edmPath = fieldDef.GetEdmPath(true);
                                if (!edmPath.IsNullOrEmpty())
                                {
                                    if (edmPath.StartsWith(parent.PrimaryEntity + "."))
                                    {
                                        if (edmPath.Left(".") == bmParentName)
                                            edmPath = ("#" + edmPath).Right("#" + edmPath.Left(".") + ".");
                                        body += "\r\n" + indent + ", " + fieldDef.Name + " = " + entityParent + "." + edmPath;
                                    }
                                    else
        ...
        public string GetPropertyDefinitions(string contextName, string indent, bool byParentComposition, List<string> attributes, bool isPoco = false, bool generateFreeProperty = false)
        {
            ...
            string body = String.Empty, formula, tmpName;
            ...
            foreach (EntityAdapterAttribute fieldDef in (byParentComposition ? this.GetAllInheritanceAttributes(byParentComposition).Where(e => !attributes.Contains(e.Name)) : this.GetAllAttributes().Where(e => !attributes.Contains(e.Name))))
            {
                ...
                if (!isPoco)
                {
                    ...
                    body += "\r\n" + indent + "partial void On" + fieldDef.Name + "Changing(" + fieldDef.Datatype + " value);";
                    body += "\r\n" + indent + "partial void On" + fieldDef.Name + "Changed();";
                }

                if (createPropertyField)
                    body += "\r\n\r\n" + indent + "private " + fieldDef.Datatype + " _" + fieldDef.Name + ";";
                ...
                body += "\r\n" + indent + "public " + fieldDef.Datatype + " " + fieldDef.Name;
                body += "\r\n" + indent + "{";
                ...
                else
                {
                    ...
                    if (createPropertyField)
                    {
                        ...
                        if (formula.IsNullOrEmpty() && fieldDef.Datatype.ToLower().Contains("string") && (this.IsIndependentKey(fieldDef) || ((fieldDef is EntityAdapterProperty && this.IsPrimaryKey(((EntityAdapterProperty)fieldDef))) || (!fieldDef.IsNull && this.IsAggregationView && !isFreeProperty && fieldDef.AggregationFunction == UIAggregationFunctions.None))))
                        {
            ...
            if (!isPoco)
            {
                ...
                if (!byParentComposition)
                {
                    ...
                    if (!temporaryKey.IsNull())
                    {
                        ...
                        body += "\r\n" + indent + "[DataMember(Name = \"EntityKeyLocalRelation\", EmitDefaultValue = " + this.DataMemberEmitDefaultValue.ToString().ToLower() + (!this.GenerateDataMemberOrder ? "" : ", Order = " + this.GetMemberOrder(tmpName)) + ")]";
                        body += "\r\n" + indent + "[XmlAttribute()]";
                        body += "\r\n" + indent + "[Editable(true)]";
        ...
        private string GenReportDesignerFile(string reportName, bool isMaster, string title, string[] reportChildEntities = null, string[] reportChildNames = null, Dictionary<string, List<string>> propertySelection = null)
        {
            ...
            if (isMaster)
            {
                ...
                headerFields = "this.TextBoxHeader, this.TextBoxDateTime, this.PictureBox1, this.TextBoxCompanyName";
                ...
                footerFields = "this.TextBoxPageCount, this.TextBoxFilter";
            ...
            method.Statements.Add(new CodeSnippetStatement(string.Format("this.Width = new Telerik.Reporting.Drawing.Unit({0}, Telerik.Reporting.Drawing.UnitType.Cm);", totalSize > pageSize ? "28.686336517333984D" : "20D")));
        ...
        public void AddClientEvent(string eventName)
        {
            ...
            switch (eventName)
            {
                ...
                case "OnDetailSearching":
                    ...
                    customEvent.Parameters = "string detailName";
                    break;
                ...
                case "OnPropertyChanged":
                    customEvent.ReturnType = "void";
                    customEvent.Parameters = "string propertyName#object oldValue#object newValue";
                    break;
                default:
                    if (LookUpAdapter.IsOnLookedUp(eventName))
                    {
                        customEvent.Parameters = "object lookUpElement";
                    }
                    ...
                    {
                        customEvent.Parameters = "object data";
                    }
                    break;
            }
