...
namespace Linx.EntityAdapterDesigner.CustomCode
{
    ...
    public partial class FrmEntityBuilder : Form
    {
        ...
        private bool IsObjectContext
        {
            get
            {
                return this.model.EdmInfo.EdmType == EdmReader.EdmTypeEnum.ObjectContext;
            }
        }
        ...
        private void FillTypes()
        {
            ...
            if (this.model != null && this.entity != null)
            {
                ...
                {
                    this.txEdmContext.Text = this.model.EdmInfo.ContextType.FullName;

                    cmbEdmTypes.Items.Clear();

                    var edmSets = this.model.EdmInfo.GetEdmsSet();
                    var allEdmTypes = this.model.EdmInfo.GetTypes();

                    foreach (var memberType in edmSets)
                    {
                        if (!memberType.IsAbstract && (memberType.BaseType == typeof(object) || memberType.BaseType == typeof(EntityObject)))
                        {
                        ...
                        //Inherited types
                        foreach (var specializedType in allEdmTypes.Where(e => !e.IsAbstract &&
        ...
        private List<string> GetPKOfEntity(string EntityName)
        {
            ...
            if (!type.IsNull())
            {
                ...
                if (IsObjectContext)
                {
                    foreach (PropertyInfo propInfo in members.Where(m => m is PropertyInfo).Select(m => (PropertyInfo)m))
                    {
                            ...
                            foreach (EdmScalarPropertyAttribute attrib in attributes)
                            {
                                if (attrib is EdmScalarPropertyAttribute)
                                {
                                    if (attrib.EntityKeyProperty)
                                        pks.Add(propInfo.Name.PrepareName());
                ...
                {
                    foreach (PropertyInfo pKey in
                        members.Where(m => !m.GetCustomAttribute<KeyAttribute>().IsNull() && m is PropertyInfo).Select(m => (PropertyInfo)m))
        ...
        private void ApplyChanges()
        {
            ...
            for (int propIndex = 0; propIndex < propertiesList.Count; propIndex++)
            {
                ...
                if (compare != null)
                {
                    ...
                    compare.PublicationRelatedKey = propertiesList[propIndex].PublicationRelatedKey;
                    compare.PublicationSuffix = propertiesList[propIndex].PublicationSuffix;
                    compare.ForceAsFilter = propertiesList[propIndex].ForceAsFilter;
                    compare.DataRelationKey = propertiesList[propIndex].DataRelationKey;
                    compare.IsMeasure = propertiesList[propIndex].IsMeasure;
                    compare.MeasureFormula = propertiesList[propIndex].MeasureFormula;
                    compare.IgnoreForQuery = propertiesList[propIndex].IgnoreForQuery;
                    compare.IsAutomaticSequency = propertiesList[propIndex].IsAutomaticSequency;
                    compare.CustomMediaTable = propertiesList[propIndex].CustomMediaTable;
                    compare.DenormalizedDataInfo = propertiesList[propIndex].DenormalizedDataInfo;
                    compare.LinqMethod = propertiesList[propIndex].LinqMethod;
                    ...
                    if (node.Tag.ToString() == EdmReader.IsProperty)
                    {
                        if (node.Checked)
                        {
                            if (this.Entity.EntityAdapterProperties.Where(e => !e.IsDeleted && e.IsCustomized && e.EdmKey == node.Name).Count() == 0)
                            {
                                ...
                                if (hasNullableJoin && property.Datatype != "System.String" && !property.Datatype.Contains("Nullable<") && !property.Datatype.Contains("?"))
