...
namespace Linx.EntityAdapterDesigner.CustomCode
{
    public partial class frmBusinessViewBuilder : Form
    {
        ...
        public void PopulateTypes()
        {
            ...
            if (_curDM != null)
            {
                var edmSets = _curDM.EdmInfo.GetEdmsSet();
                ...
                foreach (var memberType in edmSets)
                {
                    ...
                    try
                    {
                        PublicationEntity bmEntity = new PublicationEntity()
                        {
                            ...
                            Namespace = memberType.Namespace,
                            CompositionHierarchy = "",
                            DisplayName = memberType.Name.Replace("_", " ").Proper(),
                            EdmName = _curDM.EdmInfo.ContextType.Name,
                            EntitiesDescription = ""
                        };

                        foreach (var propInfo in memberType.GetProperties().Where(p => p.GetCustomAttribute<ColumnAttribute>() != null))
                        {
                            var displayAttr = propInfo.GetCustomAttribute<DisplayAttribute>();
                            var funcPoint = propInfo.GetCustomAttribute<FunctionalPoint>();
                            var pubField = propInfo.GetCustomAttribute<LinxPublicationFieldAttribute>();
                            var fKey = propInfo.GetCustomAttribute<ForeignKeyAttribute>();
                            var column = propInfo.GetCustomAttribute<ColumnAttribute>();
                            var precision = propInfo.GetCustomAttribute<PrecisionAttribute>();
                            var scale = propInfo.GetCustomAttribute<ScaleAttribute>();
                            var maxlength = propInfo.GetCustomAttribute<MaxLengthAttribute>();
                            var required = propInfo.GetCustomAttribute<RequiredAttribute>();

                            PublicationProperty pubProp = new PublicationProperty()
                            {                                
                                Name = propInfo.Name,
                                DisplayName = (displayAttr != null ? displayAttr.Name : propInfo.Name.Replace("_", " ").Proper()),
                                IsPrimaryKey = (propInfo.GetCustomAttribute<KeyAttribute>() != null),
                                DataType = (propInfo.PropertyType.FullName.Contains("Nullable") ? "System.Nullable<" + propInfo.PropertyType.GetElement().FullName + ">" : propInfo.PropertyType.FullName),
                                IsNull = (required == null),
                                DomainName = (funcPoint == null ? "" : funcPoint.FunctionName.Extract("DomainName[", "]")),
                                DisplayOrder = (column == null ? 0 : column.Order),
                                Precision = (precision != null && scale != null ? precision.Value.ToString() + ":" + scale.Value.ToString() : (maxlength != null ? maxlength.Length.ToString() + ":0" : "0:0"))
                            };
        ...
        private void LoadClassRelations(TreeNode node)
        {
            ...
            {
                foreach (var link in relatedClass.GetAllLinkDefinitions(_curDM.EdmInfo.ContextType.Assembly))
                {
