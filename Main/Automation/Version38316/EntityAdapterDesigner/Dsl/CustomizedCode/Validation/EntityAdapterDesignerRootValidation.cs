...
namespace Linx.EntityAdapterDesigner
{
    ...
    public partial class EntityAdapterDesignerRoot
    {
        ...
        private void ValidateStateNamesUnique(ValidationContext context)
        {
            ...
            try { document = GetDiagramProjectItem(); }
            catch (Exception ex)
            {
        ...
        private void CheckEdmReferencesByEntityRepresentation(EnvDTE.Project diagramProject)
        {
            ...
            foreach (var rep in this.EntityAdapterRepresentations.Where(e => !e.BusinessObject.IsNullOrEmpty() && !e.TargetEdmName.IsNullOrEmpty()))
            {
                ...
                {
                    this.AddReference(diagramProject, edmLib, System.IO.Path.GetFileName(rep.BusinessObject) == System.IO.Path.GetFileName(this.GetOutputLibFile()));
                }
            }
        }
        ...
        List<string> GetAssemblyNames()
        {
            ...
            //Add BVs from SPAs
            foreach (var fileName in assemplyNames.Where(e => e.Right(4) == ".spa").Select(e => e.Left(e.Length - 4)).ToArray())
            {
        ...
        private void AdjustProjects()
        {
            ...
            if (!diagramProject.IsNull())
            {
                ...
                this.AddReference(diagramProject, "Linx.LinqExtensions.dll");
                ...
                if (!this.ExistsReference(diagramProject, "System.IdentityModel.dll"))
                    this.AddReference(diagramProject, "System.IdentityModel.dll");

                if (!this.ExistsReference(diagramProject, "System.Net.Http.dll"))
                    this.AddReference(diagramProject, "System.Net.Http.dll");

                //Update WF suport references
                if (!this.ExistsReference(diagramProject, "PresentationFramework.dll"))
                    this.AddReference(diagramProject, "PresentationFramework.dll");
                if (!this.ExistsReference(diagramProject, "PresentationCore.dll"))
                    this.AddReference(diagramProject, "PresentationCore.dll");
                if (!this.ExistsReference(diagramProject, "System.Xaml.dll"))
                    this.AddReference(diagramProject, "System.Xaml.dll");
                if (!this.ExistsReference(diagramProject, "System.Activities.dll"))
                    this.AddReference(diagramProject, "System.Activities.dll");
                if (!this.ExistsReference(diagramProject, "System.Activities.Presentation.dll"))
                    this.AddReference(diagramProject, "System.Activities.Presentation.dll");

                //AdoMD
                this.UpdateLibReferences(diagramProject, "Linx.AdomdClient", false, false, true);

                //MEF Extension
                if (!this.ExistsReference(diagramProject, "System.ComponentModel.Composition"))
                    this.AddReference(diagramProject, "System.ComponentModel.Composition");
        ...
        private void UpdateTemplates()
        {
            ...
            try
            {
                this.UpdateDataEntityFunctionsTemplate();
                this.UpdateEntityAdapterDynamicModelsTemplate();
                this.UpdateEntityAdapterDynamicModelsTemplate(true);
                ...
                {
                    //this.GenerateWebApiAppStartCode();
                    this.GenerateWebApiClientCode(this.GetEadProject());
                ...
                this.UpdateDomainViewsTemplate("DomainViewsTemplate", "DomainViews");
                this.UpdateDomainViewsTemplate("ClientDataDomains", "DataDomains");

                if (this.KeyPerformanceIndicators.Count > 0)
                    this.UpdateKPIViewsTemplate();

                if (!(this.EntityAdapters.Count == 0 && this.DomainServiceExtensions.Count == 0))
                {
                    this.UpdateHelpService();
                    this.UpdateHelpService(true);
                    ...
                    this.UpdateDomainServiceTemplate();
                    this.UpdateFormulasTemplate();
                    this.UpdateExtendedFiltersTemplate();
                    this.UpdateLookUpsTemplate();
                    this.UpdateStateRulesTemplate();

                    //Operations
                    this.GenerateBusinessEvents(false);
                    this.GenerateBusinessEvents(true);
                    this.GenerateBusinessOperations(false);
                    this.GenerateBusinessOperations(true);
                    this.GenerateDomainServiceExtensions();
                    this.CheckCustomValidationClass();
                    this.GenerateCustomAuthorizationClass();

                    //User Extension Project
                    if (this.HasDomainServiceExtension())
                        this.GenerateDomainServiceInterfaceImplementation();
                }
            }
