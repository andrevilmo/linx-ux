...
namespace Linx.EntityAdapterDesigner
{
    ...
    public class LookUpStruct
    {
        ...
        public static List<LookUpStruct> GetLookUpStructures(ContextEntity dataEntity)
        {
            var lookupDict = new Dictionary<string, string>();

            if (dataEntity != null)
            {
                foreach (var member in dataEntity.Properties.OrderBy(e => e.Name))
                {
                    string relatedName = member.Name.PrepareName();
                    if (!lookupDict.ContainsKey(relatedName))
                    {
                        var value = member.GetCustomAttribute("LinxPublicationField").Right("LookUpInfo=");
