	...
	public static class TextSelectionExtension
	{
		...
		private static void AddAttributes(this CodeFunction function, GenericOperation operation)
		{
            ...
            if (operation is WebApiAction)
            {
                ...
                //Base route
                if (apiAction.EnableAccessControl && apiAction.WebApiController.EntityAdapterDesignerRoot.EnableAutomaticAuthorization && !apiAction.WebApiController.EntityAdapterDesignerRoot.IsAspNetCore)
                    attributes += (attributes.IsNullOrEmpty() ? String.Empty : "#") + apiAction.WebApiController.Name + "ControllerAuthorize";

                if (apiAction.WebApiController.EntityAdapterDesignerRoot.IsAspNetCore)
                {
                    attributes += (attributes.IsNullOrEmpty() ? String.Empty : "#") + "Http" + verb.Proper() + "(\"" + routeactionName + "\")";
                }
                else 
                {
