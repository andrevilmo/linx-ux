...
namespace Linx.EntityAdapterDesigner
{
    ...
    internal sealed class ModelElementStartConfiguration : AddRule
    {
        public override void ElementAdded(ElementAddedEventArgs e)
        {
            ...
            if (root == null)
                root = e.ModelElement.Partition.ElementDirectory.FindElements<EntityAdapterDesignerRoot>().FirstOrDefault();
            if (root != null && root.InitializedModel && !root.HasStructuralChanges)
            {
                root.HasStructuralChanges = true;
            }
    ...
    [RuleOn(typeof(DslModeling::ModelElement), FireTime = TimeToFire.Inline, InitiallyDisabled = false)]
    internal sealed class ModelElementChangeConfiguration : ChangeRule
    {
        public override void ElementPropertyChanged(ElementPropertyChangedEventArgs e)
        {
            if (e.OldValue != e.NewValue)
            {
                var root = e.ModelElement.Partition.ElementDirectory.FindElements<EntityAdapterDesignerRoot>().FirstOrDefault();
                if (root != null && root.InitializedModel && !root.HasStructuralChanges)
                {
                    root.HasStructuralChanges = true;
                }
    ...
    [RuleOn(typeof(DslModeling::ModelElement), FireTime = TimeToFire.Inline, InitiallyDisabled = false)]
    internal sealed class ModelElementDeleteRuleConfiguration : DeleteRule
    {
        ...
        public override void ElementAdded(ElementAddedEventArgs e)
        {
            ...
            if (subscription != null && (subscription.BusinessObjectPath.IsNullOrEmpty() || !File.Exists(subscription.BusinessObjectPath)))
            {
                ...
                if (subscription.BusinessObjectPath.IsNullOrEmpty() || !File.Exists(subscription.BusinessObjectPath))
                {
                    ...
                    string dirName = (subscription.EntityAdapterDesignerRoot.IsAspNetCore ? "Linx.CoreBusinessViews" : "Linx.Business.Objects");
                    fileDlg.InitialDirectory = subscription.EntityAdapterDesignerRoot.GetFullPath(dirName);
                    ...
                    {
                        MessageBox.Show("The selected assembly represents this BO. Each BO subscribes its publications automatically.", "Alert", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                    }
                    ...
                    {
                        MessageBox.Show("This Publisher is already being used!!!", "Alert", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                    }
            ...
            {
                subscription.Publisher = this.GetPublicationStructure(subscription.BusinessObjectPath);
            }

            base.ElementAdded(e);
        }

        internal CustomizedCode.PublicationStructure GetPublicationStructure(string path)
        {
            if (path.IsNullOrEmpty())
                return null;

            CustomizedCode.PublicationStructure result = null;

            string error = "";
            if (!File.Exists(path))
                error = String.Format("Assembly [{0}] does not exist!", path);

            string metaPath = path + ".meta.json";

            if (error.IsNullOrEmpty() && !File.Exists(metaPath))
                error = String.Format("Metadata [{0}] does not exist!", metaPath);

            if (error.IsNullOrEmpty())
            {
                result = new CustomizedCode.PublicationStructure(metaPath, path);
                if (result.HasError)
                    error = String.Format("JSON serialization problem in [{0}]!", metaPath);
            }

            if (!error.IsNullOrEmpty())
            {
                MessageBox.Show(error, "Publication Alert", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return null;
            }
    ...
    [RuleOn(typeof(ClientLocalService), FireTime = TimeToFire.TopLevelCommit, InitiallyDisabled = false)]
    internal sealed class ClientLocalServiceDeletingConfiguration : DeletingRule
    {
        public override void ElementDeleting(ElementDeletingEventArgs e)
        {
            ClientLocalService ui = e.ModelElement as ClientLocalService;
            if (ui != null)
            {
                if (MessageBox.Show("Do you really want to delete the service '" + ui.Name + "'?", "Alert", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                {
                    throw new Exception("Operation cancelled!!!");
                }
                ui.DeleteMobileSourceFiles();
                ui.DeleteClientAppSourceFiles();
            }
            base.ElementDeleting(e);
        }
    }
    ...
    [RuleOn(typeof(ClientLocalService), FireTime = TimeToFire.TopLevelCommit, InitiallyDisabled = false)]
    internal sealed class ClientLocalServiceChangeConfiguration : ChangeRule
    {
        public override void ElementPropertyChanged(ElementPropertyChangedEventArgs e)
        {
            ClientLocalService userInterface = e.ModelElement as ClientLocalService;
            if (userInterface != null)
            {
                //Change File Name
                if (e.DomainProperty.Name == "Name")
                {
                    userInterface.RenameMobileSourceFiles(e.OldValue as string);
                    userInterface.RenameClientErpSourceFiles(e.OldValue as string);
                }
            }
    ...
    public partial class EntityAdapterDesignerDomainModel
    {
        protected override Type[] GetCustomDomainModelTypes()
        {
            return new System.Type[] { typeof(ModelElementStartConfiguration),
                typeof(ModelElementChangeConfiguration),
                typeof(ModelElementDeleteRuleConfiguration),
                ...
                typeof(ClientLocalServiceDeletingConfiguration),
                typeof(ClientLocalServiceChangeConfiguration),
