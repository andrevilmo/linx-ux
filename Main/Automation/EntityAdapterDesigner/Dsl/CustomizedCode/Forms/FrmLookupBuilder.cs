                    ...
                    if (node.Checked)
                    {
                        if (node.Tag.ToString() == Linx.EntityAdapterDesigner.CustomizedCode.EdmReader.IsProperty)
                        {
                            if (this.lookUp.LookUpProperties.Where(e => !e.IsDeleted && e.IsCustomized && e.EdmKey == node.Name).Count() == 0)
                            {
                                ...
                                if ((node.Parent != null && node.Parent.Parent != null) && (!property.Datatype.ToLower().Contains("string") && HasParentNullable(node) && !property.Datatype.Contains("System.Nullable<")))
        ...
        private void FillTypes()
        {
            ...
            if (this.model != null && this.lookUp != null)
            {
                ...
                else
                {
                    ...
                    var edmTypes = this.model.EdmInfo.GetTypes();
                    ...
                    foreach (var memberType in edmTypes)
                    {
                        ...
                        //Inherited types
                        foreach (var specializedType in edmTypes)
                        {
