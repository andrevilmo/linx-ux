    ...
    public class EntityQueryNode
    {
        ...
        internal void SyncPropertiesWithView(EntityAdapter _entity, bool updateFormulas, bool createFormulas, bool syncData)
        {
            if (updateFormulas || createFormulas)
            {
                ...
                {
                    string recision = (attr.Precision.Contains(":") ? attr.Precision : (attr.Precision.IsNullOrEmpty() ? "0" : attr.Precision) + ":0");
                    ...
                    if (prop == null && createFormulas)
                    {
                        this.Properties.Add(new EntityQueryProperty()
                        {
                            ...
                            SourceName = "",
                            Type = attr.Datatype,
                            Precision = short.Parse(recision.Left(":")),
                            Scale = short.Parse(recision.Right(":")),
                            MaxLength = int.Parse(recision.Left(":"))
                        });
                    }
                    else if (prop != null && updateFormulas)
                    {
                        ...
                        prop.Precision = short.Parse(recision.Left(":"));
                        prop.Scale = short.Parse(recision.Right(":"));
                        prop.MaxLength = int.Parse(recision.Left(":"));
                        prop.Selected = true;
                    }
                }
            }
