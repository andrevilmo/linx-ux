...
namespace Linx.EntityAdapterDesigner.GeneratedCode.CustomizedCode
{
    public partial class CustomizingLayout : UserControl
    {
        ...
        public CustomizingLayout()
        {
            ...
            this.cmbRadioPosition.Items.AddRange(new string[] { "Vertical", "Horizontal" });
                ...
                verify = (element) =>
                    {
                        ...
                        {
                            if (element.ColumnSpan == 0 || element.ColumnSpan > 12)
                            {
                                MessageBox.Show("ColumnSpan should be greater than 0 and less or equal than 12.", "UI Inconsistency: [" + this.currentLayout.Name + "]", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                hasError = true;
                            }

                            if (element.ClassName.InList("DataGrid", "TreeListView") && ((LayoutContainer)element).EnableMultiSelection)
                                multiSelectorCount++;
                            ...
                            else
                            {
                                ...
                                foreach (var domainControl in ((LayoutContainer)element).Controls.Where(e => e is LayoutControlV2 && !((LayoutControlV2)e).DomainName.IsNullOrEmpty() && !((LayoutControlV2)e).ClassName.InList(Linx.Tools.LayoutControlClass.ComboBox.ToString(), Linx.Tools.LayoutControlClass.RadioButtonGroup.ToString())).ToArray())
                                {
                                    MessageBox.Show(String.Format("There is a domain field that is not used as Combobox or RadioButtonGroup. Verify the element called [{0}].", (domainControl.DisplayName.IsNullOrEmpty() ? domainControl.Name : domainControl.DisplayName)), "UI Inconsistency: [" + this.currentLayout.Name + "]", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                    hasError = true;
                                    break;
                                }
                                ...
                                if (!hasError)
                                {
                                    ...
                                    var container = ((LayoutContainer)element).Controls.FirstOrDefault(e => e is LayoutContainer && !(element.ClassName.InList("DataGrid", "TreeListView") && e.ClassName == "ExternalUI"));
        ...
        private void FillGroups()
        {
            ...
            if (entityAdapter != null)
            {
                ...
                foreach (var element in entityAdapter.Properties.Where(e => e.IsBrowsable).OrderBy(r => r.DisplayOrder))
                {
                    ...
                    {
                        if (lcontainer.PivotDataSource == "Olap")
                            continue;
        ...
        private void AdjustAuxiliaryInformations(LayoutControlV2 lControl, PublicationProperty element, bool isOnLoad)
        {
            ...
            //todo: implementar a verificacao do RemoveValidations, para o controle.
            lControl.IsNullable = element.IsNull || element.RemoveValidations;
            lControl.BrandDecimalsControl = element.BrandDecimalsControl;

            if (!lControl.AllowNegativeValue && element.IsPrimaryKey)
                lControl.AllowNegativeValue = true;
            if (lControl.DomainName != element.DomainName)
                lControl.DomainName = element.DomainName;

            //Adjust Domain Control
            if (!lControl.DomainName.IsNullOrEmpty() && !lControl.ClassName.InList(Linx.Tools.LayoutControlClass.ComboBox.ToString(), Linx.Tools.LayoutControlClass.RadioButtonGroup.ToString()))
            {
                lControl.ClassName = Linx.Tools.LayoutControlClass.ComboBox.ToString();
            }
        ...
        private void AdjustAllAuxiliaryInformations(bool isOnLoad)
        {
            ...
            if (parentEntity == null && !this.layoutDefinition.RemoveDataToolbar) return;

            Action<LayoutElement, LayoutElement> ajustInfo = null;
            ajustInfo = (element, parentElement) =>
            {
                if (element is LayoutContainer)
                {
                    if (((LayoutContainer)element).ColumnSpan == 0)
                    {
                        ((LayoutContainer)element).ColumnSpan = 12;
                    }

                    if (((LayoutContainer)element).ColumnCount == 0)
                    {
                        ((LayoutContainer)element).ColumnCount = 1;
                    }

                    //Adjus DataGrid Editing
                    if (((LayoutContainer)element).ClassName.InList("DataGrid", "TreeListView"))
                    {
                    ...
                    ((LayoutContainer)element).Controls.ForEach(e => ajustInfo(e, element));
                }
                else if (element is LayoutControlV2)
                {
                    var control = ((LayoutControlV2)element);
                    if (control.DataGridWidth == 0)
                    {
                        int ctrlWidth = 100;
                        if (parentElement.ClassName != "TreeListView")
                        {
                            ctrlWidth = (control.ControlWidth != ControlWidth.Automatic ? Builder.Resources.HtmlCodeGen.GetMaxWidth(control.ControlWidth) :
                                    Builder.Resources.HtmlCodeGen.GetElementWidth(control.ClassName, control.DataType, control.DisplayName, control.ClassName == "MultimediaControl" ? control.MediaWidth.ToString() : control.DataFormatString, true, control.Precision,
                                    (parentElement != null ? parentElement.ClassName == "TreeListView" : false)));
                        }
                        control.DataGridWidth = ctrlWidth;
                    }

                    if (control.ColumnSpan == 0)
                    {
                        if (control.ControlWidth != ControlWidth.Automatic)
                        {
                            switch (control.ControlWidth)
                            {
                        ...
                        ((LayoutControlV2)element).ControlWidth = ControlWidth.Automatic;
                        }
                        else
                        {
                            var _width = LayoutCodeGen<CustomizedLayoutV2>.GetControlWidth(control.ClassName, control.DataType, control.DisplayName, control.DataFormatString, control.Precision);
                            switch (_width)
                            {
                                case ControlWidth.Mini:
                                    control.ColumnSpan = 1;
            ...
            this.layoutDefinition.Containers.ForEach(e => ajustInfo(e, null));
        }
                ...
                if (lGroup == null)
                {
                    ...
                    if (lGroup == null)
                    {
                        ...
                        if (lGroup == null)
                        {
                            ...
                            if (lGroup == null)
                            {
                                lGroup = new LayoutContainer() { Name = entity.Name, ClassName = "DataGrid", DisplayName = string.Empty, ImageIndex = GetIconIndexFromClassName("DataGrid"), ParentName = newPage.Name };
        ...
        {
            return (ClassName == "Expander" || ClassName == "GroupBox" || ClassName == "CustomContainer" || ClassName == "TabItem" || ClassName.InList("DataGrid", "TreeListView") || ClassName == null);
        }
        ...
        private void PrepareMenu(TreeNode NodeSelected)
        {
            ...
            if (treeTopGroups.SelectedNode.Tag is LayoutContainer)
            {
                ...
                if (((LayoutContainer)treeTopGroups.SelectedNode.Tag).ClassName.InList("DataGrid", "TreeListView"))
                {
                    mnuConvertToTreeListView.Visible = true;
                    mnuContentControl.Visible = true;
                    mnuConvertToDatagrid.Visible = false;
        ...
        private string GetEntityAdapter(LayoutContainer currentLayout)
        {
            var entityAdapter = string.Empty;

            if (currentLayout.Controls != null && currentLayout.Controls.Any())
                entityAdapter = currentLayout.Controls.First().ParentName;

            return entityAdapter;
        }

        private void UpdateTreeView(string entityData = null)
        {
            var currentEntityAdapterName = ((LayoutContainer)selectedTree.SelectedNode.Tag).EntityData;

            if (entityData.IsNullOrEmpty())
            {
                ((LayoutContainer)selectedTree.SelectedNode.Tag).Controls.Clear();
                selectedTree.SelectedNode.Nodes.Clear();
            }
            else if (currentEntityAdapterName != entityData)
            {
                ((LayoutContainer)selectedTree.SelectedNode.Tag).Controls.Clear();
                selectedTree.SelectedNode.Nodes.Clear();

                var entityAdapter = this.GetEntityAdapterByName(entityData);

                if (entityAdapter != null)
                {
                    var bindingPath = this.GetBindingPath(entityAdapter);

                    ((LayoutContainer)selectedTree.SelectedNode.Tag).BindingPath = bindingPath;

                    foreach (var property in entityAdapter.Properties)
                    {
                        var lControl = new LayoutControlV2()
                        {
                            Name = property.Name,
                            BindingPath = bindingPath + "." + property.Name,
                            Sync = true,
                            DataType = property.DataType,
                            SourceViewName = entityData,
                            DisplayName = property.DisplayName,
                            ImageIndex = GetIconIndexFromClassName(property.DisplayControl.ToString()),
                            ClassName = property.DisplayControl.ToString(),
                            ActionEvent = String.Empty,
                            IsCustomized = false,
                            IsDataField = true,
                            IsVisible = property.IsBrowsable,
                            AggregationFunction = property.AggregationFunction.ToString(),
                            IsEditable = property.IsEditable,
                            ConnectedAttribute = property.ConnectedAttribute,
                            DataFormatString = property.DataFormatString,
                            Precision = property.Precision,
                            ToolTip = property.Description,
                            IsMeasure = property.IsMeasure,
                            MeasureFormula = property.MeasureFormula,
                            Mask = property.Mask,
                            MaskType = property.MaskType,
                            BrandDecimalsControl = property.BrandDecimalsControl
                        };
                        ((LayoutContainer)selectedTree.SelectedNode.Tag).Controls.Add(lControl);

                        lControl.ImageIndex = GetIconIndexFromClassName(lControl.ClassName);

                        var newNode = selectedTree.SelectedNode.Nodes.Add(lControl.Name, lControl.DisplayName,
                            GetIconIndexFromClassName(lControl.ClassName), GetIconIndexFromClassName(lControl.ClassName));

                        newNode.Tag = lControl;
                    }
                }
            }
        }
        ...
        private void tree_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            if (selectedTree != null && !selectedTree.SelectedNode.IsNull() && selectedTree.SelectedNode.Tag is LayoutElement)
            {
                ...
                this.tabCustomControl.IsAccessible = (selectedTree.SelectedNode.Tag is LayoutControlV2 && className == "CustomControl");
                ...
                //this.chkEditorWithinGrid.Visible = (className.InList("DataGrid", "TreeListView"));
                this.ckHasGroupBy.Visible = (className.InList("DataGrid", "TreeListView"));
                this.chkColumnFixing.Visible = (className.InList("DataGrid", "TreeListView"));
                this.lblPageSize.Visible = this.numPageSize.Visible = (className.InList("DataGrid", "TreeListView"));
                this.ckEnableMultiSelection.Visible = (className.InList("DataGrid", "TreeListView"));
                this.ckEnableGridSelector.Visible = (className.InList("DataGrid", "TreeListView"));
                this.ckDgEnableVirtualization.Visible = (className.InList("DataGrid", "TreeListView"));
                //this.ckSelectorStartOpen.Visible = (className.InList("DataGrid", "TreeListView"));
                this.txtFieldsSelector.Visible = (className.InList("DataGrid", "TreeListView"));
                this.txtDisplaySelector.Visible = (className.InList("DataGrid", "TreeListView"));
                this.grdSeletorGrid.Visible = (className.InList("DataGrid", "TreeListView"));
                ...
                this.grdGridDataOptions.Visible = (className.InList("DataGrid", "TreeListView"));
                ...
                this.cmbRadioPosition.Visible = className.Equals("RadioButtonGroup");
                this.lblRadioButtonPosition.Visible = className.Equals("RadioButtonGroup");
                ...
                this.grDataFormat.Visible = className.InList("NumericTextBox", "DateTimeTextBox", "Dashboard", "ComboBox");
                if (this.lblStringFormatHelp.Visible)
                    this.lblStringFormatHelp.Text = (className == "NumericTextBox" ? "Ex: N2 or C2 or #,##0.00 CR;#,##0.00 DB;#,##0.00" : (className == "ComboBox" ? "ex: none, * none is used to remove the thousand separator." : "Ex: d (02/09/2014), D (Segunda-feira, 2 de Setembro de 2014), t (13:09), T (13:09:53), g (02/09/2014 13:09), G (02/09/2014 13:09:53) or dd/MM/yyyy HH:mm:ss"));

                this.lblColumnHeaderGrid.Visible = this.txtColumnHeaderGrid.Visible = isDataField;
                ...
                if (this.tabGroupInformations.IsAccessible)
                {
                    ...
                    this.txGroupDisplayName.Text = layoutContainer.DisplayName;
                    ...
                    if (this.chkColumnFixing.Visible)
                        this.chkColumnFixing.Checked = layoutContainer.HasColumnFixing;
                    ...
                    //Adjust deprecated label position
                    if (layoutContainer.LabelPosition == LabelPosition.Left)
                        layoutContainer.LabelPosition = LabelPosition.Top;
                    ...
                    this.cboGridWidth.Visible = this.lblGridWidth.Visible = layoutContainer.ClassName.InList("DataGrid", "TreeListView");
                    if (this.cboGridWidth.Visible)
                        this.cboGridWidth.SelectedItem = layoutContainer.GridWidth.ToString();



                    grpLightDataGrid.Visible = (className == "TreeListView");
                    this.ckEnableFilterTextInGrid.Checked = layoutContainer.EnableFilterTextInGrid;
                    this.chkAutoFitLastColumn.Checked = layoutContainer.AutoFitLastColumn;
                    this.nbGridVisibleRowsNumber.Value = layoutContainer.GridVisibleRowsNumber;


                    this.cboGridHeight.Visible = this.lblGridHeight.Visible = layoutContainer.ClassName.InList("DataGrid", "FlatPivotGrid");
                    ...
                    if (this.chkEditorWithinGrid.Visible)
                        this.chkEditorWithinGrid.Checked = layoutContainer.EditorWithinGrid;

                    this.txtFieldsSelector.Text = layoutContainer.SelectorGridColumns;
                    this.txtDisplaySelector.Text = layoutContainer.DisplaySelectorGridColumns;

                    if (this.ckEnableGridSelector.Visible)
                        this.ckEnableGridSelector.Checked = layoutContainer.EnableGridSelector;

                    if (this.ckSelectorStartOpen.Visible)
                        this.ckSelectorStartOpen.Checked = layoutContainer.StartOpenSelector;

                    if (this.ckDgEnableVirtualization.Visible)
                        this.ckDgEnableVirtualization.Checked = layoutContainer.Virtualization;
                    ...
                    if (this.grpExternalUI.Visible)
                    {
                        ...
                        this.ckGrNoBusyLoading.Checked = ((LayoutContainer)selectedTree.SelectedNode.Tag).NoBusyLoading;
                    }
                ...
                if (this.tabFieldInformations.IsAccessible)
                {
                    ...
                    this.grpLookupAutoComplete.Visible = className == cLookUpTextBox;
                    this.ckEnableLookupAutoComplete.Checked = layoutElement.EnableLookupAutoComplete;
                    this.ckEnableLookupAutoComplete.Enabled = layoutElement.DataType.ToString().ToLower().Contains("string");//só habilita se for string
                    this.numLookupAutoCompleteMaxResult.Value = layoutElement.LookupAutoCompleteMaxResults;
                    ...
                    this.nbDataGridWidth.Value = layoutElement.DataGridWidth;
                    ...
                    this.ckWordWrap.Checked = layoutElement.DataGridWordWrap;
                    this.ckAutoFitColumn.Checked = layoutElement.GridColAutoFit;
                    this.cmbControlWidth.SelectedItem = layoutElement.ControlWidth.ToString();
                    this.cmbRadioPosition.SelectedItem = layoutElement.RadioPosition;
                    ...
                    this.txtColumnHeaderGrid.Text = layoutElement.ColumnMultiHeader;
                    this.txtHtmlContent.Text = layoutElement.HtmlCode;
                ...
                else if (this.tabPivotGrid.IsAccessible)
                {
                    ...
                    this.txPivotCubeName.Text = layoutContainer.PivotCube;
                    this.ckbCallServiceOkEvent.Checked = layoutContainer.CallServiceOkEvent;
                    this.cbxPivotRelationshipLoadData.Checked = layoutContainer.PivotRelationshipQuery;
                    this.cbxIsLayoutToolbar.Checked = layoutContainer.IsLayoutToolbar;
                    this.cbxPivotToggleView.Checked = layoutContainer.PivotToggleView;
                    this.cbxFullScreen.Checked = layoutContainer.PivotFullScreen;
                    this.cbxOpenReport.Checked = layoutContainer.PivotOpenReport;

                    var spaCodeGen = new SpaCodeGen(currentLayout.EntityAdapterDesignerRoot);
                    var layoutFiles = spaCodeGen.GetLayoutFiles(currentLayout, layoutContainer.GetControlName(""), layoutContainer.PivotFileLayout);

                    this.cmbLayoutPivotFile.Items.Clear();
                    this.cmbLayoutPivotFile.Items.AddRange(layoutFiles.ToArray());

                    var selectedItem = this.cmbLayoutPivotFile
                                          .Items.Cast<ComboboxItem>()
                                          .FirstOrDefault(item => item.Value == layoutContainer.PivotFileLayout);

                    this.cmbLayoutPivotFile.SelectedIndex =
                        (selectedItem != null) ? this.cmbLayoutPivotFile.FindStringExact(selectedItem.Text) : 0;

                    this.cboEntityData.Items.Clear();
                    foreach (var entityAdapter in currentLayout.EntityAdapterDesignerRoot.EntityAdapters.ToList())
                        this.cboEntityData.Items.Add(entityAdapter.Name);

                    if (!layoutContainer.PivotDataSource.IsNullOrEmpty())
                        this.cboDataSource.SelectedItem = layoutContainer.PivotDataSource;
                    else
                    {
                        this.cboDataSource.SelectedItem = "Olap";
                    }

                    if (!layoutContainer.EntityData.IsNullOrEmpty())
                        this.cboEntityData.SelectedItem = layoutContainer.EntityData;
                    else
                    {
                        this.cboEntityData.SelectedItem = "";
                    }


                    if (this.cboDataSource.SelectedItem.ToString() == "Olap")
                    {
                        groupEntityData.Visible = false;
                        groupPivotCubeName.Visible = true;
                    }
                    else if (this.cboDataSource.SelectedItem.ToString() == "Entity Data")
                    {
                        groupEntityData.Visible = true;
                        groupPivotCubeName.Visible = false;
                    }

                    if (!layoutContainer.PivotViewType.IsNullOrEmpty())
                        this.cmbViewType.SelectedItem = layoutContainer.PivotViewType;
                    else
                        this.cmbViewType.SelectedItem = "Grid";

                    if (!layoutContainer.PivotChartTypeGridChart.IsNullOrEmpty())
                        this.cmbModelChart.SelectedItem = layoutContainer.PivotChartTypeGridChart;
                    else
                        this.cmbModelChart.SelectedItem = "bar";

                    if (!layoutContainer.PivotChartPosition.IsNullOrEmpty())
                        this.cmbPositionChartPivot.SelectedItem = layoutContainer.PivotChartPosition;
                    else
                        this.cmbPositionChartPivot.SelectedItem = "left";

                    if (!layoutContainer.PivotEixoMeasure.IsNullOrEmpty())
                        this.cmbEixoMeasure.SelectedItem = layoutContainer.PivotEixoMeasure;
                    else
                        this.cmbEixoMeasure.SelectedItem = "Default";

                    this.txtPivotDimensionsRows.Text = layoutContainer.PivotDimensionsRowsCustom;
                    this.txtPivotDimensionsCols.Text = layoutContainer.PivotDimensionsColsCustom;
                    this.txtPivotMeasure.Text = layoutContainer.PivotMeasuresCustom;
                    this.txtPivotTopData.Text = layoutContainer.PivotTopData;
        ...
        {
            LayoutContainer lGroup = new LayoutContainer() { Name = "Expander_" + Guid.NewGuid().ToString().Replace("-", String.Empty), DisplayName = string.Empty, ImageIndex = 0, ColumnCount = 2, ClassName = "Expander" };
            ...
            if (targetNode != null)
            {
                ...
                if (e.Data.GetDataPresent(typeof(TreeNode)))
                {
                    ...
                    if (nodSource != targetNode && !IsParentNode(nodSource, targetNode))
                    {
                        ...
                        {
                            if (((LayoutContainer)targetNode.Tag).ClassName == "TabControl" && ((LayoutContainer)nodSource.Tag).ClassName.InList("DataGrid", "TreeListView"))
                            {
                                tabNodeControl = CreateTabItem(targetNode);
                                treeTopGroups.Nodes.Remove(nodSource);
                                tabNodeControl.Nodes.Add(nodSource);
                        ...
                        {
                            if (((LayoutContainer)targetNode.Tag).ClassName.InList("DataGrid", "TreeListView") && ((LayoutContainer)nodSource.Tag).ClassName.InList("DataGrid", "TreeListView"))
                            {
        ...
        {
            return className.InList("CheckBox", "RadioButtonGroup", "ComboBox", "EditBox", "EconomicGroup", cLookUpTextBox, "NumericTextBox", "TextBox", "TextBlock", "MaskedTextBox", "KpiBox", "Gauge", "DateTimeTextBox", "ColorPicker", "CustomControl");
        }
        ...
        private TreeNode CreateWizardControlAndWizardItem(TreeNode selNode)
        {
            ...
            {
                this.layoutDefinition.Containers.Add(lpage);
            }
        ...
        private void AddTreeContainer(string className)
        {
            ...
            LayoutContainer lGroup = new LayoutContainer() { Name = className + "_" + Guid.NewGuid().ToString().Replace("-", String.Empty), DisplayName = string.Empty, ImageIndex = GetIconIndexFromClassName(className), ColumnCount = 2, ClassName = className };
            ...
            SetDefaultValuesForContainer(lGroup, className);
        ...
        private void ConvertToElement(TreeNode expandTreeNode, string elementName)
        {
            ...
            //lctrl.DisplayName = string.Empty;

            expandTreeNode.ImageIndex = lctrl.ImageIndex;
            expandTreeNode.SelectedImageIndex = lctrl.ImageIndex;

            //getting default values for controls
            SetDefaultValuesForContainer(lctrl, elementName);

            if (expandTreeNode.Text.IsNullOrEmpty())
                expandTreeNode.Text = lctrl.ClassName;
        ...
        private void SetDefaultValuesForContainer(LayoutContainer lctrl, string elementName)
        {
            if (elementName.InList("FlatPivotGrid"))
            {
                lctrl.GridHeight = GridSizeHeight.Small;
            }
        }
            ...
            if (grpExternalUI.Visible && !selectedTree.IsNull() && !selectedTree.SelectedNode.IsNull() && sender is CheckBox && selectedTree.SelectedNode.Tag is LayoutContainer)
            {
                switch (((CheckBox)sender).Name)
                {
                    ...
                    case "ckGrCanNavigate":
                        ...
                        break;
                    case "ckGrNoBusyLoading":
                        ((LayoutContainer)selectedTree.SelectedNode.Tag).NoBusyLoading = ((CheckBox)sender).Checked;
                        break;
                    default:
                        break;
                }
        ...
        {
            LayoutContainer lc = ((LayoutContainer)selectedTree.SelectedNode.Tag);
            if (this.ckHasGroupBy.Checked)
            {
                if (!this.chkColumnFixing.Checked)
                {
                ...
                {
                    MessageBox.Show("Para habilitar esta função desabilite a função 'Enable Column Fixing'.");
                    lc.HasGroupBy = false;
                    this.ckHasGroupBy.Checked = false;
                }
            }
            else
                lc.HasGroupBy = ((CheckBox)sender).Checked;
        ...
        private void txtHtmlContent_TextChanged(object sender, EventArgs e)
        {
            if (this.tabCustomControl.Visible)
            {
                LayoutControlV2 lc = ((LayoutControlV2)selectedTree.SelectedNode.Tag);
                lc.HtmlCode = txtHtmlContent.Text;
            }
        }
        ...
        {
            LayoutContainer lc = ((LayoutContainer)selectedTree.SelectedNode.Tag);
            if (this.ckEnableMultiSelection.Checked)
            {
                if (!this.chkColumnFixing.Checked)
                {
                    if (this.ckEnableMultiSelection.Visible)
                ...
                {
                    MessageBox.Show("Para habilitar esta função desabilite a função 'Enable Column Fixing'.");
                    lc.EnableMultiSelection = false;
                    this.ckEnableMultiSelection.Checked = false;
                }
            }
            else
                lc.EnableMultiSelection = ((CheckBox)sender).Checked;
        ...
        private void ckbCallServiceOkEvent_CheckedChanged(object sender, EventArgs e)
        {
            if (sender is CheckBox && !selectedTree.IsNull() && !selectedTree.SelectedNode.IsNull() && selectedTree.SelectedNode.Tag is LayoutContainer)
                ((LayoutContainer)selectedTree.SelectedNode.Tag).CallServiceOkEvent = ((CheckBox)sender).Checked;
        }

        private void chkEditorWithinGrid_CheckedChanged(object sender, EventArgs e)
        {
            if (this.chkEditorWithinGrid.Visible)
            {
                LayoutContainer lc = ((LayoutContainer)selectedTree.SelectedNode.Tag);
                lc.EditorWithinGrid = ((CheckBox)sender).Checked;
            }
        }

        private void txtPivotRelationship_TextChanged(object sender, EventArgs e)
        {
            if (sender is TextBox && !selectedTree.IsNull() && !selectedTree.SelectedNode.IsNull() && selectedTree.SelectedNode.Tag is LayoutContainer)
                ((LayoutContainer)selectedTree.SelectedNode.Tag).PivotRelationship = ((TextBox)sender).Text;
        }

        private void cbxPivotRelationshipLoadData_CheckedChanged(object sender, EventArgs e)
        {
            if (sender is CheckBox && !selectedTree.IsNull() && !selectedTree.SelectedNode.IsNull() && selectedTree.SelectedNode.Tag is LayoutContainer)
                ((LayoutContainer)selectedTree.SelectedNode.Tag).PivotRelationshipQuery = ((CheckBox)sender).Checked;
        }

        private void cmbLayoutPivotFile_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (sender is ComboBox && !selectedTree.IsNull() &&
                !selectedTree.SelectedNode.IsNull() &&
                selectedTree.SelectedNode.Tag is LayoutContainer)
            {
                ((LayoutContainer)selectedTree.SelectedNode.Tag).PivotFileLayout = ((ComboboxItem)((ComboBox)sender).SelectedItem).Value.ToString();

            }
        }

        private void txtColumnHeaderGrid_TextChanged(object sender, EventArgs e)
        {
            LayoutControlV2 lc = ((LayoutControlV2)selectedTree.SelectedNode.Tag);
            lc.ColumnMultiHeader = txtColumnHeaderGrid.Text;
        }

        private void chkColumnFixing_CheckedChanged(object sender, EventArgs e)
        {
            LayoutContainer lc = ((LayoutContainer)selectedTree.SelectedNode.Tag);
            if (this.chkColumnFixing.Checked)
            {
                if (!ckHasGroupBy.Checked && !ckEnableMultiSelection.Checked)
                {
                    if (this.chkColumnFixing.Visible)
                        lc.HasColumnFixing = ((CheckBox)sender).Checked;
                }
                else
                {
                    MessageBox.Show("Para habilitar esta função desabilite as funções 'GROUP BY' e/ou 'Multi-Selection'.");
                    lc.HasColumnFixing = false;
                    this.chkColumnFixing.Checked = false;
                }
            }
            else
                lc.HasColumnFixing = ((CheckBox)sender).Checked;
                ...
                selectedTree.SelectedNode.Tag is LayoutContainer)
            {
                switch (cboDataSource.SelectedItem.ToString())
                {
                    case "Olap":
                        ...
                        ((LayoutContainer)selectedTree.SelectedNode.Tag).PivotDataSource = "Olap";
                        ((LayoutContainer)selectedTree.SelectedNode.Tag).EntityData = string.Empty;

                        groupPivotCubeName.Visible = true;
                        groupEntityData.Visible = false;
                        cbxPivotRelationshipLoadData.Visible = false;
                        break;
                    case "Entity Data":
                        ((LayoutContainer)selectedTree.SelectedNode.Tag).PivotDataSource = "Entity Data";

                        groupEntityData.Visible = true;
                        groupPivotCubeName.Visible = false;
                        cbxPivotRelationshipLoadData.Visible = true;
                        break;
                }
            }
        }
        ...
        private void ckEnableGridSelector_CheckedChanged(object sender, EventArgs e)
        {
            if (!this.ckEnableGridSelector.Checked)
            {
                this.grpConfigSelector.Enabled = false;
                this.ckSelectorStartOpen.Checked = false;
                this.txtFieldsSelector.Text = string.Empty;
                this.txtDisplaySelector.Text = string.Empty;
            }
            else
                this.grpConfigSelector.Enabled = true;

            if (this.ckEnableGridSelector.Visible)
            {
                LayoutContainer lc = ((LayoutContainer)selectedTree.SelectedNode.Tag);
                lc.EnableGridSelector = ((CheckBox)sender).Checked;
            }
        }

        private void ckSelectorStartOpen_CheckedChanged(object sender, EventArgs e)
        {
            if (this.ckSelectorStartOpen.Visible)
            {
                LayoutContainer lc = ((LayoutContainer)selectedTree.SelectedNode.Tag);
                lc.StartOpenSelector = ((CheckBox)sender).Checked;
            }
        }

        private void txtFieldsSelector_TextChanged(object sender, EventArgs e)
        {
            LayoutContainer lc = ((LayoutContainer)selectedTree.SelectedNode.Tag);
            lc.SelectorGridColumns = ((TextBox)sender).Text;
        }
        ...
        private void ckWordWrap_CheckedChanged(object sender, EventArgs e)
        {
            if (sender is CheckBox && !selectedTree.IsNull() && !selectedTree.SelectedNode.IsNull())
            {
                if (selectedTree.SelectedNode.Tag is LayoutControlV2)
                {
                    ((LayoutControlV2)selectedTree.SelectedNode.Tag).DataGridWordWrap = ((CheckBox)sender).Checked;
                }
            }
        }
        ...
        private void cmbViewType_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (sender is ComboBox && !selectedTree.IsNull() &&
               ...
               selectedTree.SelectedNode.Tag is LayoutContainer)
            {
                ((LayoutContainer)selectedTree.SelectedNode.Tag).PivotViewType = ((ComboBox)sender).SelectedItem.ToString();
                switch (cmbViewType.SelectedItem.ToString())
                {
                    case "Grid":
                        cmbModelChart.Enabled = false;
                        cmbPositionChartPivot.Enabled = false;
                        break;
                    case "Charts":
                        cmbModelChart.Enabled = true;
                        cmbPositionChartPivot.Enabled = false;
                        break;
                    case "Grid_Charts":
                        cmbModelChart.Enabled = true;
                        cmbPositionChartPivot.Enabled = true;
                        break;
                }
            }
        }

        private void cmbModelChart_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (sender is ComboBox && !selectedTree.IsNull() &&
              !selectedTree.SelectedNode.IsNull() &&
              selectedTree.SelectedNode.Tag is LayoutContainer)
            {
                ((LayoutContainer)selectedTree.SelectedNode.Tag).PivotChartTypeGridChart = ((ComboBox)sender).SelectedItem.ToString();
            }
        }

        private void cmbPositionChartPivot_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (sender is ComboBox && !selectedTree.IsNull() &&
              !selectedTree.SelectedNode.IsNull() &&
              selectedTree.SelectedNode.Tag is LayoutContainer)
            {
                ((LayoutContainer)selectedTree.SelectedNode.Tag).PivotChartPosition = ((ComboBox)sender).SelectedItem.ToString();
            }
        }

        private void cmbEixoMeasure_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (sender is ComboBox && !selectedTree.IsNull() &&
               !selectedTree.SelectedNode.IsNull() &&
               selectedTree.SelectedNode.Tag is LayoutContainer)
            {
                ((LayoutContainer)selectedTree.SelectedNode.Tag).PivotEixoMeasure = ((ComboBox)sender).SelectedItem.ToString();
                if (cmbEixoMeasure.SelectedItem.ToString().ToLower().Contains("default"))
                {
                    txtPivotDimensionsRows.Enabled = false;
                    txtPivotDimensionsCols.Enabled = false;
                    txtPivotMeasure.Enabled = false;
                    txtPivotTopData.Enabled = false;
                    lblDimensaoObrigatoria.Visible = false;
                    lblMedidaObrigatoria.Visible = false;
                }
                else
                {
                    txtPivotDimensionsRows.Enabled = true;
                    txtPivotDimensionsCols.Enabled = true;
                    txtPivotMeasure.Enabled = true;
                    txtPivotTopData.Enabled = true;
                    lblDimensaoObrigatoria.Visible = true;
                    lblMedidaObrigatoria.Visible = true;
                }

            }
        }

        private void txtPivotMeasure_TextChanged(object sender, EventArgs e)
        {
            ((LayoutContainer)selectedTree.SelectedNode.Tag).PivotMeasuresCustom = txtPivotMeasure.Text;
        }

        private void txtPivotDimensionsRows_TextChanged(object sender, EventArgs e)
        {
            ((LayoutContainer)selectedTree.SelectedNode.Tag).PivotDimensionsRowsCustom = txtPivotDimensionsRows.Text;
        }

        private void txtPivotDimensionsCols_TextChanged(object sender, EventArgs e)
        {
            ((LayoutContainer)selectedTree.SelectedNode.Tag).PivotDimensionsColsCustom = txtPivotDimensionsCols.Text;
        }

        private void cmbRadioPosition_SelectedIndexChanged(object sender, EventArgs e)
        {
            ComboBox combobox = sender as ComboBox;
            if (combobox.SelectedIndex == -1) return;

            LayoutControlV2 lc = selectedTree.SelectedNode.Tag as LayoutControlV2;
            lc.RadioPosition = combobox.SelectedItem.ToString();
        }

        private void txtPivotTopData_TextChanged(object sender, EventArgs e)
        {
            ((LayoutContainer)selectedTree.SelectedNode.Tag).PivotTopData = txtPivotTopData.Text;
        }

        private void ckEnableLookupAutoComplete_CheckedChanged(object sender, EventArgs e)
        {
            if (sender is CheckBox && !selectedTree.IsNull() && !selectedTree.SelectedNode.IsNull())
            {
                if (selectedTree.SelectedNode.Tag is LayoutControlV2)
                {
                    ((LayoutControlV2)selectedTree.SelectedNode.Tag).EnableLookupAutoComplete = ((CheckBox)sender).Checked;
                }
            }
        }

        private void numLookupAutoCompleteMaxResult_ValueChanged(object sender, EventArgs e)
        {
            if (sender is NumericUpDown && !selectedTree.IsNull() && !selectedTree.SelectedNode.IsNull())
            {
                if (selectedTree.SelectedNode.Tag is LayoutControlV2)
                {
                    ((LayoutControlV2)selectedTree.SelectedNode.Tag).LookupAutoCompleteMaxResults = (byte)((NumericUpDown)sender).Value;
                }
            }
        }

        private void chkAutoFitLastColumn_CheckedChanged(object sender, EventArgs e)
        {
            ((LayoutContainer)selectedTree.SelectedNode.Tag).AutoFitLastColumn = chkAutoFitLastColumn.Checked;
        }

        private void ckAutoFitColumn_CheckedChanged(object sender, EventArgs e)
        {
            if (sender is CheckBox && !selectedTree.IsNull() && !selectedTree.SelectedNode.IsNull())
            {
                if (selectedTree.SelectedNode.Tag is LayoutControlV2)
                {
                    ((LayoutControlV2)selectedTree.SelectedNode.Tag).GridColAutoFit = ((CheckBox)sender).Checked;
                }
            }
        }


        private void ckEnableFilterTextInGrid_CheckedChanged(object sender, EventArgs e)
        {
            if (sender is CheckBox && !selectedTree.IsNull() && !selectedTree.SelectedNode.IsNull())
            {
                if (selectedTree.SelectedNode.Tag is LayoutContainer)
                {
                    ((LayoutContainer)selectedTree.SelectedNode.Tag).EnableFilterTextInGrid = ((CheckBox)sender).Checked;
                }
            }
        }

        private void nbGridVisibleRowsNumber_ValueChanged(object sender, EventArgs e)
        {
            if (sender is NumericUpDown && !selectedTree.IsNull() && !selectedTree.SelectedNode.IsNull())
            {
                if (selectedTree.SelectedNode.Tag is LayoutContainer)
                {
                    ((LayoutContainer)selectedTree.SelectedNode.Tag).GridVisibleRowsNumber = (byte)((NumericUpDown)sender).Value;
                }
            }
        }

        private void mnuConvertToLightDatagrid_Click(object sender, EventArgs e)
        {
            ConvertToElement(treeTopGroups.SelectedNode, "TreeListView");
        }
        ...
        private void telerikUIChart1_Load(object sender, EventArgs e)
        {

        }

        private void nbDataGridWidth_ValueChanged(object sender, EventArgs e)
        {
            if (sender is NumericUpDown && !selectedTree.IsNull() && !selectedTree.SelectedNode.IsNull())
            {
                if (selectedTree.SelectedNode.Tag is LayoutControlV2)
                    ((LayoutControlV2)selectedTree.SelectedNode.Tag).DataGridWidth = (int)((NumericUpDown)sender).Value;

                this.selectedTree.Invalidate();
            }
        }

        private void txtDisplaySelector_TextChanged(object sender, EventArgs e)
        {
            LayoutContainer lc = ((LayoutContainer)selectedTree.SelectedNode.Tag);
            lc.DisplaySelectorGridColumns = ((TextBox)sender).Text;
        }

        private void cbxIsLayoutToolbar_CheckedChanged(object sender, EventArgs e)
        {
            if (sender is CheckBox && !selectedTree.IsNull() && !selectedTree.SelectedNode.IsNull() && selectedTree.SelectedNode.Tag is LayoutContainer)
                ((LayoutContainer)selectedTree.SelectedNode.Tag).IsLayoutToolbar = ((CheckBox)sender).Checked;
        }

        private void cbxPivotToggleView_CheckedChanged(object sender, EventArgs e)
        {
            if (sender is CheckBox && !selectedTree.IsNull() && !selectedTree.SelectedNode.IsNull() && selectedTree.SelectedNode.Tag is LayoutContainer)
                ((LayoutContainer)selectedTree.SelectedNode.Tag).PivotToggleView = ((CheckBox)sender).Checked;
        }

        private void cbxFullScreen_CheckedChanged(object sender, EventArgs e)
        {
            if (sender is CheckBox && !selectedTree.IsNull() && !selectedTree.SelectedNode.IsNull() && selectedTree.SelectedNode.Tag is LayoutContainer)
                ((LayoutContainer)selectedTree.SelectedNode.Tag).PivotFullScreen = ((CheckBox)sender).Checked;
        }

        private void cbxOpenReport_CheckedChanged(object sender, EventArgs e)
        {
            if (sender is CheckBox && !selectedTree.IsNull() && !selectedTree.SelectedNode.IsNull() && selectedTree.SelectedNode.Tag is LayoutContainer)
                ((LayoutContainer)selectedTree.SelectedNode.Tag).PivotOpenReport = ((CheckBox)sender).Checked;
        }

        private void ckDgEnableVirtualization_CheckedChanged(object sender, EventArgs e)
        {
            if (this.ckDgEnableVirtualization.Visible)
            {
                LayoutContainer lc = ((LayoutContainer)selectedTree.SelectedNode.Tag);
                lc.Virtualization = ((CheckBox)sender).Checked;
            }
        }

        private void cboEntityData_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (sender is ComboBox && !selectedTree.IsNull() &&
                !selectedTree.SelectedNode.IsNull() &&
                selectedTree.SelectedNode.Tag is LayoutContainer)
            {
                var entityData = ((ComboBox)sender).SelectedItem.ToString();

                ((LayoutContainer)selectedTree.SelectedNode.Tag).PivotDataSource = cboDataSource.SelectedItem.ToString();

                UpdateTreeView(entityData);

                ((LayoutContainer)selectedTree.SelectedNode.Tag).Name = entityData;
                ((LayoutContainer)selectedTree.SelectedNode.Tag).EntityData = entityData;
