...
namespace Linx.EntityAdapterDesigner
{
    ...
    internal partial class EntityAdapterDesignerCommandSet
    {
        ...
        private Guid cmdCleanServiceBusGUID = new Guid("{e024017a-a1a6-42ed-84d6-96b12b174aa9}");
        private const int cmdCleanServiceBusID = 0x8830;

        private Guid cmdClientServiceEventsGUID = new Guid("{e024017a-a1a6-42ed-84d6-96b12b174ab9}");
        private const int cmdClientServiceEventsID = 0x8831;

        private Guid cmdShowDistributorNameGUID = new Guid("{e024017a-a1a6-42ed-84d6-96b12b174ac9}");
        private const int cmdShowDistributorNameID = 0x8832;

        private Guid cmdidPreviewEntityGUID = new Guid("{e024017a-a1a6-42ed-84d6-96b12b174ad9}");
        private const int cmdidPreviewEntityID = 0x8833;

        private Guid cmdSaveCurDocGUID = new Guid("{e024017a-a1a6-42ed-84d6-96b12b174af9}");
        private const int cmdSaveCurDocID = 0x8834;

        private Guid cmdCustomDomainMethodGUID = new Guid("{e024017a-a1a6-42ed-84d6-96b12b174afa}");
        private const int cmdCustomDomainMethodID = 0x8835;

        private Guid cmdGenMetaDataGUID = new Guid("{e024017a-a1a6-42ed-84d6-96b12b174afb}");
        private const int cmdGenMetaDataID = 0x8836;
        ...
        protected override System.Collections.Generic.IList<System.ComponentModel.Design.MenuCommand> GetMenuCommands()
        {
            ...
            new EventHandler(OnPopUpMenuDisplayAction),
            new EventHandler(OnPopUpMenuClick),
            new CommandID(cmdCleanServiceBusGUID, cmdCleanServiceBusID)));

            commands.Add(new DynamicStatusMenuCommand(
            new EventHandler(OnPopUpMenuDisplayAction),
            new EventHandler(OnPopUpMenuClick),
            new CommandID(cmdClientServiceEventsGUID, cmdClientServiceEventsID)));

            commands.Add(new DynamicStatusMenuCommand(
            new EventHandler(OnPopUpMenuDisplayAction),
            new EventHandler(OnPopUpMenuClick),
            new CommandID(cmdShowDistributorNameGUID, cmdShowDistributorNameID)));
            ...
            new EventHandler(OnPopUpMenuDisplayAction),
            new EventHandler(OnPopUpMenuClick),
            new CommandID(cmdSaveCurDocGUID, cmdSaveCurDocID)));

            commands.Add(new DynamicStatusMenuCommand(
            new EventHandler(OnPopUpMenuDisplayAction),
            new EventHandler(OnPopUpMenuClick),
            new CommandID(cmdCustomDomainMethodGUID, cmdCustomDomainMethodID)));

            commands.Add(new DynamicStatusMenuCommand(
            new EventHandler(OnPopUpMenuDisplayAction),
            new EventHandler(OnPopUpMenuClick),
            new CommandID(cmdGenMetaDataGUID, cmdGenMetaDataID)));
        ...
        internal void OnPopUpMenuDisplayAction(object sender, EventArgs e)
        {
            ...
            foreach (object selectedObject in this.CurrentSelection)
            {
                ...
                if (selectedObject is EntityAdapterShape && (command.CommandID.ID == cmdidPreviewEntityID))
                {
                    command.Visible = true;
                    command.Enabled = true;
                    break;
                ...
                if (selectedObject is ClientLocalServiceShape && (command.CommandID.ID == cmdClientServiceEventsID))
                {
                    command.Visible = true;
                    command.Enabled = true;
                    break;
                ...
                if (selectedObject is DomainViewShape && command.CommandID.ID == cmdCustomDomainMethodID)
                {
                    command.Visible = true;
                    command.Enabled = true;
                    break;
                ...
                if (selectedObject is EntityAdapterDesignerDiagram && (command.CommandID.ID == cmdGenMetaDataID || command.CommandID.ID == cmdShowDistributorNameID || command.CommandID.ID == cmdSaveAllDocsID || command.CommandID.ID == cmdSaveCurDocID || command.CommandID.ID == cmdUpdateSpaID || command.CommandID.ID == cmdCleanServiceBusID))
                {
                    command.Visible = true;
                    command.Enabled = true;
                    break;
        ...
        internal void OnPopUpMenuClick(object sender, EventArgs e)
        {
            ...
            foreach (object selectedObject in this.CurrentSelection)
            {
                if (selectedObject is EntityAdapterShape && command.CommandID.ID == cmdidPreviewEntityID)
                {
                    ((EntityAdapter)((EntityAdapterShape)selectedObject).ModelElement).PreViewEntity();
                    break;
                }

                if (selectedObject is EntityAdapterDesignerDiagram && (command.CommandID.ID == cmdSaveAllDocsID || command.CommandID.ID == cmdSaveCurDocID))
                {
                    ((EntityAdapterDesignerRoot)((EntityAdapterDesignerDiagram)selectedObject).ModelElement).SaveAllDocuments(command.CommandID.ID == cmdSaveCurDocID);
                    break;
                }
                ...
                if (selectedObject is EntityAdapterDesignerDiagram && command.CommandID.ID == cmdCleanServiceBusID)
                {
                    ((EntityAdapterDesignerRoot)((EntityAdapterDesignerDiagram)selectedObject).ModelElement).CleanServiceBus();
                    break;
                }
                ...
                if (selectedObject is DomainViewShape && command.CommandID.ID == cmdCustomDomainMethodID)
                {
                    var dv = ((DomainViewShape)selectedObject).ModelElement as DomainView;
                    dv.OpenCustomMethod();
                    break;
                }
                ...
                if (selectedObject is ClientLocalServiceShape && command.CommandID.ID == cmdClientServiceEventsID)
                {
                    using (Transaction transaction =
                        this.CurrentEntityAdapterDesignerDocData.Store.TransactionManager.BeginTransaction("Change designer by service events."))
                    {
                        CustomCode.FrmAddServiceClientEvents frmEvents = new CustomCode.FrmAddServiceClientEvents();
                        frmEvents.Service = ((ClientLocalServiceShape)selectedObject).ModelElement as ClientLocalService;
                ...
                if (selectedObject is EntityAdapterDesignerDiagram && (command.CommandID.ID == cmdGenMetaDataID))
                {
                    var dsnRoot = ((EntityAdapterDesignerDiagram)selectedObject).ModelElement as EntityAdapterDesignerRoot;

                    if (dsnRoot != null)
                    {
                        dsnRoot.UpdateContextMetadata();
                    }

                    break;
                }

                if (selectedObject is EntityAdapterDesignerDiagram && (command.CommandID.ID == cmdShowDistributorNameID))
                {
                    var dsnRoot = ((EntityAdapterDesignerDiagram)selectedObject).ModelElement as EntityAdapterDesignerRoot;

                    if (dsnRoot != null)
                    {
                        MessageBox.Show(dsnRoot.GetDistributorProductName(), "Distributor Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
