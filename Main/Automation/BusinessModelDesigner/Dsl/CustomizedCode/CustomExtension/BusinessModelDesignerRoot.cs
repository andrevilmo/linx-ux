...
using Linx.BusinessModelDesigner.CustomizedCode.DatabaseScriptGenerator;
...
namespace Linx.BusinessModelDesigner
{
    ...
    public partial class BusinessModelDesignerRoot
    {
        ...
        {
            var shape = this.GetPresentation<BusinessModelDesignerDiagram>();
            if (shape != null)
            {
        ...
        public void SelectShape(string name)
        {
            ...
            if (element != null)
            {
                var diagram = this.GetPresentation<BusinessModelDesignerDiagram>();
                if (diagram != null)
                {
        ...
        {
            Project project = this.GetBmdProject();
            if (this.IsAspNetCoreEnabled())
            {
                project = this.GetBmdCoreProject(project);
            }
            if (project.IsNull())
                return;
            ...
            if (!item.IsNull())
            {
                if (designElement is BusinessModelDesignerRoot)
                {
                    className = this.GetDefaultContextName();
                    elementName = String.Empty;
                    fileName = "BusinessDataModel";
                    if (BusinessModelDesignerRoot.ExistsProjectItem(item.ProjectItems, fileName + ".tt"))
                    {
                ...
                {
                    if (BusinessModelDesignerRoot.ExistsProjectItem(project.ProjectItems, "Domains"))
                    {
                        item = project.ProjectItems.Item("Domains");
                        if (BusinessModelDesignerRoot.ExistsProjectItem(item.ProjectItems, "DomainViews.tt"))
                        {
                ...
                {
                    var webApiProject = (this.IsAspNetCoreEnabled() ? this.GetWebApiCoreProject(((WebApiController)designElement).ProjectSuffix) : this.GetWebApiProject(((WebApiController)designElement).ProjectSuffix));
                    ...
                    elementName = String.Empty;
                    if (!BusinessModelDesignerRoot.ExistsProjectItem(item.ProjectItems, fileName))
                    {
                ...
                else if (designElement is ModelImplementation)
                {
                    ...
                    if (implProject == null)
                    {
                        MessageBox.Show("This implementation project does not exists! Save the designer before this operation.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                    }
                ...
                else if (designElement is ModelInterface)
                {
                    ...
                    {
                        MessageBox.Show("This contract project does not exists! Save the designer before this operation.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                    }
                ...
                else
                {
                    ...
                    {
                        MessageBox.Show(String.Format("File [{0}] does not exists!", fileName + ".tt"), "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                    }
        ...
        public void GenerateContractsCode(List<ModelInterface> interfaces)
        {
            ...
            if (bmdProject != null)
            {
                ...
                {
                    contractProject = this.GetContractProject(bmdProject);
        ...
        public void GenerateImplementationsCode(List<ModelInterface> interfaces)
        {
            ...
            if (bmdProject != null)
            {
                ...
                foreach (var intf in interfaces)
                {
                    ...
                    {
                        implementProject = this.GetImplementationProject(repository, bmdProject);
        ...
        private void GenerateImplementationCode(StringBuilder codeBuilder, ModelImplementation implement, Project implementProject)
        {
            ...
            codeBuilder.AppendLine("using " + implement.BusinessModelDesignerRoot.GetNamespace() + ";");
            codeBuilder.AppendLine("using " + this.GetContractProjectName(this.GetBmdProject()) + ";");
        ...
        public Project GetImplementationProject(ModelImplementation implement, Project bmdProject = null)
        {
            if (bmdProject == null)
                bmdProject = GetBmdProject();

            if (bmdProject != null)
                return this.GetProjectByName(GetImplementationProjectName(implement, bmdProject));
            else
                return null;
        }

        public Project GetContractProject(Project bmdProject = null)
        {
            if (bmdProject == null)
                bmdProject = GetBmdProject();

            if (bmdProject != null)
                return this.GetProjectByName(GetContractProjectName(bmdProject));
            else
                return null;
        }
        ...
        public string GetImplementationProjectName(ModelImplementation implement, Project bmdProject)
        {
            return bmdProject.Name + "." + GetImplementationFolderName() + (implement.ProjectSuffix.IsNullOrEmpty() ? String.Empty : "." + implement.ProjectSuffix);
        }

        public string GetContractProjectName(Project bmdProject)
        {
            return bmdProject.Name + "." + GetContractFolderName();
        }
        ...
        public void UpdateContractProject(Project bmdProject, ModelInterface intrf)
        {
            if (this.IsAspNetCoreEnabled())
                return;

            EnvDTE.DTE appDTE = bmdProject.DTE;
            string contractProjectName = GetContractProjectName(bmdProject);
            Project contractProject = GetProjectByName(contractProjectName);
            string folderName = GetContractFolderName();

            if (contractProject == null)
            {
                string repositoryDir = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(bmdProject.FullName), "..\\" + contractProjectName));
            ...
            this.AdjustMissingReferences(contractProject);
            this.UpdateVersion(contractProject);

            //Update library references
            string gacPath = this.GetFullPath("Linx.GAC") ?? "";
            this.AddNewReference(contractProject, "System.ComponentModel.Composition.dll");
            this.UpdateReference(contractProject, Path.Combine(gacPath, "Linx.Tools.dll"));
        ...
        public void UpdateImplementationProject(Project bmdProject, ModelImplementation implement)
        {
            if (this.IsAspNetCoreEnabled())
                return;

            EnvDTE.DTE appDTE = bmdProject.DTE;
            string implementProjectName = GetImplementationProjectName(implement, bmdProject);
            Project implementProject = GetProjectByName(implementProjectName);
            Project contractProject = this.GetContractProject(bmdProject);
            string folderName = this.GetImplementationFolderName();

            if (implementProject == null)
            {
                string repositoryDir = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(bmdProject.FullName), "..\\" + implementProjectName));
            ...
            this.AdjustMissingReferences(implementProject);
            this.UpdateVersion(implementProject);


            //Add project references
            AddProjectReference(implementProject, bmdProject, false);
            AddProjectReference(implementProject, contractProject, false);

            //Update library references
            this.AddNewReference(implementProject, "System.ComponentModel.Composition.dll");
            string gacPath = this.GetFullPath("Linx.GAC") ?? "";
            this.UpdateReference(implementProject, Path.Combine(gacPath, "Linx.Tools.dll"));
            this.UpdateReference(implementProject, Path.Combine(gacPath, "Linx.LinqExtensions.dll"));
        ...
        {
            var webApiProject = (this.IsAspNetCoreEnabled() ? this.GetWebApiCoreProject(((WebApiAction)targetOperation).WebApiController.ProjectSuffix) : this.GetWebApiProject(((WebApiAction)targetOperation).WebApiController.ProjectSuffix));
        ...
        public void GenerateWebApiAppStartCode(List<ModelClass> modelClasses)
        {
            if (this.IsAspNetCoreEnabled())
                return;
            ...
            if (bmdProject != null)
            {
                ...
                {
                    webApiProject = this.GetWebApiProject(api.ProjectSuffix, bmdProject);
                    if (webApiProject != null)
                    {
                        ...
                        {
                            item = this.GetWebApiAppStartItem(api, webApiProject);
                            if (!item.IsNull())
                            {
                                codeBuilder = new Linx.Tools.CodeBuilder();
                                //Get code
                                this.GenerateWebApiAttributeRoutingHttpCode(modelClasses, codeBuilder, api, webApiProject, bmdProject);
                                string codeBody = codeBuilder.GetBody();

                                className = api.Name + "AttributeRoutingHttp";
        ...
        private void GenerateWebApiAttributeRoutingHttpCode(List<ModelClass> modelClasses, Linx.Tools.CodeBuilder codeBuilder, WebApiController api, Project webApiProject, Project eadProject)
        {
            ...
            codeBuilder.AddLine("using " + api.BusinessModelDesignerRoot.GetNamespace() + ";");
        ...
        {
            return WriteFile(outputFilePath, codeBuilder.ToString(), projectItems);
        }
        private bool WriteFile(string outputFilePath, string fileContent, ProjectItems projectItems)
        {
            ...
            if (!File.Exists(outputFilePath) || !existsInProject)
            {
                ...
                System.IO.File.WriteAllText(outputFilePath, fileContent, Encoding.UTF8);

                projectItems.AddFromFile(outputFilePath);
            }
            else if (System.IO.File.ReadAllText(outputFilePath) != fileContent)
                System.IO.File.WriteAllText(outputFilePath, fileContent, Encoding.UTF8);

            return true;
        }

        public void GenerateWebApiControllersCode(List<ModelClass> modelClasses, List<BusinessModelDesignerRoot> models)
        {
            if (this.IsAspNetCoreEnabled())
                return;
                ...
                foreach (var api in this.WebApiControllers.Where(e => (modelClasses == null && !e.ExposeAllContext) || (modelClasses != null && e.ExposeAllContext)))
                {
                    ...
                    if (webApiProject != null)
                    {
                        ...
                        if (!item.IsNull())
                        {
                            ...
                            //Get automatic code
                            codeBuilder = new Linx.Tools.CodeBuilder();
                            this.GenerateWebApiAutomaticControllerCode(modelClasses, models, codeBuilder, api, webApiProject, bmdProject, (this.EnableAutomaticAuthorization ? api.Name + "ControllerAuthorize" : ""));
        ...
        private void GenerateWebApiControllerCode(Linx.Tools.CodeBuilder codeBuilder, WebApiController api, Project webApiProject, bool isNetCore = false)
        {
        ...
        {
            var bmdProject = this.GetBmdProject();
            string folderName = "Model";
            ProjectItem folder = GetProjectItemByName(bmdProject, folderName);
            if (!folder.IsNull())
            {
        ...
        public void GenerateOperationalEvents(Project bmdProject, bool isCore = false)
        {
            if (bmdProject != null)
            {
                ...
                ProjectItem folder = GetProjectItemByName(bmdProject, folderName);
                if (folder == null)
                    folder = bmdProject.ProjectItems.AddFolder(folderName);
                ...
                {
                    string contextEventsName = this.GetOperationalEventsClassName();
                    outputFile = Path.Combine(Path.Combine(GetProjectPath(bmdProject), folderName), contextEventsName + ".cs");
                    if (!File.Exists(outputFile) || !ExistsProjectItem(folder.ProjectItems, System.IO.Path.GetFileName(outputFile)))
                    {
                        ...
                        //Add Events
                        this.GenerateOperationalEventsCode(codeBuilder, bmdProject, contextEventsName, isCore);
                        System.IO.File.WriteAllText(outputFile, codeBuilder.ToString());
                        //Add project item.
                    ...
                    if (!File.Exists(outputFile) || !ExistsProjectItem(folder.ProjectItems, System.IO.Path.GetFileName(outputFile)))
                    {
                        ...
                        //Add Events
                        this.GenerateStartEventsCode(codeBuilder, bmdProject, contextEventsName, isCore);
                        System.IO.File.WriteAllText(outputFile, codeBuilder.ToString());
                        //Add project item.
                        folder.ProjectItems.AddFromFile(outputFile);
                    }
        ...
        private void GenerateOperationalEventsCode(Linx.Tools.CodeBuilder codeBuilder, Project bmdProject, string contextEventsName, bool isCore = false)
        {
            codeBuilder.AddLine("using System;");
            codeBuilder.AddLine("using System.Collections.Generic;");
            codeBuilder.AddLine("using System.Linq;");
            codeBuilder.AddLine("using Linx.Tools;");
            codeBuilder.AddLine("using Linx.LinqExtensions;");

            if (isCore)
            {
                codeBuilder.AddLine("using Microsoft.EntityFrameworkCore;");
            ...
            codeBuilder.AddLine("");
            codeBuilder.AddLine("namespace " + this.TargetNamespace);
            codeBuilder.AddLine("{");
        ...
        private void GenerateStartEventsCode(Linx.Tools.CodeBuilder codeBuilder, Project bmdProject, string contextEventsName, bool isCore = false)
        {
            codeBuilder.AddLine("using System;");
            codeBuilder.AddLine("using System.Collections.Generic;");
            codeBuilder.AddLine("using System.Linq;");
        ...
        private void GenerateWebApiAutomaticControllerCode(List<ModelClass> modelClasses, List<BusinessModelDesignerRoot> models, Linx.Tools.CodeBuilder codeBuilder, WebApiController api, Project webApiProject, Project eadProject, string authorizeAttribute, bool isNetCore = false)
        {
            ...
            if (isNetCore)
            {
                ...
                codeBuilder.AddLine("// Documentation: http://localhost:" + port + "/swagger/ui");
            ...
            {
                if (api.ExposeAllContext)
                    codeBuilder.AddLine("// Default Call: http://localhost:" + port + "/" + api.GetRoutePrefix());
                else
                {
            ...
            if (!authorizeAttribute.IsNullOrEmpty() && !isNetCore)
                codeBuilder.AddLine("[ODataBasicAuthenticationFilter]");

            codeBuilder.AddLine("public partial class " + api.Name + "Controller : " + (isNetCore ? "Controller" : (api.ExposeAllContext ? "ODataController" : "ApiController")));
            codeBuilder.AddLine("{");
            codeBuilder.IncreaseIndent();
            string contextName = this.GetDefaultContextName();
            codeBuilder.AddLine("private " + contextName + " _context;");
            codeBuilder.AddLine("public " + contextName + " Context { get {  if (_context == null) { _context = new " + contextName + "(\"\", null as Dictionary<string, string>); } return _context; }  }");
            ...
            if (api.ExposeAllContext)
            {
                if (isNetCore)
                {
                    ...
                    codeBuilder.AddLine("public " + api.Name + "Controller(IConfiguration config)");
                    codeBuilder.AddLine("{");
                ...
                if (modelClasses != null && modelClasses.Count > 0)
                {
                    foreach (var entity in modelClasses.Where(e => !((ModelClass)e).InStudy && !((ModelClass)e).NotMapped).Select(e => (ModelClass)e).OrderBy(e => e.Name))
                    {
                        if (isNetCore)
                        {
                            codeBuilder.AddLine("[HttpGet(\"" + entity.Name + "\")]");
                            codeBuilder.AddLine("public IQueryable<" + entity.Name + "> Get" + entity.Name + "()");
                            codeBuilder.AddLine("{");
                            codeBuilder.AddLine("    this.HttpContext.AdjustOdataFeature(App_Start.ModuleInitializer_" + api.Name + ".Model, _routePrefix, \"" + entity.Name + "\", App_Start.ModuleInitializer_" + api.Name + ".ServiceProvider);");
                        ...
                        else
                        {
                            ...
                            if (ClassKindHelper.IsModelViewBehavior(entity.Kind) || pKeys.Count() > 0)
                            {
                                if (pKeys.Count() > 0)
                                {
                                    codeBuilder.AddLine();
                                ...
                                if (entity.Kind == ClassKind.Table && pKeys.Count() > 0)
                                {
                                    ...
                                    List<string> relationships = entity.GetForeignKeyProperties(models, "").Split(new string[] { "public virtual " }, StringSplitOptions.RemoveEmptyEntries).Select(e => e.Left(" ")).Where(e => !e.IsNullOrEmpty()).Distinct().ToList();
                                    var collections = entity.GetForeignKeyCollecions(models, "").Split(new string[] { "public virtual " }, StringSplitOptions.RemoveEmptyEntries).Select(e => e.Left(" ")).Distinct().ToArray();
            ...
            if (!authorizeAttribute.IsNullOrEmpty())
            {
                ...
                codeBuilder.AddLine("    {");
                codeBuilder.AddLine("        return Linx.Business.Tools.LinxAutorization.CheckAuthorization(actionContext, string.Format(\"{0}#{1}#{1}/{2}\", \"" + api.BusinessModelDesignerRoot.TargetNamespace + "\", \"" + api.GetRoutePrefix() + "\", actionContext.ActionDescriptor.ActionName));");
                codeBuilder.AddLine("    }");
                codeBuilder.AddLine("}");
                codeBuilder.AddLine();
            }
        ...
        public void UpdateWebApiProject(Project bmdProject, WebApiController api)
        {
            if (this.IsAspNetCoreEnabled())
                return;

            EnvDTE.DTE appDTE = bmdProject.DTE;
            string webApiProjectName = GetWebApiProjectName(api.ProjectSuffix, bmdProject);
            ...
            AddProjectReference(webApiProject, bmdProject, false);

            //Update library references 
            string gacPath = this.GetFullPath("Linx.GAC") ?? "";
            string domainServicesPath = this.GetFullPath("Linx.DomainServices") ?? "";
            this.AddNewReference(webApiProject, Path.Combine(domainServicesPath, "System.ServiceModel.DomainServices.Server.dll"));
            ...
            this.UpdateLibReferences(webApiProject, "Linx.LinxDataService.Library", false);
        ...
        public void SetPostBuildEventToServiceBus(Project current = null)
        {
            ...
            if (current != null)
            {
                ...
                if (!serviceBusPath.IsNullOrEmpty() && Directory.Exists(serviceBusPath))
                {
                    ...
                    string postEbentValue = current.Properties.Item("PostBuildEvent").Value as string;
        ...
        {
            return GetProjectPath(this.GetBmdProject());
        }
        ...
        public void GenerateScript(BusinessModelDesignerDiagram diagram)
        {
            ...
            var project = this.GetBmdProject();
            if (project != null)
            {
                string scriptPath = Path.Combine(this.GetProjectPath(), "BusinessModelPath");
                if (!Directory.Exists(scriptPath))
                    Directory.CreateDirectory(scriptPath);
                string scriptFile = Path.Combine(scriptPath, "script.sql");

                ScriptGeneratorBase scriptGen = null;
                switch (this.GetDefaultProvider())
                {
                    case Provider.SQLServer:
                        scriptGen = new ScriptGeneratorSqlServer(new ScriptGeneratorOptions() { CreateAutomaticIndexes = !this.RemoveAutomaticIndexes });
                        break;
        ...
        public void ValidSqlMigratorFKs(List<BusinessModelDesignerRoot> models, string projectPath)
        {
            ...
            foreach (var model in models)
            {
                foreach (ModelClass modelClass in model.Types.Where(e => e is ModelClass && ((ModelClass)e).Kind == ClassKind.Table))
                {
                    foreach (var link in Association.GetLinksToSourceModelClasses(modelClass))
                    {
                        ...
                        {
                            fkControl.Add(objectName, link.SourceModelClass.BusinessModelDesignerRoot.DocumentName + "::" + command);
                        }
                        else
                        {
                            if (fkControl[objectName].Right("::") != command)
                            {
                                string description = String.Format("-Warning: Foreignkey differences detected between the tables {0} and {1} on files {2} and {3}.", link.SourceModelClass.Name, link.TargetModelClass.Name, fkControl[objectName].Left("::"), link.SourceModelClass.BusinessModelDesignerRoot.DocumentName);
                ...
                foreach (ModelClass modelClass in model.Types.Where(e => e is ModelClass && ((ModelClass)e).Kind == ClassKind.Table))
                {
                    ...
                    if (lma != null && lma.MultipleAssociation != null)
                    {
                        foreach (var link in MultipleAssociationOrigin.GetLinksToOriginTypes(lma.MultipleAssociation))
                        {
                            ...
                            if (!fkControl.ContainsKey(objectName))
                            {
                                fkControl.Add(objectName, link.OriginType.BusinessModelDesignerRoot.DocumentName + "::" + command);
                            }
                            else
                            {
                                if (fkControl[objectName].Right("::") != command)
                                {
                                    string description = String.Format("-Warning: Foreignkey differences detected between the tables {0} and {1} on files {2} and {3}.", link.OriginType.Name, link.MultipleAssociation.TargetType.Name, fkControl[objectName].Left("::"), link.OriginType.BusinessModelDesignerRoot.DocumentName);
                                    errors.Add(description);
                                }
                            }
                        }
        ...
        {
            var bmdProj = this.GetBmdProject();
            string errorFile = System.IO.Path.Combine(bmdProj.Properties.Item("FullPath").Value.ToString(), "_fkWarnings.err");
        ...
        private void ValidBmds(ValidationContext context)
        {
            List<string> bmdList = new List<string>();
            foreach (var item in GetProjectModels())
            {
                bmdList.Add(item.Value.Properties.Item("FullPath").Value.ToString().ToLower());
            }

            //Check inconsistent files
            var bmdProj = this.GetBmdProject();
            string alertFileList = String.Empty;
            string[] bmdFiles = System.IO.Directory.GetFiles(bmdProj.Properties.Item("FullPath").Value.ToString(), "*.bmd", SearchOption.AllDirectories);
            foreach (var file in bmdFiles.Where(e => !bmdList.Contains(e.ToLower())))
            {
                alertFileList += (alertFileList.IsNullOrEmpty() ? "" : ", ") + file.ToLower().Right(((string)bmdProj.Properties.Item("FullPath").Value).ToLower()).ToUpper();
            }
        ...
        public void UpdatePackages(Project project = null)
        {
            if (this.IsAspNetCoreEnabled())
                return;

            if (project == null)
                project = this.GetBmdProject();
        ...
        public void InstallNuGetPackage(string packageID, string version, Project project = null)
        {
            if (project == null)
                project = this.GetBmdProject();

            BusinessModelDesignerDiagram diagram = this.GetPresentation<BusinessModelDesignerDiagram>();
            ...
            if (componentModel != null)
            {
                ...
                if (installerPackage != null)
                {
                    try
                    {
                        ...
                        {
                            installerPackage.InstallPackage("https://api.nuget.org/v3/index.json", project, packageID, version, true);
                        }
                    }
        ...
        public string GetConfigProviders(string indent)
        {
            var defaultProvType = this.GetDefaultProvider();

            Tools.CodeBuilder builder = new Tools.CodeBuilder(indent);
            builder.AddLine("<system.data>");
            builder.AddLine("   <DbProviderFactories>");
            switch (defaultProvType)
            {
                case Provider.Oracle:
                    builder.AddLine("       <remove invariant=\"Devart.Data.Oracle\"/>");
                    builder.AddLine("       <add name=\"Devart.Data.Oracle\"");
                    builder.AddLine("           invariant=\"Devart.Data.Oracle\"");
                    builder.AddLine("           description=\".Net Framework Data Provider for Oracle\"");
                    builder.AddLine("           type=\"Devart.Data.Oracle.OracleProviderFactory, Devart.Data.Oracle\"/>");
                    break;
                case Provider.SQLite:
                    builder.AddLine("       <remove invariant=\"System.Data.SQLite\"/>");
                    builder.AddLine("       <add name=\"SQLite Data Provider (Entity Framework 6)\"");
                    builder.AddLine("           invariant=\"System.Data.SQLite\" description=\".NET Framework Data Provider for SQLite (Entity Framework 6)\"");
                    builder.AddLine("           type=\"System.Data.SQLite.SQLiteFactory, System.Data.SQLite\"/>");
                    break;
                default:
                    break;
            }
        ...
        public bool IsRemoveAutomaticIndexes()
        {
            ...
            string key = "RemoveAutomaticIndexes";
            return contextInfo.ContainsKey(key) && contextInfo[key] == "true";
        }
        ...
        public bool HasAutomaticAuthorization()
        {
            //DataContextName, ProviderType, ConnectionString
            var contextInfo = this.GetSharedInfo();
            string key = "EnableAutomaticAuthorization";
            return contextInfo.ContainsKey(key) && contextInfo[key] == "true";
        }

        public bool GetDisableAllAudits()
        {
            //DataContextName, ProviderType, ConnectionString
            var contextInfo = this.GetSharedInfo();
            string key = "DisableAllAudits";
            return contextInfo.ContainsKey(key) && contextInfo[key] == "true";
        }
        ...
        public bool HasIdLinxAsIdGpecon()
        {
            //DataContextName, ProviderType, ConnectionString
            var contextInfo = this.GetSharedInfo();
            string key = "SetIdLinxWithIdGpecon";
            return contextInfo.ContainsKey(key) && contextInfo[key] == "true";
        }
        ...
        {
            var contextInfo = this.GetSharedInfo();
            string key = "EnableAccessConnectionControl";
            return contextInfo.ContainsKey(key) && contextInfo[key] == "true";
        }

        public bool HasControlIdLinxByApplicative()
        {
            var contextInfo = this.GetSharedInfo();
            string key = "ControlIdLinxByApplicative";
            return contextInfo.ContainsKey(key) && contextInfo[key] == "true";
        }
        ...
        {
            var contextInfo = this.GetSharedInfo();
            string key = "ForceDynamicForeignKeyNames";
            return contextInfo.ContainsKey(key) && contextInfo[key] == "true";
        }
        ...
        public Provider GetDefaultProvider()
        {
            ...
            if (contextInfo.ContainsKey(key))
                return (Provider)Enum.Parse(typeof(Provider), contextInfo[key]);
            else
                return Provider.SQLServer;
        }

        public bool IsAspNetCoreEnabled()
        {
            var contextInfo = this.GetSharedInfo();
            string key = "IsAspNetCore";
            return (contextInfo.ContainsKey(key) && contextInfo[key] == "true");
        }
        ...
        public void FindElement()
        {
            ...
            finder.ShowDialog();
            if (finder.ElementsSelection.Count > 0)
            {
                foreach (var elementSelection in finder.ElementsSelection)
                {
                    ShowClassElement(elementSelection.Value, elementSelection.Key.Right("###"));
                }
            }
        }
        ...
        public bool ExistsAnyReference(ProjectItem item, string[] typeNames)
        {
            ...
            try
            {
                ...
                // Create a reference to the target model:
                ModelBusReference modelReference = manager.CreateReference(item);
                BusinessModelDesignerRoot modelRoot = this.GetModelRoot<BusinessModelDesignerRoot>(modelReference);
        ...
        public Dictionary<string, ProjectItem> GetProjectModels(bool removeCurrent = false)
        {
            ...
            var currentProject = this.GetBmdProject();
        ...
        {
            Dictionary<string, ProjectItem> innerModels = new Dictionary<string, ProjectItem>();
            EnvDTE.Project project = this.GetBmdProject();
            if (project != null)
            {
                foreach (var element in GetModels(project.ProjectItems, removeCurrent))
                {
        ...
        private Dictionary<string, ProjectItem> GetModels(ProjectItems items, bool removeCurrent)
        {
            ...
            {
                if (item.Name.Right(4).ToLower() == ".bmd")
                {
        ...
        public void SyncScriptColumns(string classNamwe, string sqlScript)
        {
            string connectioString = GetConfigConnectionString(Provider.SQLServer, Provider.SQLite);
            if (!connectioString.IsNullOrEmpty())
            {

                CustomizedCode.ReverseEngineeringModel.IProviderDatabaseLoader databaseLoader = null;
                switch (getConfigProvider())
                {
                    case Provider.SQLServer:
                        databaseLoader = new CustomizedCode.ReverseEngineeringModel.SqlServerDatabaseLoader(connectioString);
                        break;
                    case Provider.SQLite:
                        databaseLoader = new CustomizedCode.ReverseEngineeringModel.SQLiteDatabaseLoader(connectioString);
                        break;
                    case Provider.MySQL:
                        databaseLoader = new CustomizedCode.ReverseEngineeringModel.MySqlDatabaseLoader(connectioString);
                        break;
                    case Provider.PostgreSQL:
                        databaseLoader = new CustomizedCode.ReverseEngineeringModel.PostgresqlDatabaseLoader(connectioString);
                        break;
                    default:
                        throw new NotSupportedException("Provider not supported: " + connectioString.ToString());
                }

                var columns = databaseLoader.GetScriptColumns(sqlScript);

                using (Transaction t = this.Store.TransactionManager.BeginTransaction("Sync script columns"))
                {
                    try
                    {
                        this.IsLocked = true;
                        ...
                        {
                            this.AddColumnAttribute(classNamwe, column.Name, DbToModelDataType(column.DbDataType, column.MaxLength), column.IsPK, column.IsIdentity, column.IsNullable, column.Precision, column.Scale, column.MaxLength, indeOrder, column.SqlDefault);
                            indeOrder++;
                        }
                        t.Commit();
                    }
        ...
        public void ExecuteReverseEngineer(BusinessModelDesignerDiagram designer)
        {
            Provider defaultProv = this.GetDefaultProvider();
            if (!defaultProv.In(Provider.SQLServer, Provider.SQLite))
            {
                MessageBox.Show("This action is not allowed for " + defaultProv.ToString() + "!", "Alert", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            var reverseEng = new frmReverseEngineer() { ConnectionString = connectioString, ConnectionProvider = getConfigProvider(), SuggestedSelection = this.Types.Where(e => e is ModelClass && !(e is ReferenceModelClass)).Select(e => ((ModelClass)e).GetTableName()).ToList() };
            ...
            if (reverseEng.OK)
            {
                ...
                using (Transaction t = this.Store.TransactionManager.BeginTransaction("Reverse Engineering"))
                {
                    try
                    {
                        ...
                        {
                            kind = (table is Linx.Tools.Migration.View ? ClassKind.DatabaseView : (table is Linx.Tools.Migration.FunctionBase ? ClassKind.DatabaseScript : ClassKind.Table));
                            pkName = (kind == ClassKind.Table && ((Table)table).PrimaryKey != null ? ((Table)table).PrimaryKey.Name : String.Empty);
                            isClustered = (kind == ClassKind.Table && ((Table)table).PrimaryKey != null ? ((Table)table).PrimaryKey.IsClustered : false);


                            //Add Clsses
                            var tClass = this.AddTableClass(table.Name, kind, table.Schema.Name, pkName, isClustered);
                            if (table is Linx.Tools.Migration.FunctionBase)
                            {
                                var dbFunc = (Linx.Tools.Migration.FunctionBase)table;
                                tClass.DatabaseScriptCommand = "";
                                foreach (var paramet in dbFunc.Parameters)
                                {
                                    tClass.DatabaseScriptCommand += (tClass.DatabaseScriptCommand.IsNullOrEmpty() ? "" : "#") + paramet.DataType.FullName + (paramet.IsNullable ? "? " : " ") + paramet.Name;
                                }
                                tClass.DatabaseScriptCommand = (table is Linx.Tools.Migration.Procedure ? "PROC:" : "FUNC:") + tClass.DatabaseScriptCommand;
                            }
                            //Add Attributes
                            int indeOrder = 0;
                            ...
                            {
                                indeOrder = 0;
                                this.CheckIndexes(table.Name, ((Table)table).Indexes.Select(e => e.Name).ToArray());
                                foreach (var index in ((Table)table).Indexes)
                                {
                        ...
                        //Add Associations
                        foreach (Table table in reverseEng.SelectTables.Where(e => e is Table))
                        {
                            this.CheckAssociations(table.Name, table.ForeignKey.Select(e => e.Name).ToArray());
                            foreach (var link in table.ForeignKey)
                            {
                                this.AddTableAssociation(link.Name, link.Referenced.Name, link.Parent.Name, GetDbMultiplicity(link), link.DeleteAction == ForeignKey.ReferentialAction.Cascade, link.ForeignKeyColumns.Select(e => e.ParentColumn.Name + ":" + e.ReferencedColumn.Name).ToArray());
                            }
                        }
        ...
        public void AddExternalReferences(IModelBus modelBus, ModelBusReference modelReference, List<string> externalClasses, bool useTableNameComparison)
        {
                        ...
                        if (modelRoot != null)
                        {
                                ...
                                this.Store.TransactionManager.BeginTransaction("Adding External References."))
                            {
                                foreach (string className in externalClasses)
                                {
                                    ...
                                    {
                                        var classRef = modelAdapter.GetElementReference(element);
                                        string sourceTableName = element.GetTableName();
                                        existentElement = this.Types.FirstOrDefault(e => e.Name == className || (useTableNameComparison && ((ModelClass)e).GetTableName() == sourceTableName)) as ModelClass;

                                        if (existentElement != null && existentElement is ReferenceModelClass)
                                        {
                                            //Update Broken Reference
                                            if (((ReferenceModelClass)existentElement).HasReferenceError || existentElement.Name != className || classRef.GetReferenceFile() != ((ReferenceModelClass)existentElement).ModelClassReference.GetReferenceFile())
                                            {
                                                hasChanges = true;
        ...
        public List<ModelBusAdapter> GetModelAdapters()
        {
            List<ModelBusAdapter> modelBusAdapters = new List<ModelBusAdapter>();
            var models = this.GetModelsFromCurrentProject();
            // Get ModelBus
            var modelBus = this.GetModelBus();
            if (modelBus == null)
                return modelBusAdapters;

            string errorMessage = "";
            models = this.GetModelsFromCurrentProject();
        ...
        public string GetConfigConnectionString(params Provider[] compare)
        {
            var providerType = getConfigProvider();

            if (compare.Contains(providerType))
                return this.GetSharedInfo()["ConnectionString"];

            return vsInvalidProvider;
        }
        ...
        {
            return this.HasAutomaticAuthorization() && !this.GetDisableAllAudits();
        }

        public string GetAuditEntities(List<ModelClass> entities)
        {
            if (!this.EnableAuditing()) return "";

            var auditEntities = entities.Where(e => e.EnableAudit).Select(e => this.TargetNamespace + "." + e.Name);

            return auditEntities.Count() == 0 ? "" : "\"" + string.Join("\",\"", auditEntities) + "\"";
        }
        ...
        {
            return GetProjectItemByName(this.GetBmdProject(), this.DocumentName);
        }

        public Project GetBmdProject()
        {
            return GetBmdProject((EnvDTE.DTE)null);
        }

        public Project GetBmdProject(EnvDTE.DTE vs)
        {
        ...
        private static bool IsBmdProject(Project project)
        {
            return IsBmdProject(project.ProjectItems);
        }

        private static bool IsBmdProject(ProjectItems projectItems)
        {
            bool result = false;
            ...
            if (!projectItems.IsNullOrEmpty() && projectItems.Count > 0)
            {
                ...
                {
                    if (Path.GetExtension(item.Name).ToLower() == ".bmd")
                    {
                        result = true;
                        break;
                    }

                    if (IsBmdProject(item.ProjectItems))
                    {
                        result = true;
                        break;
                    }
        ...
        private Project GetBmdProject(Project project)
        {
            Project current = null;

            if (BusinessModelDesignerRoot.IsBmdProject(project, System.IO.Path.Combine(this.DocumentPath, this.DocumentName)))
            {
                current = project;
            }
            else
            {
                if (project.ProjectItems != null && project.ProjectItems.Count > 0)
                {
                    foreach (ProjectItem projItem in project.ProjectItems)
                    {
                        ...
                        {
                            current = GetBmdProject(projItem.SubProject);
                            if (current != null)
                                break;
                        }
                    }
        ...
        private static bool IsBmdProject(Project project, string itemPath)
        {
            return IsBmdProject(project.ProjectItems, itemPath);
        }

        private static bool IsBmdProject(ProjectItems projectItems, string itemPath)
        {
            bool result = false;
            string fullName;
            ...
            if (!projectItems.IsNullOrEmpty() && projectItems.Count > 0)
            {
                foreach (ProjectItem item in projectItems)
                {
                    ...
                    if (IsBmdProject(item.ProjectItems, itemPath))
                    {
                        result = true;
                        break;
                    }
        ...
        public string GetSqlMigratorChanges(List<BusinessModelDesignerRoot> models, string indent, Provider defaultProvider, string templateFile)
        {
        ...
        {
            if (prj == null)
                prj = this.GetBmdProject();

            if (prj == null)
                return "Linx";
            else
                return (((string)prj.Properties.Item("DefaultNamespace").Value) == "" ? (string)prj.Properties.Item("AssemblyName").Value : (string)prj.Properties.Item("DefaultNamespace").Value);
        }
        ...
        public void AdjustColumnNames()
        {
            //Adjust name space.
            var classes = (from cls in this.Types
                           where cls is ModelClass && !(cls is ReferenceModelClass) && ((ModelClass)cls).Kind != ClassKind.DatabaseScript
                           select (ModelClass)cls).Where(e => (!String.IsNullOrWhiteSpace(e.Table) && e.Table == e.Name) || e.Attributes.Any(p => !String.IsNullOrWhiteSpace(p.ColumnName) && p.ColumnName == p.Name));
            if (classes.Count() > 0)
            {
                using (Transaction transaction =
                           this.Store.TransactionManager.BeginTransaction("Changing ColumnNames."))
                {
                    foreach (var entity in classes)
                    {
                        if (!String.IsNullOrWhiteSpace(entity.Table) && entity.Table == entity.Name)
                            entity.Table = "";
                        foreach (var prop in entity.Attributes.Where(p => !String.IsNullOrWhiteSpace(p.ColumnName) && p.ColumnName == p.Name))
                        {
                            prop.ColumnName = "";
                        }
                    }
        ...
        public string GenerateStoreScriptsCode(string indent, List<BusinessModelDesignerRoot> models, bool isCore = false)
        {
            Tools.CodeBuilder builder = new Tools.CodeBuilder(indent);
            string parameters;
            ...
            if (scripts.Count > 0)
            {
                ...
                foreach (var script in scripts.OrderBy(e => e.Name))
                {
                    ...
                    foreach (var storeQuery in script.StoreQueries)
                    {
                        ...
                        builder.AddLine("{");
                        if (isCore)
                            builder.AddLine("   return this.Set<" + storeQuery.GenericType + ">().FromSql(\"" + storeQuery.Command + "\"" + parameters + ");");
                        else
        ...
        {
            string dirPart = null;
            if (this.GetInstalledFrameworkPath("").IsNullOrEmpty())
            {
        ...
        public void AdjustMissingReferences(Project project)
        {
            if (!project.IsNull())
            {
                ...
                foreach (string reference in references)
                {
                    ...
                    if ((reference.Right(2) == "BM" || reference.Contains(".BM.")) && File.Exists(filePath))
                    {
                        this.UpdateReference(project, filePath);
                    }
                }
            }
        }
        ...
        {
            return this.AddNewReference(this.GetBmdProject(), strAssemblyName, copyLocal, specificVersion);
        }
        ...
        private void UpdateReference(EnvDTE.Project project, string reference, bool remove = false, bool copyLocal = false, bool specificVersion = false)
        {
        ...
        {
            this.RemoveReference(this.GetBmdProject(), strAssemblyName);
        }
        ...
        {
            return VerifySourceControl(fileName, this.GetBmdProject());
        }

        public static bool VerifySourceControl(string fileName, Project project)
        {
            //if (File.Exists(fileName) && project != null)
            //    return TfsAccess.VerifySourceControl(project.DTE, fileName);

            return true;
        }


        public List<ModelClass> GetModelClasses(List<BusinessModelDesignerRoot> models)
        {
        ...
        public List<ModelClass> SortElementsByInheritance(List<ModelClass> types)
        {
            ...
            var topClasses = types.Where(e => e.Superclass == null && e.SuperclassSh == null && (e.Subclasses.Count > 0 || e.SubclassesSh.Count > 0)).ToArray();
        ...
        public BusinessModelDesignerRoot GetMainModel(List<BusinessModelDesignerRoot> models)
        {
        ...
        public string GetUniqueValuesDefinition(string indent, List<BusinessModelDesignerRoot> models)
        {
            Linx.Tools.CodeBuilder builder = new Tools.CodeBuilder(indent);
            List<string> objects = new List<string>();
            string objectName;
        ...
        public string GetProviderUsing(string indent, Provider provider)
        {
            string result = "";
            ...
            {
                case Provider.SQLServer:
                    result = indent + "optionsBuilder.UseSqlServer(GetConnectionString(\"" + GetConnectionName() + "\", null));";
                    break;
                case Provider.Oracle:
                    break;
                case Provider.SQLite:
                    result = indent + "optionsBuilder.UseSqlite(GetConnectionString(\"" + GetConnectionName() + "\", null));";
                    break;
                case Provider.MySQL:
                    result = indent + "MySQL.Data.EntityFrameworkCore.Extensions.MySQLDbContextOptionsExtensions.UseMySQL(optionsBuilder, GetConnectionString(\"" + GetConnectionName() + "\", null));";
                    break;
                case Provider.PostgreSQL:
                    result = indent + "optionsBuilder.UseNpgsql(GetConnectionString(\"" + GetConnectionName() + "\", null));";
                    break;
                default:
                    break;
            }
        ...
        public string GetConstraintDescriptors(string indent, List<BusinessModelDesignerRoot> models, Provider defaultProvider)
        {
            Linx.Tools.CodeBuilder builder = new Tools.CodeBuilder(indent);
            List<string> objects = new List<string>();
            string objectName;
        ...
        public string GetConnectionString(string indent, List<string> connections, bool netCore = false)
        {
            var defaultProvider = this.DbProviders.FirstOrDefault(e => e.IsDefault);
            if (defaultProvider != null && !connections.Contains(defaultProvider.ConnectionName))
            {
                Linx.Tools.CodeBuilder builder = new Linx.Tools.CodeBuilder(indent);
                connections.Add(defaultProvider.ConnectionName);

                if (netCore)
                {
                    builder.AddLine("\"" + defaultProvider.ConnectionName + "\": \"" + defaultProvider.GetConnectionString().Replace("\\", "\\\\") + "\"");
                }
                else
                {
                    builder.AddLine(defaultProvider.GetConnectionConfiguration());
                }
        ...
        {
            string outputFile = "";
            Project current = this.GetBmdProject();
            ProjectItem newItem = null;
            string body = ReadResourceContent(@"Linx.BusinessModelDesigner.Templates.AppConfigTemplate.txt");

            if (!current.IsNull())
            {
                if (ExistsProjectItem(current.ProjectItems, "App.config"))
                    current.ProjectItems.Item("App.config").Delete();

                outputFile = Path.Combine(GetProjectPath(current), "App.tt");
                if (!this.VerifySourceControl(outputFile))
                    return;

                if (!ExistsProjectItem(current.ProjectItems, "App.tt"))
                {
                ...
                else
                {
                    if (File.ReadAllText(outputFile) != body)
                        File.WriteAllText(outputFile, body);
                    newItem = current.ProjectItems.Item("App.tt");
                }
                //Run Template
        ...
        const string sharedFolderName = "Shared";
        ...
        public Dictionary<string, string> GetSharedInfo()
        {
            ...
            if (defProvider != null)
            {
                ...
                contextInfo["ControlIdLinxByApplicative"] = this.ControlIdLinxByApplicative.ToString().ToLower();
                contextInfo["IsAspNetCore"] = this.IsAspNetCore.ToString().ToLower();
                contextInfo["DisableAllAudits"] = this.DisableAllAudits.ToString().ToLower();
            ...
            contextInfo["ControlIdLinxByApplicative"] = "false";
            contextInfo["IsAspNetCore"] = "false";
            contextInfo["DisableAllAudits"] = "false";
        ...
        public void UpdateContextTemplate()
        {
            string outputFile = "";
            Project current = this.GetBmdProject();
            ProjectItem folder = null, newItem;
            ...
            {
                string folderName = "Model";
                string body = AdjustGacPath(ReadResourceContent(@"Linx.BusinessModelDesigner.Templates.BusinessModelDesignerTemplate.txt"));
                ...
                if (folder != null)
                {
                    outputFile = Path.Combine(Path.Combine(this.GetProjectPath(), folderName), ctxFile);
                    if (!this.VerifySourceControl(outputFile))
                        return;
                    ...
                    if (newItem.ProjectItems.Count > 1)
                    {
                        foreach (ProjectItem item in newItem.ProjectItems)
                        {
                            item.Delete();
                        }
                    }
        ...
        #region Customer Customization
        public void GenerateCustomizationProject()
        {
            ///generate INTERFACE
            GenerateCustomizationAbstractInterface();

            //create project for send to client
            CreateCustomizationtProject();

        }

        private void GenerateCustomizationAbstractInterface()
        {
            Project bmdProject = GetBmdProject();
            var FileName = this.DataContextName + "Extended.cs";
            var modelFolder = GetProjectItemByName(bmdProject, GetContractModelFolderName());

            if (modelFolder != null)
            {
                var outputFileName = Path.Combine(this.GetProjectPath(), modelFolder.Name, FileName);

                WriteFile(outputFileName, getCustomizationAbstractInterface(), modelFolder.ProjectItems);
            }
            else
            {
                throw new DirectoryNotFoundException("BmdProject\\Model");
            }

        }

        private string GetCustomizationProjectName(Project bmProject)
        {
            return bmProject.Name + ".Customizations";
        }

        private Project GetCustomizationProject(Project bmProject = null)
        {
            if (bmProject == null)
                bmProject = GetBmdProject();

            if (bmProject != null)
                return this.GetProjectByName(GetCustomizationProjectName(bmProject));

            throw new ArgumentOutOfRangeException("Not found BM Project.");
        }

        private string GetCustomizationFolderName()
        {
            return "External Customization";
        }

        private void CreateCustomizationtProject()
        {
            Project bmdProject = GetBmdProject();
            EnvDTE.DTE appDTE = bmdProject.DTE;
            string customizationProjectName = GetCustomizationProjectName(bmdProject);
            Project customizationProject = GetProjectByName(customizationProjectName);
            string folderName = GetCustomizationFolderName();
            string implementationFileName = this.DataContextName + "Customized.cs";

            if (customizationProject == null)
            {
                string repositoryDir = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(bmdProject.FullName), "..\\" + customizationProjectName));
                ...
                if (businessDesignerFolder == null)
                    businessDesignerFolder = (EnvDTE80.SolutionFolder)((EnvDTE100.Solution4)appDTE.Solution).AddSolutionFolder(folderName).Object;
                var projPath = Path.Combine(repositoryDir, customizationProjectName + ".csproj");
                if (File.Exists(projPath))
                {
                    if (businessDesignerFolder != null)
                        businessDesignerFolder.AddFromFile(projPath);
                    else
                        ((EnvDTE100.Solution4)appDTE.Solution).AddFromFile(projPath, false);

                    customizationProject = GetProjectByName(customizationProjectName);
                }
                else
                {
                    // Get the location of the project templates
                    string templateName = ((EnvDTE100.Solution4)appDTE.Solution).GetProjectTemplate("Class Library (.NET Framework)", "CSharp");
                    if (businessDesignerFolder != null)
                        businessDesignerFolder.AddFromTemplate(templateName, repositoryDir, customizationProjectName);
                    else
                        ((EnvDTE100.Solution4)appDTE.Solution).AddFromTemplate(templateName, repositoryDir, customizationProjectName, false);

                    customizationProject = GetProjectByName(customizationProjectName);

                    //Delete Class1.cs from template project
                    if (ExistsProjectItem(customizationProject.ProjectItems, "Class1.cs"))
                        customizationProject.ProjectItems.Item("Class1.cs").Delete();

                    //Set Assembly Name
                    customizationProject.Properties.Item("AssemblyName").Value = customizationProjectName;
                    //Set Default Namespace
                    customizationProject.Properties.Item("DefaultNamespace").Value = customizationProjectName;
                }
            }
            var implementationFile = GetProjectItemByName(customizationProject, implementationFileName);
            if (implementationFile == null)
            {
                var outputFile = Path.Combine(GetProjectPath(customizationProject), implementationFileName);
                WriteFile(outputFile, getCustomizationImplementation(), customizationProject.ProjectItems);
            }

            //this.RemoveReferencesWithoutFile(customizationProject);
            this.AdjustMissingReferences(customizationProject);
            this.UpdateVersion(customizationProject);

            //Update library references
            //add project BM reference
            this.AddProjectReference(customizationProject, bmdProject, false);

            if (!this.ExistsReference(customizationProject, "System.ComponentModel.Composition.dll"))
                this.AddNewReference(customizationProject, "System.ComponentModel.Composition.dll");

            this.UpdatePackages(customizationProject);
            ...
            //Upgrade to last framework version
            UpgradeVersion(customizationProject);
        }

        private string getCustomizationImplementation()
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("using System.ComponentModel.Composition;");
            sb.AppendLine("using System.Data.Entity;");
            sb.AppendLine();
            sb.AppendLine(string.Format("namespace {0}.Customizations", this.TargetNamespace));
            sb.AppendLine("{");
            sb.AppendLine(string.Format("    [Export(typeof({0}Extended))]", this.DataContextName));
            sb.AppendLine(string.Format("    public class {0}Customized : {0}Extended", this.DataContextName));
            sb.AppendLine("    {");
            sb.AppendLine();
            sb.AppendLine("    }");
            sb.AppendLine("}");
        ...
        private string getCustomizationAbstractInterface()
        {
            StringBuilder sb = new StringBuilder();
            string t = new string(' ', 4);
            Action<string, string, string, string> GenerateMethods = (methodName, returnType, returnDefault, parameters) =>
            {

                sb.AppendLine(t + t + string.Format("#region method: {0}", methodName));
                sb.AppendLine(t + t + string.Format("public virtual bool Has{0}Customized", methodName) + " { get { return false; } }");
                sb.AppendLine();
                sb.AppendLine(t + t + string.Format("public virtual {1} {0}Customized({2})", methodName, returnType ?? "void", parameters));
                sb.AppendLine(t + t + "{");
                sb.AppendLine(t + t + string.Format("    return {0};", returnDefault));
                sb.AppendLine(t + t + "}");
                sb.AppendLine(t + t + string.Format("#endregion method: {0}", methodName));
                sb.AppendLine();
            };
            ...
            sb.AppendLine("using System.Data.Entity; ");
            sb.AppendLine();
            sb.AppendLine(string.Format("namespace {0}", this.TargetNamespace));
            sb.AppendLine("{");
            sb.AppendLine(string.Format("    public abstract class {0}Extended", this.DataContextName));
            sb.AppendLine("    {");
            GenerateMethods("Load", null, null, "DbContext context");
            sb.AppendLine();
            GenerateMethods("BeforeSaveChanges", "bool", "true", "DbContext context");
            sb.AppendLine();
            GenerateMethods("AfterSaveChanges", null, null, "DbContext context");
            sb.AppendLine("    }");
            sb.AppendLine("}");
        ...
        public void UpdateDomainViewsTemplate(Project current = null)
        {
            string outputFile = "";
            if (current == null)
                current = this.GetBmdProject();
            ...
            if (current != null)
            {
                ...
                string body = AdjustGacPath(ReadResourceContent(@"Linx.BusinessModelDesigner.Templates.DomainViewsTemplate.txt"));
                ...
                if (folder != null)
                {
                    outputFile = Path.Combine(Path.Combine(GetProjectPath(current), folderName), "DomainViews.tt");
                    if (!this.VerifySourceControl(outputFile))
                        return;

                    if (!ExistsProjectItem(folder.ProjectItems, "DomainViews.tt"))
                    {
                        File.WriteAllText(outputFile, body);
                    ...
                    else
                    {
                        if (File.ReadAllText(outputFile) != body)
                            File.WriteAllText(outputFile, body);
                        projItem = folder.ProjectItems.Item("DomainViews.tt");
                    }
                    //Run Template
        ...
        public void GenerateBusinessOperations(bool isShared, Project project = null)
        {
            if (project == null)
                project = this.GetBmdProject();
            if (project.IsNull())
            ...
            if (!item.IsNull())
            {
                ...
                foreach (ModelClass modelClass in this.Types.Where(e => e is ModelClass))
                {
                    ...
                    {
                        outputFile = Path.Combine(GetProjectPath(project), item.Name + "\\" + modelClass.Name + ".Operations" + (isShared ? ".shared" : "") + ".cs");
        ...
        private void GenerateEntityOperationsCode(StringBuilder codeBuilder, ModelClass modelClass, bool isShared)
        {
            ...
            codeBuilder.AppendLine("");
            codeBuilder.AppendLine("namespace " + modelClass.BusinessModelDesignerRoot.TargetNamespace);
            codeBuilder.AppendLine("{");
        ...
        public void UpdateDynamicModelsTemplate(Project current, bool directDiagram = false, Project reference = null)
        {
            string outputFile = "";
            ProjectItem folder = null, projItem;
            ...
            if (current != null)
            {
                ...
                string body = AdjustGacPath(ReadResourceContent(@"Linx.BusinessModelDesigner.Templates.DynamicModelsTemplate.txt"));
                if (directDiagram)
                ...
                if (folder != null)
                {
                    string templateName = (directDiagram ? "Direct" : String.Empty) + "DynamicModels.tt";
