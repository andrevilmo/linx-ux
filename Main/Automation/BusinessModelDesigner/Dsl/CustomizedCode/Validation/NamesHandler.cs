...
namespace Linx.BusinessModelDesigner
{
    ...
    public partial class NamedElement
    {
        ...
        public void ValidDataContextName(BusinessModelDesignerRoot root, ValidationContext context)
        {
            if (root.DataContextName == "BusinessModel" && root.DbProviders.Count > 0)
        ...
        public void ValidDiscriminators(IEnumerable<ModelClass> types, ValidationContext context)
        {
            ...
            foreach (ModelClass element in types.Where(e => e.SuperclassSh != null))
            {
                ...
                foreach (var link in links)
                {
                    ...
                    {
                        string description = String.Format(CultureInfo.CurrentCulture, "The entity '{0}' has errors over its discriminator. Check if only one discriminator property is used.", element.SuperclassSh.Name); // and if all the used values are distinct
        ...
        {
            //Check ModelViews
            foreach (ModelClass element in types.Where(e => e is ModelClass && ClassKindHelper.IsModelViewBehavior(((ModelClass)e).Kind)))
            {
