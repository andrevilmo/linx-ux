...
namespace Linx.BusinessModelDesigner
{
    ...
    internal partial class BusinessModelDesignerCommandSet
    {
        private Guid cmdidBusinessModelDesignerToolWindowGUID = new Guid("{e024017a-a1a6-42ed-84d6-96b12b183a70}");
        private const uint cmdidBusinessModelDesignerToolWindow = 0x8700;
        ...
        private Guid cmdidSyncSqlScriptGUID = new Guid("{e024017a-a1a6-42ed-84d6-96b12b183a87}");
        private const int cmdidSyncSqlScriptID = 0x8816;
        ...
        private Guid cmdRemoveStudyModeGUID = new Guid("{e024017a-a1a6-42ed-84d6-96b12b174aca}");
        private const int cmdRemoveStudyModeID = 0x8833;

        private Guid cmdGenMetaDataGUID = new Guid("{e024017a-a1a6-42ed-84d6-96b12b174afb}");
        private const int cmdGenMetaDataID = 0x8834;
        ...
        protected override System.Collections.Generic.IList<System.ComponentModel.Design.MenuCommand> GetMenuCommands()
        {
            ...
            new EventHandler(OnPopUpMenuDisplayAction),
            new EventHandler(OnPopUpMenuClick),
            new CommandID(cmdidSyncSqlScriptGUID, cmdidSyncSqlScriptID)));
            ...
            new EventHandler(OnPopUpMenuDisplayAction),
            new EventHandler(OnPopUpMenuClick),
            new CommandID(cmdRemoveStudyModeGUID, cmdRemoveStudyModeID)));
        ...
        internal void OnPopUpMenuDisplayAction(object sender, EventArgs e)
        {
            ...
            foreach (object selectedObject in this.CurrentSelection)
            {
                ...
                if (selectedObject is ShapeElement && ((ShapeElement)selectedObject).ParentShape is BusinessModelDesignerDiagram && (command.CommandID.ID == cmdidSendToBackID || command.CommandID.ID == cmdidBringToFrontID))
                {
                    command.Visible = true;
                    command.Enabled = true;
                    break;
                ...
                if (selectedObject is ClassShape && (command.CommandID.ID == cmdidConfigureBusinessViewID) && ((ModelClass)((ClassShape)selectedObject).ModelElement).Kind == ClassKind.ModelView)
                {
                    command.Visible = true;
                    command.Enabled = true;
                    break;
                ...
                if (selectedObject is ClassShape && (command.CommandID.ID == cmdidSyncSqlScriptID))
                {
                    var entity = ((ModelClass)((ClassShape)selectedObject).ModelElement);
                    bool isEnabled = entity.IsFreeSelectCommand();
        ...
        internal void OnPopUpMenuClick(object sender, EventArgs e)
        {
            ...
            foreach (object selectedObject in this.CurrentSelection)
            {
                ...
                if (selectedObject is BusinessModelDesignerDiagram)
                {
                    BusinessModelDesignerRoot designer = (BusinessModelDesignerRoot)((BusinessModelDesignerDiagram)selectedObject).ModelElement;
                    switch (command.CommandID.ID)
                    {
                        case cmdShowDistributorNameID:
                            MessageBox.Show(designer.GetDistributorProductName(), "Distributor Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            break;

                        case cmdRemoveStudyModeID:
                            designer.SetStudyMode(false);
                            break;

                        case cmdGenMetaDataID:
                            designer.UpdateContextMetadata();
                            break;
                        ...
                        case cmdidGenerateScriptID:
                            designer.GenerateScript(((BusinessModelDesignerDiagram)selectedObject));
                            break;

                        case cmdidReverseEngineerID:
                            designer.ExecuteReverseEngineer(((BusinessModelDesignerDiagram)selectedObject));
                            break;
                        ...
                        case cmdidArrangeLayoutID:
                            ((BusinessModelDesignerDiagram)selectedObject).ArrangeLayout();
                            break;
                ...
                {                    
                    var entity = ((ModelClass)((ClassShape)selectedObject).ModelElement);
                    entity.BusinessModelDesignerRoot.SyncScriptColumns(entity.Name, entity.DatabaseScriptCommand);
                    break;
                }

                if (selectedObject is DomainViewShape && command.CommandID.ID == cmdDomainValuesID)
                {
                    Linx.BusinessModelDesigner.CustomCode.FrmDomainValues builder = new Linx.BusinessModelDesigner.CustomCode.FrmDomainValues(((DomainViewShape)selectedObject).ModelElement as DomainView);
                    builder.ShowDialog();
                    break;
                }
                ...
                if (selectedObject is ShapeElement && ((ShapeElement)selectedObject).ParentShape is BusinessModelDesignerDiagram && (command.CommandID.ID == cmdidSendToBackID || command.CommandID.ID == cmdidBringToFrontID))
                {
                    BusinessModelDesignerRoot designer = (BusinessModelDesignerRoot)(((ShapeElement)selectedObject).ParentShape).ModelElement;
