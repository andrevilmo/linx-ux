...
namespace Linx.BusinessDataModelDesigner
{
    public partial class BusinessDataModelDesignerRoot
    {
        ...
        {
            ModelClass newClass = this.Types.FirstOrDefault(e => e is ModelClass && ((ModelClass)e).Table == name) as ModelClass;
            if (newClass != null)
            {
                foreach (var attrib in newClass.Attributes.Where(e => !e.ColumnName.IsNullOrEmpty() && !columns.Contains(e.ColumnName)).ToList())
                {
                    attrib.Delete();
                }
            }
        ...
        {            
            ModelClass targetClass = this.Types.FirstOrDefault(e => e is ModelClass && ((ModelClass)e).Table == name) as ModelClass;
            if (targetClass != null)
            {
        ...
        public void AddTableClass(string name, ClassKind kind, string schema, string primaryKeyConstraintName, bool isClustered)
        {
            ModelClass newClass = this.Types.FirstOrDefault(e => e is ModelClass && ((ModelClass)e).Table == name) as ModelClass;
            if (newClass == null)
            {
                            ...
                            Int16 scale, Int16 maxLength, int indexOrder, string sqlDefault)
        {
            ...
            ModelClass modelClass = this.Types.FirstOrDefault(e => e is ModelClass && ((ModelClass)e).Table == parentTable) as ModelClass;
            if (modelClass != null && !(modelClass is ReferenceModelClass))
            {
                ModelAttribute newAttr = modelClass.Attributes.FirstOrDefault(e => e.ColumnName == name);
                if (newAttr == null)
                {
                            ...
                            bool deleteCascade, string[] columns)
        {
            ...
            ModelClass source = this.Types.FirstOrDefault(e => e is ModelClass && ((ModelClass)e).Table == tableFrom) as ModelClass;
            ModelClass target = this.Types.FirstOrDefault(e => e is ModelClass && ((ModelClass)e).Table == tableTo) as ModelClass;
            if (source != null && target != null && !(target is ReferenceModelClass))
            {
                ...
                if (link != null)
                {
                    ...
                    {
                        ModelAttribute attrTarget = target.Attributes.FirstOrDefault(e => e.ColumnName == columnRef.Left(":"));
                        if (attrTarget != null)
                        {
                            ModelAttribute ettrSource = source.Attributes.FirstOrDefault(e => e.ColumnName == columnRef.Right(":"));
                            if (ettrSource != null)
                            {
                                attrTarget.ForeignKey = link.GetFkId() + "." + ettrSource.Name;
                                if (!attrTarget.IsPrimaryKey && attrTarget.Name == attrTarget.ColumnName)
                                    attrTarget.Name = attrTarget.ColumnName;
                            }
                        }
                    }
                }
