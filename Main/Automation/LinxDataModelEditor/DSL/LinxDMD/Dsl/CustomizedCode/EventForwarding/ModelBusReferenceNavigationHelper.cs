...
namespace Linx.BusinessDataModelDesigner
{
    ...
    public static class ModelBusReferenceNavigationHelper
    {
        public static void OpenCode(this object element)
        {
            ...
            {
                ((ClassOperation)element).ModelClass.BusinessDataModelDesignerRoot.OpenCodeElement(((ClassOperation)element));
            }
            else if (element is ModelAttribute)
            {
                ((ModelAttribute)element).ModelClass.BusinessDataModelDesignerRoot.OpenCodeElement(element);
            }
            else if (element is ClassShape)
            {
                ((ModelClass)((ClassShape)element).ModelElement).BusinessDataModelDesignerRoot.OpenCodeElement(((ClassShape)element).ModelElement);
            }
        }
        ...
        public static void NavigateTo(this ModelElement caller, ModelBusReference referenceToNavigateTo)
        {
            ...
            using (ModelBusAdapter modelAdapter = manager.CreateAdapter(referenceToNavigateTo))
            {
                if (modelAdapter != null)
                {
                    ...
                    if (referencedModelClass != null)
                    {
                        ...
                        referencedModelClass.BusinessDataModelDesignerRoot.SelectShape(referencedModelClass.Name);
                    }
        ...
        public static T GetModelRoot<T>(this ModelElement caller, ModelBusReference reference, ModelBusAdapterManager manager) where T : ModelElement
        {
            // We cannot navigate to a null modelbus reference.
            if (reference == null)
            {
            ...
            // Get the ModelBus
            IModelBus modelbus = manager.ModelBus;
            T referencedModelRoot = null;

            try
            {
                using (ModelBusAdapter modelAdapter = manager.CreateAdapter(reference))
                {
                    if (modelAdapter != null)
                    {
                        // Get the ModelRoot
