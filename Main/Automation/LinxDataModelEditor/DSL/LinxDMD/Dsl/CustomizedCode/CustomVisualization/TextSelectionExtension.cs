...
namespace Linx.BusinessDataModelDesigner
{
	...
	{
		public static void OpenOperation(this TextSelection selection, Operation operation, string className)
		{
			OpenOperation(selection, operation, className, String.Empty);
		}

		public static void OpenOperation(this TextSelection selection, Operation operation, string className, String insertCommandText)
		{
			try
			{
		...
		private static TextPoint GetOperationLine(this TextSelection selection, Operation operation)
		{
			try
			{
        ...
        public static Dictionary<string, string[]> GetOperationSignatureInfo(this TextSelection selection, Operation operation)
        {
		...
		private static bool CheckParameters(this CodeFunction function, Operation operation)
		{
			bool result = true;
		...
		private static void AddParameters(this CodeFunction function, Operation operation)
		{
		...
		private static void AddAttributes(this CodeFunction function, Operation operation)
		{
			string attributeName, attributeParams;
            ...
            if (operation is WebApiAction)
            {
                ...
                //Base route
                //if (apiAction.WebApiController.BusinessDataModelDesignerRoot.EnableAutomaticAuthorization)
		...
		private static void AddOperation(this TextSelection selection, Operation operation)
		{
			try
			{
				// Retrieve the CodeClass at the insertion point.
        ...
        public static void MoveToCodeElement(this TextSelection selection, string className, string elementName, string attributes)
		{
			try
			{
                string textElement = (elementName.IsNullOrEmpty() ? className : elementName);
				selection.StartOfDocument(false);
                if (!selection.FindText("'" + textElement + "'", 0))
				{
                    selection.FindText(className, 0);
				}
			}
			catch (Exception ex)
			{
