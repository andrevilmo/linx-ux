...
namespace Linx.BusinessDataModelDesigner
{
    public partial class ClassShape
    {
            ...
            foreach (var element in e.DiagramHitTestInfo.HitDiagramItem.RepresentedElements)
            {
                if (element is ModelAttribute)
                {
                    if (!((ModelAttribute)element).ForeignKey.IsNullOrEmpty())
                    {
                        ...
                        {
                            ((ModelAttribute)element).ModelClass.BusinessDataModelDesignerRoot.SelectLink(link);
                            if (link is Association)
                            {
                                ((Association)link).SelectProperties();
                                ((ModelAttribute)element).ModelClass.BusinessDataModelDesignerRoot.RefreshFocusedDiagramView();
                            }
        ...
        protected override void InitializeFromMappings()
        {
            ...
            {
                ElementListCompartmentMapping operationMapping = ClassShape.compartmentMappings.First().Value.FirstOrDefault(e => e.CompartmentId == "OperationsCompartment") as ElementListCompartmentMapping;
                ...
                ElementListCompartmentMapping attributeMapping = ClassShape.compartmentMappings.First().Value.FirstOrDefault(e => e.CompartmentId == "AttributesCompartment") as ElementListCompartmentMapping;
                if (attributeMapping != null)
                {
                    attributeMapping.ImageGetter = ClassShape.GetElementImage;
                    ClassShape.initializeFromMappings = true;
                }
            }
        ...
        private static Image GetElementImage(ModelElement mel)
        {
            ...
            {
                ClassOperation operation = mel as ClassOperation;
                if ((operation != null) && operation.IsEvent)
                {
