    ...
    public abstract class ScriptGeneratorBase
    {
        ...
        protected virtual void GenerateScriptIndexes()
        {
            ...
            foreach (var index in indexes)
            {
                ...
                {
                    AddLine("IF EXISTS(SELECT * FROM sys.indexes WHERE name = '{0}' AND object_id = object_id('[{1}].[{2}]'))", index.Name, index.Table.Schema.Name, index.Table.Name);
                    AddLine(_indent + "DROP INDEX [{0}] ON [{1}].[{2}]", index.Name, index.Table.Schema.Name, index.Table.Name);
                }
                AddLine("CREATE{0}{1} INDEX [{2}] ON [{3}].[{4}]([{5}]){6}",
                    (index.IsUnique || index.IsUniqueConstraint ? " UNIQUE" : ""), //unique
                    index.IsClustered ? " CLUSTERED" : " NONCLUSTERED", //Clustered
                    index.Name, //index Name
                    index.Table.Schema.Name, //schema
                    index.Table.Name, //table
                    String.Join("],[", index.Columns.Select(c => c.Name)), //columns
                    (index.Include.Count == 0 ? "" : " INCLUDE ([" + string.Join("],[", index.Include.Select(c => c.Name)) + "])") //include
                    );
            }
        }
