...
namespace Microsoft.VisualStudio.Project
{
    ...
    public class BuildableProjectConfig : IVsBuildableProjectCfg
    {
        ...
        private void Build(uint options, IVsOutputWindowPane output, string target)
        {
            ...
            IVsProjectCfg prjConfig;
            this.get_ProjectCfg(out prjConfig);
            string nodePath = Path.Combine(((Microsoft.VisualStudio.Project.ProjectConfig)prjConfig).ProjectMgr.ProjectFolder, "NodePack");

            string npmInstall = "";
            if (!Directory.Exists(Path.Combine(nodePath, "node_modules")))
                npmInstall = " & npm install -g nodemon & npm install";

            ExecuteFile("cmd.exe", @"/c cd """ + nodePath + @"""" + npmInstall + " & nodemon server.CodeGen.js");

            this.NotifyBuildEnd(MSBuildResult.Successful, target);
            //MessageBox.Show("This project cannot be builded!", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
