define(['durandal/app', 'pkg_linx-framework-bv-spa/services/UsuarioContext', 'plugins/router', 'plugins/widget', 'managers/__auth', 'viewmodels/shared/modal', 'viewmodels/shared/modal2', 'managers/brand', 'managers/predefinedFilters', 'services/logger', 'viewmodels/shared/modalMultimidia', 'common', 'pkg_linx-framework-bv-spa/viewmodels/CadastroUsuarioComplement', 'viewmodels/shared/modalCustomSearch'],
...
var vmConstructor = function () {
    ...
    var objectLayout = function () {
       return {Name: 'CadastroUsuario', Items: [
	 ...
	 {Name: "CadastroUsuario_ckUtilizaSso", DisplayName: "Utiliza SSO", ColumnSpan: 3, Visible: true, Key: "IndicaUtilizaSso"},
	 {Name: "CadastroUsuario_ckUtilizaMfa", DisplayName: "Utiliza MFA", ColumnSpan: 3, Visible: true, Key: "IndicaUtilizaMfa"},
	 {Name: "CadastroUsuario_btnRevogaMfa", DisplayName: "Revoga MFA", ColumnSpan: 3, Visible: true, Key: ""},
	 {Name: "CadastroUsuario_btnRevogaSso", DisplayName: "Revoga SSO", ColumnSpan: 3, Visible: true, Key: ""},]},
    ...
    var changeLanguage = function() {
        ...
        require([fName],
            function(result) {
            vm.flattenLayout(ko.observable(flattenObjectByProperty(result.objectLanguage_CadastroUsuario(), 'Name'))());
        }, function (err) {
      ...
      if ((typeof settings === 'object') && (settings != null) && settings.objectQuery) {
          ...
          if ((typeof settings === 'object') && (settings != null) && settings.uiSettings) {
              ...
              if (uiSettings.executeQuery === true) {
                  ...
                  filteredEntities = [];
                  if (settings.parentVM) { settings.parentVM.internalUIs = [ 'CadastroUsuario' ]; settings.parentVM.CadastroUsuario = getVM; }
                  clear();
                  ...
                  if ((typeof settings.parentVM === 'object') && settings.parentVM != null) {
                      ...
                      parentVM.CadastroUsuario = getVM;
                      if (isLookup()) { 
                      ...
                      if ($.inArray('CadastroUsuario', parentVM.internalUIs) === -1){
                           if (parentVM.internalUIs) {
                               parentVM.internalUIs.push('CadastroUsuario');
                           }
                           else {
                               parentVM.internalUIs = ['CadastroUsuario'];
                           }
                      }
                  }
              }
    ...
    var adjustModuleSecurity = function () {
        ...
        managerAuth.getFormAccess('linx-framework-bv-spa-CadastroUsuario', function (data) {
    ...
    var canActivate = function() {
        ...
        if (data.lxShellCompiledVersion != managerAuth.shellVersion) {
            app.showMessage('Versão de formulário incompatível com a versão de ambiente [' + managerAuth.shellVersion + '].', 'Formulário: CadastroUsuario', ['Ok']);
            return false;
        }
        return true;
    };
    ...
    var compositionComplete = function() {
        //changeLanguage();
        $('#CadastroUsuario_tcTcsUsuarioTabControl').on('shown.bs.tab', function (e) { vm.notifyInnerElements($(e.target.hash)); });
    initializeTabControl('#CadastroUsuario_tcTcsUsuarioTabControl');

    complement.renderCadastroUsuario_dGridTcsUsuarioPerfil(vm);

    complement.renderCadastroUsuario_dGridTcsUsuarioRegraModulo(vm);

    complement.renderCadastroUsuario_dGridTcsUsuarioRegraTransacao(vm);

    complement.renderCadastroUsuario_dGridTcsUsuarioRegraColuna(vm);

    complement.renderCadastroUsuario_dGridTcsUsuarioBandeiraRede(vm);

    complement.renderCadastroUsuario_dGridTcsUsuarioFilial(vm);

    complement.renderCadastroUsuario_dGridTcsUsuarioLayout(vm);

    complement.renderscyCadastroUsuario_dGrid(vm);
        ...
        vm.currentBrands.subscribe(function(newValue) {
            ...
            var reset = (!newValue || searchedBrands.cod === ''), decimals = searchedBrands.decimals;
                               complement.ChangedBrandCadastroUsuario_dGridTcsUsuarioPerfil(vm, decimals, reset);
                           complement.ChangedBrandCadastroUsuario_dGridTcsUsuarioRegraModulo(vm, decimals, reset);

                           complement.ChangedBrandCadastroUsuario_dGridTcsUsuarioRegraTransacao(vm, decimals, reset);

                           complement.ChangedBrandCadastroUsuario_dGridTcsUsuarioRegraColuna(vm, decimals, reset);

                           complement.ChangedBrandCadastroUsuario_dGridTcsUsuarioBandeiraRede(vm, decimals, reset);

                           complement.ChangedBrandCadastroUsuario_dGridTcsUsuarioFilial(vm, decimals, reset);

                           complement.ChangedBrandCadastroUsuario_dGridTcsUsuarioLayout(vm, decimals, reset);
    ...
    var getMaxLength = function(entityName, propertyName){
        if (isNullOrEmpty(entityName)) entityName = 'TcsUsuario';
    ...
    var visibleColumns = 'NomeUsuario,LxPfjFisicaJuridica,CnpjCpf,InscrEstadualRg,DataAlteracao,DataCadastro,LxTipoLogradouro,Logradouro,Numero,Complemento,Cep,Bairro,Municipio,Uf,ObsEndereco,FoneFixo,Ramal,FoneCelular';
    ...
    var exportData = function (forceAdd, isExcelDataSource) {
        if (forceAdd)
            require(['viewmodels/shared/addCustomExport'],
                function(addCustomExport){ addCustomExport.showModal(vm, null, 'TcsUsuario', getVisiblePropertiesForExcel('dataView'), null, true, isExcelDataSource); } );
        else
            require(['viewmodels/shared/customExport'],
                function(modalExport){ modalExport.showModal(vm, 'TcsUsuario', getVisiblePropertiesForExcel('dataView'), null, { canAdd: true, canEdit: true, canDel: true }, isExcelDataSource); } );
    };
    ...
    var getAddedEntities = function () {
        return dataContext.getEntities('TcsUsuario', [dataContext.breeze.EntityState.Added]);
    }
    var getRelatedElementsInCache = function () {
        if (parentEntityRelated != null && preserveDataCurrentState()) {
           var cacheElements = dataContext.getEntities('TcsUsuario');
           var result = [];
           var relationExpr = '';
    ...
    var query = function (lookupInitializing, parentEntity, quickSearchJExpression, externalQueryCallBack, noMessages, noDetails) {
        ...
        if (freeEntityForQuerying == null && isChildVM()) freeEntityForQuerying = dataContext.createFreeEntity('TcsUsuario');
        ...
        return dataContext.getTcsUsuarioByEntitySearchNoAssociations(lastJEntitySearch, 0, pageSize(), (pageSize() > 0), preserveDataCurrentState(), status() !== 'E', sortInfo, querySucceeded, complete);
        ...
        function querySucceeded(data) {
            if (vm.status() !== 'E') { for (var idx = 0; idx < data.results.length; idx++) { dataContext.initializePOCO(data.results[idx], 'TcsUsuario'); } }
            hasError = false;
    ...
    var refresh = function (curPage, goLast) {
        ...
        return dataContext.getTcsUsuarioByEntitySearchNoAssociations(lastJEntitySearch, curPage * pageSize(), pageSize(), false, false, status() !== 'E', sortInfo, querySucceeded, complete);
...
if(control.length){
    ...
    var clear = function (noBindingReport) {
        ...
        else return dataContext.clearTcsUsuario(getBandeiraRede(), clearComplete);
    ...
    var save = function (isExclusion, externalSaveSucceeded, transactionId, saveCompleteCallback, internalUiCallback) {
        ...
        function saveSucceeded(saveResult) {
            ...
            dataBind();
            resizeToolbar();
            if (!isChildVM()) currentDataItem().fillDetails(true, 'TcsUsuarioPerfil');
        }
    var dataForUndo = [];
    ...
    var createTcsUsuario = function() {
        dataBind('dataView', true);
        var entity = dataContext.createTcsUsuario();
    ...
    var createAndNotifyTcsUsuario = function() {
        var entity = createTcsUsuario();
        notifyPresentation('');
        return entity;
    };
    
    var createTcsUsuarioPerfil = function(parent, noCurrent) {
        dataBind('TcsUsuarioPerfilList', true);
        var entity = dataContext.createTcsUsuarioPerfil(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsUsuarioPerfil(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsUsuarioPerfil = function(parent) {
        var entity = createTcsUsuarioPerfil(parent);
        notifyPresentation('TcsUsuarioPerfilList');
        return entity;
    };
    
    var createTcsUsuarioRegraModulo = function(parent, noCurrent) {
        dataBind('TcsUsuarioRegraModuloList', true);
        var entity = dataContext.createTcsUsuarioRegraModulo(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsUsuarioRegraModulo(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsUsuarioRegraModulo = function(parent) {
        var entity = createTcsUsuarioRegraModulo(parent);
        notifyPresentation('TcsUsuarioRegraModuloList');
        return entity;
    };
    
    var createTcsUsuarioRegraTransacao = function(parent, noCurrent) {
        dataBind('TcsUsuarioRegraTransacaoList', true);
        var entity = dataContext.createTcsUsuarioRegraTransacao(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       ...
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsUsuarioRegraTransacao(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsUsuarioRegraTransacao = function(parent) {
        var entity = createTcsUsuarioRegraTransacao(parent);
        notifyPresentation('TcsUsuarioRegraTransacaoList');
        return entity;
    };
    
    var createTcsUsuarioRegraColuna = function(parent, noCurrent) {
        dataBind('TcsUsuarioRegraColunaList', true);
        var entity = dataContext.createTcsUsuarioRegraColuna(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsUsuarioRegraColuna(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsUsuarioRegraColuna = function(parent) {
        var entity = createTcsUsuarioRegraColuna(parent);
        notifyPresentation('TcsUsuarioRegraColunaList');
        return entity;
    };
    
    var createTcsUsuarioBandeiraRede = function(parent, noCurrent) {
        dataBind('TcsUsuarioBandeiraRedeList', true);
        var entity = dataContext.createTcsUsuarioBandeiraRede(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsUsuarioBandeiraRede(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsUsuarioBandeiraRede = function(parent) {
        var entity = createTcsUsuarioBandeiraRede(parent);
        notifyPresentation('TcsUsuarioBandeiraRedeList');
        return entity;
    };
    
    var createTcsUsuarioLayout = function(parent, noCurrent) {
        dataBind('TcsUsuarioLayoutList', true);
        var entity = dataContext.createTcsUsuarioLayout(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsUsuarioLayout(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsUsuarioLayout = function(parent) {
        var entity = createTcsUsuarioLayout(parent);
        notifyPresentation('TcsUsuarioLayoutList');
        return entity;
    };
    
    var createTcsUsuarioFilial = function(parent, noCurrent) {
        dataBind('TcsUsuarioFilialList', true);
        var entity = dataContext.createTcsUsuarioFilial(parent, noCurrent);
        if(!entity) return null;
        entity.setBandeiraRede(getBandeiraRede());
        entity.setGpecon(getGpecon());
       if ((noCurrent !== true) && !isNullOrEmpty(parent)) { parent.currentTcsUsuarioFilial(entity); entity.fillDetails(); } 
        return entity;
    };
    
    var createAndNotifyTcsUsuarioFilial = function(parent) {
        var entity = createTcsUsuarioFilial(parent);
    ...
    var removeFormViewControl = function () {
        var front = $('#CadastroUsuario_formViewer_front')[0];
        if (front) front.removeClassName('front');
        var back = $('#CadastroUsuario_formViewer_back')[0];
    ...
    var changeFormView = function () {
        ...
        var panel = $('#CadastroUsuario_formViewer')[0];
        if (panel) {
    ...
    var entitySearchRange = {
        predefinedFilters: ko.observableArray(managerPredefined.predefinedFilters),
            TcsUsuarioDataAlteracao_typeRange: ko.observable('R'), TcsUsuarioDataAlteracao_begin: ko.observable(null), TcsUsuarioDataAlteracao_end: ko.observable(null), TcsUsuarioDataAlteracao_predefFilter: ko.observableArray([]), TcsUsuarioDataAlteracao_predefValue: ko.observable(null),
        TcsUsuarioDataCadastro_typeRange: ko.observable('R'), TcsUsuarioDataCadastro_begin: ko.observable(null), TcsUsuarioDataCadastro_end: ko.observable(null), TcsUsuarioDataCadastro_predefFilter: ko.observableArray([]), TcsUsuarioDataCadastro_predefValue: ko.observable(null),
        TcsUsuarioNomeUsuarioCopia: ko.observable(null),
        TcsUsuarioPerfilDescPerfil: ko.observable(null),
        TcsUsuarioRegraModuloDescModulo: ko.observable(null),
        TcsUsuarioRegraTransacaoDescTransacao: ko.observable(null),
        TcsUsuarioRegraTransacaoClasseNome: ko.observable(null),
        TcsUsuarioRegraColunaDescTransacao: ko.observable(null),
        TcsUsuarioRegraColunaClasseNome: ko.observable(null),
        TcsUsuarioBandeiraRedeDescBandeiraRede: ko.observable(null),
        TcsUsuarioLayoutDescLayout: ko.observable(null),
        TcsUsuarioFilialCodigoFilial: ko.observable(null),
        TcsUsuarioFilialNomeFilial: ko.observable(null)
    };
    entitySearchRange.clear = function(){
            entitySearchRange.TcsUsuarioDataAlteracao_typeRange('R'); entitySearchRange.TcsUsuarioDataAlteracao_begin(null); entitySearchRange.TcsUsuarioDataAlteracao_end(null); entitySearchRange.TcsUsuarioDataAlteracao_predefFilter([]); entitySearchRange.TcsUsuarioDataAlteracao_predefValue(null);
        entitySearchRange.TcsUsuarioDataCadastro_typeRange('R'); entitySearchRange.TcsUsuarioDataCadastro_begin(null); entitySearchRange.TcsUsuarioDataCadastro_end(null); entitySearchRange.TcsUsuarioDataCadastro_predefFilter([]); entitySearchRange.TcsUsuarioDataCadastro_predefValue(null);
        entitySearchRange.TcsUsuarioNomeUsuarioCopia(null);
        entitySearchRange.TcsUsuarioPerfilDescPerfil(null);
        entitySearchRange.TcsUsuarioRegraModuloDescModulo(null);
        entitySearchRange.TcsUsuarioRegraTransacaoDescTransacao(null);
        entitySearchRange.TcsUsuarioRegraTransacaoClasseNome(null);
        entitySearchRange.TcsUsuarioRegraColunaDescTransacao(null);
        entitySearchRange.TcsUsuarioRegraColunaClasseNome(null);
        entitySearchRange.TcsUsuarioBandeiraRedeDescBandeiraRede(null);
        entitySearchRange.TcsUsuarioLayoutDescLayout(null);
        entitySearchRange.TcsUsuarioFilialCodigoFilial(null);
        entitySearchRange.TcsUsuarioFilialNomeFilial(null);
    };
    entitySearchRange.has_TcsUsuarioDataAlteracao = ko.computed(function(){ return (entitySearchRange.TcsUsuarioDataAlteracao_typeRange() == 'R' && (entitySearchRange.TcsUsuarioDataAlteracao_begin() != null || entitySearchRange.TcsUsuarioDataAlteracao_end() != null) || (entitySearchRange.TcsUsuarioDataAlteracao_typeRange() == 'P' && entitySearchRange.TcsUsuarioDataAlteracao_predefFilter().length > 0)); });
    entitySearchRange.has_TcsUsuarioDataCadastro = ko.computed(function(){ return (entitySearchRange.TcsUsuarioDataCadastro_typeRange() == 'R' && (entitySearchRange.TcsUsuarioDataCadastro_begin() != null || entitySearchRange.TcsUsuarioDataCadastro_end() != null) || (entitySearchRange.TcsUsuarioDataCadastro_typeRange() == 'P' && entitySearchRange.TcsUsuarioDataCadastro_predefFilter().length > 0)); });
           ...
           if (vm.status() !== 'C') {
               ...
               colunas.forEach(function (entry, index) {
                   if (entry.hidden && entry.key !== 'RowDataId') {
                       var control = $('#CadastroUsuario_div' + (!dataView ? '' : '' + dataView + '_') + entry.key + 'Template');
    ...
    var vm = {
            ...
            rootBmTypeName: 'TCS_USUARIO',
            rootDataTypeName: 'TcsUsuario',
            rootNamespace: 'Linx.Framework.BV.Usuario',
            ...
            createAndNotifyTcsUsuario: createAndNotifyTcsUsuario,
            createTcsUsuarioPerfil: createTcsUsuarioPerfil,
            createAndNotifyTcsUsuarioPerfil: createAndNotifyTcsUsuarioPerfil,
            createTcsUsuarioRegraModulo: createTcsUsuarioRegraModulo,
            createAndNotifyTcsUsuarioRegraModulo: createAndNotifyTcsUsuarioRegraModulo,
            createTcsUsuarioRegraTransacao: createTcsUsuarioRegraTransacao,
            createAndNotifyTcsUsuarioRegraTransacao: createAndNotifyTcsUsuarioRegraTransacao,
            createTcsUsuarioRegraColuna: createTcsUsuarioRegraColuna,
            createAndNotifyTcsUsuarioRegraColuna: createAndNotifyTcsUsuarioRegraColuna,
            createTcsUsuarioBandeiraRede: createTcsUsuarioBandeiraRede,
            createAndNotifyTcsUsuarioBandeiraRede: createAndNotifyTcsUsuarioBandeiraRede,
            createTcsUsuarioLayout: createTcsUsuarioLayout,
            createAndNotifyTcsUsuarioLayout: createAndNotifyTcsUsuarioLayout,
            createTcsUsuarioFilial: createTcsUsuarioFilial,
            createAndNotifyTcsUsuarioFilial: createAndNotifyTcsUsuarioFilial,
            ...
            __moduleId__: 'pkg_linx-framework-bv-spa/viewmodels/CadastroUsuario',
            pivots : pivots
        };
